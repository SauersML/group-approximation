...
If `M_p` is Connes embeddable, then any trace-preserving embedding restricts
on `SL_3(Z)` to a regular-type hyperlinear approximation `(rho_n)` that is,
along the ultrafilter, uniformly bounded away from every genuine
finite-dimensional unitary representation: an **outlier**.  Equivalently,
