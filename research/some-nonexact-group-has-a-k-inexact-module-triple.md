...
- **Where every attempt stops.** A C*-defect needs an element of C that
  dies on Z but is not approximable from J. A K-defect needs an actual
  class in either degree outside the ideal image. The
  [quotient localization theorem](module-k-defect-is-liftable-quotient-kernel.md)
  identifies it with a nonzero element of `im p_* ∩ ker q̄_*` in `K_j(C/J)`.
  Equivalently the quotient-kernel class must have zero six-term boundary.
  [Liftable-class existence](some-module-quotient-has-a-nonzero-liftable-kernel-class.md)
  is wired in both directions to this input; neither is established.
- **All bounded positive traces, 2026-09-20.** The
  [relation-ideal lemma](module-evident-ideal-is-kernel-relation-ideal.md)
  identifies J with the ideal generated by `w−1`. The
  [amenable-kernel factorization theorem](amenable-kernel-quotient-traces-factor-reduced.md)
  therefore makes every trace on C/J descend to B. Thus
  [every bounded positive trace misses a degree-zero defect](module-quotient-k0-defects-are-trace-invisible.md).
  For torsion V this also applies to any trace on C annihilating the ideal
  K_0 image. This strengthens the invariant-measure argument; it neither
  constructs nor rules out the missing class. The
  [torus example](torus-has-nonzero-k0-class-invisible-to-every-trace.md)
  refutes the trace-to-K-theory vanishing inference.
