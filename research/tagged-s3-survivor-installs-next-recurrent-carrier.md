---
rg: 2
id: tagged-s3-survivor-installs-next-recurrent-carrier
kind: claim
title: Install the tagged S3 survivor as the next reducing recurrent carrier
distinct_from:
  two-bit-tag-routing-splits-s3-exit-and-survivor: that proves first-hit and survivor-prefix placement for the endpoint atoms; this must attach the survivor image to the next complete Fanizza/Schur occurrence and verify the branching trace ledger.
...
- **Plain free-occurrence HNN return.**  Invalidated by
  `free-occurrence-hnn-return-does-not-preserve-first-hit-prefix`: it returns
