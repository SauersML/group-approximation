...
distinct_from:
  ...
  binary-jacobson-gl3-k-branching-has-regular-counterprofile: that shows that branching multiplicities alone admit a positive marked profile; this uses the actual head involution blocks to prove a positive first-braid defect for arbitrary mixtures of the two three-dimensional head types.
  binary-jacobson-joint-shortest-cells-exclude-natural-completions: that excludes natural characteristic-two head completions satisfying both cells; this gives an operator-norm bound over complex three-dimensional head types with arbitrary tail actions using only the first cell.
---
...
including every dimension-twelve packet with tail `1 directSum rho_+`.
