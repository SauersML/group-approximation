...
2. `(MLF1)` is residually finite as an `F_m x F_m`-set (finite equivariant quotients separate every finite subset) if and only if `H` is residually finite. Equivalently, `M_H` is separable in `F_m x F_m` if and only if `H` is residually finite.
