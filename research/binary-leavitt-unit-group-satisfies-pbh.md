...
**OPEN.** Let `R = L_(F_2)(1,2)` and `U = R^×`. Then `U` embeds in a finitely
...
satisfies Boone--Higman. A positive answer upgrades all of them to the permutational
form, which is stable under commensurability, direct products and free products
(BFFHZ Corollary F), and it would put a nonsofic finitely presented simple group
(the Leavitt route `fp-simple-nonsofic-via-leavitt-unit-group` into
`fp-simple-nonsofic-group`) in the class `B_A`. No node on main
states that `U` is highly transitive, MIF, or of type (A).
...
- `leavitt-pbh-via-cantor-module-affine-actor`: `U` acts faithfully and linearly on
  ...
  of `C(C, F_2) ⋊ U` (`leavitt-cantor-module-semidirect-product-is-fp`, OPEN).
