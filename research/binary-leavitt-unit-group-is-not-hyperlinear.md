...
distinct_from:
  ...
  kun-thom-lamp-wreath-is-not-hyperlinear: That concerns the binary Kun--Thom wreath; this concerns a different explicit group and uses the two-use native heat argument rather than a wreath embedding.
artifacts:
...
depends on `kazhdan-commutants-are-internal`, which remains open under
independent review. Neither the stronger arbitrary-channel conjecture nor
...
**Attribution.** The proposed new analytic input is Jihao Liu,
...
analytic theorem, not a theorem claimed in his paper. The distinct two-use
Leavitt argument is credited to the September 8 Cairn artifacts cited above.
This attribution does not promote Liu's theorem or this conclusion from its
current conditional graph status.
