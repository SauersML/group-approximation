...
`atlas-six-relator-simple-index-is-at-least-991`: every such simple target has
`A8` index at least nine hundred ninety-one.  The screens cover the unique
...
and the natural `A11` boundary.  Each final survivor is killed by
`c_19243`, while the intervening CFSG order intervals contain no `A8`
