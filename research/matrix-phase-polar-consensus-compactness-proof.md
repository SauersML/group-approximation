...
alpha-plane energy.  No joint spectral measurability is assumed.  For each
standard `m`, put all affine evaluations `u |->h_(L(u))` in the tracial
ultraproduct of `ell^infty(K_n^m) tensor M_(d_n)`.  Uniform commutation of the
corrected family makes the von Neumann algebra generated by these evaluations
and the scalar coordinate algebra commutative.  Close this graded family under
the finitely many coordinate-forgetting conditional expectations and affine
substitutions in the tester proof; coordinatewise averaging preserves
commutation and trace.  The Kaufman--Sudan argument uses only those operations,
rejection projections, finite joins, and Boolean plurality projections, so it
runs verbatim in this commutative probability algebra.  This proves its scalar
`27/2` trace inequality directly for the Loeb affine averages, without
spectral disintegration or an interchange of hyperfinite and spectral
integrals.  Hence the full parallelogram energy is zero.  Since `h=q` almost
everywhere and gauging preserves all three energies, a sequence with `I+A->0`
cannot keep `F` positive.
