...
`torsion-free-central-cover-forces-abelian-finite-subgroups`.  And the
opposite direction is now excluded as a class: by
`full-mf-radical-seed-has-no-torsion-free-quotient` there is a finitely
presented full-MF-radical group normally generated by two elements of order
`2` and `3`, whose universal torsion-free quotient is trivial, so no procedure
returning a homomorphic image of its input is torsion-free on every permitted
input.  Together these say the live direction is upward along a cover with
abelian noncentral kernel, which is exactly where the open implication sits.
