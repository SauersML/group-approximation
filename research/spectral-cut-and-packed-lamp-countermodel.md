...
[
 ker L_{mathcal R}
 =igcap_{rinmathcal R}ker(pi(r)-1).
]
...
[
 pi(g)mathcal K=mathcal Kqquad(ginGamma),
]
...
[
 D_{mathcal R}(V)^2
 =rac1doperatorname{Tr}(V^*L_{mathcal R}V).          	ag{1}
]

Combining (1) with (ANR3) yields

[
 rac1doperatorname{Tr}(V^*(1-P)V)leqdelta.          	ag{2}
]
...
[
 ...
 leq2operatorname{Tr}(1-a)
 leq2delta d.                                         	ag{3}
]

The compression (eae) is invertible on (emathbb C^d). Define
...
[
 pi(b)W=Weta(b)eqquad(bin B).                       	ag{5}
]
...
[
 egin{aligned}
 |W-Ve|_{mathrm F}^2
 &=2operatorname{rank}e
   -2operatorname{Tr}((eae)^{1/2})\
 &leq2operatorname{Tr}(e(1-a))
 leq2delta d,                                         	ag{6}
 end{aligned}
]
...
((e_q)_{qin Q}). The signed-permutation representation of
((C_2)^Qtimes Q) satisfies
...
[
 rac1{M(M-1)}
 sum_{qin Q}|(pi_Q(z_q)-1)V_Q|_{mathrm F}^2
 =rac{4(M-1)}{M(M-1)}=rac4M.                        	ag{7}
]
...
Fix a section (qmapstowidetilde q) of
(Gamma	oGamma/Lambda), with (widetilde1=1). Reading a word in the
fixed symmetric marking and repeatedly returning to the chosen section
gives the factors

[
 widetilde{sq}^{, -1}swidetilde qinLambda.
]
