---
rg: 2
id: conjugacy-addressed-opnorm-challenges-are-lossless
kind: claim
...
More generally, if every member `q` of an arbitrarily deep challenge family
has a free-word identity

```text
q = a_q^-1 r_(i(q)) a_q                                     (COC2)
```

for one of finitely many prototype words `r_1,...,r_m`, then

```text
sup_q ||q(U)-I||_op
 <= max_i ||r_i(U)-I||_op.                                  (COC3)
```
