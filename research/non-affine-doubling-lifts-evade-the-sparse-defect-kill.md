...
- **2026-09-19, swarm-0917-w17-w17-bh-follow (reframing): P1 holds topologically, and PL
  realizations are a countable set.** See `chain-group-germ-models-are-generically-faithful` and
  its proof. P1 itself stays OPEN.
  - *Established there.* For comeagre `ψ` commuting with `2x`, the germ group at `+∞` of
    `⟨x+1, 2x, ψ(x|x|)ψ^-1⟩` is `≅ G_1`, and each nontrivial element moves points in every
    interval. So no obstruction valid for all homeomorphism germs can kill P1. A kill must use PL
    or `V` rigidity, as the affine kill does.
  - *Criterion.* For any realization with `σ = x+1` and `δ` a lift with an expanding iterate
    (this covers `δ_f`, since `δ_f^2` has slopes `≥ 2`), `θ` is faithful iff the conjugates
    `θ(a N_B a^-1)`, for `a` in a transversal of `A/C`, form a free family. Here
    `N_B = ⟨⟨σ⟩⟩_B ≅ Z[1/2]`. The part `θ(N) ∩ θ(A) = 1` is automatic.
  - *Topological linearization.* Lemma 6 there conjugates `δ_f` to `2x` by a homeomorphism `h`
    that commutes with `x+1`. This is the linearization that stalled the w16 attempt, and it does
    not need to be Lipschitz. If the linearized `s` also has an expanding iterate, the
    realization is `θ_ψ^∞` for one explicit `ψ`. P1 for it then says exactly that `ψ` lies in a
    comeagre set, which genericity cannot decide for a countable family.
  - *Where it stops.* Nothing here decides whether `ψ(δ_f, s)` is in the faithful set. The next
    step is a ping-pong for the `A/C`-indexed conjugates of the root group. These are lifts of a
    Prüfer group of PL circle maps.
