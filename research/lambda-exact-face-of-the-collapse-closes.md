...
`sigma_a` of `Lambda` (isotypic weights exactly Plancherel at each level — the canonical
enemy format; if the weights are only within a factor `c < 16` of
Plancherel the same argument gives the weaker floor
`epsilon_0 sqrt(1 - c/16)`)
...
- pure 2-power and mixed levels `2^b q` (`q` odd) with `b >= 2`, OF
  REGULAR TYPE: closed by `(LE1)` with the constant `sqrt 15 / 4`, by
  the mixed-level extension `(DSO5)` of
  `depth-shifted-regular-overlap-is-exactly-one-sixteenth`; at `b = 1`
  the shifted kernel packet has order `8`, not `16`, and the floor is
  `epsilon_0 sqrt 14 / 4` (still positive, so the sector conclusion
  stands); `b = 0` is the coprime sector.
...
- **Audit notes (2026-08-21).**  `(LE1)` is the same inequality as
  `(DSO7)` of `depth-shifted-regular-overlap-is-exactly-one-sixteenth`
  (up to `V <-> V^*`); the new content here is the sector bookkeeping.
  The constant `sqrt 15 / 4` is correct exactly at Plancherel weights
  and levels `2^b q`, `b >= 2`; the earlier text asserted it for weights
  "uniformly comparable to Plancherel" and for `b = 1`, both corrected
  above.  The bounded-gauge clause previously cited the unrouted
  consolidation bullet of `two-power-sector-conjugator-dichotomy`; it is
  now justified by the mixed-level `(DSO5)` directly.  Step 2 of the route
  contains a vacuous "general module" clause (the maximum of a fraction
  over constituents can be `1`), harmless because the SCOPE paragraph
  already restricts to regular type.
