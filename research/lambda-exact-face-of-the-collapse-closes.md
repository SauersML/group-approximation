...
kind: claim
title: No twisted conjugator exists, and the entire Lambda-exact face of the collapse closes
distinct_from:
...
2-power congruence tower of genuine representations `sigma_a` of
`Lambda` (any multiplicities) there is a uniform floor
...
- bounded `p`-part and pure 2-power towers: closed by `(LE1)`.

CONSEQUENCE.  Any enemy of `(RC3)` — hence of the higher-rank goal
edge — must have `Lambda`-microstates at uniform positive distance
from EVERY genuine finite-dimensional representation of `SL_3(Z)`:
the uniform-outlier sector is no longer one residue among several
but the ONLY habitat, and the correctability hole
`sl3z-regular-microstates-are-congruence-correctable` is now
exactly equivalent to the full collapse on this face.
