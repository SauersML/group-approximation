...
kind: route
title: Intersect quasitrace null ideals over all trace-preserving central copies
...
`K_Z subset K_Z^sat`.  If `theta in Theta` and `x in E`, trace preservation
gives
...
`stw93-tensor-factor-central-quasitrace-rigidity`.  Therefore

```text
q(theta(x)*theta(x))=tau_(Z,omega)(x*x)=0       (x in E). (1)
```
...
is a closed two-sided ideal.  Equation (1), simultaneously for every
...
bounded `2`-quasitrace.  No abundance, conjugacy, or exhaustion assertion for
the family `Theta` is used.
