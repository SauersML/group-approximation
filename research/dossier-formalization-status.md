...
| 37.1 tower | pushforward of abelian lamps along a map of sites; functoriality; kernels increase; equivariance | DONE-NEW | `Algebra/LampPushforward` (`push`, `push_comp`, `ker_le_ker_comp`, `push_comm_of_equivariant`), on `Finsupp.mapDomain` |
| 37.5 union step | the limit kernel is the supremum of the stage kernels | DONE-NEW | `Algebra/LampTelescopeKernel.ker_push_eq_iSup`, from the fibre-sum formula `mapDomain_apply_eq_sum` and the finite-separation hypothesis |
