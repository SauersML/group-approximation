...
The `K_0` equality must hold in the ideal generated by the two projections,
not just in the ambient algebra.  The ideal-local protection theorem
`stw99-lxxvi-ideal-local-k0-protection` proves that a nonzero local difference
survives tensoring with Z.  In particular, index-boundary noncancellation from
a Toeplitz-type extension cannot work.  The pure Z-stable algebra
...
The remaining compact counterexample frontier is stated without hidden
