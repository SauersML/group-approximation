...
## Attempt (bh-last1, 2026-09-19): what free-stabilizer tree hosts can certify

*The free-base case escapes the existing tree-host kills. A Hall host certifies nothing new unless
`G ∩ Y` is infinitely generated, and for `χ(G) < 0` that is forced.*

- **The existing kills miss free bases.** Let `G = F*_φ` with `F` free.
  - `G` contains no Gersten pair. If `bab^-1 = a^2` with `b = tat^-1`, then `a` and `b` are both
    elliptic. If their fixed sets are disjoint, `<a, b> ≅ <a> * <b>`. Otherwise `<a, b>` lies in a
    conjugate of `F`. Either way `a` is not conjugate to `a^2`.
  - `G` also has no nontrivial FA subgroup, since such a subgroup lies in a conjugate of `F`.
  - So neither `gersten-pairs-lie-in-no-locally-finite-tree-host` nor
    `profinite-tree-hosts-make-fa-subgroups-residually-finite` touches the Hall hosts of bh-free-05.
- **First fold (worked, not a node).** One-sided `Φ = φ * α` from bh-free-05.
  - Take `B̄ = F ∩ C_2 = B * L_0` with `B` a free factor. Choose `S = α^-1(L_0)` generated by
    loops that start and end with `b`-edges.
  - Then `<F, S> = F * S` in `Y`, and `<F, t^-1 B̄ t>` maps isomorphically. This re-presents `G` as
    `(F * L_0') *_{A * L_0' -> B * L_0}`.
  - The new base `F_1` has `F_1 ∩ C_1 ⊋ A * S`, which forces a second fold, and so on without end.
  - The same happens for the one-sided amalgam `A *_C B` with `[B:C] = 2`. There `(B * M) ∩ Ĉ` has
    finite index in `B * M`, while `C * M` has infinite index, so no immersion is possible.
- **Why the folds never stop.** `negative-euler-inputs-get-infinitely-generated-tree-stabilizers`
  (ESTABLISHED) shows the following.
  - If `G ∩ Y` is finitely generated for any torsion-free host with virtually free stabilizers on a
    locally finite tree, then `G` itself is a reduced finite graph of f.g. free groups with
    finite-index edges. Then `G ∈ BS_{F_2} ∪ BS_Z`, the host was unnecessary, and
    `χ(G) = Σ χ(G_v)(1 - deg v / 2) >= 0`.
  - So for every non-free `G = F*_φ` with `rk A < rk F` (`χ = rk A - rk F < 0`), every host has
    infinitely generated `G ∩ Y`. Every finite folding certificate dies at the restriction of the
    host tree to `T_G`, and the invariant is `χ`.
- **What remains for Hall hosts.** Injectivity proofs must control an infinitely generated
  commensurated free stabilizer. Surface groups show this is not absurd: `ker(π_1 Σ -> Z)` is such
  a stabilizer on a line. Inputs with `χ >= 0` are untouched, including the one-sided case and the
  Artin reduction `F_3 *_{F_7}` (`χ = 4`).
- **Dead lead, recorded to save a search.** A search-engine summary claimed that the survey
  arXiv:2306.16356v3 (Theorem 5.1) settles Boone--Higman for large-type Artin groups. It does not.
