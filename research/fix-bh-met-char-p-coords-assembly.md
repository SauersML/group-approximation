...
**Module.** `GroupApproximation/BooneHigman/Metabelian/CharPCoordsAssembly.lean`. It is red on origin/main at 3c8b417428 with 3 hard errors, from bh-pal-integrate's trusted MSI build
...
**First errors.**
```
    line 60:12: Unknown constant `Algebra.fg_adjoin_finset`
    line 61:33: failed to synthesize instance of type class
    line 61:9: Tactic `rcases` failed: `x✝ : ?m.175` is not an inductive datatype
```
...
**Resolve when.** A trusted build (`gqprobe-lean.sh`) of `GroupApproximation.BooneHigman.Metabelian.CharPCoordsAssembly` is green with no errors, and the repaired bytes are on
