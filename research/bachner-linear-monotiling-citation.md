---
rg: 2
id: bachner-linear-monotiling-citation
kind: route
title: Apply Bachner's linear monotiling theorem
target: amenable-domain-linear-sofic-models-are-unique
requires: []
---
...
vectors identify copies of the same large `A`-module on subspaces of
asymptotically full dimension. An invertible change of basis matches these
copies, so the two maps differ on only a vanishing-rank complement. This is
the normalized-rank conjugacy asserted in the target.

The zero-divisor hypothesis is essential to the stated notion: maximal
