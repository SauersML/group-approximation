...
kind: claim
title: One two-generator finitely presented group tests major all-group approximation, assembly, ring, determinant, Bass, and dynamical conjectures
distinct_from:
  ...
  universal-local-approximation-fixed-two-generator-test-group: that records only finite-table approximation properties; this packages those with independent assembly, ring-theoretic, analytic, Bass, and cellular-automaton testers.
...
3. `U` satisfies Full Farrell--Jones iff every group does.
4. `U` satisfies A-theoretic Farrell--Jones with coefficients and finite
...
property, field, coefficient algebra, alphabet, or matrix is chosen. These
are reductions, not positive resolutions of the remaining open fixed-host
assertions.
