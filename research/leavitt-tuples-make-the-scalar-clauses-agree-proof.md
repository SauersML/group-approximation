...
- **Refinement.** Voronetsky's Proposition 1 applies to `M_(N+d-1)(R)`, with the standard family and the coarse family
  that merges the last `d` idempotents. The fine family has `N + d - 1 >= 4` members, and each `E_aa` is equivalent to
  `E_bb`. So it gives an isomorphism `D : St_N(R) -> St_(N+d-1)(R)` with `φ D = α φ`. This is the `d`-tuple form of the
...
- The converse inclusion up to `N`-th powers is item 2 of `ring-gate-k1-clause-is-unstable`. A subgroup of a
  finitely generated abelian group is finitely generated, and `Λ_N -> ker ι`, `λ -> λ^N`, is injective. So the two
