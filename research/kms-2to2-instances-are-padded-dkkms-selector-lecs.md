...
distinct_from:
  dkkms-2to1-instances-satisfy-selector-decoding-hypotheses: that checks (E), (M) and (B) for the DKKMS 2-to-1 game G_folded and its multi-edge form, and is established; that node explicitly leaves the 2-to-2 instances untreated, and this is the 2-to-2 side of the same question -- whether the instance family behind Theorem 1.4 of ECCC TR18-006 is a presentation of G_folded carrying the same hypotheses, and whether a 2-to-2 selector, which gets a matching choice per K_(2,2) block and a free seed bit, is killed by the same decoder.
  two-to-two-games-theorem: that is the imported statement of Theorem 1.4 of ECCC TR18-006, a hardness gap; this asks what the instances behind that gap are and annotates them with the selector-decoding hypotheses, proving nothing about the gap itself.
