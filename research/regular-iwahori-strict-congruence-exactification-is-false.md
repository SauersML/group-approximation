...
and let (p) run through primes congruent to (5) modulo (8).  There
are exact same-dimensional modular-vertex pairs
...
regular character of **SL2**, and yet one constant (c>0) satisfies

```text
inf_(sigma:Gamma->U(2d_p)) d_2(pi_p,sigma)>=c           (RSC2)
```

for all sufficiently large (p).  Here an exact (sigma) is read as its
two compatible vertex restrictions and all fixed generating-set metrics
are interchangeable up to universal word-length constants.
...
of dimension (d_p), on which (-I) acts by (-1).  Thus the normalized
trace of (-I) is exactly zero, rather than one as it is for the
central-trivial compressed packet alone.
...
((2d_p+1)/(2d_p)) tends to one.  The surviving open theorem must therefore
allow (o(d_p)) padding, or equivalently use generalized (d_2).
