...
- **Assembly.** Take towers of height `≥ 64w` with atoms refined to fix the local types. Then `H_1 = ⊕ A_B ∈ GL_3(A_𝒫)`, and `H_1^{-1}h − I` is supported within `12w` of the cuts. So `H_2 := H_1^{-1}h ∈ GL_3(A_𝒫′)` for the towers based at midpoint levels, and `h = H_1H_2` needs at most `39 + 39 = 78` elementary matrices.
