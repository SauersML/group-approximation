...
- **Group-free reformulation (2026-09-12).** `C*(1, h, k)` is the universal unital
  C*-algebra of one order pair `0 <= k <= h - h^2`
  (`stw01-modular-test-pair-algebra-is-universal-order-pair`). So no structure of `Q`
  beyond that relation is involved. A sufficient route is additivity on the universal pair
  (`stw01-universal-order-pair-quasitrace-additivity`, route
  `stw01-modular-single-pair-via-universal-order-pair`). This moves the question without
  settling it.
- **Commuting sandwich, ideal extension, Cuntz classes, pinching.** These are recorded on
  `stw01-universal-order-pair-quasitrace-additivity` and in Section 4 of
  `research/artifacts/stw01-universal-order-pair-test-2026-09-12.md`. The defect is at most
  `sigma(h - h^2)`, and every counterexample from the reduction has positive room. The ideal
  generated by `k` has an exact quotient `C([0,1])` but is itself not exact. None of these
  decides the additivity.
