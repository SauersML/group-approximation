...
**Attempt 2 (bh-shell-arith, 2026-09-18): the question is local; bounded local search, no certificate.**
- *Reduction (proved).* By `finite-index-in-a-full-group-is-local-fullness-at-one-cone`, with `<tau, w>`
  giving compressibility via `T_3`: `R = <tau, w, x_1^-1 a x_1>` contains `V` iff `R` contains `V` on a single
  cone, i.e. local copies (supported in `[0]`, identity on `[1]`) of a generating set of `V`. It suffices
  to find `x_0, x_1, tau, pi_[00][01], pi_[00][10]`. Then `T = <F, tau>` by orbit--stabilizer, since
  `Stab_T(0^inf) = F`, and `V` is generated by all cone transpositions, which are `T`-conjugates of
  those two and `tau`.
- *Search (artifact §8, evidence only).* 1.19M commutators of words of length ≤ 5, localized into `[0]`,
  gave 285 local elements with ≤ 16 leaves. The closure under products, kept to ≤ 10 leaves, reached
  871k elements. No target was found. The pipeline recovers a planted local `x_0`.
- *Correction of a test.* Several small carry-free local elements that the search found,
  e.g. `[0]→[01], [10]→[1], [11]→[00]`, are automorphisms of `T_3`. So is `x_0` itself, as a rotation
  sending vertex `[1]` to `[0]`. So "depth-changing" is not a non-tree test on `T_3`. The non-tree
  targets are `x_1` and `pi_[00][10]`, and a certificate must contain one of them.
