...
5. **The locally synchronous full-shift case (bh-q14-lift, 2026-09-19).** *Reduced to one finite-state group.*
   See [[synchronous-rsg-full-closures-are-rn-groups]] (lane proof, not independently
   reviewed).
   - **The reduction.** If every element of `G` is a table over finite-state tree automorphisms, then
     `[[G]] = V_d(H_G)`, where `H_G` is the finite-state group generated by the table entries. By
     [[rover-nekrashevych-fp-iff-finite-lift-presentation]] (Theorem A), `[[G]]` is finitely presented iff
     the relator kernel of `H_G` is a finitely generated lift ideal.
   - **Consequences.**
