...
3. **Single-machine singular schedules (2026-09-18, lane bh-free-34).** *Dies.*
   - **The design.** Run a fixed reversible machine `m` for `f(d)` steps on the `d`-th annulus around a singular
     point, with `f` an Ackermann-type diagonal.
   - **Why it fails.** `block-power-germ-schedules-with-fp-germ-group-are-exponential`: the germ group generated by
     the annulus contraction and this germ is metabelian. If it is finitely presented, as Belk–Hyde–Matucci
     certification requires, `f` satisfies a linear recurrence and is at most exponential.
