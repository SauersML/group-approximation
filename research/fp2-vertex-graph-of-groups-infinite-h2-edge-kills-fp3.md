...
edge group, whatever the edge maps are. Higman's rope trick uses the double
as its edge group. There the vertex group is `P × G`, which has infinite
rational `H_2` when `G` does, and that case is handled separately by
