...
target: stw99-lxxvi-circle-formation-preserves-cu-regularity
requires:
...
Put `E=D tensor_min Z`.  A simple stable-rank-one algebra is stably finite,
and the simple stably finite Z-stable algebra `E` has stable rank one.
...
application of the assumed isomorphism `Cu(D)->Cu(E)`.  It preserves
compactness in both directions, so it gives an isomorphism of the two
`Lsc_nc` summands.
...
Its `V(D)` coordinate is an isomorphism because a Cu isomorphism preserves
compact elements and, for stable-rank-one algebras, compact Cu classes are
exactly projection classes.  Its `K_1` coordinate is an isomorphism because
`C->Z` is a KK-equivalence.  Hence the compact summands in (1) are
isomorphic as well.
