...
kind: claim
title: A scalar-twisted Bernoulli crossed product of a residually finite group is CE exactly when its twisted group factor is CE
distinct_from:
  ...
  nonce-twisted-factor-gives-nonhyperlinear-group: that converts a non-CE twisted group factor to a nonhyperlinear central extension; this identifies when adjoining a classical Bernoulli base changes Connes embeddability.
---
...
set `D subset Gamma`, and approximate the finite-dimensional distribution
of its values by a finite atomic probability space `X_(0,r)`.
...
of the corresponding Maslov-twisted group factors.
