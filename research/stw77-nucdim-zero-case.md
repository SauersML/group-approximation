...
- `stw77-near-duplicate-fd-blocks-merge` now proves that a finite support
  block is harmless when its finite tuple is close to blocks with properly
  infinite aggregate support. The route
  `stw77-zero-via-proper-block-clustering` isolates the remaining selection
  statement. Conversely,
  `stw77-counterexamples-force-separated-block-clusters` shows that every
  counterexample must keep some block type uniformly separated from every
  such proper-support cluster on one fixed finite packet.
