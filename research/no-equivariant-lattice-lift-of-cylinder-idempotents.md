...
kind: claim
title: No Thompson-equivariant Boolean lattice lift of the cylinder idempotents is compatible with Dirac evaluation
distinct_from:
...
3. `delta_x in Phi(C)` iff `x in C`, for all `x` and all clopen `C`.

Equivalently, `B = C(M_-, F_3) x| V` has no `V`-equivariant commuting family of idempotent lifts of the
cylinder idempotents that is multiplicative and additive on disjoint unions.
...
Proof sketch. By Stone duality `Phi = r^-1` for a continuous `r : M_- -> X`. Item 3 gives
`r(delta_x) = x`, and item 2 gives `r(g mu) = g r(mu)`. The Haar measure `lambda` of `M_-` is
