...
operator-norm limsup.  Rescale it to contractions, choose `eta>0` below
that limsup, and pass to a subsequence on which `||D_n||>=eta`.
...
Taking normalized matrix traces yields the explicit bound
...
Hilbert--Schmidt multiplicativity sends the right side to zero.  This proves
all assertions and uses no exactness, UCT, local lifting, or faithfulness.
