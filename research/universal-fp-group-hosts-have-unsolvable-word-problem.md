...
2. `H` contains `Z^k` for every `k`, so `cd(H) = ∞` and `H` does not have
...
Item 2 alone already excludes groups of finite virtual cohomological
dimension, and item 3 excludes groups with a bound on the orders of their
finite subgroups.
