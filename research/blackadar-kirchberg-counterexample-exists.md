...
- **Projection-generated radicals: dead (2026-09-13).**
  `bk-uct-counterexample-radical-not-projection-generated`: suppose the
  quotients of `E` by ideals inside `R_qd(E)` satisfy the UCT.  Then `R_qd(E)`
  contains no full projection.
  - Spielberg's criterion makes the boundary subgroup of `E/R_qd(E)` singular.
  - A trace on a full corner of the radical vanishing on it has kernel ideal
    `J ⊊ R_qd(E)`.
  - `E/J` is quasidiagonal (Schafhauser's realisation plus Brown--Dadarlat
    Theorem 3.4), contradicting minimality.
  Consequence `bk-projection-generated-ideals-with-uct-quotients-force-qd`: no
  counterexample has UCT quotients and all ideals generated by projections.  In
  particular none has finitely many ideals with projections and UCT
  subquotients, including the real rank zero case.
- **Boundary-free corner traces: dead (2026-09-13).**
  `bk-radical-traces-vanishing-on-boundary-are-zero`: if `R_qd(E)` has an
  approximate unit of projections, no nonzero compatible family of corner traces
  vanishes on the boundary.  In real rank zero,
  `(K_0(R) ⊗ Q)/(H ⊗ Q)` has no nonzero positive functional.  The surviving
  real rank zero shape is a non-archimedean exhaustion of the radical
