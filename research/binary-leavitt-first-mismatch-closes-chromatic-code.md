...
branch pairs and all equal-length tail pairs.  Equal-depth vertices retain
equal-length tails after free removal of their longest common prefix.  The
second requirement then supplies the diagonal conjugate cells from `(MTR3)`
and uses the complete graphs on `{0,1}^n`.  These are exactly
