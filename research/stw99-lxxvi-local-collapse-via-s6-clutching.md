...
`[p]=[q]` in `K_0(A)`.  The survival hypothesis says the two `D`-module
bundles are not isomorphic, so their projection classes in `Cu(A)` are
distinct.  The
Z-stabilization theorem makes the clutching maps homotopic after tensoring
with Z, so
