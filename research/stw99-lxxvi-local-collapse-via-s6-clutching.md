...
Let `D` satisfy the witness and put `A=C(S^6) tensor D`.  Clutching the trivial
...
`[p]=[q]` in `K_0(A)`.  Condition 2 says the two `D`-module bundles are not
isomorphic, so their projection classes in `Cu(A)` are distinct.  Condition 3
