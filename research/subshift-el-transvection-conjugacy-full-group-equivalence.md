...
- The root-transvection conjugacy classes `e_13(1_A)` are exactly the classes `[1_A]` of `K^0(X,T)` between `0` and `[1]`, so the Giordano–Putnam–Skau dimension group is visible inside `G_X` by conjugation.
