...
sofic and contains an isomorphic copy of `Γ`. One may take
    ...
    S = EL_3(LC(X, F_q) ⋊ Δ) / Z,   Δ = C(Γ), the half-line overgroup (the swap double fails),
...
Routes: `amenable-groups-embed-in-kazhdan-linear-sofic-via-half-line` (fires, unreviewed); `amenable-groups-embed-in-simple-kazhdan-linear-sofic-proof` (does not fire).
...
- **Half-line overgroup (works, unreviewed; sk-perfect-overgroup, 2026-09-13).** `C(Γ) = ⟨σ, h_s⟩ ≤ Sym(Γ × Z)`,
  with `σ` the level shift and `h_s` translating the half `m ≥ 0` by `s`, is `E ⋊ Z`, where `E` is a directed union of
  finite powers of `Γ`. So it is finitely generated, infinite and amenable. `[σ, h_s]` is the level-0 copy of `s^(-1)`,
  so `Γ ≤ [C(Γ), C(Γ)]` (`groups-embed-in-derived-subgroup-of-half-line-overgroup`). The route
  `amenable-groups-embed-in-kazhdan-linear-sofic-via-half-line` reruns steps 2–6 over `Δ = C(Γ)` and embeds `Γ` through
