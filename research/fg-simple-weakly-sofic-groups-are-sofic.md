...
- **Deep wreath targets: mostly settled (2026-09-13, lane `ex-weak-sofic-deep-wreath`, unreviewed).**
  ...
  - **Towers of bounded degree with bounded commutator width.** The width is bounded over iterated
    wreath products of bounded degree whose derived subgroups have bounded commutator width
    (`relator-width-over-towers-of-bounded-commutator-width`). This covers towers of nonabelian simple
    groups (`relator-width-bounded-over-simple-iterated-wreath-products`) and towers of vertex groups with
    full cycles, including solvable towers (`full-cycle-wreath-towers-bounded-commutator-width`).
...
- **What a refutation would look like now.** A simple nonsofic group with relator width tending to
  ...
  - tower classes where no commutator width bound is on record
    (`simple-nonsofic-relator-width-over-iterated-wreath-products`);
  - bases of unbounded size under symmetric tops;
  - finite groups of unbounded chief length that are not full wreath products, such as the subgroup
    generated by the tuple, where conjugators are scarce.
