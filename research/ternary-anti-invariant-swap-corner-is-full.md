...
- *Sharper unit form and support filter (2026-09-12, lane `gk-kdf-structure`).*
  - **Unit form.** For all `a, c` in `S_-`, `c(1 + [w]) e_- a = c(1 + [w]) a`. So this claim holds
    exactly when some `c(1 + [w]) a` is a unit of `S_-`. For a one-sided pair `c a = 1` the
    condition is that `1 + m` is invertible, which is weaker than invertibility of `1 - m^2`. It is
    a normalization, not a relaxation (Section 3 of
    `research/artifacts/ternary-swap-separated-pairs-2026-09-12.md`).
  - **Support filter** (`swap-separated-pairs-avoid-finitely-represented-supports`). Let `H` be
    generated by the supports of `b, c`, together with `z` and `w`. No solution exists if `H` has a
    finite-dimensional characteristic-three representation with `z -> -1` in which `w` has
    eigenvalue `-1`. For separated pairs, no condition on `w` is needed. The filter excludes:
