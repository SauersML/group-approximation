...
- **Operator fault calculus: the precise remaining crux (heuristic, 2026-09-13, ex2-labbe-quantum-rigidity).**
  - **Side observables.** For a lattice sheet of a boundary segment, its side observable is defined from
    letters at hit sites, which have bounded gaps. It is one operator along the whole sheet, since consecutive
    balls share hits. Crossing sheets give commuting observables, and ball algebras are generated by the side
    observables of the sheets through them.
  - **Localization.** Through one ball, `O(D)` sheets of three directions commute and pin the torus point to
    precision `1/D`.
  - **What is killed and what survives.** Mixing two strands of far parallel sheets is killed: the strands differ
    along a connected network, which forces a constant decomposition. What survives is multi-strand mixing
    between far parallel golden sheets whose internal separation is about `1/D` (Fibonacci lattice
    distances).
  - **Why it is a renormalization problem.** No ball sees both sheets, and localization in a ball near one of
    them rescales the problem to the supertile level. That matches Labbé's self-similarity, but the effective
    scale shrinks.
  - Details in `research/artifacts/quantum-rigidity-group-shifts-and-gluing-2026-09-13.md`, Section 3, and in
