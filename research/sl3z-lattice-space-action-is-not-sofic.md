...
**OPEN (two-sided).** Let `X_3 = SL_3(R)/SL_3(Z)`, the space of unimodular lattices in `R^3`, with its
`SL_3(R)`-invariant probability measure, and let `SL_3(Z)` act by left multiplication, `γ·L = γL`. Claim:
this p.m.p. action is **not sofic** (Păunescu Definition 1.4).
