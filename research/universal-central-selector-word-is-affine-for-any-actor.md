...
**ESTABLISHED.**  Let `Gamma` be any discrete group, with no finiteness,
...
```text
p_chi=|D|^-1 sum_(d in D) overline(chi(d)) d.                       (UCS2)
```

Suppose universal group identities, evaluated in the left regular finite
factor, make `w` a scalar Boolean predicate on every marked selector atom:

```text
p_chi w=epsilon(chi)p_chi,             epsilon(chi) in {+1,-1}.     (UCS3)
```
