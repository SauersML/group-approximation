...
distinct_from:
  ...
  property-t-does-not-kill-boolean-fiber-holonomy: that classifies the residual block cocycle by stabilizer representations, refutes property-(T)-only scalarization, and proves canonical-character monomialization for every fixed finite exact holonomy.
artifacts:
...
The follow-up
`property-t-does-not-kill-boolean-fiber-holonomy` identifies the cocycle on
each finite pattern orbit with a stabilizer representation.  Property (T)
alone cannot make this representation monomial: an exact
`SL(2,5)` two-dimensional holonomy gives a padding-stable countermodel.
On the positive side, a fixed finite exact holonomy with canonical
conditional character is quantitatively close to a regular-plus-trivial
monomial representation.  Therefore any persistent obstruction must use
growing loop groups or nonuniform exactification, jointly with the moving
lamp-window scale in `(BAO9)`.

Extracting one common point refinement, or controlling those growing
holonomies at logarithmic lamp scale, is exactly the missing
hyperlinear-to-sofic theorem.  Nothing here proves that `W` is
nonhyperlinear.
