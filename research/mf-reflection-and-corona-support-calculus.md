...
having finitely many coordinate orbits.  Symmetrized Stallings--Bieri kernels
then yield, for every `n>=2`, a one-word internal radical whose MF reflection
is of type `F_(n-1)` but not `FP_n`, with every classical residual and
completion still pulled back exactly.
...
normalizers, cores, indices, normality, and quotients.  A swap-symmetric
Mihailova fiber product in two free actors then produces one fixed finitely
generated subgroup of `U` with undecidable membership, a radical normally
generated by one involution, and a residually finite characteristic-zero
linear reflection.  Every finite-dimensional representation still factors
...
normal form makes the word problem of `U` and of the finite-wreath ambient
