...
Finite block diagonals combine distinct reflection carriers, so the internal
reflection class is closed under heterogeneous finite products as well.
Together with finite wreath lifting this gives a finite expression calculus
generated by countable virtually free groups.  Every resulting reflection is
residually finite, and every corresponding internal group has exact closure
pullback and the same profinite, Bohr, compact, linear, and MF-visible data as
its reflection.
Taking the cyclic wreath actor `C_m` over the free reflection realizes every
positive virtual cohomological dimension `m` while retaining a radical
normally generated by one involution.

The calculus also passes to finitely generated subgroups of product actors
having finitely many coordinate orbits.  Stallings--Bieri kernels then yield,
for every `n>=2`, a one-word internal radical whose MF reflection is of type
`F_(n-1)` but not `FP_n`, with every classical residual and completion still
pulled back exactly.  A diagonal commutator isolates the finitely many
coordinate orbits, so the quotient remains torsion-free and residually `2`.

In full generality, every finitely generated subdirect product of finitely
many finite-rank free groups occurs as such a one-word internal reflection.
Its quotient is torsion-free, characteristic-zero linear, and residually
`p` for every prime.  The single perfect kernel is consequently every
finite-`p` residual simultaneously, as well as the finite, nilpotent,
solvable, compact, linear, and MF residual and both terminal commutator cores.
Both the quotient and its internal lift have decidable word problem.

More precisely, the preimage construction identifies the complete subgroup
lattice of every finite product actor with the full interval above the common
invisible base inside `U`; it preserves arbitrary meets and joins,
normalizers, cores, indices, normality, and quotients.  A Mihailova fiber
product in two free actors then produces one fixed finitely
generated subgroup of `U` with undecidable membership, a radical normally
generated by one involution, and a torsion-free residually-`2`
characteristic-zero linear reflection.  Every finite-dimensional representation still factors
uniquely through that visible linear group.  The actor is in fact residually
`2`; perfection of the invisible base consequently identifies the same
one-word radical with the finite-`2`, nilpotent, and solvable residuals and
with both the lower-central and derived-series intersections.  The Leavitt
normal form makes the word problem of `U` and of the product ambient
group decidable, so the fiber product gives an undecidable generalized word
problem inside a fixed group with decidable word problem.
