...
3. **A hard test input** (bh-free-02, 2026-09-18). The Wächter--Weiß automaton groups
   ...
   group `R`, such as `A_5`:
   - the subgroup `H_R` generated by the initial states is PSPACE-complete, and it is
     locally-finite-by-`Z`, so it is not finitely presented;
   - `G_R` itself is locally-finite-by-(check-mark group), and whether it is finitely
     presented is open (`finite-commutator-wachter-weiss-groups-are-not-fp`, corrected
     09-18).
   - *Why it matters.* Any finitely presented self-similar overgroup of `G_R` or `H_R` would
