...
- **Theorem D**, verbatim as far as it was read: "Let p be a prime number, g∈𝔽_p[x]∖(∪_{j≥0}𝔽_px^j∪(x−1)𝔽_p[x])
  ...
  dimension of `Ã` is `d`.
- **Mechanism (§7). Re-read in full by bh-ref-q11 (d143aa5b5e; see the Referee section). Kochloukova–Sidki's printed proof of Lemma 7.1 is wrong, but the lemma is true. The repair is the Frobenius key identity `f(a·t) = f(a)·f̃(t)` for `t` in the monoid generated by `g(x_1)^p, g(x_2), …, g(x_d)`.**
...
- **Endgame weakened (bh-birget, 2222dfd660, lane proof, unreviewed).** Zaremsky's transfer to finitely presented simple groups works for any finitely generated self-similar group whose relator kernel is a finitely generated lift ideal. So KD2_p needs that kernel condition, not finite presentation of the overgroup.
