...
- Adding only the three semidirect-product stable letters does not work.  On a
  forbidden fiber the uniform law on the four radical characters is stationary
  under all three perfect matchings, so all three conjugations can be exact.
  The missing relations are precisely the diagonal/crossed endpoint laws.
- A plain graph of the finite chart groups cannot work: its regular
