...
- **Trivialize the holonomy cohomologically.**  Unavailable.
  ...
  displacement length is inflated from `0` to the full diameter by a single
  conjugation, so the coefficient hypothesis of every conjugation-invariant
  metric cone or filling theorem — including the `A_3(F_q)` gadget of
  `a3-cone-repairs-to-coboundaries-not-cocycles` — fails at the outset.  What
  survives is that the length is invariant to within `2 rho` under conjugation
  by a shadow map, which is the only conjugation `(SHG2)` performs.  Any
  filling argument is therefore restricted to paths of bounded length, with the
  bound fixed by the table rather than by the dimension.
- **Transport the regular action along near-orbits.**  Circular.
  `orbit-transport-gauge-needs-a-closed-label-set` shows that declaring blocks
  `{sigma_a(b)}` and setting `tau_s(sigma_a(b))=sigma_(sa)(b)` makes every
  tested relation hold *exactly* on the packed region with correction radius
  `5 rho+zeta`, reducing the whole claim to one uncoupled near-perfect packing
  — but only when the label set is closed under the tested products, or almost
  invariant in the sense of `(OTC4)`.  Demanding the latter of every finite
  chunk of the ambient group is the Folner criterion, hence amenability, hence
  soficity already.  The same node records that no counterexample can come from
  a closed table or an amenable ambient group.
