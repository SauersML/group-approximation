...
The finite-normalizer option is now closed exactly.
`extraspecial-fourier-zero-cell-is-fixed-face-pruning` shows that `SC`
normalizes the enlarged abelian packet generated by `X_i` and
`S W_i S^-1` through the shear `(a,chi)->(a+chi,chi)`.  The unwanted trivial
payload character is precisely its fixed face.  Inducing that fixed
character produces an exact marked countermodel for every finite (and every
residually finite faithful) normalizer completion.  Collapsing the packet to
remove it identifies `J` with an affine parity word.  Thus a successful
bounded cell must be nonnormal/payload-sensitive, not another finite
Clifford or character-permutation table.
This is not repaired by adjoining a finite-packet Householder reflection.
By `central-pinning-forbids-nonlinear-predicates`, uniqueness of the marked
central character makes the spin character vanish off `{1,J}`; hence for
`2^k>2` no noncentral group involution in such a uniquely pinned packet can
have a one-dimensional negative eigenspace.  Any larger packet containing
the desired rank-one reflection must therefore reintroduce marked sector
types that the controller has not authenticated.
