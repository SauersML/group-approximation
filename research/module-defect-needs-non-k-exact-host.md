...
kind: claim
title: A module K-defect forces the host to fail Baum--Connes with coefficients, a condition non-exactness does not imply; Haagerup non-exact groups cannot host one
distinct_from:
...
2. Hence, for any module triple `(V, W)` (setting of
   ...
   with coefficients**. A defect independently forces `G` non-exact (an exact `G` has exact
   reduced crossed-product sequences, hence six-term exact K-theory, hence no defect), but
   non-exactness is the weaker of the two necessary conditions and is not enough.
