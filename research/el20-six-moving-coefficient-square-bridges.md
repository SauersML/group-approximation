...
That constant-root search has now also been exhausted through reduced word
length ten.  By
`length-ten-external-returns-have-no-active-singer-centralizer`, every such
word which commutes with either complete Singer head lies in the subgroup
generated by `L_0` and the two disjoint spectator returns for that row.  The
enumeration reaches `208137` distinct matrices per row at depth ten.  Thus
no bounded word in this range supplies the desired same-chart commutation
lock.  This is only a length-bounded constant-root fence: longer words,
nonconstant Leavitt coefficients, and mixed returns not expressible as
direct Singer centralizers remain live.
