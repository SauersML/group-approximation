---
rg: 2
id: central-marked-two-cycle-zero-compression-cell
kind: claim
title: Two gradings groupify a selected zero compression on a payload two-cycle
distinct_from:
  ...
  tailored-checks-to-common-marked-group-extensions: that asks for arbitrary LCU payloads; this closes its final zero-compression gate when the prepared source has already been compiled to a two-cycle of one payload word.
---
...
space.  Take involutions `y,z` and an arbitrary unitary group word `w`, with
relations

```text
[y,w]=[y,z]=1,                 w z w^-1=y z.           (MZC1)
```
...
representation in the one-grading construction.  The return `w^2` on `q`
may carry arbitrary multiplicity holonomy.

The cell is also complete for exactly the intended class.  If `W` is any
unitary and `Q` is a projection with

```text
QWQ=0,                                                   (MZC4)
```

and, writing `Q'=WQW^*`, one also has

```text
QQ'=0,                    W(Q+Q')W^*=Q+Q',             (MZC5)
```

set

```text
P=Q+Q',        Y=1-2P,        Z=1-2Q.                  (MZC6)
```
...
and arbitrary `W`.  Hence the presentation imposes no hidden condition
beyond zero compression and two-periodicity of the selected orbit, up to
this harmless existential choice of gradings.

There is a dimension-free robust estimate.  For exact commuting involutions
`y,z` and unitary `w`, if
...
```text
tau(q)=1/4(1-tau(y)-tau(z)+tau(yz)) ->1/4.             (MZC8)
```
...
LCU compiler.  Relation `(MZC1)` forces the marked source to lie in a
two-cycle of `w`; for a general unitary,
...
```text
compile the losing check with a two-periodic selected orbit of one word.
                                                               (MZC9)
```
...
cell.  What remains is a payload-sensitive two-cycle block encoding, not
the zero-compression relation.  An involutive Hermitian word is one
sufficient implementation, but is stronger than necessary.
