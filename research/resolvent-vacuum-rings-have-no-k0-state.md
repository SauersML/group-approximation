...
- `R_Toep`, the Toeplitz resolvent ring of `q-embeds-in-a-corner-of-a-fp-toeplitz-ring`, in
  its concrete model, which is a `Q`-algebra: `E = 1 − ST`, `x_m = S^(m−1) E`, `y_m = E T^(m−1)`. Here `E` is not
  full; it generates the finitary ideal. `R_Toep` is not the SL_3 Toeplitz harmonic ring `R_T`
  (`sl3-toeplitz-harmonic-ring-is-simple-and-k1-rational`), which is tracial and gets `Q` by harmonic
  elimination, not from a vacuum;
