...
The group-host premise needs more than this, so failing the budget excludes a design **that declares the budget**.

**Scope of the `K_1` clause** (bh-ck-kernel, `ring-gate-k1-clause-is-unstable`, 2768a5be1). The clause above is
*stable*. The necessary condition for a finitely presented `PE_N(R)` is the *unstable* one: `Λ_N(R) = {λ :
λ I_N in E_N(R)}` finitely generated (`rational-scalars-obstruct-fp-elementary-hosts`, part 2). Stable implies
unstable, but not conversely unless `GL_N(R)/E_N(R) -> K_1(R)` is injective on rational scalars. So:
- the branches below that conclude "no `E_N(R)` is finitely presented" exclude hosts of that shape outright;
- the branches that conclude "`ker(Q^x -> K_1(R))` is not finitely generated" exclude designs that declare the stable
  clause. They exclude hosts only where rank-`N` `K_1` injectivity holds on scalars. Otherwise the design falls
  under loophole (i) of What remains.
