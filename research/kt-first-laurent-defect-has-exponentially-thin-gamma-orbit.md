...
fixes `h_N Gamma_N`. Indeed, for `gamma=I+x_1^2 A`, conjugation by
`h_N=I+yE_12` produces only the coefficients
...
and still has zero density in `Omega_N`. The value `M_F` can be taken as
the maximum, over `g in F`, of the sum of the positive Laurent deficits of
the entries of `g` and `g^(-1)`.
...
~~~text
||[p_N,U_h,N]||_2=1/sqrt(2).                             (PAG5)
~~~

Nevertheless every Pauli or Clifford packet generated from the base site,
the named site, and all of their Gamma conjugates is supported on the
Gamma-invariant tensor factor indexed by `{o_N} union O_N`. Even granting
the full matrix algebra on that factor, its normalized adjoint-rank fraction
inside the lamp matrix algebra is only

~~~text
4^(1+|O_N|-|Omega_N|) ->0.                               (PAG6)
~~~

Thus neither the finite Kazhdan packet nor Reynolds averaging over the
entire finite Gamma image converts the pointed inverse failure into
positive normalized-HS mass. The complement is an exact multiplicity
reservoir, while the distinguished operator remains an op-norm-one,
positive-`L^2` wall.
...
not repair (PAG6). A successful amplification must use Laurent words outside
every fixed intermediate packet so that their denominator depth grows with
`N` and connects a positive fraction of the missing Gamma orbits. That is
precisely the still-open moving-window coherence step.
