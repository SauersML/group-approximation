...
|O_N| <= 2^(8b_N),
|Omega_N| = 2^(8v_N),
|O_N|/|Omega_N| <= 2^(8(b_N-v_N))
                    =2^(-2N^3/3+O(N^2)) ->0.             (PAG3)
~~~
...
In fact the ambient cardinality is exact because both coefficient rings
are finite local rings:

~~~text
|Q_N|/|Gamma_N|=2^(8(dim T_N-dim P_N))=2^(8v_N).
~~~

This sharpens the growing-window result. If packets `F_N` have
`log_2|F_N|=o(N^3)` and maximum denominator deficit
`M_N=floor(cN)+O(1)` for any fixed `c<1`, then their complete
`Gamma_N`-saturation still has zero density in `Omega_N`; its base-two
logarithmic density is at most

~~~text
-(2/3)(1-c)^3 N^3+o(N^3).                                 (PAG4a)
~~~

Near the boundary, write `L=N-M_N`. The exact exponent comparison is

~~~text
v_N-b_(N,M_N)
 =sum_(t=0)^L floor((t+2)^2/4)-floor((N+2)^2/4).           (PAG4b)
~~~

Thus the saturation is still thin when `L/N^(2/3)->infinity` and
`log_2|F_N|=o(L^3)`. At `L=C N^(2/3)`, it is thin for
`C>3^(1/3)` and packet log-size `o(N^2)`.

A packet-density proof must therefore reach the near-full truncation
boundary `N-O(N^(2/3))), or use an exponentially cubic number of words.
...
not repair (PAG6). A successful amplification must use Laurent words whose denominator depth
reaches the near-full truncation boundary and connects a positive fraction
