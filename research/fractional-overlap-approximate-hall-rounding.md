...
```text
sum_(i,j) x_(i,j) >= d-floor(beta d),
sum_j x_(i,j) <= rank(P_i),
sum_i x_(i,j) <= rank(Q_j).
```

Equivalently, after leaving unmatched at most `floor(beta d)` scalar
dimensions on each side, the remaining source and target dimensions admit an
exact integer matching along allowed edges.  In particular, if `beta d<1`,
there is already a perfect scalar matching.
