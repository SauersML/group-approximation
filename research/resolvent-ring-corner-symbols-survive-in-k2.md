...
**ESTABLISHED (2026-09-18)** through `resolvent-ring-corner-symbols-survive-in-k2-proof`. Lane proof (gq-k2-q), not
...
3. **Corner symbols.** For `λ, μ ∈ Z_(l)^x` and `N >= 3`, let `c(λ,μ) ∈ K_2(N, R_l)` be the image of the Steinberg
   ...
   - The stable classes of the `c(λ,μ)` generate a subgroup of `K_2(R_l)` isomorphic to the symbol subgroup of
     `K_2(Z_(l))`.
   - That subgroup is not finitely generated. Its image in `K_2(Q)` has a nonzero tame symbol `∂_q` for every prime
     `q ≠ l`, from `{q, g_q}` with `g_q` a primitive root modulo `q`, and a finitely generated torsion group has
