...
- **Tooth tree with universal stages** (sk-popa-lgx, artifact part 1 §3). The plan, where it stands, and what is missing:
  ...
  - *Missing input 1:* residual finite-dimensionality of `U_{i+1}`. That is a full amalgamated free product over `C²` of an RFD algebra with a matrix algebra, with trace-compatible ranks `rank(e) : rank(1−e) = 1 : r_i − 1`. Brown–Dykema `thm:rfd` (arXiv:math/0207005) covers only `M_n *_D M_{n′}`. General RFD theorems over finite-dimensional amalgams were not checked at source.
