...
`p-adic-dilation-normalizers-act-by-power-maps-proof`. Lane proof. The one
non-elementary input is the p-adic six exponentials theorem (cited; theorem
number not pinned).
...
2. **No prime shifts.** For every prime `q in Q_0`, `β(q)` is `±` a rational
   power of `q`. In particular, the relations
...
3. **Two relations.** For a cycle `q_0 -> q_1 -> q_0` the elementary part of the
   ...
   conjecture (open) would exclude. The argument does not exclude a single
