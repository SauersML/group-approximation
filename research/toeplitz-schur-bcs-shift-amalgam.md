...
The exact replacement is now available:
`schur-child-toeplitz-morita-wire` identifies the doubled `A_f` multiplicity
with the algebra generated by the two transverse child commutants, places one
child in the compressed tail, and recovers the other with one fixed head
actuator. Thus the abstract module-type mismatch in clause 3 is closed. What
