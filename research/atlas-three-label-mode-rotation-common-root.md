...
and test its intersection with the unit circle away from roots of unity.

Any common root `z=exp(2 pi i theta)` with `theta` irrational compiles,
through rational Weyl approximants, to full-coefficient regular-A8
...
already excluded by finite-dimensional invisibility, and `z=1` is the
small-phase commutative wall.
