...
- **Every refuter is a smooth `SL_3(Q_2)`-covariant vertex process; the
  real place carries none (2026-09-17, obstruction-miner, host-geometry).**
  `sl3-arithmetic-leaks-are-building-vertex-processes` proves a normal form
  for the refutation side of `(PTS1)`.  For any leak certificate
  `(M, u, U)`, the algebra generated by the `A`-translates of `U` is a union
  of fixed algebras of `A cap W`, `W` compact open in `L = SL_3(Q_2)`.  So the
  `A`-action extends canonically to a smooth trace-preserving `L`-action with
  `N^C = N^K` and `N^A = N^L`.  The leak is an `L`-covariant unitary vertex
  field `(U_v)_(v in L/K)` with `|tau(U_o U_(ho)^*)| < 1`.

  Consequences:
  - *Commutative leaks.*  These exist iff some `L`-invariant random subset of
    the building vertex set that is not a.s. empty or full has a
    Connes-embeddable coordinate crossed product.  `(SH1)` (the lattice-orbit
    colouring) and the coset Bernoulli firewall (the product measure) are two
    points of this one class.
  - *Real place.*  Any system whose `A`-action extends continuously to
    `SL_3(R)`, or to `SL_3(R) x SL_3(Q_2)` diagonally, has
    `N^C <= N^(SL_3(R))` (Howe--Moore).  Real homogeneous and Gaussian
    constructions therefore yield no refuter.
  - *Separating the firewalls.*  The `C`-fixed dimension is `r` for `(SH1)`
    and infinite for Bernoulli, so neither firewall's CE transports to the
    other through base factor maps in that direction.
  - *Finite `C`-fixed dimension.*  Commutative refuters of this kind are
    random equitable Hecke colourings of the building.

  This decides nothing about `(PTS1)`.  A proof of `(PTS1)` must rule out CE
  for every such invariant random subset, and separately for noncommutative
  vertex fields.  The missing step is the building Leighton-type question:
  is every invariant random equitable colouring a factor of a
  lattice-quotient colouring?
