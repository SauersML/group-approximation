...
identity holds at `v` as soon as `psi(m)psi(n)v = psi(nm)v` for the memories
`N` of the decoder and `M` of the encoder, where `psi(g) = phi(g^-1)` puts the
composition in the right order. The set of `v` where that condition fails for a
fixed pair is the complement of the fixed set of the defect
`psi(m)psi(n)psi(nm)^-1`, which is the multiplicative defect of `phi` at the
pair `(m^-1, n^-1)`, of normalized size exactly `l_V` of that defect. By the
displayed computation that size is `1` for a nontrivial defect, so Lemma 1.1
guarantees the identity at no vertex unless the defect is trivial. A union bound
over the finitely many pairs gives the stated form. Lemma 1.1 is only a
sufficient condition, so this does not show that the identity fails anywhere.
...
Applied 2026-09-12 by gk-free-pos: the body above now carries this correction, with the corrected transport along `psi(g) = phi(g^-1)` from Section 1 of the artifact.
