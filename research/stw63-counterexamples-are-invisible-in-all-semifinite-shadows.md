---
rg: 2
id: stw63-counterexamples-are-invisible-in-all-semifinite-shadows
kind: claim
title: Every LXIII counterexample is invisible in every separably acting semifinite von Neumann shadow
distinct_from:
...
Let `phi,psi:Z->B` be unital embeddings into a unital C-star algebra.  For
every unital star homomorphism

```text
pi:B->M
```

into a separably acting semifinite von Neumann algebra, the maps `pi phi` and
`pi psi` are approximately unitarily equivalent in `M`.
...
after passing to a tracial factor completion, but in every separably acting
semifinite von Neumann shadow of the pair-generated algebra.
