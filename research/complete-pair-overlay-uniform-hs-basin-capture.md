...
There is now a second, sharper diffuse reduction which avoids a global
Newton estimate on every noncentral block.
`uniform-generated-adjoint-gap-rounds-to-central-code` proves that if the
algebra generated by the current shared reflections has a dimension-free
adjoint Poincare constant, conditional expectation onto its commutant sends
each generator near the **center** of that same algebra.  Spectral sign and
the scalar LTC decoder then give central commuting code reflections with
the explicit bound `(UGA4)`.  Therefore a far low-defect countermodel cannot
live in any uniformly gapped generated block; its Poincare constants must
tend to zero.

The alternative route
`generated-adjoint-gap-dichotomy-closes-balanced-basin` isolates the sole
remaining mass as `balanced-overlay-gapless-adjoint-sector-rounding`:
almost-central elements inside the generated algebra have cheap spectral
boundaries, but those cuts must be pasted or iterated without repeating
boundary mass.  This is narrower than the previous unrestricted
multiplicity-centralization problem and cleanly explains why sparse
Clifford cycles evade generic pair rounding: their generated-adjoint gap
itself vanishes with length.
