...
distinct_from:
  lin-gap-wrappers-retain-oracular-atlas-holonomy: that analyzes the projection atlas of Lin's separated game when its perfect strategy is oracularizable; this claim isolates the theorem-level hypothesis and the subsequent BCS-versus-LCS boundary.
...
same-algebra commutation of the two question PVMs.

If a particular Lin instance is separately equipped with the preserved
perfect oracularizable commuting strategy, Mastel--Slofstra Lemma 3.1 and the
BCS construction immediately following it give a weighted BCS `B(G)` with

```text
omega_co(B(G))=1,             omega_q(B(G))<=1-delta       (LMS2)
```

for some absolute `delta>0`.  Definitions 4.1--4.2, Lemma 4.3, Lemma 5.2,
...
Thus even granting `(LMS2)`, the output is not an LCS, has no associated LCS
