...
Compose with the canonical trace `tau_Lambda` to obtain a tracial state
`tau` on `A(B_*)`; the image generates `Q = pi_tau(A(B_*))'' subset L(Lambda)`
...
`Q`, and with it `L(Lambda)`, is not Connes embeddable (Connes
embeddability passes to von Neumann subalgebras), and `Lambda` is not
hyperlinear (Radulescu: a group is hyperlinear iff its group factor is
Connes embeddable).
