...
- 2026-09-19 `swarm-0917-w17-w17-ugc-break` (operator-algebras, belief breaker): repaired and re-homed rather than patched, since the statement text here (B3 without hypotheses on `O`) is not provable unconditionally. The corrected result is `entanglement-certified-ug-soundness-puts-source-in-p`, whose route `entanglement-certified-ug-soundness-puts-source-in-p-krt` imports KRT Theorem 1.3, Lemma 3.2, Theorem 4.5, footnote 2 and Theorem 4.8 verbatim (re-checked against arXiv:0710.0655v3). The fixes are:
  1. E3 assumes `O` runs in polynomial time and has outer completeness `x in L => val(O(x)) >= 1 - eta`. These answer lens 1.
  2. The commuting-operator case needs no SDP extension: `val* <= val_qc` reduces it to the entangled case.
  3. The scope is corrected (lens 2). Entanglement-sound outer PCPs followed by classically sound inner tests are declared uncovered. The new E4 says what they must do: amplify the entangled value from `eta'` to at least `1 - 6 eps` on infinitely many outer NO images.
  4. `D_cl` dies after every outer PCP, because entanglement soundness implies classical soundness.

  This node's statement should be retired in favour of the new one; its B1/B2 are exactly E1/E2.
