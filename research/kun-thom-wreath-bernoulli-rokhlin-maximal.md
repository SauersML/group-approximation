...
- **Generation-pattern escape hatch (w3-inf-b, 2026-09-12; corrected forward).** The counting bound
  `H(P) >= (log|A| - eps) * sup_(F') |F'|/|F'F|` needs Følner sets. Two ways of getting them without
  a subgroup, and where each stops. Both are mechanism analyses, not theorems.
  - **Schreier graphs of the action.** The Bernoulli action of `W` is essentially free, so almost
    every orbit's Schreier graph is the Cayley graph of `W`. `W` is nonamenable, since it maps onto
    the nonamenable actor `G`. These graphs have no Følner sets, and the bound is the calibrated
    `sup |F'|/|F'F|`, unchanged.
  - **Subrelations generated by the partition.** Inducing from a subrelation `S` of the orbit
    relation `R` consumes relation-level co-amenability of `S` in `R` (Section 3 of
    `rokhlin-lower-bounds-without-finite-models-2026-09-12`). If `S` were hyperfinite and co-amenable,
    composing means would make `R` amenable. But `R` is the orbit relation of an essentially free pmp
    action of a nonamenable group, so it is not amenable (Zimmer; mentioned, not imported). So an
    amenable `S` supplies no Følner sets, and a nonamenable `S` returns the original problem. The
    composition of means is standard for groups (Eymard) and is not re-derived for relations here.
  - **Withdrawn.** An earlier version of this bullet said the witness actor `π(K)` is Kazhdan and that
