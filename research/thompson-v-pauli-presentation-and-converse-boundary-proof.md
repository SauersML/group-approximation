...
There is a sharp positive theorem on the other side of this boundary.  If
the point stabilizer has a finite Kazhdan set, the ordered-pair rank is
finite, and the actor matrices form a genuine finite-dimensional
representation, stabilizer-commutant rounding followed by finite-dimensional
compactness gives
...
Section 1 supplies the two finiteness hypotheses needed for the presentation,
but it supplies no Kazhdan hypothesis.  More importantly, a general
hyperlinear approximation has only asymptotically multiplicative actor
coordinates, so even in a Kazhdan example the exact-coordinate theorem does
not apply until one proves actor exactification or uniform double-coset
propagation.
