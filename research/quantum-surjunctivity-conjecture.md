...
**2026-09-18 (swarm-0917-w6-w6-gs-break, per-site support counting).**

- **Established: every group, commuting site supports.** `commuting-site-supports-force-quantum-automorphisms`.
  - Let `S(h,k)` be the algebra generated by the site-`k` slices of `Phi(A_h)`.
  - If the supports arriving at each site pairwise commute, the count
    `d <= prod_m k_min(S(e,m)) <= d` closes on every group. Here `k_min` is the least irreducible dimension. The lower
    bound comes from `M_d` embedding in the tensor product of supports; the upper bound from commuting supports acting
    on `C^d`.
  - The equality case splits each site into tensor slots, so `Phi` is an on-site isomorphism followed by a slot
    wiring, hence an automorphism with neighbourhood `N^-1` for its inverse.
  - Commutation holds whenever `N` is left Sidon (`|N cap gN| <= 1` for `g != e`), by a slice lemma at one-site
    overlaps.
- **Belief corrected.** The "Where it stops" item above says support commutation needs the order of `Z`. It does not:
  it needs one-site overlaps, and then no approximation of `G` is used.
  - The step-3 remark (for prime `d` a single slot carries `M_d`) is now proved for every `d` and every host. It
    explains the `3 |Sp_2(F_p)|` census count.
- **Obstruction for counterexample search.** A strict automaton on any group must have noncommuting supports at some
  site. So its minimal neighbourhood has a two-site overlap: `|N cap gN| >= 2` for some `g != e`.
  - Over torsion-free groups that needs at least three points with a quotient coincidence.
  - The cluster entangler (artifact E2) shows that two-site overlaps really do break commutation.
- **Where it stops.** At noncommuting supports the per-site bound (b) fails, since noncommuting subalgebras of `M_d`
  have unbounded `k_min` product. Nothing replaces it. The strict tree automaton shows the count also needs the
  group's in-slot/out-slot matching. See
