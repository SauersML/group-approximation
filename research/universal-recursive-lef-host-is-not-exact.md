---
rg: 2
id: universal-recursive-lef-host-is-not-exact
kind: claim
title: The simple Kazhdan LEF host containing every recursively presented f.g. LEF group is not exact
distinct_from:
  some-simple-kazhdan-lef-groups-are-not-exact: that gives some non-exact host through Osajda's group; this shows that the one universal host for recursively presented LEF groups is itself not exact.
artifacts:
  - research/artifacts/sk-free-1-exactness-2026-09-13-part2.md
---

**ESTABLISHED (unreviewed).** Route: `universal-recursive-lef-host-is-not-exact-proof`.

The infinite, finitely generated, simple Kazhdan LEF group of `simple-kazhdan-lef-host-for-recursive-lef-groups` has no property A, so it is not exact and its reduced C*-algebra is not exact. Its group von Neumann algebra embeds in R^ω.
