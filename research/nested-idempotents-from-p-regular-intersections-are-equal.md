...
**ESTABLISHED** (route `nested-idempotents-p-regular-intersection-proof`; re-derivation
...
- **Binary candidates killed.** Over `F_2` this kills every candidate built from two finite
  ...
  Klein-type and dyadic permutation constants survive only when their intersection contains an
  involution; that case is `modular-nested-idempotents-equivalent-over-leavitt-units`.
