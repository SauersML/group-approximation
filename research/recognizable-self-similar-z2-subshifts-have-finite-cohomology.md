...
- **Integral coefficients.** Every finitely generated subgroup of `H^2(Z^2; C(X, Z))` lies in a
  subgroup generated by `N_{3,3}` classes, so the rank is at most `N_{3,3}`. Integral `H^2` need not
  be finitely generated. For the Thue–Morse square `X_TM × X_TM`, Künneth gives
  ...
  finite generation.)
- Integral `H^1` need not be finitely generated either: it can be an increasing union of
  bounded-rank groups, as `Z[1/2]` is.
