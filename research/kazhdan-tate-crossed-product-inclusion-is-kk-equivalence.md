...
- **Banach KK (dies).** Lafforgue's `γ = 1` in Banach KK controls only K-theory of the crossed
  product.
- **Γ-level generation (dies at K-amenability).**
  - The obstruction functor `Ψ(A) = W ⋉_r (cone ε ⊗ T(A))` is additive and exact.
  - It kills every object induced from an amenable subgroup
    (`tate-norm-functor-kills-amenably-induced-objects`).
  - For infinite Kazhdan `Γ`, the unit is not generated by such objects: `λ : C*_max Γ → C*_r Γ` is
    not a KK-equivalence. So this claim is about the unit of the non-K-amenable quotient.
