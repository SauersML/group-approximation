...
**ESTABLISHED (lane proof, bh-sz-vdg, 2026-09-19; elementary; not independently reviewed).** It is a scoping
...
3. **Trichotomy.** By contraction, for every `g ∈ G` there is a level `m` at which all sections of `g` lie in `N`.
   ...
   - In case (E), an SCC containing two distinct cycles through a common vertex `v` gives two distinct return
...
- **Most concrete next case in (E).** Sub-hyperbolic PCF rational maps, i.e. those with strictly preperiodic
  critical points. `hyperbolic-pcf-rational-nekrashevych-groups-are-f-infinity` lists them as "not covered here":
  the natural cover is an orbifold fundamental group, which is `F_∞`, but the dying-kernel property needs an
  orbifold version of Nekrashevych's `pr:expandingpi1`. The torsion obstruction in
  `contracting-groups-have-dying-kernel-covers-of-type-fn` is consistent with this, since orbifold groups have
  exactly the torsion that recurrent germs force. It is not checked here.
