...
**Step 0. `φ` is injective.** If `φ(g) = 1`, then `ρ_g = s^m s^-m ρ_g s^m s^-m` is the
...
3. *Sizes.* For `a >= M`, the orbit of `a + m` is `f` of the `≈`-class of `a`, which lies
   inside the orbit of `a`. So `|orbit(a + m)| <= |orbit(a)|`. Orbit sizes on `[M, ∞)` are
...
4. *Conclusion.* A countable FW group has property FA, so it is finitely generated. So `P`
