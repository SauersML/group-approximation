---
rg: 2
id: stw74-cu-axioms-do-not-extract-projective-roots
kind: claim
title: The standard Cu axioms do not extract compact subelements from finite roots of a compact infinite unit
distinct_from:
...
```text
u << u,                 2u=u,
for every 0!=x in S, some finite n satisfies n x=u,
```

but some nonzero `x` dominates no nonzero compact element.  In fact, `S` can
...
This is an abstract Cu countermodel only.  No claim is made that `S` is
`Cu(A)` for a C\*-algebra; realizing such a semigroup with the unit represented
by a projection would amount to crossing the unresolved LXXIV boundary.
