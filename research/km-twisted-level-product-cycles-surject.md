...
- **Torsor reduction: the remaining cell is a linear span in the
  ...
  it acts transitively on the class set `V_j` with stabilizer the
  center `Z(U)`: for `B_2`, `U = q^(1+2)` and `V_j ~ U/Z ~ F_q^2`
  (`|V_j| = q^2`); for `G_2`, `U = q^(1+4)` and `V_j ~ U/Z ~ F_q^4`
  (`|V_j| = q^4`, counting points at distance five with fixed foot).
