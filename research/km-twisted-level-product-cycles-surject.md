...
- **Complete single-slot proof for large `q` (2026-08-23, final; held
  for the audit round before marking established).**  Fix a class `i`,
  points `p'_0` (A-side) and `p''_0` (B-side) of class `i`, targets
  `a in Z_0(N p'_0)`, `b in Z_0(N p''_0)`, and disjoint class sets
  `C, C'` avoiding `i` with `|C| = |C'| = d` (the Frattini dimension:
  four for hexagon line-panels, three for `B_2` line-panels, two for
  point-panels; possible once `q + 1 >= 2d + 1`).  (1) The `C`-walk
  graph is connected and contains every line: the walk group is
  generated by the class stabilizers `S_c`, `c in C`, whose Frattini
  images are `d` distinct points `(1, lambda, lambda^2, ...)` of the
  rational normal curve, independent by Vandermonde in every
  characteristic, so they generate the radical modulo Frattini, hence
  the radical, which is simply transitive on the lines.  (2) For an
  elementary difference `e_l - e_(l')` at `p'_0`: a path between `l`
  and `l'` in the `C`-graph never visits `p'_0` or any other class-`i`
  point (its interior points have classes in `C`), so closing it
  through `p'_0` gives an integral cycle with local datum
  `e_l - e_(l')` at `p'_0` and zero local data at every other class-`i`
  point; sums realize any `a`.  (3) With `c` as in (2) and `c'` built
  from `C'` on the `B` side, the only matched-class slot where both
  local data are nonzero is `(p'_0, p''_0)` (the point-support classes
  are `{i} cup C` and `{i} cup C'`, meeting only in `i`, where each
  side touches only its base point), so the image is `a (x) b` in that
  slot and zero elsewhere: single-slot generation over `Z`, hence
  integral surjectivity.  Inputs still under external audit: the wall
  dictionary and torsor identifications (computationally validated
  four ways) and the structure theorem's level formula (validated in
  both regimes).  Once confirmed, this claim is established for
  `q >= 8` in any characteristic, and with it the `F_3` and
  `H^2(Lambda;Z Lambda) = 0` chain for all the program's lattices.
