...
**SUPERSEDED by `fix-bhmet-char-zero-coords-denominators`.** Duplicate fix node for the same module, landed in parallel; its classification and target are merged there. Track this module only on `fix-bhmet-char-zero-coords-denominators`.
