...
This escape cannot be closed by abstract complete-order arguments.
`split-ucp-matrix-retracts-can-hide-defect-in-a-boundary-ideal` gives a
finite-dimensional split ucp complete-order embedding into a matrix
amplification for which the retraction kills a nonzero generator defect even
on the C-star algebra generated by the range.  The defect is a Shilov
boundary summand.  Any further obstruction here must therefore use special
Maslov/even-fibre structure rather than the formal retract axioms.

Even several apparently Maslov-like structural inputs are insufficient.
`v4-twisted-fibre-has-a-nonfaithful-untwisted-matrix-retract` realizes the
same escape between the simple nontrivial twisted fibre and the untwisted
fibre of one finite central extension, with a finite-dimensional target.
Thus simplicity, central-character origin, RFD target structure, and finite
amplification still do not detect the hidden defect.  The remaining usable
structure has to involve canonical generator covariance (or a quantitative
surrogate), not just ideals and fibres.
