...
5. **The only finite-presentation criterion found presupposes a type (A) actor
   ...
   presentation of `V ⋊ W` for `W` finitely presented and `V` a quotient of a sum of
   permutation modules `F_p[W/H_i]` by a finitely generated submodule, with finitely
   generated `H_i` and finite double coset spaces. By the consequence recorded there,
   the acting group `W` is then already of type (A) on the generating orbits. So an
   affine actor built through this criterion never adds an input beyond one already
   inside a type (A) actor: via this criterion the universal affine target is no easier
   than `permutational-boone-higman-conjecture` itself. Whether finite presentation of
   `V ⋊ W` forces these hypotheses for some generating set is not known outside
   permutation modules.
   - **Calibration settled up to classical inputs.** For `W = V` and `S = C(C, F_p)`,
     `Stab_V(χ_[0]) ≅ V × V`, the double cosets correspond to emptiness patterns of Venn
     atoms, and the kernel is generated by the additivity relator on one triple
     partition. So `C(C, F_p) ⋊ V` is finitely presented once `V` is (Cannon--Floyd--Parry;
     not re-read), giving affine type (A) actors with at most `2^p - 1` orbits for every
     prime `p`. That settles the "expected, not established" item of Attempt 1 modulo
     finite presentation of `V`, not as a node.
   - **Leavitt instance reduced.** Through the same criterion
     `leavitt-cantor-module-semidirect-product-is-fp` reduces to
     `leavitt-unit-clopen-indicator-stabilizer-is-fg` (route
     `leavitt-cantor-module-fp-via-indicator-stabilizer`), and by the consequence above
     that premise gives PBH for `U` directly (`leavitt-pbh-via-clopen-orbit-action`).
6. **Linear actors are linear Cantor actors; Attempt 1 does not cover them.** For `S` a
   countable-dimensional `F_p`-space, `S^* = Hom(S, F_p) ≅ F_p^N` is a Cantor space,
   `W` acts on it by linear homeomorphisms, and evaluation embeds `S` in `C(S^*, F_p)`
   equivariantly, with the `W`-orbits on `S` being the orbits on nonzero linear
   functionals. Such `W` need not contain `V` or be closed under clopen cut-and-paste
   (linear homeomorphisms fix `0`), so Attempt 1's reduction to full Cantor groups does
   not apply. Candidates tried and failing orbit-finiteness: `SL_n(F_p[t])` and
   `SL_n(F_p[t, t^(-1)])` on `F_p[t^(±1)]^n` (the gcd of the coordinates is an invariant
   with infinitely many values); finitary `GL(∞, F_p)` is transitive on nonzero vectors
   but not finitely generated. Monomial groups `C(C, F_p^×) ⋊ W_0` with `W_0 <= Homeo(C)`
   are homeomorphism groups of `C × F_p^× ≅ C`, back in Attempt 1's setting.
7. **Walls, as they stand (2026-09-13).**
   - **Complexity** (`complexity-bounded-host-classes-are-not-universal`): no shared
     recursive bound is known for finitely presented `F_p`-linear orbit-finite actors,
     so this wall does not yet apply. The word problem of `W` reduces to fixing every
     translate of the module generators, with no finite test known.
   - **Finite presentation of non-finite-state full groups** (lanes bh-fp-criterion,
     solve-bh-shell-envelopes): these concern full Cantor groups; linear `W` escape them
     by 6, but the only criterion available for `V ⋊ W` (5) routes back through a type
     (A) action of `W`.
   - **Exact gap.** A finitely presented `W <= GL(⊕_N F_p)` containing a given decidable
     input, with finitely many orbits on nonzero vectors and `V ⋊ W` finitely presented
     without `W` being a type (A) actor on any generating orbit, or a proof that finite
     presentation of `V ⋊ W` always yields such an action.
