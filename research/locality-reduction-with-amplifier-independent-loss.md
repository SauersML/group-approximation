...
every integer `t>=2` constants `S_t,A_t>=1`, together with a deterministic
...
maps to some `H'` in `L(k_0,g_0,omega_0)` on between `N` and `S_t N` qubits with at most
...
If `B` is the maximum bit length of a term description (including support
and layer-weight descriptions), require `B'<=S_t(B+N+M+1)`, absorbing fixed
amplification factors into `S_t`. This linear bound is needed through
logarithmically many rounds; polynomial size for one call is insufficient.
The lower site-count bound keeps the additive completeness error negligible
after padding the seed. These explicit resource obligations were added in
the September 20 precision audit.
...
(Dinur--Reingold). No-cloning forbids an isometry that duplicates every unknown witness;
it does not forbid an honest prover from supplying tensor powers, and swap
symmetry alone does not imply tensor-power structure. The precise fixed-code
obstruction is [[correctable-data-tests-are-logically-blind]]: tests reading
only erasure-correctable data regions cannot read the logical state even
with auxiliary proofs. Neither observation is a general no-go theorem for
quantum composition.
