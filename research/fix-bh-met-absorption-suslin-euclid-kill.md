---
id: fix-bh-met-absorption-suslin-euclid-kill
kind: claim
rg: 2
title: "Fix GroupApproximation.BooneHigman.Metabelian.AbsorptionSuslinEuclidKill: Type mismatch (2 errors on main 3c8b417428)"
---
...
**SUPERSEDED by `fix-bhmet-absorption-suslin-euclid-kill`.** Duplicate fix node for the same module, landed in parallel; its classification and target are merged there. Track this module only on `fix-bhmet-absorption-suslin-euclid-kill`.

**Module.** `GroupApproximation/BooneHigman/Metabelian/AbsorptionSuslinEuclidKill.lean`. It is red on origin/main at 3c8b417428 with 2 hard errors, from bh-pal-integrate's trusted MSI build
...
**First errors.**
```
    line 108:12: Type mismatch
    line 109:12: Type mismatch
```
...
**Resolve when.** A trusted build (`gqprobe-lean.sh`) of `GroupApproximation.BooneHigman.Metabelian.AbsorptionSuslinEuclidKill` is green with no errors, and the repaired bytes are on
