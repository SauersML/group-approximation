...
group (INF, `bernoulli-rokhlin-entropy-maximal-for-every-group`) proves the
conjecture. INF is equivalent to positive Rokhlin entropy for every group
(Seward). It passes to subgroups, is decided by finite witnesses, and reduces to
finitely generated simple Kazhdan hosts
(`rokhlin-maximality-via-fg-simple-kazhdan-hosts`). Every deficit witness folds
on a sofic-invisible element (`sofic-radical-localizes-bernoulli-deficit-witnesses`),
which recovers the sofic case by a direct count and says nothing on simple
nonsofic hosts. Universal preservation of the uniform Bernoulli measure by
injective automata (`every-injective-ca-preserves-uniform-bernoulli-measure`) is
...
`g -> s0 g t0 + s1 t1` meets the scalars only in 1, so `L_k(1,2)^x` embeds in
`L_k(1,2)^x / k^x` (`leavitt-units-embed-in-their-scalar-quotient`). Over every
finite field the quotient is therefore nonsofic
(`projective-leavitt-unit-groups-mod-scalars-are-nonsofic`), and nonsurjunctivity
ascends from the unit group to `PG`. Every element of the adjoint image algebra
fixes the line of 1, which rules out the first image witnesses. The Klein group
generated by the swap and the sign unit gives an expectation that splits as
`delta Psi` with `Psi delta = id`, both fixing 1. Whether that splitting lies in
the span of `Ad(PG)`, or lifts to `F_3[PG]`, is open
(`projective-ternary-klein-idempotent-is-full`).

**Invariant architectures (2026-09-12).** An injective automaton is a measure
conjugacy onto its image measure and a topological conjugacy onto its image, so
every isomorphism invariant of the image action equals that of the Bernoulli
action, and no invariant comparison can certify surjectivity
(`measure-conjugacy-invariants-cannot-certify-surjectivity`). Among invariants
bounded by the Shannon entropy of generating partitions, Rokhlin entropy is the
largest (`rokhlin-entropy-is-the-largest-partition-bounded-invariant`). So every
saturation-plus-deficit architecture needs Bernoulli Rokhlin maximality as its
lower bound. A proof outside these architectures must use coordinate data (window
laws, completion counts, block-code radii) together with the group structure of
the index set, since the end-fixing tree calibration has the first three and a
strict automaton.

**Finite-field Leavitt hosts (2026-09-12).** Every host `F_q[L_(F_q)(1,d)^x]`
splits over the `q - 1` scalar characters, and the evaluation sees only the
identity-character factor `A_iota`. Permutation units of `m`-ary families give an
image-level corner solution on every host. When `m` divides `q - 1`, a diagonal
twist makes `A_iota = M_m(T)`, and fullness becomes an `m`-pair Cohn family in
`T`. The ternary corner is the case `(3,2,2)`, and `F_4` gives the
characteristic-two analogue. The problem is monotone in `q`, so it may be posed
over the colimit of finite fields (`leavitt-visible-cyclic-corner-is-full-on-some-host`).
...
**Index lane (sw-001, 2026-09-17).** The lane looked for a Fredholm-type index of an injective
automaton `tau` that is valued in a group with a unique state and that exists over every group. Three
candidates died, each at one step.
- **Homoclinic groupoid.** The homoclinic relation `R` on `X = A^G` (configurations that differ at
  finitely many sites) is AF. So `H_0(R) = H_0(R ⋊ G) = Z[1/|A|]`, identified by the Bernoulli
  measure `mu`, over every `G`. `tau` is not an open map on the unit space when it is not surjective,
  because `tau X` is a proper subshift and so has empty interior. Therefore `tau` is not étale, and
  there is no pushforward: `1_(tau U)` is not in `C_c(X, Z)`. The pullback `f -> f o tau` does act on
  `C(X, Z)`. It descends to `H_0` exactly when `mu(tau^-1 V) - mu(tau^-1 V')` vanishes whenever
  `mu(V) = mu(V')`, that is, when `tau_* mu = mu`. That is already
  `injective-measure-preserving-ca-is-surjective`, which is equivalent to the goal. So the index
  invariant carries no new information.
- **Crossed product `C(X) ⋊ G`.** Here `tau^*` gives a unital endomorphism, and its `K_0` map fixes
  `[1]`. The states are the invariant measures, and `tau_*` acts on that simplex (Poulsen or Bauer) as
  an affine self-map. Such simplices have proper affine injective self-embeddings, so neither a trace
  nor an index can detect non-surjectivity.
- **Von Neumann dimension on local-function modules.** The modules `W_R` of functions reading a
  translate of `B_R` are finitely generated projective over `C[G]`, and `tau^*` maps `W_R` into
  `W_(R+r)`. The `dim_(L G)` count loses exactly the boundary term `W_(R+r) / W_R`, which is the
  amenable counting argument and needs Følner sets.

What survived from the lane is a transplant from the linear case rather than an index. The conditional
expectation `E_H` onto `F[H]` (`a in F[H]` and `ab = 1` give `a E_H(b) = 1`) has a nonlinear analogue:
freeze the input off a subgroup. This gives `surjunctive-decoder-subgroups-force-surjectivity`. Every
strict pair descends to a strict pair over the nonsurjunctive, hence nonsofic, subgroup generated by the
differences of its decoder memory. Consequently the three-point decoder case is closed for sofic and
free `<s,t>`.

**Operator-algebraic dimension lane (sw-003, 2026-09-17).** The seed was a dimension on a crossed
product or groupoid algebra that is monotone under injective automata and strictly drops under
non-surjective ones. As stated it cannot exist.
- **Monotone forces invariant.** For a unital endomorphism, monotonicity applied to `1 - e` forces
  `dim o Phi = dim`. So any finite additive dimension that is monotone under `Phi_tau` is invariant.
- **Diagonal sector.** On `LC(A^G) x| G` the dimension is an invariant measure `mu`, and invariance means
  `tau_* mu = mu`. With `mu` uniform this is the goal-equivalent measure statement.
- **Reframing.** The lane replaced the object with local *-endomorphisms of the full quasi-local
  algebra `(x)_G M_d` (quantum cellular endomorphisms). There the trace is automatically invariant and
  strictness is carried by the relative commutant. `sofic-groups-are-qca-surjunctive` proves three
  things. First, an endomorphism is onto iff its local relative commutant is trivial. Second, sofic
...
**Collision traces of the decoder (sw-014, 2026-09-17, dead: subsumed).**
Idea: kill strict pairs by reading a finite decoder collision `D = supp(x - z)`, with
`sigma x = sigma z`, through the traces `hS ∩ D`.
- At `d = max D` for a left order, the trace at `h = d s_min^-1` is `{d}`.
- In general a trace is a singleton whenever `1` lies outside the semigroup generated by
  `s_0^-1 S \ {1}`.
- So a decoder permutive at such a coordinate is pre-injective, and the encoder is surjective,
  because each `sigma`-fibre of the homoclinic class holds exactly one image point.

Where it dies: this is the positivity filter of
`strict-pairs-need-inverted-memory-or-positive-memory-relations`, read on the decoder instead of
the encoder. That node is already stronger on the encoder side: in its case 2 the encoder is
identity-permutive with context `E`.

What is left: in case 2, `S ∩ M^-1 = {1}`, so the decoder is identity-permutive on image contexts.
The pair becomes a `Sym(A)`-valued cocycle `c(x,g)`, read on `gE`, that the output must recover
from `gS`. Any new obstruction has to use that readability. Singleton traces and pointed-semigroup
positivity cannot give one.
