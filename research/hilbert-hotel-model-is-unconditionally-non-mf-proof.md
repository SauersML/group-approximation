...
kind: route
title: Build the Property-T compression core in coordinates and let the defect be everything
...
Machine-checked and premise-free, but **not Property-`(T)`-free**.  The matrix
identities are computations in the `16 x 16` frame; the passage from those
identities to MF invisibility uses the model's Kazhdan property as a
...
Thus `requires: []` means only that Cairn has no open prerequisite for this
route.  It does not mean the proof avoids Property `(T)`, and this route does
not establish `property-t-free-leavitt-full-mf-radical`.
