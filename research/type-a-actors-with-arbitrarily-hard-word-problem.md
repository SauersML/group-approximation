...
5. **Normalizing groups.** By `normalizer-image-of-type-a-actor-is-type-a`,
   ...
   - *Status.* No oligomorphic finitely presented actor with a hard normalizer is
     known.
6. **Hardness carried by half-finite generators.** Route
   `hard-type-a-actors-via-hard-half-finite-shift-envelopes` reduces this claim to
   the OPEN `hard-half-finite-permutations-have-fp-shift-envelopes`.
   - *What is proved.* `half-finite-shift-envelope-wp-computes-generators`: the
     word problem of `<gamma_1, ..., gamma_k, E_2>` evaluates each `gamma_i`,
     with polynomial overhead in the values. Mallery--Zaremsky's proposition
     makes the group strongly shift-similar, and Zaremsky's Proposition 5.3 then
     gives a type (A) action.
   - *What changes.* No input group with a hard word problem is needed. One
     finitely presented envelope per bound suffices, generated by any linearly
     displaced half-finite permutations that are hard to evaluate.
   - *Status.* Eventually periodic generators die as a hardness source. Minimal
     aperiodic patterns look infinitely presented through their germ groups
     (heuristic). Rich patterns with SFT orbit closure are untested.
