...
Take a Drinen--Tomforde desingularization `F` of `E`.  At each infinite emitter
which belongs to a cycle, enumerate first all emitted edges which themselves
belong to a cycle.  There are only finitely many such edges in total.
...
every `h in H` by lifting the corresponding old path.  If an old vertex `v` is
reached by a lifted cycle, collapse the path to see that an old cycle reaches
`v`; an old hub reaches `v`, and that path lifts.  If a new tail vertex based
at `v` is reached by a lifted cycle, the collapsed path reaches `v`; some hub
therefore reaches `v`, and then reaches the new vertex by following the tail.
