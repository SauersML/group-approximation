...
Lane `ex2-k1-defect-ideal` (2026-09-13) added the following.

First, `o2-quotient-extension-ideal-k1-injective-iff-algebra`. Let
`0 -> I -> B -> Q -> 0` be an extension whose quotient Q is O_2, or
more generally any unital, Z-stable, properly infinite algebra with
`K_0(Q) = 0`. Then `I~` is K1-injective iff B is. Loops in `U(Q)` lift
through exponentials. No Cuntz pair in `M(I)` is needed.

Second, this gives one-ideal forms of LXI. Let `I_c = ker(T_2 * T_2 ->
O_2 ⊗ C(T))` and `I_f = ker(T_2 * T_2 -> O_2)`, and let J be the ideal
generated by the Toeplitz defect `iota_1(p_3)`. Then `J ⊆ I_c ⊆ I_f`,
and LXI holds iff `U(I_c~)` is connected, iff `U(I_f~)` is connected
(Blanchard 0804.4624v13, Proposition 4.2, read in the PDF).

Third, `toeplitz-defect-ideal-connected-iff-extension-k1-injective`
...
ω-comparison, properly infinite projections in `M(C(X) ⊗ K)` with
nonzero fibres are full (Ortega--Perera--Rørdam 0903.2917v4,
Propositions 4.5 and 4.8). So commutative witness ideals `K(E)`, and
commutative detectors of `J~` with compact defect, need a counterexample
to the open claim `properly-infinite-nonzero-fibre-projection-is-full`
over a base such as `∏ S²`.
