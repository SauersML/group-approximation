...
operator-MF realized (for example residually finite).  More precisely,

```text
A *_phi is regularly operator-MF realized
  iff A embeds in a regularly operator-MF realized D in which phi is inner. (IE2)
```

The regular-realization qualifier in `(IE2)` is essential to the argument.
An embedding of `D` into a norm-matrix corona with no regular-character trace
does not supply the group-level faithfulness step for the central HNN
extension, and no bare-operator-MF permanence assertion is made here.
...
For `(IE2)`, if `A *_phi` is regularly operator-MF realized take `D =
A *_phi` and `u = t`.  Conversely, if `D` is regularly operator-MF realized,
[[regular-mf-central-hnn-closure]] makes the central HNN extension in `(IE1)`
regularly operator-MF realized, and restriction to its subgroup `A *_phi`
retains both injectivity and the regular character.  If `D` is only known to
embed in the unitary group of an MF C-star algebra, Shulman's central-HNN
theorem (Theorem 25 of arXiv:2603.13564) gives MF of the universal C-star HNN
algebra, but the group-level conclusion still needs the trace argument of the
regular realization; the residually finite case is where both are available
at once.
