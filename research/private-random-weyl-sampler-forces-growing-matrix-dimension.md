...
There are universal constants `C,eta>0` such that, for every sufficiently
large `r`, one can choose
...
For every `d<=r` and every two exact additive unitary representations
...
Hence sampled energy below `eta` forces `d>r` on the same Hilbert space.
