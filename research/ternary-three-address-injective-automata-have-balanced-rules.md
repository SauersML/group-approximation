...
- **Several patches** (same artifact, Section 3; open claim
  ...
  - **Seed decided on paper, awaiting verification** (w5-two-patch, open claim
    `ternary-two-patch-seed-rule-is-never-injective`; verification requested from w3-vf-nonlinear).
    - **Why.** `{0, 1}` is blind in all three coordinates. One- or two-site collisions cover every
      nonamenable placement, and on amenable placements the Garden of Eden theorem forces balance. So the
      candidate proof shows the seed is not injective over any group.
