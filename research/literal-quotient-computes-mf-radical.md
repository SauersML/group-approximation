...
requires: [literal-mark-quotient-mf]
artifacts: [non_mf_groups_exist.tex]
---

## Why sufficient

The proved non-MF theorem puts `w` in every norm-matrix-corona kernel, so the
central subgroup `<w> = {1,w}` lies in `Rad_MF(E)`.  If `E/<w>` is MF, compose
