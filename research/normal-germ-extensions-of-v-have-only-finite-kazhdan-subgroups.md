...
kind: claim
title: In a pointwise variant of the germ regime of Belk--Hyde--Matucci Corollary 2.10, finite germ extensions of V have no infinite Kazhdan subgroups
distinct_from:
  ...
  almost-v-fw-subgroups-act-through-finitely-many-germs: that reduces a Kazhdan subgroup of any finite germ extension of V to finitely many germ groups; this shows that in this regime those germ groups have no infinite Kazhdan subgroups, so nothing is left.
...
landed nodes; gq-referee-b PASS, research/artifacts/gq-referee-b-kazhdan-almost-v-central-and-normal-germ-extensions.md, f6b4126d9, fixes W1–W2 applied; no novelty claimed beyond the generalization of
...
This is a pointwise variant of the germ hypothesis of Corollary 2.10 there, as in Theorem B of
`normal-germ-extensions-of-thompson-v-contain-no-sl3z`, and not a restriction of it.
Corollary 2.10(3) is a global either/or, and its normal alternative also asks that
`(G)_p/(V)_p` have type `F_n`. Here the alternative may vary with `p`, there is no `F_n`
clause, and (b) is required only at eventually periodic points. So hosts in Corollary 2.10
whose normal alternative holds at an aperiodic singular point are outside this theorem.
...
- `Aut(F_n)` for `n >= 5`, which has property (T) (Kaluba--Nowak--Ozawa, arXiv:1712.07167, for
  `n = 5`; Kaluba--Kielak--Nowak, arXiv:1812.03456, for `n >= 6`; pinned from the abstracts by
  gq-referee-b), and `Out(F_n)` for `n >= 5`, which has property (T) as a quotient of `Aut(F_n)`. So this regime cannot
...
- every infinite simple Kazhdan group. By `boone-higman-iff-simple-kazhdan-decidable-inputs`,
  ...
  such input, leave this regime at some singular point. That means an
