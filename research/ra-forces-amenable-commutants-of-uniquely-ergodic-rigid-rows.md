...
kind: claim
title: A non-amenable finitely generated group of automorphisms of a minimal free quantum-rigid Z^d-SFT, meeting the shifts trivially and fixing an invariant measure, refutes (RA); so (RA) forces uniquely ergodic rigid rows to have amenable commutants and no free subgroups, the SU(2) target T1 decides (RA) rather than calibrating it, and the infinite-fibre time-lift line and the finite-fibre Toeplitz line are two measured instances of one refutation principle
requires:
...
distinct_from:
  time-lifts-reduce-ra-prime-to-automorphism-groups-of-rigid-rows: that shows a non-EXACT commutant refutes (RA′); this shows a non-AMENABLE commutant that fixes an invariant measure already refutes the stronger (RA), and derives the amenability consequence for uniquely ergodic rows.
  rigid-subshifts-over-free-factors-are-tight-almost-everywhere: that shows invariant measures over Λ_0 × F_n force tightness and leaves tight spacetimes open; this identifies the time lifts of rigid rows as tight spacetimes and reduces the measured (RA) for them to amenability of commutants.
...
- **Open:** the T1 construction itself is **not built**. This node shows T1 is decisive for (RA), and lists
...
- (RA) and (RA′) are as in `quantum-rigid-minimal-topfree-subshift-actions-are-amenable`.
...
1. **Measured refutation.** Suppose `G` is non-amenable and fixes a `Z^d`-invariant Borel probability
   ...
   invariant probability measure. So its action is not topologically amenable, and (RA) fails.
2. **What (RA) predicts for rigid rows.** Assume (RA). Then:
   ...
   (`quantum-rigid-subshifts-are-determined-by-thick-collars`).
3. **T1 decides (RA).** Let `Y` be as in target T1 of
   ...
   - so T1 refutes (RA) by 1.

   T1 is therefore not a calibration: it decides the strong form (RA), and T2 decides (RA′).
4. **Time lifts are the tight regime.** `X_G` is determined by its restriction to the slab
   `{1} × Z^d`. So the measured (RA) question over `G × Z^d`, restricted to time lifts, is exactly item 2.
...
| what it refutes | (RA′) directly (`G` non-exact) | (RA) via T1 (`SU(2)`); (RA′) via T2 (`∏ SU(n_i)` ⊇ Osajda's `G`) |
...
an invariant probability measure refutes (RA). Both lines are searches for such an action.
...
**Invariant measures turn automorphism groups into (RA)-tests.**
- A rigid, uniquely ergodic row with a non-amenable commutant is, through its time lift, a rigid minimal
  free action of a non-amenable group with an invariant measure, and (RA) forbids exactly that.
- So (RA) contains a clean statement about commutants: rigid, uniquely ergodic rows have amenable
