...
ESTABLISHED. Let `G` be a countable group with property (T) and no nontrivial homomorphism to a
finite group, and `(X, mu)` a Bernoulli action of `G` with nontrivial base. Let
`pi: (Z, m) -> (X, mu)` be an extension with `(Z, m)` ergodic, whose conditional measures `m_x` are
finitely supported for almost every `x`. Then `pi` is an isomorphism mod null sets.
...
* It applies to every finitely generated infinite simple Kazhdan group, including `R^x` and every
  host of `countable-group-embeds-in-fg-simple-kazhdan-group`.
