...
`balanced-bernoulli-projection-transfer-proof`, identifies its exact
...
unitary restriction excludes thin-corner witnesses but leaves no
...
`ell^2(G/Lambda)`, hence is outside the FD-tempered sector.  Writing
`W=(direct_sum_(G/Lambda) C_2) semidirect G`, this crossed product is
exactly `L(W)`.  Thus hyperlinearity of `W` would produce a literal
matrix-ultraproduct counterexample to FDT7 and (CCR).  No such generalized
coset-wreath model has been constructed from the available actor microstates,
so the refuter remains conditional.  Unconditionally it rules out every
proof using only abstract finite-tracial, property-(T), Jones-index, polar,
or Powers--Størmer input.
