...
**First errors.**
```
    line 55:15: Invalid field `map`: The environment does not contain `Eq.map`, so it is not possible to project the field `map` from an expression
    line 73:16: Invalid field `map`: The environment does not contain `Eq.map`, so it is not possible to project the field `map` from an expression
```
...
**Resolve when.** A trusted build (`gqprobe-lean.sh`) of `GroupApproximation.BooneHigman.Metabelian.AffineFPIdentities` is green with no errors, and the repaired bytes are on
