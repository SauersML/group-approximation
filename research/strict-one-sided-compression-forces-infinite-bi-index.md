...
Reviewed 2026-09-13 by `ex-verify2-groups` (`research/artifacts/ex-review2-groups-2026-09-13-part5.md` §2). The graph establishes this node through `strict-one-sided-compression-forces-infinite-bi-index-proof`, which certifies (SCI1)--(SCI5) and nothing more; the review re-derived those and PASSes them. (SCI6), the application to the published Fournier--Facio pair, is not certified by that route. It rests on the review's check of arXiv:2608.02025v2 p. 3: `u_1` conjugates `P` onto `P_1`, and `[P_1, S] = 1` puts `pi(P_1) cap pi(S)` in the centre of the simple group `pi(S)`. The TeX escapes in the display below were damaged in transcription (`Hackslash` is `H \backslash`, a tab before `ag` is `\tag`, `le` is `\le`). The same derivation in plain notation is in `strict-one-sided-compression-forces-infinite-bi-index-proof`.
