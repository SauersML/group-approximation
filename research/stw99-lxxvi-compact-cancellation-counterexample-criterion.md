...
Condition 1 is only a first filter.  By
`stw99-lxxvi-ideal-local-k0-protection`, condition 2 forces `[p]=[q]` already
in the `K_0` group of the ideal generated by `p` and `q`.  Thus equality only
after mapping into `K_0(A)`, for example equality caused by an index boundary
map in an extension, can never produce the required collapse.
