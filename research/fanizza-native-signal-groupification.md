...
kind: claim
title: Groupify the Fanizza family while retaining its native signal involution and a computable decoder modulus
artifacts:
...
1. Every BCS representation extends, after amplification if necessary, to an
   exact representation of `Gamma_m` whose restriction to the original
   involutions is unchanged.  In particular the membership GNS model in
   `(NSM1)` makes `d_m` nonidentity.
