...
distinct_from:
  odd-congruence-lambda-exact-sector-collapses: that closes the Lambda-exact sector whose congruence levels are coprime to p, by the exact slot for h and the Weyl relations; this bounds the weight of the complementary p-divisible levels, for which no exact slot exists, in terms of depth times defect.
...
`lambda-exact-p-divisible-ultra-deep-proof`).**  Let `p` be any prime,
...
`12 D epsilon /(c_p kappa_0)` on constituents of positive `p`-level.  So
...
depth is at least `c_p kappa_0 (1 - nu_0) / (12 epsilon)`: an **ultra-deep
tower**, deeper than the inverse defect.  The same count at depth one gives
...
levels, and `(UD2)` says those levels lie beyond `1/epsilon`.  No
...
intertwiner `T` with `||H - T||_2 <= 3 epsilon/kappa_0`, and `rho'(J) T^k`
is then an exact intertwiner between `rho'|_(C^((k)))` and its
`Ad(g_k)`-twist, within `3 k epsilon/kappa_0` of a unitary; hence the
isotypic weight measures of the two are within `12 k epsilon/kappa_0` in
total variation.  A constituent of exact `p`-level `a >= 1` has, by Clifford
