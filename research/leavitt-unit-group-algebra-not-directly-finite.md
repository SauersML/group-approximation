...
The support reduction restricts possible exterior kernels to asymmetric
...
period-four kernel for z=omega^2. For z=omega, a subsequent
endpoint and full-target proof excludes that asymmetric class too.

For the omega choice, constant-core components are eliminated.
On mixed periodic cores the outside equations reduce to a scalar
tensor on zero-run endpoints. An adjoint-idempotence argument
removes the bridges and forces this tensor to be symmetric.
