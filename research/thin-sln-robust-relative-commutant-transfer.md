...
distinct_from:
  ...
  thin-selected-module-is-profinite-continuous: that is a strictly smaller selected-module continuity assertion which, together with the already proved profinite tau gap, implies this full quantitative transfer modulus.
---
...
## Smallest isolated surviving modulus

`thin-profinite-selected-gap-lemma` shows that one does not need to correct
the approximate representation or make its full adjoint complement
tempered.  It is enough that the single cyclic module generated by
`(1-P_A)v` be weakly contained in the direct sum of the non-invariant parts
of all finite-dimensional representations of `A`.  This is the open claim
`thin-selected-module-is-profinite-continuous`; the route
`thin-selected-profinite-continuity-gives-transfer` derives `(TRC1)` from it.

The reduction is sharp on the canonical HNN enemy.  Britton normal form
makes the stable-letter conjugation orbit the quasiregular `A/C` frame.  Its
base vector is `C`-fixed and has no `A`-fixed component, whereas the
finite-dimensional norm has a uniform `C`-gap.  Hence that selected module
