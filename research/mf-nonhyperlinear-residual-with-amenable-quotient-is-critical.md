...
**Spectral constraint (2026-09-13).** Every corona map of a critical `R` has
a hyperlinear tracial image, so `R` equals its operator-to-HS shadow
residual. By `hs-invisible-corona-maps-are-full-trace-paddings`, every corona
map of `R` is a full-trace trivial summand plus a trace-null remainder,
iterated inside every remainder, and `R` has no nontrivial
finite-dimensional unitary representation. By part 4 of
`gapped-normal-generators-give-a-hyperlinear-quotient`, if `R` is finitely
generated, then in every nontrivial corona map every finite normal
generating set has a generator whose image has spectrum accumulating at `1`.
So a finitely generated critical `R` is not normally generated by finitely
many elements of finite order.
