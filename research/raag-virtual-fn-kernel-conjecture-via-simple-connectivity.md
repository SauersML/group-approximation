...
- `n ≥ 2`: the kernel is finitely presented and of type `FP_n(Z)`. The second requirement makes
  ...
  `L` is `(n−1)`-connected. By `bestvina-brady-kernel-finiteness-theorem`, `A_L → Z`, `v ↦ 1`, has
  kernel of type `F_n`.

This route fires only when all three requirements are established. A second route with the
same target, `raag-virtual-fn-kernel-conjecture-via-sigma-invariants`, goes directly through
nonempty `Σ^n`.
