...
- **The escaping witness must occupy positive-mass supercritical range
  blocks.**  `finite-range-block-mass-upgrades-diverging-positivity-to-ucp`
  applies the codomain Smith lemma to the concrete algebra generated by a
  diverging-order positive lift.  Every irreducible range block of degree at
  most `k_n` is already completely positive.  Blocks above `k_n` can be
  overwritten by a state at normalized-HS cost at most twice the square root
  of their total trace mass.  Therefore liftability follows if that mass
  vanishes.  This criterion is invariant under common amplification and
  defeats the vanishing-corner reduction-map obstruction.  The Maslov seam
  is now the arithmetic claim
  `maslov-diverging-positive-lifts-have-vanishing-high-degree-mass`: Kassel's
  root/Weyl relations must prevent irreducible degree greater than the
  available positivity order on a positive fraction of coordinate trace.
