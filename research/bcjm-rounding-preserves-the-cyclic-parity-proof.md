...
Call a point of its permutation basis good if:

1. every multiplication row
   `beta_n(a)beta_n(b)x=beta_n(ab)x`, for `a,b in B_0`, holds on the
   entire `B_0`-orbit generated by that point; and
2. the orbit map `a |-> beta_n(a)x` is injective.
...
The good points therefore form `1-o(1)` of the basis and split into free
four-point `B_0`-orbits on which the table is already exact. Keep that
...
literal half-set projections and preserves `(BPR6)--(BPR9)`.
