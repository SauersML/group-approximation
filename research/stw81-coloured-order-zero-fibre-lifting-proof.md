...
Fix a finite self-adjoint set `F subset A`, put

```text
M=max(1,max_(a in F)||a||),
```

and fix `epsilon>0`.  Choose `h in C_c(X)_+`, `0<=h<=1`, such that
...
At `x`, the first element has norm at most one and the second has norm less
than `delta`.  Upper semicontinuity therefore gives a neighborhood `U_x`
on which
...
s=(1+eta)^(-1),
Phi^(c,l)((z_i)_i)
 =s sum_(i:c(i)=c) g_i Phi_(x_i)^(l)(z_i).              (7)
```
...
The sum `Phi` of all `(n+1)(d+1)` colours is c.p.c.  Indeed, on the support
of `g_i`, equation `(5)` gives `||H_(x_i)||<1+eta`, so positivity and `(6)`
give fibrewise

```text
||Phi(1)_y||
 <=s sum_i g_i(y)||H_(x_i)(y)||
 <=s(1+eta)h(y)<=1.                                    (8)
```

For every `y`, the same partition-of-unity calculation gives
...
Choose `delta,eta` so that the last expression is below `2epsilon/3` and
combine `(1)` and `(9)`.  This proves the upper bound in `(FBP1)`.  The
lower bound follows because every evaluation `A->A_x` is a quotient.
