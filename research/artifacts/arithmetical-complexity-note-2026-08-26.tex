...
\end{proof}
Attribution: the countably generated case of (b) is Theorem~3.1 of Bilanovic--Chubb--Roven, \emph{Detecting properties from descriptions of groups}, Arch.\ Math.\ Logic 59 (2019) 293--312, who prove every Markov property $\Pizt$-hard on recursively presented groups by the same construction, and who also prove $\Sith$-completeness of finiteness, decidable word problem, cyclic, nilpotent, solvable and finite presentability on recursively presented groups; the two-generator syntax, the fixed ambient $V$, the finite-presentation cells and all upper bounds for approximation properties are new here. Kernel-checked half: \path{HereditaryPropertySwitchCompleteness.enumeratedCodeProperty\_pi02Hard} proves (b) on enumerated presentations for every hereditary $P$ with a finitely presented forbidden seed.
...
Torsion-freeness is $\Pizt$-complete on recursive presentations and, through the gate of the previous section, on finite presentations; perfectness and every bounded nilpotency class or derived length are $\Pizt$-complete (seeds $\Z$ and $F_2$); solvability of the word problem is $\Sith$-complete; and inside Higman's universal group the three levels one, two and three are realized by property (T), amenability and residual finiteness. Thus each of the first three arithmetical levels is realized by a natural, classically studied property; the fourth is realized as well, by ``residually a finite $p$-group for some prime $p$'' --- see \S12, where the index sets whose pro-$p$ closedness is controlled independently for each prime are constructed.
...
\item \path{TrivialEnumeratedPi02.trivial_enum_pi02Complete}, \path{AbelianEnumeratedPi02.comm_enum_pi02Complete}, \path{PerfectEnumeratedPi02.perfect_enum_pi02Complete}, \path{TorsionFreeEnumeratedPi02.torsionFree_enum_pi02Complete}: triviality, commutativity, perfectness and torsion-freeness as exact $\Pizt$-completeness on enumerated presentation codes (perfectness through commutator-product words; torsion-freeness by carrying a certificate for the power's triviality, which keeps the matrix a single existential over a decidable check).
