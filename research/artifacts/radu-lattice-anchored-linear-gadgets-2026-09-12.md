...
- Theorem 3.1 does not use Remark 3.6. Without it, the only unconditional length information is
  `delta^4 in Gamma_0` (Lemma 3.5), and no radius bound follows by hand.
...
entry pattern is coincidence modulo `Gamma_0`, and it is blind from `r + s = 4` on.
...
- Whether any invariant reaches `r + s >= 4`. The defect must separate the cosets of `delta^4`; no
  construction uses this, and no obstruction beyond it is known.
