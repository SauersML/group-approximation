# bh-free-35 certificate-search driver (2026-09-18)

This is the exact driver of MSI job 1326491 (run 2), md5 `5e2efa4e673a13a032b6e0dfd74d512d`. It ran under
`module load sagemath/10.7` (Singular letterplace via `sage -singular`) in one `msismall` job with `-c 1`.
Results are in `research/artifacts/gq-bh-free-35-certificate-search-2026-09-18.md`.

Run 1 (job 1325784, md5 `18b35d2dade673d94fbf6e95177fe561`) differs only in two respects:
- it had no `use_ext` switch, so every case used the extendability relations;
- its `main()` cases were:

```
    V6 = [(2, 1), (1, 2), (2, -1), (1, -2), (3, 0), (0, 3)]
    case(led, V6, 2, 4, 60, 'led')
    case(ledk, [(2, 1), (3, 0)], 2, 4, 60, 'ledk')
    case(full, [(2, 1), (3, 0)], 0, 4, 40, 'full')
    case(colp, [(2, 1), (3, 0)], 0, 5, 60, 'colp')
    case(labbe, [(2, 1), (1, 2), (2, -1), (1, -2)], 0, 4, 75, 'labbe')
```

The batch script sets `DOT_SAGE` and `TMPDIR` under `/projects/standard` and runs
`timeout 575 python3 driver.py <outdir>`.
