...
2. **Multi-bump elements.** Is `(1 + lambda g) R` Ore against everything when `g` has two
   ...
   - *Partial.* For `g` whose bumps lie in `F`,
     `thompson-f-multibump-binomials-meet-stabilizer-ideals` proves property O against `b`
     in `K[H_i × Z_i]`, where `H_i` fixes the closed supports of the other
