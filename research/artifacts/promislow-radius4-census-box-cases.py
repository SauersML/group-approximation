...
and the case split on the enlarged pairs.
  ...
  cases k sel TL  case split of census.py over QQ on (S, T') with T' = adj(S),
                  ...
                  optional: only=i,j;i,j (run only these cases), from=i (skip i < from)
"""
...
if mode == "bounds":
    ...
    for arg in sys.argv[5:]:
        if arg.startswith("only="):
            only = {tuple(map(int, c.split(","))) for c in arg[5:].split(";")}
        elif arg.startswith("from="):
            ifrom = int(arg[5:])
        else:
            raise SystemExit(f"bad argument {arg}")
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if i < ifrom or (only is not None and (i, j) not in only):
                continue
    ...
    stamp(f"orbit {k} {sel} (only {sorted(only) if only else None}, from {ifrom}): nonempty cases {nonempty}; timeouts {timeouts}; slowest {worst}")
