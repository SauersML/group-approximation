# Proof attempt for hs-commutative-microstate-bernoulli-entropy-is-base-blind (held OPEN)

This was the direct-proof route `hs-commutative-microstate-amplification-proof` (requires: []) into
`hs-commutative-microstate-bernoulli-entropy-is-base-blind`. It was withdrawn from the graph on 2026-09-17
because referee lens 1 refuted Step 3: with the letter pair `a0 != a1` arbitrary and `|F|>=2`, the bound
`s^2 >= 2/q-2delta` from "the F={e} moments" does not follow, and it is false in an explicit example. Lens 2
returned *survives* but flagged the same step. Steps 2, 4, 5 and the scaling in 6 were checked by both. See
`## Attempts` in the claim. Restore it as a route once Step 3 is repaired (swap the two heaviest letters) and the
scope of Step 7 is corrected, and a full referee pass survives.

Original title: Rotate a swapped block microstate by the amplification commutant and pack the operator-norm ball.
