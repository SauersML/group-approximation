...
* **Live questions (current).**
  * **Strict deficit, both characteristics.**
    * **What is established.** The defect gap and two-sided descent hold for abstract Sylvester rank
      ...
      every Sylvester rank function as well, with no regularity (w7-sylv-global; verifier PASS
      w4-vf-linear-b §39 at ae0a4005ef; L9).
...
* **Live question.** Open node `leavitt-disjoint-cylinder-defects-strictly-submultiplicative`, with route
  ...
  * **Graph-of-groups firewall and the mismatched pair** (lane w6-mismatch-c2, family SUB; d0f2b3f648;
    artifact `mismatched-factor-graph-of-groups-firewall-2026-09-12.md`; established, verifier PASS §29 at
    4373d18c44, covering Theorem 1.1, the pair and the Steinberg residue remark).
...
* **What changed.** The first version of this table said that the gap, descent and positivity mechanisms
  ...
  **Later still** (lane w7-sylv-global; artifact
  `sylvester-exact-globality-and-kernel-compression-2026-09-12.md`; 696ae35bc7, 0a9c95f793; verifier PASS
  on all four claims, w4-vf-linear-b §39 at ae0a4005ef, recorded on the nodes at 98d80a42cc).
  ...
  * **Exact globality, constant `1`.** `sylvester-leavitt-kazhdan-fixed-ideals-are-global`. The loss `1/m`
    ...
    is used.
    * *Correction* (w4-vf-linear-b §39, artifact Remarks 2.4). Normal generation alone gives the `1/m`
      bound only at `Z = 1`. Over `F_2`, some `Z` acts on the `A_5` sum-zero module as a projection onto
      `Fix(A_4)`, so `psi(Z) = 1/4` and `psi(X_S Z) = 0`. So the `1/m` bound for general `Z` also needs the
      compression hypothesis, which `R^x` has. No node stated the wrong sentence.
  ...
  * **Route.** `sylvester-rank-triviality-via-strict-defect-submultiplicativity` targets
    ...
    `sylvester-rank-functions-on-leavitt-units-are-regular` is off this route. The route is not in the
    verifier's queue. §39.5 checks its arithmetic, `(1 - theta) delta^2 <= 2 eps delta` with `delta >= c_0`,
    and adds no verdict line. The lead spot-check agrees: the contradiction holds once
    `eps < (1 - theta) c_0 / 2`. The subsumption scan of §39.5 finds nothing subsumed or refuted.
...
* **Characteristic three, later landings** (another family; verified in `gk-vf-gate-verification-2026-09-12.md`).
  ...
  * **Extension claim, reduced** (lane w6-v-extend; 0144b771c7; artifact
    ...
    * **Reduction.** Link 1, the minus-one gate, says no rank function on `F_3[G_3]` moves `z`. The V gate
      ...
      when link 1 holds and `F_3[V]` has a detecting rank function.
    * **No Hamming extension.** `G_3` and `PG_3` are nonsofic, and the normal subgroups of `G_3` are `1`,
      `<z>` and `G_3`. So every homomorphism from `G_3` into a metric ultraproduct of symmetric groups is
      trivial, and an extension has to give the transvection `x` linear data rather than a permutation
      profile.
    * **No averaging.** `G_3` has property (T) and `[G_3 : V]` is infinite, so `V` is not co-amenable.
    * **Lifts of the transvection** to `F_3[E x| V]`.
  ...
  * **Depth-monotone firewall** (lane w6-mismatch-c3, family SUB; 74247b5cff, a5226d4756; artifact
    ...
    * `depth-monotone-configurations-cannot-force-ternary-minus-one` (artifact Sections 3--4).
      * **Statement.** Let `Gamma_0 <= G = L_(F_3)(1,2)^x` contain `z = -1`. Suppose some conjugate of
        `Gamma_0` lies in `H^omega_<=` or `H^omega_>=`. Then `eps_- F_3[Gamma_0]` has a nonzero weakly
        finite image, so no finite certificate for `ternary-anti-central-summand-has-no-weakly-finite-image`
        has all its group elements in `Gamma_0`. Every such `Gamma_0` is sofic, so the content is the
        placement.
      * **Covered** (`a = b`): `z`, `E`, level `V`, all degree-zero units, every `T_(sigma,kappa)(lambda)`
        with `|sigma| <= |kappa|`, and the mismatched pair with its product of infinite order. So the
        relations in `W` give neither a one-sided pair nor a contradiction with `z = -1`.
      * **Not covered.**
        * The models send the two-root defect to `0` when both roots lie inside. So the defect form
          `ternary-weakly-finite-representations-kill-two-root-defect` is not firewalled.
        * By Kraft's inequality, `V ∩ R^omega_<=` is the part of `V` that preserves the Bernoulli measure
          `mu_omega`. So a non-level element such as `x_0` lies in no weighted unit group, and neither does
      ...
      * **Open** (artifact Section 4).
        ...
        * Is there a conjugation invariant of units that detects depth vectors?
        * Does `H^1_<=` carry a weakly finite representation with `z -> -1` and `D != 0`?
      * The lane adds an Attempts entry to `ternary-leavitt-units-have-no-weakly-finite-representation`
        (OPEN).
    * *(lead)* The two mismatch firewalls complement each other. The graph-of-groups model (L5,
      characteristic two) keeps `D != 0` but realizes only vertex-group relations. The depth-monotone model
      realizes every relation of an actual subgroup containing the pair but sends `D` to `0`. So a
      certificate for the anti-central form must generate a subgroup that no conjugation puts into a
      weighted unit group. For the defect form it must use relations that no graph of locally finite
      subgroups routes. The last open question above asks for one model that has both properties.
