...
1. The Titz Mite--Witzel finite residuals are finitely presented,
   ...
   soficity and MF status are unknown.  Building rigidity computes their
   outer automorphism group orders divide `2,32,32,64,64`, so every cyclic
   mapping torus is virtually a direct product.  These kernels solve the target if and
   only if one of them is both sofic and non-MF; no extrinsic action can repair
   an MF instance.
