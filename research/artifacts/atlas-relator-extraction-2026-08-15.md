...
## Source pass 2026-08-15 (late): the presentation-completeness import, pinned

Fetched Ershov–Jaikin-Zapirain, arXiv:0809.4095v2 (the recorded source of
the Krstić–McCool quote) and read pages 33–35, 44–47 directly.

**The Steinberg presentation, verbatim (p. 35, Definition).**  "Let
`n ≥ 3`.  The Steinberg group `St_n(R)` is the group generated by the
symbols `{E_ij(r) : 1 ≤ i ≠ j ≤ n, r ∈ R}` subject to the following
relations: (St1) `E_ij(r)E_ij(s) = E_ij(r+s)`; (St2)
`[E_ij(r), E_kl(s)] = 1` if `i ≠ l, k ≠ j`; (St3)
`[E_ij(r), E_jk(s)] = E_ik(rs)` if `i ≠ k`."  With generating set
`Σ^st = {E_ij(x_m)}` over ring generators `x_0 = 1, x_1, …, x_d` (6.3).
Theorem 6.2 (p. 35, verbatim): "The Steinberg group `St_n(R)`, `n ≥ 3`,
has property (T)" with the explicit Kazhdan constant
`κ ≥ 1/(8(12√(2d) + 2√(3n) + 36√2))`.

**The Krstić–McCool citation, verbatim (p. 35).**  "By a theorem of
Krstić and McCool [KrM, Theorem 3], the Steinberg group `St_n(R)` is
finitely presented for any `n ≥ 4` and any finitely presented ring `R`
[…].  By [KrM, Corollary 2], the group `St_3(R)` is not finitely
presented whenever `R` surjects onto `F[t]` for some field `F`."

**Bibliography entry, verbatim (p. 47).**  "[KrM] S. Krstić, J. McCool,
*Presenting GL_n(k⟨T⟩)*.  J. Pure Appl. Algebra 141 (1999), no. 2,
175–183."

**What this does and does not settle.**  Finite presentability of
`St₅(L)` is now pinned to a named refereed theorem (rank five is safely
above both the `n ≥ 4` threshold and the sharp `n = 3` failure).  What
remains open for the usable-list program is whether the *specific*
emitted family — the Steinberg relations (St1)–(St3) instantiated on the
ring generators together with the lifted Cuntz–Krieger ring relations —
is itself a complete presentation, i.e. whether it coincides with or
derives the presentation scheme of [KrM, Theorem 3].  That requires
reading the KrM paper itself (JPAA, not on arXiv), which is the next
source-pass target; until then the completeness input stays a named,
pinned, unverified import.
