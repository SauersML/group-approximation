...
## First look at the increment/decrement loop (not settled)

- **The machine `M_2`.**
  - `s_1` is type I: `m ↦ m+1`, then go to `s_2`.
  - `s_2` is type IX: if `m = 0` go to `s_†`, else `m ↦ m−1` and go to `s_1`.
  - From `(s_1, 0, 0)` it never halts.
  - Letters: `0, 1, †₁, †₂, a` (the type I letter) and `b₁, b₂` (the type IX letters).
- **The counter `x`.**
  - It passes `0` with section `x`.
  - `x|_a = x²`: it doubles.
  - At `b₁` it goes to `b₂` with section `s_1^(-1) s_† x`; at `b₂` it goes to `b₁` with section
    `x^(-1) s_†^(-1) s_1 x`.
  - So `x²|_(b₁) = x`: it halves.
  - On `{b₁,b₂}`-words it is the odometer up to twists by state generators, and those twists act on the tail.
- **The second counter `y`** is letter-preserving in a one-counter machine, so it is trivial, and `y ∈ R`.
- **Why the survival induction fails.** `x^k|_a = x^(2k)`, so sections of relators along `a`-paths are
  "doubled" relators, which are longer.
- **What might replace it.** This is BS(1,2)-type self-similarity: doubling at `a`, halving at `b₁`. The
  natural attack is a two-sided induction on the 2-adic valuation of `x`-exponents, to show that the doubled and
  halved relator families are generated by a finite core. Not done.
