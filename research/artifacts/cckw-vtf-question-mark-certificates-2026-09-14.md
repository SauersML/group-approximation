...
|---|---|---|---|---|
| (3,3,4) | `G_0^{14,14,48}` | 3 | `[1,1,2]` | pending |
| (3,3,4) | `G_1^{14,14,48}` | 2 | `[1,1,3]` | `q=9`, `[1,1,3]` |
| (3,3,4) | `G_5^{14,14,48}` | 2 | `[1,1,2]` | `q=2`, `[1,1,24]` |
...
| (3,3,4) | `G_0^{18,18,48}` | running | — | pending |
...
| (3,3,4) | `G_5^{26,26,48}` | 4 | `[1,1,2]` | pending |
...
| (4,4,4) | `G_0^{48,48,48}` | incomplete | — | not run |
...
- **Groups with small central kernels** (`K` containing only 2 or 3) are the next candidates for cover
  lifts (`lift_vtf.g`, running at this version).
- **Untested targets.** GAP cannot construct `2F4(2)'` by `SimpleGroup`, and two `GQuotients` runs hit the
  12 GB workspace limit. Those rows are marked incomplete; reruns with 44 GB are queued.
