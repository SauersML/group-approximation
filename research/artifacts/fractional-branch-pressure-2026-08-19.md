...
| ZPC: bounded-degree context-collision code / PCP | already present: `zpc-context-overlap-mismatch-transducer` states the `S_c` selector-overlap observable and its Attempts already names the bounded-degree spanning-tree consistency PCP.  Nothing authored. |
...
- Any ZPC node.  `zpc-context-overlap-mismatch-transducer` and
