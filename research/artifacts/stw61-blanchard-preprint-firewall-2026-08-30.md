...
contains an unsupported unstable-homotopy passage.
...
That conclusion does not follow from injectivity of the C\*-homomorphisms.
...
inductive limit can only be approximated at a later stage, so the missing
input is injectivity of the induced maps on the relevant *unstable unitary
homotopy classes*, not injectivity of the algebra maps.
...
No subsequent step repairs this.  Proposition 2(3) reduces the claimed
...
preprint is useful only as a false-positive firewall until its missing
unstable-homotopy injection is independently supplied.
