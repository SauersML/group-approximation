...
lengths (1,1,1,1,8) are tested.  Seven immutable Leavitt packet coefficients
are injected into twelve slots; the other five slots are identities.  A hit
must pass the complete ordered post-pivot test in faithful Bergman normal
form: the second unary relator vanishes and the quotient of the two residual
relators is conjugate to the marked target.
"""
...
def orbit_word(slots, signs, start):
    ...
    for name, sign in zip(slots, signs):
        ...
        if name != "1":
            word.append(("g", residue, UNITS[name]))
        append_z(word, -quotient)
        height += sign
    append_z(word, 1)
    expanded = []
...
for topology_index, topology in enumerate(TOPOLOGIES):
    ...
    for layout_index, positions in enumerate(permutations(range(12), 7)):
        if (layout_index + topology_index) % args.shards != args.shard:
            continue
        slots = ["1"] * 12
        for position, name in zip(positions, PACKET):
            slots[position] = name
