...
*Update 2026-09-12.* For `F_n x Z` this is superseded whenever some letter's
height set is symmetric up to translation, which includes the example above.
Then `Aut(X)` has a closed quasi-transitive nonunimodular subgroup, and
Hutchcroft's theorem applies:
`fpbs-tree-projected-reversible-letter-nonunimodular`. The open subcase is
height sets none of which is symmetric up to translation.
