...
This does not yet split their minimal projections.  The exact new frontier is
projection return: choose the finite-dimensional models so that one minimal
projection in each matrix block is uniformly close to a positive element of
`phi(A)`.  Functional calculus then turns the nearby image element into an
actual image projection, close projections are unitarily equivalent, and the
AF matrix-unit transport proof applies.  The evaluation map
