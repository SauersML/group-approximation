...
2026-09-07. This is a proved restriction on the Jacobson two-braid
problem. It rules out all mixtures of the two three-dimensional head
types, including the new dimension-twelve mixed certificate. It does
...
The exact representation and the restriction
`rho|_K=sign directSum standard` are supplied by the
[cyclotomic mixed-packet certificate](jacobson-mixed-packet-certificate-2026-09-07.md).
Here is an elementary way to obtain the specific angle needed below,
without evaluating its cyclotomic matrices numerically.
...
The dimension-twelve certificate uses `rho_+` on the head and
`sigma=1 directSum rho_+` on the tail. Its two defects are strictly below
two, but the stronger bound proves that its first defect, and the first
defect of every possible overlap-central involution on that same packet,
is at least one third. Increasing only the tail multiplicities or changing only the
...
The all-dimensions infimum in the
...
control those cases. It introduces no
