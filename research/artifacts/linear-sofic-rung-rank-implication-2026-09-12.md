...
**(c) ⇒ (b).** Let `K` be `F`-linear sofic and `lambda` realize `D` in `K`. The countable group `lambda(F(D))`
realizes `D`, and it is `F`-linear sofic as a subgroup of `K`.
1. Compose `lambda` with an embedding `lambda(F(D)) -> Q = prod_omega GL_(n_k)(F) / N_omega`. An injective
   homomorphism keeps realization, so the composite `lambda'` realizes `D` in `Q`.
...
- **The weak sofic rung gives linear control, over every field.** Suppose one `w` lies in `cl(Cons_b^F(R_f(D)))`
  for some width `b` ([MU] Theorem 1.1(b)).
  - *Finite fields.* For a tuple over a finite field, the finite group `rho(F(D))` puts `w(rho)` in
    `Cons_b(R_f(rho))`, and the rank length gives `C = b`.
  - *Any field.* Let `R` be the subring generated by the entries of the matrices `rho(x_a)` and their inverses. It
    is finitely generated over the prime ring, so it is Jacobson and its maximal ideals have finite residue fields.
    Choose a maximal nonzero minor `m` of `rho(w) - 1`, and a maximal ideal of `R` missing `m`.
    - The reduced tuple is invertible, since each determinant is a unit of `R`.
    - It keeps the rank of `rho(w) - 1`, and relator ranks only drop.
    - The finite-field case then gives `C = b` for `rho`.
  - This is the inclusion `N_ws ⊆ N_lin` seen in matrices. w4-vf-positive-b (its Section 4.1.4) supplied the
...
- Every forward relator and reverse word of a strict datum has length at most `4`. Write `w_mon` and
  `delta_mon(eps)` for the reverse word and constants of Theorem 2.1.
  - For `eps > 4 theta` put `delta = delta_mon(eps - 4 theta) - 4 theta`, and suppose `delta > 0`.
  - If the forward relators of `rho` have rank at most `delta n`, those of `rho'` have rank at most
    `delta_mon(eps - 4 theta) n`.
  - So `rk(rho'(w_mon) - 1) <= (eps - 4 theta) n`, hence `rk(rho(w_mon) - 1) <= eps n`.
- Word ranks are invariant under simultaneous conjugation `rho -> g rho g^-1`. So the same transfer holds within
  rank `theta n` of any conjugate of a monomial tuple.

So a family of tuples that witnesses failure of the rung for `D` fails the implication at `w_mon` with some margin
`eps`. At small forward relator rank it stays at rank distance at least `theta n` from the whole conjugation orbit of
monomial tuples, generator by generator, for every `theta < eps/4` with `delta_mon(eps - 4 theta) > 4 theta`.
w4-vf-positive-b supplied the precise constants and the conjugation invariance. Unipotent tuples are the natural far-from-monomial family. For instance the
...
**Remark 3.2 (coordinate dependence).** `E` depends on the chosen coordinate algebra, while word ranks do not. For
invertible `g` and a permutation matrix `P`, the defect of `g P g^-1` computed in the basis `g e_i` vanishes. So
Proposition 3.1 does not say that some tuples are bad for evaluation in every basis. It says that no fixed coordinate
algebra is stable under small rank errors. w4-vf-positive-b pointed this out.

**Heuristic 3.3 (not a theorem).**
- **Discontinuity in a fixed basis.** The permutation `I` has `E(I) = 0`. A perturbation of normalized rank `1/n`
  gives normalized defect at least `(n - 1)/n`. So in a fixed coordinate algebra the multiplicativity defect is not
  continuous in normalized rank. An argument that runs nonlinear rules coordinatewise on a rank model with small
  rank errors has no control of the evaluation.
- **What this does not identify.** Rules are exact on generators that preserve some coordinate algebra, that is, on
  conjugates of permutation matrices in a fixed basis. This is not the structure used in Section 2: the monomial
  Hamming models there count on `F^x × {1, ..., N}`, not on the coordinates of `F^n`, and they prove the rank
  implication without evaluating the rules at all.
- **Suggested consequence.** A proof of the rung for nonlinear automata would linearize on the group side before it
  touches models, as the recorded route does through stable formalizability and
  `linear-sofic-group-algebra-is-stably-finite`. Or it would find a rank-visible invariant of the pair `(mu, nu)`
  that is not a coordinatewise evaluation.
...
- **Far from the monomial orbit.** By Remark 2.3, witnessing tuples fail the implication at `w_mon` with some margin
  `eps`. So at small forward relator rank they stay at rank distance at least `theta n` from the whole conjugation
  orbit of monomial tuples, generator by generator, for every `theta < eps/4` with
  `delta_mon(eps - 4 theta) > 4 theta`.
- **Invisible to counting (heuristic).** By Remark 3.2 and Heuristic 3.3, no fixed coordinate algebra is stable
  under small rank errors, so coordinatewise evaluation of the rules is uncontrolled on such tuples. That suggests a
  witness offers no configuration space on which Gromov–Weiss counting could act. This is a heuristic, not a theorem.
- **Linear-algebraically finite.** `F_p[K]` is stably finite. So every strict automaton on the witness group `K` is
  nonlinear, and over `p`-power alphabets none is formalizable after identity tracks.
...
- **What linear soficity adds.** A Sylvester rank function on `F_p[G]` that separates group elements. It gives stable
  finiteness, and no converse is recorded.
  - The per-datum form shows exactly what the rank function must control: ranks, in all tuples, of words of length
    at most four.
  - Heuristic 3.3 suggests no coordinatewise structure is available to help.
  - No argument on record uses the rank function past direct finiteness.
