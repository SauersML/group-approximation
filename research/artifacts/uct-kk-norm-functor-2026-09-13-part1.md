...
2. `M_i · J'_i ⊆ K(Ẽ)`, where `J'_i` is the C\*-subalgebra of `J_i`
   generated by `φ̃(A^(⊗p)) (F_i^2 − 1)`, `[F_i, φ̃(A^(⊗p))]` and
   `F_i − F_i*`, all of which lie in `J_i`, together with their adjoints.
   (Correction, 2026-09-13: an earlier version put an extra factor
   `φ̃(A^(⊗p))` in front of the commutator.  The first bullet of
   Proposition 1 needs the bare commutator.)
