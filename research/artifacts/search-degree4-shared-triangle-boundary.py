...
# The target lives in the two-involution subgroup generated by c,d.  Include
# its whole short alternating ball; this is much broader than just the target
# and its two square roots while remaining an exact, transparent search.
for length in range(2, 13):
    for first_name, first, second_name, second in (
            ("c", P.C, "d", P.D), ("d", P.D, "c", P.C)):
        name, unit = "", P.ONE
        for index in range(length):
            if index % 2:
                name += second_name
                unit = P.mul(unit, second)
            else:
                name += first_name
                unit = P.mul(unit, first)
        add(library, name, unit)
