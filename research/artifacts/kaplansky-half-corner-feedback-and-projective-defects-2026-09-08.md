...
Here every `δh` means `1+[h]`. Under the preceding `3→5` coordinate expansion, `u` carries both creation coefficients `s_0,s_1`, and `v` carries both deletion coefficients `t_0,t_1`. Thus the deletion-only or creation-only specialization does not apply to these displayed actors. Nevertheless this adjusted `Z` has an explicit kernel on a genuine group module, so no `L` can satisfy `LZ=f_0`.
