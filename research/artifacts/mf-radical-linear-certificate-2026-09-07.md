...
(Dahmani--Guirardel--Osin, Theorem 2.35, which gives both at once for
countable `G`; see the retraction in section 4, item 1), of stable rank one (Gerasimova--Osin,
...
1.  **RETRACTED 2026-09-07, after reading both sources.**  This item
    previously said: "Simplicity and unique trace are not a consequence of
    Dahmani--Guirardel--Osin Theorem 2.35 alone [...] DGO 2.35 identifies the
    amenable radical of an acylindrically hyperbolic group with its finite
    radical, and BKKO 1.4 converts a trivial amenable radical into the unique
    trace property.  Both citations are needed."  **Both clauses are false**,
    and the manuscript's single citation is correct.

    DGO Theorem 2.35 (arXiv:1111.7048v5, = their Theorem 8.14) reads: for a
    group `G` containing a non-degenerate hyperbolically embedded subgroup,
    (a) no nontrivial finite normal subgroup, (b) ICC, and (c) not inner
    amenable are equivalent, and *if in addition `G` is countable* these are
    further equivalent to (d) `C*_r(G)` simple and (e) `C*_r(G)` has a unique
    normalized trace.  So 2.35 gives simplicity AND unique trace directly, at
    exactly the hypotheses the manuscript's proof supplies.  It says nothing
    about the amenable radical.

    BKKO Theorem 1.4 (arXiv:1410.2518v3) reads: for `N <= G` normal, `G` is
    C\*-simple iff both `N` and `C_G(N)` are C\*-simple; in particular
    C\*-simplicity is closed under extension.  It is about extensions, not
    about the amenable radical or the trace.

    Gerasimova--Osin cite the pair because their sentence is about *products*
    of acylindrically hyperbolic groups: DGO 2.35 for each factor, BKKO 1.4 to
    pass to `G_1 x ... x G_k`.  The manuscript needs only `k = 1`, so BKKO is
    not part of its chain and no BKKO bibitem should be added.

    The Lean architecture built against the retracted reading survives intact:
    `SimpleUniqueTraceAtHypEmbedded` is exactly 2.35 (a) => (d) and (e) for
    countable `G`, and `DGOTheorem235Printed` was correctly named throughout.
