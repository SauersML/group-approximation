...
embedding.  Let `Theta` be all trace-preserving unital homomorphisms
`theta:F(𝒵)->D`, and let `K_𝒵^sat` be the ideal generated by every
`theta(E)`, where `E=J_τ intersect F(𝒵)`.  The canonical embedding belongs
to `Theta`, and trace preservation puts every generator in `I`, so

```text
K_𝒵 subset K_𝒵^sat subset I.
```

For every normalized quasitrace `q` on `D`, the pullback `q o theta` is the
...
an unconditional enlargement of the removable slice-generated ideal.  It
