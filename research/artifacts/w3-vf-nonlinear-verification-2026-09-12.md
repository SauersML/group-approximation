...
**Follow-up.** Fixed forward at f36897eb7 in both the artifact and `higman-group-nonsurjunctive`. The text now reads "memory lies in a conjugate of the subgroup generated by three consecutive generators". The four such subgroups `<a,b,c>`, `<b,c,d>`, `<c,d,a>`, `<d,a,b>` are the vertex groups `G_1, ..., G_4` of the two splittings, and a nonsofic `S` fixes no vertex. The added parenthetical is also right: `B(a,b)`, `B(b,c)` and `B(c,d)` together contain all four generators. Correct.
...
**Follow-up (efd28946a, dcf7872d2).**
- **Route restored.** `ternary-two-patch-seed-never-injective-proof` moves the candidate proof from the claim's Attempts into a route with its two ESTABLISHED inputs. The claim is marked ESTABLISHED by it and cites this section, which matches the verdict above.
- **Lemma 2 pair list.** It now lists `{a = b^-2, bab = a}` and disposes of it as above. `{a = b^-2, b^2 = a}` is recorded as the single relation `b^2 = a`, which makes `H = <b>` cyclic.
- **Move B summary.** The route's "abelian, cyclic or virtually cyclic, or generated by two involutions" matches the conflict list of Lemma 3.
- **Wording and pointer.** "No single flip is invisible" is applied. The distinct-symbol artifact gains a two-line forward pointer to Theorem 2, and nothing else in that lane's text changes.
- **Marker record (dcf7872d2).** The marker artifact records the Section 27 follow-up as a PASS on the corrections and on Sections 2.3–2.4, with 2.4 a necessary condition for its gate class and not a decision. Accurate.

## 29. The Brin–Thompson group 2V as a Gottschalk host (w6-free-neg-d, 4d9ce3b2d)

Artifact `research/artifacts/brin-thompson-2v-gottschalk-host-2026-09-12.md`, Sections 1–3. The landing adds the claims `brin-thompson-2v-nonsurjunctive` and `brin-thompson-2v-order-three-averaging-idempotent-is-full` (both OPEN), three routes and one `refuted_by` line on the goal. **PASS, with one scope note.** Nothing is marked ESTABLISHED, so there is no decision. A scan of main at 55673da21 finds no other node that pairs Brin–Thompson groups with surjunctivity.

**Section 1 (unit model). Correct.**
- A cross term of `u u^(-1)` is nonzero only if both word pairs are comparable, and then the cylinders `[β_k] × [δ_k]` and `[β_l] × [δ_l]` meet. The diagonal refines to `1 ⊗ 1`.
- The point map determines the element. On a common refinement, `(βx, δy) -> (αx, γy)` pins down `α` and `γ`. So the units are exactly the piecewise product prefix replacements, which form `2V`.
- `D(s_α t_β ⊗ Y) = Σ_i s_(w_i α) t_(w_i β) ⊗ Y` gives `D(2V) ⊆ 2V`, and `(t_1 ⊗ 1) D(X) (s_1 ⊗ 1) = X` gives injectivity.

**Proposition 2.1 (transfer). Valid; the scope wording should change.**
- **Scope note.** The Scope paragraph of the Theorem B node, and the sentence after Theorem B in [CS] Section 2, cover subgroups `H <= G = L_(F_2)(1,2)^x`. `2V` sits in `(R ⊗ R)^x` and is not such a subgroup, so the first proof bullet should not credit the case to that paragraph. The transfer is still valid, because [CS] Sections 0–2 use three kinds of input.
  - **Identities among `g`, `D^j(g)` and the `π_M`.** These give the tower `C_3^k`, `A_9`, the `f_K` and `π_M f_K π_M^(-1) = f_(MK)`. They hold in `V`, and `x -> x ⊗ 1` carries them into `2V` because it intertwines `D_2` on `V` with `D` on `V_1`.
  - **`D` is an injective group endomorphism of `H`.** Section 1 gives this for `2V`.
  - **`[D(h)]` commutes with `[g]` for every `h` in `H`** (Lemma 1.1). The proof's commutation bullet checks this for `2V`, using `π s_i = s_(τ(i))` and `t_i π^(-1) = t_(τ(i))`.
  - The rest is ring theory: Lemma 1.3, census Theorem A(b) for 5 <=> 6, and the absorption argument for 6 => 2.
  - Suggested wording: "the proof of [CS] Theorem B uses only these three inputs, and each holds in `2V`". The second "Why this host" bullet of the claim node has the same phrasing.
- **Directly finite fails.** `d a = c e b = 1`. If `a d = e b c = 1`, then `e = e (e b c) = e b c = 1`, but the support of `e` is `{1, g ⊗ 1, g² ⊗ 1}`. Correct.
- **Not surjunctive.** `stable-finiteness-failure-refutes-surjunctivity` applies with `n = 1` and `k = F_2`. Correct.

**Corollary 2.2. Correct.** `x -> x ⊗ 1` restricts to a unital ring map `f F_2[V] f -> T`. A state on `(K_0(T), [T])`, composed with `K_0` of that map, is a state that sends `[f]` to `1`.

**Lemma 2.3 (baker unit). Correct.**
- (a)–(d) recompute from `t_i s_j = δ_ij`. The double sums collapse to `D_2(X) ⊗ 1` and `1 ⊗ D_2(Y)`, and `t_i π s_j = [i = τ(j)]` collapses the third to `π ⊗ 1`.
- (e) `h V h^(-1) = V`, so `V ⊆ D_2(V)` would make all of `V` commute with `g`. The transposition of `w_1, w_2` conjugates `(1 2 3)` to `(1 3 2)`, that is, `g` to `g²`.

**Proposition 2.4 (product tower). Correct.**
- Conjugation by `u^(-1)` sends `D_2^j(g) ⊗ 1` to `D_2^(j-1)(g) ⊗ 1` for `j >= 1`, `g ⊗ 1` to `1 ⊗ g`, and `1 ⊗ D_2^j(g)` to `1 ⊗ D_2^(j+1)(g)`.
- So one step turns `(C_3^(k-j) ⊗ 1)(1 ⊗ C_3^j)` into `(C_3^(k-j-1) ⊗ 1)(1 ⊗ C_3^(j+1))`. The factors commute and meet trivially, so the averaging idempotents multiply, and induction gives (b).
- (a) follows from `D(X) = u X u^(-1)` on `F_2[V_1]` and `u^(-1) e^(1) u = e^(2)`.

**Proposition 2.5. Correct.**
- The four orthogonal idempotents give `a + b + c + d = 1`, and the conjugacy of `e^(1)` and `e^(2)` gives `b = c`.
- `[f] = 3[E_1 - E_2]` holds in `F_2[V]`, which is inside the Scope with `H = V`, and passes to `A`. Since `u^(-1) E_1 u = e^(2)` and `u^(-1) E_2 u = e^(1) e^(2)`, it becomes `c + d = 3c`.
- Endpoints. `b = 0` gives `(1, 0, 0, 0)`, the augmentation. `b = 2/9` gives `(1/9, 2/9, 2/9, 4/9)`, the product of the regular values `1/3` and `2/3`.
- The artifact claims only that these values are consistent with the relations, not that `b = 2/9` comes from a state, and that is all this checks.

**Routes. All three are valid, and each now carries the verdict.**
- **`thompson-v-nonsurjunctivity-ascends-to-brin-thompson-2v`.** `x -> x ⊗ 1` is an injective group homomorphism `V -> 2V`, and non-surjunctivity ascends along subgroups.
- **`brin-thompson-2v-nonsurjunctive-from-averaging-fullness`.** This is Proposition 2.1, bullets 2–3.
- **`brin-thompson-2v-averaging-fullness-ascends-from-v`.**
  - **Conventions.** Section 1 multiplies as operators, and `s_α t_β` sends `βx` to `αx`. So codes `(α_k) -> (β_k)` mean `α_k x -> β_k x`, and `g = (0, 10, 11) -> (10, 11, 0)` agrees with `g = s_2 t_1 + s_3 t_2 + s_1 t_3`.
  - **Trace.** `φ^(-1)` sends `1x -> 0x`, `01x -> 10x` and `00x -> 11x`. Composing gives `1x -> 0x -> 10x -> 01x`, `01x -> 10x -> 11x -> 00x` and `00x -> 11x -> 0x -> 1x`.
  - **Result.** `φ g φ^(-1) = (1, 01, 00) -> (01, 00, 1) = h^(-1)`, so `φ e_g φ^(-1) = e_h`. The group-ring map of `V -> 2V` carries `(c' φ) e_g (φ^(-1) b') = 1` to the target.
  - **Note.** "Whichever orientation is used" covers how the codes of `h` are read. It does not cover the opposite product convention. Under that convention the same word computes `φ^(-1) g φ`, which sends `010z -> 11z` and so is neither `h` nor `h^(-1)`. The conjugator is then `φ^(-1)`, and the conclusion is unchanged.
