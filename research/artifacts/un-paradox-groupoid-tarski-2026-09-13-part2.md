...
**Corollary F (families).** In each case below `A_k(𝒢)` is finitely generated with a properly
infinite unit. So for every `N >= 3`, `EL_N(A_k(𝒢))` is a finitely generated Kazhdan group whose
...
- Boundary actions of `F_d`: every MF quotient is trivial from rank 2, and the group is Kazhdan from
  rank 3.
...
`u_h 1_(C(g)) u_h^(-1)`. Hence the algebra is finitely generated, and Ershov–Jaikin-Zapirain give (T)
for `N >= 3`. For `d > 2` use `C(a)`, `C(a^(-1))` for one generator, and the union of the other
...
- It is generated by the `s_x`, the `t_x` and `1_(germs of g)` for `g` in a finite generating set
  of `G`, so it is finitely generated.
