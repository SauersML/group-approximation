...
- the hole `modular-nested-idempotents-equivalent-over-leavitt-units`, now REFUTED;
- the route `modular-nested-equivalence-refutes-leavitt-direct-finiteness`, now dead.

**Correction (2026-09-12).** Theorem 2 is a special case of
`nested-two-finite-subgroup-idempotents-have-strict-rank` (established 2026-09-07). That node gives a
strict rank gap for properly nested idempotents from any two finite subgroups, over any field, with no
condition on the intersection or the join. So the even-intersection survivors of Section 4 are dead
too, and the hole is refuted (flagged by `w4-kap-join`; `w3-vf-linear` Section 12.3). This lane missed
the node because its duplicate scan truncated the list of candidate names.
...
## 4. Where the semisimple proof stops (superseded)

All of the even-intersection cases below are excluded anyway by
`nested-two-finite-subgroup-idempotents-have-strict-rank`. They record only where this proof's
method fails. The genuine survivor is idempotents not supported in finite subgroups.
...
No candidate pair with even intersection was constructed in this lane; by the 09-07 node none exists.
