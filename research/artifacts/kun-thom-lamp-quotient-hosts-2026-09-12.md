...
## 5. The closure is decided by finitely many lamps and by infinite coset spaces

**Proposition 5.1 (finitely generated submodules suffice).** Let `U <= P` be a submodule and
suppose `H_U` carries a strict pair. Then some finitely generated submodule `U_0 <= U` has
`H_(U_0)` carrying a strict pair. So if every finitely generated submodule of `U` is closed,
`H_U` is surjunctive.

*Proof.* Take the finite set `F <= H_U` of the transfer theorem, choose a lift `f~` in `W` of each
`f in F`, and let `U_0` be generated by the finitely many lamp configurations
`s~ m~ (sm)~^-1` and `w~ m~ (wm)~^-1`. They lie in `U` because their images in `H_U` are trivial.
Put `phi(f) = image of f~ in H_(U_0)`. Then `phi` respects the products of `F`. Distinct points of
`F` stay distinct, since their lifts differ by an element outside `U`, hence outside `U_0`. By the
transfer theorem `H_(U_0)` carries a strict pair. If `U_0` is closed this contradicts Corollary
3.3. QED

So the root claim may assume `U` finitely generated, by finitely many fibre-even configurations.

**Lemma 5.2 (finite permutation images see nothing).** If `Y` is a finite `G`-set and `c` is in
`F_2[Y]^Gamma`, then `phi_c` kills `K_even`.

*Proof.* `G` acts on `Y` through a finite quotient `G/M`, so `c` is fixed by `Gamma M`. The profinite
closure of `Gamma` in `G` is `N` (`kun-thom-wreath-sofic-radical-is-fibre-parity-kernel`), so
`Gamma M >= N` for every normal subgroup `M` of finite index. Hence `c` is `N`-fixed, `phi_c`
factors through `P -> F_2[G/N]`, and it kills `K_even`. QED

So the permutation closure of a submodule of `K_even` is decided only by infinite `G`-sets.

**Lemma 5.3 (Hecke operators).** For a transitive `G`-set `G/L`, the maps `P -> F_2[G/L]` are the
Hecke operators

    T_D(e_(g Gamma)) = sum_(x in D) e_(g x),

where `D` is a finite union of finite `Gamma`-orbits in `G/L`, that is, of double cosets
`Gamma h L / L` with `[Gamma : Gamma ∩ h L h^-1]` finite. A map into a general `Y` is a sum of
such operators over the `G`-orbits of `Y`, and its kernel is the intersection of theirs. So

    U-bar = intersection of ker T_D over all L and all D with T_D(U) = 0.

*Proof.* Lemma 3.2 with `c = 1_D`. Different `G`-orbits of `Y` have disjoint supports, so the
kernel of the sum is the intersection. QED

**Remarks.**
- One `Gamma`-orbit `Gamma h L/L` contributes the push-pull correspondence
  `e_(g Gamma) -> sum_(gamma in Gamma/Gamma_0) e_(g gamma h L)`, with `Gamma_0 = Gamma ∩ h L h^-1`. When the
  orbit is a single point with `h L h^-1 >= Gamma`, this is the permutation map of `U_(hLh^-1)`.
- Several `Gamma`-orbits inside one `G`-orbit add in the same module and can cancel. So the kernel
  of `T_D` is in general not an intersection of kernels of single correspondences. This
  cancellation is what the closure can use.
- Sanity check: for `G = Z` and `Gamma = 1`, the `T_D` are convolutions on `F_2[Z/n]` and on
  `F_2[Z]`. A finitely generated proper quotient `F_2[t^(±1)]/(f)` is finite, hence a module over
  some `F_2[Z/n]`, and it embeds in a free one because group algebras of finite groups are
  self-injective. So every finitely generated submodule is closed there.
- `U` generated by `v = sum_i e_(g_i Gamma)` is killed by `T_D` iff every point of `G/L` occurs an
  even number of times in the multiset `{ g_i x : i, x in D }`.

**Example 5.4 (commutator test).** Let `a, b in N`, `v = (1+a)(1+b) e_Gamma` and
`w = (1+b)(1+a) e_Gamma`, both fibre-even. For a `Gamma`-fixed `c`, the difference of the two
operators is `(ab + ba) c`. So `w` lies in the closure of `F_2[G] v` iff every `Gamma`-fixed
finitely supported solution of `(1+a)(1+b) c = 0` in a permutation module is also fixed by
`b^-1 a^-1 b a`. Membership of `w` in `F_2[G] v` itself is a separate linear question in `P`.
