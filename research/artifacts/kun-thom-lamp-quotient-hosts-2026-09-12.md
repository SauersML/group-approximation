...
**Lemma 5.3 (Hecke correspondences).** Every map `phi_c : P -> F_2[Y]` is a finite sum of maps
of the form

    T_(Gamma_0, L) = push_(Gamma_0 -> L) o pull_(Gamma -> Gamma_0),
    e_(g Gamma) -> sum_(gamma in Gamma/Gamma_0) e_(g gamma L),

composed with an injection `F_2[G/L] -> F_2[Y]` onto one orbit. Here `Gamma_0 <= Gamma` has finite
index and `Gamma_0 <= L <= G`.

*Proof.* `c` is a finite sum of indicators of finite `Gamma`-orbits `Gamma y`, with at most one
term per orbit. For one orbit, put `L = Stab_G(y)` and `Gamma_0 = Gamma ∩ L`, which has finite
index in `Gamma`. The orbit is `{gamma y : gamma in Gamma/Gamma_0}`, so
`phi(e_(g Gamma)) = sum_(gamma) g gamma y`, which is `T_(Gamma_0, L)` followed by `hL -> h y`. QED

**Remarks.**
- If `L >= Gamma`, then `T_(Gamma_0, L) = [Gamma : Gamma_0] push_(Gamma -> L)`. That is either zero or
  the permutation kernel map of `U_L`.
- New kernels therefore come only from overgroups `L` of finite-index subgroups `Gamma_0` that do
  not contain `Gamma`.
- A submodule `U` generated by `v = sum_i e_(g_i Gamma)` is killed by `T_(Gamma_0, L)` iff every coset
  of `L` occurs an even number of times in `{ g_i gamma L : i, gamma in Gamma/Gamma_0 }`.
- `U` is closed iff it is the common kernel of the correspondences with that parity property.
