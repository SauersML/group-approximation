...
trivial, hence `ρ_v(u) = 1`. By BMS, Introduction (i) (p. 60), `Γ'` contains some `E_q` with `q != 0`,
and `E_q` has finite index in `SL_3(Z)`. `E_q` is the normal subgroup generated by the elementary matrices
congruent to `1` mod `q`, so it is generated by unipotent matrices of `Γ'`, and `ρ_v(E_q) = 1`. So
`ker ρ_v` has finite index.
*Correction (ex-verify2-dynamics, 2026-09-13).* The earlier text said that the `e_ij^N` generate
`E_3(Z, N)`. They generate a subgroup that need not be normal. The conclusion is unchanged.
