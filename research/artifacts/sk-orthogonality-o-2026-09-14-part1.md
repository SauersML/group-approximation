...
Lane sk-orthogonality-o. Plan: `research/artifacts/sk-orthogonality-o-plan-2026-09-14.md`. Everything here is
UNREVIEWED.
...
- `ind` is the index of `subshift-gl3-index-kernel-is-elementary-group` (O1, unreviewed).
...
So `α` is standard, `M_3(R_X) ≅ M_3(R_Y)`, and `X` and `Y` are strongly orbit equivalent.
...
2. **Module type.**
   ...
   - A composition factor `1`, `3*` or `8` of such a submodule restricts to `F21_X` with a constituent `1`, `3*`, or
     `2` (F1), since restriction is exact. These are forbidden by (O′) and (Sing_X).
...
4. **Scalar supports.**
   ...
   - So `S′_V = b_VI` with `b_V` an idempotent. By Proposition F (PASS) with (O) and (O′), `V ↦ b_V` is a Boolean
     embedding. Put `𝔅′ = span{b_V}`.
5. **The diagonal.**
   - By Proposition F item 2, `α′(q_V) − I ∈ b_VM_3b_V` and `α′(q_{X∖V}) − I ∈ (1−b_V)M_3(1−b_V)`.
...
6. **Roots.**
   - `D_X` is maximal commutative in `R_X`, so `C_{G_X}C_{G_X}(A_ij ∩ L_X) = A_ij(R_X)` (Theorem R Step 3, with
     `el3-root-subgroups-are-bicommutant-closed`, PASS).
...
7. **Standardness.** `peirce-root-homomorphisms-of-e3-are-standard` with `E_k = E_kk` gives a ring isomorphism
   `Θ: M_3(R_X) → M_3(R_Y)` extending `α′`. Then `cantor-crossed-product-matrix-ring-iso-forces-soe` gives strong orbit
   equivalence. ∎
