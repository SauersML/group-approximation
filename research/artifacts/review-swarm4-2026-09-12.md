...
## 44. `kt-wreath-models-never-round-on-the-vertex` and `bowen-chapman-witness-is-kt-double-at-q2-r3-d3` (91acaa96f): PASS

Read at `1d83afbdd`, unchanged at `f606d06f0`:
- both claims and routes;
- the artifact `research/artifacts/q34-bc-witness-hyperlinear-2026-09-12.md`, §§6–7;
- item 4 of `unit-type-vertex-rounding-forces-compressor-commutant-rigidity` with its route, and
  `kun-thom-wreath-carries-rigid-defect`;
- the frontmatter of the three equivalence premises;
- `GroupApproximation/BowenChapman/LaurentPair.lean`, `Endpoint.lean` and
  `GroupApproximation/Sofic/WeakMFNonsoficDouble.lean`.

### 44.1 The normalization step: PASS

- **What step 4 needs.** `σ(g) N σ(g)^* = N` for every `g` in `G`, where `N = σ(Γ)' ∩ M`. It suffices to check this on
  a generating set, provided each generator gives equality and not only inclusion.
- **Generators.**
  - `Γ` normalizes its own commutant.
  - `SL_d(Z)` is generated by the elementary matrices `e_ij(1)`. These are nonnegative, so substitution by them sends
    polynomials to polynomials, and they are compressors.
  - `EL_r(F_q[x^(±1)])` lies in `<Γ, SL_d(Z)>`. Conjugating `e_ij(x^w)` by `M` in `SL_d(Z)` gives `e_ij(x^(Mw))`.
    Every `v` in `Z^d` is `Mw` with `w = k e_1` in `N^d`, where `k` is the content of `v` and `M` has first column
    `v/k`.
  - So `G = <Γ, SL_d(Z)>`.
- **Compressors.** Item 4 of the premise gives `Ad σ(A) N = N` for a nonnegative compressor `A`. The defect of the
  intertwining is bounded by `eps_n <= 4 τ(1 − P_n)^(1/2)`, which vanishes along `U` by step 3. It is an equality, so
  `e_ij(−1)` needs no separate argument.
- **One model.** Every unitary the premises conjugate by is `σ` of an element of `G`: `σ(A)`, `σ(t)`, `σ(A^k)`,
  `σ(B)`. So step 0 is accurate, and no premise quantifies over a second model.

### 44.2 The rigid defect and the lamp contradiction: PASS

- **In `N`.** `γ e_Γ γ^(-1) = e_(γΓ) = e_Γ` for `γ` in `Γ`, so `σ(e_Γ)` lies in `N`. By 44.1,
  `σ(e_(t_0Γ)) = σ(t_0) σ(e_Γ) σ(t_0)^*` lies in `N`.
- **Equal images.** `σ(e_(t_0Γ))` commutes with `σ(γ)`, so `σ(e_(γ t_0 Γ)) = σ(e_(t_0 Γ))`.
- **Distinct lamps.** `γ` is not in `t_0 Γ t_0^(-1)` iff `γ t_0 Γ ≠ t_0 Γ`. Distinct lamps have a nontrivial product in
  `⊕ Z/2`.
- **Norm.** `σ` of an involution is a self-adjoint unitary, so `||u − v||_2^2 = 2 − 2 Re τ(uv)`. Trace preservation
  gives `τ(σ(w)) = 0` for `w ≠ 1`, hence `2`, against `0` from the equality. The contradiction is correct.
- **What `t_0` needs.** Step 4 covers all of `σ(G)`, so `t_0` only has to lie in `G` with `Γ ⊄ t_0 Γ t_0^(-1)`.
- **Diagonal form.**
  - `A_ε` is monotone in `ε`, and `k(n)` is defined on `A_1`, which lies in `U`.
  - `{n : k(n) >= k_0}` contains `A_(1/k_0) ∩ [k_0, ∞)`, which lies in `U` because `U` is nonprincipal.
  - So `1/k(n) -> 0` along `U`. The chosen `π_n` satisfy both `n'/n -> 1` and distance `-> 0`, which is an (H1)
    rounding. Correct.

### 44.3 Scope: PASS

- **Matrix models.** Models are trace-preserving homomorphisms into `∏_U M_n`, and (H1) is a dimensional statement
  (`n'/n -> 1`, `o(n)` padding). Both claims use exactly this.
- **`R^ω` models.** They appear only in artifact §6, and there only hedged:
  - `R^ω` embeds into a matrix ultraproduct over `N × N` with the product ultrafilter;
  - so an `R^ω`-valued `W`-model yields a matrix model only after that embedding, and the vertex statement concerns
    the new model.
  - The claim's scope bullet says nothing is claimed for non-matrix coordinates. That is accurate.
- **Double models.**
  - The claim disclaims vertex restrictions of `G *_Γ G` models, correctly.
  - The contradiction uses a lamp commuting with `Γ`, and the double has no such element.
  - `RE/C` rebuilds the model rather than restricting it.
- **Corollary 1.** It uses the standard fact that a countable group is hyperlinear iff it has a trace-preserving
  homomorphism into some `U(∏_U M_n)`. Given the claim, the three-way equivalence is immediate.

### 44.4 The equivalence chain and the Lean identification: PASS

- **Lean names at the tip.**
  - `LaurentPair.lean`:
    - `Laurent := AddMonoidAlgebra (ZMod 2) (Fin 3 → ℤ)` (34);
    - `Poly := AddMonoidAlgebra (ZMod 2) (Fin 3 → ℕ)` (38);
    - `polyToLaurent_injective` (68);
    - `Actor := ↥(elementaryGroup (Fin 3) ℤ)` (85);
    - `exponentAction` (88), `substitution` (119), `actorAction` (146);
    - `Ambient` (189), `peripheralHom` (193), `Peripheral` (197), `peripheralHom_injective` (217).
  - `WeakMFNonsoficDouble.lean:34`: `SymmetricDouble := PushoutI (doubleMap G Γ)`.
  - `Endpoint.lean:43`: `symmetricDouble_fg_surjunctive_not_isSofic`.
- **Actor = EL_3(Z).**
  - `Z` is Euclidean, so `EL_3(Z) = SL_3(Z)`, and `Poly = F_2[x_1, x_2, x_3]`.
  - Theorem E substitutes through the contragredient, and Lean through the standard representation. They differ by
    the automorphism `φ(M) = (M^T)^(-1)`.
  - `(l, M) ↦ (l, φ(M))` respects the semidirect multiplication because `φ` is a homomorphism and the actions agree
    through it. It is the identity on the `EL_3` factor, so it fixes `Γ`.
  - So `E ≅ D` sends `Peripheral` to `Γ`.
- **The chain.**
  - `1 ⇔ 2` is the isomorphism.
  - `2 ⇔ 3 ⇔ 4` applies `kt-wreath-hyperlinear-iff-double-hyperlinear` and
    `kt-q2-double-hyperlinear-iff-relative-embeddable`. Their hypotheses hold at `(2, 3, 3)`: `q = 2` is prime and
    `r = d = 3 >= 3`.
  - `3 ⇔ 5` is `kt-free-action-hyperlinear-iff-wreath-hyperlinear`.
  - `3 ⇔ 6` is the standard fact of 44.3.
  - `6 ⇔ 7` is the new claim.
- **No cycle.** None of the four required claims routes through this node. The frontmatter at the tip requires:
  - the wreath/double premise: `binary-free-and-ordinary-coset-wreaths-share-hyperlinearity` and the `q = 2` node;
  - the `q = 2` node: the binary-free node;
  - the free-action node: `gkp-connes-embeddable-generalized-wreath-permanence` and `gkp-sofic-action-toolkit`;
  - the wreath-models node: the five rigidity premises.
- **Solver note.**
  - Both routes list only claim ids in `requires:`.
  - The artifact, and for the Bowen--Chapman route also `GroupApproximation/BowenChapman/LaurentPair.lean`, are
    `artifacts:` entries.
  - No solver step checks the identification, so it is checked by hand here.

### 44.5 Invalidation: none warranted

- **What the claim decides.** Its only unconditional content is: if `W` is hyperlinear, some `G`-model has no (H1)
  rounding. Both sides are open at the tip:
  - `hs-stable-vertex-rounding-for-every-model` is OPEN and has no routes;
  - `theorem-e-vertex-rounds-to-unit-type-representations` is OPEN.
  So no ESTABLISHED claim is contradicted, and nothing should get `refuted_by:`.
- **Routes that assume a rounding at every model.** The artifact's §7 table finds none live, and the rows check out:
  - `amenable-charge-route` and `wreath-amenable-near-representation` use a Kahl--Schneider near representation, not
    a rounding;
  - `positive-core-dilation-splice` is gated on Connes embeddability;
  - the double routes work at the algebra level;
  - `kun-thom-finitary-action-lifts-to-normalizer` states a necessary condition;
  - `induced-charge-extension` is already dead;
  - (H1) itself is not refuted. It is shown to be unavailable at `W`-models only.
- **Routes into `hyperlinear-wreath-model`.** None of the 14 has a vertex or (H1) premise:
  - `amenable-charge-route`, `block-monomial-fiber-models`, `finite-quotient-models`, `free-probability-carriers`;
  - `glebsky-verbatim-hs-transport`, `growing-fiber-models`, `half-space-tensor-models`,
    `halo-target-class-enrichment`;
  - `kun-thom-finitary-normalizer-lift-proves-hyperlinear-wreath`, `masa-normalizing-tracial-action`,
    `one-variable-laurent-models`;
  - `positive-core-dilation-splice`, `sofic-action-permanence`, `tower-architectures`.
  The claim's "**Extend a coordinatewise-genuine actor model.** Dead." (line 80) describes a strategy that no route
  carries.
- **`growing-fiber-models` stays live.** Neither of its two invalidators is established:
  - `invariant-cartan-completion` has no routes;
  - `kt-centralizer-normalization-hs` is open. Each of its routes needs an OPEN or routeless premise:
    - `hyperlinear-groups-kill-rigid-compression-defects` (OPEN);
    - `approximate-collapse-for-kt-compressor` (no routes);
    - `hs-stable-vertex-rounding-for-every-model` (OPEN), needed by two routes.
  - The new claim gives normalization only at models that have an (H1) rounding. That does not establish
    `kt-centralizer-normalization-hs`, so neither invalidator activates.
- **Model tests.** Artifact §8 runs three tests, all consistent with the claim:
  - genuine actor models with `P_n = 1`;
  - a `BS(1,2)` calibration where normalization fails;
  - a Leavitt calibration.

### 44.6 Premise statuses, subsumption and display

- **Displays.** Both claims read "**ESTABLISHED.**", and no `invalidates:` list names either proof route.
- **Premises.**
  - These premises read ESTABLISHED:
    - the five rigidity premises of the wreath-models route;
    - `kt-wreath-hyperlinear-iff-double-hyperlinear`;
    - `kt-free-action-hyperlinear-iff-wreath-hyperlinear`.
  - `kt-q2-double-hyperlinear-iff-relative-embeddable` has no bold display, but it computes ESTABLISHED:
    - `kt-q2-double-gao-relative-embedding-proof` →
      `binary-free-and-ordinary-coset-wreaths-share-hyperlinearity` → `tensor-gaussian-binary-wreath-equivalence-proof`;
    - that route's three premises are:
      - `binary-coset-lamp-coefficient-forces-relative-embedding`, by `tensor-lamp-expectation-relative-embedding-proof`;
      - `relatively-embeddable-pair-makes-the-hnn-group-hyperlinear`, by
        `relatively-embeddable-pair-hnn-hyperlinear-proof`;
      - `hnn-stable-letter-rounds-to-finite-order`, by `hnn-stable-letter-rounding-proof` →
        `hnn-hyperlinearity-is-amalgamated-free-haar-unitary` → `hnn-amalgamated-free-haar-unitary-proof`.
  - Every leaf route has `requires: []`, and no `invalidates:` list names a route on the chain.
  - No claim on the chain carries `refuted_by:`. Neither do the two new claims, their premises, or the three
    equivalence premises.
- **Subsumption.** Neither claim restates a listed node:
  - The wreath-models claim lists as distinct:
    - `hs-stable-vertex-rounding-for-every-model`, `theorem-e-vertex-rounds-to-unit-type-representations`;
    - `unit-type-rigidity`, `leavitt-pair-vertex-rounding-is-nonhyperlinearity`;
    - `ccr-holds-for-all-genuine-fd-coordinate-models`, `theorem-e-congruence-projections-vanish-at-regular-trace`;
    - `laurent-vertex-rounds-for-every-model`.
  - These are the rounding and rigidity statements it bears on.
  - The Bowen--Chapman claim lists as distinct:
    - `kt-pair-group-double-is-nonsofic`, `kun-thom-group-doubles-are-surjunctive`;
    - `kt-wreath-hyperlinear-iff-double-hyperlinear`, `surjunctive-nonsofic-group-exists`.
  - These are nonsoficity, surjunctivity and the general equivalence.
- **Leverage.** No status changes. The two claims isolate one question, whether `W` at `(2, 3, 3)` is hyperlinear.
  The vertex-rounding lane and the Bowen--Chapman witness both reduce to it at the group identified in Lean.

## 45. `thompson-f-one-bump-binomials-have-common-multiples` (3bca7e85e) and `thompson-f-multibump-binomials-meet-stabilizer-ideals` (4afb4efa6, scope fix 1a6c17f6c): PASS

Read at `1d83afbdd`, unchanged at `f606d06f0`: both claims and routes, Lemmas B and C of
`thompson-f-rescaled-x0-binomials-have-common-multiples-proof`, and the 1a6c17f6c diff.

### 45.1 Step 0, the coset test: PASS

- **Decomposition.** The right cosets `<g>h` give `R = ⊕_h K[<g>] h` as a left `K[<g>]`-module. For
  `r = sum_h r_h h`, `(g - mu) r = sum_h (t - mu) r_h h`, so `J = ⊕_h (t - mu) K[t^(±1)] h`. A Laurent polynomial
  lies in `(t - mu) K[t^(±1)]` iff it vanishes at `mu`, which is the test.
- **Consequence.** Left multiplication by `g^k` acts on each component as `t^k`, and `(t^k - mu^k) p` vanishes at
  `mu`. So `g^k y - mu^k y` lies in `J`.
- **The ideal.** `g - mu = lambda^(-1) (1 + lambda g)` at `mu = -1/lambda`, so `J = (1 + lambda g) R`.

### 45.2 Proposition 1, Steps 1–6: PASS

- **Step 1.** `e(g) = -1`, so `g^(e(f)) f` has `e = 0` and is the identity near `0`. Step 0 gives
  `f ≡ mu^(-e(f)) g^(e(f)) f`. Since `b - b_1` lies in the right ideal `J`, `b v ≡ b_1 v`.
  - Adversarial check: `b_1` can vanish (when `b` lies in `J`), and `b_2`, `b_3` with it. Nothing downstream needs
    them nonzero. Nonvanishing of `b v` comes from `b ≠ 0`, `v ≠ 0` and `R` being a domain, and Step 5 produces
    `w' ≠ 0` for any `b_3`.
- **Step 2.** `g^(-k) f g^k` has support `g^(-k)(supp f) ⊆ [g^(-k)(eps), 1]`, and `g^(-k)(eps) -> 1` because
  `g^(-1)(t) > t` on `(0,1)`. The transfer back is `g^(-k) y ≡ mu^(-k) y`.
- **Step 3.** Conjugation by `tau^j` transports the survey's Lemma 3.8, and `b_2 w = b_3 w'` for `w = s w'`. The
  lemma is quoted and was not re-read (45.5).
- **Step 4.** `A^(-1)(y) = 1 - 2^(-n)(1 - y) = tau^n(y)`.
  - For `t <= A^(-1)(c)`: `g` is increasing with `g(A^(-1)(c)) = c`, so `h` fixes `g(t)` and `g^(-1) h g (t) = t`.
    `phi^n(h) = tau^n h tau^(-n)` is the identity on `[0, tau^n(c)]`, the same interval.
  - For `t >= A^(-1)(c)`: `g = A` there, `h g (t)` lies in `[c, 1]`, and `g^(-1) = A^(-1)` on `[c, 1]` because
    `c >= c_0 > g(c_0)`.
  - `phi^n` maps `M_j` into `M_(j+n) ⊆ F_[1-2^(-j-n),1]`, which stays inside `[c_0, 1]`. So the induction
    `b_3 g^k = g^k phi^(nk)(b_3)` runs.
- **Step 5.** `b_3 w' = sum_k mu^(-k) g^k phi^(nk)(b_3) r_k ≡ sum_k phi^(nk)(b_3) r_k = 0`. For nonvanishing:
  `g^k m = g^l m'` with `m, m'` in `F_[1/2,1]` makes `g^(k-l)` trivial on `[0, 1/2]`, and `g` moves every point of
  `(0,1)`. So the monomials `g^k m` are distinct, and a nonzero `(r_k)` gives `w' ≠ 0`.
- **Step 6.** A monomial of `phi^(nk)(b_3)` has letters `<= I + nk` and degree `d <= D`. Inserting its letters into
  `a` in `A` keeps the letters of `a` and creates letters `<= I + nN + c + D <= L`. So the image lies among the normal
  forms of degrees `c, ..., c + D` on `[1, L]`, at most a constant multiple of `|A|` once `L + 1 >= 2D`. The unknowns
  have dimension `(N + 1)|A|`, with `N + 1 >= (L/2 - I - D)/n` unbounded. So the kernel is nonzero.

### 45.3 Lemma 2, Proposition 3 and items 1–3: PASS

- **Lemma 2.**
  - `varphi_0` exists. Its source intervals have lengths `2^(-m), ..., 2^(-1)` and total `1 - 2^(-m)`. Its target
    intervals have lengths `2^(-2), ..., 2^(-m), 2^(-m)` and total `1/2`. Both lists have `m` powers of `2`, so
    every piece has slope in `2^Z`.
  - The junction identities `varphi_0(2^(-m)) = 1/2` and `varphi_0(1) = 1` hold.
  - Near `0`, `varphi f varphi^(-1)` is `y -> 2^k y` for `e(f) = mk`, and it is dyadic PL away from `0`.
  - Hand test at `m = 2`: `x_0` is `t/2` near `0`, so `e(x_0^2) = -2`. `psi(x_0^2)` is one-bump below the diagonal
    with `e = -1`.
- **Proposition 3.**
  - `c_t = t^(-1) g t` lies in `H_m`, has `e = -m` and stays one-bump below the diagonal. So `psi(c_t)` is `y/2`
    near `0`, and Proposition 1 applies to each `c_t`.
  - The iterated intersections stay nonzero because `R` is a domain.
  - `t (1 + lambda c_t) = (1 + lambda g) t` gives `b v` in `(1 + lambda g) R`.
  - The case `g(t) > t` reduces through the left unit factor `lambda g`.
- **Item 1.** A support endpoint is a breakpoint: one side is the identity, and on the other `g` is linear, fixes the
  endpoint and moves points, so its slope is `≠ 1`. So endpoints are dyadic and `theta` exists by matching dyadic
  subdivisions. Lemma B transports property O.
- **Items 2–3.** Item 2 substitutes `O_1` = all one-bump elements into items 1–3 of the rescaled node. Item 3 is the
  right unit factor `u`.

### 45.4 The multi-bump claim and the scope fix: PASS

- **Step 1.** `H_i` fixes each `[p_j, q_j]` pointwise, so it preserves the complement and commutes with `g_j`. `Z_i`
  meets `H_i` trivially because `g_j^(a_j)` is nontrivial on `(p_j, q_j)` when `a_j ≠ 0`.
- **Step 2.** `iota` is a ring map because `Z_i` is central in `H_i × Z_i`, and it is injective as a tensor product
  over a field. `iota(1 + lambda g) = 1 + mu g_i`.
- **Step 3.**
  - The one-bump claim is stated over an arbitrary field, and here it is applied over `L`.
  - Left multiplication by `1 + mu g_i` and by `beta` preserves the coset components of `L[F]` over `L[H_i]`.
  - `beta v ≠ 0` gives a nonzero component, and that component lies in `L[H_i]`.
- **Step 4.** A nonzero central polynomial `d` clears the denominators. Injectivity of `iota` pulls the identity back
  into `K[H_i × Z_i] ⊆ R`.
- **Scope fix 1a6c17f6c.**
  - *Criterion.* The bumps of `g` lie in `F` iff every component endpoint is dyadic, since a bump's endpoints are
    breakpoints.
  - *Example.* `4x - 1` fixes `1/3` with slope `4` on both sides, so `1/3` is a shared endpoint of two components.
    The map is affine with slope in `2^Z` and dyadic intercept, so some element of `F` equals it near `1/3`. The
    two bumps there break at the non-dyadic point `1/3` and are not in `F`. The fix is needed and correct.
  - *The uncovered `b = 1 + t`.*
    - If `b = b' s` with `b'` in `K[H_1 × Z_1]`, then `s^(-1)` and `t s^(-1)` lie in `H_1 × Z_1`, and so does `t`.
      But `H_1 × Z_1` fixes `p_2`.
    - `H_2` preserves `(p_1, q_1)` and its complement, and `g_1` fixes `p_2`. So `H_2 × Z_2` keeps `p_2` outside
      `(p_1, q_1)`.
    - When `t` is one-bump, the pair `1 + t` against `1 + lambda g` is item 2(b) of the one-bump claim, and the
      intersection is symmetric.

### 45.5 Trust surface, subsumption and display

- **Quoted and not re-read here.**
  - the survey's Lemma 3.8 (arXiv:2305.07113), used in Step 3;
  - the normal form of `M` and `M ⊆ F`;
  - left-orderability of `F`, so that `R` is a domain;
  - from the `O_1` route, Lemma C (`tau h tau^(-1) = phi(h)`) and the supports of `x_i`. Lemma B was checked (45.3).
- **Premises and display.**
  - The one-bump route requires `[thompson-f-rescaled-x0-binomials-have-common-multiples]`, which is ESTABLISHED at the
    tip.
  - The multi-bump route requires `[thompson-f-one-bump-binomials-have-common-multiples]`.
  - Both claims read "**ESTABLISHED.**", which is consistent.
- **Subsumption.** The one-bump claim extends `O_1` from the rescaled class of its premise to every one-bump element.
  It does not duplicate that node. No other node on property O for one-bump or multi-bump elements was found. Novelty
  against Guba's survey is unchecked, as both nodes say.
- **Leverage.** `thompson-f-is-not-amenable` stays OPEN.
  - Item 3's consequence is accurate: a binomial side of a non-Ore witness needs a ratio with at least two bumps.
  - The multi-bump claim does not close that case, since it reaches only `b` in `K[H_i × Z_i] F`.

## 46. `bs-no-wq-normal-residual` (212e6b571): the five non-import claims PASS, the four imports MATCH

Read at `1d83afbdd`, unchanged at `f606d06f0`: the five claims and routes, the four imports and their routes, and
the sources named in 46.6.

### 46.1 `fpbs-wq-normal-hull-is-almost-malnormal`: PASS

- **Stabilization.** `beta ↦ x_beta` in `W_(beta+1) \ W_beta` is injective into the countable `Gamma`, so the chain
  stops at a countable ordinal.
- **Item 1.**
  - The least `beta` with `W_beta ⊄ K` is neither `0` nor a limit, so it is `gamma + 1`.
  - Some generator `g` of `W_(gamma+1)` lies outside `K`, and `g K g^(-1) ∩ K ⊇ g W_gamma g^(-1) ∩ W_gamma` is
    infinite.
- **Item 2.** This is the fixed-point equation `W_alpha = W_(alpha+1)`.
- **Item 3.**
  - Induction puts every `W_beta` inside an almost malnormal `M >= H`.
  - The second bullet takes `K = L ∩ W(H)`. Then `g K g^(-1) ∩ K ⊆ g W(H) g^(-1) ∩ W(H)`, so item 2 puts `g` in
    `W(H)`, and hence in `K`, a contradiction.
- **Item 5.** At finite index every `g W(H) g^(-1) ∩ W(H)` has finite index in the infinite `W(H)`, so it is infinite.
- **Item 6.** Every such `g` is a generator of `W_1`.
- **Definitions.** The intermediate-subgroup criterion is Peterson--Thom's Lemma `alter`, and the claim "every subgroup
  is wq-normal in some almost malnormal subgroup" is their Remark `remark1`, stated without proof (46.6). The node
  proves the remark and adds minimality, uniqueness, infinite index and item 6, as it says.
- **(R0)/(R1).** Correct: an element of infinite order generates `Z`, and (R1) is items 2, 4, 5 and 6.

### 46.2 `fpbs-wq-normal-price-one-subgroup-forces-price-one`: PASS

- **Gaboriau's notes.** Exercise 1.17 (i)⇒(iii), Proposition 2.15 with Corollary 2.35, Theorem 2.20 and the successor
  argument of Theorem 2.45 match the notes as extracted on MSI.
- **Graphing.** The cost is at most `1 + eps/2 + sum_k eps/2^(k+1) = 1 + eps`.
- **Fixed price at `H`.** The restricted action of `H` is free, although not ergodic in general. Fixed price one
  quantifies over every free p.m.p. action, so `C(R_H) = 1` applies.
- **Absorption.** A null exceptional set has a null `Gamma`-saturation, and countably many conull invariant sets
  intersect to one. So the absorbed elements form a subgroup and countable generation is safe.
- **Successor step.** Each of the three memberships is used at a point of the invariant set `X_beta`.
  - `sigma x`: `sigma` lies in `W_beta`.
  - `g_k sigma x`: this is `phi_k(sigma x)`, since `sigma x` lies in `A_k`.
  - `g_k x`: `g_k sigma^(-1) g_k^(-1)` lies in `g_k Sigma_k g_k^(-1) ⊆ W_(beta_k) ⊆ W_beta`.
- **Transfinite indexing.** Theorem 2.45 indexes by `N`. Enumerating `∪_beta S_beta` with least indices `beta_k` puts
  every generator of every successor step into one countable graphing. Correct.
- **Conclusion.** `R_Phi = R_Gamma` almost everywhere, so `C(R_Gamma) <= 1 + eps`. Classes are infinite, so the cost is
  at least `1` by Proposition 2.15.

### 46.3 `fpbs-amenable-splittings-are-ah-or-amenable-wq-normal`: PASS

- **Case 1(a).** A nonamenable group is not virtually cyclic, so Minasyan--Osin item 1 leaves acylindrical hyperbolicity.
- **Case 1(b): the ascending case is excluded.**
  - With `t A t^(-1) = D <= A`, the terms `t^(-n) A t^n` increase.
  - `a` in `A` normalizes each term, since `t^n a t^(-n)` lies in `A`, and conjugation by `t^(±1)` shifts the terms.
    So `N` is normal.
  - `A <= N`, so `Gamma/N` is cyclic, and `Gamma` is amenable, a contradiction.
  - `D = A` is symmetric, with `D ≅ C` amenable.
- **Case 2.** The negation of weak malnormality is `s`-normality, and `g = 1` gives `C` infinite. The one-step chain and
  the intermediate criterion both give wq-normality.
- **Consequences.** A finite `C` is weakly malnormal, so only case 1 occurs. The two corollaries follow.

### 46.4 `fpbs-residual-class-has-torsion-products`: PASS

- **No infinite amenable subgroup.** The projections of an amenable `S <= H × K` are amenable quotients of `S`, hence
  finite, and `S <= pi_H(S) × pi_K(S)`.
- **Hypotheses.** Hypothesis 3 is vacuous. Fixed price one comes from the Khezeli import (both factors are infinite and
  countable). Non-AH comes from Osin item 1, since both factors are infinite.
- **Examples.** They come from the monster import, and products of torsion groups are torsion.
- **Nit (wording only).** "a torsion group with no infinite amenable subgroup. So it lies in part (R0)": torsion is
  already implied by (R0) and is not needed for membership.

### 46.5 `fpbs-torsion-free-tarski-monster-no-amenable-wq-normal`: PASS

- **Item 1.**
  - Two noncommuting elements generate `T_0`.
  - A free subgroup of rank `>= 2` is either proper, hence cyclic, or all of `T_0`. In the second case `<a^2, b>` is
    proper and noncyclic, through the map to `Z/2`.
- **Item 2.** The center is proper, hence cyclic, and infinite because `T_0` is torsion-free.
- **Item 3.**
  - *Zorn.* Chains of cyclic subgroups have abelian, hence proper cyclic, unions.
  - *Centralizers.* `Z(y) = M` because the center is trivial.
  - *Normalizer.* `|Aut(Z)| = 2` gives index `<= 2` if `M` is normal, which would make `T_0` virtually cyclic.
  - *Malnormality and wq-normality.* Malnormality follows from `Z(x) = M` for `x ≠ 1` in `M`. Lemma `alter` at
    `K = M` rules out wq-normality.
- **Example, correctly hedged.**
  - Simplicity of Ol'shanskii's group is taken from the title, and its fixed price is not known.
  - So membership is conditional on fixed price one, and no member of (R1) is asserted. This agrees with "No member of
    part (R1) is known" on the residual-class node.

### 46.6 The four imports and Peterson--Thom against their sources: MATCH

Each was compared line by line with the LaTeX source the node names, fetched on MSI.

- **`fpbs-khezeli-products-have-fixed-price-one`.**
  - `Cost5.tex` (arXiv:2509.08325), lines 80–83, `thm:fixedpriceGeneral`: "The product of any two infinite countable
    groups has fixed price one."
  - Line 85 says the finitely generated case is proved first.
  - arXiv:2608.20472, `main_arxiv.tex` line 208, independently lists products of infinite groups with fixed price one.
  - The node's statement quantifies over free p.m.p. actions of `H × K` with `H`, `K` infinite and countable. Match.
- **`fpbs-minasyan-osin-weakly-malnormal-splittings`.** In `acyl-trees-Dec-2017-revised.tex` (arXiv:1310.6289):
  - line 217 defines weakly malnormal;
  - lines 219–222 are `cor:amalg-intr`, with `A ≠ C ≠ B`;
  - lines 229–231 are `cor:HNN-intr`, with `C ≠ A ≠ D`.
  - Items 1–2 match, hypotheses included.
- **`fpbs-monster-groups-have-only-finite-amenable-subgroups`.**
  - *Sources.*
    - Ol'shanskii--Sapir, `main.tex` (arXiv:math/0208237), lines 243–246, with the bibliography entries `OlTar` and
      `OlTar1`.
    - arXiv:2608.20472, `main_arxiv.tex`, lines 179 and 182: Adian, `B(m,n)` infinite and nonamenable for `m >= 2` and
      odd `n >= 665`.
    - The same file, lines 323–325: `thm:atabekyan`, citing Ivanov 2003 and Atabekyan 2011 Theorem 4.1.
  - *The route's derivations.*
    - A torsion cyclic group is finite.
    - A noncyclic subgroup of `B(m,n)` contains `B(∞,n)`, which retracts onto `B(2,n)`, and `B(2,n)` is nonamenable by
      Adian since `1003 >= 665`.
    - So amenable subgroups are cyclic of exponent `n`, hence finite.
  - The node's hedge that the Adian and Ivanov--Atabekyan inputs are imports is accurate.
- **`fpbs-osin-ah-groups-not-products-not-torsion`.**
  - `acyl-final-revised-4-1-2015.tex` (arXiv:1304.1246), lines 1156–1164: Corollary `ar`(b) and the first sentence of
    its proof, which gives free subgroups by ping-pong.
  - Items 1–2 match, and "no torsion group is AH" is immediate.
- **Peterson--Thom** (arXiv:0708.4327, source lines 708–766, Section `notionsofnormality`).
  - Lemma `alter` (744–748) is the intermediate-subgroup criterion for infinite `H`.
  - Remark `remark1` (759–763) states without proof that every subgroup is wq-normal in some almost malnormal subgroup.
  - The hull node and the Tarski-monster route quote both correctly.

### 46.7 Subsumption and display

- **Statuses.** All five non-import claims read "**ESTABLISHED.**". Their routes require only each other and the four
  imports, whose routes have `requires: []`:
  - price-one requires the hull;
  - splittings requires Minasyan--Osin;
  - torsion products requires Khezeli, Osin and the monsters;
  - the Tarski monster requires Osin.
  There is no cycle.
- **The class targets stay OPEN.** `fpbs-residual-price-one-non-ah-class-nonuniqueness` and
  `fpbs-no-amenable-wq-normal-class-nonuniqueness` are OPEN at the tip.
  - The new claims describe the class: it is nonempty through torsion products in (R0), and it has a conditional (R1)
    candidate.
  - They do not decide it.
- **Credit.**
  - The existence half of the hull is Peterson--Thom's Remark `remark1`, which is credited, and minimality, uniqueness
    and infinite index are added.
  - 46.2 is the transfinite version of Gaboriau's Theorem 2.45, which is credited.
  - The splitting dichotomy is a short corollary of Minasyan--Osin plus the ascending-HNN exclusion.
- **Leverage.**
  - The price-one consequence is accurate: Lyons' non-fixed-price-one theorem reaches only groups with no infinite
