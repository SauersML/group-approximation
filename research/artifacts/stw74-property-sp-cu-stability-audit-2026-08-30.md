...
### Homogeneous coefficient descent

There is now a concrete condition which turns a stabilized projection into
a level-one projection.  Suppose the algebra generated by the coefficients
of `p=(p_ij) in M_n(B)` is homogeneous of finite degree.  It is the section
algebra of a locally trivial finite matrix bundle over a locally compact
Hausdorff space.  The set on which the projection section is nonzero is
clopen because its fibre norms lie in `{0,1}`, and compact because the
section vanishes at infinity.  The fibre identity on this compact-open set
is a nonzero central projection in the coefficient algebra inside `B`.

In degree one this reduces to an especially elementary formula.  The
...
is fibrewise the integer rank of `p(x)`, and its positive support is the same
compact-open projection.

Under the LXXIV hypothesis, every nonzero hereditary algebra already has a
nonzero projection over it at some finite matrix level.  Thus it is enough
to require that one such stabilized projection have finite-degree
homogeneous coefficients; this local homogeneous-frame condition forces
property (SP).  It is not a comparison, cancellation, algebraicity, or
property-(QQ) hypothesis.

The contrapositive sharpens the projective-space obstruction.  Every
stabilized projection over a counterexample's projectionless corner has a
coefficient algebra which is not homogeneous of any finite degree.  Scalar
unitary changes of matrix basis do not help, because they preserve the
generated coefficient algebra.  Hence the fixed projection behind the
uniformly gapless `CP^(m-1)` family cannot have finite matrix-bundle
coordinate data; such a model would force the forbidden level-one
projection through its compact-open support.
