#!/usr/bin/env python3
"""Substitute copy retractions and exact R2 branches in inverse I(4;115)."""
...
COMPILER = Path(__file__).with_name("compile-l14-fifteen-i4115.py")
SPEC = spec_from_file_location("compile_l14_fifteen_i4115", COMPILER)
...
def reduce_coefficient(word):
    ...
    for name, sign in word:
        ...
        if out and out[-1] == (name, -sign):
            out.pop()
        elif name == "c" and out and out[-1] == letter:
            out.pop()
        else:
            out.append(letter)
    return tuple(out)
...
# Free s,t,u,v,w and involution c; r=g10=cst.
SUBSTITUTION = {
    "g0": (), "g1": (("t", 1),), "g2": (("u", 1),),
    "g3": (("v", 1),), "g4": (("w", 1),),
    "g5": (("v", -1),), "g6": (("w", -1), ("u", -1)),
...
def branch_reduce_coefficient(word, branch):
    killed = "v" if branch == "I" else "w"
    return reduce_coefficient(tuple(letter for letter in word
                                    if letter[0] != killed))
