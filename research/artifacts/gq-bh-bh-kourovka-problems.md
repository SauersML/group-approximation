...
| 7.19 | Cannonito | Explicit f.p. simple group whose WP is not primitive recursive. | Not on main. Any BH host of an explicit f.g. group with solvable non-p.r. WP answers it. Every BH class currently on main (linear, hyperbolic, metabelian, contracting self-similar; automatic groups are not among them, since `automatic-groups-satisfy-boone-higman` is OPEN) has p.r. WP, which is presumably why this stays open. Unowned. |
