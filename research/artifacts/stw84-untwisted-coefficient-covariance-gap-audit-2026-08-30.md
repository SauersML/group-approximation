...
1. A contraction within `epsilon` of a unitary has both left and right
   Schwarz defects at most `2 epsilon`.
...
2. The Stinespring factorization of a multiplicative defect uses one left
   ...
   `2 sqrt(epsilon_U epsilon_d)`; doing this in both orders gives the
   coefficient `4` in `(UCG2)`.
...
4. Exact return of either unitary puts its source in the multiplicative
   domain of a unital completely positive map.  Commutation in the tensor
   product then forces exact commutation with the other returned element,
   giving the sharper one-sided estimate.
