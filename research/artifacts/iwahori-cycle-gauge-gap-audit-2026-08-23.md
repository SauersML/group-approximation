...
Iwahori estimate and distinguishes the proved same-core calculation from the
still-open endpoint-authentication problem.  The reproducible implementation
is `experiments/iwahori_cycle_gauge_gap.py`.
...
proves the universal floor `1/sqrt(2)` and explains the computation.
...
same-core quotient gap.  Instead, they show why the missing global theorem
is selection of an exact endpoint after flexible boundary padding.

Thus the finite-cycle singular-value problem is completely controlled in
the authenticated basin.  A counterexample to the global aggregate claim
must be a genuine endpoint-authentication failure; it cannot be assembled
from low Fourier modes around exact extenders, heterogeneous direct sums of
those modes, or the existing compressed Weil boundary packets.
