...
static char g6[256];
static int pwe[MAXR], nr0;   /* debug: exponents of added power relators, cycle relator count */
      ...
      for (int i = 0; i < k && ok; i++) {
        ...
        for (int e = 1; e <= pwK && ok && (d = trace(d, u, L)) >= 0; e++) {
          ...
          if (e >= 2) {
            ...
            memcpy(rel[nr], u, L * sizeof(int)); pwe[nr] = e; rlen[nr++] = L; added++;
          }
          ok = 0;
        }
      }
...
static void report_pw(void) {                /* debug: a leaf excluded by the power sieve */
  printf("PWPRUNE %s root %d ports", g6, rootv);
  for (int v = 0; v < n; v++) { putchar(' '); for (int j = 0; j < 3; j++) putchar('0' + portof(v, inc[v][j])); }
  printf(" rels");
  for (int r = 0; r < nr0; r++) { putchar(' '); for (int j = 0; j < rlen[r]; j++) putchar("xXyY"[rel[r][j]]); }
  printf(" pw");
  for (int r = nr0; r < nr; r++) { putchar(' '); for (int j = 0; j < rlen[r]; j++) putchar("xXyY"[rel[r][j]]); printf("^%d", pwe[r]); }
  printf(" edges");
  for (int e = 0; e < m; e++) {
    int u = ends[e][0], p = eport[e][0];
    putchar(' ');
    if (!wlen[u] && !p) putchar('1');
    for (int j = 0; j < wlen[u]; j++) putchar("xXyY"[word[u][j]]);
    if (p) putchar("xXyY"[PL[p]]);
  }
  printf(" cosets %d\n", nc); fflush(stdout);
}
static int sieve(int k, int leaf) {
  ...
  for (int round = 0; pwL && k >= n - pwD && round < 4; round++) {
    ...
    if (!a) break;
    st_pwrel += a;
    if (!(r = enumerate())) { st_pw++; if (k == n) report_pw(); return 0; }
  }
