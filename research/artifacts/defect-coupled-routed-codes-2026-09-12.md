...
## 4. Proposition E: the non-closed-stabilizer localization is vacuous over U

**Proposition E.** Every proper subgroup of `U` fails to be closed in the profinite topology of `U`. So
the localization of `kun-thom-wreath-deficit-witnesses-need-nonclosed-stabilizers` has no analogue over
`U` that could filter codes.
...
Over the Kun–Thom wreath the residually finite actor makes non-closedness a real restriction. Over `U`
it is automatic, just as invisibility is automatic (`Rad(U) = U`). The only filters that bite over `U`
are the nonsoficity of the combining window (Section 2) and the measure obstructions of the compression
artifact.
