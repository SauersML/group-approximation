...
Zaremsky's type (A) theorem is not really about coordinate permutations. The proof of `W` shows
it works whenever the splittings of a Cantor space form a finite-field geometry. The checked
proof uses exactly four inputs:
1. a finitely presented group `Γ` of global structure maps;
2. finitely many `Γ`-orbits on pairs of splitting directions (for cocompactness of `X_m(2)`);
3. finitely generated stabilizers of one direction;
4. a descending-link nerve that is highly connected. For colors this is the boundary of a
   simplex, i.e. the Boolean lattice; for `W` it is the subspace lattice of `F_q^r`, via
   Solomon–Tits.

**The obstruction the method bypasses.** Discrete actions of `GL_n(Z)` have Z-valued pair
invariants, such as the index of `v ∧ w`. These are exactly what killed type (A) for vector
actions (`bh-universal`, 09:30). A splitting of `Z_ℓ^D`, by contrast, only sees
`X/ℓX = F_ℓ^(D)`, so its pair invariants are finite.

**How to reuse it for a new input `G`.** Look for a compact `G`-space whose clopen splittings
factor through a finite (or finite-field) incidence geometry on which `G` has finitely many
orbits on pairs. Its nerve should be Cohen–Macaulay: buildings, matroids, geometric lattices.
Then the Stein–Farley machinery gives a finitely presented full group, and Matui or
Bleak–Elliott–Hyde gives a simple subgroup of finite index.

**The matching necessary check.** The germ cocycle at a point is Zaremsky's quasi-retraction.
It must land in a finitely presented local group; for `W` this is `GL(Z[1/2]; D) ⋊ F`. It is
the first test to run on any new host design.
