...
**SUPERSEDED by `fix-bhmet-char-p-host-tree-word`.** Duplicate fix node for the same module, landed in parallel; its classification and target are merged there. Track this module only on `fix-bhmet-char-p-host-tree-word`.
