...
requires:
  ...
  - weakly-bernoulli-liftable-finite-extensions-are-sofic
artifacts:
  - research/artifacts/gk3-rokhlin-tester-2026-09-14.md
  - research/artifacts/gk3-rokhlin-tester-2026-09-14-part2.md
---

Section 2 of the part 1 artifact. The sofic class contains the sofic groups, so it suffices that each operation
carries sofic groups to sofic groups. Then the sofic class is closed and contains `R`.
...
4. **Finite normal quotients.** On the points whose `N`-orbit is free and good, `τ_ḡ(O(v)) = O(σ_g v)` is well defined
   and injective. It is almost multiplicative because `N` is normal, and almost free because a fixed orbit forces
   `σ_(n^-1 g) v = v`. The bad set has proportion `O(δ)`.
5. **Weakly Bernoulli finite ascent.** `weakly-bernoulli-liftable-finite-extensions-are-sofic`, Corollary T3 of the part
   2 artifact, covers cases (i) and (ii), for every finite normal kernel.
