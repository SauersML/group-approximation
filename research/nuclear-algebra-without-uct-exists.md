...
* At such a prime `r`, the witness crossed product `C = O_2 ⋊ Z_r` is
  ...
  every action on `O_2`, UCT or not.  The realized range is everything.  Every
  countable Z/2-graded `Z[ζ_r, 1/r]`-module is `K_*(A ⋊ Z_r)` for a
  bootstrap-class `A` with contractible restriction.  For every action on `O_2`
  there is a class from such a model inducing an isomorphism on Köhler's complete
  equivariant K-theoretic invariant
...
* **Ozawa's Kazhdan-projection construction** (arXiv:2609.08892, imported as
  `hyperfinite-factor-not-quasidiagonal`).  Corrected 2026-09-12 by lane
  `uct-fresh`: faithfulness is not the obstruction, because the non-quasidiagonal
  trace in that theorem is `τ_R`, which is faithful.  What fails is exactness:
  - R is not exact;
  - by `mf-min-tensor-exact-mf-is-mf`, no separable exact MF subalgebra of R can
    carry the proper isometry;
  - the natural right-leg algebra is not nuclear
    (`ozawa-leg-algebra-carries-non-amenable-trace`).
  Its tensor form, the certificate
  `nuclear-factor-with-infinite-mf-tensor-exists` routed into this claim by
  `nuclear-non-uct-via-infinite-mf-tensor`, is refuted (2026-09-12,
  `mf-min-tensor-exact-stably-finite-is-stably-finite`).  An MF algebra tensored
  with any exact algebra with stably finite unitization is stably finite, so this
