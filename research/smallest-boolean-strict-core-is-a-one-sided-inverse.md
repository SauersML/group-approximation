...
kind: claim
title: The one-coincidence two-by-two table is strict over every alphabet exactly when the reverse table does not repeat the coincidence
distinct_from:
...
So at the Boolean level, strict cores exist in the smallest shape with a coincidence, uniformly
over every alphabet with at least two symbols. The one obstruction is that a one-sided inverse in
a group is two-sided. For group-realized 2 x 2 data the reverse table is always the transpose of
