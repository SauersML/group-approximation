...
* **Ihara--Bass reformulation.**  Not a proof; it relocates the difficulty.
  For a free pair `A, B`, Bass's identity makes "no outlier for
  `A + A^-1 + B + B^-1` in a sector" equivalent to square-root growth of the
  character sums over cyclically reduced words, at lengths `n >> log p`
  (`free-group-ihara-bass-outlier-trace-criterion`).  In the degenerate
  principal series those characters are arithmetic
  (`cubic-division-ihara-traces-count-roots-mod-p`):
  - on `P^2(F_p)` they count the roots of `x^3 - Trd(w) x^2 + Trd(w^-1) x - 1`
    mod `p`, i.e. Frobenius traces of the Artin representations of the cubic
    fields `Q(w)`;
  - on `F_p^3 minus 0` they detect `p | Trd(w^-1) - Trd(w)`.

  So the `P^2` part of this claim is an optimal single-prime Chebotarev
  statement for the fields generated by long words
  (`cubic-division-free-pair-p2-schreier-asymptotically-ramanujan`).
  Effective Chebotarev, even under GRH, reaches only `n = O(log p)` and only
