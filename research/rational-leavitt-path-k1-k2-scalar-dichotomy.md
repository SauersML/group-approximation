...
- it must cut the image of `K_2(Q)` in `K_2(R)` down to a finitely generated group.
  This is necessary with no centrality hypothesis. If `E_N(R)` is finitely presented
  (`N >= 3`), that image is finitely generated by
  `infinitely-generated-symbols-block-fp-elementary-groups`. If `PE_N(R)` is finitely
  presented, then `Z(E_N(R))` is finitely generated
  (`rational-scalars-obstruct-fp-elementary-hosts`, part 2). So `E_N(R)` is an extension
  of a finitely presented group by a finitely generated abelian group, hence finitely
  presented, and the same conclusion follows;
