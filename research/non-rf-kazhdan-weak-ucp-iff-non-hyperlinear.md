...
(ii)  every hyperlinear approximation of Gamma is flexibly HS-correctable;
...
proved directly in `non-rf-kazhdan-weak-ucp-iff-proof`.  For a hyperlinear
`Gamma`, the stronger theorem
`kazhdan-weak-ucp-stability-is-flexible-stability` upgrades `(i)` all the
way to full flexible stability on **every** asymptotic representation.
That stronger conclusion is not asserted on the non-hyperlinear side of
`(NK1)`: there `(i)` and `(ii)` are both vacuous because there is no
hyperlinear approximation, while arbitrary nonseparating asymptotic
representations may still exist.
