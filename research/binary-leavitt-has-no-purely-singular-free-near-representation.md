...
The direct `(TT)/T` conversion is now sharply fenced as well.
`singular-near-defects-vanish-on-canonical-genuine-sector` identifies the
cyclic GNS sector generated by the near unitaries with the genuine regular
representation, but every two-sided translate of every multiplicative defect
is the zero vector on that sector.  On the complementary singular GNS
directions the defects can survive, but the near unitaries need not satisfy
the group law there.  Thus trace-kernel quotienting cannot manufacture the
nonzero quasi-cocycle needed by `(TT)/T`; a positive argument must propagate
the group law to a larger singular sector or construct a coefficient module
by a genuinely different mechanism.
