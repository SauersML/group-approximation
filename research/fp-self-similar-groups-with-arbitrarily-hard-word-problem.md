...
5. **The Wächter--Weiß reduction groups are not finitely presented; the automaton groups
   are open** (bh-free-02, 2026-09-18; **corrected** the same day). Wächter--Weiß
   (arXiv:1906.03424v3, read from the TeX) build PSPACE-complete automaton groups
   `G_R` from a Turing-machine checking mode and a conjunction group `R`.
   - *What is proved* (`finite-commutator-wachter-weiss-groups-are-not-fp`). Take finite
     `R` acting off the encoding letters, for example `A_5`.
     - `G_R` is locally-finite-by-`C`, where `C` is the check-mark automaton group
       acting on the encoding subtree.
     - The subgroup `H_R` generated by the *initial* states carries all reduction words,
       so it is PSPACE-complete. It is locally-finite-by-`Z`, so by Bieri--Strebel it is
       not finitely presented. `H_R` is not self-similar.
   - *Correction.* The first version claimed this for `G_R` itself. That was wrong: the
     intermediate check-mark states are generators too, and they do not act as powers
     of the adder. **Whether `G_R` is finitely presented is open** for every `R`,
     including the binary headline group (`R` = Aleshin.s `F_3`). The paper says nothing
     about finite presentation.
   - *Design lesson (for the reduction groups only).* A finitely presented PSPACE-hard
     group cannot read a finite conjunction group off below a virtually cyclic checking
     group.
