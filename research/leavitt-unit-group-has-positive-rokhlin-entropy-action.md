...
* **Induction along subgroups and treeings (gk-p-inf-f).** `U` is infinite, simple and Kazhdan. By `simple-kazhdan-groups-have-no-proper-co-amenable-subgroups`, no proper subgroup of `U` is co-amenable. Every lower bound induced from a subgroup's entropy theory (amenable, sofic, or free through Bowen's f-invariant) needs co-amenability, so none of them transfers to `U`. Tree and cube splittings are excluded by FA and FW, and whole-relation treeings by Adams–Spatzier (`research/artifacts/rokhlin-lower-bounds-without-finite-models-2026-09-12.md`, Sections 1–3).
