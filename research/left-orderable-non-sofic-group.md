...
**Tables instead of groups (2026-09-17, second pass).**  A witness needs only an order-consistent forbidden table.

- *Reduction, both directions.*  `orderable-nonsofic-iff-order-realizable-forbidden-table`: for `P` in {LO, BO, LI},
  some `P`-group is nonsofic iff some sofic-forbidden finite table embeds in a `P`-group.  The largest `P`-quotient
  of the table group is then a finitely generated, recursively presented witness.  Both halves of a certificate are
  `Pi^0_1`, so each cell is a `Sigma^0_2` sentence.  For LO the witness can be taken simple (Darbinyan--Steenbock).
- *Obstruction.*  `kazhdan-certified-tables-not-order-realizable`: a table that contains an Ozawa
  certificate ball forces (T) on the subgroup generated by the table's generators in every realization.  So it embeds
  in no LI or BO group, and in an LO group only through Navas's Question 3.  This is the finitary form of the dead
  end above, and it exposes what that dead end did not cover.  A Kun--Thom group may have a smaller forbidden
  sub-table that omits the certificate and is order-realizable.
