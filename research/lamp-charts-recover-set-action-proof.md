---
rg: 2
id: lamp-charts-recover-set-action-proof
kind: route
title: Read the labels off the single lamps
...
`j_s(z)=pi_s(a_z)` turn the covariance equation into the orbit-approximation
equation, and the accompanying almost-multiplicative map into `Sym(A)`
witnesses soficity of `G` acting on `X`.
