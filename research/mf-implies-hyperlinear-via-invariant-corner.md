...
The prerequisite is strictly stronger than the target, so this route reduces
rather than renames.  It can fail while `mf-implies-hyperlinear` holds:
`mf-invariant-corner-visibility` demands trace visibility *inside an
operator-norm model*, which is elementwise trace-controlled MF, whereas the
target is satisfied by any Hilbert--Schmidt models whatever.  The converse
implication is unavailable because one cannot combine an operator-norm model
with a Hilbert--Schmidt one: their tensor or direct sum is only
Hilbert--Schmidt asymptotically multiplicative, so hyperlinearity never
returns operator-norm data.  The genuine step performed here is the
domination `||.||_2 <= ||.||_op` together with elementwise assembly.
