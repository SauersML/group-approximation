...
`V=w(U)^2`.  Its projection form returns `Q_U in M_d` satisfying

```text
tr(Q_U)>=alpha/(4-alpha),
||(w(U)^2-1)Q_U||_2=o(1),
```

with no dependence on the activated pencil size.  This is exactly the
unnamed positive-density projection return required by the target.  Then
