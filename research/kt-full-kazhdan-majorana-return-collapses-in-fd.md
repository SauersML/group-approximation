...
representation `rho:E->U(D)` of the Laurent elementary actor and every
`X in M_D`,
...
representation of `E`; a representation of the complete actor `G` is more
than enough by restriction.  Extending `(FKM2)` to approximate actor matrices
with an error controlled by their normalized-Hilbert--Schmidt relator defect
is exactly the still-open source-specific actor-exactification/collapse gate;
no operator-norm or Frobenius stability transfer is asserted here.
