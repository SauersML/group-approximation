...
- **Idempotents are classified (established, unreviewed; sk-algebraic-putnam 2026-09-13).** Every idempotent of `M_n(R)` is similar to a diagonal of clopen indicators (`cantor-crossed-product-idempotents-similar-to-clopen-ones`), and `R` has internal cancellation (`minimal-cantor-crossed-products-have-internal-cancellation`). So the exchange question is exactly the missing input for stable rank one (`cantor-crossed-product-idempotent-pairs-are-stable`, via Camillo–Yu). Idempotents are plentiful and well behaved; what remains open is producing, for a general `a`, an idempotent inside `aR` with complement inside `(1−a)R`.
- **Membership is local for every a (established, unreviewed; sk-exchange 2026-09-14).** `z ∈ aR` iff every column of `z` has an `a`-preimage within one uniform radius (`cantor-crossed-product-right-ideal-membership-is-local`). So exchange witnesses can be checked column by column for any `a`, not only Laurent `a`.
- **Cleanness suffices (standard).** If `a = e + g` with `e² = e` and `g` a unit, then `g(1−e)g^{−1} ∈ aR` is idempotent with complement `geg^{−1} ∈ (1−a)R`. So a proof that `R` is clean would settle exchange. Han–Nicholson (Comm. Algebra 29 (2001)) reduce cleanness to corners `eRe`, `(1−e)R(1−e)`, but here corners over clopen sets are the same kind of ring, so that is circular.
- **Square-zero cut correction (sk-exchange 2026-09-14, artifact part1 §3).** With a tall Kakutani–Rokhlin partition, write `a = D + N` (block compression plus cross-cut entries). If an idempotent `f ∈ A_𝒫` makes `g = D − f` a unit with `g^{−1}` mapping the boundary windows into block interiors, then `(g^{−1}N)² = 0`, and `a = f + g(1 + g^{−1}N)` is clean.
- **The naive block lemma fails (sk-exchange 2026-09-14).** For scalar compressions `D = 0` or `D = 1`, invertibility of `D − f` forces `f ∈ {0,1}`, so `g^{−1}` fixes the windows. This happens over every field and for every scalar block over `F_2`. The general criterion reduces to an `m×m` Schur complement (artifact §3). One such configuration is repaired by taking the pieces straddling the cuts (artifact §4). Open: hybrid gluing with local witnesses `e = a` where `a` is locally idempotent, or an adaptive choice of piece boundaries.
