...
- `cd Γ ≤ 2`, so in any faithful action of `Γ` on `R` at most two pairwise disjoint open
  intervals have nontrivial rigid stabilizers
  (`finite-cd-groups-have-no-micro-supported-line-action`).
...
- (z1-22-lo-simple-bm, 2026-09-13) **Expected answer: no, for the explicit Burger–Mozes
  lattices.** Their factors are boundary-2-transitive, their projections are dense, and
  they act simply transitively on the vertices of `T_1 × T_2`. By the Deroin–Hurtado
  theorem, irreducible lattices in `p`-adic semisimple groups of rank at least two have no
  nontrivial action on the line. The analog for products of trees is
  `irreducible-tree-product-lattices-are-not-left-orderable` (open). It would refute this
  claim for every simple lattice whose projection closures are boundary-2-transitive, but
  not for simple lattices with other local actions.
...
- (z1-22-lo-simple-bm, 2026-09-13) **Circle actions do not reach left orders.** A line
  action compactifies to a circle action with a global fixed point, whose bounded Euler
  class vanishes. So bounded-cohomology rigidity for circle actions (Burger,
  arXiv:0905.0136) says nothing about left orders here.
