...
- **The nonedge anti-relation does not itself descend to one of the `42`
  chart labels.** `nonedge-spectrum-cannot-descend-to-forty-two-chart`
  takes

  ```text
  kappa=[c_(hC),c_(h^(-1)C)],       d=x_21(1).
  ```

  Here `d` lies in the chart stabilizer
  `D=C cap h C h^(-1)`, fixes `hC`, and moves `h^(-1)C`. The cyclic groups
  generated by `kappa` and `d kappa d^(-1)` have trivial intersection by
  graph-product normal form. Consequently every spectral projection
  `E in W^*(kappa)` of trace `t` satisfies the exact identity

  ```text
  ||dEd^(-1)-E||_2^2=2t(1-t).
  ```

  The same limit holds for exact boundary-null spectral cuts in canonical
  matrix microstates. Thus no polynomial or arbitrary Borel functional
  calculus of the single separating word can furnish a proper positive
  `D`-invariant base atom; increasing degree or spectral resolution does not
  help. The next and now minimal raw-algebra target is a projection in the
  joint algebra of the full finite `D`-orbit of pair commutators. It must be
