...
- FSG is not proved.
  - The full shift is not quantum rigid, so `F(T_(A^Λ))` and `N_(A^Λ)` are not finitely presented. FSG asks
    ...
    presentation.
  - `A^Λ` has points with nontrivial stabilizers, and `K_(A^Λ)` may contain relations born there. The weaker
    sufficient statement replaces `A^Λ` by any topologically free subshift `Y ⊇ X`.
