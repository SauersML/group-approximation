...
11. **Kernel removal by enlarging the action: exactly one clause fails for the
    invisible coordinates (2026-09-18, lane `bh-typeA-universal`).** Let `S` be finitely
    presented simple, and let `G ↷ X` be its FFWZ Theorem C actor, with kernel `K` and
    `S ∩ K = 1`.
    - **Reformulation.** By `abstract-twisted-bt-type-fn-criterion`, for any `G`-set `T` the
      twisted Brin--Thompson group of `G ↷ X ⊔ T` is finitely presented iff `G ↷ X ⊔ T` is of
      type `[A_2]`. It is simple iff `K` acts faithfully on `T`. So this question holds for
      `S` as soon as `G` has a subgroup `H` with:
      - `H` finitely generated;
      - `H\G/H` and every `G_x\G/H` (for `x ∈ X`) finite;
      - `core_G(H) ∩ K = 1`.
      Take `T = G/H`. Then `G` itself is a faithful type (A) actor. This covers Attempt 8's
      precomposition targets as the special case `H = Stab(b)`.
    - **The invisible coordinates are the case `H = 1`.**
      `abstract-twisted-bt-group-is-colored-subgroup-of-faithful-one` realizes the abstract
      `SV_G`, which is finitely presented and relatively simple and contains `S` sharply,
      as the `X`-colored subgroup of the faithful `(X ⊔ G)V_G`. That is `T = G` with
      `H = 1`. It meets the stabilizer and faithfulness clauses and fails only the
      double-coset clause, because `G` is infinite.
    - **Precise sub-target.** A finitely generated `H ≤ G` that is "co-finite" in the
      double-coset sense, yet whose core misses the mixed-identity kernel `K`. `H` cannot
      contain a finite-index subgroup of any `G_x ⊇ K`, so it must be transverse to all point
      stabilizers while having finitely many double cosets with each of them.
    - **Normalization (for FFWZ Question 5.9).** `(G, K)` sharply embeds in
      `(SV_G, SK_G)`. `SV_G` acts on the proper clopen subsets of `C^X` with type `[A_2]`:
      it is finitely presented, the stabilizers `D(U) × D(U^c)` are copies of `SV_G`, there
      are finitely many Boolean-pattern orbits on pairs, and the kernel is `SK_G`.
      - So kernel removal may be posed with a finitely presented relatively simple ambient
