...
3. **Nonabelian finite lamps and finitary placements.** *Dead* (2026-09-13).
   - By `kms-configuration-letters-avoid-finite-orbit-placements`, take any action of an
     overgroup of `G(MM_k)` in which the `<a_1, a'_1>`-submodule `V` generated by
     `x(q_j)` acts faithfully. Then `V` has an infinite orbit, or `x(q_j)` moves points in
     infinitely many `V`-orbits.
   - So `x(q_j)` is never a finitary permutation. It never lies in the base of
     `B wr_Omega Q` for a finite lamp group `B`, abelian or not.
   - The lemma, `finite-orbit-actions-carry-no-invertible-difference-module`, is sharp.
     `<x(q_j), a_1, a'_1>` is Baumslag's group `F_p[t^(±1), (t-1)^(-1)] x| Z^2`, which has
     faithful actions satisfying either finiteness hypothesis alone and already lies in
     `B_A`.
   - What remains are placements where `V` has infinite orbits or infinite orbit
     support: translations, or germs.
4. **The natural affine actor.** *Dead* (2026-09-13).
   - `G(MM_k) ≅ T_1 x| M` with `M <= Aut(T_1)`, by KMS Lemmas 4.10, 4.11 and 4.13.
   - The letter automorphisms never shorten words. So the spans `U_n` over words with at
     least `n` a-letters, together with all machine words, are `M`-invariant and strictly
     decreasing.
   - So `M` has infinitely many orbits on `T_1`, and the affine action is not of type (A)
     (`kms-natural-affine-actor-has-infinitely-many-orbits`).
   - Any affine host needs a module with finitely many submodules. Every surviving
     placement is a hard host, so it needs `type-a-actors-with-arbitrarily-hard-word-problem`.
   - Artifact: `research/artifacts/solve-bh-kms-lamps-2026-09-13.md`.
5. **Translations over commutative algebras.** *Dead for the hard members* (2026-09-13).
   - By `commutative-algebra-affine-groups-have-polynomial-word-problem`, every finitely
     generated subgroup of `S x| Aut_R(S)` has polynomial-time word problem. Here `R` is
     any finitely generated commutative algebra over a finite field and `S` any
     finitely generated `R`-module. This covers rings with nilpotents and torsion
     modules, which are not linear over a field.
   - So the class shares the bound `2^n`. By `kms-groups-avoid-commutative-algebra-affine-hosts`,
     some finitely presented residually finite group in `A_p^2 A` embeds in no such
     host: not in `GL_n(R)`, and not in the self-similar hosts `R^N x| E_N(R)` of the
     positive-characteristic linear case.
   - What survives:
     - affine or linear groups over non-commutative algebras carrying the Minsky
       semigroup. `T_1` is a module over the operator algebra generated by right
       multiplications `ρ_d`, the `A`-content projections and `ψ`, and this algebra is
       not commutative.
     - infinite-state self-similar groups;
     - germ placements outside the rational class.
     Any finitely presented type (A) or self-similar host among these settles
     `type-a-actors-with-arbitrarily-hard-word-problem` or
     `fp-self-similar-groups-with-arbitrarily-hard-word-problem`.
   - Artifact: `research/artifacts/solve-kms-self-similar-affine-2026-09-13.md`.
