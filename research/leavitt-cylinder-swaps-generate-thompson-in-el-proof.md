...
property, and no external theorem: the fact that cylinder swaps generate
`V` (also Bleak--Quick, arXiv:1511.02123, Theorem 1.1, whose statement is
not pinned in this repository) follows from simplicity of `V`, which
`thompson-v-not-lef` already cites -- for `h in V` and `k` large enough
that `h` acts on every cylinder `[rho w]`, `[sigma w]` (`w` of length `k`)
by prefix replacement, `h tau_(rho w, sigma w) h^(-1) = tau_(h(rho w), h(sigma w))`
is again a cylinder swap and `tau_(rho,sigma) = prod_w tau_(rho w, sigma w)`,
so the subgroup generated by cylinder swaps is a nontrivial normal
subgroup of the simple group `V`, hence all of `V`.
