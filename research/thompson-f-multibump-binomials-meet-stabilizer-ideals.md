...
**ESTABLISHED.** Let `K` be a field and `R = K[F]`. Let `g` in `F` have bumps
`g_1, ..., g_k` with `k >= 2`: one-bump elements with pairwise disjoint supports
`(p_j, q_j)` and `g = g_1 ... g_k`. Fix an index `i` and put
...
- Not covered: `b` outside `K[H_i × Z_i] F` for every `i`. For example, with two bumps,
  ...
  elements of `H_2 × Z_2` map `p_2` outside `(p_1, q_1)`.
