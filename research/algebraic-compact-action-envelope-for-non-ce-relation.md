...
The restriction can also be made finite and effective.
`mixed-envelope-finitely-extracts-sl3-module-witness` uses locality of
hyperlinearity: write the restricted dual as the directed union of the
`F_p[SL_3(Z)]`-submodules generated by finitely many elements.  If every
finite stage were hyperlinear, their union would be hyperlinear, contradicting
the preceding stripping theorem.  Hence some finitely generated r.e.
`SL_3(Z)`-submodule `A_0` already has `A_0 rtimes SL_3(Z)` non-hyperlinear.
That semidirect product is finitely generated and recursively presented, so
