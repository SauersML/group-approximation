...
**ESTABLISHED (2026-09-17)** through the proof in Attempts 1 (lane `gq-deep-free-1`).
Three referees returned PASS:
- `research/artifacts/gq-referee-a-one-steinberg-group-contains-every-gl-n-q.md`, the
  proof-gap lens;
- `research/artifacts/gq-referee-b-one-steinberg-group-contains-every-gl-n-q.md`, the
  citation lens, which checked the centrality input at arXiv:2004.08551v2;
- `research/artifacts/gq-referee-c-one-steinberg-group-contains-every-gl-n-q.md`, an
  independent re-derivation.

All three PASS verdicts are conditional on the classical `K_2` inputs cited in steps (a)
and (c).
...
- **The ring.** The claim holds for any `S` with `Q` and a Leavitt pair. For the explicit
  host, the reviewed ring `R_L` of `leavitt-resolvent-ring-is-fp-and-contains-q` works
  (route `gl-n-q-explicit-via-st-4-of-resolvent-ring`). So does `D`, which
  `gq-referee-c` reviewed in `gq-referee-c-finitely-presented-divisible-ring-engine.md`.

## Attempts

1. **Proof (lane `gq-deep-free-1`); referees a, b and c returned PASS.**
   ...
   **(a) Block isomorphism over a field.** For `k >= 1` the block map
   ...
   - `St_4(M_k(Q))` is perfect (`x_ij(a) = [x_il(a), x_lj(1)]`), and `K_2(4, M_k(Q))` is
     ...
     is central if n ≥ sr(A) + 2". Here [2] is Dennis, LNM 353 (1973); `gq-referee-b`
     checked the quote in v2. Apply it with `sr(M_k(Q)) = 1` (Bass: semisimple Artinian
     rings have stable rank 1) and `n = 4 >= 3`.
   ...
   **(b) Scalar blocks multiply K_2 by N.** Let `j_N : M_m(Q) -> M_(mN)(Q)`,
   ...
   `b_N` is a homomorphism and the factors have disjoint indices, so they commute.
   Next, `ŝ_c(y) = ŝ_1(y)`. This argument is `gq-referee-c`'s, and it needs no symbol
   calculus. Let `P` be the permutation matrix swapping index block `1` with block `c`.
   The blocks have even size `4m`, so `det P = 1` and `P ∈ SL_(4mN)(Q) = E_(4mN)(Q)`.
   Choose a lift `ω ∈ St_(4mN)(Q)`. Conjugation by `P` sends `x_ij(q)` to
   `x_(σi,σj)(q)` exactly. So `d(y) = ω ŝ_1(y) ω^(-1) ŝ_c(y)^(-1)` lies in `K_2(4mN, Q)`,
   which is central, by the citation in (a) with `sr(Q) = 1`. Centrality gives
   `d(yy') = ω ŝ_1(y) ω^(-1) d(y') ŝ_c(y)^(-1) = d(y') d(y)`, so `d` is a homomorphism
   from the perfect group `St_(4m)(Q)` (`4m >= 4`) to an abelian group, and `d = 1`. For
   `y ∈ K_2(4m, Q)` the element `ŝ_1(y)` is central, so
   `ŝ_c(y) = ω ŝ_1(y) ω^(-1) = ŝ_1(y)`, and `b_N(y) = ŝ_1(y)^N = ŝ_1(y^N)`.
   ...
   **(c) K_2(4m, Q) is torsion.** `K_2(4m, Q) -> K_2(Q)` is injective, since for a field
   ...
   torsion, `Z/2 ⊕ ⊕_(p odd) F_p^x` (Tate). Both appear in Milnor, *Introduction to
   algebraic K-theory*, Annals of Math. Studies 72, §§11–12; they were not re-read there.
   ...
   *Trust surfaces:*
   - the centrality input in (a), checked in Voronetsky v2 but not in Dennis [2];
   - Kervaire's central closedness for `n >= 5`, as quoted in the same paper;
   - Matsumoto and Tate in (c), cited from Milnor and not re-read.

   *Calibration (`gq-referee-c`):* `L_Q(1,3)` has Cuntz families of odd size only, and
   there the symbol `{-1,-1}` survives: `K_2(L) ⊇ K_2(Q)/2` by Ara–Brustenga–Cortiñas. So
   the proof really uses Cuntz families of every size `N`, which the binary pair supplies.
   *Citation-free variants:* `gq-steinberg-q`'s room lemma, Khanh arXiv:2609.08428
   Lemma 5.2, gives centrality one rank up with no citation, at the cost of `St_5`. See
   also `leavitt-pairs-embed-sl-n-q-in-steinberg-groups`.
