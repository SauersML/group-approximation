...
**ESTABLISHED** through `leavitt-resolvent-stable-k2-class-is-computable-proof`. Lane proof, not independently
reviewed. Elementary given the two nodes named above. No priority is claimed.
...
4. **The splitting without (i).** `λ -> {λ, N+1}_N` is a homomorphism `Q^x -> K_2(N, R_L)` and a section of the
   ...
   word problem of `St_N(R_L)` is solvable if and only if nontriviality in `U_N` is recursively enumerable. So (i),
