...
- **Established subclass (2026-09-13).**
  `bk-projection-generated-ideals-with-uct-quotients-force-qd`, item 2: ideals
  `K⊗A` with `A` nuclear and quasidiagonal, every quotient of `A` UCT, and every
  nonzero ideal of `A` generated by a projection.  The proof never pushes a
  class of `A` into a quotient, where singularity can fail.  It kills the
  boundary of `E/R_qd(E)` inside the radical, using a trace on a full corner.
