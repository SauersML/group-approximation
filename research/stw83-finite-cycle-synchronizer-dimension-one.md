...
`E` be a countable row-finite graph with Condition (K).  Suppose that the
cyclic vertices and all edges belonging to directed cycles lie in a finite
subgraph.  Suppose in addition that there is a finite set `H` of vertices such
that:

1. every directed cycle has a path to every `h in H`;
2. every vertex which is reached by at least one directed cycle is reached by
   some `h in H`.
...
the entire cycle-reachable region.  Several hubs permit finitely many
downstream branches.  Cycles in the finite nucleus may be mutually disjoint.
