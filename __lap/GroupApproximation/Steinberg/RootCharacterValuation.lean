...
The analytic half.  `SteinbergRootCharacterValuationBase` defines the valuation
...
namespace SteinbergRootCharacterValuation
...
theorem sum_norm_planeABInteriorLeadingSignSet_sq_le_target_add_error
  ...
  have hmoved :
    ...
    have hnorm : ‖moved q‖ = ‖z‖ := by
      exact (rho (SteinbergGroup.x j i hij.symm
...
theorem sum_norm_planeCBInteriorLeadingSignSet_sq_le_target_add_error
  ...
  have hmoved :
    ...
    rw [sub_le_iff_le_add] at hdiff
    have hnorm : ‖moved q‖ = ‖z‖ := by
      exact (rho (SteinbergGroup.x i j hij
        (FreeAlgebra.ι (ZMod 2) (generatorEnumeration X q)))).norm_map z
    calc
      _ ≤ (‖moved q‖ + ‖z‖) * ‖moved q - z‖ +
          ∑ coarseSign ∈ planeCBForwardInteriorImageSignSet
...
theorem firstCoefficientEigenvalue_oppositeUnitConjugatedRestriction_of_valid
  ...
  have hconj : oppositeUnitConjugatedPlane X i j k hij hik hjk n
    ...
    apply Subtype.ext
    change SteinbergGroup.x j i hij.symm 1 * SteinbergGroup.x i k hik a.1 *
        (SteinbergGroup.x j i hij.symm 1)⁻¹ =
      SteinbergGroup.x i k hik a.1 * SteinbergGroup.x j k hjk a.1
    rw [conjugate_by_coefficient X (ZMod 2) j i k hij.symm hik hjk,
      one_mul]
    exact (SteinbergGroup.x_commute_of_ne j k i k hjk hik
...
theorem secondCoefficientEigenvalue_oppositeUnitConjugatedRestriction
  ...
  have hconj : oppositeUnitConjugatedPlane X i j k hij hik hjk n
    ...
    apply Subtype.ext
    have hcomm := SteinbergGroup.x_commute_of_ne j i j k hij.symm hjk
      hij hjk.symm (1 : SteinbergRootPlaneFourier.FreeRing X) b.1
    change SteinbergGroup.x j i hij.symm 1 * SteinbergGroup.x j k hjk b.1 *
        (SteinbergGroup.x j i hij.symm 1)⁻¹ = SteinbergGroup.x j k hjk b.1
    rw [hcomm.eq]
    simp
  rw [hconj]
  rfl
...
theorem firstCoefficientEigenvalue_forwardUnitConjugatedRestriction
  ...
  have hconj : forwardUnitConjugatedPlane X i j k hij hik hjk n
    ...
    apply Subtype.ext
    have hcomm := SteinbergGroup.x_commute_of_ne i j i k hij hik
      hij.symm hik.symm
        (1 : SteinbergRootPlaneFourier.FreeRing X) a.1
    change SteinbergGroup.x i j hij 1 * SteinbergGroup.x i k hik a.1 *
        (SteinbergGroup.x i j hij 1)⁻¹ = SteinbergGroup.x i k hik a.1
    rw [hcomm.eq]
    simp
  rw [hconj]
  rfl
...
theorem secondCoefficientEigenvalue_forwardUnitConjugatedRestriction_of_valid
  ...
  have hconj : forwardUnitConjugatedPlane X i j k hij hik hjk n
      ...
      firstCoordinate X (ZMod 2) i j k hij hik hjk n b *
        secondCoordinate X (ZMod 2) i j k hij hik hjk n b := by
    apply Subtype.ext
    change SteinbergGroup.x i j hij 1 * SteinbergGroup.x j k hjk b.1 *
        (SteinbergGroup.x i j hij 1)⁻¹ =
      SteinbergGroup.x i k hik b.1 * SteinbergGroup.x j k hjk b.1
    rw [conjugate_by_coefficient X (ZMod 2) i j k hij hjk hik,
      one_mul]
  rw [hconj, hvalid]
  rfl
...
theorem sum_norm_planeRegionSignSet_A_sq_le_B_add_unit_error
    ...
    (∑ sign ∈ planeRegionSignSet X i j k hij hik hjk n .A,
      ...
      (∑ sign ∈ planeRegionSignSet X i j k hij hik hjk n .B,
        ‖planeComponent X i j k hij hik hjk n rho sign z‖ ^ 2) +
      2 * ‖z‖ * ‖rho (SteinbergGroup.x j i hij.symm 1) z - z‖ := by
  classical
  ...
  let moved := rho (SteinbergGroup.x j i hij.symm 1) z
  ...
  rw [sub_le_iff_le_add] at hdiff
  have hnorm : ‖moved‖ = ‖z‖ :=
    (rho (SteinbergGroup.x j i hij.symm 1)).norm_map z
  calc
    _ ≤ ∑ sign ∈ target,
...
theorem sum_norm_planeRegionSignSet_C_sq_le_B_add_unit_error
    ...
    (∑ sign ∈ planeRegionSignSet X i j k hij hik hjk n .C,
      ...
      (∑ sign ∈ planeRegionSignSet X i j k hij hik hjk n .B,
        ‖planeComponent X i j k hij hik hjk n rho sign z‖ ^ 2) +
      2 * ‖z‖ * ‖rho (SteinbergGroup.x i j hij 1) z - z‖ := by
  classical
  ...
  let moved := rho (SteinbergGroup.x i j hij 1) z
  have hsubset : validSource ⊆ fineRestrictionSignSet
      (Nat.card (Plane X i j k hij hik hjk n))
      (Nat.card (Plane X i j k hij hik hjk n)) index target := by
    intro sign hsign
  ...
  rw [sub_le_iff_le_add] at hdiff
  have hnorm : ‖moved‖ = ‖z‖ :=
    (rho (SteinbergGroup.x i j hij 1)).norm_map z
  calc
    _ ≤ ∑ sign ∈ target,
          ‖planeComponent X i j k hij hik hjk n rho sign moved‖ ^ 2 := htransport
    _ ≤ (‖moved‖ + ‖z‖) * ‖moved - z‖ +
...
theorem sum_planeRegionMass_nonzero_le_explicit_errors
          ...
          ((∑ q : Fin (Fintype.card X),
            ...
            ∑ q : Fin (Fintype.card X),
              2 * ‖z‖ *
                ‖rho (SteinbergGroup.x i j hij
                  (FreeAlgebra.ι (ZMod 2) (generatorEnumeration X q))) z - z‖) +
        2 * ‖z‖ * ‖rho (SteinbergGroup.x j i hij.symm 1) z - z‖ +
        2 * ‖z‖ * ‖rho (SteinbergGroup.x i j hij 1) z - z‖ +
        (9 : ℝ) / 2 *
          (planeFirstTopBoundaryMass X i j k hij hik hjk rho z (n + 2) +
            planeSecondTopBoundaryMass X i j k hij hik hjk rho z (n + 2)) := by
  ...
  change planeRegionMass X i j k hij hik hjk rho z (n + 2) .A ≤
    planeRegionMass X i j k hij hik hjk rho z (n + 2) .B +
      2 * ‖z‖ * ‖rho (SteinbergGroup.x j i hij.symm 1) z - z‖ at hA
  change planeRegionMass X i j k hij hik hjk rho z (n + 2) .C ≤
    planeRegionMass X i j k hij hik hjk rho z (n + 2) .B +
      2 * ‖z‖ * ‖rho (SteinbergGroup.x i j hij 1) z - z‖ at hC
...
theorem norm_joinRootMovingProjection_sq_le_explicit_errors [CompleteSpace E]
    ...
    ‖KazhdanFixedSpace.subgroupMovingProjection rho
        (rootSubgroup i k hik ⊔
          rootSubgroup j k hjk) z‖ ^ 2 ≤
      ‖rho (SteinbergGroup.x i k hik 1) z - z‖ ^ 2 +
        ‖rho (SteinbergGroup.x j k hjk 1) z - z‖ ^ 2 +
        (3 : ℝ) / 2 *
          ((∑ q : Fin (Fintype.card X),
              2 * ‖z‖ *
                ‖rho (SteinbergGroup.x j i hij.symm
...
end SteinbergRootCharacterValuation
