...
\hypersetup{
  pdftitle={Property (TT)/T and Coordinate-Block Factorization over Strongly Purely Infinite Rings},
...
\title[Property \texorpdfstring{\((TT)/T\)}{(TT)/T} over strongly purely infinite rings]
{Property \texorpdfstring{\((TT)/T\)}{(TT)/T} and Coordinate-Block
Factorization over Strongly Purely Infinite Rings}
...
\[
  ...
  \qquad m\ge3
\]
\cite[Proposition~9.2.8]{MimuraThesis}.  Relative \((TT)/T\)
...
Assume that \(R\) has a binary Leavitt family, satisfies
\eqref{eq:sandwich}, and satisfies
\(\diag(u,1)\in\EL_2(R)\) for every \(u\in R^\times\).  Then
\[
...
\begin{proof}
Choose a finite free-algebra presentation of \(R\).  Theorem
\ref{thm:root} descends through this quotient and gives relative
...
\(\norm{b(1)}\le D\).  For a nonempty list, apply
\eqref{eq:quasicocycle} to the head and the product of the tail, use
