...
\end{align}
This calculation accounts for the coefficient \(3/2\) in
\eqref{eq:plane-energy}.
...
exhausts \(H_{ijk}\), and the left side of
\eqref{eq:finite-mass} converges to \(\norm{z-Pz}^2\).  Passing to
...
\hline
Theorem~\ref{thm:A} &
\texttt{PropertyTTPaper.coordinateBlock\_factorization} \\
Lemma~\ref{lem:pivot} &
\texttt{PropertyTTPaper.fiveMove\_pivot} \\
Lemma~\ref{lem:plane-energy} &
\texttt{PropertyTTPaper.freeCharacteristicTwo\_planeEnergyEstimate} \\
Lemma~\ref{lem:localized-plane} &
\texttt{PropertyTTPaper.freeCharacteristicTwo\_localizedPlaneEstimate} \\
Lemma~\ref{lem:row-symmetry} &
\texttt{PropertyTTPaper.freeCharacteristicTwo\_rowEmbedding\_controlRoot} \\
Theorem~\ref{thm:root} &
\texttt{PropertyTTPaper.freeCharacteristicTwo\_root\_hasRelativeTT} \\
Lemma~\ref{lem:kazhdan-globalization} &
\texttt{PropertyTTPaper.kazhdan\_normalizedSet\_globalization} \\
Lemmas~\ref{lem:rank-equivalence} and~\ref{lem:gl-el} &
\texttt{PropertyTTPaper.leavitt\_elementaryRankEquivalence},
\texttt{PropertyTTPaper.elementaryGroup\_eq\_generalLinear} \\
Proposition~\ref{prop:rank-four} &
\texttt{PropertyTTPaper.finiteFreeQuotient\_rankFour\_hasTTmodT} \\
Theorem~\ref{thm:B} &
\texttt{PropertyTTPaper.finiteType\_elementaryGroup\_hasTTmodT} \\
Proposition~\ref{prop:diag} &
\texttt{KOnePaper.diagUnit\_mem\_elementary} \\
Theorem~\ref{thm:C} and Corollary~\ref{cor:nonsofic} &
\texttt{PropertyTTPaper.binaryLeavitt\_elementaryGroup\_hasTTmodT},
\texttt{PropertyTTPaper.binaryLeavitt\_elementaryGroup\_hasTTmodT\_and\_not\_isSofic} \\
\hline
\end{tabularx}
