...
\[
  ...
  \qquad(n\ge3).
\]
The proof has two complementary parts: a quantitative root
quasi-cocycle estimate and a Gaussian-elimination argument based on
the single-sandwich property.  We also give a direct tree-theoretic
factorization of the unstable diagonal matrices needed for the
Leavitt specialization.  Together with the previously established
nonsoficity of these elementary Leavitt groups, the result gives
explicit nonsofic groups with property \((TT)/T\).
\end{abstract}
...
nonsoficity theorem gives the following consequence.
...
Corollary~\ref{cor:nonsofic} combines two logically independent
results: Theorem~\ref{thm:C} supplies property \((TT)/T\), while the
previous nonsoficity theorem supplies the other assertion.
...
Within this framework, the new algebraic input is the fixed-coordinate
...
globalize relative quasi-cocycle bounds.  Sections~\ref{sec:quasicocycles}
and~\ref{sec:leavitt} also give direct quantitative constructions for
the root estimate and the unstable diagonal reduction, respectively;
the corresponding qualitative facts can also be recovered from the
earlier structural results just cited.
...
\[
  ...
  \le 2\norm{b(s)}+2D\le B.
\]
A quantitative rank-three plane estimate propagates these finitely
...
For qualitative boundedness, Proposition~\ref{prop:mimura-input}
gives a shorter proof of Theorem~\ref{thm:root}.  Split
...
The proposition has two complementary proofs.  The first places it
inside the known structure theory of purely infinite simple rings;
the second produces the elementary factors directly.

\subsection{A proof from structural results}
...
\subsection{A direct tree-and-pencil factorization}

There is also a direct tree-and-pencil reduction.  Finite prefix
codes encode the self-similar pieces of a unit.  Elementary row and
column moves refine the two codes to a common tree, isolate
code-scalar blocks, and reduce the remaining one-sided factor.  The
output is an actual finite list of elementary matrices whose product
is \(\diag(u,1)\).
...
Thus the direct proof is effective for each given unit.  Extracting
uniform word-length or complexity estimates from the construction is
a separate quantitative problem.
