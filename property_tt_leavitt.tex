...
\begin{proof}
We apply the Fourier argument to the underlying real Hilbert space.
The unitary operators are orthogonal there, and all norms, fixed
spaces, and displacements are unchanged.  Set
\[
 U_{ab}=2\norm z\,d_{ab,1},\qquad
 G_{ab}=\sum_{s\in X}2\norm z\,d_{ab,s}.
\]
...
equal-valuation region.  If \(m_d^S\) denotes the mass in a union
\(S\) of valuation regions, restriction of characters from
\(V_{d+1}\) to \(V_d\) gives
...
coordinates.  Remove the characters whose second coordinate is new
in top degree.  Their mass is at most \(\partial_{2,d+2}\).  For
each remaining character, choose the least free generator in the
leading word of its second coordinate.  The opposite shear
\(x_{ji}(s)\) lowers that leading degree, and restriction to
\(V_{d+1}\) places the resulting character in \(C\cup D\).  The
chosen generator is retained in the receiving functional, so the
receiving sets for distinct generators are disjoint.  Fourier
covariance and the variation inequality above charge the source
mass to \(m_{d+1}^{C\cup D}\), with error
\(2\norm z\,d_{ji,s}\) on the part labeled by \(s\).  Summing over
\(s\in X\) proves~\eqref{eq:AB-descent}.  The forward shear proves
~\eqref{eq:CB-descent}.  Thus generator multiplication is handled by
passing between consecutive stages, with the newly exposed terms
recorded by the two top-boundary masses.
...
\]
This clears \((i,j)\) and preserves the new \(1\) at \((j,j)\).
...
gives the asserted bounds \(2\) and \(3\).  The calculation applies
to arbitrary rings satisfying~\eqref{eq:sandwich}.
\end{proof}
...
Ershov--Jaikin-Zapirain.  It can also be obtained by descending an
explicit finite Kazhdan system for the free algebra.
...
The graph criterion of Abrams--Aranda Pino shows that \(L\) is
purely infinite simple~\cite[Theorem~11]{AbramsArandaPino};
Ara--Goodearl--Pardo's Theorem~1.6 then gives the
single-sandwich property.  It remains to discuss the unstable
diagonal condition.
...
The GE criterion of Menal--Moncasi applies to unital purely infinite
simple rings, as recalled by Ara--Goodearl--Pardo in the proof of
their Theorem~2.4~\cite{MenalMoncasi,AraGoodearlPardo}.
Ara--Goodearl--Pardo's theorem also identifies
\[
  K_1(R)\cong R^\times_{\mathrm{ab}}
\]
for such rings~\cite[Theorem~2.4]{AraGoodearlPardo}.  Thus
\(K_1(L)=0\) makes each
