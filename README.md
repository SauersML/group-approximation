...
The repository holds one `formalization.yaml` and one Comparator configuration per submission: `Palomar/comparator-lix.json` for the K1-injectivity result, `Palomar/comparator-bowen-chapman.json` for the surjunctive non-sofic group, and `Palomar/comparator-pestov91.json`, `Palomar/comparator-type-a2.json`, `Palomar/comparator-guba-thompson.json` and `Palomar/comparator-gkp-commuting-actions.json` for the four results after it. Each is its own `config` for the `Palomar comparator` workflow. They share the Lean development and no mathematics. Everything here is released under Apache-2.0, in `LICENSE`.
