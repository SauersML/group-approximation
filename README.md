...
The XXII submission package is prepared but **not submitted**. Its complete account of sources, reused formalizations, precise scope and the actual Lean proof is in [`formalization.yaml`](formalization.yaml), retained also as [`metadata/palomar-stw-xxii.yaml`](metadata/palomar-stw-xxii.yaml). The selected statements assert separability of the base, the completion presentation, and failure of the trace problem; they do not select the development's additional nuclearity or type-I claims. The Lean route uses real-projective antipodal blocks, the credited [HamSandwich Borsuk–Ulam port](GroupApproximation/ThirdParty/HamSandwich/UPSTREAM.md), and finite self-commutator constraints producing a corona trace. This differs from the complex-projective, character-based account linked above. Codex prepared this submission account and corrected the distinction between printed XXII and the factorial-pair formulation. Exact-revision verification is attached to the prepared revision in GitHub Actions. The [submission guide](Palomar/STWProblemXXIISubmission.txt) describes the selected paths and verification procedure.

The repository holds one Comparator configuration per submission, and `formalization.yaml` describes one submission at a time, currently `Palomar/comparator-stw-xxii.json`; the other configurations receive their metadata when they are submitted. The configurations are `Palomar/comparator-lix.json` for the K1-injectivity result, `Palomar/comparator-surjunctive-nonsofic.json` for the surjunctive non-sofic group, and `Palomar/comparator-pestov91.json`, `Palomar/comparator-type-a2.json`, `Palomar/comparator-guba-thompson.json`, `Palomar/comparator-gkp-commuting-actions.json`, `Palomar/comparator-stw-x1.json`, `Palomar/comparator-stw-xxii.json` and `Palomar/comparator-blanchard-toeplitz.json` for the seven results after it. Each is its own `config` for the `Palomar comparator` workflow. They share the Lean development and no mathematics. Everything here is released under Apache-2.0, in `LICENSE`.
