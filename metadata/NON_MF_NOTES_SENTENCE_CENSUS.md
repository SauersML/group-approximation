...
* **`formalized`**, line 1992 --- Finally, Peter--Weyl point separation gives the compact-target statement.
  * `GroupApproximation.PeterWeyl.separatesPoints` proves point separation from the vendored Tau Ceti density theorem, and `GroupApproximation.five_radicals_eq` gives the compact-target conclusion.
