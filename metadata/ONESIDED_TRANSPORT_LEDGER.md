...
| The Ershov--Jaikin-Zapirain graded-group theorem in the generality the source states it: the union of the six `A_2` root subgroups is a Kazhdan subset with NO exponent, characteristic or finiteness hypothesis, hence for the block roots of `EL_n(R)` in every rank `n >= 3` over EVERY ring, including `Z<X>`. This is one of the two halves of property (T); the other half over `Z` (`ColumnPlaneMassBound`) is NOT proved | `GroupApproximation.generalCoefficientBlockRootSetKazhdan` | closed |
<!-- END-LEDGER-ROWS -->
