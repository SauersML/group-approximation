...
| Maximal-algebra comparison, not a printed claim of this paper: over a strict Kazhdan compression datum the Kazhdan projection exists in `C*_max(G)`, is properly dominated by its `u_t`-conjugate, and `C*_max(G)` is therefore not Dedekind finite, not stably finite, carries no faithful tracial state and is neither residually finite-dimensional nor MF, while the reduced algebra carries a faithful canonical trace and admits no proper compression | `GroupApproximation.MaximalCStarParagraphEndpoint.manuscriptMaximalCStarStrictCompressionRemark` | closed |
| The same comparison with the compression datum discharged, quantified instead over the group-theoretic hypotheses that produce it: a property-(T) subgroup `Γ ≤ G` and an element `t` with `t Γ t⁻¹ ⊊ Γ` | `GroupApproximation.MaximalCStarPrintedHypotheses.manuscriptMaximalCStarRemarkFromPrintedHypotheses` | closed |
| Property (T) for `EL_n(R)` in every rank `n ≥ 3` over every finite-type `ZMod 2`-algebra, via the rectangular block `A₂` system inside the elementary group; the characteristic-zero and general finitely generated coefficient cases of the cited Ershov--Jaikin-Zapirain theorem are not covered | `GroupApproximation.BlockElementary.charTwoGeneralRankElementaryPropertyT` | closed |
| `lem:kazhdan-projection-order` in the printed `≤` form, with `P` built inside the target algebra: no order hypothesis on `B`, the Loewner order installed inside the proposition | `GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptLiteralOneSidedKazhdanProjectionOrder` | closed |
| `lem:kazhdan-projection-order` in the printed `≤` form, with `P` the image of the Kazhdan projection of `C*_max(L)` as the lemma prints it | `GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptLiteralMaximalCStarKazhdanProjectionOrder` | closed |
| Preusser's Theorem 3 for subgroups of `GL_i(R)` NORMALIZED BY `EL_i(R)`, not merely for normal subgroups of `EL_i(R)`: a unique two-sided ideal `I` with `EL_i(R,I) <= H <= C_i(R,I)`, over any ring with the finite right-exchange refinement (equivalent to the exchange condition). The converse direction of the source's classification -- that any such `H` is normalized by `EL_i(R)` -- is NOT formalized | `GroupApproximation.PreusserNormalizedBy.manuscriptPreusserNormalizedByTheorem` | closed |
| Ershov--Jaikin-Zapirain property (T) for `EL_n(A)` in every rank `n >= 3` over every finite-type algebra over every finite field, via an identity-padding homomorphism `EL_3 -> EL_n`; specializes to every ring of prime characteristic. Characteristic zero and non-prime positive characteristic are NOT covered | `GroupApproximation.generalRankFiniteFieldElementaryPropertyT` | closed |
| The Ershov--Jaikin-Zapirain graded-group theorem in the generality the source states it: the union of the six `A_2` root subgroups is a Kazhdan subset with NO exponent, characteristic or finiteness hypothesis, hence for the block roots of `EL_n(R)` in every rank `n >= 3` over EVERY ring, including `Z<X>`. This is one of the two halves of property (T); the other half over `Z` (`ColumnPlaneMassBound`) is NOT proved | `GroupApproximation.A2MagicExponentFree.generalCoefficientBlockRootSetKazhdan` | closed |
