...
| `QuasidiagonalMF.AmenableNuclearInput` | Lance: `C*_r(G)` is nuclear for amenable `G` | **nothing — `CStarExactness.amenableNuclearInput` inhabits it.** Delete this row and its `literaturePackages` line in the commit that first builds green; see below |
...
* `UniversalTorsionFreeHost` became derived on 2026-08-19: it is
  ...
  owed on it shrank from an existence claim quantified over every finitely
  presented torsion-free group to one embedding of one explicitly constructed
  group;
...
**Lance is proved and its row is held only by the retire-on-green rule.**
`CStarExactness.amenableNuclearInput` inhabits `AmenableNuclearInput`; see the
section below.  The line stays in the table and in `literaturePackages` until
the module that proves it builds, because this roster's own rule is that an
entry is retired in the same commit that lands the proof, and an entry that is
deleted ahead of its proof is exactly the conditional-wearing-a-clean-report
shape the gate exists to catch.  When it lands, the count above drops to
**eight**, of which **seven** are other people's theorems.
...
## Lance, discharged — and why the forward direction was reachable at all

The `Analysis/Lance*` family was, until 2026-08-19, entirely the **converse**:
nuclear ⟹ amenable, the direction the manuscript's nonnuclearity paragraph
consumes (`RE.05`).  `AmenableNuclearInput` asks for the forward direction, and
nothing had touched it.

What made it reachable is that the Day/Namioka half was already done, in a
directory nobody looks in for operator algebras:
**`Sofic/AmenableActionSofic.exists_folner`** produces Følner sets from an
invariant mean, by the repository's own Tarski–Hall theorem.  Everything after
that is the standard Følner factorization, and every piece of it was already in
the corpus:

* `folnerIncl` includes `ℂᵏ` as `k` point masses, `compressionLM` compresses
  along it, and the map back is a **Choi map** of the tuple `c_p λ_{e p}⋆`, so
  complete positivity of both maps is `Analysis/CStarChoiMap` and the
  block-operator bridge — no amplification norm `‖T ⊗ 1‖ ≤ ‖T‖` is ever needed;
* the composite is a *scalar multiple of the same translation*, the scalar being
  the Følner overlap, so the estimate is Finset counting: the overlap defect is
  `|∂_g A| / |A|`;
* contractivity — the clause `IsNuclearMap` asks for and no completely positive
  approximation supplies by construction — is Stinespring's estimate applied to
  a *unital* completely positive map, and the repository had already built the
  dilation (`Analysis/CStarUnitalCPContractive`);
* the passage from "approximates translations" to "approximates elements" is
  the `ε/3` argument over `adjoin = span` (`ReducedGroupCStarSpan`), the
  collapse being that `λ(G)` is already a star-closed submonoid so no monomials
  appear.

**The check to run on the first green build.**
`python3 scripts/check_non_mf_unconditional.py --audit-corpus` must stop listing
`AmenableNuclearInput` — `amenableNuclearInput` inhabits it, so that discharge
is the honest kind and nothing has to re-carry the dependency, because the
dependency is gone rather than hidden.  It must **still** list
`TikuisisWhiteWinterInput`, `AmenableUCTInput` and `AmenableMFInput`; if
`AmenableMFInput` also disappears, something has acquired a producer it should
not have, and the amenable-MF route has gone conditional invisibly.

**Two things the first reading of the gap got wrong**, both recorded because
they cost work:

1. *`k > 0` is not needed.*  The gap was read as requiring a positivity clause
   on the block size, on the ground that `‖1‖ = 1` fails over an empty index
   set.  That is true of the *matrix* algebra at the L2 operator norm, and
   false of the operator algebra: Mathlib's `CStarAlgebra (E →L[ℂ] E)` asks for
   no nontriviality.  Factoring through `B(ℂᵏ)` rather than `Mₖ(ℂ)` therefore
   deletes the clause instead of discharging it, and also deletes the
   ⋆-isomorphism `Mₖ(ℂ) ≅ B(ℂᵏ)` that would have had to be transported along.
2. *`cfc_mem` is not the argument.*  Corestricting complete positivity to the
   reduced algebra was read as needing the square root of a positive element of
   a subalgebra to stay in it.  It does need that — but the statement was
   already proved, as `LanceMatrixSubalgebra.exists_entries_mem_factor`, where
   the root is taken inside the C⋆-algebra the closed *entrywise* subalgebra
   carries and ambient positivity descends by spectral permanence.

**What is *not* proved, and why it is not stated either.**  The converse at
this definition — `IsNuclearCStarAlgebra (C⋆_λ G) ⟹ IsAmenable G` — is not
available.  The tempting move is to state it anyway with the missing reduction
as a premise, the way `reducedGroupCStar_not_isNuclearCStar_of_lance` carries
`lance`.  **Do not.**  That row's premise is a named `Prop` about a definition;
the reduction here is a bare logical formula, and
`scripts/check_non_mf_unconditional.py` judges such a premise by the head
symbols it mentions rather than by provability, so a theorem concluding from it
reads as a premise-free producer.  Stated as an `↔` it is worse still: the gate
takes the **left** side of an `↔` as the conclusion, so
`IsNuclearCStarAlgebra … ↔ IsAmenable …` would drop `IsNuclearCStarAlgebra`
from `--audit-corpus` outright.  Both shapes are on the recorded trap list.  The
gap is therefore recorded in prose — here and in the module docstring — and in
no type.

The reduction hides exactly two facts, **neither of them about groups**, and both
were located by reading what `LanceCPApprox.hasInvariantMean_of_cpApprox`
consumes — a *unital* completely positive map on all of `B(ℓ²(G))` with values
in `L(G)`:

1. **The finite-dimensional target must be a block.**  `IsNuclearMap` factors
   through an arbitrary `FinDimCStarAlgebra`; the Arveson extension proved here
   (`Analysis/LanceMatrixArveson`) extends into `B(ℂᵏ)` and nothing else.
   Closing that is injectivity of a finite-dimensional C⋆-algebra — a unital
   embedding into some `B(ℂᵏ)` plus a completely positive retraction — i.e.
   Wedderburn together with the conditional expectation.
2. **Contractive is not unital.**  `IsNuclearMap` asks its maps only for
   `‖α a‖ ≤ ‖a‖`; `NuclearReducedCPAP` and the Arveson step ask for `α 1 = 1`.
   The repair is conjugation by `(α 1 + δ)^{-1/2}` with the defect absorbed
   into the tolerance — continuous functional calculus, not bookkeeping.  It is
   easy to miss, because every *construction* in this lane produces unital maps
   and only the *definition* forgets.

Consequence for the other side of the roster:
`Analysis/LanceNuclearity.reducedGroupCStar_not_nuclear` must keep saying
`NuclearReducedCPAP` and not `IsNuclearCStarAlgebra` — substituting the latter
would weaken the manuscript's non-nuclearity conclusion by exactly those two
