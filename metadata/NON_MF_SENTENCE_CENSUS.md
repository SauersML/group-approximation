...
Completely formalized or definitional: **208/477** (43.6%).
...
* **`unassigned`**, line 1145 --- Choose $s,t\in R$ with $ts=1\ne st$, and let $S$ be the unital subring they generate.
* **`unassigned`**, line 1145 --- The compressor $u$ from the proof of Theorem~\ref{thm:full-defect-ring} lies in $\EL_4(S)$ and conjugates $L=\EL_3(S)$ into itself, and $L$ has property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}.
* **`unassigned`**, line 1145 --- The compression is strict: every off-diagonal entry of a matrix in $uLu^{-1}$ has the form $sat$, and $e(sat)=0$ while $e\cdot1=e\ne0$, so $e_{12}(1)\in L\setminus uLu^{-1}$.
* **`unassigned`**, line 1145 --- By Proposition~\ref{prop:max-infinite} applied to $L\le\EL_n(R)$ and $u$, $C^*_{\max}(\EL_n(R))$ contains a proper isometry.
* **`unassigned`**, line 1145 --- If $R\ne0$ satisfies the hypothesis of Theorem~\ref{thm:full-defect-ring}, take instead the subring $S$ and the map $\Psi$ from the rank-two part of its proof, with $L=\EL_3(S)$ and $u$ the image in $\EL_4(S)$ of the compressor of $B$; here $v_0w_0\ne1$, since $w_1v_0=0$ and $w_1v_1=1$.
* **`unassigned`**, line 1145 --- Then $\Psi$ is injective, since $T_i\jmath(A)S_j=A_{ij}$, so $\Psi(L)\le\EL_2(R)\le\EL_n(R)$ has property~\textup{(T)} and is compressed strictly by $\Psi(u)$, and Proposition~\ref{prop:max-infinite} gives the proper isometry for every $n\ge2$.
* **`unassigned`**, line 1166 --- For the remaining assertions, $G=\EL_n(R)$ is countable and nontrivial, and by Theorem~\ref{thm:full-defect-ring} it is not MF\@.
* **`unassigned`**, line 1166 --- The algebra $C^*_{\mathrm r}(G)$ is separable, and stably finite because its canonical trace is faithful.
...
* **`unassigned`**, line 1504 --- The group $W$ is not MF.
  * printed badge recorded; an overlay assignment is owed because the paper's proof ledger is retired
* **`unassigned`**, line 1504 --- More precisely, every homomorphism from $W$ to an MF group kills $\varepsilon$.
  * printed badge recorded; an overlay assignment is owed because the paper's proof ledger is retired
...
* **`unassigned`**, line 1536 --- The subgroup $\langle\varepsilon\rangle$ is central, so normal, and it is finite, so it has property~\textup{(T)}.
* **`unassigned`**, line 1536 --- So Theorem~\ref{thm:compression-criterion} applies to the countable group $W$ with $L=\Gamma$ and $K=\langle\varepsilon\rangle$: every homomorphism from $W$ to an MF group is trivial on $\varepsilon$.
* **`unassigned`**, line 1536 --- In particular, $W$ is not MF\@.
