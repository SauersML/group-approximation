...
identity in normalized Hilbert--Schmidt norm, and property~\textup{(T)}
of a normal subgroup they generate makes that convergence hold in
operator norm.
...
quotient of $\GL_n(R)$ is $K_1(R)$.  We also construct finitely
presented sofic groups that are not MF\@: one with an exactly computed
MF radical, whose canonical trace is amenable but not quasidiagonal, and
one whose MF quotients are exactly its cyclic quotients.  A further
construction is torsion-free.
\end{abstract}
...
subgroup contains a commutator whose square is a central involution.  Here $\Gamma$ may be any countable
...
normalize one orbit of central summands by its relative rank, which
leaves the covariance error small without correcting it.  This will allow finite perfect lamp groups whose
...
representation $U=(U_n)$ of $G$.  For $g\in L$ the automorphism
$\beta_g=\operatorname{Ad}(\rho(g))|_A$ of $A$ is defined, because
$\rho(L)$ normalizes $A$, and
\begin{equation}\label{eq:finite-algebra-covariance}
 \opnorm{U_n(g)\phi_n(a)U_n(g)^*-\phi_n(\beta_g(a))}
 \longrightarrow0\qquad(a\in A,\ g\in L).
\end{equation}
...
Fix $b\in qA$ and $g\in L$ and put
