...
\fi
\hypersetup{pdftitle={Non-MF groups from one-sided compression},pdfauthor={\paperauthor},pdfsubject={MF groups and operator-norm matrix approximation},pdfkeywords={MF groups, norm matrix coronas, MF radicals, property (T), asymptotic representations}}
...
\begin{document}
\frenchspacing
\title{Non-MF groups from one-sided compression}
\author{\paperauthor}
...
We construct groups for which every homomorphism to an MF group is
trivial.  The obstruction comes from a one-sided compression of a
property-\textup{(T)} subgroup.  For a countable unital ring $R$ with
$ts=1$ and $R(1-st)R=R$, it annihilates every homomorphism from
$\EL_n(R)$ to an MF group, for all $n\ge4$.  In particular,
$H=\EL_{12}(L_{\F_2}(1,2))$ is a finitely generated simple
property-\textup{(T)} example, and $C^*_{\mathrm r}(H)$ is separable,
stably finite, and not MF.  We also construct a sofic non-MF group
$W=K\rtimes\mathbb Z$ with $K$ locally residually finite.  The canonical
trace on $C^*_{\max}(W)$ is amenable but not quasidiagonal.
A further application gives a two-generated, finitely presented,
torsion-free, acylindrically hyperbolic property-\textup{(T)} group with
no nontrivial homomorphism to an MF group.
\end{abstract}
...
An explicit counterexample to the MF approximation problem is
\[
 H=\EL_{12}(L_{\F_2}(1,2)),
\]
...
group along a proper self-embedding is a compressor.  For elementary
groups, a one-sided inverse in the coefficient ring supplies an explicit
compressor.  Fullness of its complementary idempotent makes the
compression defect normally generate the whole elementary group.
Let
...
Here $R(1-st)R$ denotes the two-sided ideal generated by $1-st$.
The ideal condition means that $\sum_j a_j(1-st)b_j=1$ for finitely many
$a_j,b_j\in R$.  A unital ring is \emph{directly finite} if $ts=1$ always implies $st=1$.
Theorem~\ref{thm:full-defect-ring} requires no finite generation of $R$.
It applies to every countable simple unital ring
that is not directly finite and to every Leavitt algebra $L_k(1,m)$
over a countable field, for $m\ge2$
(Corollary~\ref{cor:simple-infinite-ring}).  Simplicity of the elementary
group is a separate feature of the following example.
...
write $\mathcal C_2=\mathcal C_2(V,L)$.  Since $c$ centralizes $L$,
$(V_n(c))\in\mathcal C_2$.  Theorem~\ref{thm:transport} and operator norm
asymptotic multiplicativity give $(V_n(ucu^{-1}))\in\mathcal C_2$.
Consequently,
\[
 \hsnorm{V_n([ucu^{-1},\ell])-1}
 =\hsnorm{[V_n(ucu^{-1}),V_n(\ell)]-1}+o(1)
 \longrightarrow0.
\]
The elements with this property
...
We prove Theorem~\ref{thm:full-defect-ring} using four coordinates:
three for the Kazhdan subgroup and one for its centralizer.
Only the relation $ts=1$ is needed for the compressor; fullness of
$1-st$ enters in the final normal-generation step.
We use indices $1,\ldots,n$ throughout this section.
...
$\begin{psmallmatrix}s&e\\0&t\end{psmallmatrix}$.
Consequently $u=u_3u_2u_1\in\EL_4(R)$ and its inverse are
\[
 u=
 \begin{pmatrix}
 s&0&0&e\\
 ...
 \end{pmatrix}.
\]
Multiplication gives, for $A\in M_3(R)$,
\[
...
There is an elementary centralizer element using the same one-sided
inverse:
\[
 c=[e_{41}(e),e_{14}(t)]
   =\operatorname{diag}(1,1,1,1+et).
\]
The equality follows from $te=0$ and $e^2=e$; also $(et)^2=0$.
Thus $c\in C_G(L)$.  The displayed matrices give
\[
 ucu^{-1}=e_{12}(e).
\]
Taking $\ell=e_{23}(1)$ yields
\[
...
\]
Hence $N=G$, and therefore $\mathfrak D_G(L)=G$.
Theorem~\ref{thm:compression-criterion}, with $K=G$, proves the claim
when $R$ is finitely generated.

For general countable $R$, each finite collection of elementary
coefficients lies in a finitely generated unital subring $S\subseteq R$
containing $s,t$ and all the $a_j,b_j$.
The same equations hold in $S$, so the preceding argument applies to
$\EL_n(S)$.  These subgroups cover $\EL_n(R)$.  The restriction of any
homomorphism $\EL_n(R)\to M$, with $M$ MF, is therefore trivial on every
such subgroup, and hence on all of $\EL_n(R)$.
\end{proof}

The same argument works for finitely many pairs $t_\nu s_\nu=1$ whose
complementary idempotents jointly generate the unit ideal.  Use the same
source $L$ for each compressor.  The resulting roots all lie in
$\mathfrak D_G(L)$, and the two Steinberg commutators above propagate
their coefficients through the unit ideal.
...
\begin{proof}
In the first case choose $ts=1\ne st$; simplicity makes the nonzero
idempotent $1-st$ full.  In the second case use the defining Leavitt
generators $s_1,\ldots,s_m,t_1,\ldots,t_m$, set $s=s_1$, $t=t_1$,
and observe directly that
\[
 t_2(1-s_1t_1)s_2=1.
\]
\end{proof}
...
If $R\ne0$ also satisfies the full-defect hypothesis of
...
they generate.  The displayed compressor belongs to $\EL_n(S)$ and
compresses the property-\textup{(T)} group $L=\EL_3(S)$ into itself.
This compression is strict: every off-diagonal entry of a compressed
matrix is of the form $sat$ and is annihilated on the left by
$e=1-st\ne0$, whereas $e_{12}(1)$ has an off-diagonal entry equal to $1$.
Proposition~\ref{prop:max-infinite} applies.
For the second assertion, $\EL_n(R)$ is countable and nontrivial, and
Theorem~\ref{thm:full-defect-ring} makes it non-MF.
Its reduced group algebra has a faithful canonical trace and is
therefore stably finite.  If an embedding $\iota$ of the reduced algebra into a norm matrix
corona existed, then $g\mapsto\iota(\lambda_g)+(1-\iota(1))$ would
embed the group into its unitary group, a contradiction.
\end{proof}

\subsection{The binary example}

Put $R=L_{\F_2}(1,2)$ and $H=\EL_{12}(R)$.  The Leavitt relations
identify $R$ with $R\oplus R$ as right $R$-modules: the map
$x\mapsto(t_0x,t_1x)$ has inverse $(y,z)\mapsto s_0y+s_1z$.
For the full-defect criterion they give $t_0s_0=1$ and
\[
 t_1(1-s_0t_0)s_1=1.
\]
Thus Theorem~\ref{thm:full-defect-ring} already makes every homomorphism
from $H$ to an MF group trivial.  The remaining ring-theoretic input
establishes simplicity of $H$.
...
by~\cite[Theorem~1.1]{EJZ}, and hence is finitely generated.
Proposition~\ref{prop:simple} gives nontriviality and simplicity, and
Theorem~\ref{thm:full-defect-ring} gives the assertion about MF targets.
Both $C^*$-algebra conclusions follow from
Corollary~\ref{cor:one-sided-ring-maximal}.
\end{proof}
...
Let $N$ be the normal closure of the nontrivial subgroup $S$ in $G_0$.
Apply Lemma~\ref{lem:saturation} with $\Omega=\varnothing$ to obtain a
surjection $\varphi\colon G_0\to Q$, where $Q$ is two-generated, finitely
presented, torsion-free, and acylindrically hyperbolic, and $\varphi(N)=Q$.
In particular $Q$ is nontrivial, and it has property~\textup{(T)} as a
quotient of $G_0$.
