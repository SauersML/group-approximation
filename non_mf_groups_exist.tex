...
The obstruction has a finite, quantitative form even when it kills only
part of the group.  For a finite set $\mathcal R$ of words in $F_m$ put
\[
 \delta_{\mathcal R}(U)=\max_{r\in\mathcal R}\opnorm{r(U)-1},
\]
an empty maximum being zero.

\begin{theorem}[linear certificates for MF-invisible words]
\label{thm:word-linear-certificate}
Let $G=F_m/N$ be a finitely generated group and let $w\in F_m$.  The
image of $w$ lies in $\operatorname{Rad}_{\mathrm{MF}}(G)$ if and only
if there are a finite set $\mathcal R\subseteq N$ and a constant $C$
with
\begin{equation}\label{eq:word-linear-certificate}
 \opnorm{w(U)-1}\le C\,\delta_{\mathcal R}(U)
\end{equation}
for every unitary tuple $U=(U_1,\ldots,U_m)$ in every dimension.  If
$G$ is finitely presented, $\mathcal R$ may be taken to be a set of
defining relators.  No numerical value of $C$ is asserted.
\end{theorem}

\begin{proof}
First a tensor estimate.  Let $A\in\U(d)$ with $t=\opnorm{A-1}>0$.
...
otherwise $k|\theta|<\pi/2+|\theta|\le\pi$.  So $k\theta/2$ lies in
$[\pi/4,\pi/2]$ up to sign, and tensoring an eigenvector for
$e^{i\theta}$ gives
\begin{equation}\label{eq:tensor-amplification}
 \opnorm{A^{\otimes k}-1}\ge\bigl|e^{ik\theta}-1\bigr|
 =2\bigl|\sin(k\theta/2)\bigr|\ge\sqrt2,
 \qquad kt\le k|\theta|\le\pi,
\end{equation}
the last bound because $t=2|\sin(\theta/2)|\le|\theta|$.  Telescoping
the tensor factors gives
$\opnorm{A^{\otimes k}-B^{\otimes k}}\le k\opnorm{A-B}$ for unitaries
$A,B$, and $A\mapsto A^{\otimes k}$ is a group homomorphism, so
$r(U^{\otimes k})=r(U)^{\otimes k}$ for every word $r$.

Suppose no finite $\mathcal R\subseteq N$ and constant satisfy
\eqref{eq:word-linear-certificate}.  Exhaust $N$ by finite increasing
sets $\mathcal R_n$ and choose unitary tuples $U^{(n)}$ with
\[
 t_n=\opnorm{w(U^{(n)})-1}>n\,\delta_{\mathcal R_n}(U^{(n)}).
\]
Amplify $U^{(n)}$ by the exponent $k_n\le\pi/t_n$ that
\eqref{eq:tensor-amplification} supplies for $A=w(U^{(n)})$.  The
amplified tuple displaces $w$ by at least $\sqrt2$, while
\[
 \delta_{\mathcal R_n}\bigl((U^{(n)})^{\otimes k_n}\bigr)
 \le k_n\,\delta_{\mathcal R_n}(U^{(n)})
 <\frac{\pi}{t_n}\cdot\frac{t_n}{n}=\frac{\pi}{n}.
\]
Each fixed element of $N$ lies in $\mathcal R_n$ for large $n$, so its
defect along the amplified tuples tends to zero, and the generator
classes define a corona homomorphism of $G$ that does not kill $w$.
By Lemma~\ref{prop:mf-residual-calculus} the image of $w$ is outside
the MF radical.
...
on $\mathcal R$ tend to zero, so \eqref{eq:word-linear-certificate}
sends the displacement of $w$ to zero, and the corona homomorphism
kills $w$; Lemma~\ref{prop:mf-residual-calculus} gives the assertion
for MF targets.  For the last sentence, write each $r\in\mathcal R$ as
a product of $a_r$ conjugates of defining relators and their inverses;
unitary conjugation preserves the norm, so telescoping bounds
$\opnorm{r(U)-1}$ by $a_r$ times the defect of the defining relators.
\end{proof}

Invisible relations are therefore quantitatively redundant: imposing
them changes neither the matrix representations of a group nor, up to
one constant, the defect that measures how far a tuple is from being
one.

\begin{corollary}[adding invisible relations]
\label{cor:invisible-relations}
Let $G=\langle x_1,\ldots,x_m\mid\mathcal R\rangle$ be finitely
presented, let $\mathcal W$ be a finite set of words whose images lie
in $\operatorname{Rad}_{\mathrm{MF}}(G)$, and put
$A=\langle x_1,\ldots,x_m\mid\mathcal R,\mathcal W\rangle$.  Then
there is a constant $C\ge1$ with
\[
 \delta_{\mathcal R}(U)\le\delta_{\mathcal R\cup\mathcal W}(U)
 \le C\,\delta_{\mathcal R}(U)
\]
for every unitary tuple in every dimension, and $G$ and $A$ have the
same homomorphisms into $\U(d)$ for every $d$.  So they have the same
operator norm stability, up to rescaling the defect by $C$.
\end{corollary}

\begin{proof}
The first inequality is a maximum over a larger set; the second is
Theorem~\ref{thm:word-linear-certificate} applied to each of the
finitely many words of $\mathcal W$.  A homomorphism $G\to\U(d)$ has
finite-dimensional, hence MF, image, so it kills
$\operatorname{Rad}_{\mathrm{MF}}(G)$ and factors through $A$; the
quotient map gives the converse.  The distance from a tuple to that
common set of representations is thus the same for both groups, while
their defect functions differ by at most the factor $C$.
\end{proof}

Applied to the generator words, the theorem gives the linear collapse
of a group with no nontrivial MF image: if every homomorphism from
$G=\langle g_1,\ldots,g_m\rangle$ to an MF group is trivial, then
there are finitely many relations $r_1,\ldots,r_s$ of $G$ and a
constant $C$ with
\[
 \max_i\opnorm{U_i-1}
 \le C\max_j\opnorm{r_j(U_1,\ldots,U_m)-1}
\]
for all $U_1,\ldots,U_m\in\U(d)$ and all $d\ge1$; conversely such an
inequality forces every MF image of $G$ to be trivial.  The finitely
presented group $\langle x_1,\ldots,x_m\mid r_1,\ldots,r_s\rangle$
then has the same property and maps onto $G$, and the $m$-marked
groups satisfying $r_1=\cdots=r_s=1$ form a clopen set all of whose
members have it; so the property is open in the space of marked
groups.  For the two-generated group $Q$ of
Theorem~\ref{thm:torsion-free} the inequality bounds
$\max\{\opnorm{U-1},\opnorm{V-1}\}$ by the defects of finitely many
words in two unitary matrices, uniformly in the matrix size, while the
left regular representation solves the same equations exactly with
$U,V\ne1$.
