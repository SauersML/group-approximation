...
Kazhdan projection of $L$ in that corona.  Then $U^*PU\le P$, because $uLu^{-1}\le L$ has
...
normally generated by an element of order four; and there is a
...
\[
 z=vf+fw+1-f-vfw .
\]
Expanding $z^2$ with $wv=1$ and $fv=wf=0$, every term cancels except
$1$, so $z^2=1$; likewise $zf=vf$.  The factorization
\[
...
$1,2,3$.  The ring $\mathcal C$ is finitely generated, so both groups
...
The element $\sigma=e_{12}(1)e_{21}(-1)e_{12}(1)$ of $B$ has order four,
and $\sigma e_{13}(-r)\sigma^{-1}=e_{23}(r)$ for $r\in\mathcal C$, so
\[
 [e_{13}(r),\sigma]=e_{13}(r)e_{23}(r),\qquad
 [e_{12}(1),[e_{13}(r),\sigma]]=e_{13}(r);
\]
...
$w_iv_j=\delta_{ij}$.  Then $s_i\mapsto v_i$, $t_i\mapsto w_i$ defines a
unital ring homomorphism from $\mathcal C$ onto the subring $S$ of $R$
generated by these elements, so $\EL_4(S)$ is a homomorphic image of
$B$, and every homomorphism from $\EL_4(S)$ to an MF group is trivial.
Put
\begin{gather*}
...
$\Psi(\EL_4(S))\le\EL_2(R)$.  Let $\rho$ be a homomorphism from
$\EL_2(R)$ to an MF group.  Then $\rho\circ\Psi$ is trivial on
$\EL_4(S)$, so $\rho$ kills
$\Psi(e_{12}(1))=\operatorname{diag}(1+S_1T_2,1)$, which normally
generates $\EL_2(R)$ by Lemma~\ref{lem:rank-two} with $v=S_1$, $w=T_1$,
$a=S_2$, $b=T_2$.  So $\rho$ is trivial, and the image of $B$ in
$\EL_2(R)$ contains $\Psi(e_{12}(1))$, so it normally generates
$\EL_2(R)$.
...
and for every $n\ge2$ if $R$ satisfies the hypothesis of
Theorem~\ref{thm:full-defect-ring}.  If $R\ne0$ satisfies that
hypothesis, then $C^*_{\mathrm r}(\EL_n(R))$ is separable, stably
finite, and not MF for every $n\ge2$, and the unit group $R^\times$ is
not MF\@.
\end{corollary}
...
$C^*_{\max}(\EL_n(R))$ contains a proper isometry.  Under the
hypothesis of Theorem~\ref{thm:full-defect-ring}, take instead the subring
$S$ and the map $\Psi$ from the rank-two part of its proof, with
$L=\EL_3(S)$ and $u$ the image in $\EL_4(S)$ of the compressor of
$B$; here $v_0w_0\ne1$, since $w_1v_0=0$ and $w_1v_1=1$.  Then $\Psi$
is injective, since $T_i\jmath(A)S_j=A_{ij}$, so
$\Psi(L)\le\EL_2(R)\le\EL_n(R)$ has property~\textup{(T)} and is
compressed strictly by $\Psi(u)$, and
...
Since $t_0s_0=1$ and $s_0t_0\ne1$, the ring $R$ is not directly finite,
so by Corollary~\ref{cor:one-sided-ring-maximal} the algebra
...
Write $K_1(R)=\varinjlim_n\GL_n(R)/\EL_n(R)$, the limit along
$A\mapsto\operatorname{diag}(A,1)$, and $\kappa\colon R^\times\to K_1(R)$
for the canonical map.

\begin{theorem}[MF quotients of unit groups]\label{thm:mf-quotient-units}
Let $R$ be a countable purely infinite simple ring, let $n\ge1$, and let
$N_n$ be the intersection of the kernels of all homomorphisms from
$\GL_n(R)$ to MF groups.  Then $N_n=[\GL_n(R),\GL_n(R)]$ and
$\GL_n(R)/N_n\cong K_1(R)$, a countable abelian group, which is MF\@.
\end{theorem}
...
residually finite-dimensional and MF~\cite{BK}, and an embedding
$\iota$ of $C^*_{\max}(A)$ in a norm matrix corona restricts to the
embedding $a\mapsto\iota(u_a)+1-\iota(1)$ of $A$ in its unitary group.
So $\kappa$ is a homomorphism to an MF group, and $N\le\ker\kappa$.
...
Let $u\in\ker\kappa$.  Following the proof
of~\cite[Theorem~2.4]{AGP}, choose orthogonal idempotents
$e_1,\dots,e_m$ with sum $1$, $m\ge4$, such that $e_1,\dots,e_{m-1}$
are pairwise equivalent and $e_m$ is equivalent to an idempotent
$f\le e_1$.  Then $R$ is the ring of $m\times m$ matrices over
$T=e_1Re_1$ whose last column has entries in $Tf$ and whose last row
has entries in $fT$, and the argument of Menal and
Moncasi~\cite[proof of Theorem~2.2 and the Remark after
Corollary~2.3]{MenalMoncasi} transforms the matrix of $u$ by elementary
row and column operations into one of the form
$\operatorname{diag}(1,\ast)$.  So $u=PvQ$, where $P$ and $Q$ are
products of elementary matrices $e_{ij}(x)$ of this matrix ring and
$v=e_1+(1-e_1)v(1-e_1)$.  The elementary matrices with $i,j\le m-1$
form $\EL_{m-1}(T)$ inside $(1-e_m)R(1-e_m)\cong M_{m-1}(T)$, so they
lie in $N$ by~\eqref{eq:corner-units}; for distinct $i,j\le m-1$,
\[
...
Finally let $v=e+(1-e)v(1-e)$ with $e\ne0$ and $\kappa(v)=0$.  By the
definition of $K_1$, $\operatorname{diag}(v,1,\dots,1)\in\EL_n(R)$ for
some $n\ge2$.  Following the proof of~\cite[Theorem~2.4]{AGP} again,
choose an idempotent $f<e$ equivalent to $e$ and orthogonal
idempotents $r_2,\dots,r_n\le e-f$, each equivalent to $1$, and put
$r_1=1-e+f$ and $P=r_1+\dots+r_n$.  There is a ring isomorphism
$\theta\colon M_n(R)\to PRP$ under which
$\operatorname{diag}(v,1,\dots,1)$ corresponds to
$(1-e)v(1-e)+f+r_2+\dots+r_n=v-(1-P)$.  So
$v=1-P+\theta(\operatorname{diag}(v,1,\dots,1))\in N$
...
\begin{proof}
For the first assertion, let $E\subseteq N$ be finite, put
$N_0=\langle E\rangle$, and choose a finite quotient
$\theta\colon N_0\to Q$ with $\theta(x)\ne1$ for $x\in E\setminus\{1\}$;
put $H=\ker\theta$.  In the quasi-regular representation of $N$ on
$\ell^2(N/H)$, the finite-dimensional subspace $\ell^2(N_0/H)$ is
$N_0$-invariant, and compression to it is a u.c.p.\ map $\phi_E$ on
$C^*_{\max}(N)$ that restricts to a representation of
$C^*_{\max}(N_0)$.  For $x\in E\setminus\{1\}$, the element $x$ fixes no
coset $nH$ with $n\in N_0$, because $n^{-1}xn\in H$ would give $x\in H$
by normality of $H$ in $N_0$; so $\tr(\phi_E(u_x))=0$.  Along an
exhaustion of $N$ by finite sets $E$, the maps $\phi_E$ are eventually
multiplicative on each pair of canonical unitaries, and their traces
converge to $\tau_N$ on every canonical unitary; density of the group
algebra and contractivity extend both limits to $C^*_{\max}(N)$.

For the second assertion, write $g\mapsto\bar g$ for the quotient map
