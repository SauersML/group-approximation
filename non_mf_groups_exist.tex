...
by the same results $\GL_n(R)=\EL_n(R)\cong R^\times$ for every $n\ge2$,
so every $\EL_n(R)$ is isomorphic to $H$, and $12$ is the rank checked in
the Lean development.  OpenAI used this group to construct the first
nonsofic group~\cite[Chapter~3]{OAI}.
...
not discover the key MF result.  The construction entered the public
repository between August~8 and~12, 2026, the first manuscript draft
is dated August~12, experts were consulted from August~13, and the
Lean development was registered at Palomar on August~24
...
corresponding declarations; the development uses Lean~4 toolchain
\texttt{v4.32.0} and Mathlib \texttt{v4.32.0}.
Theorem~\ref{thm:full-defect-ring} and Section~\ref{sec:torsion-free}
have no Lean counterpart.
