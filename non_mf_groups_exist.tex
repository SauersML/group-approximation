...
subgroup conjugated into a proper subgroup of itself, together with a
property-\textup{(T)} subgroup of the resulting commutator defect that is
normal in the whole group.
The unit group of the binary Leavitt algebra $L_{\F_2}(1,2)$ is a finitely
generated simple property-\textup{(T)} group with no nontrivial
homomorphism to an MF group, and its reduced $C^*$-algebra is separable
and stably finite but
not MF\@.  We also construct a sofic group that is not MF, whose canonical
trace on the maximal group $C^*$-algebra is amenable but not
quasidiagonal, and a finitely presented torsion-free group with
property~\textup{(T)} and no nontrivial homomorphism to an MF group.
\end{abstract}
...
$G/\operatorname{Rad}_{\mathrm{MF}}(G)$ is the largest MF quotient of $G$
(Section~\ref{sec:compression-radical}).
...
\end{equation}
For every homomorphism $f\colon G\to G'$,
\begin{equation}\label{eq:defect-functorial}
 f(\mathfrak D_G(L))\le\mathfrak D_{G'}(f(L)),
\end{equation}
...
The proof has two steps.  First, the conjugation maps
$\operatorname{Ad}(V_n(g))$ of an operator norm asymptotic representation
form a homomorphism into a norm matrix corona, in which the Kazhdan
projection $P$ of $L$ and the class $U$ of $(\operatorname{Ad}(V_n(u)))$
satisfy $U^*PU\le P$; stable finiteness of the corona forces equality,
so conjugation by $u$ preserves the Hilbert--Schmidt asymptotic commutant
of $L$, and every element of $\mathfrak D_G(L)$ tends to $1$ in normalized
Hilbert--Schmidt norm (Corollary~\ref{cor:defect-hs}).  Second, let
$K\le\mathfrak D_G(L)$ be a normal property-\textup{(T)} subgroup and let a
corona homomorphism be nontrivial on $K$.  It compresses to a corner on
which the Kazhdan projection of $K$ vanishes, and the corner, renormalized
by its own rank, carries an operator norm asymptotic representation to
which the first step applies.  Its limiting trace is then the trivial
...
apply to sofic or hyperlinear approximations; the sofic group of
Section~\ref{sec:amenable-nonqd} shows that its conclusion fails there.}
...
generates all of $\EL_4(R)$.  It suffices to do this once, in
the universal unital associative ring
$\mathcal C=\mathbb Z\langle s_0,s_1,t_0,t_1\rangle/
(t_is_j-\delta_{ij}:i,j\in\{0,1\})$, since $\EL_4(\mathcal C)$ maps
...
$H\cong\EL_4(R)$, and $H$ is nontrivial, finitely generated, and simple,
and has property~\textup{(T)}.  Every homomorphism from $H$ to an MF
group is trivial.  So $H$ is not MF, and
...
The obstruction also computes entire MF radicals in a different family.
For a homeomorphism $T$ of a compact metrizable zero-dimensional space
$X$, let $R_X$ be the algebraic crossed product of the locally constant
functions over a finite field by $T$.  Section~\ref{sec:chain-core}
identifies restriction to the chain-recurrent set as the universal
directly finite ring quotient, and proves that $R_X$ is directly finite
exactly when $\GL_3(R_X)$ is MF\@.  Over $\F_2$, the MF radical is
exactly the locally finite restriction kernel for $\GL_n$, $n\ge3$,
and $\EL_n$, $n\ge4$, with LEF quotients.  If $T$ is aperiodic, the
same conclusion holds already for the unit group and for every
...
\leanverified{Manuscript/NonMF/Full/AmenableCountable/TraceCountable}{GroupApproximation.Full.NM08.manuscriptAmenableNonquasidiagonalTraceCountable}
\end{mainthm}
...
\leanverified{Manuscript/NonMF/Full/Korchagin/PrintedSentence}{GroupApproximation.Full.NM09.manuscriptSentence_korchaginDirectLimit}
\leanverified{Manuscript/NonMF/Full/AmenableCountable/TraceCountable}{GroupApproximation.Full.NM08.manuscriptMFNotClosedUnderIntSemidirectCountable}

Every example above has torsion, and in the lamp construction the
obstruction is itself a torsion element.  The last group has none.
...
finite, and the obstruction is a commutator $[ucu^{-1},\ell]\ne1$
that every operator norm asymptotic representation sends to $1$ in
Hilbert--Schmidt norm; Theorem~\ref{thm:normal-kazhdan} then shows that
every corona homomorphism is trivial on each normal Kazhdan subgroup of
the defect.  The group $H$ is the group of~\cite{OAI}, and
...
relation replaces the stability hypothesis.  Eckhardt's
note~\cite{Eckhardt} gives a short, elementary proof that certain
...
MF quotient of $G$: enumerate its nontrivial elements and separate each by
an MF model, and at stage $n$ take the direct sum of the first $n$ models,
each carried far enough along its own sequence to have multiplicative
defect below $1/n$ on the first $n$ elements while still separating its
own element.  This family is asymptotically multiplicative and separates
every nontrivial element.
...
projection commuting with \(\rho(G)\).  Discard the coordinates on which
a fixed projection lift of \(q\) vanishes; infinitely many remain, and on
them there are ranks \(r_n\ge1\), an
...
$e_{12}(vR)\le N$.  Put $f=1-vw$, an idempotent with $fv=0=wf$.  Since
$r=vwr+fr$, it remains to put $e_{12}(fR)$ in $N$, and the involution
\[
 z=vf+fw+1-f-vfw
\]
does this by conjugation, since $zf=vf$.  Put $x=vf$, $y=fw$, $q=vfw$, and $c=1-f-q$, so that $z=x+y+c$.  From
...
with $a\in R$.
\leanverified{Manuscript/OneSidedMFRadical/RankFourCompressionCellPrinted}{GroupApproximation.Manuscript.OneSidedMFRadical.RankFourCompressionCellPrinted.manuscriptRankFourCompressionCell}
\end{lemma}
...
\leanverified{Manuscript/NonMF/Full/RankTwoElementary/PrintedSentence}{GroupApproximation.Full.NM10.printedOneSidedRingMaximalGroupClausesRankTwo}
\end{corollary}
...
group is not MF, and MF passes to subgroups.  So $R^\times$ is not MF\@.
\end{proof}

\begin{proposition}[torsion complementary idempotents]\label{prop:torsion-defect-ring}
Let $R$ be a countable unital ring, let $s,t\in R$ satisfy $ts=1$, and
suppose that $e=1-st$ has finite additive order.  Then for every $n\ge4$,
\[
 \EL_n(R,ReR)\le\Rad_{\mathrm{MF}}(\EL_n(R)),
\]
where $\EL_n(R,ReR)$ is the normal closure of the $e_{ij}(a)$ with $a\in ReR$.
In particular $\EL_n(R)$ is not MF when $e\ne0$, and if $R$ has positive
characteristic, then $\EL_4(R)$ MF implies that $R$ is directly finite.
\leanverified{Manuscript/OneSidedMFRadical/TorsionComplementaryIdempotents}{GroupApproximation.Manuscript.OneSidedMFRadical.TorsionComplementaryIdempotents.manuscriptTorsionComplementaryIdempotents}
\end{proposition}

\begin{proof}
If $e=0$, then $ReR=0$ and $\EL_n(R,ReR)=1$, so assume $e\ne0$.
Let $S$ be the unital subring generated by $s,t$, and let $L=\EL_3(S)$
occupy coordinates $1,2,3$ of $\EL_4(S)$; it has
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}.  The criterion is applied
to a subgroup $B\le\EL_4(S)$ with a central element of finite order inside
$\mathfrak D_B(L)$.  Put $f_{ij}=s^iet^j$.
Since $es=te=0$, $f_{ij}f_{kl}=\delta_{jk}f_{il}$, so $f_{ij}^2=0$ for
$i\ne j$.  Write $D(a)=\operatorname{diag}(1,1,1,a)$; for $i\ne j$,
$D(1\pm f_{ij})=[e_{41}(f_{ii}),e_{14}(\pm f_{ij})]$ lies in $\EL_4(S)$.
Let $u$ be the compressor of Lemma~\ref{lem:ring-compression-cell}, put
$w_{ij}=(1+f_{ji})(1-f_{ij})(1+f_{ji})$, $r=w_{14}w_{25}$, and $v=uD(r)$.
Since $D(r)$ centralizes $L$, $vLv^{-1}\le L$.  Put
\[
 \begin{gathered}
 c=D(1+f_{02}),\qquad y=e_{42}(f_{10}),\qquad z=D(1+f_{12}),\\
 \ell=e_{21}(1),\qquad B=\langle L,v,c,y\rangle .
 \end{gathered}
\]
Then $c\in C_B(L)$, and from $rf_{02}r^{-1}=f_{05}$, $rf_{12}r^{-1}=f_{45}$,
$f_{02}t^3=f_{05}$, and $t^3f_{45}=f_{12}t^3$, direct multiplication with
the displayed $u$ gives $vcv^{-1}=e_{14}(f_{02})$ and $vzv^{-1}=z$.  So
$d=[vcv^{-1},\ell]=e_{24}(-f_{02})$ lies in $\mathfrak D_B(L)$, and
$[y,d]=D(1-f_{12})=z^{-1}$ since $f_{02}f_{10}=0$ and $f_{10}f_{02}=f_{12}$;
hence $z\in\mathfrak D_B(L)$.  The element $z$ commutes with $L$ and $v$,
and with $c$ and $y$ because $f_{12}f_{02}=f_{02}f_{12}=f_{12}f_{10}=0$, so
$\langle z\rangle$ is central in $B$.  If $me=0$ then
$z^m=D(1+mf_{12})=1$, and $z\ne1$ because $tf_{12}s^2=e\ne0$.  So
$\langle z\rangle$ is a nontrivial finite central subgroup of $B$ inside
$\mathfrak D_B(L)$, and Theorem~\ref{thm:compression-criterion} applied to
$B$ kills $z$ under every homomorphism from $B$, hence from $\EL_n(R)$, to
an MF group.  Then
\[
 [z,e_{41}(f_{20})]=e_{41}(f_{10}),\qquad [e_{24}(t),e_{41}(f_{10})]=e_{21}(e)
\]
are killed, hence every $e_{ij}(aeb)$ by the Steinberg relations and
signed permutations, hence $\EL_n(R,ReR)$.  For $e\ne0$, $e_{21}(e)\ne1$.
If $R$ has characteristic $p>0$ then $pe=0$ for every such $e$, and the
contrapositive gives direct finiteness.
\end{proof}

The Toeplitz--Jacobson algebra $J=\F_2\langle s,t\mid ts=1\rangle$ has
$1-st\ne0$ of order two, while $J/J(1-st)J\cong\F_2[z,z^{-1}]\ne0$, so
$1-st$ is not full and Theorem~\ref{thm:full-defect-ring} does not apply;
Proposition~\ref{prop:torsion-defect-ring} shows that $\EL_n(J)$ is not MF
for $n\ge4$.  For $J$ the inclusion of
Proposition~\ref{prop:torsion-defect-ring} is an equality.  The algebra $J$ acts
faithfully on $V=\F_2^{(\mathbb N)}$ by the shift, $e=1-st$ is the
projection onto the first vector $b_0$ of the standard basis $(b_j)$, and
$JeJ$ is the ring of matrices with finitely many nonzero entries in this
basis.  Let $\GL_{\mathrm{fs}}(V^n)$ be the group of automorphisms $g$ of
$V^n$ such that $g-I$ has finite matrix support in this basis, a condition
stronger than finite rank of $g-I$.  The kernel of
$\EL_n(J)\to\EL_n(\F_2[z,z^{-1}])$ lies in $\GL_{\mathrm{fs}}(V^n)$.  Over
$\F_2$ every element of $\GL_{\mathrm{fs}}(V^n)$ is a product of
transvections, and each transvection is a relative root $e_{ij}(f_{uv})$
or, inside one coordinate, the commutator
$[e_{ik}(f_{uw}),e_{ki}(f_{wv})]$ of two.  So $\EL_n(J,JeJ)$ is this
kernel; the quotient $\EL_n(\F_2[z,z^{-1}])$ is residually finite, so
MF, and
\[
 \Rad_{\mathrm{MF}}(\EL_n(J))=\EL_n(J,JeJ)\cong\bigcup_N\mathrm{SL}_N(\F_2)
 \qquad(n\ge4),
\]
an infinite simple locally finite group.  The rank four is three plus
one.  Every countable subgroup of $\mathrm{GL}_2$ over a field has the
Haagerup property~\cite[Theorem~4]{GHW}, and the kernel of
$\mathrm{GL}_2(J)\to\mathrm{GL}_2(\F_2[z,z^{-1}])$ is locally finite, so a
Kazhdan subgroup of $\mathrm{GL}_2(J)$ has finite image in
$\mathrm{GL}_2(\F_2(z))$ and a locally finite subgroup of finite index,
and is finite.  A finite subgroup $L$ of a group $G$ admits no proper
compression, since $uLu^{-1}\le L$ forces $uLu^{-1}=L$; then
$uC_G(L)u^{-1}=C_G(L)$ and $\mathfrak D_G(L)=1$, so
Theorem~\ref{thm:compression-criterion} gives no obstruction at rank two.
Three coordinates carry
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}, and the fourth makes the
compression by $s$ invertible.  The two results are complementary:
fullness gives trivial MF images at every rank $n\ge2$, torsion of the
defect gives non-MF at rank four with no fullness.
...
\subsection{MF quotients of unit groups}
...
\leanverified{Manuscript/OneSidedMFRadical/MFQuotientUnitsKappaProof}{GroupApproximation.MFQuotientUnitsKOne.manuscriptMFQuotientUnitsKOne}
\leanverified{Manuscript/OneSidedMFRadical/MFQuotientCanonicalKOne}{GroupApproximation.MFQuotientUnitsKOne.manuscriptMFQuotientUnitsKOneAtBaseRing}
\end{theorem}
...
For the converse, a unit in $\ker\kappa$ is reduced modulo $N$ to a unit
of the form $e+(1-e)v(1-e)$, which is then the image of a stabilized
elementary matrix under a corner isomorphism.  We use
Theorem~\ref{thm:full-defect-ring} in the following form.  Every nonzero idempotent $e$ of $R$ is
...
of $m\times m$ matrices over $T=e_1Re_1$, with identity
$\operatorname{diag}(1_T,\dots,1_T,f)$, whose last column has entries
...
Let $u\in\ker\kappa$ and write $u=gvh$ as in (a).  The elementary
...
$H=R^\times$.  Then $H\cong\GL_d(R)$, and under this identification
$\Rad_{\mathrm{MF}}(H)=[H,H]=\EL_d(R)$ and
\[
 H/\EL_d(R)\cong K_1(R)\cong k^\times/(k^\times)^{d-1}.
\]
\leanverified{Manuscript/NonMFSentences/LeavittKOneFormulaSentences}{GroupApproximation.LeavittKOneFormulaSentences.manuscriptLeavittMFQuotientFull}
\end{corollary}
...
\section{Chain recurrence and exact MF radicals}
\label{sec:chain-core}
...
let $T\colon X\to X$ be a homeomorphism.  For a finite field $k$, put
\[
 R_X=\operatorname{LC}(X,k)\rtimes_T\mathbb Z,
 \qquad ufu^{-1}=f\circ T^{-1}.
\]
Here $\operatorname{LC}$ denotes locally constant functions and the
crossed product is algebraic: its elements have unique finite Laurent
expansions $\sum_j f_j u^j$.  The powers of $u$ remain formal even if
$T$ has finite order.  A ring is \emph{LEF} if every finite subset
admits an injective map into a finite ring preserving the additions
and multiplications that stay in that subset, and preserving $0$ and
$1$ when present.  A group is LEF with the analogous multiplicative
definition.  Finite inverse-pair tables show that LEF rings are stably
finite.  Entrywise finite ring models, including the entries of
inverse matrices, show that their general linear groups are LEF.
Countable LEF groups are MF, by the regular representations of the
finite groups in their local models.

For a compatible metric on $X$, a point $x$ is \emph{chain recurrent}
if, for every $\varepsilon>0$, there are $r\ge1$ and
$x=x_0,x_1,\dots,x_r=x$ with
$d(Tx_i,x_{i+1})<\varepsilon$.  Write $Y=\operatorname{CR}(T)$ for
the chain-recurrent set.  We first construct this set in a form that
also supplies finite ring models.

\begin{lemma}[the chain-recurrent core]\label{lem:chain-core-models}
The set $Y$ is nonempty, closed and invariant, and $R_Y$ is LEF.
The complement $X\setminus Y$ is covered by translates of sets
$P\setminus T(P)$, where $P$ is clopen and $T(P)\subseteq P$.
Each of these sets is wandering: its distinct integer translates
are pairwise disjoint.
\leanverified{Manuscript/NonMF/Full/ChainCoreModels/PrintedLemma}{GroupApproximation.Full.NM12.printedChainCoreModels_closed}
\end{lemma}

\begin{proof}
First suppose $X$ is a subshift over a finite alphabet.  Its word
graph at level $r$ has the length-$r$ words as vertices and the
length-$(r+1)$ words as edges, from prefix to suffix.  Call an edge
cyclic if it lies on a directed cycle, and say the subshift satisfies
the cycle condition if every edge of every word graph is cyclic.

A subshift satisfying the cycle condition has an LEF crossed product.
Indeed, fix a finite ring table, express its entries in Laurent form,
and choose a window determining all coefficient functions and their
translates used in that table.  At a sufficiently high word-graph
level, choose cycles containing words witnessing every nonzero
coefficient of every difference between distinct entries.  Reading
these cycles gives finite periodic models whose relevant windows
all belong to the subshift.  Repeat each cycle so its length exceeds
twice the largest absolute Laurent degree appearing in the table.
On the corresponding finite cyclic sets, evaluate coefficients as
diagonal matrices and $u$ as the cyclic shift matrix.  The allowed
windows preserve the specified products and sums; the separated
cyclic diagonals and the chosen witnesses distinguish distinct
entries.  Their product is the required finite matrix-ring model.
The periodic models need not themselves belong to the subshift.

Retain only the cyclic edges at level $r$, and let $Z_r$ be the
subshift read from bi-infinite paths in this graph.  Every finite
path in the retained graph lies in a strongly connected component
and extends to a periodic path.  Thus $Z_r$ has dense periodic points
and satisfies the cycle condition.  It is nonempty, and
$Z_{r+1}\subseteq Z_r$, since a higher-level cycle projects to a
closed walk at a lower level.  Put $Y_0=\bigcap_r Z_r$.  Compactness
makes $Y_0$ nonempty, and all its words belong to $X$, so $Y_0\subseteq X$.
At each fixed length the decreasing finite languages of $Z_r$
stabilize to the language of $Y_0$.  Consequently $Y_0$ satisfies the
cycle condition too.  It is precisely the set of points all of whose
finite words give cyclic edges in the word graphs of $X$.

Closed approximate orbits at a prescribed window scale give closed
walks through the corresponding word.  Conversely, a cycle through
a sufficiently long word centered at $x$ gives a closed approximate
orbit at $x$: realize successive edges by points of $X$, using $x$
at the initial and final positions.  Overlap of the words gives the
required accuracy.  Hence $Y_0=\operatorname{CR}(T)$.

A noncyclic edge $a\to b$ gives a forward-closed set of vertices:
those reachable from $b$, which excludes $a$.  Its cylinder union
$P$ satisfies $T(P)\subseteq P$.  If $x$ starts with that edge, then
$Tx\in P\setminus T(P)$.  These defects are wandering by nesting of
the translates of $P$.

For general $X$, choose refining finite clopen partitions
$\mathcal P_m$ with mesh tending to zero.  Let $\pi_m\colon X\to X_m$
be their itinerary subshifts, and let $Y_m$ be the cores just
constructed.  Refinement gives one-block factor maps
$\rho_{lm}\colon X_l\to X_m$.  Such maps preserve the cycle
condition, since cycles map to closed walks.  Thus
$\rho_{lm}(Y_l)\subseteq Y_m$, and
\[
 Y_* =\bigcap_m\pi_m^{-1}(Y_m),\qquad
 \pi_m(Y_*)=\bigcap_{l\ge m}\rho_{lm}(Y_l).
\]
The second equality follows from compactness in the fibers.  The
sets on its right decrease and satisfy the cycle condition; finite
language stabilization proves that their intersection does too.
Coefficient pullback gives injective unital maps of crossed products,
and every locally constant function is constant on a sufficiently
fine partition.  Thus $R_{Y_*}$ is an increasing union of LEF rings
and is LEF.

Chain recurrence passes to factors, so $Y\subseteq Y_*$.  Conversely,
if $x\in Y_*$, choose $\mathcal P_m$ with mesh below $\varepsilon$.
A sufficiently accurate closed chain at $\pi_m(x)$ makes the
zero-coordinate symbol of each next point equal the one-coordinate
symbol of the preceding point.  Lift the intermediate points
arbitrarily to $X$, with both endpoint lifts equal to $x$.  Consecutive
lifts satisfy the $\varepsilon$-chain condition because they lie in
the same partition atom after shifting.  Hence $Y_*=Y$.

Pull back the symbolic defects to cover $X\setminus Y$ by their
translates.  They avoid $Y$: for any compressed clopen $P$, set
$p=1_P$, $d=1_{P\setminus T(P)}$ and calculate
...
Call an algebra \emph{locally matricial} if every finite subset is
contained in a finite product of full matrix algebras over its
coefficient field; these subalgebras may have their own local identities.

\begin{lemma}[the transient ideal]\label{lem:transient-matrices}
Restriction fits into an exact sequence
\[
 0\longrightarrow I\longrightarrow R_X\longrightarrow R_Y
 \longrightarrow0,\qquad
 I=\operatorname{LC}_c(X\setminus Y,k)\rtimes_T\mathbb Z.
\]
The ideal $I$ is locally matricial and is generated by the indicators
$1_{P\setminus T(P)}$ of the defects in
Lemma~\ref{lem:chain-core-models}.
\leanverified{Manuscript/NonMFSentences/TransientMatricesClosed}{GroupApproximation.Manuscript.NonMFSentences.TransientMatrices.printedTransientMatrices_closed}
\leanverified{Manuscript/NonMF/Full/ClopenShrink/TransientMatricesCompact}{GroupApproximation.Full.NM11.printedTransientMatrices_compact_closed}
\end{lemma}
...
functions compactly supported in the complement.  Laurent normal
forms give exactness.  The translates of the defects cover that
complement; a finite clopen subdivision subordinate to this cover
on each compact coefficient support proves the ideal-generation claim.

Put $U=X\setminus Y$.  A compact subset $C\subset U$ has a finite
cover by wandering clopen sets, say $m$ of them, so each orbit meets
$C$ in at most $m$ points.  Also $U$ has no periodic points.
Express a prescribed finite subset of $I$ as linear combinations of
characteristic functions of compact clopen partial shift graphs.
...
between related points to at most $m-1$ steps.  Freeness on $U$
makes the arrow between related points unique.  The generated
relation therefore consists of finitely many partial shifts with
compact clopen domains.
...
levels.  The characteristic arrows between these levels form a copy
of $M_h(k)$.  The product of these finitely many matrix algebras
contains the prescribed elements, proving local matriciality.
\end{proof}

\begin{theorem}[the universal directly finite quotient]
\label{thm:core-ring-reflection}
...
\leanverified{Manuscript/NonMFSentences/ChainCoreClosures}{GroupApproximation.ChainCore.ChainCoreClosures.printedCoreRingReflection_closed}
\end{theorem}
...
other matrix units shows that $\varphi$ kills $M_m(I)$.  Its quotient
$M_m(R_Y)$ is LEF, as follows entrywise from finite ring tables.
Finally, $I=0$ exactly when $X=Y$, giving the stated equivalences.
\end{proof}

\begin{proposition}[three-coordinate bilateral compression]
\label{prop:bilateral-three}
...
Every homomorphism from $\GL_n(R_X)$ to an MF group kills
$\EL_n(R_X,I)$.
\leanverified{Manuscript/NonMFSentences/ChainCoreClosures}{GroupApproximation.ChainCore.ChainCoreClosures.printedBilateralThree_closed}
\end{proposition}

\begin{proof}
Take a clopen $P$ with $T(P)\subseteq P$ and nonempty
$D=P\setminus T(P)$.  Put $p=1_P$, $d=1_D$, and
$f_{ab}=u^a d u^{-b}$ for $a,b\in\mathbb Z$.  Wandering gives
$f_{ab}f_{ce}=\delta_{bc}f_{ae}$.  Negative levels lie outside $P$.
The unital subalgebra $S\subseteq pR_Xp$ generated by
$s=up$, $t=pu^{-1}$ and $kp$ satisfies $ts=p$, $st=p-d$.
It is finitely generated, so $L=\EL_3(S)$ has
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}.  Embed it in
$\GL_3(R_X)$ by adjoining identity on $1-p$.

Let $r$ cycle the three negative levels $-1\mapsto-2\mapsto-3\mapsto-1$:
\[
 r=1-\sum_{j=1}^3 f_{-j,-j}
       +f_{-2,-1}+f_{-3,-2}+f_{-1,-3},
 \qquad W=\operatorname{diag}(u,u,u)\operatorname{diag}(1,1,r).
\]
The correction $r$ acts outside $P$ and centralizes $L$.  Thus $W$
...
\[
 \begin{gathered}
 c=e_{13}(f_{-1,-2}),\qquad \ell=e_{21}(p),\qquad
 y=e_{32}(f_{-1,0}),\\
 z=1+E_{33}f_{-1,-2},\qquad B=\langle L,W,c,y\rangle.
 \end{gathered}
\]
Then $c\in C_B(L)$ and
\[
 WcW^{-1}=e_{13}(f_{0,-2}),\qquad
 \delta=[WcW^{-1},\ell]=e_{23}(-f_{0,-2}),\qquad
 [y,\delta]=z^{-1}.
\]
...
The identities
\[
 [z,e_{31}(f_{-2,0})]=e_{31}(f_{-1,0}),\qquad
 [e_{23}(f_{0,-1}),e_{31}(f_{-1,0})]=e_{21}(d)
\]
then kill the defect root.  Normal root commutators and root addition
kill all roots in the ideal generated by $d$.  Extending by identity
gives the same conclusion in rank $n\ge3$.

The defects generate $I$ by Lemma~\ref{lem:transient-matrices}, proving
the last assertion.  If $R_X$ is not directly finite, some defect is
nonzero, so $\GL_n(R_X)$ is not MF\@.  Otherwise
Theorem~\ref{thm:core-ring-reflection} makes $R_X$, and hence its
general linear groups, LEF.
\end{proof}

The incoming side of the bilateral shift supplies the auxiliary levels
used here.  The compressor $W$ need not be elementary, so this argument
does not give an unrestricted rank-three theorem for $\EL_3(R_X)$.

\begin{theorem}[the exact MF radical]\label{thm:core-mf-radical}
Take $k=\F_2$ and write
\[
 K_n=\ker\bigl(\GL_n(R_X)\longrightarrow\GL_n(R_Y)\bigr).
\]
For $n\ge2$, $K_n=\EL_n(R_X,I)$ is locally finite.  Moreover,
\[
 \begin{aligned}
 \Rad_{\mathrm{MF}}(\GL_n(R_X))&=K_n &&(n\ge3),\\
 \Rad_{\mathrm{MF}}(\EL_n(R_X))&=K_n &&(n\ge4).
 \end{aligned}
\]
The elementary restriction is surjective with LEF quotient
$\EL_n(R_Y)$.  The general linear quotient is the LEF restriction image.
\leanverified{Manuscript/NonMFSentences/ChainCoreClosures}{GroupApproximation.ChainCore.ChainCoreClosures.printedCoreMFRadical_closed}
\end{theorem}

\begin{proof}
First identify the full restriction kernel.  If $g\in\GL_n(R_X)$
restricts to the identity, put all entries of $g-1$ and $g^{-1}-1$
inside one finite matrix-product algebra
$B=\prod_a M_{h_a}(\F_2)\subset I$, with local identity $p$.
Then $g=(1-p)I_n+h$ with $h\in\GL_n(B)$.  For $n\ge2$,
\[
...
through another block.  Thus $g$ is generated by roots with
coefficients in $I$.  This proves equality of the elementary and
general linear restriction kernels with $\EL_n(R_X,I)$.  The same
argument for finitely many $g$ at once places them in a common
finite group, proving local finiteness.

Proposition~\ref{prop:bilateral-three} kills $\EL_n(R_X,I)$ in every
general linear MF image for $n\ge3$.  For elementary groups with
$n\ge4$, use Proposition~\ref{prop:torsion-defect-ring} on the
defects in \eqref{eq:clopen-defect-pair}, which have additive order at
most two and generate $I$.  Its countability hypothesis holds because
$X$ has a countable clopen basis and each clopen subset is a finite
union of basis sets.

Lifting elementary coefficients makes the elementary restriction
surjective.  Its target is LEF by
Lemma~\ref{lem:chain-core-models}, hence MF, which proves the opposite
radical inclusion.  The general linear restriction image is a subgroup
of the LEF group $\GL_n(R_Y)$, giving the other assertion.
\end{proof}

The matrix rank can be supplied by the dynamics itself.  The following
localization makes the passage to a return section particularly simple.
Write $K_n(I)$ for the general linear restriction kernel, also when
$n=1$, and use the same notation for corner ideals with their own identity.

\begin{lemma}[localization by one involution]\label{lem:involution-localization}
Let $C$ be a full clopen section with bounded positive return times,
put $p_C=1_C$ and $J=p_CIp_C$.  Over any finite field, for every finite
$F\subset I$ there is $w\in R_X^\times$ with $w-1\in I$, $w^2=1$,
and $wFw^{-1}\subset J$.  Over $\F_2$, every finite subset of $K_n(I)$
is simultaneously conjugate into the unitized $K_n(J)$ by $wI_n$;
for $n\ge2$ this involution lies in $\EL_n(R_X)$.
\leanverified{Dynamics/TransientCellsClosed}{GroupApproximation.ClopenCrossedProduct.printedInvolutionLocalization_closed}
\end{lemma}

\begin{proof}
Let $K\subset X\setminus Y$ contain the sources and ranges of a
finite partial-shift description of $F$, and cover $K$ by $m$
wandering clopen sets.  Each orbit meets $K$ at most $m$ times.
If $H$ bounds the return gaps, add arrows from every point of $K$
to its first $2m$ strictly future returns to $C$.  Their displacements
are bounded by $2mH$ and have finitely many clopen domains.  Apply
the finite matrix-block construction of
Lemma~\ref{lem:transient-matrices}, refining also by $K$ and $C$.
Each generated class contains an original point and its $2m$ distinct
return points.  At least $m$ of the latter lie in $C\setminus K$,
enough to match every original coordinate to a fresh section coordinate.
Swap the matched pairs and fix the other coordinates.  The finite
clopen partition makes these choices constant on each representative
cell.  Unitizing the resulting permutations gives an involution $w$
with $w1_Kw^{-1}\le p_C$, hence $wFw^{-1}\subset J$.

For kernel matrices, include all entries of their deviations and
inverse deviations in $F$.  Scalar conjugation then gives the
unitized corner kernels.  Over $\F_2$, the finite matrix argument in
Theorem~\ref{thm:core-mf-radical} puts $wI_n$ in $\EL_n(R_X)$ for
$n\ge2$; it also proves local finiteness of $K_1(I)$.
\end{proof}

\begin{corollary}[unit groups and rank two]\label{cor:dynamic-rank-budget}
...
\leanverified{Manuscript/NonMFSentences/ChainCoreClosures}{GroupApproximation.ChainCore.ChainCoreClosures.printedDynamicRankBudget_closed}
\end{corollary}
...
put
\[
 C_i=C_{i-1}\cup\left(U_i\setminus
                 \bigcup_{|j|<r}T^jC_{i-1}\right).
\]
...
For $0\le i<2r-1$ put
\[
 D_i=\bigcup_{h>i}C_h,\qquad a_i=u^i1_{D_i},\qquad b_i=1_{D_i}u^{-i}.
\]
...
\[
 \Phi\colon M_r(P_C)\longrightarrow R_X,\qquad
 \Phi(A)=\sum_{i,j<r}a_i A_{ij}b_j.
\]
...
Lemma~\ref{lem:involution-localization} conjugates every element of
$K_n(I)$ into this corner, so all of $K_n(I)$ is killed.  Core
restriction has LEF image, giving equality of radicals.

For other finite fields, if $R_X$ is not directly finite then neither
is $P_C$: otherwise Theorem~\ref{thm:core-ring-reflection} would make
$P_C$ stably finite, and the full tower corner would make $R_X$
directly finite.  Transfer one nonzero invisible defect root using
...
$nr\ge4$.  The positive implications follow from the LEF ring models.
\end{proof}
...
$\mathbb Z$ acts by the bilateral shift.  To see this, use the faithful
action on $\F_2^{(\mathbb Z)}$.  A unit has Laurent symbols
$z^{a_-},z^{a_+}$ at the two ends and acts by these translations on
the two sufficiently distant tails.  The tails starting at $-N$ and
$N$ span a subspace of codimension $2N-1$, whose image has codimension
$2N-1+a_+-a_-$.  Invertibility forces $a_+=a_-$; removing that common
...
those supports.  Thus $R_X^\times$ is LEF although $\GL_3(R_X)$ is
not MF\@.

There is also a limitation of the compression method at rank two.
If the core $Y$ consists of fixed points, every Kazhdan subgroup of
$\GL_2(R_X)$ is finite.  Its finitely generated restriction image
lies in a finite product of groups $\GL_2(k[z,z^{-1}])$, hence in a
Haagerup group by~\cite[Theorem~4]{GHW}, and is finite.  The remaining
kernel is locally finite, so a finite-index subgroup of the Kazhdan
group is finitely generated and locally finite, hence finite.
Finite subgroups have no proper self-compressions, so their
compression defects are trivial.  The same reasoning applies to unit
groups when every core point has period at most two: a finite invariant
clopen partition gives matrix blocks of size at most two over
$k[z,z^{-1}]$.  Thus the threshold three is a limit of this method
in these cases, not a proof that the unrestricted $\GL_2$ or
$\EL_3$ conclusions are false.
...
$A$ is \emph{amenable} if there are unital completely positive (u.c.p.)
maps
...
\begin{proof}
The models are compressions of a quasi-regular representation of $G$ to a
finite set of cosets indexed by a F\o lner set of $A$ and a finite
quotient of a finitely generated subgroup of $N$, and the multiplicative
defect comes from the F\o lner boundary.
...
\]
To see that $\varepsilon\ne1$, fix a total order on $X$, let $\F_2^{(X)}$ be
the $\F_2$-vector space of finitely supported functions $X\to\F_2$, and
for $f,g\in\F_2^{(X)}$ put $B(f,g)=\sum_{x>y}f(x)g(y)$.  Since $B$ is bilinear,
\[
 (a,f)(b,g)=\bigl(a+b+B(f,g),\ f+g\bigr)
\]
is a group law on $\F_2\times\F_2^{(X)}$, in which $(1,0)$ is a central
...
homomorphism $\operatorname{Cl}(X)\to\F_2\times\F_2^{(X)}$.  The relations let
...
$\varepsilon$ leaves $\F_2^{(X)}\cong\bigoplus_XC_2$, so
...
Soficity also shows where operator norm enters.  The permutation
matrices $V_n$ of a sofic approximation of $W$ satisfy
$\hsnorm{V_n(gh)-V_n(g)V_n(h)}\to0$, while
$\hsnorm{V_n(\varepsilon)-1}\to\sqrt2$, because $\varepsilon\ne1$ moves
almost every point.  So the collapse of Corollary~\ref{cor:defect-hs}
fails for asymptotic representations in normalized Hilbert--Schmidt
norm, although $\varepsilon$ is a central involution of $W$ inside
$\mathfrak D_W(\Gamma)$: Theorem~\ref{thm:transport} needs operator norm
control of the conjugation maps, and no version of
Theorem~\ref{thm:compression-criterion} for sofic or hyperlinear
approximation follows from these hypotheses.
...
Theorem~\ref{thm:torsion-free} is proved by passing from Fournier-Facio's
group to a two-generated quotient onto which a normal subgroup inside the
defect subgroup already maps.  Lemma~\ref{lem:saturation} produces such
quotients.
...
finite subgroup~\cite[Definition~1.4]{Hull}.  We use the following
consequence of Hull's small cancellation
...
hyperbolic, $\varphi(g_i)\in\varphi(N)$ for all $i$, and every element of finite order
in $Q$ is the image of an element of the same order in $G$.
\end{theorem}

Hull's proof treats $m=1$ by passing to $G/\normal{r}_G$ for one
element $r$ and the general case by induction on $m$, using his
clause~(d), that $\varphi(N)$ is again suitable~\cite[proof of Theorem~7.1]{Hull},
...
and let $N\trianglelefteq G$ be nontrivial.
...
two elements $h_1,h_2$ such that $N_0=\langle h_1,h_2\rangle$ is suitable
with respect to a generating set $A'\supseteq A$ to which Hull's small
cancellation theorem again applies.  Apply Theorem~\ref{thm:hull} to
$N_0$, with respect to $A'$, with $g_1,\dots,g_m$ a finite generating set
of $G$.  Then
...
$S$ is.  By Lemma~\ref{lem:saturation} applied to $G_0$ and $N$, there is a
