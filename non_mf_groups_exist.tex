...
if a property-\textup{(T)} subgroup $L\le G$ is conjugated into itself
by some $u\in G$, then every normal property-\textup{(T)} subgroup of
$G$ inside $\mathfrak D_G(L)$ dies in every MF image.
...
Theorem~\ref{thm:full-defect-ring} runs this for a countable unital ring
containing $s,t$ with $ts=1$ and $1-st$ generating $R$ as a two-sided
ideal: every homomorphism from $\EL_n(R)$, $n\ge2$, to an MF group is
then trivial.
...
Theorem~\ref{thm:torsion-free} removes the torsion that every other
example here carries: there is a two-generated, finitely presented,
torsion-free property-\textup{(T)} group with no nontrivial MF image,
built in Section~\ref{sec:torsion-free} from Fournier-Facio's
group~\cite[\S2]{FFF} and Hull's small cancellation
theorem~\cite{Hull}.
...
\begin{theorem}[a torsion-free finitely presented example]
