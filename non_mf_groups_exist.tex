...
MF group is trivial.  More generally, if a countable unital ring $R$
contains $s,t$ with $ts=1$ such that $1-st$ generates the unit ideal, then
every homomorphism from $\EL_n(R)$, $n\ge4$, to an MF group is trivial.
This applies to every countable simple unital ring that is not directly
finite and to every Leavitt algebra $L_k(1,m)$ over a countable field.
We also construct a two-generated, finitely presented,
...
group along a proper self-embedding is a compressor.  So is a one-sided
inverse in a ring.  For a unital ring $R$, write $e_{ij}(a)=1+aE_{ij}$ and
let $\EL_n(R)$ be the subgroup of $\GL_n(R)$ generated by the elements
$e_{ij}(a)$, $i\ne j$.  If $ts=1$ in $R$, then an explicit element $u$ of
$\EL_4(R)$ conjugates $\EL_3(R)$ into itself by
$e_{ij}(a)\mapsto e_{ij}(sat)$, and an elementary element $c$ centralizing
$\EL_3(R)$ satisfies $[ucu^{-1},e_{23}(1)]=e_{13}(1-st)$
(Proposition~\ref{prop:comb-compressor}).

\begin{mainthm}[full complementary idempotents]\label{thm:full-defect-ring}
Let $R$ be a countable unital ring containing elements $s,t$ with $ts=1$
such that the two-sided ideal generated by $1-st$ is $R$.  Then for every
$n\ge4$, every homomorphism from $\EL_n(R)$ to an MF group is trivial.
\end{mainthm}
...
field $k$ with $m\ge2$ (Corollary~\ref{cor:simple-infinite-ring}).  For a
countable unital ring $R$ that is not directly finite,
$C^*_{\max}(\EL_n(R))$ contains a proper isometry for every $n\ge4$, with
no condition on the ideal generated by $1-st$
(Corollary~\ref{cor:one-sided-ring-maximal}).  Let $L_{\F_2}(1,2)$ denote
the binary Leavitt algebra, the unital $\F_2$-algebra generated by
...
For a unital ring $R$ and $n\ge4$ we identify $\EL_3(R)$ with the
subgroup of $\EL_n(R)$ of matrices $\operatorname{diag}(A,I_{n-3})$, and
we write $4\times4$ matrices for their extensions by $I_{n-4}$.

\begin{proposition}[the comb compressor]\label{prop:comb-compressor}
Let $R$ be a unital ring with elements $s,t$ satisfying $ts=1$, and put
$e=1-st$, so that $e^2=e$ and $es=te=0$.  For $i=1,2,3$ let
\[
 u_i=e_{4i}(t-1)\,e_{i4}(1)\,e_{4i}(s-1)\,e_{i4}(-t),
 \qquad u=u_3u_2u_1\in\EL_4(R).
\]
Then
\[
 u=\begin{pmatrix}s&0&0&e\\0&s&0&et\\0&0&s&et^2\\0&0&0&t^3\end{pmatrix},
 \qquad
 u^{-1}=\begin{pmatrix}t&0&0&0\\0&t&0&0\\0&0&t&0\\e&se&s^2e&s^3\end{pmatrix},
\]
and $u\operatorname{diag}(A,1)u^{-1}=\operatorname{diag}(eI_3+sAt,1)$ for
every $A\in M_3(R)$.  In particular $ue_{ij}(a)u^{-1}=e_{ij}(sat)$ for
$1\le i\ne j\le3$, so $u\EL_3(R)u^{-1}\le\EL_3(R)$.  The element
\[
 c=[e_{41}(e),e_{14}(t)]=\operatorname{diag}(1,1,1,1+et)
\]
centralizes $\EL_3(R)$, and
\[
 ucu^{-1}=e_{12}(e),\qquad [ucu^{-1},e_{23}(1)]=e_{13}(e).
\]
\end{proposition}

\begin{proof}
The block of $u_i$ on coordinates $i,4$ is
$\begin{psmallmatrix}s&e\\0&t\end{psmallmatrix}$, and the product of the
three pieces is the displayed $u$.  Write
$u=\begin{psmallmatrix}sI_3&v\\0&t^3\end{psmallmatrix}$ with
$v=(e,et,et^2)^{\mathsf T}$, and put $w=(e,se,s^2e)$.  Since $es=te=0$,
one has $vw=eI_3$, $vs^3=0$ and $t^3w=0$, so the displayed matrix is a
two-sided inverse of $u$ and
\[
 u\operatorname{diag}(A,1)u^{-1}
 =\begin{pmatrix}sAt+vw&vs^3\\t^3w&t^3s^3\end{pmatrix}
 =\operatorname{diag}(eI_3+sAt,1).
\]
For $A=e_{ij}(a)$ this is $e_{ij}(sat)$, because $e+st=1$.  The
commutator $c$ is computed in the block on coordinates $1,4$, where,
since $te=0$,
\[
 \begin{pmatrix}1&0\\e&1\end{pmatrix}
 \begin{pmatrix}1&t\\0&1\end{pmatrix}
 \begin{pmatrix}1&0\\-e&1\end{pmatrix}
 \begin{pmatrix}1&-t\\0&1\end{pmatrix}
 =\begin{pmatrix}1&0\\0&1+et\end{pmatrix},
\]
and a diagonal matrix of this shape commutes with every
$\operatorname{diag}(A,1)$.  Next, since $t^3et=0$,
$uc=\begin{psmallmatrix}sI_3&v'\\0&t^3\end{psmallmatrix}$ with
$v'=v(1+et)=(e+et,et,et^2)^{\mathsf T}$, and $v'w=eI_3+eE_{12}$, so
$ucu^{-1}=1+eE_{12}$.  The last identity is the Steinberg relation
$[e_{12}(e),e_{23}(1)]=e_{13}(e)$.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:full-defect-ring}]
Write $e=1-st$ and fix $a_1,\dots,a_m,b_1,\dots,b_m\in R$ with
$\sum_ja_jeb_j=1$.  Suppose first that $R$ is finitely generated as a
unital ring, and put $G=\EL_n(R)$ and $L=\EL_3(R)$.  Both have
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}.  By
Proposition~\ref{prop:comb-compressor}, $u\in\operatorname{Comp}_G(L)$,
$c\in C_G(L)$, and
\[
 d=[ucu^{-1},e_{23}(1)]=e_{13}(e)\in\mathfrak D_G(L).
\]
Let $N=\normal d_G$.  For $a,b\in R$, by the Steinberg relations,
\[
 [e_{41}(a),e_{13}(e)]=e_{43}(ae),\qquad
 [e_{43}(ae),e_{32}(b)]=e_{42}(aeb),
\]
so $e_{42}(1)=\prod_je_{42}(a_jeb_j)\in N$.  The signed permutation
matrices $e_{ij}(1)e_{ji}(-1)e_{ij}(1)$ lie in $G$ and conjugate
$e_{42}(1)$ to $e_{kl}(\pm1)$ for all $k\ne l$, and
$[e_{kl}(1),e_{lm}(r)]=e_{km}(r)$ for distinct $k,l,m$ and $r\in R$.  So
$N=G$, and then $\mathfrak D_G(L)=G$.  By
...
finitely generated unital subring $S$ containing $s,t$ and
$a_1,\dots,b_m$.  Both hypotheses hold in $S$, so a homomorphism from
$\EL_n(R)$ to an MF group is trivial on each such $\EL_n(S)$, and these
subgroups cover $\EL_n(R)$.
\end{proof}

The same proof applies when finitely many pairs $t_\nu s_\nu=1$ have
complementary idempotents $1-s_\nu t_\nu$ that together generate the unit
ideal: for each pair there is a compressor and a centralizer for the same
$L$, so each $e_{13}(1-s_\nu t_\nu)$ lies in $\mathfrak D_G(L)$, and by
the two commutators above $e_{42}(1)$ lies in their normal closure.

\begin{corollary}\label{cor:simple-infinite-ring}
Let $R$ be a countable simple unital ring that is not directly finite, or
a Leavitt algebra $L_k(1,m)$ over a countable field $k$ with $m\ge2$.
...
idempotent, and since $R$ is simple the ideal it generates is $R$.  In
the second case, with generators $s_1,\dots,s_m,t_1,\dots,t_m$ subject
to $t_is_j=\delta_{ij}$ and $\sum_is_it_i=1$, take $s=s_1$ and $t=t_1$;
then $1-s_1t_1=\sum_{i\ge2}s_it_i$ and
\[
 t_2(1-s_1t_1)s_2=1.
\]
...
For finite $k$ the groups $\EL_n(L_k(1,m))$ are finitely generated and
have property~\textup{(T)}.  For infinite $k$ the conclusion holds by the
passage to finitely generated subrings in the proof of
Theorem~\ref{thm:full-defect-ring}.

\begin{corollary}\label{cor:one-sided-ring-maximal}
Let $R$ be a countable unital ring that is not directly finite.  Then
$C^*_{\max}(\EL_n(R))$ contains a proper isometry for every $n\ge4$.  If
$R\ne0$ satisfies the hypothesis of Theorem~\ref{thm:full-defect-ring},
then $C^*_{\mathrm r}(\EL_n(R))$ is separable, stably finite, and not MF\@.
\end{corollary}

\begin{proof}
Choose $s,t\in R$ with $ts=1\ne st$ and let $S$ be the unital subring
they generate.  By Proposition~\ref{prop:comb-compressor} the compressor
$u\in\EL_4(S)$ conjugates $L=\EL_3(S)$ into itself, and $L$ has
...
relations~\eqref{eq:leavitt} say that $s_0,s_1$ embed two copies of the
free module $R$ side by side and $t_0,t_1$ read them back, so one copy of
$R$ is isomorphic to two copies.  With $s=s_0$ and $t=t_0$ the
complementary idempotent of Section~\ref{sec:one-sided-inverses} is
$1-s_0t_0=q$, and $t_1qs_1=1$, so $R$ satisfies the hypothesis of
Theorem~\ref{thm:full-defect-ring}.  Set $H=\EL_{12}(R)$, with indices
$0,\dots,11$ for $12\times12$ matrices.
...
The ring $R$ is generated by $s_0,s_1,t_0,t_1$, so $H$ has
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}, which implies finite
generation~\cite[Theorem~1.3.1]{BHV}.  By
...
$s\ne1$.  By Lemma~\ref{lem:saturation} applied to $G_0$, $N$, and the
finite set $\{1,s\}$, there is a surjective homomorphism
$\varphi\colon G_0\to Q$ with $Q$ two-generated, finitely presented,
torsion-free, and acylindrically hyperbolic, $\varphi(N)=Q$, and
$\varphi(s)\ne1$.  The group $Q$ has
