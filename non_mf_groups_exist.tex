...
The repository behind this paper opened on August~1, 2026, with AI
agents set on the question whether every group is hyperlinear, starting
from the nonsofic group of~\cite{OAI}.  One of the agents was trying to
build weak MF approximations of a Clifford-twisted wreath product.  The
key discovery happened in ChatGPT, using GPT-5.6 Sol on ``high''
thinking mode.  Earlier in that chat the author had asked whether to
pivot from the hyperlinear question to the MF question, and Sol had said
it was not a good idea.  Later the author asked Sol to make progress on
the existing work (it had access to the repository), and instead of
doing that, Sol said it had come up with an MF counterexample: every
norm-matrix model of the Clifford-twisted wreath product has to kill the
Clifford sign.  That was on August~9.  Other parallel chats without the
MF idea, and the agents working only on the hyperlinear question, did
not find it.  On August~12 the obstruction was reduced to one commutator
in a finitely presented group, and the first draft of this paper was
written that evening; experts were consulted from August~13.

The author's part was to prompt the models, set the direction, find the
literature, and suggest the MF question; the proofs came from the
models, over thousands of prompts, with comments from the experts and
two September reviews by Astra, an OpenAI model, and the commit history
of the public repository shows the exchange.  The one-sided transport
step of that first proof is unchanged here.  The central sign is the
case of Theorem~\ref{thm:compression-criterion} in which $K$ is
generated by one central involution, and the original group is the one
behind Theorem~\ref{thm:amenable-trace}; the simple Leavitt example, the
ring-theoretic criterion, and the torsion-free group came later.

For the shortest route to a non-MF group, the reader may start with
Eckhardt's expository note~\cite{Eckhardt}, a good first read on the
compression mechanism, and return here for the compression criterion,
the simple and torsion-free examples, and the amenable trace.
...
not discover the key MF result.  In September 2026 the manuscript was
reviewed twice by Astra, then a new OpenAI model, which proposed the
cuts and repairs that shaped the September revision, recommended
promoting Theorem~\ref{thm:amenable-trace} to a main theorem, and
supplied the replacement argument printed in the proof of
Theorem~\ref{thm:normal-kazhdan}.  The obstruction and the
