...
the exchange.  Claude Fable~5 and Claude Opus~5 did much of the Lean
formalization and the coding, in Claude Code and Codex, with a custom
informal proof tool for scaffolding, and a supercomputer was used to
test and discard preliminary approaches.  Astra's reviews shortened
the paper and supplied the argument printed in the proof of
Theorem~\ref{thm:normal-kazhdan}.  The Lean development was registered
at Palomar on August~24 (PALOMAR-2026-08-24-000006), and the
statements checked in Lean~4 are marked in the \LaTeX{} source with
the names of the corresponding declarations.  The one-sided transport
...
generated by one central involution, and the original group is the
group $W$ of Theorem~\ref{thm:amenable-trace}; the simple Leavitt example, the
...
contains $\mathfrak D_G(L)$.\footnote{For a finite-dimensional unitary
representation $\rho\colon G\to\U(d)$ the same conclusion holds without
