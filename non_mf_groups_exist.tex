...
such elements have asymptotic matrix images that vanish in
Hilbert--Schmidt norm, and property~\textup{(T)} of a normal subgroup
they generate upgrades that to vanishing in operator norm.
...
separable, stably finite, and not MF.  For a countable purely infinite
simple ring the MF radical of $\GL_n(R)$ is its commutator subgroup and
the largest MF quotient is $K_1(R)$.  A finite-block construction gives a
finitely presented, centerless, sofic group $E$ whose MF radical is
exactly its perfect, residually finite block subgroup
$\mathop{*}_{j\ge1}(A_5)^8$, with residually finite quotient; it splits
as $J\rtimes\mathbb Z$ with $J$ locally residually finite, so its
canonical trace on $C^*_{\max}(E)$ is amenable but not quasidiagonal.  Its proof extends Kazhdan transport to fixed points in
...
simple with a unique trace, has stable rank one, and is not MF\@.  One
amalgamation of that group realizes any finitely presented sofic MF
group with an element of infinite order as the largest MF quotient of a
finitely presented sofic non-MF group, preserving its profinite
completion and its rational cohomology.  Finally, we construct
...
A corona homomorphism nontrivial on $K$ compresses to a corner where
...
The canonical trace on $C^*_{\max}(J)$ is quasidiagonal, while the
canonical trace on $C^*_{\max}(E)$ is amenable and not quasidiagonal.
\end{mainthm}
...
The trace on $C^*_{\max}(E)$ answers the question of
...
amenable trace is quasidiagonal.  Tikuisis, White and Winter proved
...
$C^*_{\max}(E)$ is not nuclear, since $E$ contains a Kazhdan subgroup
and is not amenable.  The canonical trace of $C^*_{\max}(J)$ is
quasidiagonal, so the crossed product by $\mathbb Z$ destroys
quasidiagonality of the canonical trace.  Since $J$ is a direct limit
of residually finite groups it is MF~\cite[Corollary~10 and
Proposition~13]{Korchagin}, so MF groups are not closed under
semidirect products with $\mathbb Z$, which answers a question of
...
Theorem~\ref{thm:realization} makes $P$ a device: amalgamating it with
any finitely presented sofic group $A$ along an infinite-order element
produces a finitely presented sofic non-MF group whose largest MF
quotient is $A$, with the profinite completion and the rational
cohomology of $A$ unchanged.
...
$W$
 & all kill $\varepsilon$
 & lamps over a proper\\
 & (Proposition~\ref{prop:clifford-self-embedding})
 & self-embedding\\[2pt]
$E$
 & factor through $V$, radical $B$
 & finitely presented, with radical\\
 & (Theorem~\ref{thm:fp-sofic-radical})
 & and quotient both residually\\
 & & finite, and an amenable trace\\
...
$Q$
 ...
 & & presented, torsion-free\\[2pt]
$G_A$
 & factor through $A$
 & installs a prescribed largest\\
 & (Theorem~\ref{thm:realization})
 & MF quotient on a chosen $A$\\
\hline
\end{tabular}
\end{center}
...
$1\le k\le\pi/t$ with $\opnorm{A^{\otimes k}-1}\ge\sqrt2$.
...
\begin{proof}
Since $A-1$ is normal, some eigenvalue $e^{i\theta}$ of $A$, with
$0<|\theta|\le\pi$, has $|e^{i\theta}-1|=t$.  Put
$k=\lceil\pi/2|\theta|\rceil$.  Then $k|\theta|\ge\pi/2$, and
$k|\theta|\le\pi$ as well: if $|\theta|>\pi/2$ then $k=1$, and
otherwise $k|\theta|<\pi/2+|\theta|\le\pi$.  So
$k\theta/2\in[\pi/4,\pi/2]$ up to sign and
\[
 |e^{ik\theta}-1|=2\bigl|\sin(k\theta/2)\bigr|\ge\sqrt2 .
\]
Tensoring an eigenvector for $e^{i\theta}$ shows that $e^{ik\theta}$
is an eigenvalue of $A^{\otimes k}$.  Finally
$t=2|\sin(\theta/2)|\le|\theta|$, so $kt\le k|\theta|\le\pi$.
...
\eqref{eq:word-linear-certificate} with $C=\pi/\eta$.
\end{theorem}
...
$1\le k\le\pi/t$ with $\opnorm{w(U)^{\otimes k}-1}>1$.
...
\[
 ...
 \le\frac\pi t\delta_{\mathcal R}(U),
\]
...
\]
Conversely, if such words and such a constant exist, then every
homomorphism from $G$ to an MF group is trivial.
...
We now construct the simplest group to which
Theorem~\ref{thm:compression-criterion} applies: its intrinsic defect
subgroup is generated by a single commutator and contains a central
involution.  Here $\Gamma$ may be any countable
property-\textup{(T)} group with a proper self-embedding.  Let $\Gamma$ be a
...
$\varepsilon$ leaves $E\cong\bigoplus_XC_2$, so
...
The lamp group $\operatorname{Cl}(X)$ is locally finite, so this
example locates $\operatorname{Rad}_{\mathrm{MF}}(W)$ only between
$\langle\varepsilon\rangle$ and $\operatorname{Cl}(X)$.
Section~\ref{sec:finite-blocks} keeps $\bar\Gamma$, $\alpha$ and $a$
and replaces the lamps by perfect finite blocks.  The resulting group is
finitely presented, its MF radical is computed exactly, and
Proposition~\ref{prop:locally-rf-by-z-trace} together with
Theorem~\ref{thm:factorization-nonmf-trace} then supplies the amenable
nonquasidiagonal trace of Theorem~\ref{thm:fp-sofic-radical}.
...
The second upgrade is forced rather than convenient, because perfect
blocks make the criterion of Theorem~\ref{thm:compression-criterion}
blind to the construction.  Suppose a finite group $F$ is generated by
subgroups $A_\omega$, $\omega\in\Omega$, whose images in $F/Z$ form a
direct product for some central $Z$.  For $x\in A_\omega$ and
$y\in A_\eta$ with $\omega\ne\eta$ the commutator $[x,y]$ lies in $Z$,
so $(x,y)\mapsto[x,y]$ is a homomorphism in each variable into an
abelian group and factors through
$A_\omega^{\mathrm{ab}}\times A_\eta^{\mathrm{ab}}$; if the blocks are
perfect it is trivial.  Distinct perfect blocks therefore commute in
every such extension.  The element that the construction below
centralizes $\Gamma$ with is a block coordinate transported by $t$, so
the defect it produces is a product $c_0c_1^{-1}$ of elements of two
distinct blocks, and it lies in the block kernel $B_K$.  That kernel is
a free product of finitely many copies of a finite group at each coset,
so each of its finitely generated subgroups is virtually free; an
infinite one has a finite-index subgroup mapping onto $\mathbb Z$,
which no property-\textup{(T)} group has~\cite{BHV}.  Every
property-\textup{(T)} subgroup of $B_K$ is thus finite, while a free
product of at least two nontrivial groups has no nontrivial finite
normal subgroup.  So no nontrivial normal property-\textup{(T)}
subgroup of the ambient group lies in $B_K$.

Blocks in which distinct coordinates commute up to a fixed central
involution $z$ do have visible defect: there $(c_0c_1^{-1})^2=[c_0,c_1]
=z$, and $\langle z\rangle$ is finite, central and inside the defect
subgroup, so Theorem~\ref{thm:compression-criterion} applies directly.
But such an $F$ surjects onto $C_2^{|\Omega|}$, so finite quotients no
longer annihilate the blocks and the exact radical of
Theorem~\ref{thm:fp-sofic-radical} is lost.  Perfection and a visible
defect exclude one another, and the finite-dimensional upgrade is what
makes the perfect side available.
...
\begin{proof}
A unital homomorphism from $A\cong\bigoplus_iM_{r_i}(\mathbb C)$ to a
norm matrix corona lifts, on a cofinite set of coordinates, to unital
$*$-homomorphisms $\phi_n\colon A\to M_{d_n}(\mathbb C)$.  Lift its
diagonal matrix units and round them successively to orthogonal
projections, using the complement of their sum for the last one; the
corrections tend to zero.  In each summand compress a lift of $e_{j1}$
between the chosen projections $p_j,p_1$; on a common tail its polar
partial isometry $v_j$ has precisely these final and initial
projections, and the $v_jv_k^*$ are exact matrix units lifting
$e_{jk}$.  A summand may vanish at some coordinates, and injectivity of
$\phi_n$ is not required.  Lift $\rho$ to an operator norm asymptotic
representation $U=(U_n)$ of $G$ and put
$\beta_g=\operatorname{Ad}(\rho(g))|_A$, so that
\begin{equation}\label{eq:finite-algebra-covariance}
 \opnorm{U_n(g)\phi_n(a)U_n(g)^*-\phi_n(\beta_g(a))}
 \longrightarrow0\qquad(a\in A,\ g\in G).
\end{equation}

Let $q$ be the sum of one $L$-orbit of minimal central projections of
$A$.  The maps $\beta_g$, $g\in L$, carry the corresponding summands
onto one another, so these are isomorphic.  Applying
\eqref{eq:finite-algebra-covariance} to a finite generating set of $L$
and to each minimal central projection $e\le q$, the projections
$U_n(g)\phi_n(e)U_n(g)^*$ and $\phi_n(\beta_g(e))$ are eventually at
distance less than $1$, hence eventually of equal rank: the summands of
$qA$ occur in $\phi_n$ with equal multiplicity.  Discard the
coordinates where $q_n=\phi_n(q)$ vanishes and put
$\mu_n=\tr_{d_n}(q_n)>0$.  On $qA$ the traces
$\mu_n^{-1}\tr_{d_n}\circ\phi_n$ are then all equal to the single
faithful trace $\tau_0$ that averages the normalized traces of those
summands, so
\begin{equation}\label{eq:orbit-normalization}
 \hsnorm{\phi_n(b)}=\sqrt{\mu_n}\,\lVert b\rVert_{2,\tau_0},
 \qquad \lVert b\rVert_{2,\tau_0}=\tau_0(b^*b)^{1/2}
 \qquad(b\in qA).
\end{equation}

Fix $b\in qA$ and $g\in G$ and put
$D_n=U_n(g)\phi_n(b)U_n(g)^*-\phi_n(\beta_g(b))$.  Each of the two
terms has rank at most $\operatorname{rank}q_n=\mu_nd_n$, so
$\operatorname{rank}D_n\le2\mu_nd_n$ and therefore
\begin{equation}\label{eq:rank-normalized}
 \frac{\hsnorm{D_n}}{\sqrt{\mu_n}}
 \le\sqrt2\,\opnorm{D_n}\longrightarrow0 .
\end{equation}
This rank bound is what makes the argument uniform.  It compensates
exactly for a summand whose relative size $\mu_n$ tends to zero, so no
correction of the covariance is required; an appeal to equivalence of
norms on the finite-dimensional algebra would not survive that case.

Now take $b\in A^{tLt^{-1}}$.  The orbit sums $q$ are $L$-invariant,
hence $tLt^{-1}$-invariant, so it is enough to treat $b\in qA$.  The
matrices $x_n=\phi_n(b)/\sqrt{\mu_n}$ have the constant normalized
Hilbert--Schmidt norm $\lVert b\rVert_{2,\tau_0}$ by
\eqref{eq:orbit-normalization}, and \eqref{eq:rank-normalized}, applied
to $h\in tLt^{-1}$, gives
$\hsnorm{\operatorname{Ad}(U_n(h))x_n-x_n}\to0$; so
$x\in\mathcal C_2(U,tLt^{-1})$.  Conjugation by $U_n(t)$ carries
$\mathcal C_2(U,L)$ onto $\mathcal C_2(U,tLt^{-1})$, so
Theorem~\ref{thm:transport} gives
$\mathcal C_2(U,tLt^{-1})=\mathcal C_2(U,L)$ and $x$ is asymptotically
fixed by $L$.  For $g\in L$ the number
$\lVert\beta_g(b)-b\rVert_{2,\tau_0}$ equals
$\hsnorm{\phi_n(\beta_g(b))-\phi_n(b)}/\sqrt{\mu_n}$ for every $n$ by
\eqref{eq:orbit-normalization}, and that tends to zero by
\eqref{eq:rank-normalized} together with
$\hsnorm{\operatorname{Ad}(U_n(g))x_n-x_n}\to0$.  So $\beta_g(b)=b$ by
faithfulness of $\tau_0$, and $b\in A^L$.  The reverse containment
follows from $tLt^{-1}\le L$.
...
Finite dimensionality of $A$ cannot be relaxed, even to a commutative
algebra with an eight-point spectrum of arcs.

\begin{theorem}[the hypothesis is sharp]\label{thm:transport-sharp}
Let $\bar\Gamma=\mathbb Z^3\rtimes\mathrm{SL}_3(\mathbb Z)$, let
$\alpha(v,A)=(2v,A)$, and let $V$ be the ascending HNN extension of
$\bar\Gamma$ along $\alpha$.  There is an injective homomorphism
$\sigma$ of $V$ into the unitary group of a norm matrix corona for
which the following hold.
\begin{enumerate}
\item There is a unital commutative subalgebra
$\mathcal A\cong C(T)$, normalized by $\sigma(\bar\Gamma)$, where $T$
is a wedge of eight arcs, with
\[
 \mathcal A^{\alpha(\bar\Gamma)}\ne\mathcal A^{\bar\Gamma}.
\]
The failure already occurs on the nine-dimensional operator system
spanned by the unit and the eight generators.
\item There is a finitely generated sofic MF group $G$ containing $V$
and an involution $c$ centralizing $\bar\Gamma$ such that
$f=tct^{-1}$ centralizes $\alpha(\bar\Gamma)$, the $\bar\Gamma$-orbit
of $f$ has eight elements and generates
$\mathbb Z^7\rtimes C_2$, and
$\opnorm{[f,u]-1}=2$ for the translation $u$ by the first basis vector.
\end{enumerate}
\end{theorem}

\begin{proof}
For odd $q$ let $X_q=(\mathbb Z/q)^3$ and let $\sigma_q$ be the
permutation representation of $V$ on $\ell^2(X_q)$ in which
$(v,A,k)$ acts by $x\mapsto2^kAx+v$; this is defined because $2$ is
invertible modulo $q$.  A nonidentity element of $V$ acts nontrivially
for all large odd $q$, and a nonidentity permutation matrix is at
distance at least $\sqrt2$ from the identity, so the classes
$\sigma(g)=[\sigma_q(g)]_q$ define an injective homomorphism into the
corona.

Let $S$ consist of the translations by $\pm e_i$ and the elementary
matrices $I\pm E_{ij}$, a generating set of $\bar\Gamma$, and let
$\ell_q$ be the distance from $0$ in the corresponding Schreier graph
on $X_q$, so that $|\ell_q(sx)-\ell_q(x)|\le1$.  Put
\[
 R_q=\Bigl\lfloor\tfrac14\log_2q\Bigr\rfloor,
 \qquad
 h_q(x)=\max\Bigl(1-\frac{\ell_q(x)}{R_q},0\Bigr),
\]
discarding the finitely many $q$ with $R_q=0$.  Then
$\opnorm{h_q\circ s^{-1}-h_q}\le R_q^{-1}$ for $s\in S$, so the class
$h$ of the multiplication operators by $h_q$ commutes with
$\sigma(\bar\Gamma)$.  A word of length at most $R$ in $S$ carries $0$
to the reduction of an integer vector of sup norm at most $2^R-1$,
since a shear at most doubles that norm and a translation raises it by
one; so $h_q$ is supported on reductions of integer vectors of sup norm
at most $2^{R_q}-1$.

For $b\in\{0,1\}^3$ put $h_{b,q}(x)=h_q(2^{-1}(x-b))$ and let $h_b$ be
the corresponding class, so that $h_0=\sigma(t)h\sigma(t)^{-1}$ and
$h_b$ is the conjugate of $h_0$ by the translation by $b$.  The eight
supports are disjoint for large $q$: a common point gives
$2(z-z')\equiv b'-b$ modulo $q$ with $z,z'$ integer vectors of sup norm
at most $2^{R_q}-1$, and $4\cdot2^{R_q}+1<q$ eventually, so the
congruence is an equality in $\mathbb Z^3$, whose left side is even and
whose right side has entries in $\{-1,0,1\}$; hence $b=b'$.  The values
of $h_q$ are the numbers $1-j/R_q$ for $0\le j\le R_q$, which become
dense in $[0,1]$, so each $h_b$ has spectrum $[0,1]$.  Eight orthogonal
positive elements with spectrum $[0,1]$ generate a commutative algebra
whose spectrum is eight arcs joined at their common zero, which gives
$\mathcal A\cong C(T)$.

Since $h$ commutes with $\sigma(\bar\Gamma)$, its conjugate $h_0$
commutes with $\sigma(t\bar\Gamma t^{-1})=\sigma(\alpha(\bar\Gamma))$.
The translation by $e_1$ carries $h_0$ to $h_{e_1}$, and these are
orthogonal and nonzero, so $h_0\notin\mathcal A^{\bar\Gamma}$.  This
proves~(1), the relevant elements lying in the span of the unit and the
eight generators.  The $\bar\Gamma$-action on the eight generators is the affine action on
$\mathbb F_2^3$, through the reduction
$\bar\Gamma\to\mathbb F_2^3\rtimes\mathrm{SL}_3(\mathbb F_2)$.  Note that
$\alpha(\bar\Gamma)$ is not normal in $\bar\Gamma$: conjugating $(0,A)$
by a translation $w$ has translation part $w-Aw$, which need not be
even.  So $\bar\Gamma/\alpha(\bar\Gamma)$ is a coset space, not a
quotient group.

For~(2), double the dimensions, replace $\sigma_q$ by
$\sigma_q\otimes I_2$, and set
$C_q(x)=R(\pi h_q(x))$ with
$R(\theta)=\begin{psmallmatrix}\cos\theta&\sin\theta\\
\sin\theta&-\cos\theta\end{psmallmatrix}$,
a reflection, so that $C_q^2=1$.  As
$\opnorm{R(\theta)-R(\theta')}=2\lvert\sin\tfrac{\theta-\theta'}2\rvert
\le\lvert\theta-\theta'\rvert$, the class $c$ of the $C_q$ is an
involution commuting with $\sigma(\bar\Gamma)$, and $f=\sigma(t)c\sigma(t)^{-1}$
has the eight conjugates $f_b$ given by $R(\pi h_{b,q})$.  At $x=0$ the
blocks of $f_0$ and $f_{e_1}$ are $R(\pi)$ and $R(0)$, because
$2^{-1}e_1$ lies outside the support of $h_q$; hence
$\opnorm{f_0-f_{e_1}}=2$, which is $\opnorm{[f,u]-1}$ since $f_0$ is
unitary and $u f u^{-1}=f_{e_1}$.  Products of two of these reflections
are commuting rotations, and $f_0$ inverts each of them, so the seven
elements $f_bf_0$ generate a copy of $\mathbb Z^7$ on which $f_0$ acts
by inversion: a relation among them restricts on the arc indexed by $b$
to a rotation by $\pi n_bs$ for every $s\in[0,1]$, forcing $n_b=0$, and
no odd word in the reflections is trivial because its blocks are
reflections.  Thus the orbit generates $\mathbb Z^7\rtimes C_2$.

Finally let $G=\langle V,c\rangle$ and let $B_0$ be generated by the
$V$-conjugates of $c$.  These are block-diagonal reflections, so every
finitely generated subgroup of $B_0$ is virtually abelian, and
$B_0\cap V=1$ because a permutation moving a fibre is at distance at
least $\sqrt2$ from every block-diagonal unitary.  Writing
$T_\alpha=\bigcup_n t^{-n}\bar\Gamma t^n$, the group $\bar\Gamma$ is
commensurated by $V$, so each $t^{-n}\bar\Gamma t^n$ has finite orbits
on the conjugates of $c$; a finite subset of $B_0\rtimes T_\alpha$
therefore lies in $M\rtimes t^{-n}\bar\Gamma t^n$ with $M$ finitely
generated virtually abelian and the action having finite image, and such
a group is residually finite.  So $B_0\rtimes T_\alpha$ is locally
residually finite, and $G=(B_0\rtimes T_\alpha)\rtimes\mathbb Z$ is
sofic by the permanence theorems used above.  It is MF because it is
given inside a norm matrix corona.
\end{proof}

So a finite subgroup cannot be replaced in
Theorem~\ref{thm:finite-algebra-transport} by a finitely generated
virtually abelian one, even when the element has order two and a finite
orbit.

The normalization in that proof is by the relative size $\mu_n$ of a
single central orbit, and the rank bound~\eqref{eq:rank-normalized} is
what keeps the covariance error small after dividing by
$\sqrt{\mu_n}$.  The example above is the reason some such bound is
needed: on a summand whose relative size tends to zero, equivalence of
norms on a fixed finite-dimensional algebra gives nothing uniform.
...
runs from $\bar v\Gamma$ to $\bar vt\Gamma$.  That is precisely the
incidence of the Bass--Serre tree of the ascending HNN extension $V$,
whose vertices are $V/\Gamma$ and whose edges are $V/\alpha(\Gamma)$,
and the two graphs have the same vertex and edge sets because $B_K$ is
normal with quotient $V$.  The quotient graph is therefore a tree, and a graph of groups with
...
\]
Each $\Gamma_n$ has finite index in $\Gamma_{n+1}$ because
$[\Gamma:\alpha(\Gamma)]=m$, and conjugation by $t$ carries
$\Gamma_{n+1}$ onto $\Gamma_n$; so $\Gamma$ is commensurated by $V$
and every $\Gamma_n$-orbit in $V/\Gamma$ is finite.  A finite subset of $J_K$
...
Theorem~\ref{thm:perfect-block-radical}.  This is the group
$\bar\Gamma$ and the endomorphism of Section~\ref{sec:amenable-nonqd};
it has property~(T) and is finitely presented.  For the latter fact one may use a finite presentation
...
these vertex groups along~\eqref{eq:cyclic-kernel-relations}, and gives
\[
 N_{[p,q+1]}\cong
...
\subsection{Prescribed MF quotients}
\label{subsec:realization}

One amalgamation turns $P$ into a device that installs a prescribed
universal MF quotient on a group of one's choosing.  The kernel it
attaches is invisible to MF groups and to rational homology at the same
time.

\begin{lemma}[the invisible kernel is rationally acyclic]
\label{lem:rationally-acyclic}
$H_j(N;\mathbb Q)=0$ for every $j>0$, and $\chi$ induces an isomorphism
on rational homology.
\end{lemma}

\begin{proof}
The group $B$ is a free product of finite groups, so its rational
homology vanishes in positive degrees, and the Lyndon--Hochschild--Serre
spectral sequence for $B\to E\to V$ makes $E\to V$ a rational homology
isomorphism.  Write $L=\mathbb Z[1/2]^3$ for the translation subgroup of
$V$, so that $V/L\cong\mathrm{SL}_3(\mathbb Z)\times\langle t\rangle$.
Then $H_j(L;\mathbb Q)=\bigwedge^j\mathbb Q^3$, and $t$ acts on it by
$2^j$; for $1\le j\le3$ the map $2^j-1$ is invertible over $\mathbb Q$,
so both the invariants and the coinvariants of $\langle t\rangle$ in
$H_j(L;\mathbb Q)$ vanish, and the spectral sequence leaves only the
row $j=0$.  The positive-degree rational homology of
$\mathrm{SL}_3(\mathbb Z)$ vanishes~\cite{Soule}, so
$H_*(V;\mathbb Q)\cong H_*(\mathbb Z;\mathbb Q)$ through the height map.

For $N$, use the graph of groups of~\eqref{eq:cyclic-kernel-relations}:
vertex groups $E_i$ indexed by $\mathbb Z$, infinite cyclic edge groups
$\langle d_i\rangle=\langle r_{i+1}\rangle$, and a line as underlying
graph.  In the resulting Mayer--Vietoris sequence the vertex and edge
groups have rational homology only in degrees $0$ and $1$.  In degree
one the edge generator $d_i=r_{i+1}$ has height $0$ in $E_i$ and height
$1$ in $E_{i+1}$, so the edge-to-vertex map is
\[
 \bigoplus_{i\in\mathbb Z}\mathbb Q
 \longrightarrow\bigoplus_{i\in\mathbb Z}\mathbb Q,
 \qquad e_i\longmapsto e_{i+1},
\]
up to sign, an isomorphism; and in degree zero it is injective because
the graph is a tree.  Exactness gives $H_j(N;\mathbb Q)=0$ for $j>0$,
and then the spectral sequence for $N\to P\to\mathbb Z$ gives the last
assertion.
\end{proof}

\begin{theorem}[prescribed MF quotients]\label{thm:realization}
Let $A$ be a finitely presented sofic group and let $a\in A$ have
infinite order.  Amalgamate the infinite cyclic subgroups generated by
$s$ and $a$,
\[
 G_A=P*_{\langle s\rangle=\langle a\rangle}A,
\]
and let $q\colon G_A\to A$ be the identity on $A$ and $p\mapsto
a^{\chi(p)}$ on $P$.  Then $G_A$ is finitely presented and sofic,
it contains $P$, and it is not MF\@.  Moreover:
\begin{enumerate}
\item composition with $q$ is a bijection from the homomorphisms
$A\to M$ to the homomorphisms $G_A\to M$, for every MF group $M$; so
$\operatorname{Rad}_{\mathrm{MF}}(G_A)=q^{-1}(\operatorname{Rad}_{\mathrm{MF}}(A))$,
and $A$ is the largest MF quotient of $G_A$ whenever $A$ is MF;
\item $\ker q\cong\mathop{*}_{A/\langle a\rangle}N$ is nontrivial,
perfect, rationally acyclic, and equal to its own MF radical;
\item $q$ induces an isomorphism of profinite completions, and
$H_j(G_A;q^*M)\cong H_j(A;M)$ and $H^j(A;M)\cong H^j(G_A;q^*M)$ for
every $\mathbb QA$-module $M$ and every $j$;
\item $G_A$ is acylindrically hyperbolic with trivial finite radical,
and $C^*_{\mathrm r}(G_A)$ is simple with a unique tracial state, has
stable rank one, and is stably finite but not MF\@.
\end{enumerate}
\end{theorem}

\begin{proof}
If $A=\langle a\rangle$ then $G_A=P$ and everything has been proved, so
assume $\langle a\rangle\ne A$.  The amalgamated subgroups are infinite
cyclic and proper in both factors, so normal form embeds $P$ and $A$,
and a presentation of $G_A$ is the union of finite presentations of $P$
and $A$ together with $s=a$.  Soficity is the amalgamation theorem of
Collins and Dykema over the monotileably amenable group $\mathbb
Z$~\cite[Theorem~3.4]{CollinsDykema}.  The retraction $q$ is well
defined because $\chi(s)=1$, and it splits the inclusion of $A$.

(1)  Let $f\colon G_A\to M$ with $M$ an MF group.  Its restriction to
$P$ factors through $\chi$ by Theorem~\ref{thm:sofic-cyclic-mf}, so it
is determined by $f(s)=f(a)$, and $f=(f|_A)\circ q$.  Conversely every
homomorphism from $A$ arises this way, and $q$ is surjective, so the
correspondence is bijective.  The radical formula and the statement
about the largest MF quotient follow, as does $G_A$ not being MF, since
$P\le G_A$ is not.

(2)  In the Bass--Serre tree of the amalgam, $\ker q$ meets $A$
trivially and meets $P$ in $\ker\chi=N$, and it meets the edge group
trivially.  Its quotient graph is a star: one vertex of $A$-type with
trivial stabilizer, one vertex of $P$-type for each coset in
$A/\langle a\rangle$ with stabilizer a conjugate of $N$, and one edge
from the centre to each of them.  A star is a tree, so $\ker q$ is the
free product of those conjugates.  A homomorphism from a free product to
an MF group is trivial as soon as it is trivial on each factor, so
$\operatorname{Rad}_{\mathrm{MF}}(\ker q)=\ker q$ by
Theorem~\ref{thm:sofic-cyclic-mf}; the remaining properties come from
Lemma~\ref{lem:rationally-acyclic}, since rational homology of a free
product is the sum of the rational homologies of its factors.

(3)  Finite groups are MF, so~(1) makes the finite quotients of $G_A$
and of $A$ correspond, which gives the profinite completions.  For the
rest, $\ker q$ acts trivially on $q^*M$ and, being rationally acyclic,
has $H_j(\ker q;q^*M)=H_j(\ker q;\mathbb Q)\otimes M=0$ for $j>0$; both
Lyndon--Hochschild--Serre spectral sequences of $\ker q\to G_A\to A$
therefore collapse onto a single row.

(4)  Pick a nonidentity element $z$ of finite order in one coordinate
$A_5$ of $B\le P$.  If $s^k=zs^\ell z^{-1}$ then $k=\ell$ by $\chi$, and
$k\ne0$ would give $s^kzs^{-k}=z$, which Britton's lemma forbids in the
HNN extension $P$ because $z$ lies in neither associated subgroup, both
of which are infinite cyclic while $z$ has finite order.  So
$\langle s\rangle\cap z\langle s\rangle z^{-1}=1$ and $\langle s\rangle$
is weakly malnormal in $G_A$.  Minasyan--Osin's amalgam
criterion~\cite[Corollary~2.2]{MinasyanOsin} then makes $G_A$ either
virtually cyclic or acylindrically hyperbolic, and the first is excluded
because $G_A$ contains $P$.  A finite normal subgroup fixes a vertex of
the Bass--Serre tree, hence fixes the whole tree by minimality, hence
lies in $\langle s\rangle\cap z\langle s\rangle z^{-1}=1$.  Simplicity
and uniqueness of the trace now follow
from~\cite[Theorem~2.35]{DGO}, stable rank one
from~\cite[Theorem~1.1]{GO}, stable finiteness from the faithful
canonical trace, and failure of MF from
Lemma~\ref{prop:mf-residual-calculus}.
\end{proof}

\begin{corollary}\label{cor:torus-quotients}
For every $m\ge1$ there is a finitely presented sofic group $G_m$ that
is not MF, whose largest MF quotient is $\mathbb Z^m$, whose profinite
completion is $\widehat{\mathbb Z}^{\,m}$, and whose rational cohomology
ring is that of the $m$-torus, and for which
$C^*_{\mathrm r}(G_m)$ is simple with a unique tracial state, has stable
rank one, and is stably finite but not MF\@.
\end{corollary}

\begin{proof}
Take $A=\mathbb Z^m$ and $a$ a basis vector in
Theorem~\ref{thm:realization}; the cohomology isomorphism of~(3)
respects cup products because it is induced by a group homomorphism.
\end{proof}
...
hyperbolic quotient $\widetilde G$ with property~\textup{(T)} such that
the quotient map is injective on $\Omega$ and every homomorphism from
$\widetilde G$ to an MF group is trivial.
\end{corollary}
...
common quotient $\widetilde G$ that is acylindrically hyperbolic, with
the map from $G$ injective on a prescribed finite subset, since a
torsion-free acylindrically hyperbolic group has no nontrivial finite
normal subgroup.  For finitely generated inputs, the proof of that
corollary applies Theorem~\ref{thm:hull} to the free product $G*Q$ and
then to the resulting quotient, each time with finitely many elements
$g_i$, so $\widetilde G$ is torsion-free and finitely presented, as in
the remark after Theorem~\ref{thm:hull}.  As a quotient of $Q$, the
group $\widetilde G$ is two-generated and has property~\textup{(T)}, and
every homomorphism from $\widetilde G$ to an MF group pulls back to one
from $Q$, so it is trivial.
\end{proof}
...
it?  Theorem~\ref{thm:transport-sharp} shows that local finiteness
cannot simply be weakened to virtual abelianness.
...
quasidiagonal?  The trace of Theorem~\ref{thm:fp-sofic-radical} lives on
a non-nuclear algebra, and for faithful traces on algebras satisfying
the universal coefficient theorem the answer is
positive~\cite{TWW}.

\item Is there a finitely presented \emph{sofic} torsion-free group
whose MF radical is nontrivial and proper?  Without soficity the
question is already settled by $Q\times\mathbb Z$, for $Q$ as in
Theorem~\ref{thm:torsion-free}: every homomorphism to an MF group
kills the first factor, while the projection to $\mathbb Z$ realizes
the quotient, so
$\operatorname{Rad}_{\mathrm{MF}}(Q\times\mathbb Z)=Q\times\{1\}$.
The exact radicals of Theorems~\ref{thm:fp-sofic-radical}
and~\ref{thm:sofic-cyclic-mf} contain torsion.
\end{enumerate}
...
\bibitem[So]{Soule}
C.~Soul\'e,
\emph{The cohomology of $\mathrm{SL}_3(\mathbb Z)$},
Topology \textbf{17} (1978), no.~1, 1--22,
