...
subgroup conjugated into a proper subgroup of itself.  If a countable unital ring $R$
...
A bounded sequence $(x_n)$ lies in $\mathcal C_2(V,L)$ if and only if
$\hsnorm{P_nx_n-x_n}\to0$.  In one direction, fix $\varepsilon>0$ and,
by density of the group algebra in $C^*_{\max}(L)$, a finite set
$F\subseteq L$ and scalars $(a_\ell)_{\ell\in F}$ with
$\opnorm{\sum_{\ell\in F}a_\ell u_\ell-e_L}<\varepsilon$, where $u_\ell$
is the canonical unitary of $\ell$; the trivial character gives
...
\[
 ...
 \le\varepsilon .
\]
If $(x_n)\in\mathcal C_2(V,L)$ and $\sup_n\opnorm{x_n}\le c$, then
...
coordinates and put \(r_n=\operatorname{rank}(q_n)\).  Unitary
identifications \(J_n\colon\C^{r_n}\to q_n\C^{d_n}\) identify each
corner \(q_nM_{d_n}(\C)q_n\) with \(M_{r_n}(\C)\), and then the corner
...
corona makes \((W_n)\) an operator norm asymptotic representation, and
the class of \((J_nW_n(g)J_n^*)\) is the coordinate restriction of
\(q\rho(g)\).
\end{proof}
...
$e_{12}(e)u=u+e\,(s,0,0,et)E_{1\ast}$ and $es=0$.  So
\[
 ucu^{-1}=e_{12}(e).
\]
...
of $g$.  Fix a finite set $F\subseteq G$ and a positive integer $M$, and
put $I_M=\{0,\ldots,M-1\}$.  Define
\[
 a_{g,j}=t^{-(j+k_g)}gt^j=\beta^{-(j+k_g)}(x_g)\in N,
 \qquad g\in F,\quad j\in I_M,
\]
and put $N_{F,M}=\langle a_{g,j}:g\in F,\ j\in I_M\rangle$.
This finitely generated subgroup of $N$ is residually finite, so there
is a finite quotient
\[
 \theta_{F,M}\colon N_{F,M}\longrightarrow Q_{F,M}
\]
...
$H_{F,M}\leq G$ be the image of $\ker\theta_{F,M}\leq N_{F,M}$ under the
inclusion $N\hookrightarrow G$.  Choose a representative $r_q\in N_{F,M}$
for each $q\in Q_{F,M}$ and set
\[
 S_{F,M}=\{t^jr_qH_{F,M}:j\in I_M,\ q\in Q_{F,M}\}
 \subseteq G/H_{F,M}.
\]
These cosets are pairwise distinct.  Let $\lambda_{F,M}$ be the
quasi-regular representation of $G$ on $\ell^2(G/H_{F,M})$ and let
$P_{F,M}$ be the projection onto $\ell^2(S_{F,M})$.  Put
\[
 \phi_{F,M}(b)=P_{F,M}\lambda_{F,M}(b)P_{F,M}\big|_{\ell^2(S_{F,M})}
 \qquad(b\in C^*_{\max}(G)).
\]
This map is u.c.p., since it is the compression of a unitary
representation.
For $g\in F$ and $j+k_g\in I_M$, the action satisfies
\[
 g(t^jr_qH_{F,M})
 =t^{j+k_g}a_{g,j}r_qH_{F,M}
 =t^{j+k_g}r_{\theta_{F,M}(a_{g,j})q}H_{F,M}.
\]
So for $s=t^jr_qH_{F,M}$, the point $g\cdot s$ lies in $S_{F,M}$ if and
only if $j+k_g\in I_M$.

Let $g,h\in F$, and let $\delta_s$ be the basis vector of
$\ell^2(S_{F,M})$ at $s=t^jr_qH_{F,M}$.  If $j+k_h\in I_M$, then
$h\cdot s\in S_{F,M}$, so
$\phi_{F,M}(u_g)\phi_{F,M}(u_h)\delta_s=P_{F,M}\delta_{gh\cdot s}
=\phi_{F,M}(u_{gh})\delta_s$.  If $j+k_h\notin I_M$, then
$h\cdot s\notin S_{F,M}$, so $\phi_{F,M}(u_h)\delta_s=0$, while
$\phi_{F,M}(u_{gh})\delta_s$ has norm at most one.  There are at most
$|k_h|$ heights $j\in I_M$ with $j+k_h\notin I_M$, and there are
$|Q_{F,M}|$ basis vectors of each height, out of $M|Q_{F,M}|$ in all.
So the normalized Hilbert--Schmidt norm satisfies
\[
 \hsnorm{\phi_{F,M}(u_{gh})-
   \phi_{F,M}(u_g)\phi_{F,M}(u_h)}^2
 \leq \frac{|k_h|}{L},
\]
which tends to zero as $M\to\infty$ for fixed $h$.

The normalized trace of $\phi_{F,M}(u_g)$ is the fraction of points of
$S_{F,M}$ fixed by $g$.  If $g$ fixed $wH_{F,M}$, then
$w^{-1}gw\in H_{F,M}\leq N$, and hence $k_g=0$.  Thus an element of nonzero
height fixes no point of $S_{F,M}$.  If $g\in F\setminus\{1\}$ and $k_g=0$,
fixing $t^jr_qH_{F,M}$ would give
\[
 r_q^{-1}a_{g,j}r_q\in H_{F,M}.
\]
Since $H_{F,M}=\ker\theta_{F,M}$ is normal in $N_{F,M}$ and
$r_q\in N_{F,M}$, this implies $a_{g,j}\in\ker\theta_{F,M}$.
But $a_{g,j}=t^{-j}gt^j\ne1$, contrary to separation.  Thus
\[
 \tr_{M|Q_{F,M}|}(\phi_{F,M}(u_g))=0
 \quad(g\in F\setminus\{1\}),
 \qquad
 \tr_{M|Q_{F,M}|}(\phi_{F,M}(1))=1.
\]
Take a finite exhaustion $F_1\subseteq F_2\subseteq\cdots$ of $G$ with
$1\in F_1$, and choose $M_n\ge n(1+\max_{g\in F_n}|k_g|)$.
The maps $\phi_{F_n,M_n}$ are asymptotically
...
The group $\bar\Gamma$, the map $\alpha$, and the element $a$ satisfy
the hypotheses just established, so
Proposition~\ref{prop:clifford-locally-rf} applies:
$W\cong K\rtimes\mathbb Z$ with $K$ locally residually finite, the
group $W$ is sofic and not MF, and the canonical trace $\tau_W$ on
$C^*_{\max}(W)$ is amenable and not quasidiagonal.
