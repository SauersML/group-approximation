...
positive characteristic, the weaker condition $ts=1\ne st$ already
makes $\EL_n(R)$ non-MF for every $n\ge4$.  This follows from a
compression theorem for locally finite-dimensional conjugacy orbits;
for the binary Jacobson algebra it also computes the exact MF radical.
In
...
finite-dimensional invariant $C^*$-subalgebras.  Finally, we construct
...
Kazhdan projection of $K$ vanishes.  Hilbert--Schmidt triviality makes
every ultralimit of the normalized corner traces restrict to the
trivial character of $K$.  That character takes the Kazhdan projection
to $1$, although the corner has killed it
...
Theorem~\ref{thm:finite-dimensional-orbit} gives another operator norm
upgrade.  If each finite part of a conjugacy orbit generates a
finite-dimensional algebra, local corrections of the conjugating
unitaries produce a cocycle at the scale of its largest generator
displacement.  Property~\textup{(T)} makes it a coboundary, and the
same Kazhdan projection identity forces that coboundary to vanish.
In particular, the subgroup containing the defect can be locally
finite (Corollary~\ref{cor:locally-finite-compression}).
...
generates all of $\EL_4(R)$.  This happens once, in the ring
$\mathcal C$ with generators $s_0,s_1,t_0,t_1$ and relations
$t_is_j=\delta_{ij}$, and the group $B=\EL_4(\mathcal C)$ then maps
...
trivial.  Every homomorphism from $B$ to an MF group is trivial; the
group $B$ is finitely generated, has property~\textup{(T)}, and is
normally generated by an involution; and there is a
homomorphism from $B$ to $\EL_n(R)$ whose image normally generates
$\EL_n(R)$.
...
\leanverified{Manuscript/OneSidedMFRadical/FullDefectRingEJZUnconditional}{GroupApproximation.Manuscript.OneSidedMFRadical.FullDefectRingEJZUnconditional.manuscriptFullComplementaryIdempotentsAllCharacteristics}
...
In positive characteristic the locally finite argument applies to
every nonzero complementary idempotent, including those that are
not full.

% New paper proof; the preceding full-idempotent Lean markers do not
% certify this positive-characteristic extension.
\begin{mainthm}[one-sided inverses in positive characteristic]
\label{thm:positive-characteristic}
Let $R$ be a countable unital associative ring of positive
characteristic.  If $s,t\in R$ satisfy $ts=1$ and $st\ne1$, then
$\EL_n(R)$ is not MF for every $n\ge4$.  More precisely, every
homomorphism from $\EL_n(R)$ to an MF group kills
$e_{ij}(1-st)$ for every $i\ne j$.
\end{mainthm}

The characteristic may be composite.  The ideal generated by $1-st$
in the subring generated by $s,t$ is a union of images of finite matrix rings,
which puts the compression defect in a locally finite normal
subgroup.  For the binary Jacobson algebra
$J=\F_2\langle s,t\mid ts=1\rangle$, the conclusion is exact:
\[
 \Rad_{\mathrm{MF}}(\EL_n(J))
 =\ker\bigl(\EL_n(J)\longrightarrow\EL_n(\F_2[z,z^{-1}])\bigr),
 \qquad n\ge4,
\]
where $\Rad_{\mathrm{MF}}$ is the intersection of the kernels of all
homomorphisms to MF groups.  This kernel is the finitary general
linear group over $\F_2$
(Corollary~\ref{cor:jacobson-mf-radical}).

In the following full-idempotent example $\EL_n(R)$ is moreover
simple.  Let $L_{\F_2}(1,2)$ denote the binary
...
\leanverified{Manuscript/OneSidedMFRadical/ReducedCStarConsequenceRankFour}{GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptReducedCStarConsequenceRankFour}
\leanverified{Manuscript/OneSidedMFRadical/ReducedCStarConsequenceRankFour}{GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptMaximalCStarNotFiniteRankFour}
\leanverified{Manuscript/OneSidedMFRadical/ReducedCStarConsequenceRankFour}{GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptNotEverySeparableStablyFiniteCStarAlgebraIsMFRankFour}
...
question.
\leanverified{Manuscript/OneSidedMFRadical/ReducedCStarConsequenceRankFour}{GroupApproximation.Manuscript.OneSidedMFRadical.manuscriptSeparableStablyFiniteNonMFCStarAlgebraExistsRankFour}
...
Eckhardt's proof for involutive wreath products
\cite[Theorem~4.3]{Eckhardt} uses a rescaled Hilbert--Schmidt
displacement cocycle.  Theorem~\ref{thm:finite-dimensional-orbit}
extends that method to locally finite-dimensional, possibly
noncommuting orbits.  Finite-dimensional intertwiner correction
controls the cocycle error directly relative to its displacement,
which also allows the finite orbit algebras to grow.
...
is trivial on $K$.  If a unital $C^*$-algebra generated by unitaries
$u_g$, $g\in G$, with $u_g\ne u_h$ for $g\ne h$, such as $C^*_{\max}(G)$
or $C^*_{\mathrm r}(G)$, embeds in a norm matrix corona, then $G$ is
MF\@.
...
$g\mapsto\iota(u_g)+1-\iota(1)$ is a corona homomorphism that is
injective on $G$.
\end{proof}
...
\begin{proof}
Put $H_n=(M_{d_n}(\C),\hsnorm{\,\cdot\,})$.  The estimate
\[
 \opnorm{\operatorname{Ad}(A)-\operatorname{Ad}(B)}
 \le2\opnorm{A-B}\qquad(A,B\in\U(d_n))
\]
makes
\[
 \widetilde\sigma(g)=[\operatorname{Ad}(V_n(g))]_n\in\U(\mathcal B),
 \qquad
 \mathcal B=\prod_nB(H_n)\big/\bigoplus_nB(H_n),
\]
a homomorphism.  The algebra $\mathcal B$ is a norm matrix corona with
coordinate sizes $d_n^2$.  Let $P\in\mathcal B$ be the image of $e_L$
and put $U=\widetilde\sigma(u)$.  The compression relation and
Lemmas~\ref{lem:kazhdan-projection-order} and~\ref{lem:stable-finite} give
\[
 U^*PU\le P,\qquad U^*PU=P,\qquad UP=PU.
\]

To read this identity on matrix sequences, fix a free ultrafilter
$\omega$ on $\mathbb N$.  The Hilbert ultraproduct $\mathcal H_\omega$
consists of sequences with $\sup_n\hsnorm{x_n}<\infty$, modulo those
with $\lim_\omega\hsnorm{x_n}=0$, and has inner product
\[
 \langle[x_n]_\omega,[y_n]_\omega\rangle
 =\lim_\omega\tr_{d_n}(y_n^*x_n).
\]
The corona $\mathcal B$ acts on $\mathcal H_\omega$ by
$[T_n]_n[x_n]_\omega=[T_nx_n]_\omega$.  In this representation, $P$
is the orthogonal projection onto the $L$-fixed vectors.  If
$(x_n)\in\mathcal C_2(V,L)$, then $[x_n]_\omega$ is $L$-fixed.
Since $U$ commutes with $P$, so are $U[x_n]_\omega$ and
$U^*[x_n]_\omega$.  Thus both sequences
$\operatorname{Ad}(V_n(u))^{\pm1}x_n$ commute asymptotically with
$V_n(\ell)$ in Hilbert--Schmidt norm along $\omega$, for every
$\ell\in L$.  This holds for every free ultrafilter, hence along the
ordinary sequence: a bounded nonnegative sequence with a nonzero
limsup has a nonzero limit along some free ultrafilter.  Both
$\operatorname{Ad}(V(u))$ and its inverse therefore preserve
$\mathcal C_2(V,L)$.
\end{proof}

% Written extension of the transport argument; no Lean marker is claimed.
\begin{corollary}[transport of asymptotic intertwiners]
\label{cor:intertwiner-transport}
Let $L\le G$ have property~\textup{(T)}, let $uLu^{-1}\le L$, and let
$V_n\colon G\to\U(d_n)$ and $W_n\colon G\to\U(e_n)$ be operator norm
asymptotic representations.  On $M_{d_n,e_n}(\C)$ use
\[
 \|X\|_{2,n}^2=(d_ne_n)^{-1/2}\operatorname{Tr}(X^*X).
\]
Let $\mathcal I_2(V,W;L)$ consist of the sequences $(X_n)$ with
$\sup_n\|X_n\|_{2,n}<\infty$ and
\[
 \|V_n(\ell)X_n-X_nW_n(\ell)\|_{2,n}\longrightarrow0
 \qquad(\ell\in L).
\]
Then $\mathcal T_u(X)_n=V_n(u)X_nW_n(u)^*$ satisfies
\[
 \mathcal T_u\bigl(\mathcal I_2(V,W;L)\bigr)
 =\mathcal I_2(V,W;L).
\]
Equivalently, restriction to the compressed subgroup creates no new
asymptotic intertwiners:
\[
 \mathcal I_2(V,W;uLu^{-1})=\mathcal I_2(V,W;L).
\]
\end{corollary}

\begin{proof}
On the rectangular Hilbert spaces define
$S_n(g)X=V_n(g)XW_n(g)^*$.  These are unitaries, with
\[
 \|S_n(gh)-S_n(g)S_n(h)\|
 \le\|V_n(gh)-V_n(g)V_n(h)\|
   +\|W_n(gh)-W_n(g)W_n(h)\|\longrightarrow0.
\]
Their classes give a homomorphism into the norm matrix corona with
coordinate sizes $d_ne_n$.  Apply the proof of
Theorem~\ref{thm:transport} to this homomorphism: its Kazhdan projection
commutes with $[S_n(u)]_n$, and its fixed vectors are exactly the
asymptotic intertwiners along each free ultrafilter.  Asymptotic
multiplicativity identifies $\mathcal T_u(\mathcal I_2(V,W;L))$
with $\mathcal I_2(V,W;uLu^{-1})$, giving the equivalent formulation.
\end{proof}
...
\begin{proof}
Let $\Theta\colon G\to\U(\mathcal Q_{\mathbf d})$ be a corona
homomorphism, and let $p$ be the image of the Kazhdan projection $e_K$.
Normality of $K$ makes its fixed-vector space $G$-invariant, so $p$
commutes with $\Theta(G)$.  If $\Theta$ is nontrivial on $K$, then
$q=1-p\ne0$.  Lemma~\ref{lem:central-corona-corner} gives an operator
norm asymptotic representation $W_n\colon G\to\U(r_n)$ realizing
$g\mapsto q\Theta(g)$ in a norm matrix corona.  The induced map
\[
 \pi\colon C^*_{\max}(K)\longrightarrow\mathcal Q_{\mathbf r}
\]
satisfies $\pi(e_K)=0$, since the corner kills $p$.

Fix a free ultrafilter $\omega$.  The formula
\[
 \tau([a_n]_n)=\lim_\omega\tr_{r_n}(a_n)
\]
defines a tracial state on $\mathcal Q_{\mathbf r}$.  The hypothesis
applies to $(W_n)$, so
\[
 |\tr_{r_n}(W_n(k))-1|
 \le\hsnorm{W_n(k)-1}\longrightarrow0\qquad(k\in K).
\]
Thus $\tau\circ\pi$ agrees with the trivial character $\chi_K$ on
every canonical unitary, hence on $C^*_{\max}(K)$ by linearity and
continuity.  Evaluating at the Kazhdan projection gives
\[
 0=\tau(\pi(e_K))=\chi_K(e_K)=1,
\]
a contradiction.
\end{proof}
...
\subsection{Locally finite-dimensional conjugacy orbits}
\label{subsec:finite-dimensional-orbits}

There is a second way to turn compression into operator norm collapse.
It applies when every finite part of the relevant conjugacy orbit
generates a finite-dimensional algebra.  We extend the rescaled
Hilbert--Schmidt cocycle argument of~\cite[Theorem~4.3]{Eckhardt} by
correcting covariance on each finite-dimensional algebra.  The orbit
elements may fail to commute, and the dimensions of these algebras may
grow without bound.

\begin{lemma}[finite-dimensional lifts and intertwiners]
\label{lem:finite-dimensional-covariance}
Let $D_1\subseteq D_2\subseteq\cdots$ be finite-dimensional unital
$C^*$-subalgebras of $\mathcal Q_{\mathbf d}$.  There are integers
$m(n)\to\infty$ and unital $*$-homomorphisms
$\rho_n\colon D_{m(n)}\to M_{d_n}(\C)$ such that, for each
$x\in\bigcup_mD_m$, the sequence $(\rho_n(x))$ represents $x$,
after assigning arbitrary values at the finitely many coordinates
where $x$ is not yet in the domain.

Moreover, let $D$ be a fixed finite-dimensional unital $C^*$-algebra,
let $\alpha_n,\beta_n\colon D\to M_{d_n}(\C)$ be unital
$*$-homomorphisms, and suppose $U_n\in\U(d_n)$ satisfies
\[
 \opnorm{U_n\alpha_n(x)U_n^*-\beta_n(x)}\longrightarrow0
 \qquad(x\in D).
\]
For all sufficiently large $n$ there is $W_n\in\U(d_n)$ with
\[
 \opnorm{W_n-U_n}\longrightarrow0,\qquad
 W_n\alpha_n(x)W_n^*=\beta_n(x)\quad(x\in D).
\]
\end{lemma}

\begin{proof}
We recall the finite-dimensional matrix-unit lifting argument.
For a fixed $D_m=\bigoplus_jM_{r_j}(\C)$, lift its matrix units
arbitrarily to bounded matrix sequences.  Their relations hold modulo
norm-null sequences.  Successively compressing and applying the
spectral cut at $1/2$ to the diagonal lifts gives mutually orthogonal
projections close to those lifts.  Their sum is a projection tending
in norm to $1$, hence equals $1$ for all sufficiently large coordinates.
Compress each first-column lift between its two diagonal projections
and take its polar part.  Since both products converge to the
corresponding diagonal projections, these polar parts have exactly
those initial and final projections.  Their pairwise products are
exact matrix units lifting the original ones.  This gives a unital
homomorphic lift of $D_m$ on a tail of the sequence.

Choose a basis of each $D_m$ and bounded lifts of all these basis
elements.  A diagonal choice of $m(n)$ makes the homomorphic lift at
coordinate $n$ differ from the chosen lifts by at most $1/m(n)$ on
the bases of $D_1,\ldots,D_{m(n)}$.  These are finitely many conditions
at each stage, and each holds on a tail.  Thus every fixed element
is lifted correctly.  Compatibility between different coordinates
is unnecessary.

For the second assertion, convergence is uniform on the unitary
group of $D$, because $D$ is finite-dimensional.  With normalized
Haar measure on that compact group, put
\[
 T_n=\int_{\U(D)}\beta_n(v)U_n\alpha_n(v)^*\,dv.
\]
Then $\opnorm{T_n-U_n}\to0$ and
$T_n\alpha_n(x)=\beta_n(x)T_n$ for every $x\in D$, by invariance of
Haar measure and linearity.  Eventually $T_n$ is invertible;
$T_n^*T_n$ commutes with $\alpha_n(D)$, so its polar part
$W_n=T_n(T_n^*T_n)^{-1/2}$ is the required unitary.
\end{proof}

% New paper proof; no exact Lean endpoint is claimed for this theorem.
\begin{theorem}[locally finite-dimensional orbit rigidity]
\label{thm:finite-dimensional-orbit}
Let $L\le G$ have property~\textup{(T)}, let $uLu^{-1}\le L$, and let
$\Theta\colon G\to\U(\mathcal Q_{\mathbf d})$ be a homomorphism.
Suppose $a\in\mathcal Q_{\mathbf d}$ commutes with
$\Theta(uLu^{-1})$ and, for every finite $F\subseteq L$, the unital
algebra
\[
 C^*(1,\Theta(g)a\Theta(g)^*:g\in F)
\]
is finite-dimensional.  Then $a$ commutes with $\Theta(L)$.
\end{theorem}

\begin{proof}
Write $a_g=\Theta(g)a\Theta(g)^*$, so $a_1=a$.
Enumerate $L$ and exhaust its orbit algebra by the finite-dimensional
algebras generated by the first finitely many $a_g$.
Use Lemma~\ref{lem:finite-dimensional-covariance} to choose
homomorphic lifts $\rho_n$ on growing algebras, and choose unitary
lifts $V_n(g)$ of $\Theta(g)$ for $g\in G$.  Put
\[
 c_n(g)=\rho_n(a_g-a)\qquad(g\in L),
\]
with value $0$ at the finitely many coordinates before the argument
belongs to the domain of $\rho_n$.
In this proof $\|\cdot\|_{\mathrm{HS}}$ is the \emph{unnormalized}
Hilbert--Schmidt norm.

Fix $h,g\in L$.  On the fixed algebra $D=C^*(1,a,a_g)$ compare the
two homomorphisms
$x\mapsto\rho_n(x)$ and
$x\mapsto\rho_n(\Theta(h)x\Theta(h)^*)$.
They are defined for all sufficiently large $n$ and are
asymptotically intertwined by $V_n(h)$.  The lemma gives a unitary
$W_n=W_n(h,g)$ with $\opnorm{W_n-V_n(h)}\to0$ and
\begin{equation}\label{eq:local-exact-covariance}
 c_n(hg)-c_n(h)=W_nc_n(g)W_n^*.
\end{equation}
Consequently, for each fixed pair $h,g$, eventually
\begin{equation}\label{eq:orbit-subadditivity}
 \|c_n(hg)\|_{\mathrm{HS}}
 \le\|c_n(h)\|_{\mathrm{HS}}+\|c_n(g)\|_{\mathrm{HS}}.
\end{equation}

Suppose that some $a_g\ne a$.  Choose a finite symmetric generating
set $S$ of $L$.  Some generator moves $a$, so, after passing to a
subsequence, the numbers
\[
 k_n=\max_{s\in S}\|c_n(s)\|_{\mathrm{HS}}
\]
are bounded below by a positive constant.  Repeatedly applying
\eqref{eq:orbit-subadditivity} along a word for $g$ gives, eventually,
\begin{equation}\label{eq:orbit-word-bound}
 \|c_n(g)\|_{\mathrm{HS}}\le |g|_S k_n
 \qquad(g\in L).
\end{equation}
Let $\mathcal H_\omega$ be the Hilbert ultraproduct of
$H_n=(M_{d_n}(\C),k_n^{-1}\|\cdot\|_{\mathrm{HS}})$ for a free
ultrafilter $\omega$.  The adjoint maps
$\sigma_n(g)=\operatorname{Ad}(V_n(g))$ define a homomorphism
\[
 \widetilde\sigma\colon G\longrightarrow\U(\mathcal B),
 \qquad
 \mathcal B=\prod_nB(H_n)\big/\bigoplus_nB(H_n),
\]
and hence a unitary representation $\sigma$ on $\mathcal H_\omega$.
Indeed the estimate
$\|\operatorname{Ad}(A)-\operatorname{Ad}(B)\|\le2\|A-B\|$
is unchanged by rescaling the Hilbert norm.

The vectors $c(g)=[c_n(g)]_\omega$ are well defined by
\eqref{eq:orbit-word-bound}.  Equations
\eqref{eq:local-exact-covariance} and \eqref{eq:orbit-word-bound} give
\[
 \frac{\|c_n(hg)-c_n(h)-\sigma_n(h)c_n(g)\|_{\mathrm{HS}}}{k_n}
 \le 2\opnorm{W_n(h,g)-V_n(h)}\,|g|_S\longrightarrow0.
\]
Thus $c(hg)=c(h)+\sigma(h)c(g)$.  Since $S$ is finite and $k_n$
is its maximum displacement, $\max_{s\in S}\|c(s)\|=1$.
Also $c(h)=0$ for $h\in uLu^{-1}$.

Property~\textup{(T)}, in the Delorme--Guichardet form
\cite[Theorem~2.12.4]{BHV}, gives $\xi\in\mathcal H_\omega$ such that
$c(g)=\sigma(g)\xi-\xi$ for $g\in L$.  Therefore $\xi$ is fixed by
$uLu^{-1}$.  Let $P\in\mathcal B$ be the image of $e_L$ and put
$U=\widetilde\sigma(u)$.  The algebra $\mathcal B$ is a norm matrix
corona, so Lemmas~\ref{lem:kazhdan-projection-order}
and~\ref{lem:stable-finite} give $U^*PU=P$, hence $UPU^*=P$.
Acting on $\mathcal H_\omega$, these last two projections have ranges
$\mathcal H_\omega^{uLu^{-1}}$ and $\mathcal H_\omega^L$.
Thus $\xi$ is $L$-fixed, contradicting $\max_{s\in S}\|c(s)\|=1$.
\end{proof}

The correction $W_n(h,g)$ depends on the fixed pair $h,g$.
Equation~\eqref{eq:local-exact-covariance} is all that the cocycle
argument uses; the corrections need not satisfy a multiplication law.
This is why growing finite-dimensional algebras cause no uniformity
problem.

\begin{corollary}[locally finite subgroups]
\label{cor:locally-finite-compression}
Let $L\le G$ have property~\textup{(T)}, let $uLu^{-1}\le L$, and let
$A\le G$ be locally finite and normalized by $L$.
If $b\in A$ centralizes $uLu^{-1}$, then every homomorphism from $G$
to an MF group kills $[b,\ell]$ for every $\ell\in L$.
\end{corollary}

\begin{proof}
For a corona homomorphism $\Theta$, apply
Theorem~\ref{thm:finite-dimensional-orbit} to $a=\Theta(b)$.
Finitely many $L$-conjugates of $b$ generate a finite subgroup of $A$,
whose complex group algebra has finite-dimensional image.
The conclusion passes to MF targets by
...
only part of the group.  The following tensor estimate turns a single
separation threshold into a linear bound.

\begin{lemma}[tensor amplification]\label{lem:linear-tensor-amplification}
If $A\in\U(d)$ and $t=\opnorm{A-1}>0$, there is an integer
$1\le k\le2/t$ with $\opnorm{A^{\otimes k}-1}>1$.
The universal constant $2$ is optimal.
...
\begin{proof}
Choose an eigenvalue $z=e^{i\theta}$ of $A$ with $|z-1|=t$ and
$0<|\theta|\le\pi$.  Write $\beta=|\theta|$.  If $t>1$, take $k=1$.
Otherwise $0<\beta\le\pi/3$, and
\[
 k=\left\lfloor\frac{\pi}{3\beta}\right\rfloor+1
 \quad\text{satisfies}\quad
 \frac{\pi}{3}<k\beta\le\frac{2\pi}{3},
\]
so $|z^k-1|>1$.  If $k=2$, then $kt\le2$; if $k\ge3$, then
\[
 kt=2k\sin(\beta/2)\le k\beta
 \le\frac{k}{k-1}\frac{\pi}{3}\le\frac{\pi}{2}<2.
\]
Tensoring an eigenvector for $z$ proves the first assertion.
The constant $2$ cannot be decreased, since for the scalar matrix
$A=-1$ every successful exponent is odd and $kt=2k\ge2$.
...
\eqref{eq:word-linear-certificate} with $C=2/\eta$.
The threshold also gives, for every integer $N\ge1$,
\begin{equation}\label{eq:word-sine-modulus}
 N\delta_{\mathcal R}(U)<\eta
 \quad\Longrightarrow\quad
 \opnorm{w(U)-1}<2\sin\frac{\pi}{6N}.
\end{equation}
\end{theorem}
...
$1\le k\le2/t$ with $\opnorm{w(U)^{\otimes k}-1}>1$.
...
\[
 ...
 \le\frac2t\delta_{\mathcal R}(U),
\]
which proves \eqref{eq:word-linear-certificate}.

For \eqref{eq:word-sine-modulus}, apply the threshold to each tensor
power $U^{\otimes j}$, $1\le j\le N$.  Every eigenvalue
$z=e^{i\theta}$ of $w(U)$, with $-\pi<\theta\le\pi$, then satisfies
$|z^j-1|<1$ for all such $j$.  In particular $|\theta|<\pi/3$.
If $N|\theta|\ge\pi/3$, the first $j$ with $j|\theta|\ge\pi/3$
satisfies $j\le N$ and $\pi/3\le j|\theta|<2\pi/3$, contradicting
$|z^j-1|<1$.  Thus $|\theta|<\pi/(3N)$ for every eigenvalue, proving
the bound.
...
The sine bound also gives the asymptotic slope.  For every
$\varepsilon>0$ there is $\delta_0>0$ such that, uniformly in the
dimension,
\[
 0<\delta_{\mathcal R}(U)<\delta_0
 \quad\Longrightarrow\quad
 \opnorm{w(U)-1}
 \le\left(\frac{\pi}{3\eta}+\varepsilon\right)
       \delta_{\mathcal R}(U).
\]
Indeed, take $N=\lceil\eta/\delta_{\mathcal R}(U)\rceil-1$ in
\eqref{eq:word-sine-modulus}; then
$N\delta_{\mathcal R}(U)\to\eta$ as the defect tends to zero.
At zero defect the word is exactly $1$ by
\eqref{eq:word-linear-certificate}.
...
A single rank-four calculation supplies both ring obstructions:
three coordinates carry the compressed elementary group, and the
fourth carries a centralizing element.  In positive characteristic,
the complementary ideal gives a locally finite subgroup.  Under
the full-idempotent hypothesis, normal generation instead kills the
whole elementary group and permits descent to rank two.

\begin{lemma}[a rank-four compression cell]\label{lem:ring-compression-cell}
Let $R$ be a unital ring, let $s,t\in R$ satisfy $ts=1$, and put
$e=1-st$.  In $G=\EL_4(R)$, let $L=\EL_3(R)$ act on coordinates
...
$sa\,te\,t^{j-1}=0$ in position $(i,4)$.  So $uLu^{-1}\le L$.
...
\begin{lemma}[an additively torsion complementary ideal]
\label{lem:finite-complementary-ideal}
Let $R$ be a unital ring, let $ts=1$, and suppose $q(1-st)=0$
for some integer $q\ge1$.  Let
$S\subseteq R$ be the unital subring generated by $s,t$.
Put $e=1-st$ and $I=SeS$.  Then $I$ is a union of finite subrings,
and
\[
 A_r=\GL_r(S)\cap(1+M_r(I))
\]
is a locally finite normal subgroup of $\GL_r(S)$ for every $r\ge1$.
\end{lemma}

\begin{proof}
Reducing words with $ts=1$ shows
that $S$ is spanned over $\mathbb Z$ by the elements $s^it^j$, $i,j\ge0$.
Since $te=es=0$, the ideal $I$ is spanned by
$f_{ij}=s^iet^j$.  The relations
\[
 f_{ij}f_{kl}=\delta_{jk}f_{il}
\]
show that
$I_N=\operatorname{span}_{\mathbb Z}\{f_{ij}:0\le i,j<N\}$
is a subring with identity $p_N=\sum_{i<N}f_{ii}$.
Since $qf_{ij}=0$, this subring is finite.
Thus $I=\bigcup_N I_N$.

The group $A_r$ is the kernel of reduction
$\GL_r(S)\to\GL_r(S/I)$, hence normal.  For any finite subset of
$A_r$, choose $N$ containing all entries of $g-1$ and $g^{-1}-1$
for its elements $g$.  With $P=\operatorname{diag}(p_N,\ldots,p_N)$,
each such element has the form
$1-P+v$, where $v$ is a unit of the finite ring $M_r(I_N)$.
These elements lie in the finite group
$\{1-P+v:v\in M_r(I_N)^\times\}$.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:positive-characteristic}]
Use $S,e,I$ from Lemma~\ref{lem:finite-complementary-ideal}.
In $G=\EL_4(S)$ take $L=\EL_3(S)$ and the elements $u,c$ of
Lemma~\ref{lem:ring-compression-cell}.  The ring $S$ is finitely
generated, so $L$ has property~\textup{(T)}
by~\cite[Theorem~1.1]{EJZ}.  The element
$b=ucu^{-1}=e_{12}(e)$ belongs to the locally finite normal subgroup
$G\cap(1+M_4(I))$ and centralizes $uLu^{-1}$.  By
Corollary~\ref{cor:locally-finite-compression}, every MF homomorphism
of $G$ kills
\[
 [b,e_{23}(1)]=e_{13}(e)\ne1.
\]
For $n\ge4$, restrict a homomorphism of $\EL_n(R)$ to the embedded
copy of $G$.  Elementary signed permutation matrices conjugate
$e_{13}(e)$ to every $e_{ij}(\pm e)$, so all the stated roots are
killed.  Since these roots are nontrivial, $\EL_n(R)$ is not MF.
\end{proof}

The proof works in arbitrary characteristic whenever the nonzero
idempotent $1-st$ has finite additive order: that is the only
finiteness assumption in Lemma~\ref{lem:finite-complementary-ideal}.

\begin{corollary}[the binary Jacobson MF radical]
\label{cor:jacobson-mf-radical}
Let $J=\F_2\langle s,t\mid ts=1\rangle$ and let $n\ge4$.
The symbol map $s\mapsto z$, $t\mapsto z^{-1}$ gives an exact sequence
\[
 1\longrightarrow\GL_{\mathrm{fin}}(\mathbb N\times\{1,\ldots,n\},\F_2)
 \longrightarrow\EL_n(J)
 \longrightarrow\EL_n(\F_2[z,z^{-1}])\longrightarrow1.
\]
Its kernel is exactly $\Rad_{\mathrm{MF}}(\EL_n(J))$.
Consequently the universal MF quotient is the displayed Laurent
elementary group, which is residually finite.
\end{corollary}

\begin{proof}
Represent $s,t$ as the forward and backward shifts on
$V=\bigoplus_{j\ge0}\F_2v_j$, with $tv_0=0$.
Then $e=1-st$ is the projection onto $\F_2v_0$, and
$s^iet^j$ are the ordinary matrix units on $V$.
They are linearly independent, span the ideal $I=JeJ$, and
$J/I=\F_2[z,z^{-1}]$.  This shift representation is faithful:
outside finitely many basis vectors a finite sum of $s^it^j$
acts by its Laurent symbol; if it acts as zero, its symbol is
zero, and it lies in $I$, where the matrix-unit action is faithful.

The symbol map on elementary groups is surjective by lifting root
coefficients.  Its kernel consists of invertible operators on
$V^n$ differing from $1$ by a matrix with finite support.
Every finite invertible matrix over $\F_2$ has determinant $1$
and is generated by transvections.  Transvections between different
coordinate rays are the roots $e_{rs}(s^iet^j)$.
Commuting two such transvections through another ray produces any
transvection within one ray.  Hence the entire finitary general
linear group lies in $\EL_n(J)$, proving the exact sequence.

Theorem~\ref{thm:positive-characteristic} kills the transvection
$e_{13}(e)$ in every MF homomorphism.  Conjugation by finitary
permutation matrices carries it to every finitary transvection.
These generate the kernel, so that kernel lies in the MF radical.
Conversely, every nonidentity matrix over $\F_2[z,z^{-1}]$ remains
nonidentity after reduction modulo some irreducible polynomial
different from $z$: choose one not dividing a nonzero entry of its
difference from $1$.  These reductions take values in finite
groups and separate the Laurent elementary group.  Thus an
element outside the kernel survives in a finite, hence MF,
quotient, proving the reverse inclusion.
\end{proof}

\subsection{Full complementary idempotents}
...
\emph{The group $B$.}  In $\mathcal C$ put $s=s_0$, $t=t_0$, and
$e=1-st$.  Then $ts=1$, $e^2=e$, $es=te=0$, and $t_1es_1=1$.  Put
$G=B=\EL_4(\mathcal C)$ and $L=\EL_3(\mathcal C)$ on coordinates
$1,2,3$.  The ring $\mathcal C$ is nonzero, since it maps onto
$L_{\F_2}(1,2)$, and finitely generated, so both groups
have property~\textup{(T)} by~\cite[Theorem~1.1]{EJZ}, and $B$ is
finitely generated~\cite[Theorem~1.3.1]{BHV}.

Lemma~\ref{lem:ring-compression-cell} gives $u,c\in G$ and
$\ell=e_{23}(1)\in L$ with
$d=[ucu^{-1},\ell]=e_{13}(e)\in\mathfrak D_G(L)$.
...
Write $w_{ij}=e_{ij}(1)e_{ji}(-1)e_{ij}(1)$ and
$\sigma=w_{12}w_{34}w_{13}^2$, the permutation matrix of $(12)(34)$, an
involution in $B$.  Conjugation by $\sigma$ sends $e_{13}(-r)$ to
$e_{24}(-r)$, so for $r\in\mathcal C$
\[
 [e_{13}(r),\sigma]=e_{13}(r)e_{24}(-r),\qquad
 [e_{12}(1),[e_{13}(r),\sigma]]=e_{14}(-r);
\]
with signed permutation matrices, $\sigma$ normally generates $B$.
...
hence trivial, and the image of $B$ in that copy normally generates
$\EL_n(R)$.
\end{proof}
...
they generate.  The compressor $u$ from
Lemma~\ref{lem:ring-compression-cell} lies in $\EL_4(S)$ and conjugates
...
instead $S=\varphi(\mathcal C)$ and the map $\Psi$ from the rank-two
part of its proof, with $L=\EL_3(S)$ and $u$ the image in $\EL_4(S)$ of the compressor of
$B$; here $v_0w_0\ne1$, since $w_1v_0=0$ and $w_1v_1=1$.  Then $\Psi$
...
Let $R$ be a countable purely infinite simple ring and let $n\ge1$.
...
The locally finite-dimensional orbit theorem applies to finite perfect
lamp groups whose normal closure in the ambient group is itself
residually finite.  We first record its fixed-algebra special case.
...
\begin{proof}
For $a\in A^{tLt^{-1}}$, every finite part of its $L$-conjugacy orbit
generates a subalgebra of the finite-dimensional algebra $A$.
Theorem~\ref{thm:finite-dimensional-orbit} therefore gives $a\in A^L$.
The reverse inclusion follows from $tLt^{-1}\le L$.
For a finite subgroup $F$ normalized by $L$, use
$A=C^*(1,\rho(F))$ and $a=\rho(f)$.  Homomorphisms to MF groups reduce
to corona homomorphisms by Lemma~\ref{prop:mf-residual-calculus}.
\end{proof}

The relative error estimate in the proof of
Theorem~\ref{thm:finite-dimensional-orbit} applies even when a
nonzero displacement has vanishing normalized Hilbert--Schmidt mass.
...
\]
is residually finite.  Define
\[
 \Gamma^+=t^{-1}\Gamma t,\qquad X=V/\Gamma,
 \qquad Y=V/\Gamma^+,\qquad
 p\colon X\to Y,\quad v\Gamma\mapsto v\Gamma^+.
\]
Each fiber of $p$ has $m$ points.  For a finite group $K$, let $K_x$
be a labeled copy of $K$ at $x\in X$ and put
\begin{equation}\label{eq:finite-block-group}
 B_K=\mathop{*}_{y\in Y}
          \left(\prod_{x\in p^{-1}(y)}K_x\right),
 \qquad E_K=B_K\rtimes V,
\end{equation}
where $V$ acts by permuting the labeled coordinates.  This is an
ordinary free product of the finite block groups.  Its normal form
embeds every coordinate and every block.
...
nonabelian simple, any nonidentity element of a coordinate $K_x$
normally generates $B_K$ in $E_K$.
\end{theorem}

\begin{proof}
We first verify finite presentability directly.  Choose finite
presentations $\Gamma=\langle S\mid R\rangle$ and
$K=\langle Z\mid T\rangle$, and representatives
$b_1\Gamma=\Gamma,b_2\Gamma,\ldots,b_m\Gamma$ for
$\Gamma^+/\Gamma$.  Regard $b_i$ and $\alpha(s)$ as words in
$S,t$.  A finite presentation for $E_K$ has generators $S,t,Z$
and relations
\begin{equation}\label{eq:finite-block-presentation}
\begin{gathered}
 R,\quad T,\quad
 t s t^{-1}=\alpha(s)\quad(s\in S),\qquad
 [z,s]=1\quad(z\in Z,\ s\in S),\\
 [z,b_i z'b_i^{-1}]=1
 \quad(z,z'\in Z,\ 2\le i\le m).
\end{gathered}
\end{equation}
Before the last family is imposed, the group is
$V*_{\Gamma}(\Gamma\times K)$.  Its retraction to $V$ has kernel
$\mathop{*}_{x\in X}K_x$: conjugating the root copy of $K$ gives
the coordinate $K_{v\Gamma}$, well defined because the root copy
commutes with $\Gamma$.  These assignments and the map in the
opposite direction between the two presentations are inverse.
The last family makes the root coordinate commute with every other
coordinate in its $p$-fiber.  Its $V$-conjugates impose exactly
commutation within each fiber: any pair of distinct points of one
fiber has the form $v\Gamma,vb_i\Gamma$ for some $v$ and $i>1$.
The resulting presentation is precisely
\eqref{eq:finite-block-group}.

Now let $\rho$ be a corona homomorphism from $E_K$.  The block
above $t\Gamma^+\in Y$ consists of
$\{\gamma t\Gamma:\gamma\in\Gamma\}$ and is
$\Gamma$-invariant.  Its finite group $F\cong K^m$ is normalized
by $\Gamma$.  For $k\in K$, the element $k_{t\Gamma}$ is fixed
by $t\Gamma t^{-1}$.  Theorem~\ref{thm:finite-algebra-transport}
therefore gives
\[
 \rho(k_{\gamma t\Gamma})=\rho(k_{t\Gamma})
 \qquad(\gamma\in\Gamma).
\]
Conjugation by $V$ identifies the images of the copies of $k$
throughout any fiber of $p$.  Since $m>1$, two distinct coordinates
in that fiber commute.  Their identified image is therefore an
abelian image of $K$, hence trivial by perfection.  Thus $\rho$
kills every coordinate and consequently $B_K$.  All MF
homomorphisms kill $B_K$ by Lemma~\ref{prop:mf-residual-calculus}.
Conversely, the residually finite quotient $V$ is MF and separates
every element outside $B_K$.  This proves
$\operatorname{Rad}_{\mathrm{MF}}(E_K)=B_K$.  Finite groups are MF,
and the finite quotients of $V$ give the same two containments for
the intersection of finite-index normal subgroups.

A finite free product of finite groups is residually finite.  One
way to see this is to map it to the direct product of the factors.
The kernel has finite index and meets each conjugate factor
trivially; its action on the free-product tree is free, so it is a
free group.  Free groups are residually finite, and so are their
finite-index overgroups.  For the latter assertion, separate an
element in a finite-index normal subgroup, then intersect the
finitely many conjugates of the separating finite-index normal
subgroup.  Every nontrivial element of $B_K$ survives the
retraction onto a finite sub-free-product, proving that $B_K$ is
residually finite.  It is perfect because all its factors $K^m$
are perfect and they generate it.

Write $\Gamma_n=t^{-n}\Gamma t^n$ and
$T_\alpha=\bigcup_{n\ge0}\Gamma_n$.  Then
\[
 V=T_\alpha\rtimes\mathbb Z,
 \qquad E_K=J_K\rtimes\mathbb Z,
 \qquad J_K=B_K\rtimes T_\alpha.
\]
As in Proposition~\ref{prop:clifford-locally-rf}, finite index of
$\alpha(\Gamma)$ makes $\Gamma$ commensurated by $V$, so every
$\Gamma_n$-orbit in $X$ and in $Y$ is finite.  A finite subset of
$J_K$ uses finitely many blocks and actor coordinates in one
$\Gamma_n$.  Saturating those blocks under $\Gamma_n$ gives a
finite invariant set $I\subseteq Y$.  The finite subset lies in
\[
 M_I\rtimes\Gamma_n,
 \qquad M_I=\mathop{*}_{y\in I}K^m.
\]
The action of $\Gamma_n$ on $M_I$ factors through its permutations
of the $m|I|$ labeled sites.  Its kernel $\Lambda$ has finite
index and centralizes $M_I$.  Thus $M_I\times\Lambda$ is a
residually finite subgroup of finite index in
$M_I\rtimes\Gamma_n$, proving that $J_K$ is locally residually
finite.  Soficity now follows from the directed-union and
amenable-extension permanence theorems~\cite[Theorem~1]{ElekSzabo}.
Proposition~\ref{prop:locally-rf-by-z-trace} makes the canonical
trace amenable, and Theorem~\ref{thm:factorization-nonmf-trace}
makes it nonquasidiagonal, since $B_K\ne1$ and $E_K$ is not MF.

Finally, for nonabelian simple $K$, the normal closure of a
nonidentity $k\in K_x$ contains $K_x$, then all its $V$-conjugates
$K_{vx}$, and hence $B_K$.  The reverse containment follows from
normality of $B_K$.
\end{proof}
...
$\Gamma^+$, so $Y=V/\Gamma^+$ is countably infinite.  Hence
