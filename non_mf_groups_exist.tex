...
normal property-\textup{(T)} subgroup of the resulting commutator defect.
...
quasidiagonal, and a torsion-free group with no nontrivial homomorphism
to an MF group.
\end{abstract}
...
Hilbert--Schmidt norm (Corollary~\ref{cor:defect-hs}).  Second, for a
normal property-\textup{(T)} subgroup $K\le\mathfrak D_G(L)$, a corona
homomorphism nontrivial on $K$ compresses to a corner, renormalized by its
own rank, on which the Kazhdan projection of $K$ vanishes while the
limiting trace is the trivial character of $K$, which takes the value $1$
on that projection (Theorem~\ref{thm:normal-kazhdan}).\footnote{A
...
property~\textup{(T)}~\cite[Theorem~1.1]{EJZ}.  Put $f_{ij}=s^iet^j$.
...
for $n\ge4$.  Here the inclusion is an equality.  The algebra $J$ acts
