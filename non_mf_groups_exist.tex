...
$\rho\colon G\to\U(d)$ the same conclusion holds without
property~\textup{(T)}.  If $uLu^{-1}\le L$, then conjugation by
$\rho(u)^*$ is an injective linear map of the commutant $\rho(L)'$ into
itself, so it is onto.  For $c\in C_G(L)$ the element
$\rho(ucu^{-1})=\rho(u)\rho(c)\rho(u)^*$ therefore lies in $\rho(L)'$,
and $\rho$ is trivial on $\mathfrak D_G(L)$.  In a norm matrix corona
this dimension count is not available, and Theorem~\ref{thm:transport}
uses property~\textup{(T)} and stable finiteness in its place.
