...
subgroup conjugated into a proper subgroup of itself.  The unit group of
the binary Leavitt algebra is a finitely generated simple
property-\textup{(T)} group with no nontrivial homomorphism to an MF
group, and its reduced $C^*$-algebra is separable and stably finite but
...
The hypothesis passes up from corners: if a nonzero idempotent $p\in R$
is such that the ring $pRp$, with unit $p$, satisfies the hypothesis, then
$A\mapsto A+(1-p)I_n$ embeds $\EL_n(pRp)$ in $\EL_n(R)$, and restricting
models shows that MF passes to subgroups, so $\EL_n(R)$ is not MF for
every $n\ge2$, although its homomorphisms to MF groups need no longer be
trivial.
