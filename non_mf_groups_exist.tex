...
The torsion-defect obstruction also computes entire MF radicals in a
different family.  For a homeomorphism $T$ of a compact metrizable
zero-dimensional space $X$, let $R_X$ be the algebraic crossed product
of the locally constant $\F_2$-valued functions by $T$.  In
Section~\ref{sec:chain-core} we prove that, for $n\ge4$, the MF radical
of $\EL_n(R_X)$ is exactly the kernel of restriction to the
chain-recurrent set.  This kernel is locally finite and the quotient
is locally embeddable into finite groups.  Over every finite field,
the same restriction is the universal directly finite ring quotient.
...
Take $k=\F_2$ and $n\ge4$.  Then
\[
 \Rad_{\mathrm{MF}}(\EL_n(R_X))
 =\ker\bigl(\EL_n(R_X)\longrightarrow\EL_n(R_Y)\bigr)
 =\EL_n(R_X,I).
\]
This kernel is locally finite and the restriction map is surjective
with LEF quotient.  The MF radical of $\GL_n(R_X)$ is the same kernel;
its maximal MF quotient is its restriction image in $\GL_n(R_Y)$.
\end{theorem}

\begin{proof}
The ring $R_X$ is countable: $X$ has a countable clopen basis, and
each clopen subset is a finite union of basis sets.  Each defect $d$
in \eqref{eq:clopen-defect-pair} has additive order at most two.
Proposition~\ref{prop:torsion-defect-ring} therefore kills every root
with coefficient in $R_XdR_X$ under every MF homomorphism.  These
ideals generate $I$, so root addition kills all roots with
coefficients in $I$ and their normal closure $\EL_n(R_X,I)$.

We identify the full restriction kernel.  If $g\in\GL_n(R_X)$
...
radical inclusion.  For $\GL_n(R_X)$, restriction of any MF
homomorphism to the elementary subgroup kills the same kernel, while
the restriction image is a subgroup of the LEF group $\GL_n(R_Y)$.
\end{proof}

In particular, these groups are MF exactly when $X$ is entirely
chain recurrent, in which case they are LEF.  The ring theorem holds
over every finite field; the exact kernel computation for groups
uses $\F_2$.  No finite generation or property~\textup{(T)} of the
ambient crossed-product groups is required.
