...
product of finitely many finite groups acts on its Bass--Serre tree
with finite stabilizers, so it is virtually free; free groups are
residually finite, and so is any group containing one with finite
index.  So $B_K$ is residually finite.
...
it has property~(T) and is finitely presented, by a finite presentation of $\mathrm{SL}_3(\mathbb Z)$
together with its action on the three commuting translation
generators.  Here $m=8$, and the faithful matrix model for $V$ displayed after
