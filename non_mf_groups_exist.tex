...
particular, $H=\EL_4(L_{\F_2}(1,2))$ is a finitely generated simple
...
\[
 H=\EL_4(L_{\F_2}(1,2)),
\]
the elementary group of $4\times4$ matrices over the binary Leavitt
algebra, because $H$ is simple and every homomorphism from $H$ to an
MF group is trivial.  Khanh--Thanh show that $H$ is isomorphic to the unit group of the
...
is isomorphic to $H$.}  OpenAI used this group to construct the first nonsofic
...
Put $R=L_{\F_2}(1,2)$ and $H=\EL_4(R)$.  Then $H$ is finitely
...
$C^*_{\max}(W)$ is not nuclear, since $W$ is not amenable.  Since $K$ is a direct limit of
...
$\xi=[x_n]_\omega\in\operatorname{Fix}\sigma(L)$.  By the same estimate,
\[
...
\begin{proof}
Lift \(q\) to projections \(q_n\) by functional calculus, and lift each
\(\rho(g)\) to unitaries \(U_n(g)\) by polar decomposition, with
...
norm asymptotic representation with \(W_n(1)=I_{r_n}\), and since
\(\opnorm{J_nW_n(g)J_n^*-q_nU_n(g)q_n}\to0\), the class of
\((J_nW_n(g)J_n^*)\) is the coordinate restriction of \(q\rho(g)\).
\end{proof}
...
\subsection{A finite certificate}
...
\begin{proof}
The group $G$ is perfect: a nontrivial abelianization of a finitely
generated group has a nontrivial finite quotient, and finite groups are
MF\@.  So there are words $a_{ik},b_{ik}\in F_m$ with
\[
 g_i=\prod_{k=1}^{q_i}[a_{ik}(g_1,\ldots,g_m),b_{ik}(g_1,\ldots,g_m)]
 \qquad(1\le i\le m),
\]
and the words $w_i=x_i^{-1}\prod_{k=1}^{q_i}[a_{ik},b_{ik}]$ are
relations of $G$.  For a tuple $U=(U_1,\ldots,U_m)$ of unitaries put
$D(U)=\max_i\opnorm{U_i-1}$.  A word $a$ of length $|a|$ has
$\opnorm{a(U)-1}\le|a|\,D(U)$, and unitaries $A,B$ satisfy
\[
 \opnorm{[A,B]-1}=\opnorm{AB-BA}\le2\opnorm{A-1}\,\opnorm{B-1},
\]
since $AB-BA=(A-1)(B-1)-(B-1)(A-1)$.  So there is a constant $B_0$,
determined by the chosen words, with
\begin{equation}\label{eq:bootstrap}
 D(U)\le\max_i\opnorm{w_i(U)-1}+B_0\,D(U)^2 .
\end{equation}
Enumerate the relations of $G$ as $r_1,r_2,\ldots$ with
$w_1,\ldots,w_m$ first, and put
$\mathcal R_n=\{r_1,\ldots,r_n\}$.  If no $\mathcal R_n$ works with
the constant $n$, there are $d_n$ and tuples $U^{(n)}$ in $\U(d_n)$
with
\[
 D(U^{(n)})>n\,\delta_n,
 \qquad
 \delta_n=\max_{r\in\mathcal R_n}\opnorm{r(U^{(n)})-1}.
\]
Since $D(U^{(n)})\le2$, $\delta_n\to0$, so
$g_i\mapsto[U_i^{(n)}]_n$ is a homomorphism from $G$ to the unitary
group of $\prod_nM_{d_n}(\C)/\bigoplus_nM_{d_n}(\C)$, trivial by
hypothesis; so $D(U^{(n)})\to0$.  For $n\ge m$, the
inequality~\eqref{eq:bootstrap} gives
$D(U^{(n)})\le\delta_n+B_0\,D(U^{(n)})^2$, and division by
$D(U^{(n)})$ gives $1\le\tfrac1n+B_0\,D(U^{(n)})$, which fails for
large $n$.

Conversely, a corona homomorphism has unitary coordinate lifts whose
relation defects tend to zero, so the displayed inequality forces every
generator lift to the identity, and the homomorphism is trivial;
Lemma~\ref{prop:mf-residual-calculus} extends this to every
...
three for the subgroup $\EL_3(R)$, which has property~\textup{(T)} once
$R$ is replaced by a finitely generated unital subring, and one for its
centralizer.  The compressor needs only $ts=1$; the ideal condition on
$1-st$ is used only in the last step of the proof.  Indices run over
...
$1-s_\nu t_\nu$ that together generate the unit ideal: the construction
above, applied to each pair, shows that
$e_{13}(1-s_\nu t_\nu)\in\mathfrak D_G(L)$, and by the two Steinberg
relations above $e_{42}(1)$ lies in the normal closure of these
elements.}
\end{proof}
...
Put $R=L_{\F_2}(1,2)$ and $H=\EL_4(R)$.  By~\eqref{eq:leavitt}, the
...
The group $H=\EL_4(R)$ is nontrivial and simple.
...
norm.  If $g\ne1$ and $\opnorm{V_n(g)-1}\to0$, then
$\tr_{d_n}(\phi_n(u_g))\to1$, contradicting
$\tr_{d_n}(\phi_n(u_g))\to\tau_G(u_g)=0$.  So
$\limsup_n\opnorm{V_n(g)-1}>0$ for every $g\ne1$, and $G$ is MF, contrary
to the hypothesis.  The last assertion follows.
\end{proof}
...
$t$ is the generator of the $\mathbb Z$ factor, so $txt^{-1}=\beta(x)$
for $x\in N$.  Call $k_g$ the height
...
\]
This map is u.c.p., since it is the compression of a unitary
representation.
...
each $\Gamma_n$ has finite index in $\Gamma_{n+1}$, so all the $\Gamma_n$
are commensurable.  In fact $\Gamma$ is commensurated by $V$: every
element of $T_\alpha$ lies in some $\Gamma_m$, and conjugation by $t$
maps $\Gamma_{m+1}$ onto $\Gamma_m$.  Hence the stabilizer in $\Gamma_n$ of every point
...
with injectivity on a ball of $\operatorname{Cay}(G,A)$; since every finite subset
...
consists of two applications of Theorem~\ref{thm:hull} to the free
product $G*Q$, so $P$ is torsion-free and finitely presented, as in the
remark after Theorem~\ref{thm:hull}.  As a quotient of $Q$, the group
...
faithful, so it is stably finite; and an embedding into a norm matrix
corona would embed $Q$ into the corona's unitary group, against
