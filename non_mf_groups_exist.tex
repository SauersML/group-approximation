...
In August 2026 the author set AI agents on the question whether every
group is hyperlinear, starting from the nonsofic group of~\cite{OAI};
the repository behind this paper opened on August~1.  One attempted
route went through weak MF approximations of a Clifford-twisted wreath
product.  The author had suggested pivoting to the MF question,
against the model's advice; asked instead to establish the MF property
of the Clifford extension as a step toward the hyperlinear problem,
GPT-5.6 Sol reversed the route: on August~9 the repository records the
first form of the obstruction, that every norm-matrix model kills the
Clifford sign.  Sessions not given the MF suggestion found nothing
comparable.  On August~12 the obstruction was localized to a single
marked commutator in a finitely presented group, and the first draft
of this paper was written the same evening.  Experts were consulted
from August~13, and their comments shaped the revisions.

Attributing what followed to the author or to the models separately
would misdescribe the work: over thousands of prompts, suggestions,
refusals, proofs, and counterexamples moved in both directions, and
the correspondence with experts fed the same exchange.  The commit
history of the public repository records it.  Of the original proof,
the one-sided transport step survives unchanged; the central sign
became the special case of
Theorem~\ref{thm:compression-criterion} in which $K$ is generated by
one central involution; the original witness returns as the group
behind Theorem~\ref{thm:amenable-trace}; and the simple Leavitt
example, the ring-theoretic criterion, and the torsion-free group came
later.
...
binary Leavitt algebra~\cite[Proposition~4.2 and
Corollary~4.4]{KhanhThanh}.\footnote{By the same results
$\GL_n(R)=\EL_n(R)\cong R^\times$ for every $n\ge2$, so every $\EL_n(R)$
is isomorphic to $H$; the rank $12$ is the one checked in the Lean
development.}  OpenAI used this group to construct the first nonsofic
...
For the shortest route to a non-MF group, the reader may start with
Eckhardt's expository note~\cite{Eckhardt} and return here for the
compression criterion, the simple and torsion-free examples, and the
amenable trace.
...
group is MF was a longstanding open problem~\cite{Korchagin,BDL}.
...
and its ascending HNN extension $G$, the non-finiteness of
$C^*_{\max}(G)$ in Proposition~\ref{prop:max-infinite} also appears in
Eckhardt's note~\cite{Eckhardt}, which further proves that the wreath
product $(\bigoplus_{G/\Gamma}\mathbb Z/2\mathbb Z)\rtimes G$ is not
MF\@.  His note gives a short elementary route to a non-MF group and is
a good first read on the compression mechanism.  In Section~\ref{sec:amenable-nonqd} the lamps
...
Corollary~\ref{cor:defect-hs} has a finite-dimensional counterpart that
needs no property~\textup{(T)}.  If $\rho\colon G\to\U(d)$ is a unitary
representation and $uLu^{-1}\le L$, then $x\mapsto\rho(u)^*x\rho(u)$ maps
the commutant $\rho(L)'$ into itself, hence onto it, because $\rho(L)'$
is finite dimensional; so $\rho(ucu^{-1})$ commutes with $\rho(L)$ for
every $c\in C_G(L)$, and every finite-dimensional unitary representation
of $G$ is trivial on $\mathfrak D_G(L)$.  Property~\textup{(T)} and
stable finiteness of the corona replace this dimension count.
...
subgroups cover $\EL_n(R)$.\footnote{The same argument applies when
finitely many pairs $t_\nu s_\nu=1$ have complementary idempotents
$1-s_\nu t_\nu$ that together generate the unit ideal: each pair gives a
compressor and a centralizer for the same $L$, so each
$e_{13}(1-s_\nu t_\nu)$ lies in $\mathfrak D_G(L)$, and the two Steinberg
relations above put $e_{42}(1)$ in their normal closure.}
\end{proof}
...
since every finite subset of $G$ lies in a ball of $\Gamma(G,A')$.  Apply
...
but related problem, after the author mentioned the possibility of the
...
not discover the key MF result.  The obstruction and the
non-finiteness of the maximal group $C^*$-algebra were first recorded
in the repository on August~9, 2026, the finitely presented extraction
and the first manuscript draft followed on August~12, and the Lean
development was registered at Palomar on August~24
(PALOMAR-2026-08-24-000006).  The exact prompt given to the instance
leading to the non-MF proof was:
\begin{quote}
...
result.  You got this!''
\end{quote}
The statements that were checked
