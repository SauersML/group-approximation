...
indeed $\GL_n(R)=\EL_n(R)\cong R^\times$ for every $n\ge2$ by the same
results, so $H$ does not depend on $n$ up to isomorphism, and $n=12$ is
the rank used in the accompanying Lean development.  OpenAI used this
group to construct the first nonsofic group~\cite[Chapter~3]{OAI}, so $H$
is neither sofic nor MF\@.
...
Whether every countable group is MF was a longstanding open
problem~\cite{CDE,Korchagin,BDL}.  Eckhardt's preprint~\cite{Eckhardt},
which appeared on 28 August 2026 while this manuscript was being prepared
and which cites it, answers the question by the same mechanism: for a
property-\textup{(T)} group $\Gamma$ with a proper self-embedding and its
ascending HNN extension $G$, the wreath product
$(\bigoplus_{G/\Gamma}\mathbb Z/2\mathbb Z)\rtimes G$ is not MF, and the
reduced $C^*$-algebra of that group is stably finite and not MF\@.  The
contribution here is the general criterion of
Theorem~\ref{thm:compression-criterion}, the ring-theoretic
Theorem~\ref{thm:full-defect-ring} and the finitely generated simple
group $H$, the sofic example and its trace in
Theorem~\ref{thm:amenable-trace}, and the torsion-free finitely presented
group of Theorem~\ref{thm:torsion-free}; the two arguments are compared
...
amenable trace is quasidiagonal.  Problem~X(2) there, whether the trace on
the hyperfinite $\mathrm{II}_1$ factor is quasidiagonal, is not addressed
here.  Since $K$ is a direct limit of
...
counterexample among reduced group $C^*$-algebras, as does Eckhardt's
Corollary~4.4~\cite{Eckhardt}.  Whether every countable group is MF was a
longstanding open problem~\cite{CDE,Korchagin,BDL}.
...
Kun and Thom~\cite{KT26} call $\Gamma\le G$ infranormal when
$\operatorname{Comp}_G(\Gamma)$ generates $G$, and prove that if $\Gamma$
is infranormal and not normal and both $\Gamma$ and $G$ have
property~\textup{(T)}, then $(\bigoplus_{G/\Gamma}\mathbb Z/2\mathbb
Z)\rtimes G$ is not sofic.  The group $W$ of
Section~\ref{sec:amenable-nonqd} has the same shape, but $V$ surjects onto
$\mathbb Z$ and so does not have property~\textup{(T)}; accordingly $W$ is
sofic, and the compression alone rules out MF\@.  Alekseev and
Thom~\cite{AT26} prove that the centralizer of a sofic approximation of a
Kazhdan group is again an ultraproduct of permutation groups;
Theorem~\ref{thm:transport} is the operator norm analogue, in which the
compressor preserves the Hilbert--Schmidt asymptotic commutant.
...
Z)\rtimes G$ is not MF\@.  The first step is the same: the Kazhdan
projections of $\Gamma$ and of $t\Gamma t^{-1}$ have unitarily equivalent
nested images in a finite $C^*$-algebra and therefore coincide.  The
passage from Hilbert--Schmidt norm to operator norm differs.  Eckhardt
builds a $1$-cocycle for the conjugation representation in a rescaled
Hilbert--Schmidt ultraproduct and applies the Delorme--Guichardet theorem,
which uses the lamp structure; Theorem~\ref{thm:normal-kazhdan} uses the
Kazhdan projection of a normal property-\textup{(T)} subgroup instead,
which is what allows the simple group $H$, where there are no lamps.  In
Section~\ref{sec:amenable-nonqd} the lamps satisfy Clifford relations, and
by these relations a central involution lies in the normal
subgroup~\eqref{eq:intrinsic-defect}, so
Theorem~\ref{thm:compression-criterion} applies directly.  When $\Gamma$
is residually finite and $[\Gamma:t\Gamma t^{-1}]<\infty$, the argument of
Proposition~\ref{prop:clifford-locally-rf} shows that Eckhardt's wreath
product is sofic as well, so by
Theorem~\ref{thm:factorization-nonmf-trace} and
Proposition~\ref{prop:locally-rf-by-z-trace} its canonical trace is also
amenable and not quasidiagonal.
...
The corollary has a finite-dimensional shadow that needs no
property~\textup{(T)}.  If $\rho\colon G\to\U(d)$ is a unitary
representation and $uLu^{-1}\le L$, then $x\mapsto\rho(u)^*x\rho(u)$ maps
the commutant $\rho(L)'$ into itself, hence onto it, because $\rho(L)'$ is
finite dimensional; so $\rho(ucu^{-1})$ commutes with $\rho(L)$ for every
$c\in C_G(L)$, and every finite-dimensional unitary representation of $G$
is trivial on $\mathfrak D_G(L)$.  Property~\textup{(T)} and stable
finiteness of the corona replace this dimension count.
...
suitable, possibly with respect to a larger generating set $A'\supseteq A$;
since every finite subset of $G$ lies in a ball of $\Gamma(G,A')$,
Theorem~\ref{thm:hull} applies with $A'$ in place of $A$.  Apply Theorem~\ref{thm:hull} to $N_0$ with $g_1,\dots,g_m$ a
...
quotient to be chosen acylindrically hyperbolic; we take $G_0$ to be such
a quotient.  Put $S=tJt^{-1}$.  Since $J$ is simple and
...
corresponding declarations; the development uses toolchain
\texttt{leanprover/lean4:v4.32.0} and Mathlib \texttt{v4.32.0}.  The Lean
route sometimes differs from the printed one: Proposition~\ref{prop:simple}
is proved in Lean by direct root extraction rather than through Preusser's
theorem, and the Ershov--Jaikin-Zapirain theorem is formalized only for
finite-type algebras over finite fields, which covers $H$ but not
Theorem~\ref{thm:full-defect-ring} in general.
Section~\ref{sec:torsion-free} has no Lean counterpart.
