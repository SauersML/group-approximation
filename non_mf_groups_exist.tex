...
generates all of $\EL_4(R)$, and an embedding of $\EL_4$ into $\EL_2$
carries the conclusion to every $n\ge2$.
...
with three coordinates for the subgroup $\EL_3$, which has
property~\textup{(T)} over a finitely generated ring, and one for its
centralizer; two lemmas then carry the conclusion to rank two.  The
...
\emph{Rank four over a finitely generated ring.}  Suppose first that
$R$ is finitely generated as a unital ring, and put $G=\EL_4(R)$ and
$L=\EL_3(R)$ on coordinates $1,2,3$.  Both groups have
...
\emph{Rank two.}  Now let $R$ be arbitrary.  By
Lemma~\ref{lem:two-copies} choose $v_0,v_1,w_0,w_1\in R$ with
$w_iv_j=\delta_{ij}$, and let $S$ be the unital subring they generate.
It is finitely generated, $w_0v_0=1$, and $w_1(1-v_0w_0)v_1=1$, so the
first part applies to $S$: every homomorphism from $\EL_4(S)$ to an MF
group is trivial.  Put
\begin{gather*}
...
$a=S_2$, $b=T_2$.  So $\rho$ is trivial.

\emph{All ranks.}  For $n\ge3$, a homomorphism from $\EL_n(R)$ to an MF
group is trivial on the copy of $\EL_2(R)$ on coordinates $1,2$, so it
kills $e_{12}(R)$, and signed permutation matrices conjugate $e_{12}(r)$
to $e_{ij}(\pm r)$ for all $i\ne j$.
\end{proof}
...
hypothesis of Theorem~\ref{thm:full-defect-ring}, take instead $S$,
$L=\EL_3(S)$, $u$, and $\Psi$ as in the rank-two part of its proof,
with $s=v_0$ and $t=w_0$.  Then $\Psi$ is injective, since
$T_i\jmath(A)S_j=A_{ij}$, so $\Psi(L)\le\EL_2(R)\le\EL_n(R)$ has
property~\textup{(T)} and is compressed strictly by $\Psi(u)$, and
