...
The cosets $\sigma(x)r_qH$, $x\in F$, $q\in \Delta$, are pairwise distinct,
...
\[
 g\,\sigma(x)r_qH=\sigma(\bar gx)\,r_{\theta(b(g,x))q}\Lambda\in T .
\]
...
$\bar g=1$, then $g\sigma(x)r_qH=\sigma(x)r_{\theta(b(g,x))q}\Lambda$ with
...
the first question settles this case for $n\ge4$: the compression cell
of Lemma~\ref{lem:ring-compression-cell} needs only $ts=1$, and the
normal closure of $e_{13}(1-st)$ is $\EL_n(R,I)$ for the locally finite
ideal $I=R(1-st)R$, hence locally finite.  The descent to ranks two and
three uses the two copies of Lemma~\ref{lem:two-copies}, which is where
fullness enters.
