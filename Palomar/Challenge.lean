...
Thus soficity and hyperlinearity do not imply MF.  Nontriviality of `w` makes
clauses 6 through 10 non-vacuous; clauses 9 and 10 also show that `E` has no
faithful finite-dimensional representation and is not residually finite.

## What "MF" means here

MF means the Carrión-Dadarlat-Eckhardt operator-norm property: embedding in
the unitary group of a matrix quotient, equivalently admitting separating
finite-dimensional approximate unitary representations with operator-norm
defects tending to zero.  `IsMFGroup` is its equivalent local form with
separation constant `1`; the accompanying development proves the equivalence
by tensor-power amplification.  Strong-convergence and purely matricial field
notions sometimes also called MF are not claimed here.

The presentation imposes `c ^ 2 = 1` and the eight relations making `w`
central, but not `w ^ 2 = 1` or `w ≠ 1`; both are proved.

## Scope

This Comparator entry excludes the repository's C*-algebraic consequences,
the quotient by `w`, general obstruction criteria, and undecidability results.
-/
