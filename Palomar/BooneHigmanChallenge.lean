...
* `exists_finitely_presented_infinite_simple`: there is a finitely presented infinite
  simple group. The mathematics is classical (the witness used is Hyde–Lodha's `Q₂`); the
  statement is included because its formalization was an open benchmark.
...
`Group.FG`, `IsSimpleGroup`, `PresentedGroup`, `RingQuot`, `FreeAlgebra`, `Monoid.Coprod`
and `Subgroup.IsFinitelyNormallyGenerated`. All groups are quantified over `Type`.
...
/-- A finitely presented infinite simple group: finitely many generators and relations
whose presented group is simple and infinite. -/
theorem exists_finitely_presented_infinite_simple :
    ∃ (n : ℕ) (rels : Set (FreeGroup (Fin n))), rels.Finite ∧
      IsSimpleGroup (PresentedGroup rels) ∧ Infinite (PresentedGroup rels) := by
  sorry
