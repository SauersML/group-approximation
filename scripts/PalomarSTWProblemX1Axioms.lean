...
/-!
# The axiom closure of the Problem X(1) theorems

`lake env lean scripts/PalomarSTWProblemX1Axioms.lean`, run by
`.github/workflows/palomar-check.yml` after the Palomar libraries build.  It
prints the axiom closure of each theorem `Palomar/comparator-stw-x1.json`
...
namespace PalomarSTWProblemX1Axioms

/-- The theorems `Palomar/comparator-stw-x1.json` selects. -/
def compared : List Name :=
  [`STWProblemX1.exists_separable_amenable_not_quasidiagonal,
   `STWProblemX1.not_every_amenable_trace_quasidiagonal]

/-- The axioms `Palomar/comparator-stw-x1.json` permits. -/
def permitted : List Name := [`propext, `Classical.choice, `Quot.sound]

end PalomarSTWProblemX1Axioms

run_cmd do
  let env ← getEnv
  for n in PalomarSTWProblemX1Axioms.compared do
    match env.find? n with
    | none =>
        logError m!"MISSING declaration {n}: Palomar/comparator-stw-x1.json \
selects it, and it is not in the import closure of Palomar.STWProblemX1Solution.  \
...
nothing."
    | some ci =>
        ...
        let extra := closure.toList.filter
          (fun a => !PalomarSTWProblemX1Axioms.permitted.contains a)
        unless extra.isEmpty do
          logError m!"axiom closure of {n} leaves the permitted set: {extra}.\n\
Palomar/comparator-stw-x1.json permits {PalomarSTWProblemX1Axioms.permitted}, \
