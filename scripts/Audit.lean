...
def literaturePackages : List Name :=
  [-- Tikuisis--White--Winter, Ann. of Math. (2) 185 (2017) 229--284, and the
   ...
   -- `Quasidiagonal.UCPContractive` used to sit here.  **Retired 2026-08-19 by
   -- proof, not by argument**: `Quasidiagonal.ucpContractive` inhabits it.
   -- Both of its clauses are theorems now -- `ucp_map_star` for
   -- `*`-preservation, and `ucp_norm_le` for the bound, through
   -- `isCompletelyPositive_of_form` into the Stinespring estimate.  Neither was
   -- reachable until the complete-positivity predicate was corrected to
   -- constrain the whole form rather than its real part; the two-by-two
   -- counterexample that forced the correction is in
   -- `Analysis/QuasidiagonalTrace`.  Do not re-add this line without
   -- re-establishing that the structure is uninhabited.
   ...
   -- Chiodo's Theorem 3.10 is no longer irreducible: `ChiodoUniversalHost`
   -- proves every clause of it from the group `Monsters.ChiodoAbsorber`,
   -- which is built here, and one embedding of that group into a finitely
   -- presented torsion-free group.  That embedding is Chiodo's Theorem 2.2
   -- (Higman embedding preserving the set of torsion orders), and it is the
   -- entry below.  The host stays on the roster because the hull is on it:
   -- `universalTorsionFreeHost` produces the host from the hull and nothing
   -- else, so retiring the hull retires the host in the same commit.
   ``ChiodoHost.TorsionFreeHigmanHull,
   -- ... and that hull is itself two theorems, stated apart in
   -- `Higman/Program.lean`: Chiodo's Theorem 2.2 in general (Higman embedding
   -- with the torsion orders preserved) and Chiodo's Proposition 3.8 at the
   -- absorber (that `ChiodoAbsorber` is recursively presented).  The three
