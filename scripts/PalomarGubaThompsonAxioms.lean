...
`Palomar/comparator-guba-thompson.json` selects and fails unless every closure
lies inside the permitted axioms.  A missing declaration fails too: a driver
that cannot find its input must not read like a driver that passed.
...
/-- The theorems `Palomar/comparator-guba-thompson.json` selects. -/
def compared : List Name :=
  [`GubaThompson.question_3_20,
   `GubaThompson.question_3_20_common_multiple]

/-- The axioms `Palomar/comparator-guba-thompson.json` permits. -/
...
run_cmd do
  ...
  for n in PalomarGubaThompsonAxioms.compared do
    ...
    | none =>
        logError m!"MISSING declaration {n}: Palomar/comparator-guba-thompson.json \
selects it, and it is not in the import closure of Palomar.GubaThompsonSolution.  \
