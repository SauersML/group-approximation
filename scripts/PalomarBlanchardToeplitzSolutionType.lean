...
The solution-side driver for `Palomar/comparator-blanchard-toeplitz.json`, twin of
...
this is where an instance resolved differently inside a shared definition shows
up.

Run with `lake env lean scripts/PalomarBlanchardToeplitzSolutionType.lean`.
-/
