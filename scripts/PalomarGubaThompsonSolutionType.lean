...
declares while those facts are outstanding.  When they are proved, the theorem
...
#eval show MetaM Unit from do
  ...
  let targets : List (Bool × Name) :=
    [(true,  `GubaThompson.conjBy),
     ...
     (false, `GubaThompson.question_3_20_of),
     (false, `GubaThompson.question_3_20_common_multiple_of)]
  let env ← getEnv
