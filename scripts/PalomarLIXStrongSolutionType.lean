...
The twin of `scripts/PalomarLIXSolutionType.lean` for the third Comparator
...
everything after it is printed for inspection.  The three names after the
boundary carry the `_of` suffix on this side, because that is what the solution
declares while the outstanding proposition is outstanding.  When the outstanding
...
Run with `scripts/remote-build.sh --run scripts/PalomarLIXStrongSolutionType.lean`.
-/
...
#eval show MetaM Unit from do
  ...
  let targets : List (Bool × Name) :=
    [(true,  `ProblemLIXStrong.cornerDiag),
     ...
     (false, `ProblemLIXStrong.exists_simple_separable_powers_outside_U0_of),
     (false, `ProblemLIXStrong.exists_simple_separable_order_six_witness_of),
     (false, `ProblemLIXStrong.exists_simple_separable_squarefree_witness_of)]
  let env ← getEnv
