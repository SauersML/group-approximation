...
For the Palomar theorem named by `metadata/CLAIM_DECLS.txt`, the same roster
...
The copied definitions in `Palomar/Challenge.lean` are exactly this theorem's
local statement vocabulary. Measuring the closure keeps that audit surface
visible when a definition quietly acquires a new dependency.
...
def isLocal (env : Environment) (n : Name) : Bool :=
  ...
  | some m => libRoots.contains m.getRoot
  | none => false
...
run_cmd do
  let env ← getEnv
  let (mainSec, mainUnion) ← section_ env "Palomar theorem"
...
For the theorem named by the Palomar claim roster, the number of \
...
Whole statement: **" ++ toString union.size ++ " local constants, " ++
    toString lines ++ " source lines** in its local closure.\n"
