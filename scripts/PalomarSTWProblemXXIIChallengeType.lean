...
The challenge-side driver for `Palomar/comparator-stw-xxii.json`, twin of
`scripts/PalomarSTWProblemXXIISolutionType.lean`.
...
as well, because Comparator compares them; the proofs of the two theorems are
not, because the challenge states them with holes.

Run with `lake env lean scripts/PalomarSTWProblemXXIIChallengeType.lean`.
-/
