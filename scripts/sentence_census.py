...
def write_md(records: list[dict], path: str) -> None:
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write(
            f"Of the `ledger` sentences, {one} sit under an anchor the ledger "
            f"grades with a single row, so the pairing is one-to-one; the "
            f"other {many} sit under an anchor graded by several rows, and the "
            "census names all of that anchor's rows rather than pretending to "
            "know which row is which sentence.  Narrowing those is the work "
            "the census makes visible.\n\n")
...
def main() -> int:
    ...
    records = join(records, load_map(MAP), rows)
