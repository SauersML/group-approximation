...
# names a datum that nothing produces, or that only a `sorry`-backed
# declaration produces.  Whether that is an overreach depends on what the row
# claims.  A `definition` row is right to name an uninhabited datum -- "fixes
# notation or names an object; the object is in Lean" is its whole claim, and
# it asserts nothing about the object existing.  A `formalized` or `partial`
# row says the named declarations prove the sentence, and an uninhabited datum
# proves nothing.  So these count for the asserting statuses only.
CARRIER_DETECTORS = {"carrier-data", "carrier-debt"}
...
def counts_finding(status: str, detector: str) -> bool:
    ...
    Split out so the rule can be pinned: a `definition` row naming an
    uninhabited datum is honest, and counting it would have produced six false
    positives against rows whose whole claim is that the object is in Lean.
    """
    if detector in CARRIER_DETECTORS:
        return status in ASSERTING_STATUSES
    return detector in CONDITIONAL_DETECTORS
...
def write_tsv(records: list[dict], path: str) -> None:
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write(f"# Manuscript SHA-256: {SOURCE_SHA256}\n")
...
def write_md(records: list[dict], path: str) -> None:
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write(f"Manuscript SHA-256: `{SOURCE_SHA256}`.\n\n")
        fh.write(
            "This census records source assignments for individual **sentences**, "
            "not just numbered claims. These assignments do not attest to a "
        ...
        fh.write("## Sentences without complete recorded coverage\n\n")
        if not gaps:
            fh.write("Every sentence carries a declaration, a ledger row, or a "
                     "reason no declaration applies.\n\n")
        else:
            fh.write(
                "`partial` records incomplete or conditional formal coverage; "
                "`unassigned` means no current assignment is recorded. Neither "
                "status by itself refutes the mathematical sentence. `open` "
                "records an explicitly open mathematical claim.\n\n")
            for r in gaps:
...
def declared_names() -> set[str]:
    """Fully qualified names; an unrelated namesake is not a declaration."""
    from pathlib import Path
    from lean_decls import build_index
    return set(build_index(Path(ROOT)))
...
def verify_unconditional(records: list[dict]) -> list[str]:
    ...
    for r in records:
        ...
        for name in r["decls"].split():
            # Short names collide both across modules and within a module.
            # Resolve the named namespace and file before classifying it.
            path = paths.get(name)
            if path is not None and path not in qualified_modules:
                qualified_modules[path] = m.scan_module(path, qualified=True)
            decl = qualified_modules.get(path, {}).get(name)
            if decl is None:
                continue
            for detector, detail in m.classify(corpus, decl, roster, name):
                if not counts_finding(r["status"], detector):
...
def self_test() -> int:
    ...
    import check_non_mf_unconditional as classifier

    # A cited qualified name must not resolve to an unrelated namesake.
    # Cover collisions inside one file and a noncomputable section, whose
    # closing `end` must not pop the surrounding namespace.
    with tempfile.TemporaryDirectory() as tmp:
        source = Path(tmp) / "GroupApproximation" / "Namesakes.lean"
        source.parent.mkdir()
        source.write_text(
            "namespace Good\nnoncomputable section\n"
            "theorem endpoint : True := True.intro\nend\n"
            "theorem afterSection : True := True.intro\nend Good\n"
            "namespace Bad\ndef Unproved : Prop := False\n"
            "theorem endpoint (h : Unproved) : True := True.intro\n"
            "end Bad\n", encoding="utf-8")
        qualified = classifier.scan_module(source, qualified=True)
        if (set(qualified) != {"Good.endpoint", "Good.afterSection",
                               "Bad.Unproved", "Bad.endpoint"}
                or qualified["Good.endpoint"].header
                or not qualified["Bad.endpoint"].header):
            print("self-test: qualified declaration lookup conflates namesakes",
                  file=sys.stderr)
            return 1
        records = [{"key": "namesakes", "status": "formalized",
                    "decls": "Good.endpoint Bad.endpoint Missing.endpoint MissingRoot"}]
        with patch.dict(verify_decls.__globals__, {"ROOT": tmp}):
            missing = verify_decls(records)
        if missing != ["namesakes\tMissing.endpoint", "namesakes\tMissingRoot"]:
            print("self-test: a short-name match hid a missing qualified name",
                  file=sys.stderr)
            return 1
    ...
    carrier_cases = [
        ...
        ("partial", "carrier-debt", True),
        ("definition", "carrier-data", False),
        ("definition", "carrier-debt", False),
        ("structural", "carrier-data", False),
        ("formalized", "conditional-data", True),
        ("definition", "conditional-data", True),
        ("formalized", "definition-only", False),
    ]
    for status, detector, expected in carrier_cases:
        if counts_finding(status, detector) != expected:
            print(f"self-test: counts_finding({status!r}, {detector!r}) = "
                  f"{counts_finding(status, detector)}, expected {expected}",
                  file=sys.stderr)
            return 1
