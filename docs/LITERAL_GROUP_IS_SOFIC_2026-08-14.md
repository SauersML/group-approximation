...
edit.  Companion to `docs/LITERAL_GROUP_BLOCK_AMALGAM_STRUCTURE_2026-08-14.md`
...
* *Amenable kernel.*  `ker(E → W)` is free of countably infinite rank
  ...
  `docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md` §3 already warns against it
...
`K_m = ClLamp(Y_m) ⋊ Γ` of `docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md`
...
* `E/⟨w⟩` is not residually finite.  Then, being sofic, it is **not stable in
  ...
  `docs/SOFIC_STABLE_IMPLIES_RESIDUALLY_FINITE_2026-08-14.md`), exactly as for
  `E` itself.
...
`docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md`, which separated them only at
...
`docs/SOFIC_STABLE_IMPLIES_RESIDUALLY_FINITE_2026-08-14.md`; also stated in
...
* **Corpus precedence.**  The finite-stage tower, the coset tower `X_n` of
  ...
  `docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md` (2026-08-13); the present
...
4. *Sofic kernel with amenable quotient gives sofic* (Elek--Szabó,
   ...
   the repository's own `docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md` §3
...
`docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md`).  The two statements quantify
...
Corpus checked before writing: `docs/TRUE_LITERAL_NONMF_MARK_IS_CE_VISIBLE.md`
(closest precedent, §§2--3 reused), `docs/EXPLICIT_FINITELY_PRESENTED_NON_MF_GROUP.md`
(historical snapshot of the construction), `docs/NOTEPAD.md` (searched for
...
treats `E`'s own soficity), `docs/NON_MF_FINAL_REVIEW_AND_RESOLUTION_2026-08-14.md`
...
artifacts `docs/TRUE_KUN_THOM_CLIFFORD_PHASE.md` (independent, concurrent
