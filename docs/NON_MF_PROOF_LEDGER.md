...
| KT.00 | thm:kazhdan-transport | Theorem 3.1 as printed: forward transport of the HS asymptotic commutant along a one-sided conjugator | `KazhdanAsymptoticCommutant.manuscriptKazhdanTransport`; `OmegaWeightedAmbient.omega_route_kazhdan_transport` | MISMATCH | MISMATCH | literal | unconditional | - | NO | **THE ROUTE MISMATCH IS GONE (2026-08-17); THE UNIVERSE MISMATCH REMAINS.**  The old note read "univ0 (Γ, H : Type); the conclusion is proved, but by the Appendix B route".  The second clause no longer applies: `omega_route_kazhdan_transport` proves the printed conclusion through the printed route — `B_ω = ∏_ω B(K_n)` acting on `K_ω`, with the Kazhdan projection in `B_ω`, and no `ambient` hypothesis, because `OmegaWeightedAmbient.omegaWeightedAmbient` constructs the KT.01–KT.09 ambient over `B_ω` rather than over the cofinite corona.  The link that was missing until then was `act_P_iff` at `ω`, i.e. reading `Fix ⊆ ran P` off the spectral gap through the action of `B_ω`; it is discharged the same way the cofinite route discharges it, `kt_08_rep_mul_proj` moved across the action plus `kt_08_act_proj_of_fixed`, which needed only that `omegaAct` is a ℂ-algebra action.  What keeps this row MISMATCH is the first clause alone: both declarations fix `Γ, H : Type`, while the printed theorem quantifies over groups in no fixed universe.  Grading the proof EXACT would also violate this ledger's own invariant that an EXACT proof needs an EXACT statement |
