...
| --- | --- | --- | --- | --- | --- |
| statement | 359 | 44 | 38 | 1 | 442 |
| proof | 364 | 33 | 44 | 1 | 442 |
<!-- END-LEDGER-COUNTS -->
...
| UN.03 | cor:undecidable | under the corresponding hypothesis on its complement, the set of presentation codes of non-MF groups is not recursively enumerable | `AdianRabinVariantTransform.operatorMF_negative_side_not_re_of_wordProblem` | EXACT | EXACT | literal | unconditional | - | NO | same reduction, negative side.  **This clause keeps its printed hypothesis, and it is now the only one at `cor:undecidable` that does (2026-08-17).**  The positive half's hypothesis was discharged by the Boone word map (UN.02, UN.04c--UN.04f); the negative half needs the r.e. half of `W` -- `REPred AdianRabinWordProblem.wordProblemPred`, equivalently that the complement's failure to be r.e. is available -- and that is not proved here.  `AdianRabinWordProblem.wordProblem_iff_exists_rawCertificate` supplies the mathematical content, a finite code-independent certificate for every positive instance; what is missing is `Primrec` plumbing for the certificate check, which that module's docstring scopes and which no declaration yet performs |
...
| UN.04e | p:undec-adianrabin | the code's word problem on that word is triviality of the halting element, and the halting element is trivial exactly when the configuration halts | `Computability.wordProblem_rawComm_iff`; `Computability.commElt_eq_one_iff_halts` | EXACT | EXACT | literal | unconditional | - | NO | added 2026-08-17, and it is two printed clauses, one declaration each.  The renumbering clause is `wordProblem_rawComm_iff`, which moves triviality across `stageCode`'s relabelling by `PresentedGroupRelabel.mk_relabel_eq_one_iff` without identifying elements.  The halting clause is `commElt_eq_one_iff_halts`, Simpson's Theorem 8 reached through a variable associated subgroup because `tsub` occurs in the *type* of the HNN extension and so must be substituted rather than rewritten.  **CITATION CORRECTED 2026-08-17: the row named a third declaration, `wordProblem_rawComm_iff_halts`, that does not exist**, which failed the gate's declaration check outright — `lean_decls` found no such name in the tree.  There is no composite declaration: the join in `Computability/BooneWordProblemUndecidable.lean` chains the two cited theorems inline with `Iff.trans`.  Nothing about the grade changes, because the two clauses the row states are the two declarations that remain |
| UN.04f | p:undec-adianrabin | joining these at the group of a machine whose configuration halting problem is undecidable leaves the word problem on codes undecidable outright | `Computability.not_computablePred_wordProblemPred_of_word_map`; `Computability.not_computablePred_wordProblemPred_of_boone_words`; `Computability.not_computablePred_wordProblemPred` | EXACT | EXACT | literal | unconditional | - | NO | added 2026-08-17.  The join threads a single machine through both halves, which is why it is stated with the word map quantified over machines rather than as two existentials: two existentials could pick two machines.  Computability of the word map is not removable at `_of_word_map`, since composing a computable predicate with an arbitrary map need not be computable |
| UN.04g | p:undec-adianrabin | the positive instances of the word problem carry finite certificates, which is the mathematical content of its recursively enumerable half | `AdianRabinWordProblem.wordProblem_iff_exists_rawCertificate` | EXACT | EXACT | literal | unconditional | - | NO | added 2026-08-17 with the sentence that replaced the old conditionality paragraph.  A coded word dies exactly when some `RawCertificate` decodes to it -- a `Primcodable` type of raw conjugator/relator/sign triples, code-independent by construction, with soundness unconditional because a relator the code does not list decodes to `1`.  **What the printed sentence does not claim, and what this row does not certify, is r.e.-ness itself**: `REPred wordProblemPred` needs `ComputablePred` for the certificate check, `FreeGroup` has no `Primrec` API at this Mathlib revision, and no declaration performs the search.  That gap is UN.03's, where it is the surviving printed hypothesis; the module's docstring scopes the remaining work at roughly 250--450 lines of `Primrec` bookkeeping plus `FreeGroup.Red` theory |
