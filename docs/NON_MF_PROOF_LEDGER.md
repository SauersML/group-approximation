...
| | |
| --- | --- |
| commit | `36490e17cc5d9691b213b80c7f27020399d898b8` |
| `git hash-object non_mf_groups_exist.tex` | `ad184a59acd6643ba25699f729a45a61c2ce19c4` |
| sha256 of the file | `258d76dc13119fdc10078b470f25d48559a3e66cbccba9f72b8ace5e476e5bae` |
| `wc -l` | 3678 |

Unlike the previous three pins, this blob **is** reachable from a commit: the
manuscript was clean at re-pin time and the blob above is exactly
`36490e17:non_mf_groups_exist.tex`.  `git hash-object` of the file must reproduce
it for the rows below to be read as current.  The file shrank from 4768 lines to
3678 across the two-part split and the editorial passes; a reader comparing an
older pin should expect the line count to fall, not rise.
...
prints is two atoms, BT.15 and BT.16.  `docs/P13_STEP_AUDIT.md` is graded
...
comment-stripped tex and from `docs/NON_MF_NUMBERED_CLAIMS.json`, not copied
...
| --- | --- | --- | --- | --- | --- |
| statement | 361 | 44 | 38 | 1 | 444 |
| proof | 366 | 33 | 44 | 1 | 444 |
<!-- END-LEDGER-COUNTS -->
...
**Four of the seven cleared on 2026-08-17: CY.04, CY.11, CY.12 and CO.02.**  What
changed is not a report of a green run — none is observable from this checkout — but
the condition the marker was actually waiting on.  When it was written, the modules
those rows name had been committed without being reachable from `GroupApproximation.lean`
at all, so no build could have elaborated them however green it went.  Commit
`bf6a0b8a` (2026-08-16) wired them into the root import closure, and a transitive scan
of that file on 2026-08-17 finds exactly one orphan module in the whole tree
(`Computability.UniformWordProblemUndecidable`) and none of these among it.  A module
in the closure is elaborated by the root build or the root build is red; that is a
different and much stronger position than a module the build never sees.  The three
rows left above are held for a different reason in each case, recorded in their notes.
...
| INT.12 | p:intro-history | MOVED to docs/NON_MF_META_CLAIMS.md — MF is known for many amalgamated free products | - | MISSING | MISSING | - | literature-input | Schafhauser, Shulman, GKEMP | NO | TOMBSTONE 2026-08-17: a historical attribution, not an inferential step.  The verbatim sentence, its keys and its status are in `docs/NON_MF_META_CLAIMS.md`; the ID stays so cross-references land and the anchor keeps a user |
| INT.13 | p:intro-history | MOVED to docs/NON_MF_META_CLAIMS.md — C*_red(F₂) is MF (Haagerup–Thorbjørnsen) | - | MISSING | MISSING | - | literature-input | HT | NO | TOMBSTONE 2026-08-17: historical, not a step.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.14 | p:intro-history | MOVED to docs/NON_MF_META_CLAIMS.md — the negative solution of CEP supplies abstract stably finite non-MF algebras | - | MISSING | MISSING | - | literature-input | MIPRE, FGH | NO | TOMBSTONE 2026-08-17: historical, not a step.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.15 | p:intro-thom | MOVED to docs/NON_MF_META_CLAIMS.md — Schatten p-approximation fails for p = 2, for 1 < p < ∞, and for p = 1 | - | MISSING | MISSING | - | literature-input | DGLT, LuO, BDL | NO | TOMBSTONE 2026-08-17: historical, not a step.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.16 | p:intro-nonsofic | MOVED to docs/NON_MF_META_CLAIMS.md — the Hamming case was disproved in 2026 (OpenAI, Fournier-Facio, Kun–Thom) | - | MISSING | MISSING | - | literature-input | OAI, FFF, KT | NO | TOMBSTONE 2026-08-17: historical, not a step.  The manifest flags this one as the entry most likely to go stale on its own, since it dates a result and asserts the status of the neighbouring hyperlinear problem.  See `docs/NON_MF_META_CLAIMS.md` |
...
| INT.19 | p:intro-convention | MOVED to docs/NON_MF_META_CLAIMS.md — stronger conventions exist (GKEMP trace recovery; PMF of MdlS) | - | MISSING | MISSING | - | literature-input | GKEMP, MdlS | NO | TOMBSTONE 2026-08-17: terminology, not a step.  Read it beside LI.17: the printed Question 3 is explicitly about the CDE convention and says the implication is built into the stronger ones.  See `docs/NON_MF_META_CLAIMS.md` |
...
| INT.23 | p:intro-slofstra | MOVED to docs/NON_MF_META_CLAIMS.md — Slofstra's precedents for the Clifford group, central involution, shift and HNN doubling | - | MISSING | MISSING | - | literature-input | SlofstraProfile, SlofstraVidick, BDL | NO | TOMBSTONE 2026-08-17: historical, not a step.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.24 | p:intro-bekka | MOVED to docs/NON_MF_META_CLAIMS.md — the π⊗π̄ framework of Bekka and Bekka–Valette, and Dadarlat's HS almost-invariant projections | - | MISSING | MISSING | - | literature-input | BekkaAmenable, BekkaValette, Dadarlat | NO | TOMBSTONE 2026-08-17: historical, not a step.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.25 | p:intro-slofstra-quotients | MOVED to docs/NON_MF_META_CLAIMS.md — Slofstra's invisible-element quotients precede the MF-residual language | - | MISSING | MISSING | - | literature-input | SlofstraCorrelations, Fritz | NO | TOMBSTONE 2026-08-17: historical, not a step.  The same attribution is repeated at `def:radical`, where the `RA.*` rows sit.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.26 | p:intro-dglt | MOVED to docs/NON_MF_META_CLAIMS.md — BDL's rounding, compression and polar-correction procedures, and their character-isotypic corner | - | MISSING | MISSING | - | literature-input | DGLT, BDL, GR, Gl19 | NO | TOMBSTONE 2026-08-17: historical, not a step — but the manifest flags it as the closest of the fifteen to load-bearing, because the procedures it attributes are re-proved in `sec:conj` rather than quoted and the `FN.*` rows grade those proofs.  See `docs/NON_MF_META_CLAIMS.md` |
| INT.27 | p:intro-cde | MOVED to docs/NON_MF_META_CLAIMS.md — CDE provide the group-MF framework, and an amenable MF group that is not residually finite | - | MISSING | MISSING | - | literature-input | CDE | NO | TOMBSTONE 2026-08-17: historical, not a step.  The CDE *definition* is load-bearing and stays graded EXACT at INT.18 and PRE.16; only the attribution and the Abels'-group example moved.  See `docs/NON_MF_META_CLAIMS.md` |
...
| KT.17 | thm:kazhdan-transport | hence the spectrum of h lies in [−1, 1 − κ²/(2·card S)] ∪ {1} | `UltraproductKazhdanProjection.KazhdanCompressionRep.kt_07_spectrum_subset`; `OmegaKazhdanCompression.omegaCompressionRep` | EXACT | EXACT | literal | unconditional | - | NO | **UPGRADED 2026-08-17 from MISMATCH over substituted objects, and this one was a real misattribution.**  The old cell cited `WeakMFVectorGNS.hermitianAverage_eventually_no_intermediate`, an *eventual finite-stage eigenvalue exclusion at threshold κ²/(4·card S)* — a different statement at a different rate.  `kt_07_spectrum_subset` is the displayed inclusion itself, at the printed rate.  The declaration is universally quantified over unital C-star algebras carrying a unitary representation, and the printed `B_ω` is now a *proved instance* of that: `OmegaKazhdanCompression.omegaCompressionRep` builds the bundle over `OmegaAdjointCorona`, which is `∏_ω B(K_n)`.  So the row is literal by instantiation rather than by restatement, and the old cell's finite-stage substitute was never the printed statement at all. |
...
| TV.02 | thm:transport-variants | (2) intertwiner transport between two asymptotic representations at the d¹_n + d²_n normalization | `TransportVariantsAnyUniverse.manuscriptIntertwinerTransport_anyUniverse`; `IntertwinerKazhdanTransport.manuscriptIntertwinerTransport` | EXACT | EXACT | literal | unconditional | - | NO | **RESOLVED 2026-08-17 from MISMATCH, by wrapper rather than by sweep.**  The two measurements below stay on record because they are why the sweep is not owed, rather than merely unfinished.  `KazhdanCompressionCore` no longer pins its source group — it is `(Γ : Type v)` with `kazhdan : HasKazhdanPropertyT.{v, v} Γ`, green across all forty-four files that use it — so the printed generality can now at least be *written*.  Generalizing the transport theorems' own binders does not come free, and the two attempts say where the wall is.  (i) A `whnf` heartbeat timeout appeared in `scaled_laplacian_massVanishing` with fifteen cascade errors after it; the cause was **not** that declaration but `ScaledMassVanishing.finset_sum`'s own `{I : Type}`, one pinned binder in a helper — the same misleading shape as a structure whose projections all come back unknown.  (ii) With that widened to `{I : Type*}` the timeout is gone and the real wall appears: `KazhdanCompressorCorner.cornerProjection` and its neighbours pin `IsKazhdanPair.{0, 0}`, so the cascade continues into the corner layer and reaches the representation universe, not just the group's.  Both attempts were reverted, not left half-applied.  Since `thm:transport-variants` inherits its hypotheses from `thm:kazhdan-transport`, which grants no countability, the honest closure is the derived-countability transfer that closed KT.00 rather than a sweep: `C.kazhdan` is already `HasKazhdanPropertyT.{v, v} Γ`, which is exactly what `exists_symmetric_generating_finset` consumes, and `HasKazhdanPropertyT.of_mulEquiv` already exists.  The last missing ingredient now exists: `CompressionUniverseTransfer.mapCoreSource` transports a compression core along an isomorphism of its *source* (`mapCore` moves the ambient), and `KazhdanCompressionCore.countable_source` derives `Countable Γ` from the core's own `kazhdan` field.  Property `(T)` needs three moves there and all three were already in `Kazhdan/`: `liftUniverse` raises the representation universe to `max w v` so it can see both group universes, `of_mulEquiv` carries the property across, and `of_max` brings it back down to the target's.  Both of those remaining pieces are now done, in `Sofic/TransportVariantsAnyUniverse.lean`.  The predicates carry their own universe binder: `IsScaledAsymptoticCommutantOf` already did, and `IsScaledAsymptoticIntertwinerOf` was generalized to `{G : Type v}`, every existing use instantiating `v := 0`.  The wrappers are `TransportVariantsAnyUniverse.scaled_transport_both_anyUniverse`, `manuscriptIntertwinerTransport_anyUniverse` and `scaled_intertwiner_transport_anyUniverse`.  Since `mapCoreSource` fixes `t`, the compressed sequences in the conclusion are the same terms at the `Type 0` model as they are at `Γ`, so only the homomorphism has to be read back along the isomorphism; `commutantOf_of_precomp` and `intertwinerOf_of_precomp` are that single step, one per predicate.  **This is not a weakening**: the print grants no countability and none is assumed, because property `(T)` implies it. |
| TV.03 | thm:transport-variants | (2) at every weight | `TransportVariantsAnyUniverse.scaled_intertwiner_transport_anyUniverse`; `IntertwinerKazhdanTransport.scaled_intertwiner_transport` | EXACT | EXACT | literal | unconditional | - | NO | **RESOLVED 2026-08-17 from MISMATCH, by wrapper rather than by sweep.**  The two measurements below stay on record because they are why the sweep is not owed, rather than merely unfinished.  `KazhdanCompressionCore` no longer pins its source group — it is `(Γ : Type v)` with `kazhdan : HasKazhdanPropertyT.{v, v} Γ`, green across all forty-four files that use it — so the printed generality can now at least be *written*.  Generalizing the transport theorems' own binders does not come free, and the two attempts say where the wall is.  (i) A `whnf` heartbeat timeout appeared in `scaled_laplacian_massVanishing` with fifteen cascade errors after it; the cause was **not** that declaration but `ScaledMassVanishing.finset_sum`'s own `{I : Type}`, one pinned binder in a helper — the same misleading shape as a structure whose projections all come back unknown.  (ii) With that widened to `{I : Type*}` the timeout is gone and the real wall appears: `KazhdanCompressorCorner.cornerProjection` and its neighbours pin `IsKazhdanPair.{0, 0}`, so the cascade continues into the corner layer and reaches the representation universe, not just the group's.  Both attempts were reverted, not left half-applied.  Since `thm:transport-variants` inherits its hypotheses from `thm:kazhdan-transport`, which grants no countability, the honest closure is the derived-countability transfer that closed KT.00 rather than a sweep: `C.kazhdan` is already `HasKazhdanPropertyT.{v, v} Γ`, which is exactly what `exists_symmetric_generating_finset` consumes, and `HasKazhdanPropertyT.of_mulEquiv` already exists.  The last missing ingredient now exists: `CompressionUniverseTransfer.mapCoreSource` transports a compression core along an isomorphism of its *source* (`mapCore` moves the ambient), and `KazhdanCompressionCore.countable_source` derives `Countable Γ` from the core's own `kazhdan` field.  Property `(T)` needs three moves there and all three were already in `Kazhdan/`: `liftUniverse` raises the representation universe to `max w v` so it can see both group universes, `of_mulEquiv` carries the property across, and `of_max` brings it back down to the target's.  Both of those remaining pieces are now done, in `Sofic/TransportVariantsAnyUniverse.lean`.  The predicates carry their own universe binder: `IsScaledAsymptoticCommutantOf` already did, and `IsScaledAsymptoticIntertwinerOf` was generalized to `{G : Type v}`, every existing use instantiating `v := 0`.  The wrappers are `TransportVariantsAnyUniverse.scaled_transport_both_anyUniverse`, `manuscriptIntertwinerTransport_anyUniverse` and `scaled_intertwiner_transport_anyUniverse`.  Since `mapCoreSource` fixes `t`, the compressed sequences in the conclusion are the same terms at the `Type 0` model as they are at `Γ`, so only the homomorphism has to be read back along the isomorphism; `commutantOf_of_precomp` and `intertwinerOf_of_precomp` are that single step, one per predicate.  **This is not a weakening**: the print grants no countability and none is assumed, because property `(T)` implies it. |
...
| TA.05 | cor:uniform | MOVED to docs/NON_MF_META_CLAIMS.md — the compactness argument is not quantitative; an effective modulus remains open | - | MISSING | MISSING | - | open | - | NO | TOMBSTONE 2026-08-17: an open-status claim about a quantitative strengthening of a theorem that is already EXACT, so no Lean work can move this row as it stands.  The manifest records the two named obstructions whose arrival would change its status.  See `docs/NON_MF_META_CLAIMS.md` |
...
| FD.17 | p:B-deligne | MOVED to docs/NON_MF_META_CLAIMS.md — w makes E a group of Deligne type in the sense of BDL, realizing their proposal at the operator-norm endpoint | - | MISSING | MISSING | - | literature-input | BDL | NO | TOMBSTONE 2026-08-17: terminology, not a step.  The mathematical clause underneath it — `w` dies in every homomorphism to a finite group — is a theorem and stays graded in the `FD.*` rows off `thm:exactfd`; only the naming and the attribution of the proposal moved.  See `docs/NON_MF_META_CLAIMS.md` |
...
| LI.05 | p:limits-alekseev | MOVED to docs/NON_MF_META_CLAIMS.md — Alekseev–Thom's Open Problem 6.2 concerns Hamming and tracial models, and is separate from thm:kazhdan-transport | - | MISSING | MISSING | - | open | AlekseevThom | NO | TOMBSTONE 2026-08-17: an open-status claim about the relation between two problems, neither provable nor disprovable in this ledger's sense.  `Deps` corrected from `literature-input` to `open` on the way out: the manuscript quotes no theorem here, it compares two questions.  The adjacent claim that IS mathematical stays in the ledger at LI.16.  See `docs/NON_MF_META_CLAIMS.md` |
...
| LI.06d | p:limits-trace | for faithfully traced unital C\*-coordinates A the left and right actions on L²(A,τ) generate a von Neumann algebra that need not be finite: for an infinite-dimensional II₁ factor in standard form it is all of B(L²(A,τ)) | - | MISSING | MISSING | - | unconditional | - | NO | **NEW 2026-08-17, recording a gap that the LI.06 upgrade would otherwise have hidden.**  This is the paragraph's opening sentence and it is a genuine mathematical assertion, not a remark about the development: it is the *reason* the generalization is hopeless, where the constant-coordinate counterexample of LI.06/LI.06b is the *refutation*.  Nothing in the tree speaks about standard form, and the claim is not a small one — that the algebra generated by the two commuting actions of a II₁ factor in standard form is all of `B(L²)` is the standard-form consequence of Tomita--Takesaki, of which Mathlib has no part.  It is not on the critical path: the paper's conclusion rests on the counterexample, which is formalized, so this row is `MISSING` rather than blocking |
...
| LI.11 | p:q1 | the reductions give no further information because the elements they detect are already killed: w is trivial in E/⟨w⟩ | `Mathlib:QuotientGroup.eq_one_iff`; `Mathlib:Subgroup.mem_zpowers` | EXACT | EXACT | literal | unconditional | - | NO | **REWRITTEN 2026-08-17.  The old claim — "Corollary 11.8 identifies the radical with the preimage of D as soon as that quotient is MF" — is a GHOST: no sentence of the current Question 1 says it**, and the numeral it cited was already drifting.  What the print says at this point is the sentence recorded here.  It is exactly the two cited Mathlib facts at the data of `Sofic/LiteralSignFreeQuotient.lean`: `markSubgroup` is literally `Subgroup.zpowers mark` and `proj` is `QuotientGroup.mk'` of it, so `proj mark = 1` is `(QuotientGroup.eq_one_iff _).mpr (Subgroup.mem_zpowers _)`.  No repo declaration names that composite; graded EXACT on the precedent of TRA.01, which cites Mathlib for a printed step the development does not separately restate.  `manuscriptExactRadicalFromCandidateQuotient`, this row's former citation, keeps its badge coverage at RA.13 |
| LI.12 | p:q1 | and the commutators generating D are trivial in the quotient by D | `Mathlib:QuotientGroup.eq_one_iff`; `Mathlib:Subgroup.subset_closure` | EXACT | EXACT | literal | unconditional | - | NO | **REWRITTEN 2026-08-17.  The old claim — "whether that quotient is MF is open; a detecting model must survive both collapses and by Theorem B factors through no finite quotient" — is a GHOST**: the current Question 1 draws no such consequence, and `manuscriptTheoremB` keeps its badge coverage at FD.00 and FD.01.  The printed sentence is the general fact that a normal subgroup's generators die in the quotient by it, read at `D = D_coll`; both halves are Mathlib and neither is restated in the development, so the grade follows TRA.01 as at LI.11 |
...
| LI.16 | p:limits-models | the adjoint spectral estimates require operator-norm multiplicative control, which permutation and Hilbert--Schmidt models do not supply; the soficity of E is proved separately, from its normal form | `LiteralSoficAssembly.markedGroup_isSofic`; `HilbertSchmidtAdjointGap.exists_hsClose_adjointFar` | MISMATCH | EXACT | literal | unconditional | AlekseevThom | NO | **REWRITTEN AND REANCHORED 2026-08-17.  The old claim, "Question 3: whether E is sofic or hyperlinear is not decided by the results above", is doubly dead**: the manuscript now PROVES `E` sofic (`thm:Esofic`, rows ES.01--ES.06) and hyperlinear (ES.02), and the current Question 3 is a different question, recorded at LI.17.  The anchor was misnamed as well — its probe has always sat in the *limitations* section, not in the questions list — so it is renamed `p:limits-models` and the row now records the sentence the probe actually locates.  The soficity half is `markedGroup_isSofic`, unconditional; the half about what permutation and Hilbert--Schmidt models fail to supply is the same adjoint gap LI.01 and LI.01b cite.  MISMATCH on statement because no declaration says of the *method* that it requires operator-norm control; what is proved is the counterexample that makes the sentence true |
