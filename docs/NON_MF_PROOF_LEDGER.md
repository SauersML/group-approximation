...
| --- | --- | --- | --- | --- | --- |
| statement | 421 | 69 | 50 | 1 | 541 |
| proof | 420 | 53 | 62 | 6 | 541 |
<!-- END-LEDGER-COUNTS -->
...
| rem:finite-stage | env | rem:finite-stage | b9a995aa34ad755d |
...
| rem:chaincondition | env | rem:chaincondition | 570ec3f1286494da |
...
| prop:blocknormalform | env | prop:blocknormalform | f0ca5c0dcfcb3849 |
...
| prop:sounditeration | env | prop:sounditeration | 3d059f7e96aa593a |
...
| rem:collapse-finite-stage | env | rem:collapse-finite-stage | 96d8514cd6aa25ae |
...
| thm:commutinglamp | env | thm:commutinglamp | 1202711ab680499f |
...
| p:abstract-mechanism | prose | `spectral corner of $w$ then contradicts it` | - |
...
| p:radical-sound-rules | prose | `hence inside the residual of every` | - |
...
| p:collapse-relcomm | prose | `not only to one arising from a group element` | - |
...
| p:saturation-remark | prose | `every MF quotient is trivial, and every` | - |
...
| RFS.01 | rem:finite-stage | Appendix B proves the theorem at finite stages | `KazhdanAsymptoticCommutant.transport` | EXACT | EXACT | literal | unconditional | - | NO | RE-READ 2026-08-16: the first paragraph was shortened — "the ultrafilter gives no rate" and the sentence naming the almost-fixed spectral subspace are gone — and this row now audits what remains.  It is the accurate description of what Lean actually proves |
| RFS.02 | rem:finite-stage | conditional on the quantitative spectral-gap input, every estimate of that proof is an explicit inequality in κ, card S, the bound on x_n and the multiplicative defects of U_n | `WeakMFVectorGNS.hermitianAverage_eventually_no_intermediate_eigenvalues` | EXACT | EXACT | literal | unconditional | - | NO | Lean is stronger: the gap input is proved, not granted.  RE-READ 2026-08-16: the printed adjective moved from "robust" to "quantitative"; the input is the same one |
...
| NK.05 | thm:normal-kazhdan | moving spectral subspace: cut sp(h_n) at θ = 1 − ε₀²/(4·card S); q_n = spectral projection for [−1,θ] | `KazhdanCompressorCorner.cornerProjection` | EXACT | EXACT | literal | unconditional | - | NO | same threshold constant as printed |
...
| BN.03 | prop:blocknormalform | the conjugate gcg⁻¹ is the lamp c_{gB}, and the distinguished word w is the sign ζ | `LiteralBlockNormalForm.toModel_siteLamp`; `LiteralBlockNormalForm.toModel_mark` | EXACT | EXACT | literal | unconditional | - | NO | CLOSED 2026-08-16, was MISSING for want of the declaration names.  `siteLamp (gB)` unfolds to `verticalToE g * lamp * (verticalToE g)⁻¹`, so `toModel_siteLamp : toModel (siteLamp ξ) = inl (lampAt ξ)` is the first identification and `toModel_mark : toModel mark = inl lampSign` is the second.  Both are stated along `toModel`, which `markedGroupEquivModel` is built from, so they are clauses about the same isomorphism |
| BN.04 | prop:blocknormalform | each level Γ_n has finite orbits on X | `LiteralBlockNormalForm.finite_telescopeLevel_site_orbit` | EXACT | EXACT | literal | unconditional | - | NO | NEW 2026-08-16 |
...
| WN.01 | lem:window | for every finite set S of sites the subgroup M_S = ⟨ζ, c_ξ (ξ∈S)⟩ is residually finite | `BlockCliffordTowerSofic.residuallyFinite_window`; `LiteralSoficAssembly.lampEquiv` | EXACT | EXACT | literal | unconditional | - | NO | NEW 2026-08-16.  The printed proof is the declaration's own: the free product over the blocks the window meets separates everything but the sign, and the sign is separated by the complete Clifford quotient, whose image on the window is finite because `ClLamp` is locally finite (`CliffordLamp.isLocallyFiniteGroup_cliffordLamp`).  `lampEquiv` is the identification of the printed M_S with the module's `window I B S` |
...
| RA.20 | prop:sounditeration | every subgroup reached by the two operations lies in Res_MF; a reached normal subgroup with MF quotient equals it; if the whole group is reached, every homomorphism to a countable MF group is trivial | `manuscriptSoundIterationClosure` | EXACT | EXACT | literal | unconditional | - | NO | universe-polymorphic; `IsSoundIterate` is the inductive predicate for the two printed operations |
...
| ST.02 | prop:stabradical | Res_MF(G) ≤ Rad_fd(G) always | `MatricialStabilityRadical.actualCoronaMFResidual_le_fdUnitaryResidual` | EXACT | EXACT | literal | unconditional | - | NO | - |
...
| CO.09 | eq:generator-mass | Step 4: b_n(γ) = k_n^{-1/2}(V_n(γ) − V_n(1)), ‖b_n(γ)‖_F² ≤ 4·len_S(γ), and Σ_{a∈S}‖b_n(a)‖_F² = 4 | `InvolutionCollapseProfile.bVec`; `InvolutionCollapseProfile.norm_bVec_sq_le`; `InvolutionCollapseProfile.sum_normSq_bVec_eq_four` | MISMATCH | MISMATCH | literal | unconditional | - | NO | CORRECTED against docs/COLLAPSE_STEP_AUDIT.md COL.19: the Lean `bVec` carries a guard the printed b_n(γ) does not |
...
| CO.10 | thm:collapse | Step 4: the first grouping bounds rank D_n by (len γ + len μ + len a)·k_n, the word lengths; the second makes ‖D_n‖ → 0; Tr(D*D) ≤ rank(D)·‖D‖² gives vanishing normalized defect mass | `InvolutionCollapseCocycle.eventually_coboundary_defect_small` | EXACT | EXACT | literal | unconditional | - | NO | both groupings are used, as the TeX insists |
...
| CO.17 | rem:collapse-finite-stage | (W3) is essential: it supplies the joint spectral calculus behind the rank identity; anticommuting Clifford coordinates have no joint spectral labels | `InvolutionCollapseMetric.dV` | MISMATCH | MISSING | literal | unconditional | - | NO | (W3) is used, but the necessity claim about anticommuting coordinates is not formalized.  PENDING FIRST GREEN (A9, 2026-08-16): `WreathWitnessW3.pauli_rank_identity_fails`, with `pauli_difference_sq`, `not_commute_pauli` and `cliffordCoordinates_fail_W3`, is proposed.  A9 also recommends rewording the row, and it is right: the printed sentence is a claim about the *Step 2 route* — that the rank identity is what (W3) buys — not a claim about `thm:collapse` as a whole.  The reword waits with the grade, so that both land against a built module |
...
| CO.20 | thm:projection-collapse | inside H', Θ(L) has (T), Θ(s) compresses it, w is an involutive witness; Theorem 11.22 places each [Θ(γ),w] in Res_MF(H') | `InvolutionCollapseEndpoint.involutiveCollapseDefect_le_actualCoronaMFResidual` | EXACT | EXACT | literal | unconditional | - | NO | - |
...
| SO.10 | thm:witnesssofic | w ↦ ζ and functoriality of the radical make ζ ∈ Res_MF(W); hence W is not MF | `LiteralWitnessConsequences.witnessGroup_not_isOperatorMF` | EXACT | EXACT | literal | unconditional | - | NO | - |
...
| FS.03 | app:finite-stage | H_n = Hermitian part of the S-average; θ = ½((1 − κ²/(4·card S)) + 1) < 1; P_n = spectral projection above θ | `KazhdanCornerMatrices.hermitianAverage`; `KazhdanCompressorCorner.cornerProjection`; `KazhdanAsymptoticCommutant.transport` | EXACT | EXACT | literal | unconditional | - | NO | `transport` sets `theta := ((1 - kappa^2/(4*S.card)) + 1)/2`, exactly the printed midpoint |
...
| FS.05 | app:finite-stage | the robust spectral gap: for 1 − κ²/(4·card S) < a and b < 1, eventually no eigenvalue of H_n lies in [a,b] | `WeakMFVectorGNS.hermitianAverage_eventually_no_intermediate_eigenvalues` | EXACT | EXACT | literal | unconditional | - | NO | proved, not granted; the TeX only asks that it be granted |
| FS.06 | app:finite-stage | consequently a unit η ∈ ran P_n has ⟨(1 − H_n)η,η⟩ ≤ δ, each generator moves η by ≤ √(2·card S·δ), and a word of length ℓ by ℓ√(2·card S·δ) plus accumulated defects | `KazhdanCompressorCorner.displacement_vanishing` | EXACT | EXACT | literal | unconditional | - | NO | the word-length propagation is inside `topSpectralDisplacement_vanishing_of_generates` |
...
| FS.18 | app:finite-stage | T_nξ_n is the vector of U_n(s)x_nU_n(s)*, proving the convergence with an explicit rate in κ, card S, M and the defects | `KazhdanAsymptoticCommutant.transport` | EXACT | EXACT | literal | unconditional | - | NO | - |
