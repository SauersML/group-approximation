...
| KT.00 | thm:kazhdan-transport | Theorem 3.1 as printed: forward transport of the HS asymptotic commutant along a one-sided conjugator | `KazhdanAsymptoticCommutant.manuscriptKazhdanTransport`; `OmegaWeightedAmbient.omega_route_kazhdan_transport` | MISMATCH | MISMATCH | literal | unconditional | - | NO | **THE ROUTE MISMATCH IS GONE AND THE AMBIENT GROUP'S IS TOO (2026-08-17); THE KAZHDAN SOURCE'S REMAINS.**  The old note read "univ0 (Γ, H : Type); the conclusion is proved, but by the Appendix B route", and neither clause survives whole.  (a) `omega_route_kazhdan_transport` proves the printed conclusion through the printed route — `B_ω = ∏_ω B(K_n)` acting on `K_ω`, the Kazhdan projection in `B_ω`, no `ambient` hypothesis — because `OmegaWeightedAmbient.omegaWeightedAmbient` builds the KT.01–KT.09 ambient over `B_ω`.  The link that was missing was `act_P_iff` at `ω`, discharged the same way the cofinite route discharges it.  (b) `H` is now at an arbitrary universe throughout the chain — `UltraproductAdjointModel`, `WeightedUltraproductAdjointModel`, `natU`, `compressionRep`, both ambients and both transports — so the printed "let `H` be a group" is met.  **What remains is `Γ : Type` alone.**  The print grants no countability, so the honest closure is the transfer route: property (T) forces a finite generating set, hence `Countable Γ`, and `hasKazhdanPropertyT_iff_textbook` plus `Type0Transfer.exists_type0_model` then descend it — the missing piece being property (T) transported along a `MulEquiv`.  A 249-site sweep of `HasKazhdanPropertyT.{0, 0}` is *not* the way and is not owed |
