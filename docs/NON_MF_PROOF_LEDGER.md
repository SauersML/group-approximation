...
| --- | --- | --- | --- | --- | --- |
| statement | 358 | 42 | 35 | 1 | 436 |
| proof | 358 | 35 | 42 | 1 | 436 |
<!-- END-LEDGER-COUNTS -->
...
### Deleted proofs are invisible to the gate

Every check this ledger runs is a check on a *statement*: `env` anchors digest
the printed statement with `\label` and badges stripped, `prose` anchors locate
a sentence, and `check_non_mf_claim_manifest.py` digests the same statement
again.  Nothing fingerprints a printed **proof**.  So when commit `3a45fa60`
(2026-08-16) deleted the three-step property-`(T)` proof of
`prop:literal-base-T` and left the proposition itself untouched, the anchor's
digest was still correct, every row on it still pointed at a real label, and the
gate had nothing to report — while nineteen of the twenty rows on that anchor
were grading prose that no longer existed.

The printed proof is now five lines and contains exactly two atoms: the
citation of \cite[Example~1.7.4(i)]{BHV} for property `(T)` of
`ℤ³ ⋊ SL₃(ℤ)`, routed through `rem:classical-base`; and the complexification
sentence.  They are BT.16 and BT.15.  These greps now return nothing in the
manuscript: `1/500`, `/64`, `circumcenter`, `Laplacian`, `Steinberg`,
`P_{13}`, `thm:p13-complete`.

**Nineteen rows retired 2026-08-17.**  They are listed here rather than left as
tombstones in the steps table, because a tombstone anchored at
`prop:literal-base-T` would still be *claiming* that the manuscript prints a
step there, which is the error being corrected.  Nothing about the Lean is
retracted: every declaration below exists and every verdict below was true of
the text when it was made.  What changed is that there is no longer a printed
sentence for them to be 1:1 with.

| Retired | Claim it graded | Lean it named | Grade at retirement |
| --- | --- | --- | --- |
| BT.01 | Step 1: `P₁₃` on six generators with the thirteen displayed relators | `LiteralP13HodgeCertificate.manuscriptP13Presentation` | EXACT/EXACT |
| BT.02 | Step 1: an exact SOS certificate proves the degree-one Hodge estimate with gap `1/500` | `LiteralP13HodgeCertificate.cleanP13Certificate`; `p13_hodge_gap` | EXACT/EXACT |
| BT.03 | Step 1: testing on coboundaries gives the quadratic gap in every orthogonal representation | `LiteralP13HodgeCertificate.p13_generatorLaplacian_quadratic_gap` | EXACT/EXACT |
| BT.04 | Step 1: the gap places `sp(Δ)` in `{0} ∪ [1/500,∞)` | none | MISSING/MISSING |
| BT.04b | Step 1: the kernel of `Δ` is exactly the invariant vectors | `ExactHodgeCertificate.invariant_of_generatorLaplacian_eq_zero` | MISMATCH/MISMATCH |
| BT.04c | Step 1: an almost-fixed unit vector is close to an invariant vector | `ExactHodgeCertificate.Certificate.isKazhdanPair` | MISMATCH/MISMATCH |
| BT.04d | Step 1: this yields a Kazhdan pair, so `P₁₃` has `(T)` | `LiteralP13HodgeCertificate.p13_hasKazhdanPropertyT`; `ExactHodgeCertificate.certifiedKazhdanTolerance` | EXACT/EXACT |
| BT.05 | Step 2: the six explicit words, and all thirteen relators die in `ℛ` | `LiteralBaseP13RotationQuotient.p13ToRotation` | EXACT/EXACT |
| BT.06 | Step 2: the homomorphism is surjective | `LiteralBaseP13RotationQuotient.p13ToRotation_surjective` | EXACT/EXACT |
| BT.07 | Step 2: `(T)` passes to quotients, so `ℛ` has it | `HasKazhdanPropertyT.of_surjective` | EXACT/EXACT |
| BT.08 | Step 3: `ℛ` maps onto the linear subgroup generated by `x,y,z` | `LiteralBaseRotationRetract.rotationEquivRotations` | MISMATCH/MISMATCH |
| BT.08b | Step 3: the Kazhdan pair `(S,κ)` supplied by Step 2 | `LiteralBasePropertyTBridge.base_hasKazhdanPropertyT_of_rotation` | MISMATCH/MISMATCH |
| BT.09 | Step 3: control set `S ∪ {v₂,v₃}` | `LiteralBasePropertyTBridge.base_hasKazhdanPropertyT_of_rotation` | EXACT/EXACT |
| BT.09b | Step 3: `v₁` needs no separate control because `v₁ = xv₂x⁻¹` | none | MISSING/MISSING |
| BT.10 | Step 3(1): the moving-projection estimate, constant `1/64` | `LiteralBasePropertyTBridge.exists_global_fixed_near` | EXACT/EXACT |
| BT.11 | Step 3(2): three words act as elementary transvections | `LiteralTranslationOrbit.conjugate_mem_translations`; `LiteralTranslationOrbit.e13` | EXACT/EXACT |
| BT.12 | Step 3(2): constants `3/64` and `1/8` | `LiteralBasePropertyTBridge.exists_global_fixed_near` | EXACT/EXACT |
| BT.13 | Step 3(3): the circumcenter argument | `HilbertConvexFixedPoint.exists_fixed_of_orbit_displacement_le`; `closedConvexHull_orbit_stable` | MISMATCH/MISMATCH |
| BT.14 | Step 3(3): `η` is fixed and `‖η − ξ‖ < 1` | `LiteralBasePropertyTBridge.base_hasKazhdanPropertyT_of_rotation` | EXACT/EXACT |

Three regrades were in flight for these rows when the prose went, and they are
recorded here rather than acted on, because retirement wins over regrade: BT.04
and BT.04b would have closed against `Monsters/P13SpectralGap.lean`
(`t1_04_p13_spectrum_subset`, `t1_05_p13_ker_eq_invariants`), and BT.08, BT.08b,
BT.09b and BT.13 against `Monsters/P13CircumcenterRouteStep3.lean`
(`rotationToRotations_surjective`, `t3_03_exists_normalized_pair`,
`t3_11_exists_translation_fixed_circumcenter`).  The Lean is real and the
modules stand; there is simply no printed step left for them to be 1:1 with.

**`docs/P13_STEP_AUDIT.md` is graded against the deleted text.**  Every verdict
it reaches is about the three-step proof, so as a record of that proof it is
still accurate and as a statement about the *current* manuscript it is not.  It
is not rewritten here; the eight rows this ledger inherited from it are the ones
retired above, and the cross-reference table's entry for it should be read with
this paragraph.
...
| LI.06 | p:limits-trace | finite dimensionality cannot be traded for stable finiteness plus faithful traces: constant coordinates A_n = C*_red(E) refute any such generalization | `FaithfullyTracedCoordinates.manuscriptFaithfullyTracedCoordinateNoGo`; `FaithfullyTracedCoordinates.ConstantReduced.reducedLeftRegular_injective`; `FaithfullyTracedCoordinates.literal_compressionDefect_ne_one` | EXACT | EXACT | literal | unconditional | - | NO | **RESOLVED 2026-08-17 from MISMATCH/MISSING; same defect as LI.01, one paragraph over.**  The old note said "the refutation argument (left regular embedding into the constant corona and the surviving defect) is not formalized" while citing `manuscriptTheoremD`, which is a different theorem.  Each of the three clauses of *this row's own text* is a named declaration, and the first is what the manuscript badges on the paragraph: the left regular representation embeds `E` injectively into the constant corona (`reducedLeftRegular_injective`), and the commutator of `tct⁻¹` with the base is the nontrivial defect (`literal_compressionDefect_ne_one`).  LI.06b and LI.06c have graded the same content EXACT/EXACT since the audit that added them.  **The paragraph's opening sentence is a separate claim and is genuinely missing** — see the new row LI.06d; it is recorded rather than absorbed, because upgrading this row on the strength of the counterexample would otherwise retire a real gap by silence |
