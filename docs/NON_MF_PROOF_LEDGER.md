...
| --- | --- | --- | --- | --- | --- |
| statement | 366 | 42 | 34 | 1 | 443 |
| proof | 366 | 35 | 41 | 1 | 443 |
<!-- END-LEDGER-COUNTS -->
...
| UN.03 | cor:undecidable | the word problem is recursively enumerable, so its complement is not, and neither is the set of presentation codes of non-MF groups | `WordProblemRE.rePred_wordProblemPred`; `WordProblemRE.not_rePred_compl_wordProblemPred`; `WordProblemRE.operatorMF_negative_side_not_re`; `AdianRabinVariantTransform.operatorMF_negative_side_not_re_of_wordProblem` | EXACT | EXACT | literal | unconditional | - | NO | **RESTATED 2026-08-17, twice in one day, and the second restatement is the one to read.**  The clause was printed conditional ("if the complement of `W` is not recursively enumerable"); at midday it was the only hypothesis left at this anchor, the positive half having been discharged by the Boone word map (UN.02, UN.04c--UN.04f); by evening `WordProblemRE` closed the r.e. half outright and the hypothesis went with it.  The printed chain is now three proved steps: `rePred_wordProblemPred` (no hypothesis) makes `W` recursively enumerable; `not_rePred_compl_wordProblemPred` is Post's theorem contraposed, taking undecidability of `W`, which is `Computability.not_computablePred_wordProblemPred`; and `operatorMF_negative_side_not_re` applies the Adian--Rabin reduction to that.  **One packaging gap, deliberately recorded rather than hidden:** the last two take undecidability as an explicit argument, so the composition that states the printed sentence with no argument at all is the term `WordProblemRE.operatorMF_negative_side_not_re Computability.not_computablePred_wordProblemPred` and is not yet a named declaration.  Nothing mathematical is missing -- `WordProblemRE`'s own docstring says the hypothesis is taken "rather than imported, so that nothing in this file depends on which module supplies it" -- but until a zero-input wrapper exists, no single badged declaration states this clause, and the badge on `rePred_wordProblemPred` certifies the r.e. step rather than the whole sentence |
