...
| TV.01 | thm:transport-variants | (1) transport at an arbitrary nonnegative weight sequence w_n, both directions | `ScaledKazhdanTransport.scaled_transport_both` | MISMATCH | EXACT | literal | unconditional | - | NO | UNIV0 SWEEP 2026-08-16: univ0 applies to Γ only (`{Γ : Type} {E : Type u}`); the ambient group is polymorphic.  `IsScaledAsymptoticCommutant`/`IsScaledMassBounded` are the printed Tr-conditions verbatim.  **MEASURED 2026-08-17, and the blocker moved.**  `KazhdanCompressionCore` no longer pins its source group: it is `(Γ : Type v)` with `kazhdan : HasKazhdanPropertyT.{v, v} Γ`, green across all forty-four files that use it, so the printed generality can now at least be *written*.  What does not come free is the transport theorems' own binders.  Taking `{Γ : Type v}` on the file-wide `variable` of `Sofic/ScaledKazhdanTransport.lean` produces a `whnf` heartbeat timeout in `scaled_laplacian_massVanishing`, the `Finset Γ` Kazhdan-average residual, and every later declaration in the file fails as a cascade off it.  Raising the budget is not the fix by this repository's own doctrine; the fix is per-declaration and is not done.  Since `thm:transport-variants` inherits its hypotheses from `thm:kazhdan-transport`, which grants no countability, the honest closure is the same derived-countability transfer that closed KT.00 — see `Sofic/KazhdanTransportAnyUniverse.lean` for the pattern. |
| TV.02 | thm:transport-variants | (2) intertwiner transport between two asymptotic representations at the d¹_n + d²_n normalization | `IntertwinerKazhdanTransport.manuscriptIntertwinerTransport` | MISMATCH | EXACT | literal | unconditional | - | NO | UNIV0 SWEEP 2026-08-16: univ0 applies to Γ only (`{Γ : Type} {H : Type u}`); the ambient group is polymorphic.  **MEASURED 2026-08-17, and the blocker moved.**  `KazhdanCompressionCore` no longer pins its source group: it is `(Γ : Type v)` with `kazhdan : HasKazhdanPropertyT.{v, v} Γ`, green across all forty-four files that use it, so the printed generality can now at least be *written*.  What does not come free is the transport theorems' own binders.  Taking `{Γ : Type v}` on the file-wide `variable` of `Sofic/ScaledKazhdanTransport.lean` produces a `whnf` heartbeat timeout in `scaled_laplacian_massVanishing`, the `Finset Γ` Kazhdan-average residual, and every later declaration in the file fails as a cascade off it.  Raising the budget is not the fix by this repository's own doctrine; the fix is per-declaration and is not done.  Since `thm:transport-variants` inherits its hypotheses from `thm:kazhdan-transport`, which grants no countability, the honest closure is the same derived-countability transfer that closed KT.00 — see `Sofic/KazhdanTransportAnyUniverse.lean` for the pattern. |
| TV.03 | thm:transport-variants | (2) at every weight | `IntertwinerKazhdanTransport.scaled_intertwiner_transport` | MISMATCH | EXACT | literal | unconditional | - | NO | UNIV0 SWEEP 2026-08-16: univ0 applies to Γ only (`{Γ : Type} {E : Type u}`); the ambient group is polymorphic.  **MEASURED 2026-08-17, and the blocker moved.**  `KazhdanCompressionCore` no longer pins its source group: it is `(Γ : Type v)` with `kazhdan : HasKazhdanPropertyT.{v, v} Γ`, green across all forty-four files that use it, so the printed generality can now at least be *written*.  What does not come free is the transport theorems' own binders.  Taking `{Γ : Type v}` on the file-wide `variable` of `Sofic/ScaledKazhdanTransport.lean` produces a `whnf` heartbeat timeout in `scaled_laplacian_massVanishing`, the `Finset Γ` Kazhdan-average residual, and every later declaration in the file fails as a cascade off it.  Raising the budget is not the fix by this repository's own doctrine; the fix is per-declaration and is not done.  Since `thm:transport-variants` inherits its hypotheses from `thm:kazhdan-transport`, which grants no countability, the honest closure is the same derived-countability transfer that closed KT.00 — see `Sofic/KazhdanTransportAnyUniverse.lean` for the pattern. |
