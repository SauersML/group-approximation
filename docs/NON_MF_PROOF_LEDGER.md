...
| KT.00 | thm:kazhdan-transport | Theorem 3.1 as printed: forward transport of the HS asymptotic commutant along a one-sided conjugator | `KazhdanAsymptoticCommutant.manuscriptKazhdanTransport` | MISMATCH | MISMATCH | literal | unconditional | - | NO | univ0 (Γ, H : Type); the conclusion is proved, but by the Appendix B route.  **Progress recorded 2026-08-17, and the row is not upgraded.**  Every *ingredient* of the printed route now exists over the printed algebra: `B_ω` (KT.06) with its faithful action (KT.07), the projection order read off that action (KT.08), `π : H → U(B_ω)` (KT.09), finiteness (KT.10), the Kazhdan bundle `OmegaKazhdanCompression.omegaCompressionRep` and the endgame `V P V* = P` in `B_ω` (`kt_10_omega_shift_conjugate_proj`), and the contradiction opening (KT.01, KT.02).  What is still missing is the *link*: reading `Fix ⊆ ran P` off the spectral gap through the action of `B_ω` on `K_ω`, which the development still does through the algebraic absorption identities on the cofinite corona.  Until that link exists the printed route is assembled but not walked, and this row must keep saying so |
