...
Corollary 3I below removes the property-`(T)` hypothesis from this statement.
The compression lemma remains a useful rigidity statement, but finite
generation is not needed once the skew-product reformulation is used.
...
purely scalar-retraction method.  The Pruefer calculation does not itself
produce a nonzero relation Bockstein.  Corollary 3I below shows, more
strongly, that countable infinite-height range is not an obstruction at all:
the subgroup-relation argument bypasses the nonexistent scalar retraction.
...
gauge, even after finite-index restriction.  In particular Corollary 3I
strictly subsumes Theorem 3D and Corollary 3F in their common-Cartan
application: finite generation, virtual divisibility, property `(T)`, and
the Pruefer scalar-retraction issue are all unnecessary for countable range.
