...
Approximation of Groups in Operator Norm* does not use. The first two items
are mathematical extensions awaiting Lean formalization; subsequent items name
their Lean declarations. `#audit_closed_axioms <declaration>`
(defined in `GroupApproximation/Meta/AxiomGuard.lean`) prints the axioms it
depends on.

## A universal locally finite host from Pestov 9.1 (mathematical proof)

One fixed infinite simple Kazhdan LEF group, generated by three elements
of orders `7,2,2`, contains every countable locally finite group. In particular
it contains `Q/Z` and every `GL_n(overline(F_p))`, while having no nontrivial
finite quotient. It is sofic, hyperlinear, and operator MF.

The [proof](../research/artifacts/pestov91-universal-locally-finite-host-2026-09-13.md)
chooses a minimal binary subshift with universal odometer factor inside
the existing 9.1 construction. Compatible cyclic partitions give matrix
algebras, and compatible regular representations embed increasing unions
of finite groups. The same host works for every countable locally finite
input. Cairn graph validation passed on MSI; the extension is not yet
Lean-formalized.

The [three-generator proof](../research/artifacts/pestov91-three-torsion-generators-2026-09-13.md)
improves the original eighteen-involution bound without changing the
group. More generally, any nonzero `F_2`-algebra generated by a unit, its
inverse and an idempotent has an elementary group `EL_3` generated by
the same three matrix formulas. An explicit squared commutator recovers
the idempotent coefficient; a twisted swap recovers the unit.
...
be weakened. Let `K ≤ G` have property (T), and let `H` be generated by
the elements `g` satisfying `gKg⁻¹ ≤ K`. If `[G:H]` is finite, universal
normalized-HS invisibility of `K` in operator-norm matrix models implies
`K ≤ Rad_MF(G)`. Thus the compression criterion applies to such `K` inside
the compression defect, even when `K` is not normal.

The [complete proof](../research/artifacts/infranormal-mf-detection-2026-09-13.md)
also establishes finite-index heredity of the operator-to-HS shadow and
gives a finite-set detector with a uniform positive HS gap after corner
compression. These extensions are not yet Lean-verified. The proof uses
the existing finite-corona projection calculus and a finite probability
average, with no ultrafilter in the final contradiction.
