...
If the finite square-zero tangent module \(N_k\) contains the appropriate
induced \(A_k\)-summand generated by the length-\(p^k\) Baumslag--Solitar
orbit, then
...
This supplies the unbounded Schur exponent proved necessary for the central
FALSE route.
...
The finite-module construction should now be audited for the following two
maps, rather than for simultaneous matrices:

1. an \(A_k\)-equivariant induced-module inclusion (or unit-split map)
   \[
   \operatorname{Ind}_{B_k}^{A_k}I_{p^k}\longrightarrow N_k;
   \]
