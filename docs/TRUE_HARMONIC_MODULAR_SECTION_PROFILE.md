# Exact full-plane modular section profiles at the first cuspidal levels
...
the entire rank-two cuspidal harmonic plane at both available levels.  There
is no bad linear combination hidden between the two basis vectors.
...
Thus the worst selected correction ratio actually decreases slightly from
`p=53` to `p=61`.  More importantly, it remains below `0.60` even after
testing every direction and every two-adic scale represented modulo `64`.
...
experiment: neither plane contains a narrow direction with rapidly larger
selected lift norm.
...
The repeated numerical constant is less important than the repeated local
module structure.  At both levels the correction cokernel has two-primary
torsion exactly `C_4`, while the harmonic plane maps trivially to that
torsion.  The next useful family statement is therefore:
...
Further primes are worthwhile only as a way to expose the finite templates
behind this statement.  They are not substitutes for the uniform proof.
