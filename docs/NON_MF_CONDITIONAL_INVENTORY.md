...
| 18 | `MarkovMFConsequences.presentationCodes_recognition_undecidable_of_haltingReduction`, `…_negative_side_not_re_of_haltingReduction` | conditional-data, buried-conditional | `cor:undecidable` | **None that the print conceals.** These take the Adian--Rabin reduction of row 1 as an *explicit* hypothesis, and `cor:undecidable` is now printed conditionally too ("suppose given a computable map sending each instance of the halting problem to a presentation code whose group is MF exactly when that instance halts"). Print and Lean therefore agree; the older unconditional-looking `operatorMF_recognition_undecidable` of row 1 is what the corollary used to lean on. | the honest restatement of the corollary is the fix; supplying the reduction is the open work |
| 19 | `InvolutionCollapseEndpoint.anchor_contradiction`, `numeric_transport_bound`, `stage_transport_bound` | header-hygiene | `eq:corner-reverse` | None mathematically. The binders are the quantitative data of the printed corner-reversal estimate -- gap, tolerance, stage index -- not inputs hidden from it. Same shape as rows 8--11. | fixable by moving binders after the colon, as was done for rows 15--17 |
| 20 | `MaximalCStarParagraphEndpoint.manuscriptMaximalCStarStrictCompressionRemark` | header-hygiene, open-variable | `rem:maxinfinite` | None mathematically. The binder is the compressing element the remark is about, and the in-scope `E` is the ambient group the remark already quantifies over in print. Same shape as rows 8--11 and 19. | — |
