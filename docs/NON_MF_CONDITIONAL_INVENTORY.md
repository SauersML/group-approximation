...
| 15 | `NuclearityAmenability.not_isAmenable_of_base_embeds` | header-hygiene | `thm:D`, the non-nuclearity paragraph | None mathematically. The theorem is true and unconditional; the binders are its subject matter -- an embedding of the base, and the group it embeds into, which the conclusion names. | badged in the merge of 2026-08-16; the module had never compiled before that day |
| 16 | `SoficByAmenablePermanence.isSofic_of_isSofic_ker_of_isAmenable` | header-hygiene | `\paragraph{Extensions by $\Z$.}` | None mathematically. Elek--Szabo permanence; the surjection, its surjectivity and the two premises are the statement rather than inputs concealed from it. | — |
| 17 | `ZariskiClosedSubgroup.manuscriptZariskiClosureOfRepresentation` | header-hygiene, open-variable | `rem:chaincondition` | None mathematically. `sigma`, `Gamma`, `t` and the compression hypothesis are exactly the printed data; the in-scope `n` and `k` are the space and the field, and the remark prints "over an arbitrary field", so that quantification is the printed one. | sits beside row 5, which is a genuine literature input on the same remark -- the two should not be confused |
