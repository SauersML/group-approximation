...
1. the constraint `Theta(L^infty(X)) subset D_U` is **free** — it can always
   be arranged (Lemma 2, already recorded in `docs/NOTEPAD.md`);
...
`docs/FALSE_MASA_NORMALIZING_TRACIAL_ACTION.md`: the "tracial `C`-action"
...
(This lemma is already in `docs/NOTEPAD.md`; it is restated because
Theorem 6 uses it.)
...
**Example 10 (Fourier block; recorded in `docs/NOTEPAD.md`).**  Put
