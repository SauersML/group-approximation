# Structural discovery of marked-module Q1920 slices in the singular
# 2^6:A8 parabolic of Omega_8^+(2), the next canonical moved-module host.
...
singularOrbit:=First(vectorOrbits,orbit->Length(orbit)=135);;
if singularOrbit=fail then Error("no 135-point singular orbit"); fi;
parabolic:=Stabilizer(ambient,singularOrbit[1],OnRight);;
radical:=PCore(parabolic,2);;
if Size(parabolic)<>1290240 or Size(radical)<>64 or
   not IsElementaryAbelian(radical) then
  Error("unexpected Omega8+ singular parabolic");
fi;
levi:=ComplementClassesRepresentatives(parabolic,radical)[1];;
if Size(levi)<>20160 then Error("unexpected A8 Levi"); fi;
...
for class in a5Classes do
  ...
  Append(qCandidates,List(matched,V->
      Group(Concatenation(GeneratorsOfGroup(V),GeneratorsOfGroup(H)))));
od;
qCandidates:=Set(qCandidates);;
qRepresentatives:=[];;
for Q in qCandidates do
  if not ForAny(qRepresentatives,R->IsConjugate(parabolic,Q,R)) then
    Add(qRepresentatives,Q);
  fi;
od;

Print("ambient_order=",Size(ambient)," vector_orbit_sizes=",
      List(vectorOrbits,Length)," parabolic_order=",Size(parabolic),
      " radical_order=",Size(radical)," levi_order=",Size(levi),"\n");
Print("a5_class_count=",Length(a5Classes),
      " class_[invariant_32,matched]",records,
      " raw_q_count=",Length(qCandidates),
      " parabolic_q_class_count=",Length(qRepresentatives),"\n");
for Q in qRepresentatives do
  Print("q_profile_[order,normalizer,aut]=",
        [Size(Q),Size(Normalizer(ambient,Q)),Size(AutomorphismGroup(Q))],"\n");
od;
