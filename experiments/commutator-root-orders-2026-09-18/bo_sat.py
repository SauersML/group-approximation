"""Finite-ball consistency test for a bi-order on N (normal closure of y) invariant under t (the index shift),
in G(K; al, be).  Ball B = nonempty freely reduced words of length <= r in y_0..y_{W-1}, merged into classes by
exact equality (tower_wp).  One boolean per class ("g > 1").  Clauses (all verified exactly):
  g xor g^-1;  g, h > 1 => gh > 1 (when gh is in B);  g > 1 <=> s^-1 g s > 1 (s a letter);  g <=> shift(g).
Candidate identities are found via images in finite quotients (perms file) and then checked exactly.
UNSAT = certificate that no bi-order of G exists (each clause is a necessary consequence).
usage: bo_sat.py K al be permsfile W r [nreps]
"""
...
for c, w in enumerate(reps_w):
    if max(i for i, _ in w) < W - 1:
        ...
        if d is not None and d != c:
            clauses += [[-var(c), var(d)], [var(c), -var(d)]]
