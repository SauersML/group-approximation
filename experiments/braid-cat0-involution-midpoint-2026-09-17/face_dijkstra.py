...
Usage: python3 face_dijkstra.py n thr maxface grid [x y]   (x, y like "123,48,567"; default P, rho P)
...
if len(sys.argv) > 6:
    _pp = lambda s: canon([tuple(int(c) - 1 for c in b) for b in s.split(',')] + [(i,) for i in range(n) if str(i + 1) not in s], n)
    P, Q = _pp(sys.argv[5]), _pp(sys.argv[6])
