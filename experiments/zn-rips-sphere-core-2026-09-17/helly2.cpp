...
// V = B_m^+ (m = floor(r/2)), plus (mode 1, r odd) T = { |y| = m+1, y_n > 0 }.  V is a clique.
...
int main(int argc,char**argv){ n=atoi(argv[1]); r=atoi(argv[2]); mode=argc>3?atoi(argv[3]):0; m=r/2; vector<int> cur(n);
  vector<vector<int>> all; gen(cur,0,r,all);
  for(auto& y:all){ int s=nrm(y); bool inV = (s<=m) || (mode==1 && (r%2==1) && s==m+1 && y[n-1]>0); if(inV) Cn.push_back(y); else P.push_back(y); }
  NP=P.size();
