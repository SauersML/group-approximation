...
int main(int argc,char**argv){ s=atoi(argv[1]); int Nmax=atoi(argv[2]); HMODE=argc>3?atoi(argv[3]):0;
  for(int N=s; N<=Nmax; N++){ int bad=0; vector<int> ex;
    ...
    vector<int> c(s,1); function<void(int,int,int)> gen=[&](int i,int rem,int mx){ if(i==s){ if(rem) return; x=c;
        for(K=0;K<s;K++){ if(K>0 && x[K]==x[K-1]) continue; H=HMODE?(N+1)/2:N/2; y.assign(s,0); found=false; RAD=1; rec(0,0); if(!found){RAD=-1; rec(0,0);} if(!found){ bad++; if(ex.empty()){ex=x; ex.push_back(K);} } } return; }
