...
def finite_cases(name, values, output):
    output.write("def %s" % name)
    output.write(" : Fin %d → " % len(values))
    output.write("FreeGroup P13Generator\n")
    for index, value in enumerate(values):
        output.write("  | %d => word %s\n" % (index, lean_word(value)))
    output.write("  | _ => word []\n")


def emit_product_class_table(table, output):
    for row, values in enumerate(table):
        output.write("def productClassIndexRow%d : Fin 22 → Fin 293\n" % row)
        for column, value in enumerate(values):
            output.write("  | %d => %d\n" % (column, value))
        output.write("  | _ => 0\n\n")
    output.write("def productClassIndex : Fin 22 → Fin 22 → Fin 293\n")
    for row in range(len(table)):
        output.write("  | %d => productClassIndexRow%d\n" % (row, row))
    output.write("  | _ => productClassIndexRow0\n\n")
...
def emit(data, helper, certificate, output):
    ...
    finite_cases("supportFreeWord", data["support_words"], output)
    output.write("\n")
    finite_cases("productClassFreeWord", data["product_class_words"], output)
    output.write("\n")
    ...
    for reduction in data["reductions"]:
        ...
        output.write(
            ...
            "  simpa [supportFreeWord] using reduction%d\n\n" %
            (index, lean_word(reduction["left"]), target, index))
