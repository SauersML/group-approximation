/* dist3.c -- (ex-sl3z-purely-matricial, 2026-09-13) variant of experiments/mf-kazhdan-cstar/dist.c:
   the principal SL_2 conjugated by diag(t,1,1) for t = 1,2,3 (the three cube classes mod 7),
   the block SL_2, their union, and block+Sym2^1.  Original header:
...
static uint32_t *build_H(int type,int d,int tt,uint64_t *hs){
  ...
  for(int a=0;a<n;a++)for(int b=0;b<n;b++)for(int cc=0;cc<n;cc++)for(int dd=0;dd<n;dd++){
    ...
    if(type==0){ /* columns: images of x^2, xy, y^2 under x->a x + cc y, y-> b x + dd y */
      ...
      if(tt!=1){ int ti=1; while((ti*tt)%n!=1%n) ti++; for(int j=0;j<3;j++) m[j]=(int)(((long)m[j]*tt)%n); for(int i=0;i<3;i++) m[3*i]=(int)(((long)m[3*i]*ti)%n); }
...
int main(int argc,char**argv){
  ...
  P[4]=malloc(K*sizeof(int64_t)); P[5]=malloc(K*sizeof(int64_t)); for(int k=0;k<K;k++){ P[4][k]=P[0][k]+P[1][k]+P[2][k]+P[3][k]; P[5][k]=P[0][k]+P[3][k]; }
  for(int t=0;t<6;t++){
    ...
    printf("type %s: irreps occurring (rank mod p1, p2) = %d, %d of K=%d; sum of dim^2 over missing irreps = %lld\n", (t==0?"Sym2^1":(t==1?"Sym2^2":(t==2?"Sym2^3":(t==3?"block":(t==4?"union_all":"block+Sym2^1"))))), r1, r2, K, ms); fflush(stdout);
