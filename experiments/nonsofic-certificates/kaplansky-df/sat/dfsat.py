...
def finite_closure(L, gens, cap=30000):
    """Key set of the subgroup generated by gens, or None if it has more than cap elements."""
    steps = []
    for g in gens:
        steps += [g, g.inverse()]
    seen = {L.IDENTITY.key}
...
# Finite subgroups to exclude, by generator-name prefix ("chart1_") or exact name ("C").
FINITE_SUBGROUPS = {"atlas": (("chart1_",), ("chart2_",)), "v": (("C", "P"),)}


def prune_rows(L, A, B, gens, family):
    """Clauses that remove no strict pair. Returns (xor_rows, or_rows, notes).

    Augmentation: eps(alpha) eps(beta) = eps(1) = 1, so both supports are odd.
    subgroup-supported-one-sided-inverse-rigidity with f = 1: if supp alpha or supp
    beta lies in a subgroup H with F_2[H] directly finite, the pair is not strict.
    Applied to H = degree-zero units (they lie in the ultramatricial degree-zero part
    of R, so H is locally finite) and to the finite subgroups listed above.
    """
    xor_rows = [("x", list(range(len(A))), True), ("y", list(range(len(B))), True)]
    members = [("degree-zero units", lambda u: all(len(a) == len(b) for a, b in u.val.terms))]
    for spec in FINITE_SUBGROUPS.get(family, ()):
        chosen = [g for name, g in sorted(gens.items())
                  if any(name == p or (p.endswith("_") and name.startswith(p)) for p in spec)]
        keys = finite_closure(L, chosen)
        if keys is not None:
            members.append(("finite <%s>, order %d" % (",".join(spec), len(keys)),
                            lambda u, keys=keys: u.key in keys))
    or_rows, notes = [], []
    for label, member in members:
        out_a = [i for i, a in enumerate(A) if not member(a)]
        out_b = [j for j, b in enumerate(B) if not member(b)]
        or_rows += [("x", out_a), ("y", out_b)]
        notes.append({"subgroup": label, "A_outside": len(out_a), "B_outside": len(out_b)})
    return xor_rows, or_rows, notes


def encode_and_solve(A, B, ab, ba, ident, strict, time_limit, linear_rows=(), or_rows=()):
    """linear_rows: (side, indices, rhs), one XOR per row; or_rows: (side, indices), one clause."""
    ...
    for side, idx in or_rows:
        var = x if side == "x" else y
        if not idx:
            return "UNSAT", None, {"trivial": "support forced inside a directly finite subgroup"}
        s.add_clause([var(k) for k in idx])
    for p in range(nz):
        i, j = divmod(p, nB)
