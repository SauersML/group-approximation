#! /bin/sh
# kbmag autgroup (bin/x86/autgroup, unchanged) except: programs are taken from $KBBIN, and the
# -large Knuth-Bendix pass also gets -mwd $MWD (maximum number of word differences; kbprog default 512).
# gpmakefsa and gpaxioms (the verification of the automatic structure) are run exactly as in autgroup.
DIR=${KBBIN}/
#echo $DIR
...
then
	...
	if test $large = true
	then	kbprogflags=${kbprogflags}" -me 262144 -t 500 -mwd ${MWD:-512} "
		if test $silent != true
		then	echo \#Running Knuth-Bendix Program 
                	echo "  "$KBPROG $kbprogflags $groupname
        	fi
