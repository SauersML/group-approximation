...
// ilv5 = ilv4 plus a resume position: the pass loop can be continued across budget-limited runs.
// Usage: ./ilv5 k r certfile initfile|- outfile [budgetSec=1100] [maxCand=400] [statefile]
//   statefile: if it exists, holds "pass pos"; the first pass starts at position pos of the order.  Rewritten at exit.
...
int main(int argc, char** argv) {
  ...
  double budget = argc > 6 ? atof(argv[6]) : 1100; int maxCand = argc > 7 ? atoi(argv[7]) : 400; string stf = argc > 8 ? argv[8] : "";
  int pass0 = 0; long long pos0 = 0; if (stf != "") { FILE* sf = fopen(stf.c_str(), "r"); if (sf) { if (fscanf(sf, "%d %lld", &pass0, &pos0) != 2) pass0 = 0, pos0 = 0; fclose(sf); } }
  ...
  if (getenv("ILV5_POSOF")) { Pt q{}; std::istringstream is(getenv("ILV5_POSOF")); for (int i = 0; i < n; i++) { int v; is >> v; q[i] = v; }
    for (long long pi = 0; pi < (long long)ord.size(); pi++) if (P[ord[pi]] == q) { printf("%lld\n", pi); return 0; } puts("-1"); return 0; }
  long long nD = 0; bool changed = true, oot = false; int pass = pass0; long long stopPos = 0, startPos = pos0;
  while (changed && left > 1 && !oot) { changed = false; pass++; long long del = 0;
    for (long long pi = startPos; pi < (long long)ord.size(); pi++) { int t = ord[pi]; if (!alive[t]) continue; if (el() > budget) { oot = true; stopPos = pi; break; } auto& x = P[t]; int nx = nr(x);
    ...
    if (startPos > 0) changed = true; startPos = 0;
  ...
  if (stf != "") { FILE* sf = fopen(stf.c_str(), "w"); fprintf(sf, "%d %lld\n", oot ? pass - 1 : pass, oot ? stopPos : 0LL); fclose(sf); }
  return ok ? 0 : 1; }
