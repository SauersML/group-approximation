...
usage: intsearch.py FAMILY T N i,j,... OUT.json
"""
...
def closed_x_first(m, N):
    ...
    for bs in product(rng, repeat=m - 1):
        ...
        for a in cands:
            if all(a) and max(abs(x) for x in a) <= N: out.add((a, b))
    return out
...
def solve(N, nE, verts, vm, vf, limit):
    ...
    vals = [x for x in range(-N, N + 1) if x]
...
def main():
    ...
    idxs = [int(x) for x in sys.argv[4].split(",")]; out = sys.argv[5]
