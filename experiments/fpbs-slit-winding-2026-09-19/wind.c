...
int main(int argc,char**argv){
  ...
  for(long long z=0;z<T;z++){
    ...
    if(!E0) continue;
    int vs=++stamp, Uc=1, Lc=1; double lmax=0; for(int b=0;b<NB;b++) badU[b]=0;
    for(int j=-n;j<=n;j++) for(int i=-2*n;i<=2*n;i++){ if(!inbox(i,j,n)) continue; int id=ID(i,j);
      ...
      if(cellin(m,M,0)) Uc=0; if(cellin(m,M,-M_PI)) Lc=0; if(M-m>lmax) lmax=M-m; markbad(m,M); }
