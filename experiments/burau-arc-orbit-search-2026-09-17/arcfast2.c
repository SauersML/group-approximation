 ...
 *
 * usage: arcfast2 n M [noprune] [printhits] [shard_lo shard_hi]   (shards = top-level starts)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static long long leaves[MAXP], hitsm[MAXP], nodes;
static int JL, JR, NN;
...
static void try_child(int depth,int cur,int e,int p,int q,int dir,int k,int sc,int passN){
  ...
  { int ne0=e+(dir<0? k:-k), eps0=(sc==0)?1:-1; long long c0=poly[ne0+OFF];
    if(!noprune && l1-llabs(c0)+llabs(c0+eps0) > M-(depth+1)) return; }   /* prune before insertion */
  int a=newpt();
...
int main(int argc,char**argv){
  ...
  int shlo=argc>6?atoi(argv[5]):0, shhi=argc>6?atoi(argv[6]):1<<30, start=0;
  nfree=n-3;
  ...
  for(int c=0;c<ncomb;c++){
    ...
    for(;;){
      for(int q2s=0;q2s<2;q2s++){
        if(start<shlo || start>=shhi){ start++; continue; } start++;
  ...
  long long th=0;
  for(int m=1;m<=M;m++){ th+=hitsm[m]; if(noprune || hitsm[m]) printf("m=%d leaves=%lld hits=%lld\n",m,leaves[m],hitsm[m]); }
  printf("n=%d M=%d shards=[%d,%d) of %d nodes=%lld total_hits=%lld\n",n,M,shlo,shhi<start?shhi:start,start,nodes,th);
  return 0;
}
