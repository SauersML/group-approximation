...
for fn in sys.argv[1:]:
    ...
    if nb>=6:
        def pooled(R):
            a=sum((r[0]>>1)&1 for r in R); b=sum((r[0]>>5)&1 for r in R)
            return math.log2(a/b)/4 if a and b else float('nan')
        pz=jk(R,pooled)
        print(f"# pooled slope of P(D_n), n=16->256: {pz[0]:.4f}({pz[1]:.4f})")
