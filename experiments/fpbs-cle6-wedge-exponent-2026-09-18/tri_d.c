 ...
 * usage: tri_d N samples seed [pinch=1]
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc,char**argv){
  N=atoi(argv[1]); if(argc>4) pinch=atoi(argv[4]); int S=atoi(argv[2]); s[0]=atoll(argv[3])*0x9E3779B97F4A7C15ULL+1; s[1]=0xdeadbeef^s[0]; for(int k=0;k<20;k++)nx();
  ...
  for(int t=0;t<S;t++){
    ...
    for(int n=8;n<=N;n*=2,bit++){
      ...
      int hs=st; h=0; tl=0;
      for(int j=-n;j<=n;j++) for(int i=-2*n;i<=2*n;i++){ if(!inbox(i,j,n)) continue; if(2*i+j<=0) continue; int id=ID(i,j); if(mark[id]!=st && hole[id]!=hs){hole[id]=hs;q[tl++]=id;} }
      while(h<tl){ int id=q[h++]; int j=id/W-N, i=id%W-2*N;
