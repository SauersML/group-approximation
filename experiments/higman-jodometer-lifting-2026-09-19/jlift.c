/* jlift.c -- orderly generation of j-odometer models of H4(n) (optionally / <<[a,c]^2>>) on the binary tree.
 *
...
static int rels[5][64], rlen[5], nrels;    /* letters: +(g+1) or -(g+1) */
...
static void addrel(const char *s) {
    ...
    for (; *s; s++) {
        ...
        rels[nrels][L++] = (c >= 'a') ? g + 1 : -(g + 1);
    }
    rlen[nrels++] = L;
}
static void conjrel(char x, char y, int n) {   /* y^-1 x y x^-n */
    char buf[64]; int L = 0;
    buf[L++] = y - 32; buf[L++] = x; buf[L++] = y;
    for (int i = 0; i < n; i++) buf[L++] = x - 32;
    buf[L] = 0; addrel(buf);
}
...
static int relok(const Tup *T) {
    ...
    for (int q = 0; q < nrels; q++) for (int x = 0; x < N; x++) {
        int p = x;
        for (int l = 0; l < rlen[q]; l++) { int g = abs(rels[q][l]) - 1; p = rels[q][l] > 0 ? T->p[g][p] : iv[g][p]; }
        if (p != x) return 0;
    }
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    conjrel('b', 'a', BASE); conjrel('c', 'b', BASE); conjrel('d', 'c', BASE); conjrel('a', 'd', BASE);
