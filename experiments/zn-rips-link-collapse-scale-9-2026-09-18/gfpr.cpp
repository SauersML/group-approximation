...
int main(int argc, char** argv) {
  ...
  if (argc > 4 && string(argv[4]) != "-") { vector<uint64_t> st(tot / 64 + 1, 0); FILE* f = fopen(argv[4], "r"); int z[12] = {0}; long long c = 0;
  ...
  // resumable: argv[5] = budget seconds, argv[6] = start position, argv[7] = 1 if the interrupted pass already removed something
  double budget = argc > 5 ? atof(argv[5]) : 1e18; size_t tstart = argc > 6 ? atoll(argv[6]) : 0; bool carried = argc > 7 && atoi(argv[7]);
  bool changed = true; int pass = 0; bool paused = false; size_t pausePos = 0;
  while (changed && !paused) { changed = carried; carried = false; pass++; long long rem = 0, recheck = 0;
    for (size_t t = tstart; t < P.size(); t++) {
      if ((t & 255) == 0 && chrono::duration<double>(chrono::steady_clock::now() - t0).count() > budget) { paused = true; pausePos = t; break; } if (!in[t]) continue; int x[12] = {0}; for (int i = 0; i < n; i++) x[i] = P[t][i];
  ...
  if (paused) { printf("PAUSED pos=%zu changed=%d |K|=%lld (resume: startfile=outfile, pos, changed)\n", pausePos, changed ? 1 : 0, alive); return 3; }
