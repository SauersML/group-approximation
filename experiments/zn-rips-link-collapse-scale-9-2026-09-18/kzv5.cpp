// kzv5: verifier for compact lifted certificates (D and K lines) on VR(Z^k, d_1; r), E = e_k.  It is kzv3 with a
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < N; i++) live[i / 64] |= 1ULL << (i % 64);
  if (K_ <= 8 && R_ <= 60) {
    // Row i is computed directly: byte t of q[j] is coordinate t of point j plus 64 (start points have |c| <= r <= 60),
    // and vpsadbw sums the absolute byte differences, which is exactly the l^1 distance.  The padding entries
    // j >= N are masked off after each row.
    int Np = (N + 3) / 4 * 4; vector<uint64_t> q(Np, 0);
    ...
    for (int i = 0; i < N; i++) { uint64_t* ri = &A[(size_t)i * NW]; const __m256i a = _mm256_set1_epi64x((long long)q[i]);
      for (int j = 0; j < Np; j += 4) { __m256i s = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&q[j]));
        uint64_t m = (uint64_t)_mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, s)));
        if (m) ri[j / 64] |= m << (j % 64); }
      if (N % 64) ri[NW - 1] &= (1ULL << (N % 64)) - 1; }
  } else
  for (int i = 0; i < N; i++) { A[(size_t)i * NW + i / 64] |= 1ULL << (i % 64);
