// kzh: kzf plus two search-order switches (KZ_KFIRST=1: skip the opening D-only sweep; KZ_KSEL=1: attempt (K) only at u with |u|=r, u_n=0;
// KZ_KLIST=file: attempt (K) only at the points listed in file). Same moves, same certificate format.
...
int main(int argc, char** argv) {
  ...
  if (n <= 8) { // packed rows: byte t of pk[i] is coordinate t + 64; d1 = sum of absolute byte differences (vpsadbw)
    int Mp = (M + 3) / 4 * 4; vector<uint64_t> pk(Mp, 0);
    for (int i = 0; i < Mp; i++) { uint64_t z = 0; for (int t = 0; t < 8; t++) z |= (uint64_t)(uint8_t)((t < n && i < M ? Pc[i][t] : 0) + 64) << (8 * t); pk[i] = (i < M) ? z : ~0ULL; }
    const __m256i lim = _mm256_set1_epi64x(R + 1);
    for (int i = 0; i < M; i++) { uint64_t* ri = row(i); const __m256i a = _mm256_set1_epi64x((long long)pk[i]);
      for (int j = 0; j < Mp; j += 4) { __m256i s = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&pk[j]));
        uint64_t m = (uint64_t)_mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, s)));
        if (m) ri[j / 64] |= m << (j % 64); }
      if (M % 64) ri[W - 1] &= (1ULL << (M % 64)) - 1; }
  } else
  for (int i = 0; i < M; i++) { row(i)[i / 64] |= 1ULL << (i % 64); for (int j = i + 1; j < M; j++) if (d1(Pc[i], Pc[j]) <= R) { row(i)[j / 64] |= 1ULL << (j % 64); row(j)[i / 64] |= 1ULL << (i % 64); } }
  if (getenv("KZ_ADJ_ONLY")) { uint64_t h = 1469598103934665603ULL; for (auto x : B) h = (h ^ x) * 1099511628211ULL; printf("adjhash %016llx t=%.1f\n", (unsigned long long)h, el()); return 0; }
  ...
  bool dmode = !getenv("KZ_KFIRST"); const bool ksel = getenv("KZ_KSEL") != nullptr;
