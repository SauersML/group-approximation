...
int main(int argc, char** argv) {
  ...
  int Mp = (M + 3) / 4 * 4; vector<uint64_t> pk(Mp, ~0ULL); // byte t = coordinate t + 64
  ...
  auto nbr = [&](uint64_t q, const uint64_t* P, int np, vector<int>& out) { out.clear(); const __m256i a = _mm256_set1_epi64x((long long)q);
    for (int j = 0; j < np; j += 4) { __m256i s = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&P[j]));
      int m = _mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, s))); while (m) { int t = __builtin_ctz(m); m &= m - 1; out.push_back(j + t); } } };
  ...
  while (left > 1 && !outOfTime) { changed = false; pass++;
    for (int u : ord) { if (!al[u] || u == en || left <= 1) continue; if (el() > budget) { outOfTime = true; break; }
      ...
      { int s0 = L.size(); Lp.assign((s0 + 3) / 4 * 4, ~0ULL); for (int i = 0; i < s0; i++) Lp[i] = pk[L[i]]; }
      // (D)
      ...
      for (int i = 0; i < s; i++) { uint64_t* li = lr(i); const __m256i a = _mm256_set1_epi64x((long long)Lp[i]);
        for (int j = 0; j < (int)Lp.size(); j += 4) { __m256i sd = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&Lp[j]));
          uint64_t m = (uint64_t)_mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, sd))); if (m) li[j / 64] |= m << (j % 64); } }
