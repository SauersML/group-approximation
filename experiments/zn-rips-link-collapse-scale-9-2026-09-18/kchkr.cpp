...
// Uses only pairwise distances inside K (no search over the ball).  Usage: ./kchkr k r Kfile [lo hi]
// kchkr = kchk with an optional range: only x with file index in [lo, hi) are checked (all global checks are still done),
// so the check can be split into pieces that together cover every index.
#include <bits/stdc++.h>
using namespace std;
int n, R;
...
int main(int argc, char** argv) {
  ...
  long long pairs = 0, cands = 0; size_t lo = argc > 5 ? atoll(argv[4]) : 0, hi = argc > 5 ? min((size_t)atoll(argv[5]), K.size()) : K.size();
  for (size_t ix = lo; ix < hi; ix++) { auto& x = K[ix]; if (x == ek) continue;
  ...
  if (lo == 0 && hi == K.size()) printf("OBSTRUCTION VERIFIED: k=%d r=%d |K|=%zu, every x in K\\{e_k} blocked for all %lld (x,w) pairs (%lld distance tests)\n", n, R, K.size(), cands, pairs);
  else printf("PIECE VERIFIED: k=%d r=%d |K|=%zu, x index range [%zu,%zu): all %lld (x,w) pairs blocked (%lld distance tests)\n", n, R, K.size(), lo, hi, cands, pairs);
}
