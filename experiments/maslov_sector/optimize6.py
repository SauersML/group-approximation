#!/usr/bin/env python3
"""v6 (v5 with the determinant-pin phase reduced mod 2 pi): p-power (near-max) objective, optional determinant pinning (kills the coboundary slide), large relator sets.
...
def initial(eps):
    ...
    return np.stack([base[i] @ expiH(eps * rand_herm()) for i in range(8)])
best = None
tag = f"best6_m{modulus}_{init.replace(':','_').replace('/','_')}_d{d}_p{pw}_pin{pin}_eps{eps0}_seed{seed}.npy"
