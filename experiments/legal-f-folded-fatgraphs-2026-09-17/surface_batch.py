...
cyclically legal words w of length 2, then 3 (in that order), searching an f^m-folded legal
...
usage: python3 surface_batch.py census.log m OUTDIR out.log
"""
...
for line in open(log):
    ...
    for L in (2, 3):
        ...
        if found and "independent=True" in found:
            break
    f.write("%d %s m=%d %s\n" % (n, json.dumps(phi0), m, found or "no-certificate-with-length<=3"))
    f.flush()
f.close()
