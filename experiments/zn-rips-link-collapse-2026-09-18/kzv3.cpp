// kzv3: independent verifier for compact lifted certificates (D and K lines) on VR(Z^k, d_1; r), E = e_k.
...
//             It stops early once cur lies in N[w].  Accept iff every z in the final cur has d(z,w) <= r.  Then u is deleted.
// Usage: ./kzv3 k r startfile certfile [outfile] [budgetSec]
...
int main(int argc, char** argv) {
  ...
  while (!(paused = chrono::duration<double>(chrono::steady_clock::now() - w0).count() > budget) && fscanf(f, "%3s", tag) == 1) { line++; P x, w; char bar[4];
    ...
    if (tag[0] == 'D') {
      ...
      auto okv = [&](int b, int v) { return v != b && in[v] && dist(pts[S[b]], pts[S[v]]) <= R_ && hullOK(pts[S[v]], {&x, &pts[S[b]], &E}) && domin(b, v); };
      ...
      while (again && !inside()) { again = false;
        for (int b = 0; b < s; b++) { if (!in[b] || b == wl) continue; const uint64_t* rb = &adj[(size_t)b * sw];
          ...
          if (removed) { in[b] = 0; cm[b / 64] &= ~(1ULL << (b % 64)); again = true; steps++; if (++sinceCheck % 256 == 0 && inside()) break; } } }
