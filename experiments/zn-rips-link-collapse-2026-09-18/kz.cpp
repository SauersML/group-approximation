// kz: lifted (D) + (K) engine with compact certificates (E = e_n).
    ...
    for (int u : ord) { if (!al[u] || u == en || left <= 1) continue; if (el() > budget) { outOfTime = true; break; }
      ...
      auto collapse = [&](vector<uint64_t>& cur, int prot) {
        ...
        while (prog) { prog = false;
          if (prot >= 0) { if (covers(cur, prot)) return; }
          else { for (int c : cands) if (((cur[c / 64] >> (c % 64)) & 1) && covers(cur, c)) return; }
          for (int b = 0; b < s; b++) { if (b == prot || !((cur[b / 64] >> (b % 64)) & 1)) continue; uint64_t* rb = lr(b); const Pt& pb = Pc[L[b]];
            for (int k0 = 0; k0 < SW; k0++) { uint64_t bits = cur[k0] & rb[k0]; bool found = false;
              while (bits) { int v = k0 * 64 + __builtin_ctzll(bits); bits &= bits - 1; if (v == b || !inh(Pc[L[v]], Pc[u], pb, EN)) continue; uint64_t* rv = lr(v); bool ok = true;
                for (int k = 0; k < SW; k++) if (rb[k] & cur[k] & ~rv[k]) { ok = false; break; }
                if (ok) { found = true; break; } }
              if (found) { cur[b / 64] &= ~(1ULL << (b % 64)); prog = true; break; } } } } };
