...
def hom_from_images(
    ...
    return multiply(
        power(image_r, r),
        multiply(power(image_p, a), power(image_q, b)),
    )
...
def audit_map(
    ...
    values = tuple(
        hom_from_images(value, image_r, image_p, image_q)
        for value in ELEMENTS
    )
    assert len(set(values)) == len(ELEMENTS)
    for left, right in product(ELEMENTS, repeat=2):
        assert hom_from_images(
            multiply(left, right), image_r, image_p, image_q
        ) == multiply(
            hom_from_images(left, image_r, image_p, image_q),
            hom_from_images(right, image_r, image_p, image_q),
        )
    return values
...
def audit() -> None:
    ...
    inverse_r = inverse(R)
    shear_q = audit_map(inverse_r, P, diagonal)
    shear_p = audit_map(inverse_r, diagonal, Q)
    ...
    print("opposite phase-and-conjugation-corrected shears are involutive automorphisms")
