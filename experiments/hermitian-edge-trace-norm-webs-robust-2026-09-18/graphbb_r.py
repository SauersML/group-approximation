# graphbb_r.py: experiments/hermitian-edge-trace-norm-webs-2026-09-18/graphbb.py with the per-box SDP replaced by a ROBUST SDP
# (one Z maximising the box bound over the whole phase box; see the comment at Zv below).  The certificate
# (exact rational Z, exact -I < Z < I check, outward-rounded edge-separable box_bound) is unchanged.
# Optional: env BOX=[[lo,hi],...] replaces the initial box (used only for tests).
...
#          induced on the listed vertices; i~j iff (i-j) mod n is not in {0,1,n-1}), or
#          W<n>/<d1>.<d2>...:<v1>,<v2>,... (the subgraph induced on the listed vertices of the circulant graph
#          on Z_n with i~j iff the cyclic distance of i and j is one of d1, d2, ...; e.g. webs C_n^k).
...
def parse(g):
    ...
    if g.startswith('W'):   # W<n>/<d1>.<d2>...:<v1>,<v2>,...  circulant on Z_n, i~j iff (i-j) mod n or (j-i) mod n in {d}
...
# ROBUST variant: one Z for the whole box.  For a free edge with phase arc [lo,hi] (length < pi) the
# function th -> Re(Z_ji e^{i th}) is a sinusoid; if it is >= 0 at both endpoints its minimum over the
# arc is at an endpoint.  So the SDP below maximises the box bound over Z with g >= 0 on the box.
# (Only used to FIND Z; the certificate is still box_bound on the exactly checked rational Z.)
Zv=cp.Variable((M,M),hermitian=True)
PL=[cp.Parameter(complex=True) for _ in E]; PH=[cp.Parameter(complex=True) for _ in E]
sv=cp.Variable(len(E))
cons=[np.eye(M)-Zv>>0,np.eye(M)+Zv>>0,sv>=0]
for k,(i,j) in enumerate(E):
    cons+=[sv[k]<=2*cp.real(Zv[j,i]*PL[k]), sv[k]<=2*cp.real(Zv[j,i]*PH[k])]
prob=cp.Problem(cp.Maximize(cp.sum(sv)),cons)
def solve_box(box):
    for k in range(len(E)):
        if k<NS: PL[k].value=1.0+0j; PH[k].value=1.0+0j
        else:
            lo,hi=box[k-NS]; PL[k].value=np.exp(1j*lo); PH[k].value=np.exp(1j*hi)
...
if __name__=="__main__":
    ...
    BX=_json.loads(_os.environ['BOX']) if 'BOX' in _os.environ else None
    ...
    while stack:
        ...
        if max(u[1]-u[0] for u in box)>=3.0: v,Z=-1e9,None
        else: v,Z=solve_box(box)
        solves+=1
        if v is None: v,Z=-1e9,None
        if v>-1e8 and v<minval: minval=v; worst=x
