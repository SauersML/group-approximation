// kzv6: kzv5 without the global N x N adjacency matrix, so that (8,10) sets of 250k+ points fit in 2GB.  It checks the
...
int main(int argc, char** argv) {
  ...
  if (K_ > 8 || R_ > 60) { puts("kzv6 needs k <= 8 and r <= 60"); return 1; }
  int Np = (N + 3) / 4 * 4; vector<uint64_t> q(Np, ~0ULL);
  ...
  auto nearAll = [&](uint64_t a0, const vector<uint64_t>& Q, vector<int>& out) { out.clear(); const __m256i a = _mm256_set1_epi64x((long long)a0);
    for (size_t j = 0; j < Q.size(); j += 4) { __m256i sd = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&Q[j]));
      int m = _mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, sd))); while (m) { out.push_back((int)j + __builtin_ctz(m)); m &= m - 1; } } };
  vector<int> Nx;
  fprintf(stderr, "kzv6: %d start points (implicit adjacency)\n", N);
  ...
  while (!(paused = chrono::duration<double>(chrono::steady_clock::now() - w0).count() > budget) && fscanf(f, "%3s", tag) == 1) { line++; P x, w; char bar[4];
    ...
    if (tag[0] == 'D') {
      ...
      { int sp = (s + 3) / 4 * 4; vector<uint64_t> Sq(sp, ~0ULL); for (int i = 0; i < s; i++) Sq[i] = q[S[i]];
        for (int i = 0; i < s; i++) { if (S[i] == wi) wl = i; uint64_t* out = &adj[(size_t)i * sw]; const __m256i a = _mm256_set1_epi64x((long long)Sq[i]);
          for (int j = 0; j < sp; j += 4) { __m256i sd = _mm256_sad_epu8(a, _mm256_loadu_si256((const __m256i*)&Sq[j]));
            uint64_t m = (uint64_t)_mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpgt_epi64(lim, sd))); if (m) out[j / 64] |= m << (j % 64); } } }
      ...
      map<P, int> lid; for (int i = 0; i < s; i++) lid[pts[S[i]]] = i;
      ...
      while (again && !inside()) { again = false;
        for (int b = 0; b < s; b++) { if (!in[b] || b == wl || !dirty[b]) continue; dirty[b] = 0; const uint64_t* rb = &adj[(size_t)b * sw]; setbox(b);
          bool removed = false;
          // try the lattice neighbours of b first (any valid dominator is acceptable)
          for (int i = 0; i < K_ && !removed; i++) for (int dl = -1; dl <= 1 && !removed; dl += 2) { P q = pts[S[b]]; q.c[i] += dl;
            auto it = lid.find(q); if (it != lid.end() && okv(b, it->second)) removed = true; }
          ...
          for (int q = 0; q < sw && !removed; q++) { uint64_t cand = bx[q];
            while (cand) { int v = q * 64 + __builtin_ctzll(cand); cand &= cand - 1; if (okv(b, v)) { removed = true; break; } } }
          if (removed) { in[b] = 0; cm[b / 64] &= ~(1ULL << (b % 64)); again = true; steps++;
            for (int q = 0; q < sw; q++) { uint64_t nb = rb[q] & cm[q]; while (nb) { dirty[q * 64 + __builtin_ctzll(nb)] = 1; nb &= nb - 1; } } if (++sinceCheck % 256 == 0 && inside()) break; } } }
