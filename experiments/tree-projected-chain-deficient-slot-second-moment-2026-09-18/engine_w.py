...
def desc_value(d, m):
    ...
    c, f, a, b, g2 = d
    return F(m) ** c * F(m - 1) ** f * F(P_plain(m)) ** a * F(P_neq(m)) ** b * F(m - 2) ** g2
        ...
        if PW and generic:
            for d, e in ex.items():
                ...
                if x is not None and x in parent:
                    ...
                    if pr is not None and lv == best[t] and pr[1] == M_REF - lv:
                        pars.append((lv, pr[0]))
...
def param_cap(key, m, K):
    ...
    if d1 == d2 and s1 == -s2:      # autocorrelation of one set
        cap = ((m - 2) if NONAP else (m - 1)) if nz else m
                    ...
                    if WAUG and not bad:
                        ...
                        for kap in LETTERS:
                            ...
                            if POL:
                                ...
                                parW = None
                            for nq, mult in nqs.items():
                                kk = (si, kap, ne, nq) + tuple(dW) + ((parW,) if WPW else ())
...
W_AGG = {}
WPW = False      # keep the pointwise parameter of generic W terms
