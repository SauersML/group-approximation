"""Short exact relators in ``<P,J1,J2,B2,n,m>``.

Run only through MSI.  This is a bounded faithful-normal-form calibration of
the all-word ``R/pR`` quotient proof, not a presentation computation.
"""

import argparse

from endpoint_chain_center_hecke_collision_audit import coeff, whitehead
from signed_hecke_literal_pauli_probe_audit import (
    identity_matrix,
    matrices_equal,
    matrix_multiply,
    word_matrix,
)


IDENTITY = identity_matrix()
NAMES = ("P", "J", "K", "B", "n", "m")


def root(target, source, coefficient):
    return word_matrix(((source, target, coefficient),))


GENERATORS = (
    word_matrix(whitehead(8, 2, 2)),
    word_matrix(whitehead(7, 8, 1)),
    word_matrix(whitehead(8, 9, 2)),
    root(8, 4, coeff("00", "1")),
    root(7, 8, coeff("", "")),
    root(8, 7, coeff("", "")),
)


def canonical_cycle(word):
    rotations = []
    for candidate in (word, tuple(reversed(word))):
        rotations.extend(candidate[i:] + candidate[:i] for i in range(len(word)))
    return min(rotations)


def display(word):
    return "".join(NAMES[index] for index in word)


def involutive_reduce(word):
    stack = []
    for letter in word:
        if stack and stack[-1] == letter:
            stack.pop()
        else:
            stack.append(letter)
    return tuple(stack)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--depth", type=int, default=6)
    depth = parser.parse_args().depth
    relators = set()
    hits = {}

    def visit(word, value):
        length = len(word)
        if length >= 2 and word[0] != word[-1] and matrices_equal(value, IDENTITY):
            relators.add(canonical_cycle(word))
            hits[length] = hits.get(length, 0) + 1
        if length == depth:
            return
        for index, generator in enumerate(GENERATORS):
            if word and word[-1] == index:
                continue
            visit(word + (index,), matrix_multiply(value, generator))

    visit((), IDENTITY)
    print("maximum freely reduced length", depth)
    print("identity hits by length", tuple(sorted(hits.items())))
    print("cyclic relators", tuple(display(word) for word in sorted(relators)))
    print(
        "P=J=K=B=1 reductions",
        tuple(
            (display(word), display(involutive_reduce(tuple(x for x in word if x >= 4))))
            for word in sorted(relators)
        ),
    )


if __name__ == "__main__":
    main()
