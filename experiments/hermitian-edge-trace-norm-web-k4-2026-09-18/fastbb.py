...
def parse(g):
    ...
    return len(S),E
GRAPH=sys.argv[1]; TARGET=float(sys.argv[2]); T=float(sys.argv[3])
M,E0=parse(GRAPH)
...
def pack(V):
    ...
    for u,w in itertools.combinations(rest,2):
        if adjq(v,u) and adjq(v,w) and adjq(u,w): best=max(best,2*math.sqrt(3)*(1-1e-12)+pack([x for x in rest if x not in (u,w)]))
    return best
MCUT=min(pack([x for x in range(M) if x not in e]) for e in E0)
assert 2*T+MCUT>=TARGET, (T,MCUT,(TARGET-MCUT)/2)
...
E=tree+free; NS=len(tree); NF=len(free)
PAD=1e-12
...
def split_dim(Zf,box):
    ...
    for k in range(NF):
        ...
        gc=2*(z*np.exp(1j*c)).real; gc=gc if gc>=0 else T*gc
        cm=cos_min(lo+np.angle(z),hi+np.angle(z)); gm=2*abs(z)*cm; gm=gm if gm>=0 else T*gm
        if gc-gm>best: best=gc-gm; d=k
    return d
def pieces(lo,hi):
    p=2 if hi-lo<2*(math.pi-0.05) else 3
...
if __name__=="__main__":
    ...
    while stack:
        ...
        C=centre_matrix(box); w,U=np.linalg.eigh(C); fc=float(np.abs(w).sum())
    ...
    st['stack']=[]; save()
    print(GRAPH,"edges %d tree %d (root %d) free phases %d first %s SYM %s TARGET %.7f T %.3f first phase in [%.6f,%.6f]: ALL CLOSED  boxes %d (dropped by symmetry %d; parent Z %d, centre sign %d, robust SDP %d)  solves %d  min centre value %.6f at %s  smallest half-width %.2e  time %.0fs"%(len(E),NS,ROOT,NF,E[NS],os.environ.get('SYM','-'),TARGET,T,lo_a,hi_a,st['closed'],st.get('symdrop',0),st['byparent'],st['bysign'],st['bysdp'],st['solves'],st['minfc'],[round(u,4) for u in (st['worst'] or [])],st['minw'],time.time()-t0),flush=True)
