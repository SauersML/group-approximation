...
\begin{lemma}[defect identification; the containment
$\supseteq$ is machine-checked as
\texttt{simpleSubgroup\_le\_defectNormal}]\label{lem:defect}
$D=\ncl{\pi(S)}_G$.
\end{lemma}
...
The accompanying Lean~4 development (repository
\texttt{nonsofic\_existence}, modules
\texttt{Sofic/NormalKazhdanMFRadical},
\texttt{Sofic/TorsionFreeFullMFRadical},
\texttt{Sofic/TorsionFreeFullMFConsequences},
\texttt{Criterion/CriterionAssembly}) machine-checks the following, with no
axioms and no literature-transcription premises:
...
\item all implications downstream of the saturation step: from
  ...
  of every homomorphism to an operator-MF group --- and, given the
  descended criterion data, Theorem~\ref{thm:main}(3) for the routed
  quotient (\texttt{NonsoficCriterionData.not\_isSofic}).
\end{itemize}
