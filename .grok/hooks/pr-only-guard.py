...
Tool names are Grok's real ones, read out of live PreToolUse events and the
CLI's own tool inventory: mutations are `search_replace`, `edit_file`,
`write_file` and `delete_file`; the shell is `run_terminal_command`. The
Claude Code spellings are accepted too so the guard survives being pointed at a
Claude-compatible harness. Getting these wrong is silent: a matcher that never
matches disables the guard while every listing still shows it installed, which
is exactly how this guard spent its first day doing nothing.

Exit 2 means deny. That also means a crash or a missing file denies, so this
must not raise on unexpected input -- unknown events fall through to allow.
"""
...
# Grok's mutating tools, plus the Claude Code spellings for portability.
EDIT_TOOLS = {
    "search_replace", "edit_file", "write_file", "delete_file",
    "Edit", "MultiEdit", "Write", "NotebookEdit", "edit", "str_replace",
}
# Grok's shell tool, plus the Claude Code spelling.
SHELL_TOOLS = {"run_terminal_command", "Bash", "bash", "shell"}
...
def main() -> None:
    ...
    cwd = cwd.rstrip("/") or "/"          # workspaceRoot arrives with a trailing slash
