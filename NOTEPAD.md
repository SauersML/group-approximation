...
### Haar averaging reduces kernel-compatible strictness to one conditional expectation

There is an exact way to choose a witness which commutes with the corrected
kernel grading, provided adjoining that grading does not absorb the strict
Laurent letter.

**Lemma.**  Let `W` be a unital star-subalgebra of `M_d(C)`, let `E_W` be
the trace-preserving conditional expectation onto `W`, and let `x` be a
unitary.  With normalized Hilbert--Schmidt norm,

`integral_(v in U(W')) ||[v,x]||_2^2 dv`
` =2(1-||E_W(x)||_2^2)`.

In particular there is a unitary `v in W'` with commutator squared norm at
least the right-hand side.

**Proof.**  Haar averaging conjugation by `U(W')` is the conditional
expectation onto `(W')'=W`.  Expanding the squared commutator and integrating
gives

`2-2 Re tr(x^* E_W(x))=2-2||E_W(x)||_2^2`,

using orthogonality of conditional expectation.  End proof.

At depth block `j`, let `W_j` be the algebra generated by the native left
regular image of `alpha^(j+1)(Gamma_z)` together with the finitely many
corrected toric-kernel holonomy operators required by the current window.
Apply the lemma to the strict operator

`x_j=pi_j(gamma)`.

Any resulting `z_j in W_j'` commutes simultaneously with the deeper
polynomial image and with the kernel grading.  The orbit-chain assembly then
has the same centralization and kernel-preservation properties as before.
Moreover, if

`||E_(W_j)(x_j)||_2<=kappa<1`

uniformly on a positive-density set of depth blocks, its strict commutator
has a uniform lower bound `sqrt(2(1-kappa^2))` there.

For the original witness algebra

`W_j=lambda(H_(j+1))''`,

the regular-group basis gives `E_(W_j)(lambda(g_j))=0` whenever
`g_j notin H_(j+1)`, recovering the previous `sqrt(2)` estimate.  Thus the
new finite target is sharply algebraic: compute the toric holonomies in the
regular coefficient coordinates and prove that adjoining them does not put
`lambda(g_j)` close to the generated algebra.  If the holonomies are left
translations by a subgroup `L_j`, it is enough to prove

`g_j notin <H_(j+1),L_j>`;

then the expectation is again exactly zero.  If they lie in a more general
matrix commutant, the displayed conditional-expectation bound is the right
replacement.  This route simultaneously supplies a kernel-commuting witness
and quantifies its surviving strict energy; it avoids an unsupported claim
that the original Haar-chosen witness already commutes with `K_2`.
