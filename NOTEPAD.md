...
**Correction: exact-on-`Gamma` microstates are not sterile.**  The theorem
previously written here was false.  Its invalid step applied Kun--Thom's
finite-quotient equality

`image(alpha(Gamma)) = image(Gamma)`

to an arbitrary finite-image representation of `Gamma`.  Kun--Thom prove
that equality for finite quotients of the **ambient group `G`**, where the
compressor is represented and conjugacy makes the inclusion an equality in
a finite group.  An arbitrary finite quotient of `Gamma` need not extend to
`G`; its image of `alpha(Gamma)` can be a proper subgroup.  Thus the claimed
corollary

`H_free hyperlinear => Gamma flexibly HS-unstable`

is retracted as well.  What remains valid is the earlier, correctly scoped
statement that models liftable through genuine finite quotients (or genuine
finite-dimensional representations) of the full ambient `G` are sterile.

**Theorem (endomorphism-orbit chains).**  Let `alpha:Gamma->Gamma` be any
injective endomorphism and let `sigma:Gamma->U(m)` be a finite-dimensional
unitary representation.  For `L>=1` define the genuine representation

`pi_L(g)=direct_sum_(j=0)^(L-1) sigma(alpha^j(g))`.

There is a block-cycle unitary `u_L in U(Lm)` such that for every `g in Gamma`

`||u_L pi_L(g) u_L^* - pi_L(alpha(g))||_2 <= 2/sqrt(L)`.

Consequently the assignments `g |-> pi_L(g)` and `t |-> u_L` are
asymptotic representations, as `L->infinity`, of the ascending HNN
presentation generated by `Gamma,t` with

`t g t^-1 = alpha(g)`.

**Proof.**  Write `a_j=sigma(alpha^j(g))`.  Choose the orientation of the
cycle so that

`u_L diag(a_0,...,a_(L-1))u_L^*=diag(a_1,...,a_(L-1),a_0)`.

This differs from

`pi_L(alpha(g))=diag(a_1,...,a_(L-1),a_L)`

only on the final `m`-dimensional block.  Since the difference of two
unitaries has unnormalized Hilbert--Schmidt norm at most `2 sqrt(m)`, its
normalized norm in dimension `Lm` is at most `2/sqrt(L)`.  A fixed word uses
only finitely many defining relations, so its defect is bounded by its
relation area times `2/sqrt(L)`.  End proof.

This construction shows exactly why finite image of `sigma` does not kill
the route.  The compressor is implemented only asymptotically, and the
nonextendable finite quotient can see the strict inclusion
...
`SL_d(Z)` is nonamenable.  Any successful assembly must therefore exploit
representation equivalences/multiplicities (the noncommutative analogue of
...
**Exact consequence for the main route.**  This theorem closes hyperfinite
asymptotic representations as a source of uncorrectable behavior.  It does
*not* imply flexible HS-stability, and after the orbit-chain correction above
we no longer know that a strict model must be flexibly nonliftable on
`Gamma`.  Moreover, a trace-normalized hyperlinear model of the ambient group
already restricts to the regular character of the nonamenable group `Gamma`,
so its marked limit is automatically non-hyperfinite.  Thus the theorem is a
real stability classification but only a modest localization of the main
construction problem; it must not be advertised as the terminal obstruction.

The ordinary local-HS instability theorem for infinite hyperlinear
property-`(T)` groups also does not settle the construction: its Theorem 6.1
is non-flexible, and the standard deleted-small-rank witnesses are flexibly
...
### The cyclic lamp has a nonsofic symmetric-double commutator

There is a cleaner form of the weak-MF nonsofic example, and it also shows
that the finite-central-monolith criterion cannot be obtained merely by
passing to the obvious finite-index subgroup.  Assume, as in the stable
Kun--Thom examples, that `G` is perfect, fix a prime `p`, and put

`H_p = G *_Gamma (Gamma x C_p)`.

Let `k` generate `C_p`.  The presentation is equivalently generated by `G`
and `k`, with `k^p=1` and `[k,Gamma]=1`.  Abelianizing kills `G`; it also
kills the `Gamma` coordinate in the second vertex group, and leaves exactly
the class of `k`.  Hence

`H_p^ab = C_p`

and the commutator subgroup `D_p=[H_p,H_p]` is the kernel of the total-lamp
exponent map `H_p -> C_p`.

**Lemma (cyclic Schreier kernel).**  There is a natural isomorphism

`D_p ~= G *_Gamma G *_Gamma ... *_Gamma G`  (`p` copies),

where the `i`th copy is `k^i G k^(-i)` and all copies contain the same
subgroup `Gamma` because `k` centralizes `Gamma`.

**Proof.**  Reidemeister--Schreier with transversal
`1,k,...,k^(p-1)` gives the generators `k^i g k^(-i)`, for `g in G`.
The relators inside each conjugate give a copy of `G`.  The relators
`k gamma k^(-1)=gamma` identify the `p` conjugate copies of `Gamma`, and
the relator `k^p=1` supplies no further kernel generator or relation.
This is precisely the displayed iterated amalgam presentation.  Equivalently,
it is the graph-of-groups covering associated to the kernel of
`H_p -> C_p`: one central `Gamma` vertex, `p` `G` vertices, and one edge to
each.  End proof.

The group `D_p` is weak-MF because it is a subgroup of the already weak-MF
group `H_p`.  It is nonsofic: if `D_p` were sofic, closure of soficity under
extensions with amenable quotient would make its finite extension `H_p`
sofic, contradicting the Kun--Thom free-lamp argument.  In particular,

`D_2 ~= G *_Gamma G`

is an explicit weak-MF nonsofic **symmetric double**.  Thus Shulman's
full-C-star MF theorem may be applied directly to the nonsofic group rather
than to a larger host containing it.  The unresolved issue is still exactly
whether the canonical trace of this double is Connes embeddable.

This simplification simultaneously rules out the monolith shortcut inside
`D_p`.  Let `q:D_p->G` be the fold map identifying all `p` copies.  Its
kernel is nontrivial when `Gamma` is proper: if `g in G - Gamma`, the element
represented by `g` in one copy times the inverse of `g` in a second copy is
a nontrivial reduced word killed by `q`.  On the other hand, every
finite-order element of an amalgam acting on its Bass--Serre tree fixes a
vertex, hence is conjugate into one of the copies of `G`.  The fold is
injective on every conjugate of every vertex group: it maps
`h g h^(-1)` to `q(h) g q(h)^(-1)`.  Consequently no nonidentity
finite-order element belongs to `ker(q)`.  Since `ker(q)` is a nontrivial
...
### A central Clifford-lamp witness that would finish from weak MF

The obstruction to centralizing a coordinate **difference** does not apply to
a central coordinate **commutator**.  This yields a new target on which the
finite-central spectral corner is exactly aligned with the Kun--Thom witness.

Let `X=G/Gamma`.  Define `K_X` by generators

`z`, and `a_x` for `x in X`,

and relations

`z^2=1`, `z` central, `a_x^2=1`,
`[a_x,a_y]=z` for `x!=y`.

The element `z` is nontrivial.  For every finite `F subset X`, the subgroup
on `z,{a_x:x in F}` has the normal forms

`z^epsilon product_(x in F) a_x^(epsilon_x)`

after choosing an order on `F`, and has order `2^(|F|+1)`.  Equivalently,
send `z` to `-1` and the `a_x` to anticommuting self-adjoint Clifford
unitaries to see that the normal forms do not collapse.  Hence `K_X` is the
directed union of finite `2`-groups and is amenable.

Every permutation of `X` induces an automorphism of `K_X`, fixing `z` and
permuting the `a_x`.  Form

`E_Cl=K_X semidirect G`.

This group is finitely generated by `G` and `a_o`, where `o=Gamma`; once a
coset distinct from `o` is present, `z` is one of the commutators.  It has a
retraction `r:E_Cl->G` killing `K_X`.

**Theorem (weak-MF Clifford target).**  If `E_Cl` is weak-MF, then there
exists a hyperlinear nonsofic group.

**Proof.**  The involution `z` is central.  Apply the finite-central-torsion
corner theorem to an operator-norm ultraproduct embedding of `E_Cl`.  It
produces a homomorphism

`eta:E_Cl -> product_omega U(d_n)`

with `eta(z)!=1`; `eta` need not be faithful.  Since the Kun--Thom ambient
group `G` is residually finite, choose a faithful hyperlinear embedding
`theta:G->product_omega U(e_n)`.  After harmless repetitions making the two
matrix dimensions comparable, take the block sum

`Phi=eta direct_sum (theta compose r)`.

Then `Phi(z)!=1` and `Phi|_G` is injective.  Its image `Q=Phi(E_Cl)` is
hyperlinear.

It remains to prove that `Q` is nonsofic.  Let `t` be a strict Kun--Thom
compressor, so `t Gamma t^(-1)` is a proper subgroup of `Gamma`.  Choose
`gamma in Gamma` with

`gamma t Gamma != t Gamma`.

The element `a_o` centralizes `Gamma`.  If `Q` were sofic, restrict a sofic
embedding of `Q` to the faithful copy `Phi(G)`.  Kun--Thom centralizer
normality says that the conjugate

`Phi(t a_o t^(-1))=Phi(a_(t Gamma))`

also centralizes `Phi(Gamma)`.  It would therefore commute with
`Phi(gamma)`.  But in `E_Cl`, putting `x=t Gamma` and `y=gamma t Gamma`,

`[a_x,gamma]=a_x a_y`,
`[a_x,gamma]^2=[a_x,a_y]=z`.

Thus its image cannot be the identity because `Phi(z)!=1`, a contradiction.
So `Q` is nonsofic.  End proof.

This criterion is stronger than the earlier finite-central-monolith target:
no normal-subgroup classification is needed.  The retraction summand keeps
the entire Kun--Thom group faithful, while the spectral-corner summand keeps
the one central obstruction `z`.

The remaining task is now sharply algebraic/operator-norm:

> Prove that the Clifford-lamp semidirect product
> `K_(G/Gamma) semidirect G` is weak-MF.

Finite lamp subsets have exact Clifford matrix models, and finite
permutations of their generators are implemented by spin/Pin unitaries.
What is not automatic is simultaneous operator-norm assembly of the
infinite coset action.  Unlike scalar phase flux, the relation
`[a_x,a_y]=z` acts noncentrally on the Clifford generators even though `z`
...
### Integral coefficients synchronize the varying prime, but exact quotients remain sterile

The fixed characteristic in the original Kun--Thom pair is not essential.
This gives a useful common arithmetic base for the varying Schur-curvature
sector and the polynomial-shear sector, although the theorem below also
isolates exactly why congruence models still cannot perform the coupling.

Put

`R_+=Z[N^d]=Z[x_1,...,x_d]`,

`R=Z[Z^d]=Z[x_1^(+-1),...,x_d^(+-1)]`,

`Gamma=EL_r(R_+)`,

`G=EL_r(R) semidirect SL_d(Z)`,

where `r,d>=3` and `SL_d(Z)` acts by substitutions
`alpha_A(x^v)=x^(Av)`.

**Theorem (integral-coefficient Kun--Thom pair).**  The groups `Gamma` and
`G` are residually finite Kazhdan groups, `Gamma<G` is infranormal and is
not normal, and hence the Kun--Thom nonsofic wreath-product theorem applies
to this pair.

**Proof.**  Both `R_+` and `R` are finitely generated unital rings.  Ershov--
Jaikin-Zapirain, Theorem 1.1, therefore gives property `(T)` to
`EL_r(R_+)` and `EL_r(R)`.  The group `SL_d(Z)` has property `(T)`, and
property `(T)` is closed under extensions, so `G` also has property `(T)`.

Every nonnegative matrix `A in SL_d(Z)` sends `N^d` into `N^d`, and hence
`(1,A) Gamma (1,A)^(-1) <= Gamma`.  The nonnegative elementary
transvections `I+E_ij` generate `SL_d(Z)` as a group.  Thus the group
generated by the compression semigroup contains `SL_d(Z)` and `Gamma`.
It also contains every `SL_d(Z)`-conjugate of `Gamma`.  Given any
`v in Z^d`, write `v=m u` with `u` primitive and choose
`A in SL_d(Z)` with `A e_1=u`; then `x^v=alpha_A(x_1^m)`.  The elementary
relations `e_ij(a+b)=e_ij(a)e_ij(b)` now show that those conjugates generate
`EL_r(R)`.  Hence `Gamma` is infranormal in `G`.

It is not normal: the determinant-one sign change
`A=diag(-1,-1,1,...,1)` sends `e_12(x_1)` to `e_12(x_1^(-1))`, a matrix not
belonging to `EL_r(R_+)` because its `(1,2)` entry is not in `R_+`.

For residual finiteness, let `(g,A)!=1` in `G`.  If `A!=1`, reduce
`SL_d(Z)` modulo a suitable integer.  If `A=1`, choose a nonzero Laurent
entry `f` of `g-I`.  Choose `m` so that the finitely many exponents in the
support of `f` remain distinct in `(Z/mZ)^d`, and choose a prime `p` not
dividing one of their nonzero integer coefficients.  The equivariant map

`R -> F_p[(Z/mZ)^d]`

and reduction `SL_d(Z)->SL_d(Z/mZ)` give a finite semidirect-product quotient
in which `g` remains nontrivial.  Thus `G`, and hence its subgroup `Gamma`,
is residually finite.  End proof.

The payoff is that the same varying prime `p` may now simultaneously index
the finite parabolic/Schur class in `SL_d(F_p)` and the coefficient reduction
of the elementary-ring sector.  But exact finite-ring realizations are still
incapable of separating the invisible Laurent layer.

**Proposition (coefficient-independent congruence sterility).**  For every
homomorphism from `R` to a finite ring `S`, the images of `R_+` and `R` are
equal.  Consequently the images of `EL_r(R_+)` and `EL_r(R)` are equal in
every finite ring quotient.

**Proof.**  Each image `u_i` of `x_i` is a unit of the finite monoid `S^x`,
so it has finite order, say `u_i^(n_i)=1`.  Hence
`u_i^(-1)=u_i^(n_i-1)` lies in the image of the positive polynomial ring.
The latter image therefore already contains every Laurent monomial and is
the whole image of `R`.  Apply the elementary generators.  End proof.

Thus integral coefficients are a genuine synchronization theorem but not a
congruence construction.  Any successful coupling to the sparse Schur class
...
### Unit-step upgrade removes the relative-congruence Steinberg kernel

The level-`p` compressors are unnecessary.  Keeping the prime fixed does not
require the compressor to reduce to the identity.  Fix instead, for each
ordered root,

`t_(ji)=I+E_(ji) in SL_4(Z)`.

In `Q_n=SL_4(Z/p^n Z)`, its image has order exactly `p^n`, since

`t_(ji)^m=I+mE_(ji)`.

Its action on `Q_n/B` is free.  Every nonidentity power has `p`-power order,
so it cannot be conjugate into `B` of order four.  Thus every outer orbit is
a cycle of length `p^n`, one power of `p` longer than in the level-`p`
version.

The coefficient action is now the unit shear

`alpha_t(x_i)=x_i x_j`,  `alpha_t^m(x_i)=x_i x_j^m`.

Consequently the fixed-prime orbit-chain construction uses

`S_(n,L)=F_p[x_i,x_j]/(x_i^2,x_j^(p^n L+1))`,

`B_m=F_p[x_j]+x_i x_j^m F_p[x_j]`,

and has wrap defect at most

`2/sqrt(p^n L)`.

The strict element `x_i x_j^m in B_m \ B_(m+1)` exists on every nonfinal
edge, so the macroscopic relative-commutant witness is unchanged.

This choice also restores the full substitution group.  Pick one root
`t_delta=I+E_(delta,0)` for each of the three nonzero differences in the
regular `B`-set.  Their `B`-conjugates are all twelve unit elementary
matrices `e_(ij)(1)`.  These generate

`E_4(Z)=SL_4(Z)`.

Hence the preferred target is again the original Kun--Thom pair
...
generated on the substitution side by the same Pauli group `B` and only
three positive compressors.  The finite-index subgroup `Lambda_p` remains
valid but is no longer needed.

There is a decisive Steinberg benefit.  For distinct `i,j,k`, the first
noncommuting cell is now

`u=e_(ij)(1)`,  `v=e_(jk)(1)`,  `w=e_(ik)(1)`,

with `[u,v]=w`; there is no relative depth-`p^2` generator.  The Heisenberg-
cell Følner theorem above applies verbatim with these unit shears.

Finally, the classical Dennis--Stein computation gives

`K_2(Z/p^n Z)=0`

for odd `p`.  One quick structural proof is that `K_2(Z/mZ)` is generated by
the Steinberg symbol `{-1,-1}`, hence is `2`-primary, whereas for
`m=p^n` the nilpotent-relative calculation and `K_2(F_p)=0` make it
`p`-primary.  For odd `p` both statements force zero.  Taking `p>=7` also
puts the rank-four Steinberg presentation safely in the stable local-ring
range.  Therefore

`St_4(Z/p^n Z) -> SL_4(Z/p^n Z)`

has trivial central kernel.

**Consequence.**  After shared root operators in the unit-step `A_2` cells
are identified, the Curtis--Tits/Steinberg overlap has no residual finite-
...
### Why regular stabilization does not yet couple the coefficient algebra

There is an exact coefficient analogue of the regular shared-root lemma,
but it also identifies the sterility boundary.

**Lemma (finite regular absorption).**  Let `H` be a finite quotient of
`Gamma` and let `V` be any finite-dimensional representation of `H`.  Then

`V tensor lambda_H ~= dim(V) lambda_H`

as `H`-representations.  Consequently any finite list of representations of
`Gamma` which factor through one common `H` become exactly unitarily
equivalent after tensoring with `lambda_H` and equalizing multiplicities.

**Proof.**  The character of `lambda_H` is `|H|` at the identity and zero
elsewhere.  Multiplication by the character of `V` therefore gives
`dim(V)` times the regular character.  End proof.

If a polynomial compression `alpha_s` descends to an automorphism `beta_s`
of `H`, the permutation unitary

`P_(beta_s) delta_h = delta_(beta_s(h))`

also implements the covariance

`P_(beta_s) lambda_H(h) P_(beta_s)^* = lambda_H(beta_s(h))`.

Thus common-quotient regular stabilization would solve both the coefficient
identification and covariance **if** all the positive compressions became
automorphisms of the common finite quotient.  But that hypothesis is
exactly sterile.  In every finite ring quotient of the Laurent coefficient
ring, each variable is a finite-order unit, so its inverse is a positive
power; the polynomial and Laurent images coincide.  Equivalently, once all
compressions are automorphisms of one common finite quotient, the relative
commutant absorbs the profinitely invisible layer and the strict witness is
killed.

Therefore the regular shared-root stabilization is valid, and the analogous
coefficient stabilization is valid on liftable sectors, but it cannot close
the construction.  A successful four-chamber coupling must retain the
nonautomorphic compression chains on a macroscopic sector while making only
their cross-chamber defects negligible.  This is the precise reason the
remaining overlap is a defect-localization problem rather than a spectral-
...
functor on every finite stage.  If finite-dimensional fibers `pi_F` of
`Gamma` and transition unitaries along every substitution generator satisfy

* exact covariance on the fibers;
* exact composition around every gallery relation; and
* exact path independence,

then their direct sum is a genuine finite-dimensional representation of the
group generated by `Gamma` and the substitution sector.  The finite-
dimensional commutant argument then normalizes `pi(Gamma)'` under the whole
substitution group and kills the Kun--Thom/free-lamp witness.  In other words,
making the coefficient gallery completely flat recreates the liftable model.

Trivial `K_2(Z/p^nZ)` only says that there is no *forced scalar central*
gallery holonomy.  The useful exotic model must still carry noncentral
coefficient holonomy.  Its correct quantitative specification is:

* coefficient-gallery curvature supported on a rank fraction `eta_n -> 0`;
* invisible relative-commutant escape on a rank fraction `rho_n`; and
* a strict scale separation, at least `sqrt(eta_n)=o(rho_n)` for the usual
  tensor-spreading argument (or the corresponding pointwise square-root
  operator-norm profile).

The Pauli--flag complement already contributes only
`eta_n=O_p(p^(-4(n-1)))`.  The unresolved task is to choose the adjacent-flag
coefficient intertwiners so that their additional curvature stays on that
...
### The two chambers generate the affine parabolic, but Thom's phase is trivial there

The preceding two-cell problem is naturally an affine-group problem.  Let

`P_0=<U_(O_+),U_(O_*)><SL_4(Z)`.

Both chambers contain the common row-star

`V=<e_01(Z),e_02(Z),e_03(Z)> ~= Z^3`.

Their root groups on the indices `{1,2,3}` are respectively the upper and
lower elementary unipotents.  Together those generate `SL_3(Z)`, and `V` is
normalized by this lower-right block.  Hence

`P_0 = V semidirect SL_3(Z) ~= Z^3 semidirect SL_3(Z)`.

This identifies a tempting source for the missing transition: the second
example in Thom's 2010 paper is built from de Cornulier's block group
`K_0(R)`, which visibly contains an affine parabolic of this form.  The
temptation is false for a precise reason.

**Theorem (Thom-phase sterility on the affine parabolic).**  Let `R` be a
commutative ring and let `K_0(R)` be Thom--de Cornulier's block group with
central coordinate `a_14 in R`.  Let `P(R)<K_0(R)` consist of the matrices
for which only the first row module `a_12 in M_(1,3)(R)` and the first
diagonal `SL_3(R)` block `a_22` may be nontrivial.  Then:

1. `P(R) ~= R^3 semidirect SL_3(R)` and `P(R) intersect Z(K_0(R))=1`;
2. the central extension cocycle
   `alpha:K_1(R) x K_1(R)->R` from Thom's Section 3 restricts to zero on
   `P(R) x P(R)`; and
3. every finite twisted-monomial model used in Thom's second proof, when
   restricted to the finite image of `P(R)`, is diagonally conjugate to a
   direct sum of ordinary permutation regular representations.

**Proof.**  Multiplying two matrices in `P(R)` never creates any of the
other off-diagonal blocks and, in particular, never creates the central
`(1,4)` entry: every possible path contributing to that entry uses one of
the omitted column modules.  Thus setting `a_14=0` is a homomorphic section
of `P(R)` into `K_0(R)`.  This proves the semidirect-product description,
the trivial intersection with the centre, and `alpha|_(P x P)=0`.

For the last assertion, fix a finite reduction and a central character
`chi`.  Thom's operator attached to `p in P` on the twisted group basis has
the form

`U_p delta_h = omega(p,h) delta_(ph)`.

Because the multiplier is trivial on `P x P`, `p |-> U_p` is an honest
representation of `P`.  Left multiplication of `P` on the ambient finite
quotient is free, so every `P`-orbit is a regular `P`-set.  Choose an origin
in each orbit and recursively rephase its basis along the unique left
translate from the origin.  The representation law makes this rephasing
path-independent and removes every `omega(p,h)`.  Hence the restriction is
diagonally conjugate to a sum of copies of `lambda_P`.  End proof.

**Consequence.**  Thom's published phase mechanism cannot be transplanted
directly to the two-chamber transition.  On precisely the affine parabolic
generated by those chambers, its phase is gauge-trivial and gives a
liftable/permutation model.  The nontrivial central phase in `K_0(R)` uses
the complementary dual column modules whose matrix products land in the
central `(1,4)` coordinate.  Therefore any use of Thom's mechanism here must
add an auxiliary dual module (a finite Heisenberg/Schur layer) and couple it
nontrivially to the coefficient compression chains; restriction to
`Z^3 semidirect SL_3` alone cannot solve the transition.
...
### Audit boundary for the square-zero Fourier bridge

The last paragraph is a genuine remaining theorem, not a formal tensor
product.  The one-variable statements above are correct: the Frobenius
pairing is perfect, `I_m^perp=I_(N-m)^vee`, Fourier transform exchanges
translations with multipliers, and the single-chain wrap has rank fraction
`1/N`.  But three independent copies do not by themselves define one
coefficient representation compatible with the complementary `UT_3`
substitutions.

The obstruction is visible on a mixed monomial.  A complementary root sends

`x_i |-> x_i x_j`  for `1<=i!=j<=3`.

If the three variables are sent to independent square-zero summands, the
right side is zero although the source variable is not; if they are sent to
three unrelated tensor factors, the target lies in a mixed tensor degree
which the one-variable annihilator flag did not specify.  Consequently the
claimed contragredient `UT_3` action has not yet been constructed on the
image of the polynomial coefficient ring, and the quantitative curvature
bound cannot yet be attributed to a `Gamma`-microstate.

A valid multivariate bridge must provide a single finite commutative
Frobenius algebra `A_N`, one ring map from the full polynomial ring, and
bulk maps `T_A` for both complementary `UT_3` monoids satisfying

`T_A pi(f) T_A^* ~= pi(alpha_A(f))`

simultaneously for every mixed coefficient `f` in the window.  It must also
retain the reverse-annihilator identity at the common star.  Truncated
multivariate monomial algebras

`F_p[y_1,y_2,y_3]/(y_1^N,y_2^N,y_3^N)`

are natural candidates because they are Frobenius and do not kill mixed
products, but the orthogonal complement of a principal monomial ideal is a
sum of coordinate ideals, not the single reverse principal flag used in the
...
### The auxiliary base variable makes the square-zero jet model global

The chamber-anisotropic iterated-nilpotent candidate is not needed.  The
fixed variable `z` repairs the original square-zero jet model more directly
and avoids the rank-one-half shear obstruction.

Fix

`A_N=F_p[y]/(y^N)`,  `J_N=A_N^4`,  `S_N=A_N semidirect J_N`,  `J_N^2=0`,

with basis `xi_0,...,xi_3`.  For a depth vector
`d=(d_0,...,d_3)` and a common jet offset `h` satisfying
`d_j+h<N`, define
...
`Phi_(d,h)(x_j)=y^(d_j)(1+xi_j y^h)`.

**Theorem (fixed-base global jet covariance).**

1. The image is

   `im(Phi_(d,h))=A_N semidirect`
   ` directSum_j xi_j y^(d_j+h) A_N`.

   Hence increasing one `d_j` by one gives a proper image subring whenever
   `d_j+h<N`.
2. If `C in M_4(N)` has determinant prime to `p` and

   `alpha_C(x_j)=product_k x_k^(C_(kj))`,  `alpha_C(z)=z`,

   let `Chat_C` fix `A_N` and act by

   `Chat_C(xi_j)=sum_k C_(kj)xi_k`.

   Then `Chat_C` is an automorphism of `S_N` and

   `Phi_(d,h) after alpha_C = Chat_C after Phi_(C^T d,h)`

   whenever the displayed depths are below `N`.
3. Under the componentwise Frobenius pairing with `J_N^vee`, the tangent
   ideal has annihilator

   `(directSum_j xi_j y^(d_j+h)A_N)^perp`
   ` =directSum_j xi_j^vee y^(N-d_j-h)A_N`,

   and Fourier transform intertwines `Chat_C` with its contragredient.

**Proof.**  Since `z` maps to `y`, the whole base algebra `A_N` is in the
image.  Subtracting the base term `y^(d_j)` from the image of `x_j` gives
`xi_j y^(d_j+h)`, proving (1).  For (2), square-zero binomial expansion
gives

`Phi_(d,h)(product_k x_k^(C_(kj)))`
` =y^((C^T d)_j)`
`  (1+y^h sum_k C_(kj)xi_k)`,

which is exactly the right-hand side generator by generator.  Invertibility
of `C mod p` makes `Chat_C` an automorphism.  The last assertion is the
one-variable Frobenius annihilator identity in each component.  End proof.

This formula resolves both sides of the earlier audit simultaneously.
Mixed polynomial coefficients do not vanish: for example

`Phi_(d,h)(x_i x_j)`
` =y^(d_i+d_j)(1+y^h(xi_i+xi_j))`.

At the same time, the depth-dependent base term is no longer an overlap
obstruction, because it lies in the **common** base algebra generated by the
auxiliary variable `z`; the Fourier transition acts only on the tangent
ideal and fixes that base.  Unlike the iterated-nilpotent shear, `Chat_C`
has full rank on the tangent module for every unimodular chamber matrix, so
there is no exponential amplification of a coefficient-space kernel.

For a fixed compressor `t=I+E_(ji)`, take `d_j=1`.  Its action sends
`d_i|->d_i+1`, giving a strict chain of length `N-h-d_i` with one wrap and
macroscopic witness.  Complementary `UT_3` transformations act by
`d|->C^T d`; the micro--macro carry cocycle controls these integer depths,
while the exact `Chat_C` action controls the full four-dimensional tangent
module.  Componentwise annihilator reversal supplies the opposite chamber.

### Use `GL_4(Z)` so every Weyl-chart permutation is polynomial

There is also a harmless ambient correction.  Odd coordinate permutations
do not have polynomial-preserving representatives in `SL_4(Z)` without a
sign inversion.  Replace the substitution factor by `GL_4(Z)` and put

`G_z^GL=EL_r(R) semidirect GL_4(Z)`.

This is an index-two overgroup of `G_z`, so it remains residually finite and
Kazhdan; the same explicit compression-semigroup proof makes
`Gamma_z<G_z^GL` infranormal and nonnormal.  Every one of the twenty-four
total-order charts is now obtained by an honest permutation matrix, acting
on `S_N` by the corresponding permutation of the `xi_j` and of the depth
coordinates.  These permutation transports preserve the common base,
Frobenius pairing, and strict-chain profile exactly.

Consequently the corrected adjacent-wall target returns to the exact
square-zero Fourier--Weil setting.  There are only three asymptotic losses:
the macro chamber boundary, the one depth-chain wrap, and the discarded
Pauli--flag complement.  The next audit is to spell out the generator maps
for `G_z^GL` and check that the partial Fourier wall operator fixes the
...
### Relative regular absorption constructs the missing adjacent wall

The full Fourier transform was too large because it moved the five common
root operators.  The correct Fourier transform lives in the multiplicity
commutant **after** the two chart models have been identified on their
common nilpotent face.

Let `C,D` be adjacent total-order chambers.  Their maximal unipotent groups
share the subgroup `U_(C,D)` generated by the five roots whose orientations
do not change across the wall.  This is a finitely generated nilpotent
group.  Choose a weighted Følner box `F_M` in `U_(C,D)`.

At one face vertex, let `pi^+` and `pi^-` be the two genuine finite
fixed-base representations of `Gamma_z`.  They factor through finite
quotients.  Let

`K=Gamma_z/(ker(pi^+) intersect ker(pi^-))`.

Both become finite-dimensional representations of the same finite group
`K`.  Tensor each with the left regular representation `lambda_K`.  The
regular absorption identity gives

`pi^+ tensor lambda_K ~= dim(pi^+) lambda_K`,

`pi^- tensor lambda_K ~= dim(pi^-) lambda_K`.

After scalar amplification their restrictions are exactly unitarily
equivalent.  In the common model they have the form

`lambda_K tensor 1_M`.

Its commutant contains `1 tensor B(M)`.  Put the desired square-zero
kernel Fourier transform, together with the Pauli/Weil phase, in this
multiplicity algebra.  Thus choose an origin intertwiner `J_1` which
intertwines all of `Gamma_z` but is not forced to be the sterile canonical
regular-absorption intertwiner.

For `x in F_M`, choose a word in the five common root generators from `1`
to `x` and propagate

`J_x=U_x^- J_1 (U_x^+)^*`.

**Theorem (relative adjacent-wall intertwiner).**  On every path contained
in the interior of `F_M`:

1. `J_x pi_x^+(gamma)J_x^*=pi_x^-(gamma)` for every
   `gamma in Gamma_z`;
2. for each of the five common root generators `s`,

   `J_(sx) U_s^+=U_s^- J_x`;

3. `J_x` is independent of the chosen interior word from `1` to `x`; and
4. a strict commutant witness and its Fourier polarization are transported
   with unchanged commutator norm.

The exceptional rank fraction is `O(1/M)`.

**Proof.**  Item (1) holds at the origin by regular absorption.  If it holds
at `x`, exact coefficient covariance along a common-root edge gives it at
`sx`; the definition gives item (2).  Two interior paths differ by
same-root, commuting-root, and `A_2` relations inside the common nilpotent
face.  Both native chart models satisfy those relations exactly there, so
the two propagated intertwiners agree.  This proves (3).  Multiplication of
`J_1` by a unitary in the multiplicity commutant does not change (1), and
unitary propagation preserves every witness norm, proving (4).  Only paths
meeting the Følner boundary use the arbitrary completions; the weighted
nilpotent boundary estimate is `O(1/M)`.  End proof.

This theorem supplies exactly the partial isometry required by the native
quantum-atlas construction: it intertwines `Gamma_z` and **fixes all five
common external roots**, while the relative multiplicity factor carries
the Fourier transition associated with the sixth, flipped root.  Full
kernel Fourier is used only inside that multiplicity factor, so its
contragredient action never moves a common root operator.

Regular absorption at one wall does not make the global construction
sterile.  It identifies only the common-face representation.  The
compressor-chain endpoint and the relative Fourier choice remain in the
multiplicity commutant, and the propagated model still has its
`O(1/M)` boundary.  The classical property-`(T)` no-go is also avoided:
the wall identifications act in nonorthogonal multiplicity fibers, not by
permuting a common depth set.

The finite atlas now has the local wall theorem previously missing.  The
next adversarial check is whether propagating `J_1` around a closed loop in
the **full chamber atlas** returns the same multiplicity Fourier operator,
or leaves a noncentral holonomy not removed by the free Pauli groupoid
...
### Origin absorption is nonsterile and every compressor wrap stays on the boundary

There are two separate points: regular absorption at one origin does not
erase the strict operator, and propagation from that origin never forces a
regular absorption at the other depths.

The first point is visible in explicit coordinates.  For a representation
`pi:K_0->U(V)`, define

`T_pi:V tensor ell^2(K_0)->ell^2(K_0) tensor V`

by

`T_pi(v tensor delta_h)=delta_h tensor pi(h^(-1))v`.

Then

`T_pi (pi(k) tensor lambda(k)) T_pi^*=lambda(k) tensor 1`.

If `z in pi(K_0)'`, then

`T_pi(z tensor 1)T_pi^*=1 tensor z`.

For an arbitrary operator `b in B(V)`, however,

`T_pi(b tensor 1)T_pi^*(delta_h tensor v)`
` =delta_h tensor pi(h^(-1))b pi(h)v`.

Thus the absorbed ambient operator is an operator-valued multiplicity
cocycle, not an operator which ignores the multiplicity factor.  Since `z`
commutes with every `pi(h)`, each fiber commutator is a unitary conjugate of
`[z,b]`, and therefore

`||[T_pi(z tensor 1)T_pi^*,T_pi(b tensor 1)T_pi^*]||_2`
` =||[z,b]||_2`.

This is the exact nonsterility statement missing from the informal
regular-absorption discussion.  Tensoring by a regular representation and
then changing coordinates relocates the strict interaction into the
multiplicity cocycle; it does not destroy it.

The wrap issue is purely combinatorial.  Before completing the compressor
shift cyclically, delete the closing edge

`L-1 -> 0`

from every length-`L` depth chain.  Also delete the boundary edges of the
common-face Følner box.  On the remaining good graph, choose one absorbed
origin in each component and define all other wall maps only by native
common-face propagation

`J_x=U_x^- J_0 (U_x^+)^*`.

Every cycle contained in this good graph is generated by the exact
same-root, commuting-root, and `A_2` face relations, so propagation is
path-independent there.  A path which returns by the cyclic compressor
completion necessarily uses the deleted closing edge.  Hence it is not an
interior holonomy relation and cannot imply that a later depth
representation factors through `K_0`.

More quantitatively, for a fixed word `w`, a defect can occur only when a
prefix of `w` meets a deleted edge.  The prefix-union estimate therefore
gives bad density

`O(|w|/L)+O(|w|/M)`.

For each fixed word this tends to zero with the already scheduled depth and
face parameters.  Arbitrary cyclic completion is performed only after the
wall maps have been propagated on the good graph.  This proves that origin-
only absorption preserves the macroscopic strict chain and that compressor
wrap holonomy costs no new error term.

One local equation is still not supplied automatically by this argument.
For the five common face roots, the propagation definition proves the
required wall covariance.  For the sixth, flipped root with native chart
operators `b_C,b_D`, the absorbed origin models must additionally satisfy

`(1 tensor mathcalF) T_C(b_C tensor 1)T_C^*`
` =T_D(b_D tensor 1)T_D^*(1 tensor mathcalF)`

on their common-good sector (with source and target absorption maps inserted
when the root changes depth).  Regular absorption alone does not imply this
operator-valued cocycle equation.  It must follow from the native square-
zero primal/dual Fourier polarization.  Thus the sterility and cyclic-wrap
audits pass, while explicit flipped-root covariance is the next remaining
local audit before the fusion assembly can again be treated as complete.
...
### Conjugate tensoring kills the residual scalar `K_2`/Weil holonomy

The relation families above first present the relevant Steinberg group.  A
word which is trivial in `EL_r(R)` may lift to a central element of
`K_2(R)`, so it is not legitimate simply to omit the kernel.  In the
polarized finite model this remaining central element acts, on each matched
irreducible Heisenberg/Weil block, by a scalar.  Scalar holonomy has a
dimension-free removal which preserves the strict witness.

**Lemma (conjugate cancellation).**  Suppose finite-stage unitaries `u(g)`
satisfy, on their common-good corner,

`u(g)u(h)=c(g,h)u(gh)+e(g,h)`,

where `c(g,h)` is circle-valued (or blockwise scalar on a fixed central
decomposition) and `||e(g,h)||_2<=epsilon`.  On the matched blockwise tensor
product define
...
`||uhat(g)uhat(h)-uhat(gh)||_2<=2 epsilon+epsilon^2`,

up to the same bad-corner error, because

`c(g,h) conjugate(c(g,h))=1`.

If `v` approximately centralizes the `Gamma_z` operators and has strict
commutator norm `kappa` with one compressed conjugate, then

`vhat=v tensor 1`

has the same centralizing defect and exactly the same strict commutator
norm in the conjugate-tensored model.

**Proof.**  Expand the tensor product of the two defective products and
insert the exact scalar cancellation.  The two linear error terms have
normalized Hilbert--Schmidt norm at most `epsilon`, since tensoring with a
unitary is an isometry, and the quadratic term has norm at most
`epsilon^2`.  The commutator assertions are

`[v tensor 1,a tensor conjugate(a)]=[v,a] tensor conjugate(a)`.

End proof.

Consequently a scalar Steinberg kernel or a projective Weil multiplier is
not an obstruction to the counterexample construction.  One may either
contract it on the free Pauli groupoid as above or cancel it by this
conjugate tensor, with no loss of escape.  The conjugate tensor is safer for
the algebraic audit because it does not require choosing a global scalar
`1`-cochain.

The exact remaining presentation claim can therefore be weakened to:

> after imposing the local same-root, commuting-root, and `A_2` relations,
> every word trivial in the fixed-base elementary semidirect group has, on
> the common-good fusion corner, only central blockwise-scalar holonomy plus
> the already estimated non-scalar overlap curvature.

The central part follows from the Steinberg central extension once all root
occurrences have been identified.  The non-scalar part is precisely what
the native-relator transfer lemma bounds.  A final audit must check that the
chosen Heisenberg decomposition is matched blockwise so that the central
kernel is scalar on each tensor-paired summand; no assertion that
`K_2(R)` itself vanishes is needed.
...
### Use auxiliary `GL_4` charts but keep the active ambient group `SL_4`

The opposite-root target simplifies if the earlier index-two enlargement is
not made part of the group.  Return to

`G_z=EL_r(R) semidirect SL_4(Z)`.

The twenty-four coordinate permutations remain legitimate **auxiliary
chart gauges**: each is an automorphism of the fixed-base jet ring and a
permutation of the common finite regular space.  A passive chart gauge need
not belong to the group being approximated.  Hence no pure odd coordinate
transposition needs a matrix in the active microstate.

The active substitution group is generated by the unit transvections
`x_(ij)(1)` and their inverses.  For an opposite pair define, rather than
postulate,

`W_(ij)=X_(ij) X_(ji)^* X_(ij)`.

This is the finite-stage operator assigned to the signed Weyl word

`w_(ij)=x_(ij)(1)x_(ji)(-1)x_(ij)(1)`.

**Lemma (active signed-Weyl cell).**  On the intersection of the three
native prefix-good projections, `W_(ij)` implements the exact fixed-base
jet covariance of the signed exponent matrix

`w_(ij)=[[0,1],[-1,0]]`

on the `i,j` plane.  Its conjugation sends every root operator occurring in
that good rank-two/rank-three cell to the root prescribed by the actual
matrix conjugation.  The only error is the union of the three native reset,
macro, face, and Pauli boundaries.

**Proof.**  Apply the exact jet covariance successively to the three root
letters.  Away from their prefix boundaries the accumulated depth update,
tangent automorphism, base action, and full regular coefficient action are
the ones for the product of the three integer matrices.  Direct
`2 by 2` multiplication gives the displayed signed Weyl matrix.  Conjugate
a further root letter through the same good product and use functoriality of
the genuine ring maps.  The prefix-union bound gives the stated support.
End proof.

The finite Fourier--Weil signed-Weyl operator is still useful as a passive
polarization formula, but equality with it need only hold projectively.  On
each irreducible Heisenberg block, the product `W_(ij)` and the Weil lift of
the same symplectic signed-Weyl map implement the same automorphism, hence
differ by a scalar by finite Stone--von Neumann.  The conjugate-cancellation
lemma removes that scalar without changing the strict commutator.

There is no separate square relation to assume.  Define the active diagonal
sign word by

`D_(ij)=W_(ij)^2`.

Then `D_(ij)` implements `diag(-1,-1)` on the plane on the six-prefix good
set, and all of its conjugation relations follow from the same word
calculation.  Coordinate conjugacy by **even signed Weyl words**, rather
than by passive odd chart swaps, supplies every opposite-root instance in
`SL_4(Z)`.

This closes the missing owner cell without confusing passive and active
operators.  The fixed-base pair with the `SL_4` factor already satisfies
the Kun--Thom hypotheses, so the index-two `GL_4` extension and its active
permutation relations can be discarded from the final candidate.  The
next presentation audit is reduced to the standard Steinberg-to-elementary
central kernel, which is blockwise scalar in the polarized model and is
handled by conjugate cancellation as above.
...
### Pulling back finitely many cell corners proves the native word bound

The remaining invariance condition on native bad projections does not
require any new equivariant block set.  It is a finite noncommutative
intersection argument.

Fix one finite derivation diagram for a relation word `w=1`.  Enumerate its
cells by `e=1,...,m`.  In the source fiber of cell `e`, let `p_e` be a good
projection on which that native, active-Weyl, covariance, or passive-wall
cell is exact, with

`tr(1-p_e)<=epsilon_n`.

Let `A_e` be the unitary product along any fixed path from the diagram base
vertex to the source of cell `e`.  Pull the condition back to the base fiber
and put

`r_w=meet_(e=1)^m A_e^* p_e A_e`.

The projections need not commute.  Finite-dimensional codimension
subadditivity gives

`tr(1-r_w)<=sum_e tr(1-p_e)<=m epsilon_n`.

Every rewrite cell in the chosen derivation is exact on `r_w`, because a
vector satisfying the pulled-back condition reaches `p_e` after its prefix.
Applying the cells in the finite derivation therefore shows that the two
boundary word operators agree on `r_w`.  Since a difference of unitaries
has operator norm at most two,

`||U_n(w)-1||_2<=2 sqrt(m epsilon_n)`.

Here `m` depends on the fixed word and its chosen derivation but not on the
matrix stage.  Thus the defect tends to zero.  No probability measure on a
global depth orbit is produced: the `A_e` move between chart Hilbert fibers,
and `r_w` is an intersection of noncommuting operator corners.

This supplies the cell-union bound required by both fixed-word fusion
factorizations above.  The algebraic coverage can be organized by normal
form rather than by a finite presentation:

* coefficient letters are multiplied inside the genuine finite
  `EL_r(S_N)` representations, so coefficient `K_2(R)` creates no local
  projective defect;
* external transvection letters use the Steinberg cells, with any residual
  central external kernel handled only after the explicit matched-sector
  audit above;
* coordinate permutations use the active exact permutation cells; and
* moving an external letter past a coefficient letter uses the fixed-base
  semidirect covariance cell.

Any fixed ambient word has a finite sequence of these moves to semidirect
normal form.  If the word is the identity, its coefficient component is an
exact coefficient identity and its external component is an external
Steinberg-kernel word.  Hence all **noncentral** native curvature is now
bounded by the displayed pulled-back-corner estimate.  The only remaining
algebraic issue is the explicitly isolated external central-kernel
cancellation and preservation of strict mass on its matched summands.
...
### The coefficient `K_2` kernel is generated by base words and toric chart loops

Choose the elementary rank in a stable range for

`R=Z[z,x_0^(+-1),...,x_3^(+-1)]`.

Since this regular ring has Krull dimension six, taking `r>=9` is a safe
choice for the standard `K_2(r,R)=K_2(R)` stability input.  At the finite
jet prime also require `p` not to divide `r` (for example `r=9,p=11`), so
the trace-Frobenius form used on the square-zero elementary kernel remains
nondegenerate.

Put `A=Z[z]`.  Homotopy invariance and the iterated fundamental theorem for
regular Laurent extensions give, noncanonically but with the standard
symbol generators,

`K_2(R)=K_2(A)`
` directSum (directSum_i K_1(A)*[x_i])`
` directSum (directSum_(i<j) K_0(A)*[x_i] wedge [x_j])`.

Here `K_2(A)=K_2(Z)`, `K_1(A)={+-1}`, and `K_0(A)=Z`.  Thus the new Laurent
kernel is generated by the torsion symbols `{-1,x_i}` and the free toric
symbols `{x_i,x_j}`, together with words already over the positive base.

In the coefficient atlas these three families have distinct, controlled
realizations.

1. A positive-base `K_2(A)` word is already an identity in every genuine
   local `EL_r(S_N)` representation, so its coefficient operator is one.
2. `{-1,x_i}` is the signed one-coordinate inversion loop assembled from
   the active signed-Weyl cells.
...
common-good fusion corner.  Prefixes meeting a native boundary contribute
only the already counted `O_w(sqrt(eta_n))` error.  Therefore the stable
Steinberg kernel acts trivially in the **coefficient** ultraproduct model.

Any residual finite-stage phase in these loops comes solely from the
separate irreducible Pauli/Weil factor.  It is circle-valued and is removed
either by the free-good-groupoid cochain or by conjugate cancellation.  This
verifies the weakened presentation claim for the stable-rank fixed-base
pair: after native-relator transfer, no non-scalar `K_2(R)` holonomy remains.
...
### August 2026 status and the exact published sofic endpoint

The surrounding open-problem status changed during this audit.  OpenAI's
first nonsofic group was announced in early August 2026, and Kun--Thom's
follow-up

`https://arxiv.org/abs/2608.06222`

now proves that the generalized wreath product associated with every
infranormal nonnormal Kazhdan pair is nonsofic.  Thus this project must no
longer claim to prove the first nonsofic group or to answer whether all
groups are sofic.  Its still-distinct target is to prove that the inclusion

`sofic groups subset hyperlinear groups`

is proper by constructing a **hyperlinear** nonsofic group.  The current
literature search found no published assertion that the new generalized
wreath examples are hyperlinear; the hyperlinearity of a generalized wreath
product requires a soficity hypothesis on the underlying action in the
standard closure theorem, while Kun--Thom prove that this coset action is
nonsofic.

The same paper does, however, verify exactly the endpoint used here.  Its
Section 4 proves that for a sofic representation

`sigma:G->S_U`

of an infranormal Kazhdan pair, the full universal-sofic centralizer

`C_(S_U)(sigma(Gamma))`

is normalized by `sigma(G)`.  This is stronger than merely saying that the
internal centralizer `C_G(Gamma)` is normal.  Therefore, if the constructed

`Q=<rho(G_z),v>`

were sofic, an injective sofic embedding of `Q` would send `v` into the
universal centralizer of the faithful copy of `Gamma_z`; normalization by
`rho(t)` would force `v` to commute with

`rho(t)^*rho(gamma)rho(t)=rho(t^(-1) gamma t)`,

contradicting the strict ultraproduct commutator.  This validates the final
image-quotient argument with the current published theorem.

Kun--Thom Theorem E also independently supplies explicit residually finite
Kazhdan infranormal pairs over polynomial/Laurent rings.  Our fixed-base
variant keeps one additional non-inverted variable for the jet functor, so
its structural pair check is still required, but it is a mild variant of
their published construction rather than an unsupported endpoint theorem.
...
### Explicit structural audit of the final fixed-base Kun--Thom pair
...
with `r>=9`.  The `SL_4` action fixes `z` and acts by monomial substitution
on the four Laurent variables.

**Kazhdan property.**  `R_+` and `R` are finitely generated commutative
rings.  The universal-lattice theorem gives property `(T)` for their
elementary groups in this rank, and `SL_4(Z)` is Kazhdan.  Property `(T)` is
closed under extensions, so both `Gamma_z` and `G_z` are Kazhdan.

**Residual finiteness.**  A nonidentity substitution matrix is separated by
reduction `SL_4(Z)->SL_4(Z/mZ)`.  For a nonidentity coefficient matrix,
some nonzero Laurent polynomial entry separates after evaluation in a
finite field extension: choose nonzero values for the `x_i` and a value for
`z` at which it does not vanish.  To make one finite quotient of the full
semidirect product, take the kernel of the combined finite orbit of that
evaluation under the finite image of the finitely many substitution
matrices, equivalently use an invariant finite quotient generated by the
evaluation maps.  More concretely one may quotient by

`(p, z^(p^a)-z, x_0^N-1,...,x_3^N-1)`

after choosing `a,N` large enough; the Laurent part is invariant under every
integer monomial substitution, and `z` is fixed.  Thus `G_z` is residually
finite.

**Compression semigroup.**  Every positive unit transvection

`t_(ji)=1+E_(ji)`

has a nonnegative exponent matrix and hence maps `R_+` injectively into
itself.  Therefore

`t_(ji) Gamma_z t_(ji)^(-1)<=Gamma_z`.

The subgroup generated by the positive unit transvections is all of
`SL_4(Z)` (their group inverses are available after taking the generated
subgroup).  Also `Gamma_z` itself normalizes `Gamma_z`.  Hence the subgroup
generated by the compression semigroup contains `Gamma_z` and the full
substitution factor.  Conjugating `Gamma_z` by that generated subgroup
produces elementary matrices with every Laurent monomial coefficient: for
`v in Z^4`, write `v=a w` with `a>=0` and `w` primitive, extend `w` to an
integral unimodular basis, and transport the positive monomial `x_0^a` to
`x^v`.  Powers of fixed `z` remain available.  Root additivity builds every
Laurent polynomial coefficient.  Thus the compression semigroup generates
all of `G_z`.

**Strictness.**  For a positive shear with

`alpha_t(x_i)=x_i x_j`,

the image `alpha_t(R_+)` is proper: `x_i` would require the inverse of
`x_j`.  Hence `t Gamma_z t^(-1)<Gamma_z`.  The single fixed element

`gamma=e_(ab)(x_i)`

lies outside the compressed subgroup and is exactly the element used by
the square-zero depth-chain witness.

These are precisely the hypotheses of Kun--Thom Theorem 4.1/Theorem A:
both groups are Kazhdan, the compression semigroup generates the ambient
group, and the inclusion is nonnormal.  The auxiliary odd coordinate
permutations used as chart gauges never enter this group-theoretic input.
...
### Kernel-compatible regular-quotient witnesses reduce to one membership test

There is a finite-dimensional way to impose the missing commutation from the
outset.  It is more general than the group-algebra application needed here.

**Lemma (enlarged-commutant energy).**  Let `N subset M_d(C)` be a unital
star-subalgebra and let `x in U(d)`.  If `u` is Haar in `U(N')`, then

`E ||[u,x]||_2^2=2(1-||E_N(x)||_2^2)`.

Consequently there is a unitary `u in N'` with at least the displayed
squared commutator energy.

**Proof.**  Haar averaging over `U(N')` is the trace-preserving conditional
expectation onto `N`.  Expanding the commutator gives

`||[u,x]||_2^2=2-2 Re tau(u x u^* x^*)`.

After averaging, the last trace is

`tau(E_N(x)x^*)=||E_N(x)||_2^2`.

End proof.

For a finite group `H`, a subgroup `K`, and a subgroup `Z` normalized by
`K`, apply the lemma in the left regular representation with

`N=lambda(<K,Z>)''`.

If `g notin <K,Z>`, the group unitaries are orthonormal and
...
which commutes simultaneously with the next-depth subgroup and the kernel
holonomy, while

`||[u,lambda(g)]||_2^2>=2`.

This turns the second matched-descent target into an explicit native test.
At depth `m`, let `K=pi_(m+1)(Gamma_z)` and let `g=pi_m(gamma)` be the fixed
strict coefficient.  If the finite-stage toric holonomy is represented by
a subgroup `Z_m` satisfying

`g notin <K,Z_m>`,

choose the native witness from

`lambda(<K,Z_m>)'`

instead of merely `lambda(K)'`.  It then commutes with the kernel grading by
construction and retains the same order-one energy.  The required
nonmembership is automatic if `Z_m` lives in a separated phase factor.  In
a coefficient realization it remains to show that multiplying by `Z_m`
cannot create the missing tangent term

`xi_i y^(m+h) mod xi_i y^(m+h+1)`.

Thus the kernel repair now has two finite algebraic deliverables with no
hidden analytic step:

1. realize the toric holonomy through an equivariant finite abelian grading;
2. verify `pi_m(gamma) notin <pi_(m+1)(Gamma_z),Z_m>`.

Once these hold, balanced matched-character descent and the enlarged-
commutant lemma preserve both multiplicativity and the strict witness.

### The external `K_2(Z)` kernel is avoidable, not a descent problem

The final construction need not use `St_4(Z)` at all.  Its substitution
operators are indexed by actual matrices in `SL_4(Z)` and by their exact
finite congruence actions on the Pauli--flag and macro-depth data.  Define

`Gtilde_z=St_r(R) semidirect SL_4(Z)`,

using functoriality of `St_r(R)` for the honest action.  Then

`Gtilde_z -> EL_r(R) semidirect SL_4(Z)=G_z`

has kernel only `K_2(R)`.  An external relation word is checked as a word in
the actual substitution matrices; there is no need to derive it in
`St_4(Z)` and hence no external `K_2(Z)` element to kill.  The signed-Weyl
word is simply one chosen word for its genuine matrix in `SL_4(Z)`, and its
fourth power is an ordinary external group relation.

The Pauli/Weil operator attached to this actual `SL_4` action may still be
projective.  That multiplier is circle-valued on the free good groupoid and
is removed by the already explicit groupoid `1`-cochain (or by scalar
conjugate cancellation).  It should not be conflated with descent across an
unnecessarily introduced `St_4(Z)` kernel.

This removes one half of the reopened kernel audit.  The sole algebraic
...
### Audit correction: the chamber cover misses antipodal coefficient pairs

Keeping the Steinberg kernels removes every **descent** obligation, but it
does not by itself prove that the existing atlas models the two Steinberg
groups.  The phrase "the two-chamber cover contains every Steinberg
relation" previously referred only to the orientation of the matrix roots.
There is a separate Laurent-coefficient obstruction.

**Lemma (positive-chart criterion for two monomials).**  Let `d>=3` and
let `v,w in Z^d`.  If `v,w` do not lie on opposite rational rays, then there
is `A in SL_d(Z)` such that

`Av,Aw in N^d`.

If `w=-qv` for some positive rational `q`, no such `A` exists.

**Proof.**  In the first case the rational cone generated by `v,w` is
pointed, so its dual has a rational interior point.  After clearing
denominators and dividing by the gcd, choose a primitive integral covector
`ell` which is positive on every nonzero member of `{v,w}`.  Extend `ell`
to an integral dual basis `ell,b_2,...,b_d`.  Replacing each `b_i` by
`b_i+N_i ell`, for sufficiently large integers `N_i`, makes all its values
on `v,w` nonnegative without changing the determinant.  If necessary swap
two of the last `d-1` rows to obtain determinant one.  These rows form the
required matrix.  The converse follows from
`A(-qv)=-qAv`: two nonzero opposite vectors cannot both lie in `N^d`.
End proof.

Consequently a coefficient `A_2` relation

`[x_(ij)(x^v),x_(jk)(x^w)]=x_(ik)(x^(v+w))`

has a native positive owner whenever the two exponent vectors are not
antipodal.  The exceptional family is already visible at

`[x_(ij)(x^v),x_(jk)(x^(-kv))]=x_(ik)(x^((1-k)v))`.

There are equally basic missing commutations, for example

`[x_alpha(x^v),x_alpha(x^(-v))]=1`.

No total-order chart or unimodular change of exponent coordinates contains
both coefficients in its positive polynomial subring.  The earlier
minimal-chamber enumeration only covered root-index pairs and therefore
does not prove these relations.  Keeping `K_2` is irrelevant to this gap:
these are defining Steinberg relations, not quotient relations.

Thus the Steinberg image-pair reroute is presently conditional on one
additional native construction:

> **antipodal-coefficient cell:** transfer same-root, commuting-root, and
> `A_2` relations whose Laurent supports contain an opposite pair, while
> retaining the strict relative-commutant witness under fusion.

It is enough to solve the monomial cases, since arbitrary Laurent
polynomials can then be expanded using root additivity and the usual
centrality of the `A_2` output root in its rank-two unipotent subgroup.  A
finite Laurent-quotient cell realizes these relations exactly but is
sterile by itself; coupling that cell to the nonliftable jet chart without
forcing total absorption is precisely the unresolved operator step.  Until
...
tripods.  In a maximal unipotent group it is one orbitwise automorphism of a
single nilpotent complement.

Fix a total order on the matrix indices and let `U^+` be the corresponding
positive Steinberg unipotent subgroup.  Let `beta` be a simple root and let
`N_beta` be the subgroup generated by all positive root groups except
`X_beta`.

**Lemma (simple-root semidirect decomposition).**

`U^+=N_beta semidirect X_beta`.

In particular `X_beta` normalizes `N_beta`, and its conjugation formulas on
`N_beta` are exactly all same-root, commuting-root, and `A_2` formulas in
the chamber which involve `beta`.

**Proof.**  Positive roots other than the simple root `beta` are closed
under addition whenever the sum is a root: a simple root cannot be a sum of
two positive roots.  Hence their root groups generate a subgroup.  Adding
`beta` to another positive root, when possible, produces a positive root
different from `beta`, so the Chevalley commutator formula shows that
`X_beta` normalizes this subgroup.  Collection in the fixed positive-root
order gives the semidirect decomposition.  End proof.

Now prescribe an arbitrary Laurent exponent `b in Z^4` for `beta`.  Choose
row weights `d_1,...,d_r in Z^4` such that

`d_beta=b`

and `d_alpha in N^4` for every other positive root `alpha`.  This is always
possible after making `beta` simple.  Coordinate by coordinate, take all
adjacent gaps other than the prescribed `b` nonnegative and large enough
that every longer interval crossing the `beta` edge has nonnegative total;
the only interval allowed to remain signed is the single edge `beta`.

The row-torus automorphism `delta_d` therefore sends

`N_beta(R_+)` into the positive polynomial Steinberg group,

while

`delta_d(x_beta(1))=x_beta(x^b)`.

Conjugation by the latter element induces an automorphism `theta_b` of the
positive shifted complement `delta_d(N_beta(R_+))`.  Its inverse is the same
formula with parameter `-x^b`; all output coefficients remain in the shifted
positive complement because the row-weight differences telescope in every
`A_2` relation.

For a fixed finite word window, choose the fixed-base coefficient truncation
past every polynomial occurring in these conjugation formulas and their
inverses.  The image `L_(b,N)` of the shifted complement is then a finite
nilpotent subgroup of the jet group `H_N`, and `theta_b` descends to an
automorphism of that image.  Apply the orbitwise-normalizer construction to
`L_(b,N)<=H_N`.  The resulting permutation `P_(theta_b)` represents the
single missing root `x_beta(x^b)` and simultaneously satisfies **every**
tested chamber relation involving it.  There is one transversal and one
operator, so the marginal-tripod holonomy is absent.

The right-regular expectation calculation applies unchanged.  Whenever the
conjugation action is nontrivial, some right translation centralizes the
entire positive jet image and has order-one commutator with
`P_(theta_b)`; diagonal ambient absorption then places that witness in the
protected second factor.

Finite matrix-index induction makes any chosen root simple and propagates
this joint cell to all root indices.  The residual gap is purely between
...
### The prime-field complement supplies the common owner copy for monomial cells

For the monomial relation cells actually needed after root additivity, the
common-copy hypothesis in item (1) has a canonical solution.  No large
finite Laurent quotient is required.

Fix an odd prime `p` and put

`L_0=N_beta(F_p)`,

the finite simple-root complement in the constant-coefficient maximal
unipotent group.  Its automorphism is

`theta=Ad(x_beta(1))|L_0`.

Consider one monomial `A_2` cell.  After a coefficient `SL_4(Z)` chart
change, choose the input/output pair which lies on one positive ray.  Choose
row weights with

`d_beta=b`,  `d_alpha=a`,  `d_(alpha+beta)=a+b`,

where `a` is the native input exponent and `b` is the missing-root exponent;
make every other positive-root difference except `beta` nonnegative by the
large-gap construction above.  At a fixed-base stage where all displayed
monomials survive, define on root generators

`iota_d(x_gamma(c))`
` =x_gamma(c Phi_(depth,h)(x^(d_gamma)))`,  `c in F_p`.

**Lemma (prime-field shifted complement).**  This extends to an injective
homomorphism
...
`iota_d theta iota_d^(-1)`

on its image is exactly the positive-coordinate automorphism prescribed by
conjugation with the missing formal root `x_beta(x^b)`.

**Proof.**  In every `A_2` tensor the row differences telescope:

`d_gamma+d_delta=d_(gamma+delta)`.

Since `Phi` is a ring homomorphism on the positive monomials, the shifted
coefficients therefore satisfy exactly the same constant-field Steinberg
relations as `L_0`.  For injectivity, use collected unitriangular matrix
coordinates.  The coordinate at a positive root `gamma` is the corresponding
`F_p` coordinate multiplied by the one nonzero monomial
`Phi(x^(d_gamma))`; all terms contributing to that matrix coordinate carry
the same telescoping monomial.  A nonzero scalar in the `F_p`-algebra cannot
kill that surviving monomial, so the first nonzero collected coordinate
remains nonzero.  The conjugation assertion is the same `A_2` calculation
with `d_beta=b`.  End proof.

Thus every monomial owner contains an isomorphic copy of the **same** finite
group `L_0` with the same automorphism `theta`.  The selective row-owner
intertwiner applies exactly, and the orbitwise missing-root operator is one
canonical operator transported among the owners.  Arbitrary Laurent
polynomials are reduced to these cells by the already audited canonical
same-root product of their finitely many integer-monomial summands; reduction
modulo `p` handles the integer scalar on each finite stage.

This discharges item (1) of the preceding section for the active monomial
atlas.  It remains essential that `L_0` is only the nilpotent relator
incidence, not a common finite quotient of the full positive group.  The
terminal analytic statement is item (2): realize the selective `L_0`
incidences as sparse nonorthogonal fusion overlaps whose Gram errors vanish,
...
sentence asserting that its automorphism descends to `L_(b,N)<=H_N` after
taking `N` past the tested coefficients is false.  The issue is invariance of
the **entire kernel** of the finite coefficient map, not correctness on a
fixed list of tested elements.

The rank-three `A_2` cell is already decisive.  Work over `F_p[x]`, take
`beta=(1,2)`, and choose row weights

`d_12=-1`,  `d_23=1`,  `d_13=0`.

The shifted complement is generated by

`x_23(x f)` and `x_13(g)`,  `f,g in F_p[x]`.

Conjugation by the missing root `x_12(x^(-1))` is a genuine automorphism of
this infinite polynomial complement and has the formula

`x_23(x f) |-> x_23(x f) x_13(f)`,

`x_13(g) |-> x_13(g)`

(up to the harmless Steinberg sign convention).  Now use the fixed-base
truncation `x |-> y` in `F_p[y]/(y^N)`.  The element

`x_23(x^N)=x_23(x x^(N-1))`

lies in the kernel, whereas its conjugate has the additional factor

`x_13(x^(N-1))`,

whose image `x_13(y^(N-1))` is nontrivial.  Hence the kernel is not invariant
and there is no induced automorphism of the finite image `L_(b,N)`.  Making
...
### A noncommutative Kun--Thom pair from positive words in a free group

There is a structurally valid alternative to the commutative
polynomial/Laurent pair which removes commutative monomial collisions from
the coefficient algebra.

Fix `d>=5`, `r>=4`, and a finite field `F_q`.  Let

`A_+=F_q<x_1,...,x_d>`

be the free associative algebra, viewed as the positive-word subalgebra of

`A=F_q[F_d]`,

the group algebra of the rank-`d` free group.  Put

`Lambda=SAut(F_d)`,

acting on `A` through its action on `F_d`, and define

`G=St_r(A) semidirect Lambda`,

`Gamma=im(St_r(A_+)->St_r(A))`.

**Theorem (free-word image pair).**  Both `G` and `Gamma` have property
`(T)`, the compression semigroup `P_Gamma` generates `G`, and `Gamma` is not
normal in `G`.

**Proof.**  Ershov--Jaikin-Zapirain prove property `(T)` for `St_r(B)` (and
`EL_r(B)`) when `r>=3` and `B` is any finitely generated associative ring.
Thus `St_r(A_+)` and `St_r(A)` are Kazhdan, and `Gamma`, being an image of
the former, is Kazhdan.  Kaluba--Nowak--Ozawa and
Kaluba--Kielak--Nowak prove property `(T)` for `Aut(F_d)` for `d>=5`; it
passes to the finite-index subgroup `SAut(F_d)`.  Extensions of Kazhdan
groups are Kazhdan, so `G` is Kazhdan.

For `i!=j`, let `R_(i,j)` and `L_(i,j)` be the positive Nielsen
transvections

`x_i |->x_i x_j`,  `x_k|->x_k` for `k!=i`.

and

`x_i |->x_j x_i`,  `x_k|->x_k` for `k!=i`,

respectively.  They map the free monoid into itself and therefore send
`A_+` into `A_+`.  Hence their external lifts lie in `P_Gamma`.  Nielsen's
standard generation theorem says that the `L_(i,j),R_(i,j)` group-generate
`SAut(F_d)` (their group inverses provide the negative moves), so the
external factor lies in the group generated by `P_Gamma`.

The `SAut(F_d)`-conjugates of `A_+` generate `A` as a ring.  Indeed an even
inversion sends, for example, `x_1,x_2` to `x_1^(-1),x_2^(-1)` and belongs
to `SAut(F_d)`; conjugating coordinate choices supplies every `x_i^(-1)`.
The elements `x_i^(+-1)` generate `F_q[F_d]`.  Root additivity and the
Steinberg relation

`[x_(ab)(u),x_(bc)(v)]=x_(ac)(uv)`

then show that the conjugates of `Gamma` contain every root generator of
`St_r(A)`.  They therefore generate the entire coefficient factor,
including its central Steinberg kernel.  Thus `P_Gamma` generates `G`.

Finally take `t=R_(1,2)`.  Its image of `A_+` is the proper free subalgebra
generated by `x_1x_2,x_2,...,x_d`; the positive word `x_1` is not in it.
For `gamma=x_(ab)(x_1)`, projection to `EL_r(A)` shows

`gamma notin t Gamma t^(-1)`,

because every matrix entry coming from the latter subgroup belongs to
`t(A_+)`.  Hence the compression is strict and `Gamma` is nonnormal.  End
proof.

The strict image-pair criterion applies to this pair just as it does to the
commutative one.  A normalized-HS model with a nonnormalized relative
commutant would therefore produce a hyperlinear nonsofic group immediately.
This is not yet that model.  Its possible constructive advantage is exact:
coefficient monomials have unique reduced-word normal forms, so there is no
commutative antipodal-ray ambiguity or coefficient `K_2` descent.  Its new
analytic obstacle is the nonamenable boundary of the free monoid; any finite
model must use prefix/Fock or Fourier branching rather than an ordinary
...
### A Clifford central cover isolates the required quantum microstate

There is a sharper target than the commuting-lamp wreath product.  Let
`X=G/Gamma` for a Kun--Thom pair.  Define the **Clifford lamp group** `C(X)`
by generators
...
`z^2=1`,  `[z,a_x]=1`,  `a_x^2=1`,

`[a_x,a_y]=z` for `x!=y`.

Every permutation of `X` induces an automorphism of this presentation, so
the `G`-action on `X` gives

`H_Cl=C(X) semidirect G`.

The group `C(X)` is locally finite: a subgroup generated by `z` and `n`
coordinate generators has at most `2^(n+1)` elements, since every word has
normal form

`z^epsilon product_(x in F) a_x`

for a finite set `F`.  In particular it is amenable.

Quotienting by the central involution gives

`H_Cl/<z> ~= (directSum_X C_2) semidirect G`,

the Kun--Thom generalized wreath product, because the coordinate lamps
commute after imposing `z=1`.  That quotient is nonsofic.  Thom's central
quotient theorem (Lemma 3.4 and Remark 3.5 of *Examples of hyperlinear
groups without factorization property*) says that a quotient of a
hyperlinear group by a central subgroup is hyperlinear.  Therefore

> proving `H_Cl` hyperlinear proves the existence of a hyperlinear nonsofic
> group immediately.

The cover is necessarily invisible to genuine finite quotients.  Choose a
strict compressor `t` and `gamma in Gamma` with

`h=t^(-1) gamma t notin Gamma`.

For every homomorphism `q:G->Q` to a finite group, the inclusion

`q(t Gamma t^(-1)) <=q(Gamma)`

is an equality: its left side is conjugate to `q(Gamma)` and hence has the
same finite cardinality.  Thus `q(t)` normalizes `q(Gamma)`, and

`q(h) in q(Gamma)`.

Now let `pi:H_Cl->F` be any finite quotient and apply this to `q=pi|_G`.
There is `delta in Gamma` with `pi(h)=pi(delta)`.  Since `Gamma` fixes the
base coset `o=Gamma`,

`pi(a_(h o))=pi(h a_o h^(-1))`
`            =pi(delta a_o delta^(-1))=pi(a_o)`.

But `h o!=o`, so the Clifford relation gives

`pi(z)=pi([a_(h o),a_o])=1`.

Hence every genuine finite quotient kills the distinguished center.  A
hyperlinear model of `H_Cl` must retain `z` through projective/Clifford
matrix microstates rather than finite group quotients.  This is exactly the
non-scalar Pauli phenomenon sought in the compressor--shear construction,
...
### Correction: weak MF of the Kun--Thom symmetric double is not established

The earlier sections claiming that

`D_2=G *_Gamma G`

and `H_p=G *_Gamma (Gamma x C_p)` are weak-MF overstate the available
amalgamated-free-product theorems.  Their group-theoretic statements remain
valid: the cyclic Schreier calculation identifies `D_p` with the iterated
double, the fold kernel has the stated free normal form, and Kun--Thom's
centralizer theorem proves nonsoficity.  What is not proved is weak MF.

Shulman's Theorem 10 in *The MF property for amalgamated free products*
(arXiv:2603.13564v2) states that if a concrete separable C-star algebra `A`
is MF and `C subset A`, then the full C-star double `A *_C A` is MF.  It does
not by itself turn an arbitrary faithful weak-MF group embedding
`rho:G->U(A_0)` into a faithful embedding of the abstract group double.  The
subalgebra generated by `rho(Gamma)` may impose extra relations between the
two vertex groups.  For example, a faithful irrational character of
`G=Z` has `C^*(rho(2Z))=C^*(rho(Z))=C`, so the corresponding concrete
C-star double folds even though the abstract group double does not.  Thus
the group-level sentence in the introduction is insufficient for the
Kun--Thom application without an additional faithful-amalgam argument or a
stronger hypothesis such as MF of the relevant full/reduced group C-star
algebra.

The newer theorem of Gao--Kunnawalkam Elayavalli--Manzoor--Patchell,
*A new source of purely finite matricial fields* (arXiv:2603.24502), also
does not apply.  Its theorem for

`G *_H (H x K)`

requires `H` to be separable in `G`, besides strong group MF/PMF/PFF and
exactness hypotheses.  The Kun--Thom pair has precisely the opposite
feature: for a strict compressor `t` and suitable `gamma in Gamma`,

`h=t^(-1) gamma t notin Gamma`

has image in the image of `Gamma` in every finite quotient of `G`.  Hence
`Gamma` is not separable in `G`.  This is the same relative profinite
collapse used throughout the construction.

Accordingly the correct status is:

* `D_2` is an explicit nonsofic symmetric double with the proved Bass--Serre
  and free-by-Kazhdan structural descriptions;
* weak MF of `D_2`, `D_p`, and `H_p` is **open here**;
* none of these groups currently proves the existence of a weak-MF
  nonsofic group;
* the weak-MF Clifford implication remains a valid conditional bridge,
  since it uses the finite-central spectral-corner theorem rather than an
  amalgamated-free-product permanence claim.
...
theorem applies, and assume in addition that `Gamma` is perfect.
...
`[atilde,x] in C`,  `x in Gammatilde`,

are central.  The commutator identities therefore make

`x |-> [atilde,x]`

a homomorphism from `Gammatilde` to the abelian group `C`.  It vanishes on
`C`, because `C` is central, and on `[Gammatilde,Gammatilde]`.  Perfectness
of the quotient `Gamma` gives

`Gammatilde=C [Gammatilde,Gammatilde]`.

Therefore this homomorphism is trivial and `atilde` centralizes all of
`Gammatilde`.  No splitting and no `H_2(Gamma,Z)` computation is needed.
...
lamp sites `t Gamma` and `gamma t Gamma` differ, and choose any lift
`gammatilde in Gammatilde`.  The commutator

`wtilde=[ttilde atilde ttilde^(-1),gammatilde]`
...
The perfectness hypothesis is automatic for the polynomial elementary
stabilizer `Gamma=EL_r(F_q[x_1,...,x_d])` in the ranks used here, by the
elementary Steinberg commutator relations.
...
**Theorem (all-central-cover rigidity).**  Assume that `Gamma` and `G` are
perfect and that `H_2(G,Z)` is finite.  Then for
...
A lift `atilde` of the base lamp centralizes `Gammahat` by the elementary
perfect-quotient argument in the preceding proof: the commutator character
on `Gammahat` kills both `[Gammahat,Gammahat]` and its central kernel, and
these two subgroups generate `Gammahat`.
...
`H_2(G,Z)` finite.  The polynomial stabilizer `Gamma` is perfect.  Therefore
the all-central-cover theorem applies
...
The proof uses no commutativity relation among different lamps.  It has the
following abstract form.

**Corollary (central-cover inheritance of a Kun--Thom witness).**  Under the
hypotheses of the all-central-cover theorem, let `H` be any group containing
`G` and an element `a` such that

`[a,Gamma]=1`

and, for some compressor `t` and some `gamma in Gamma`,

`[t a t^(-1),gamma]!=1`.

Then every central extension of `H` is nonsofic.

Indeed, the universal-central-extension image `Ghat` and its subgroup
`Gammahat` are constructed inside the inverse image of `G` exactly as above.
The perfect-quotient commutator argument makes every lift of `a` centralize
`Gammahat`, while the displayed witness remains nontrivial after projection.
Kun--Thom normalization gives the same contradiction.

Consequently the theorem applies simultaneously to the original commuting
Kun--Thom wreath product, the free-lamp amalgams, and the Clifford central
cover.  Passing to another central cover of any of these groups can never
...
# Kazhdan universal covers make every Leavitt central cover nonsofic (2026-08-09)

The finite-`H_2` hypothesis in the preceding all-central-cover theorem is
only a convenient sufficient condition.  The actual input is property `(T)`
of the universal central extensions of the ambient group and the compressed
subgroup.  This stronger formulation applies to the binary Leavitt witness
without computing its unstable `K_2`.

**Theorem (universal-cover central rigidity).**  Let `Gamma<G` be an
infranormal inclusion of perfect countable groups.  Suppose that the universal
central extensions

`U_G -> G` and `U_Gamma -> Gamma`

have property `(T)`.  Let `H` contain `G` and let `a in H` satisfy
...
for some `t in G` and `gamma in Gamma`.  Then every countable central
extension of `H` is nonsofic.

**Proof.**  Let `p:E->H` be a central extension.  Restriction to `G` is a
central extension, so universality gives a lift

`f:U_G -> E`

over `G`.  The inclusion `i:Gamma->G` induces a homomorphism

`U(i):U_Gamma -> U_G`.

Put

`Ghat=f(U_G)` and `Gammahat=f(U(i)(U_Gamma))`.

Both groups are perfect and have property `(T)`, since they are quotient
images of the corresponding universal central extensions, and they map onto
`G` and `Gamma` respectively.

The inclusion `Gammahat<Ghat` is infranormal.  Indeed, for
`q in P_Gamma={g:g Gamma g^(-1)<=Gamma}`, choose a lift `qtilde in U_G`.
Functoriality and uniqueness of universal central lifts give

`qtilde U(i)(U_Gamma) qtilde^(-1) <= U(i)(U_Gamma)`.

Thus `f(qtilde)` belongs to the compression semigroup of `Gammahat`.  Let
`L0` be the subgroup generated by `Gammahat` and these lifted compressors.
Its image in `G` is all of `G`.  Consequently

`Ghat=L0 Z0`,

where `Z0=ker(Ghat->G)` is central.  Since `Ghat` is perfect,

`Ghat=[Ghat,Ghat]=[L0 Z0,L0 Z0]<=[L0,L0]<=L0`.

Hence `L0=Ghat`, proving infranormality.

Choose a lift `ahat in E` of `a`.  For `x in Gammahat`, the commutator
`[ahat,x]` belongs to the central kernel of `p`.  Therefore

`x |-> [ahat,x]`

is a homomorphism from the perfect group `Gammahat` to an abelian group, and
is trivial.  Thus `ahat` centralizes `Gammahat` exactly.

If `E` were sofic, restrict a faithful sofic representation to `Ghat`.
Kun--Thom Theorem 4.1 applies to the Kazhdan infranormal pair
`Gammahat<Ghat`: the permutation-ultraproduct centralizer of `Gammahat` is
normalized by `Ghat`.  Hence the image of

`that ahat that^(-1)`

centralizes the image of every lift `gammahat` of `gamma`, where `that` is a
lift of `t` in `Ghat`.  The sofic representation kills

`[that ahat that^(-1),gammahat]`.

This element is nontrivial because its image under `p` is the displayed
nontrivial commutator in `H`, contradicting faithfulness.  End proof.

**Application to the binary Leavitt group.**  Put

`L=L_(F_2)(1,2)` and `Q=EL_4(L)`.

Let `Gamma=EL_3(L)` be the upper-left core in the proved rank-four
compression setup, let `u` be the distinguished compressor, and put

`K=u Gamma u^(-1)`.

The subgroup `K` is infranormal because it is conjugate to the infranormal
core.  The complementary-corner witness subgroup `J<Gamma` centralizes `K`.
Choose `1!=a in J`.  Then `u a u^(-1)` is a nontrivial element of `K`.
Moreover `K` is centerless: the kernel-checked result `GL_3(L)=EL_3(L)`, the
standard identity `Z(GL_3(L))=Z(L)^x I`, and `Z(L)=F_2` give `Z(K)=1`.
Thus some `gamma in K` satisfies

`[u a u^(-1),gamma]!=1`.

Both `Q` and `K` are perfect.  Their universal central extensions are
Kazhdan without any unstable-`K_2` calculation.  The proved Leavitt rank
equivalence identifies each of `EL_3(L)` and `EL_4(L)` with `EL_5(L)`.
Kervaire--Steinberg says that, for every associative unital ring and
`n>=5`,

`St_n(R) -> EL_n(R)`

is the universal central extension.  Ershov--Jaikin-Zapirain prove property
`(T)` for `St_n(R)` for every finitely generated associative unital ring and
`n>=3`.  Universal central extensions are invariant under group
isomorphism, so `U_Q` and `U_K` have property `(T)`.  Applying the theorem to
`K<Q` and the element `a` gives

`every countable central extension of EL_4(L_(F_2)(1,2)) is nonsofic`.

By the all-positive-ranks Leavitt equivalence, the same holds with `EL_m(L)`
for every `m>=2`.

**Concrete corollary (explicit Steinberg nonsofic groups).**  For every
`n>=5`,

`St_n(L_(F_2)(1,2))`

is a nonsofic property-`(T)` group.  It is finitely presented: the Leavitt
ring is finitely presented and Krstic--McCool prove finite presentability of
`St_n(R)` for finitely presented `R` and `n>=4`.  More generally, every
central quotient

`St_n(L)/C`,  `C<=K_2(n,L)`,

that still maps onto `EL_n(L)` is nonsofic.  This is a canonical explicit
family of finitely presented Kazhdan nonsofic central covers; it is stronger
than the earlier existence-only finite-presentation cover and does not use
quotient closure of soficity.

There is also a new one-certificate route to the main open problem.  The
group `Q` is simple: `L` is a purely infinite simple exchange ring;
Preusser's Theorem 3 sandwiches every `EL_4(L)`-normal subgroup between the
relative elementary and congruence subgroups of an ideal; simplicity of `L`,
`GL_4(L)=EL_4(L)`, and the trivial scalar center over `F_2` leave only the
trivial and whole subgroups.  Hence the universal central extension
`U=St_5(L)` is quasisimple: every proper normal subgroup of `U` lies in its
center.  Indeed, the image of a normal subgroup in simple `Q` is trivial or
all of `Q`; in the latter case `U=N Z(U)`, and perfectness forces `U=N`.

If `St_5(L)` is weak/operator-norm MF, Kazhdan-corner extraction produces a
nontrivial hyperlinear quotient `U/N` with `N` proper.  Quasisimplicity gives
`N<=Z(U)`, so `U/N` is a central extension of `Q` and the theorem above says
it is nonsofic.  Therefore

`St_5(L_(F_2)(1,2)) weak-MF  ==>  a hyperlinear nonsofic group exists`.

This sits beside, rather than replaces, the older certificate that `Q`
itself be weak-MF.  Its gain is that the candidate is now a canonical
finitely presented Steinberg group and no simplicity hypothesis is needed at
the Kazhdan-corner output: every nontrivial quotient produced there is
automatically a nonsofic central cover.

**Citation pins checked.**

* Ershov--Jaikin-Zapirain, arXiv:0809.4095v2, Introduction and Section 6:
  `St_n(R)` has property `(T)` for finitely generated associative `R`,
  `n>=3`; the same paper quotes Krstic--McCool for finite presentability when
  `n>=4` and `R` is finitely presented.
* Kervaire--Steinberg, in Weibel, *The K-book*, III, Proposition 5.5.1:
  `St_n(R)` is the universal central extension of `E_n(R)` for `n>=5`.
