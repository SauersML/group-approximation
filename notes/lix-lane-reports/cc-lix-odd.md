...
| `CharClass/LemmaTwoStepD.lean` | 8804 | **Step D at the real objects**, off the endpoint path: `WuStepDData` (the seven remaining hypotheses with their owners), `WuStepDData.gamma_top_eq_zero`, `stepD_of_wu` (the Step D half of `LemmaTwoInput`), and `ne_zero_of_degree_ne_zero` (the single-degree to `TotalH` bridge) |
