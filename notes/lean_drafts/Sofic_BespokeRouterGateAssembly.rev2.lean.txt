...
the Greendlinger conclusion by `SmallCancellationRouter.injOn_mk'_of_greendlinger`,
which is proved, against the design's own `protectedBall` field.  That is the
same data-not-axiom idiom `protected_injOn_of_gate` already uses upstream.
...
through the proved engine and the design's `protectedBall` field; the
torsion half is passed through, because it is a second classical theorem and
not a consequence of the first.  See the module docstring. -/
