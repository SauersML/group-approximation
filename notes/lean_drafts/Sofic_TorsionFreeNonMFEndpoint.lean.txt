...
theorems below.  It is not atomic; it decomposes into three separately
tracked obligations --
...
The third is deliberately not a hypothesis of the theorems below: that
would turn the headline into a conditional statement, which is exactly
what this development exists to avoid. -/
...
is built and its `RouterConclusions` supplied, `bespoke_routing_exists`
closes by applying this lemma.
...
`Sofic/BespokeRouterGateAssembly.lean`; that gate still has open leaves,
and a declaration which merely calls an incomplete constant inherits the
incompleteness axiom without Lean warning at the call site, so only the
kernel audit would see it.  Keeping it a hypothesis
is what keeps this file, and the root import closure it will eventually
join, clean.  When the gate leaves close, this becomes a call to
`RouterRelatorDesign.routerDataOfDesign` and the hypothesis disappears. -/
