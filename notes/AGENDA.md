...
There is one deliberately nonstandard permanence theorem:
...
`amenable-kazhdan-models-absorb-compression`.  The remaining arithmetic
candidate is now closed too: `sl3-atomic-ambient-is-absorbed-by-lattice`
shows that every hyperfinite model of
`SL_3(Z[1/p])` is already generated by its `SL_3(Z)` image, so the required
exteriority fails maximally.  Thus this new permanence theorem currently has
no candidate on the held nonsofic groups.  A positive solution must build a
candidate's genuinely nonamenable matrix model directly; a negative solution
must give a genuinely tracial obstruction.
...
entropy at infinity.
In fact `fournier-facio-fixed-block-actor-is-charge-haar` computes the whole
marginal: every nonzero charged Fourier moment of that actor vanishes, so its
positive fixed-block spectrum is Haar in aggregate, and every fixed power has
zero diagonal coefficient columnwise in charge.  Hence every fixed Krylov
window of a charged basis column is asymptotically orthonormal.
Haar-distributed irrational diagonal phases are therefore excluded; the actor
must mix columns genuinely.
The signed-square cyclic splice attains this stronger marginal exactly, which
shows that the next theorem must control its joint coupling to the other
generators/compressors rather than another one-operator recurrence statistic.
...
More precisely, the discrepancy between the conjugated Gamma copy
`u_C u_Gamma u_C^*` and the named copy `u_(alpha_C(Gamma))` is a
`U(R' cap R^omega)`-valued one-cocycle.  A kernel gauge on `u_C` closes the
finite seam exactly when this cocycle is a coboundary.  In the canonical CE
amalgam crossed product no such gauge exists even as a nonzero intertwiner:
its relative commutant is `L(ker(P_A->G))`, and the coefficient action has no
finite orbit by the Bass--Serre tree.  Therefore the correction, if it
exists, must be genuinely extrinsic to the pasted algebra after embedding
it in `R^omega`.  This is now the narrow positive target; proving that the
extrinsic relative commutant adds no such intertwiner would instead certify
the finitary extension nonhyperlinear.

The internal no-go has a Kazhdan gap `kappa` and survives every finite
diagonal amplification.  Hence cyclic/block symmetrization does not close
the seam: the two-block flip intertwines `v directSum rho` with
`rho directSum v`, not repeated copies of one `H_A` lift, and a genuinely
`S`-covariant version is induction over the infinite set `S/<A>` rather than
the three-element `C` orbit.  The extrinsic problem has an exact compactness
formulation: for one CE embedding of the AFP paste, find unitaries which
approximately commute with longer finite subsets of `R`, approximately
intertwine the fixed finite Kazhdan packet, and approximately preserve the
finite `A,C` presentation of `SL_3(Z)`.  Countable saturation turns such a
sequence into the required exact relative-commutant gauge.  This finite
satisfiability test is now the first positive construction target.

Passive McDuff absorption does not construct it.  Tensoring the AFP paste by
any finite tracial spectator tensors the seam representation by a trivial
module and retains the same Kazhdan gap.  More generally, an exact corrector
in any finite ambient extension has expectation zero onto the AFP algebra
and is at `L^2` distance exactly one from it; an approximate corrector of
defect `delta` has AFP expectation at most `delta/kappa`.  Thus every positive
model must create a dynamically nontrivial extrinsic Gamma bimodule with
asymptotically all of the gauge mass, rather than exploit McDuff absorption
or a perturbative enlargement of the current paste.

The minimal escaped object is an orthogonal dimension-one correspondence
between the two crossed-product copies generated by `R` and the conjugate
Gamma representations, with an `R`-central unitary cyclic vector.  Standard
Bernoulli/coinduced tensor actions cannot supply it.  For every honest actor
representation `sigma` of the quotient `G`, tensoring gives

```text
k'_gamma=k_gamma tensor 1,
```

because `sigma_C sigma_gamma sigma_C^*` already equals
`sigma_(alpha_C(gamma))`.  The same cancellation covers every finite block
symmetrization that actually satisfies the full actor table.  A block model
that only closes `C^3` is not an `SL_3(Z)` representation.  Hence the next
construction must be a genuinely non-tensorial coupling producing the
orthogonal index-one correspondence; this is the mixed-kernel problem in
its minimal form, not another Bernoulli permanence problem.
