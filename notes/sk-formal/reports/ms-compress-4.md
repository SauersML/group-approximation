...
| 109–110 | "So the matrices `e_ij(s)` with `s ∈ {u, u⁻¹} ∪ {e_a}` generate `G`, as `1 = ∑_a e_a`." | partial | REWRITTEN: `1` left the generating set. New carrier `printedElementaryGenerationSumOne` (1 ∈ additive closure of `gens`), probing. Instance consumes ct-involution's generation of `R` | waits |
