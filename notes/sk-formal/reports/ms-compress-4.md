...
| 114–115 | Commuting with every `e_ij(1)` forces a central element of `G` to be `c I_3`; commuting with all `e_ij(r)` gives `c ∈ Z(R)^× = {1}`. | formalized (generic) + instance | GAP for the printed hypothesis (commutes with the `e_ij(1)`, weaker than centre membership); `Pestov91.exists_central_scalar_of_mem_center`, `center_elementaryGroup_eq_bot`, `SplitSimplicity.central_unit_eq_one_of_central_zero_or_one`; `Z(R) = F₂` from ct-involution 110–113 | build |
