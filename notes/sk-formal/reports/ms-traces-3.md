...
## Item 4 (09-14, oa-expanders fidelity audit): a named endpoint for `427203e368c1`
- Finding (oa-expanders, census d91e3fbac): row `427203e368c1` listed only lemmas with binders, and no named closed `Printed…` Prop for the sentence "Multiplying out a word in the generators in LC(A^ℤ,F₂)⋊ℤ, which maps onto R, gives a matrix with entries Σ_j f_j u^j, each f_j given by a table on the words of some length, using uf=(f∘T⁻¹)u." Accepted.
- CLAIM `GroupApproximation/Manuscript/SimpleKazhdanSofic/WordMultipliesOut.lean`: `PrintedWordMultipliesOutToTables`, with `#audit_closed_axioms`. For every word w in the generators there are K, a finite exponent set E and tables F on words of length 2K+1, independent of X. For every subshift S, the matrix of w in G_X has entries Σ_{j∈E} coeff(f_j)·u^j, with f_j(x) = F(x_{[-K,K]}). The row is re-graded on landing, or regraded `partial` if the module does not land.
- LANDED 2b74a128a (WordMultipliesOut, probe 0914-110518-89899 BUILT, closed audit): `printedWordMultipliesOutToTables`, with `eval_eq_sum` (a table is the Laurent sum of its coefficient functions). Row `427203e368c1` now names the endpoint first.
