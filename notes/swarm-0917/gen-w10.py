...
J='/root/.claude/projects/-home-user-group-approximation/6559a301-b52c-57ef-bef5-35eb6d3deb43/subagents/workflows/wf_c7b10dee-750/journal.jsonl'
a=json.load(open(S+'/w9-args.json'))
lab={};work={}
for l in open(J):
  d=json.loads(l)
...
for l in a['lanes']:
  ...
  if kind=='follow':
    ...
    n['text']=(f"FOLLOW-THROUGH toward {g}. Wave 9 of this swarm (landed on main in the last 4 hours) reported the following; each note says what it proved and what is left:\n"+"\n".join(notes)+
    ...
    n['text']=n['text'].replace('waves 7 and 8','waves 8 and 9')
  lanes.append(n)
a['lanes']=lanes; a['prefix']='swarm-0917-w10'; a['maxWorkers']=20
json.dump(a,open(S+'/w10-args.json','w'))
js=open(S+'/swarm-w9.js').read()
m=re.search(r'const W6 = (.*?)\nconst REFEREE',js,re.S)
js=js[:m.start(1)]+json.dumps(a)+js[m.end(1):]
js=js.replace("'swarm-0917-w9'","'swarm-0917-w10'")
open(S+'/swarm-w10.js','w').write(js)
