...
J='/root/.claude/projects/-home-user-group-approximation/6559a301-b52c-57ef-bef5-35eb6d3deb43/subagents/workflows/wf_79a5b6bf-237/journal.jsonl'
a=json.load(open(S+'/w22-args.json'))
lab={};work={}
for l in open(J):
  d=json.loads(l)
...
for l in a['lanes']:
  ...
  if kind=='follow':
    ...
    n['text']=(f"FOLLOW-THROUGH toward {g}. Wave 22 of this swarm reported the following; each note says what it proved and what is left:\n"+"\n".join(notes)+
      f"\nPick the single most promising 'what is left' step above that would move {g} closest to ESTABLISHED (prefer a step that closes a route's last hole, or a strong counterexample that redirects the program), check `bin/cairn context` on it and git log --since=4.hours (your worktree starts from the swarm branch, which carries wave 22's results; origin/main also carries waves 10-17) for anyone who already took it, and attack it for the full statement. Do not attempt narrow special cases. Write long files in several smaller Write/Edit calls and keep your final returned summary under 1500 words; single responses over 32000 tokens fail.")
  elif 'text' in n:
    n['text']=n['text'].replace('waves 20 and 21','waves 21 and 22')
  lanes.append(n)
a['lanes']=lanes; a['prefix']='swarm-0917-w23'; a['maxWorkers']=14
json.dump(a,open(S+'/w23-args.json','w'))
js=open(S+'/swarm-w22.js').read()
m=re.search(r'const W6 = (.*?)\nconst REFEREE',js,re.S)
js=js[:m.start(1)]+json.dumps(a)+js[m.end(1):]
js=js.replace("'swarm-0917-w22'","'swarm-0917-w23'")
open(S+'/swarm-w23.js',"w").write(js)
