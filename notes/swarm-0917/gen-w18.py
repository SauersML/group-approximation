...
J='/root/.claude/projects/-home-user-group-approximation/6559a301-b52c-57ef-bef5-35eb6d3deb43/subagents/workflows/wf_a44c68f7-2a6/journal.jsonl'
a=json.load(open(S+'/w17-args.json'))
lab={};work={}
for l in open(J):
  d=json.loads(l)
...
for l in a['lanes']:
  ...
  if kind=='follow':
    ...
    n['text']=(f"FOLLOW-THROUGH toward {g}. Wave 17 of this swarm reported the following; each note says what it proved and what is left:\n"+"\n".join(notes)+
      f"\nPick the single most promising 'what is left' step above that would move {g} closest to ESTABLISHED (prefer a step that closes a route's last hole, or a strong counterexample that redirects the program), check `bin/cairn context` on it and git log --since=4.hours (your worktree starts from the swarm branch, which carries wave 17's results; origin/main also carries waves 10-17) for anyone who already took it, and attack it for the full statement. Do not attempt narrow special cases. Write long files in several smaller Write/Edit calls and keep your final returned summary under 1500 words; single responses over 32000 tokens fail.")
  elif 'text' in n:
    n['text']=n['text'].replace('waves 15 and 16','waves 16 and 17')
  lanes.append(n)
a['lanes']=lanes; a['prefix']='swarm-0917-w18'; a['maxWorkers']=20
json.dump(a,open(S+'/w18-args.json','w'))
js=open(S+'/swarm-w17.js').read()
m=re.search(r'const W6 = (.*?)\nconst REFEREE',js,re.S)
js=js[:m.start(1)]+json.dumps(a)+js[m.end(1):]
js=js.replace("'swarm-0917-w17'","'swarm-0917-w18'")
open(S+'/swarm-w18.js',"w").write(js)
