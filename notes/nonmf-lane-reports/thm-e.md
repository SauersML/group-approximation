...
| `Manuscript/NonMF/TorsionFreePrintedSentences.lean`, with `PrintedDefectParagraph.lean` and `TorsionFreePrintedTheorem.lean` edited | 29 `manuscriptSentence_*` carriers (table in §3c), plus `conjFactor_ne_bot` and `normalClosure_conjFactor_ne_bot` | `1486bce106b05e85a3e38a1a1c6db473f1072a3e` | 4456 |
...
act on.  The root import list needs the eight new modules.  And the sentence
...
## 3c.  Sentence carriers for the four printed proofs

`Manuscript/NonMF/TorsionFreePrintedSentences.lean`, commit
`1486bce106b05e85a3e38a1a1c6db473f1072a3e`, probed green at 4456 jobs.  It
answers the census work order's four rows for this section
(`lem:saturation` proof, `thm:torsion-free` proof, and the two corollary
proofs), on the pattern of
`Manuscript/OneSidedMFRadical/NormalKazhdanPrintedRoute.lean`.

**Correction to that commit's message.**  It says "twenty-two carriers".  The
count is **29**; I miscounted when writing the message.  The commit itself is
right, only its prose is wrong, and the table below is the authority.

| printed sentence | carrier | hypothesis |
| --- | --- | --- |
| *`lem:saturation`* | | |
| "The subgroup `N` is normal, and it is infinite because `G` is torsion-free and `N ≠ 1`" | `manuscriptSentence_saturationNormalInfinite` | proved |
| "so it acts non-elementarily on `Cay(G,A)` by Osin [Lemma 7.1]" | `manuscriptSentence_saturationOsinNonElementary` | `OsinLemma71Printed` |
| "since `G` is torsion-free, `N` normalizes no nontrivial finite subgroup, so `N` is suitable" | `manuscriptSentence_saturationSuitable` | proved |
| "By Hull [Corollary 5.7 and Lemma 5.8], `N` contains two elements `h₁,h₂` …" | `manuscriptSentence_saturationTwoGeneratedSuitable` | **proved**, with `A' = A` |
| "and Theorem `thm:hull` holds for `A'` as well, since every finite subset of `G` lies in a ball" | `manuscriptSentence_saturationHullAtLargerAlphabet` | `HullSmallCancellationPrinted` |
| "Apply Theorem `thm:hull` to `N₀` with `g₁,…,g_m` a finite generating set of `G` and with the set `Ω`" | `manuscriptSentence_saturationApplyHull` | `HullSmallCancellationPrinted` |
| the display `Q = ⟨φ(g₁),…,φ(g_m)⟩ ≤ φ(N₀) ≤ φ(N) ≤ Q`, and its two consequences | `manuscriptSentence_saturationDisplay` | proved |
| "The quotient is torsion-free by Theorem `thm:hull`" | `manuscriptSentence_saturationQuotientTorsionFree` | proved from the lift clause |
| "and finitely presented because `ker φ` is normally generated by `m` elements" | `manuscriptSentence_saturationQuotientFinitelyPresented` | proved |
| *`thm:torsion-free`* | | |
| "Let `N` be the normal closure of `S` in `G₀`; it is nontrivial because `S` is" | `manuscriptSentence_theoremNormalClosureNeBot` | proved |
| "By Lemma `lem:saturation` applied to `G₀`, `N`, and `Ω = ∅` …" | `manuscriptSentence_theoremApplySaturation` | `HullPrintedInputs` |
| "The group `Q` is infinite because it is acylindrically hyperbolic" | `manuscriptSentence_theoremQuotientInfinite` | proved |
| "and it has property (T) as a quotient of `G₀`" | `manuscriptSentence_theoremQuotientKazhdan` | proved |
| "By `eq:defect-functorial`, `φ(S) ≤ 𝔇_Q(φ(Γ))`" | `manuscriptSentence_theoremDefectFunctorial` | proved |
| "The subgroup `𝔇_Q(φ(Γ))` is normal in `Q`, … so `𝔇_Q(φ(Γ)) = Q`" | `manuscriptSentence_theoremDefectEqTop` | proved |
| "Both `Q` and `φ(Γ)` have property (T), as quotients of `G₀` and `Γ`" | `manuscriptSentence_theoremBothKazhdan` | proved |
| "By the last assertion of `thm:compression-criterion`, every homomorphism from `Q` to an MF group is trivial" | `manuscriptSentence_theoremHomsTrivial` | proved |
| "If a quotient `Q̄` of `Q` is MF, then the quotient map is trivial, so `Q̄ = 1`" | `manuscriptSentence_theoremQuotientTrivial` | proved |
| *`cor:relative-quotient`* | | |
| "since a torsion-free acylindrically hyperbolic group has no nontrivial finite normal subgroup" | `manuscriptSentence_relativeQuotientTrivialFiniteRadical` | **proved** |
| "Hull's common quotient theorem, applied to `G` and to the group `Q` …" | `manuscriptSentence_relativeQuotientCommonQuotient` | `HullCommonQuotientPrinted` |
| "the proof of that corollary consists of two applications of `thm:hull` … so `P` is torsion-free and finitely presented" | `manuscriptSentence_relativeQuotientTorsionFreeFinitelyPresented` | proved, over the data those applications produce |
| "As a quotient of `Q`, the group `P` is two-generated and has property (T)" | `manuscriptSentence_relativeQuotientTwoGeneratedKazhdan` | proved |
| "and every homomorphism from `P` to an MF group pulls back to one from `Q`, so it is trivial" | `manuscriptSentence_relativeQuotientHomsTrivial` | proved |
| *`cor:regular-nonmf-algebra`* | | |
| "and has no nontrivial finite normal subgroup" | `manuscriptSentence_algebraTrivialFiniteRadical` | **proved** |
| "Dahmani, Guirardel, and Osin give simplicity and uniqueness of the trace …, and Gerasimova and Osin give density of the invertible elements" | `manuscriptSentence_algebraSimpleUniqueTraceStableRank` | `DGOTheorem235Printed`, `GerasimovaOsinTheorem11Printed` |
| "The algebra is separable and generated by the canonical unitaries of the two generators" | `manuscriptSentence_algebraSeparableGenerated` | proved |
| "its canonical trace is faithful, so it is stably finite" | `manuscriptSentence_algebraStablyFinite` | proved |
| "an embedding into a norm matrix corona would embed `Q` into the corona's unitary group" | `manuscriptSentence_algebraNotMF` | proved |
| "against Theorem `thm:torsion-free`" | `manuscriptSentence_algebraNotCDEOperatorMF` | proved |

**One printed clause has no carrier.**  *"it contains a non-degenerate
hyperbolically embedded subgroup [Osin, Theorem 1.2]"*.  This development has no
vocabulary for *hyperbolically embedded*, and the printed proof uses that clause
only to reach the hypothesis of Dahmani--Guirardel--Osin, which
`DGOTheorem235Printed` states directly.  The omission is recorded in the
docstring of `manuscriptSentence_algebraTrivialFiniteRadical`, which carries the
rest of that sentence.

**A duplicate removed rather than created.**  `conjFactor_ne_bot` and
`normalClosure_conjFactor_ne_bot` went into `PrintedDefectParagraph.lean`, and
`TorsionFreePrintedTheorem.lean` now calls the second instead of repeating its
fifteen-line proof inline.
