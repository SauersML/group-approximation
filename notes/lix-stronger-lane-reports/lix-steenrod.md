...
Notation: `u ∈ C^q(X; F_p)` is a cocycle, and `D_j(u)(σ) = tupEval (ZMod p) X p (p*q) (fun _ => Pi.single q u) (Δ(e_j ⊗ σ))` for a `(pq − j)`-simplex `σ`. Here `Δ` is `oddDiagApp p X (p*q)` on the source generator of `W`-degree `j`. `D_j` is lix-descent's planned `oddD`. No `OddPCochain` / `OddPClasses` / `OddPAdditive` file exists, and nothing imports `OddPDiagonal` yet. Their residual statements R1–R9 use Δ1–Δ6 exactly as landed at 7e2fb5081: `oddD`, `oddD_cocycle`, `oddD_natural`, class-level `oddD_coboundary` via a cylinder, the exactness lemma, `redPow` (index `j = (q−2i)(p−1)`, `dite` on `2i ≤ q` with `cohCast`), instability, `redPow_mk` and `redPow_add`. They are under SCOPE 3 of `notes/lix-stronger-lane-reports/lix-descent.md`. `EvalIsHeadCoeff`'s only consumer is `eDescent_value_of_eval`, and no theorem rests on it.
