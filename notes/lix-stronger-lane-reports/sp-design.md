...
| **L4a′** | *The division is a Pascal identity, not a delicate step.*  `N·[C(N−1,i) + p·C(N−1,i−1)] = C(N,i)·(N + i(p−1))` over `ℤ`: from `i·C(N,i) = N·C(N−1,i−1)` the right side is `N[C(N,i) + (p−1)C(N−1,i−1)]`, and Pascal turns the bracket into the left one.  So the integral coefficient is `C(N−1,i) + p·C(N−1,i−1)` on the nose and **no division survives into the statement** — in Lean, state the identity and define the coefficient by the bracket.  VERIFIED (model): `p ∈ {2,3,5,7,11}`, `N ≤ 13`, all `i`, zero counterexamples. |
| **L4a″** | *No extraction operator is needed.*  The proof below works in the quotient `Λ⁺/(Λ⁺)²`, where "the coefficient of `e_N`" is the image of a basis element and there is nothing to identify.  A layer that instead defines an extraction function `eCoeff` owes the identification lemma: for a polynomial in the abstract `E_j`, the square-zero substitution `E_j ↦ ε_j` sends a monomial with exponent vector `a` to `1` if `a = 0`, `ε_j` if `a = δ_j`, and `0` if `|a| ≥ 2`, so its `ε_N`-component is the `E_N`-coefficient.  **Trap:** the ring must satisfy `ε_i ε_j = 0` for *all* pairs, `i = j` included; with independent duals satisfying only `ε_j² = 0` the cross terms survive and the identification fails on exactly the monomials it exists to kill, while staying true on every monomial one would test first. |
...
|---|---|---|
| `CharClass/ChernRelation.lean` | Chern classes from a `PowerBasis` (Leray–Hirsch as pure algebra), `γ_k = e_k(roots)` in the split case, Whitney sum via an injective base change | **coefficient-generic already in spirit**; the file is the **single authority** on the root convention and this report points at it rather than restating it (program note §1.5 "Ruling on the roots"; the `∏(X + y_k)` polynomial is satisfied by the classical roots under the dual class).  Its "over `ZMod 2` signs are invisible" sentence has been replaced by `sp-lh` with the two-identity justification. |
...
1. **Relative homotopy invariance over `F_p`.**  The single blocking item for the odd side.
   **Scoping corrected 2026-09-10:** it is consumed by **`hsq` at `k = 1`**, not by the
   `k`-zero argument.  §4.1's Half B is retired by `sp-oddside-n`'s chart design (the local
   groups are identified with the model directly, never transported along `ρ`), and Half A
   needs only relative *naturality* plus *absolute* homotopy invariance, which is already
   generic.  And `LIXHsq` has **two** `F₂`-only calls, not one: `relPullback_eq_of_homotopy_of_line`
   **and** `relPullback_lixKHomeo_eq_id` (via `relPullback_eq_id_of_line`, "an injective
   self-map of an `F₂`-line is the identity", false over `F_p`).  The second is covered by the
   same theorem — a linear automorphism carries the punctured set to itself, so a path to the
   identity is a homotopy of pairs — but it needs that path.  **Do not reach for
   path-connectedness of `GL_r(ℂ)`:** `lixKLin` (`LIXHsqLinear.lean:193`) is built from a
   corner unitary through two coordinate equivalences, so an explicit path for *that* map is
   far cheaper than the general theorem, and the lead struck the analogous `U(n)` connectedness
   from the powers side for the same reason.
...
* **The lead, needs a ruling (2026-09-10, raised 16:10):** **three lanes name three
  different spellings of `ψ_k`, and two do not typecheck where a third applies them.**
  `sp-powers` owns two scalars — `joinC` (wall on the positive real axis) and `joinCneg`
  (negative wall, `= (−1)^k joinC k`, because `Complex.arg` cuts on the non-positive reals and
  their clamp saturates there) — and two vector forms `joinPow`/`joinPowNeg` built from
  `planeSub`.  `sp-oddside` uses the **scalar** `joinC` as
  `Function.update x 0 (joinC k (x 0))` and recorded that `joinPow` is **not usable** at
  `Fin (n+1) → ℂ`, since `planeSub` wants `[InnerProductSpace ℝ E]` and the LIX odd sphere is a
  sup-normed pi type without one.  `sp-endpoint`'s docstring names `joinPowNeg` applied at
  exactly that type.  One lane (naturally `sp-powers`) should fix one spelling and one wall and
  the rest point at it, as `ChernRelation` now is for the roots.  The choice has content:
  `sp-oddside-n`'s `κ_j` local sections depend on evaluating `arg` at `−w` to keep the cut off
  the target, so a wall flip is not free for them.
...
* **A symbolic sign check must name the ring it was run in, in the same breath as its
  conclusion.**  `sp-coeff` verified that `φ` fails to commute with `P` at `p = 2` — a check
  run in `ℤ`, where the two sides differ by `2h^p`, while the classes live in `F_p`, where they
  agree.  `ℤ` and `F_p` disagree on exactly the statements this program turns on (`−1 = 1` at
  `p = 2`, and `p = 0`), so a sign check in the wrong ring is not a weaker check, it is a check
  of a different claim.
* **The fleet has machinery against false greens and none against false reds.**  That check
  erred toward *caution*, so it produced a false negative, and a false negative here looks
  exactly like diligence: it says "this does not hold" about something that does, and nobody
  pushes back on a colleague being careful.  Every protocol in §3 of the program note —
  Built-never-Replayed, the transitive purge, the job-count rule — is aimed at the other
  direction.  Ask of a caution what you would ask of a green: in what ring, at which prime,
  against which statement.
...
## Lemma list for `sp-oddside-n` (Step C at general rank `n`, `F₂`)

Same split as §4.5.  The engine is already rank-free; **only the geometry moves**, and the
rank enters the whole Step C chain through exactly two definitions.

**The two definitions the rank enters through.**

```text
   lixRank n dd     = (∑ⱼ dⱼ) + (n+1)          -- was  + 3
   lixTopDegree n dd = 2·(∑ⱼ dⱼ) + 2·(n+1)      -- was  + 6
```

with `lixTopDegree = 2·lixRank` unchanged (`1 + (2n+1) + 2∑dⱼ = 2((∑dⱼ)+n+1)`).  Everything
in `LIXStepCOdd*` is stated over `2 * lixRank dd` and mentions no other numeral, so **those
files are reused with `lixRank` redefined** — a parameter change, not new mathematics.

**Reused verbatim** (rank-free as they stand):

| declaration | why |
|---|---|
| `LIXKCount.*` — `LocalSplit`, `LocalClassesAgree`, `map_eq_nsmul_of_localSplit`, `map_ne_zero_of_localSplit` | abstract over a field and a finite index set; the counting engine never sees `n` |
| `LemmaTwoZero.mtSection_manuscript_eq_zero_iff` | abstract in `(a,b,c)` and in the index types `ι, κ` |
| `LIXSectionPath.unitVectors_seam_eq_zero_iff` | abstract in the index type |
| `RelativeSupport.lixHexact`, `lixHnat`, `lixHsection`, `lixS_comp_lixPi` | generic in the subspace and the maps |
| `ThomStepCEuler.*`, `ThomStepCSection.*`, `ThomSectionDetect.*`, `ThomStepCOddLocal.*` | stated over abstract `N`, `twoR`, `z` |
| `LIXThomClassTerm`, `lixThomClassTerm_ne_zero`, `LIXBundlePair.*` | the Thom class does not know where the section vanishes |
| `Ψ_k` and its zero set (`sp-design` §2.1, `sp-oddside`'s `LIXKMap`) | `Function.update x 0 (joinC k (x 0))` is already written at `Fin (n+1) → ℂ`; the `k+1` roots of `−1` in the `x₀`-line are rank-agnostic |

**Needs a parameter** (same mathematics, `n` threaded):

| declaration | change |
|---|---|
| `LIXSectionLocalHomeo.lixTopDegree`, `LemmaTwoTopClass.lixRank` | the two displayed formulas |
| `LemmaTwoParity.trace_Vmat` (`= (∑dⱼ) + 3`), `trace_mappingTorus` | `+ (n+1)`; the `1 : Matrix (Fin 3) (Fin 3)` block is `sp-tower`'s `Gen` `VIdx`/`Vmat` |
| `LIXLocalPairClosed.two_le_two_mul_lixRank` (proved from `3 ≤ lixRank`) | `n+1 ≤ lixRank`, still trivial |
| `LIXLocalPair.lixTrivSet/lixTrivBall`, `lixLocalPairIsoClosed` | indexed by the zero, and by `lixRank n` |
| `LemmaTwoStepC.lixZero`, `LemmaTwoGlue.lixZeroPoint`, `lixSection_eq_zero_iff` | the zero set, `a = e₁ = Pi.single 0 1` in `Fin (n+1) → ℂ` |
| `LIXStepCOddRelative.lixJ`, `lixI` | the punctured set |
| `LIXStepCOddLocal/Thom/Wired`, `LIXKStepC*` | `2 * lixRank n dd` throughout; binder list unchanged |

**New** (in dependency order):

1. **The chart, generically in the pole.**  This is §1.3's recommendation and `sp-oddside`
   already has the ruling.  Fix a unit vector `a : Fin (n+1) → ℂ` with `Re (a n) = 0` and an
   ℝ-linear isometry `L : ℝ^{2n+1} ≃ {ξ : Re ⟪ξ,a⟫ = 0}`; then
   `chart_a w = L w − √(1−‖w‖²)·a`, with `chart_a 0 = −a`, and
   `trivialBlockChart (w,u) = (1−τ)a + τ·chart_a w` has strict derivative
   `(dw,du) ↦ ½·L dw − du·a` at the origin, an isomorphism because `L` is one onto `a^⊥_ℝ`
   and `a ∉ a^⊥_ℝ`.  At `n = 2`, `a = e₃`, `L = chartLinearMap` this is
   `LIXSectionChart.eulerLinearModel` character for character, so the mod-2 instance is an
   instantiation and stays green.  `LIXSectionDeriv.lean:79` already factors through the
   coordinate-free identity, which is why this costs one definition rather than a rewrite.
2. **The radial clamp and the chart inverse at general `n`.**  `LIXSphereChart`'s clamp is
   index-free; `LIXSphereChartHomeo`'s `sphereLower = {Re z₂ < 0}` becomes
   `{z : Re ⟪z, a⟫ < 0}` and `sphereChartInv` becomes `L⁻¹` composed with the projection.
   The key identity `1 − ‖w‖² = (Re ⟪z,a⟫)²` is the same computation.
3. **`unitVectors (Fin (n+1)) ≃ₜ S^{2n+1}`.**  `LemmaTwoStepCAbsEquiv.nonempty_absEquiv_lix`
   consumes `unitVectorsThreeHomeoSphere`; the general-`n` homeomorphism is the only new
   input to the top line.  Everything else in `absEquiv_lixN` is a Künneth count.
4. **`H^{2r}(N; F₂) ≅ F₂` at rank `n`** — the top line over `S¹ × S^{2n+1} × ∏ℂP^{dⱼ}`.
   Needs `sp-lh`/`sp-thom`'s cohomology of `S^{2n+1}` in place of the hard-wired `S⁵`.
5. **`PuncturedAcyclic (lixN dd) (2·lixRank n dd) z` at rank `n`, at an arbitrary point.**
   The recursion of `ThomPuncturedRecursion` over Künneth factors, with `S^{2n+1}` for `S⁵`.
   The "arbitrary point" part is §4.5(5) and is needed at every `z_i`, not only at `lixZero`.
6. ~~**`k`-point punctured vanishing**~~ — **not new work** (`sp-oddside-n`, and my §4.5(3)
   overstated it): `LIXKPunctured.isZero_punctured_finite` is already abstract in
   `[T1Space N]` and in the dimension, so a new rank needs only its single hypothesis,
   `puncturedAcyclic` at the rank-`n` base.

**Signed off (2026-09-10): `sp-oddside-n`'s chart design supersedes items 1–2 above.**
Rather than differentiate `Ψ_k` at each zero, chart the sphere at `z_j` so that `Ψ_k` is the
**identity in the charts**, using the explicit local section of `joinC k` on `{Re w < 0}`

```text
   κ_j(w) = |w|·exp( i·(arg(−w) + π + 2πj)/(k+1) ) ,   joinC k (κ_j w) = w ,  κ_j(−1) = ζ_j .
```

VERIFIED (model): both identities, exponents `≤ 8`, 60 random points each, `0` failures.  The
`arg` is evaluated at `−w`, which is what keeps the cut away from the target `−1`.  Then the
section read in `chart_j` is literally the `k = 1` file's `trivialBlockChart`, the same map at
every `j`, and **no derivative of `Ψ_k` is ever taken**.  Three riders:

* **`κ_j = ζ_j · κ_0` exactly**, with `κ_0(w) = |w|·exp(i·arg(−w)/(k+1))` carrying no `j`
  (VERIFIED (model), same range).  Build one chart and rotate; the rotation is where the
  `ρ`-orbit reappears, demoted from a property of the geometry to a description of an open
  set.
* **Trap:** `Complex.arg` has range `(−π, π]`, so a target sector straddling the negative
  reals has no `arg`-interval description.  For `k` **even** the exponent `k+1` is odd and one
  root is exactly `−1` (at `k+1 = 1,3,5,7` it is the branch `j = k/2`), dead centre on the
  cut.  `target_j = ζ_j • target_0` dodges it; an `arg`-interval does not.
* **This retires the local half at every prime and the global half at none.**  §4.1's Half A
  compares the *global* maps `j_i`, which no chart can supply; at `p = 2` neither half is
  needed, so the `F₂` deliverable is unaffected either way.

**And the factor moves where the theorem wants it.**  In the naive route `k+1` sits in the
local derivative (`det J = −(k+1)/2^{2n+1}`, confirmed independently at ranks 2, 3, 4 by
`sp-oddside-n` and at rank 2 by `sp-oddside`, matching `diag(k+1,1,…,1)` with the `½` from
`τ = 1/2` on `2n+1` dimensions).  In the chart route the local derivative is the `k = 1` one
and the factor appears instead in the **number of zeros** — which is §1.3.4's "no local degree
is ever computed", made literal rather than moral.

**What does not appear on this list, and should not.**  Nothing about `ρ`, the isotopy, or
homotopy invariance: at `F₂` the local-classes obligation is discharged by "an `F₂`-line has
one nonzero element" (Review of `sp-oddside`), and the `ρ`-transport is only needed over
`F_p`.  Keep `LocalClassesAgree` an abstract hypothesis so the two discharges can differ.

## Lemma list for `sp-evenside-n` (Step D at general even rank `n`, `F₂`)

**The numeral dictionary.**  Four literals, three of which move:

```text
   3  ↦  n+1        rank V = (∑ⱼ dⱼ) + (n+1)          (lixRank, trace_Vmat, the γ index)
   6  ↦  2(n+1)     deg z = deg(t·x) = 1 + (2n+1)      (sq_b's threshold, lixTopDegree, hdeg)
   5  ↦  2n+1       the odd sphere's degree             (σ₅, xClass, HasSphereCohomology)
   1  ↦  1          the CIRCLE stays degree 1           (σ₁, tClass) — do NOT generalise it
```

**The hypothesis that is invisible at `n = 2` and must be added: `Even n`.**  The conclusion
`γ_r(W) = 0` needs `r` odd, and `r = m + n + 1`.  `Even (dⱼ)` gives `Even m`, so `r` is odd
**iff `n` is even**.  At `n = 2` this is free and the existing statement records only
`Even m`.  It is exactly `p ∣ n` at `p = 2`, i.e. §1.4's hypothesis.  Add it to
`WuStepDData.gamma_top_eq_zero` and to `stepD_of_wu`, and say in the docstring that it is the
`p = 2` case of the general condition.

**Reused verbatim** (rank-free):

| declaration | why |
|---|---|
| `ParityEven.ParityData.b_odd_eq_zero` | the induction is about `N` odd; the rank only chooses at which `N` it is applied |
| `ParityEven.ParityData.sum_a_mul_b_eq_zero` | the convolution identity in odd total index; its *derivation* uses `sq_b` at the moved threshold, its *statement* does not |
| `WuDiagonal.SqData.wu_diagonal`, `SqData.sq_gamma_succ` | no rank |
| `WuSymmetric.esymm_halfAntidiagonal_eq`, `sum_antidiagonal_self_eq_zero` | pure symmetric functions |
| `ParityEvenSlice.sliceClass_eq_mul_self`, `sliceClass_coeff_odd_eq_zero`, `sliceClass_coeff_eq_zero_of_lt`, `even_sum_of_forall_even` | only `Even (dⱼ)`; the slice decomposition `∏ⱼ(1+hⱼ)^{dⱼ}` is **unchanged in form** |
| `Wu.HasSplitting`, `Wu.CartanTotal` | quantified over the space |
| `LemmaTwoStepD.ne_zero_of_degree_ne_zero` | a statement about one graded piece |

**Needs a parameter** (the numeral dictionary, and nothing else):

| declaration | change |
|---|---|
| `ParityEven.ParityData.sq_b` | `2*k < j + 6` ⟶ `2*k < j + 2*(n+1)` |
| `ParityEven.ParityData.gamma_top_eq_zero`, `gamma_top_eq_zero_of_slice` | index `m + 3` ⟶ `m + n + 1`, plus `Even n` |
| `ParityInstance.xClass`, `hdeg` (`2*k = c + 6`), the two `HasSphereCohomology` binders, `gamma_top_eq_zero_of_*` | `5` ⟶ `2n+1`, `6` ⟶ `2(n+1)`, index `+3` ⟶ `+(n+1)` |
| `LemmaTwoStepD.WuStepDData` (`σ₅`, `sq_b`, the `b` docstring "degree `2k − 6`"), `gamma_top_eq_zero`, `stepD_of_wu` | same dictionary |
| `LemmaTwoParity.trace_Vmat`, `LemmaTwoTopClass.lixRank`, `LIXSectionLocalHomeo.lixTopDegree` | shared with `sp-oddside-n`; **one lane must own each**, and the natural owner is `sp-tower`'s `Gen` shape layer |

**New**: only `Even n` and its threading.  There is no new mathematics on this side.

**The overlap to settle before either lane authors.**  `sp-evenside` is proving the uniform
theorem generic in `p` in the `ParityData` shape, and **its `p = 2` instance is exactly this
deliverable**: my §3.2 reads "`b_N = 0` for `N ≡ 1 mod p`, hence `γ_r(W) = 0` when
`r ≡ 1 mod p`", and at `p = 2` that is "`b_N = 0` for `N` odd, hence `γ_r = 0` when `r` odd",
with `r = m+n+1` odd ⟺ `Even n`.  So `sp-evenside-n` should **instantiate
`ParityPData` at `p = 2`** and prove the two agree, not re-derive the `F₂` argument at general
`n`.  If the lead wants the `F₂` bridge landed before `sp-steenrod`'s operations exist, the
instance still works: `ParityPData` needs no odd-primary input at `p = 2`, where `P^i` is
`Sq^{2i}` and the tree already has it.  Two lanes proving one theorem is the thing to avoid
here.

## Two rulings (2026-09-10, third pass)

### The Chern-root sign convention at odd `p`: immaterial, and here is why

`sp-lh` proposed keeping the tree's root convention; `sp-coeff` declined to wave it through
at odd `p`.  **The convention itself is now ruled on once, in the program note §1.5 "Ruling on
the roots" (15:50), and this report does not restate it** — over `F₂` nothing can distinguish
the readings, so a restatement here would diverge silently and first show up as a mod-`p` Wu
relation.  What follows is only the **transfer** argument — that a statement proved in either
convention carries to the other — which is itself convention-independent and is the part this
lane owns.

Let `φ` be the ring automorphism of `H^*(Y; F_p)` induced by `h_i ↦ −h_i` (it exists because
`H^*(∏ ℂP^{d_i})` is generated by the degree-2 classes; there is no such map on a general
space, so this is a statement about *this* `Y`).

* **`φ` commutes with the total `P`.**  `P(φ h) = P(−h) = −P(h) = −h − h^p`, and
  `φ(P h) = φ(h + h^p) = −h + (−h)^p = −h − h^p` for `p` odd.  Both are ring maps, so they
  agree on the algebra generated by the `h_i`.  At `p = 2`, `φ = id`.
* **The Wu relations are covariant.**  With `c'_j := φ(c_j) = (−1)^j c_j`:
  `P(c'_j) = (−1)^j P(c_j) = (−1)^j E_j(c)`, while a monomial of `E_j` at weight
  `j + i(p−1)` picks up `(−1)^{j+i(p−1)}` under `φ`, which is `(−1)^j` **because `p−1` is
  even**.  So `E_j(c') = (−1)^j E_j(c) = P(c'_j)`.  The covariance is not generic: it holds
  because every weight component of `E_j` picks up the *same* sign, and that is `(p−1)` even
  again — the same fact `sp-steenrod`'s §3.3 uses twice and §6.1 once.
* **The Frobenius step is covariant.**  `φ(γ(V)) = ∏(1−h_i)^{d_i} = ∏(1 − h_i^p)^{d_i/p}`,
  still supported in weights `≡ 0 mod p`, so L1/L3a transform term by term.
* **The conclusion is convention-free.**  `φ(B_m) = (−1)^m B_m`, so `γ_r(W) = 0` says the
  same thing in both conventions, and the certificates are unchanged.

**Update (`sp-lh`, 2026-09-10): the choice is free, and `φ` is the licence for exercising
it.**  Which class the relation is written against, and hence what the roots are, is settled
in the program note §1.5 "Ruling on the roots" (lead, 15:50, on `sp-evenside`'s check); this
report does not restate it.  The two consequences that belong here: the earlier reading was an
artefact of the class the tree happened to use, not a necessity, and is retired; and on the
split locus the `i`-th factor now dies by `neg_add_cancel` rather than by `add_self`, which is
a genuine site where characteristic two was doing real work.

**But the switch is a change of convention, not an escape from one**, and `φ` is exactly it:
`φ(c_j) = e_j(−a) = (−1)^j e_j(a)`.  So the two clauses above are its licence, one per half of
the tree — at `p = 2`, `φ = id`, so the `F₂` layer does not see the switch and stays green
without re-proof; at odd `p`, covariance transfers anything proved in either convention term
by term.  `sp-lh` had recorded `φ` as answering a counterfactual; it answers the live
question.

**The two sign identities are uniform in `p` (`sp-coeff`, refined 2026-09-10).**  The
covariance rests on exactly two identities, **read in `F_p` and not in `ℤ`**:

```text
   (−1)^p = −1                and                (−1)^{i(p−1)} = 1 .
```

Both hold at **every** prime — at odd `p` because the relevant exponent is odd resp. even,
at `p = 2` because `−1 = 1` — and those are **different reasons, neither an instance of the
other**.  VERIFIED (model): both identities, `p ∈ {2,3,5,7,11,13}`, `i ≤ 11`, zero failures.

**Which identity does which job**, because otherwise a reader cannot tell why there are two.
`φ` acts on weight `w` by `(−1)^w`.  So:

* `(−1)^p = −1` is what makes `φ` commute with the **total** `P` on the degree-2 generators
  (`P(φh) = −h − h^p` against `φ(Ph) = −h + (−h)^p`), hence everywhere, both being ring maps;
* `(−1)^{i(p−1)} = 1` is what makes `φ` commute with each **graded piece** `P^i`
  (`P^i(φx) = (−1)^w P^i x` against `φ(P^i x) = (−1)^{w+i(p−1)} P^i x`) — equivalently, what
  makes every weight component of `E_j` scale by the same `(−1)^j`.  This is the one the Wu
  covariance uses, and it does double duty.

**Do not collapse this to one sentence.**  "Every sign that appears is `+1` in `F_p`" is one
step away from "over `ZMod 2` signs are invisible", which is the sentence this whole exchange
exists to delete, and a reader who inherits the uniform form without the two reasons will read
it as the old claim wearing a hat.  State the identities as what is being verified, then the
two reasons as their proof.

**Ruling:** either convention, used consistently across the Chern layer, the splitting
principle and the even side.  I recommend keeping the mod-2 one, because it makes the `F₂`
instance a re-elaboration rather than a re-proof and the `F₂` surface must stay green.  The
thing to write down where the convention is fixed is **not** "signs are invisible" but "the
change of convention is the automorphism `h ↦ −h`, which commutes with `P` because `p−1` is
even" — the first sentence is what was true at `F₂` and is not a reason at odd `p`.

### `sp-endpoint`'s four questions

**(1) The instance pair — they are right and my §(4) draft cited the wrong one, and the tree
contains a hazard worth naming.**  There are **two** declarations of the spectral pair:
`GroupApproximation.instSpectralPartialOrder` (`LIXEndpointStatement.lean:66,69`, which that
file's own docstring tells consumers to re-register) and
`GroupApproximation.LIX.instSpectralPartialOrder` (`LIXLimitMatrixTransport.lean:37,40`),
and **eight** files re-register the second at priority `100000`.  That is precisely the
"fresh pair, definitionally equal but not the same term" `LIXEndpointStatement`'s docstring
forbids, present in the tree.  It has not bitten because `HasK1InjWitness` is a `def` whose
body is fixed at elaboration, so consumers need only definitional equality (`exact`), never
syntactic (`rw`).  **Use the `LIX`-namespaced pair at priority `100000`, matching the eight
consumers.**  And note the Palomar files are immune either way: the existing surface uses the
inline `letI : PartialOrder A := CStarAlgebra.spectralOrder A` and neither named pair, which
is what the new surface should do too.

**(2) `LemmaTwoHolds n p k` is over-parameterised — `p` does not belong in the statement.**
They are right and the program note §1.6 and my §(4) are both wrong here.  The topological
input is `¬ ContinuousMvNEquiv FHmat_k EHmat` at every stage; `p` appears nowhere in it.  `p`
enters only in the *discharge*: Step C needs `p ∤ k`, Step D needs `p ∣ n`.  So the shapes are

```lean
def LemmaTwoHoldsPow (n k : ℕ) : Prop :=
  ∀ j, LIX.Gen.LemmaTwoFor n (Gen.lixDD n j) (fun m => suspPsi k (Gen.bVec n m))

theorem lemmaTwoHoldsPow_of (n k p : ℕ) (hp : p.Prime) (hpn : p ∣ n) (hpk : ¬ p ∣ k) :
    LemmaTwoHoldsPow n k

theorem lixLimit_hasK1InjPowerWitness_of (n : ℕ) [NeZero n]
    (h : ∀ k : ℕ, (∃ p : ℕ, p.Prime ∧ p ∣ n ∧ ¬ p ∣ k) → LemmaTwoHoldsPow n k) :
    HasK1InjPowerWitness (LIX.Gen.LIXLimit n) n
```

Their observation that `Gen.LemmaTwoFor` already abstracts the section slot is the reason
this costs nothing.  **One correction to their sketch:** they wrote
`Gen.bVec n (suspPsi k m)`, applying the suspension to the *base point*; it should be
`suspPsi k (Gen.bVec n m)`, applying it to the *sphere vector* — `b = x ∘ Σψ_k` means
`b(m) = Ψ_k(x(m))`.  `Ψ_k` preserves the norm, so `bVec_normSq` survives, which the
zero-locus lemma needs.  This is the same one-character composition-order slip as §1.3.2's
pre- versus post-composition; it is worth a named lemma `norm_suspPsi` beside the definition.

**(3) Keep `Nontrivial A`.**  It is derivable (`IsSimpleRing` is `IsSimpleOrder` on the
two-sided ideals, which extends `Nontrivial`), and it should stay: a challenge is read
against the printed problem, which names nontriviality, and the existing surface itself keeps
it in `not_all_simple_unital_k1Injective`.  One `⟨…⟩` against one lookup by every reader.

**(4) The `k = 1` derivation, approved as scoped.**  `Nat.exists_prime_and_dvd (n ≠ 1)` then
`Nat.eq_one_of_dvd_one`; on the **solution** side only, with a docstring in the challenge.
The challenge must stay exactly three theorems or the Comparator configuration changes.

### Correction to the sign ruling (`sp-evenside`, and one retraction of my own)

**`sp-evenside` is right: my proposed docstring sentence is false at `p = 2`.**  "The
automorphism `h ↦ −h` commutes with `P` because `p − 1` is even" is vacuous at `p = 2`, where
`p − 1 = 1` is odd.  Worse, the covariance *argument* fails there too: the weight components
of `E_j` scale by `(−1)^{j+i(p−1)}`, which at `p = 2` is `(−1)^{j+i}` and is **not** uniform
in `i`.  The conclusion holds at `p = 2` for an unrelated reason.  So the sentence I wanted to
replace ("over `ZMod 2` signs are invisible") and the sentence I offered are each true at
exactly the primes the other is false at, which is the worst possible pair to choose between.

Their replacement covers both and I accept it, with one tightening.  At `p = 2` the sharp
statement is not "`−1 = 1`" but **`φ = id`**: the automorphism induced by `h ↦ −h` in
characteristic two *is* the identity, so there is no change of convention to justify.  The
form to ship:
...
**The single source.**  The convention is fixed where the splitting principle produces the
roots, i.e. `CharClass/ChernRelation.lean`'s `∏(X + y_k)` and `chernClass_eq_esymm` — and
that file's docstring is the one that currently carries "over `ZMod 2` … signs are invisible
anyway".  `sp-lh` owns its `K`-generic successor.  Every other layer, `sp-evenside`'s
`PowerData` included, should **point at it** and not restate it.  `PowerData` is already
compliant: it takes the roots abstractly and asks only `P^1 y = κ y^p`, so it fixes nothing.

**My retraction.**  I told `sp-evenside` that under `φ` the `(W)` leading coefficient "picks
up `(−1)^{ip+1} = −1` and stays a unit".  That is wrong twice.  `(−1)^{ip+1} = (−1)^{i+1}` for
`p` odd, not `−1`; and the coefficient is in fact **invariant**: applying `φ` to
`P^i(γ_{i+1}) = u_i γ_{ip+1} + D` gives `(−1)^{i+1}` on the left and `(−1)^{ip+1}` on the
right, whose ratio is `(−1)^{i(p−1)} = 1` for `p` odd (and `φ = id` at `p = 2`).  So the
identity is invariant as it stands and `u_i` does not move.  The `(W)` field must still carry
a **unit** — that requirement comes from `sp-steenrod`'s `κ` normalisation and is unaffected —
but it does **not** get a second, independent justification from the convention change, and I
should not have offered one.
