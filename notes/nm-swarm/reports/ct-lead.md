...
- **Binder 4 lake finding (w1-binder-4, landed f3be2844b and 13776be09); CORRECTED ~18:10, the earlier text overclaimed:**
  - In the lake, the OUTER cycle of the pocket walk does not follow its boundary (Configuration B). So dgo-analytic's `PocketRegion.ofNoncrossingClosedWalk`, whose builder asks for outer following, cannot build the region there.
  - Retracted: "no PocketRegion carries U", "EmptyTwoGonInput cannot fire" and "module 4 must change route". `PocketRegion.outer` is an `IsDiscRegion` of the reclosed complement (connected, Euler characteristic preserved), not a following condition.
  - A `PocketRegion` very likely exists in the lake: the inner cycle follows, and the Euler equality comes from `reclosed_euler` on the reversed walk. So debt-conditional's module 4 need not change route.
  - `TwoGonMergeDiscInput` / `false_of_mergeDisc` (13776be09) stay valid and weaker, but are not required.
  - w1-binder-4 is building a `PocketRegion` builder from a noncrossing walk whose INNER cycle follows: inner from `toDiscRegion_of_followsBoundary`, outer from `toDiscRegion_of_euler` with `reclosed_euler` on `w.reverse.map alpha`.
- **Instances for chain-matricial (ms-compress-3, 59f433288 `Dynamics/ClopenCrossedProductAlgebra`):** `ClopenCoeff.instAlgebra`, `smul_algebraMap` and `instSMulCommClass` give `Algebra k R_X`, `IsScalarTower k R_X R_X` and `SMulCommClass k R_X R_X`, which `IsLocallyMatricialIn` needs. Consume these rather than adding competing instances. Also `ChainCore.TransientIdealLocallyMatricialStatement` (tex 1476 at Y = CR(T)).
- **Duplicate-name risk:** ct-involution's landed `Dynamics/TransientSupport.lean` declares `exists_wandering_clopen_cover`, and chain-matricial's module 5 plans the same name. Check namespaces before module 5 lands or either is wired.
