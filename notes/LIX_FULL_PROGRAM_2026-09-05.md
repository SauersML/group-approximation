...
| dualization duplicate (20:40 EDT) | THREE copies of the `Hom(−,F₂)`-exactness argument: `RelativeDual` (cc-relative), the `cohDualFunctor` block of `CohomologyMayerVietoris` (cc-cohom-api), `MayerVietorisDual` (cc-thom-s, 3c331f766); ruling: first green wins, the other two delete their copies and import it. REVERSED 21:20 EDT on consumer reality: cc-thom's fourteen-module MV stack (`MayerVietorisDual`/`Biproduct`/`Element`/`Vanishing`/`Sequence`, namespace `CharClass.MV`, green as a unit at 8744 jobs, with the consumers `MVSequence` and A1) is CANONICAL; cc-cohom-api's `CohomologyMayerVietoris` (green 8768 but consumer-less) is retired; cc-relative imports cc-thom's dual layer and deletes `RelativeDual`. Still owed by cc-thom for Leray–Hirsch: δ-linearity for global classes and naturality in a map of covered spaces (needs the missing chain-level map between subordinate-chain complexes) |
