...
| dualization duplicate (20:40 EDT) | THREE copies of the `Hom(−,F₂)`-exactness argument: `RelativeDual` (cc-relative), the `cohDualFunctor` block of `CohomologyMayerVietoris` (cc-cohom-api), `MayerVietorisDual` (cc-thom-s, 3c331f766); ruling: first green wins, the other two delete their copies and import it. DECIDED 20:16 EDT: cc-cohom-api's `CohomologyMayerVietoris` green at 8768 jobs (1b9601b02), two minutes before cc-thom's `MayerVietorisDual`; the dual layer moves to `CharClass/CohomologyDual.lean` and both other lanes import it |
