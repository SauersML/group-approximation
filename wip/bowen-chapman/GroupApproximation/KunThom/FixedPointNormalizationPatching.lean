...
* `card_hammingDisagreement_patch_le`: `patch` disagrees with a permutation
  ...
  disagrees with `w`.
* `card_commutationDefect_patch_le`: for an ambient labelled action together
  with labelled actions on the blocks, `patch` fails to commute with a label
  only off the glued domain, at a point the label moves off the domain, where
  the block action and the ambient action disagree, or at a point of the
  equivariance defect of its block arrow.
...
/-- An ambient labelled action together with labelled actions on the blocks.
The block actions need not agree with the ambient action;
`BlockAction.compatFailure` records where they do not. -/
structure BlockAction {Y : FiniteModel} {I : Type u} (E : BlockEmbedding Y I)
    (L : Type*) where
  /-- The ambient action of the labels. -/
  act : L → Equiv.Perm Y
  /-- The action of the labels on a block. -/
...
/-- Block points at which the block action and the ambient action disagree. -/
noncomputable def compatFailure (C : I) (s : L) : Finset (E.model C) := by
  classical
  exact Finset.univ.filter fun x ↦
    E.embed C (A.blockAct C s x) ≠ A.act s (E.embed C x)

theorem embed_blockAct_of_not_mem {C : I} {s : L} {x : E.model C}
    (hx : x ∉ A.compatFailure C s) :
    E.embed C (A.blockAct C s x) = A.act s (E.embed C x) := by
  by_contra h
  exact hx (by simp [compatFailure, h])

/-- All compatibility failures of a label, read in the ambient model. -/
noncomputable def globalCompatFailure [Fintype I] (s : L) : Finset Y :=
  Finset.univ.biUnion fun C ↦ (A.compatFailure C s).image (E.embed C)

theorem embed_mem_globalCompatFailure [Fintype I] {C : I} {s : L}
    {x : E.model C} (hx : x ∈ A.compatFailure C s) :
    E.embed C x ∈ A.globalCompatFailure s := by
  simp only [globalCompatFailure, Finset.mem_biUnion, Finset.mem_univ, true_and,
    Finset.mem_image]
  exact ⟨C, x, hx, rfl⟩

theorem card_globalCompatFailure_le [Fintype I] (s : L) :
    (A.globalCompatFailure s).card ≤ ∑ C, (A.compatFailure C s).card :=
...
theorem card_labelDefect_le {L : Type*} [Fintype L] (A : BlockAction E L)
  ...
  · intro x hx
    exact Finset.mem_coe.mpr (Finset.mem_filter.mp (Finset.mem_coe.mp hx)).2
  · intro x _ z _ hxz
    exact Subtype.ext (congrArg Prod.snd hxz)

/-- **Patched commutation.**  A patched permutation fails to commute with a
label only off the glued domain, at a point the label moves off the domain,
at a point where it or its image meets a compatibility failure of the block
actions, or at a point of the equivariance defect of its block arrow. -/
theorem card_commutationDefect_patch_le {L : Type*} [Fintype L]
  ...
  have hmoved : ((t ∩ β.domain).filter fun y ↦ ¬ A.act s y ∈ β.domain).card ≤
    ...
    · intro y hy
      ...
      exact Finset.mem_coe.mpr (Finset.mem_sdiff.mpr ⟨Finset.mem_univ _, hy'.2⟩)
    · intro y _ z _ hyz
      exact (A.act s).injective hyz
  set K := (t ∩ β.domain).filter fun y ↦ A.act s y ∈ β.domain with hK
  have hfilterK := Finset.card_filter_add_card_filter_not
    (s := K) (fun y ↦ y ∈ F ∨ β.patch y ∈ F)
  -- the point or its patched image meets a compatibility failure
  have hfail : (K.filter fun y ↦ y ∈ F ∨ β.patch y ∈ F).card ≤ 2 * F.card := by
    have hsub : (K.filter fun y ↦ y ∈ F ∨ β.patch y ∈ F) ⊆
        F ∪ F.image β.patch.symm := by
      intro y hy
  ...
  have hkept : (K.filter fun y ↦ ¬ (y ∈ F ∨ β.patch y ∈ F)) ⊆
    ...
    intro y hy
    rw [hK, Finset.mem_filter, Finset.mem_filter, Finset.mem_inter] at hy
    obtain ⟨⟨⟨hyt, hyd⟩, hayd⟩, hnot⟩ := hy
    obtain ⟨⟨C, x, hx⟩, rfl⟩ := β.mem_domain.mp hyd
    have hxF : x ∉ A.compatFailure C s := fun hmem ↦
      hnot (Or.inl (A.embed_mem_globalCompatFailure hmem))
    have hcompat : A.act s (E.embed C x) = E.embed C (A.blockAct C s x) :=
      (A.embed_blockAct_of_not_mem hxF).symm
    ...
    refine ⟨Finset.mem_univ _, fun hx₁ hsx₁ hsame ↦ ?_⟩
    dsimp only at hx₁ hsx₁ hsame
    have happF : (β.arrow _).apply x hx₁ ∉
        A.compatFailure (β.objEquiv _) s := fun hmem ↦ by
      apply hnot
      refine Or.inr ?_
      change β.patch (E.embed _ x) ∈ F
      rw [β.patch_embed _ x hx₁]
    ...
    rw [hcompat, β.patch_embed _ _ hsx₁, β.patch_embed _ x hx₁, hsame]
    exact A.embed_blockAct_of_not_mem happF
  have hkeptCard : (K.filter fun y ↦ ¬ (y ∈ F ∨ β.patch y ∈ F)).card ≤
      (β.labelDefect A s).card :=
    (Finset.card_le_card hkept).trans Finset.card_image_le
  have hlabel := β.card_labelDefect_le A s
  omega

/-- Normalized form of `card_hammingDisagreement_patch_le`. -/
theorem hammingDistance_patch_le (w : Equiv.Perm Y) :
    hammingDistance Y β.patch w ≤
      (((Finset.univ \ β.domain).card + (β.arrowDisagreement w).card : ℕ) : ℝ) /
        Fintype.card Y := by
  rw [hammingDistance]
  gcongr
  exact_mod_cast β.card_hammingDisagreement_patch_le w

/-- Normalized form of `card_commutationDefect_patch_le`. -/
theorem hammingDistance_commutator_patch_le {L : Type*} [Fintype L]
    (A : BlockAction E L) (s : L) :
    hammingDistance Y (β.patch * A.act s) (A.act s * β.patch) ≤
      ((2 * (Finset.univ \ β.domain).card + 2 * (A.globalCompatFailure s).card +
        ∑ C, ((β.arrow C).equivarianceDefect (A.blockAct C)
          (A.blockAct (β.objEquiv C))).card : ℕ) : ℝ) / Fintype.card Y := by
  rw [hammingDistance]
  gcongr
  exact_mod_cast β.card_commutationDefect_patch_le A s
