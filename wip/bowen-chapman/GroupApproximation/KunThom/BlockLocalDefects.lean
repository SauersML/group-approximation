...
it over many blocks counts every failure once per block.  Two landed modules
charge a bridge only with the failures on its own block:
`BlockEmbedding.card_equivarianceDefect_bridge_le_local` in
`StepNineHammingDefects`, with `BlockEmbedding.bridgeLabelFailure`, and
`RelativeFunctorEstimate.card_equivarianceDefect_bridge_le_local` and
`RelativeFunctorEstimate.card_equivarianceDefect_symm_bridge_le_local`, with
`RelativeFunctorEstimate.conjFailure`.  This file sums those bounds over a set
`good` of blocks matched by a map `π` injective on `good`.  Every conjugation
failure is then counted once, and the total is a Hamming disagreement.
...
* `card_hammingDisagreement_inv_mul_eq`: `q⁻¹` fails to carry `a'` back to `a` at
  as many points.
* `BlockEmbedding.sum_card_bridgeLabelFailure_le_hamming`,
  `BlockEmbedding.sum_card_bridgeLabelFailure_inv_le_hamming` and
...
`A'.act s * q`. -/
theorem sum_card_bridgeLabelFailure_le_hamming [Fintype I] {L : Type*}
  ...
  E.sum_card_bridgeLabelFailure_le q A A' s
...
theorem sum_card_bridgeLabelFailure_inv_le_hamming [Fintype I] {L : Type*}
    (E : BlockEmbedding Y I) (q : Equiv.Perm Y) (A A' : BlockAction E L) (s : L) :
    ∑ C, (E.bridgeLabelFailure q⁻¹ A' A C s).card ≤
      (hammingDisagreement (q * A.act s) (A'.act s * q)).card :=
  (E.sum_card_bridgeLabelFailure_le q⁻¹ A' A s).trans
...
theorem sum_card_conjFailure_inv_comp_le_hamming [Fintype I] {L : Type*}
  ...
  calc ∑ i ∈ good, (RelativeFunctorEstimate.conjFailure E q⁻¹ A' A (π i) s).card
      = ∑ C ∈ good.image π, (RelativeFunctorEstimate.conjFailure E q⁻¹ A' A C s).card :=
        (Finset.sum_image hπ).symm
    ...
    _ = (hammingDisagreement (q * A.act s) (A'.act s * q)).card :=
        card_hammingDisagreement_inv_mul_eq q (A.act s) (A'.act s)
...
theorem sum_card_equivarianceDefect_bridge_le_matched [Fintype I] {L : Type*}
  ...
  have hlabel : ∀ s : L, ∑ i ∈ good, ((E.bridge q i (π i)).sourceDefect +
        (A.compatFailure i s).card + (E.bridgeLabelFailure q A A' i s).card +
    ...
    have h₂ : ∑ i ∈ good, (E.bridgeLabelFailure q A A' i s).card ≤
        ∑ C, (E.bridgeLabelFailure q A A' C s).card :=
      Finset.sum_le_sum_of_subset (Finset.subset_univ good)
    have h₃ := E.sum_card_bridgeLabelFailure_le_hamming q A A' s
    have h₄ : ∑ i ∈ good, (A'.compatFailure (π i) s).card ≤
        ∑ C, (A'.compatFailure C s).card := by
      classical
      calc ∑ i ∈ good, (A'.compatFailure (π i) s).card
          = ∑ C ∈ good.image π, (A'.compatFailure C s).card := (Finset.sum_image hπ).symm
  ...
  calc ∑ i ∈ good, ((E.bridge q i (π i)).equivarianceDefect (A.blockAct i)
      ...
      ≤ ∑ i ∈ good, ∑ s : L, ((E.bridge q i (π i)).sourceDefect +
          (A.compatFailure i s).card + (E.bridgeLabelFailure q A A' i s).card +
          (A'.compatFailure (π i) s).card) :=
        Finset.sum_le_sum fun i _ ↦ E.card_equivarianceDefect_bridge_le_local q i (π i) A A'
