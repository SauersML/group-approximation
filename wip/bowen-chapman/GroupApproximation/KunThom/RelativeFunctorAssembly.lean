...
presentations with a `ClusterMetric`.
...
noncomputable def transportRep
    (himprove : ∀ (X Y : R) (b : P.Rep X.1 Y.1), ∃ r : Q.Rep (obj X).1 (obj Y).1,
      (MQ.val r).twoSidedDisagreement (sandwich (bridge X) (bridge Y) (MP.val b)) < ρ X)
...
theorem transportRep_close
    ...
    (MQ.val (transportRep MP MQ obj bridge ρ himprove b)).twoSidedDisagreement
      (sandwich (bridge X) (bridge Y) (MP.val b)) < ρ X :=
...
noncomputable def relativeMorphism
    ...
    (hrespects : ∀ {X Y : R}, P.Rep X.1 Y.1 →
      2 * ρ X + β X + β Y + MP.radius X.1 ≤ 8 * MQ.radius (obj X).1)
    (hcomp : ∀ {X Y Z : R}, P.Rep X.1 Y.1 → P.Rep Y.1 Z.1 →
      2 * ρ X + ρ Y + 2 * β X + 2 * β Y + 2 * β Z + 2 * MP.radius X.1 + MP.radius Y.1 +
        MQ.radius (obj X).1 ≤ 8 * MQ.radius (obj X).1) :
...
theorem relativeFunctor_faithful
    ...
    (hfaithful : ∀ {X Y : R}, P.Rep X.1 Y.1 →
      2 * ρ X + 5 * β X + 5 * β Y + 2 * MP.radius X.1 + MQ.radius (obj X).1 ≤
        8 * MP.radius X.1) :
