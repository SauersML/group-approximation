...
ms-traces-2), `EnclosedSubdiagramLoopCutSuccStatement` (ms-traces-2 alone, waiting on ms-compress-2's A2 arc lemmas; w1-binder-8 on the partial-row tracker), `OsinLemma94ClassCaseGapSpanStatement` (ct-bilateral-cell,
ct-return-tower), `OsinTwoGonCleanCopySectionStatement` (w1-binder-1 and ms-inverses-2; reduces to six step Props a7e2c216e, 73835dd09, kept whole until a step is produced), `CellPocketOuterPinchStepSectionDistinctStatement` (w1-binder-7),
`CellPocketWalkProperArcsBelowSectionStatement` (ms-cite-1, routing binder 5 through `GeodesicCollarEulerStatement` and w1-binder-6's `PocketCellTransportEulerStatement`), `OsinSectionPocketFaceSetProperSectionStatement` (w1-binder-6, w1-binder-3,
