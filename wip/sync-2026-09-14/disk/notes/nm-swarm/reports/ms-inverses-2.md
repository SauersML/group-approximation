...
- CLAIM GroupApproximation/GGT/VanKampen/Estimating/OsinAppendixEulerTwoGonDoublingInner.lean: the CellSide and RegionPair steps through
  `FaceEdgeDoubling`. Split files `OsinAppendixEulerTwoGonDoublingInner*`.
- ~~CLAIM Estimating/OsinAppendixEulerTwoGonDoublingOuter.lean~~ dropped. At w1-binder-1's request (second split, agreed 10:4x), the Spur,
  OuterCell, OuterSide and CellHair steps are its own, over this lane's generic transport. Mine are CellSide, RegionPair and the generic
  transport.
