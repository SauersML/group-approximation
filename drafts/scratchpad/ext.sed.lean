...
theorem rel_of_mem {p : (H →L[ℂ] H) × Ultrapower σ ω} (hp : p ∈ Gr) : GraphRel τ Gr p.1 p.2 := by
...
theorem rel_unique (htr : ∀ p ∈ Gr, ultratrace σ ω p.2 = ⟪τ.vec, p.1 τ.vec⟫_ℂ)
    {T : H →L[ℂ] H} {v w : Ultrapower σ ω} (h1 : GraphRel τ Gr T v) (h2 : GraphRel τ Gr T w) : v = w := by
...
theorem rel_mul (hGM : ∀ p ∈ Gr, p.1 ∈ M) {T S : H →L[ℂ] H} {v w : Ultrapower σ ω}
    (hTM : T ∈ M) (hT : GraphRel τ Gr T v) (hS : GraphRel τ Gr S w) : GraphRel τ Gr (T * S) (v * w) := by
  obtain ⟨B, hT⟩ := hT
  obtain ⟨B', hS⟩ := hS
...
theorem rel_star (hGM : ∀ p ∈ Gr, p.1 ∈ M) {T : H →L[ℂ] H} {v : Ultrapower σ ω} (hTM : T ∈ M)
    (h : GraphRel τ Gr T v) : GraphRel τ Gr (star T) (star v) := by
  obtain ⟨B, h⟩ := h
  refine ⟨B, fun ε hε ↦ ?_⟩
