...
/-!
# Strict negativity under a full `B`-stack

The downward kill of `EntrywiseKill` stops at degree `1`, giving the
nonpositive window.  But the same induction step applies verbatim at
degree `0`: the degree-`(+1)` strip equation pairs the `s`-part of the
pencil against the *balanced* components of the inverse entries, and
the scalar left inverse of `[B₀; B₁]` kills them as well.  So a full
`B`-stack pins every inverse entry into a strictly negative window —
the input to the padded reshaping exit of the session-54 discharge.
-/
...
/-- **Strict negativity**: a scalar left inverse of the column stack
...
theorem entry_window_negative_of_B_full
    ...
    ∃ N : ℕ, ∀ (j : κ) (i : ι),
      (family k).wordT (C.word j) *
        ...
        (family k).wordS (R.word i) ∈
      Submodule.span k ((family k).degreeMonomials (-(N : ℤ)) (-1)) := by
  classical
