...
theorem eD_eA (hp : 1 < p) (hodd : Odd p) (hchar : (p : K) = 0) (k : ℕ) :
  ...
  | succ k ih =>
      have hY1 : eD K (eY1 K k) = eA K k := by
        have h := eD_eH_add K (eA K k)
      ...
      have hY2 : eD K (eY2 K k) = eMid K k := by
        have h := eD_eH_add K (eMid K k)
...
theorem eX_succ (hp : 1 < p) (k : ℕ) :
  ...
  have hshift : ((eT K) ^ (p - 1)) (eWedge K 0 (((eR K) ^ k) (eBot K)))
  ...
  rw [hRHS, eN_eWedge_zero_one K hp hZinv, eX, ← Module.End.mul_apply, ← pow_succ']
...
theorem eTopWord_update_zero (hp : 1 < p) :
  ...
  · subst hj
    ...
    symm
    simp only [eIntervalWord, decide_eq_false_iff_not, not_and, not_lt, Fin.val_zero]
    omega
...
theorem eDescent_value (m : ℕ) (hm : p = 2 * m + 1) (hp : 1 < p) :
  ...
  have hstart : (eA K m) (eHeadWord p) = ePair K (eA K m) (eX K 0) := by
  ...
  rw [hstart, eA, ePair_eRound_pow K hp m (eTop K) 0, Nat.add_zero, eTop,
    ...
    ePre_zero_of_card, pow_zero, one_mul, eTopWord_update_zero K hp,
...
theorem eDescent_value_isUnit (m : ℕ) (hprime : p.Prime) (hm : p = 2 * m + 1) :
  ...
  intro hcon
  rw [CharP.cast_eq_zero_iff (ZMod p) p _] at hcon
  have := (Nat.Prime.dvd_factorial hprime).1 hcon
  omega
...
theorem eDescent_value_of_eval (ev : EMod K p → K) (hev : EvalIsHeadCoeff K ev)
  ...
  rw [hev, eDescent_value K m hm hp]
