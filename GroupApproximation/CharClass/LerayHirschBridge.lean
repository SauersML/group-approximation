...
The ladder is run on `lhDomainOf K X r n`, the subgroup of `Fin r`-indexed families
...
bijection onto `lhDomainOf K`, and `lhMap_eq_lhSum` says the two Leray–Hirsch
...
def lhExtend (X : TopCat.{0}) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) :
    lhDomainOf K X r n :=
...
theorem lhExtend_apply_of_lt (X : TopCat.{0}) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) {j : Fin r}
    (h : (j : ℕ) < lhDomainCard r n) :
    ((lhExtend X r n d : (i : Fin r) → Hmod K X (n - 2 * (i : ℕ))) j)
      = d ⟨(j : ℕ), h⟩ :=
  dif_pos h
...
theorem lhExtend_apply_of_ge (X : TopCat.{0}) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) {j : Fin r}
    (h : ¬ ((j : ℕ) < lhDomainCard r n)) :
    ((lhExtend X r n d : (i : Fin r) → Hmod K X (n - 2 * (i : ℕ))) j) = 0 :=
  dif_neg h

/-- Restrict a column element to the present summands. -/
def lhRestrictFun (X : TopCat.{0}) (r n : ℕ) (c : lhDomainOf K X r n) :
    (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ)) :=
  fun i => (c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ)))
...
theorem lhExtend_lhRestrictFun (X : TopCat.{0}) (r n : ℕ) (c : lhDomainOf K X r n) :
...
theorem bijective_lhExtend (X : TopCat.{0}) (r n : ℕ) :
    Function.Bijective (lhExtend (K := K) X r n) := by
  constructor
  · intro d d' h
...
def lhTermCard (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) (k : ℕ) :
    Hmod K P n :=
...
theorem lhTermCard_of_lt (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) {k : ℕ}
...
theorem lhTermCard_of_ge (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) {k : ℕ}
    (h : ¬ k < lhDomainCard r n) : lhTermCard π ξ r n d k = 0 :=
  dif_neg h

theorem lhMap_eq_sum_range (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) :
    lhMap π ξ r n d
...
theorem lhSum_lhExtend_eq_sum_range (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) :
...
theorem lhMap_eq_lhSum (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
    (d : (i : Fin (lhDomainCard r n)) → Hmod K X (n - 2 * (i : ℕ))) :
...
theorem lhMap_eq_comp (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ) :
    lhMap π ξ r n = (fun c => lhSum π ξ r n c) ∘ lhExtend X r n :=
  funext fun d => lhMap_eq_lhSum π ξ r n d

theorem bijective_lhMap_of_lhSum (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
...
theorem bijective_lhSum_of_lhMap (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ)
...
theorem lerayHirschGraded_of_bijective (π : P ⟶ X) (ξ : Hmod K P 2) (r : ℕ)
...
theorem bijective_lhSum_of_graded {π : P ⟶ X} {ξ : Hmod K P 2} {r : ℕ}
