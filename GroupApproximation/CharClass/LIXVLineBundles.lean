...
* `trivLineY`, `hLineY`, `vLineY` — the lines over the projective base.
* `isStarProjection_vLineY`, `continuous_vLineY` — the two obligations, separately.
...
def hLineYblock (b : HBlk dd) (y : baseY dd) : Matrix (HIdx dd) (HIdx dd) ℂ :=
...
def vLineY (b : Fin 3 ⊕ HBlk dd) (y : baseY dd) : Matrix (VIdx dd) (VIdx dd) ℂ :=
  ...
  | Sum.inr b => Matrix.fromBlocks 0 0 0 (hLineYblock b y)

theorem vLineY_eq (b : Fin 3 ⊕ HBlk dd) (m : baseM dd) : vLineY b m.2 = vLine b m := by
  cases b <;> rfl
...
theorem isStarProjection_hLineYblock (b : HBlk dd) (y : baseY dd) :
    IsStarProjection (hLineYblock b y) := by
  constructor
  · show hLineYblock b y * hLineYblock b y = hLineYblock b y
    rw [hLineYblock, ← Matrix.blockDiagonal'_mul]
  ...
  · show star (hLineYblock b y) = hLineYblock b y
    rw [Matrix.star_eq_conjTranspose, hLineYblock, Matrix.blockDiagonal'_conjTranspose]
    refine congrArg Matrix.blockDiagonal' (funext fun b' => ?_)
    by_cases h : b' = b
    · rw [if_pos h]
...
theorem isStarProjection_vLineY (b : Fin 3 ⊕ HBlk dd) (y : baseY dd) :
  ...
  | inl i =>
      ...
      constructor
      · show vLineY (Sum.inl i) y * vLineY (Sum.inl i) y = vLineY (Sum.inl i) y
        ...
        · simp
        · simp
      · show star (vLineY (Sum.inl i) y) = vLineY (Sum.inl i) y
  ...
  | inr b =>
      ...
      constructor
      · show vLineY (Sum.inr b) y * vLineY (Sum.inr b) y = vLineY (Sum.inr b) y
        show Matrix.fromBlocks (0 : Matrix (Fin 3) (Fin 3) ℂ) 0 0 (hLineYblock b y) *
            Matrix.fromBlocks 0 0 0 (hLineYblock b y) = _
        rw [Matrix.fromBlocks_multiply]
        refine Matrix.fromBlocks_inj.mpr ⟨?_, ?_, ?_, ?_⟩
        · simp
        · simp
        · simp
        · simpa using hP.isIdempotentElem.eq
      · show star (vLineY (Sum.inr b) y) = vLineY (Sum.inr b) y
        ...
        · exact Matrix.conjTranspose_zero
        · exact Matrix.conjTranspose_zero
        · have : star (hLineYblock b y) = hLineYblock b y := hP.isSelfAdjoint.star_eq
          rwa [Matrix.star_eq_conjTranspose] at this

/-! ## 3. Continuity -/

theorem continuous_hLineYblock (b : HBlk dd) :
    Continuous (hLineYblock b (dd := dd)) :=
...
theorem continuous_vLineY (b : Fin 3 ⊕ HBlk dd) : Continuous (vLineY b (dd := dd)) := by
  cases b with
...
@[simp] theorem coe_vLineBundle (b : Fin 3 ⊕ HBlk dd) :
    ⇑(vLineBundle b) = vLineY (dd := dd) b := rfl
