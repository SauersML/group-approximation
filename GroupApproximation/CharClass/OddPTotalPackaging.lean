...
def toEven (hp : 2 ≤ p) : EvenReducedPowers p where
  ...
  mul_single X q hq z ν hz := by
    ...
    have hz' : ptotOf R.D X q z = ν • TotalHOf.of (ZMod p) X q z := by
      rwa [coe_Ptot, coe_evenOf, R.totalPtot_of, R.piecePtot_apply, if_pos hq2] at hz
    refine ⟨R.μinv ^ (q / 2) * ν, fun i w => ?_⟩
    rw [Gen.coe_evenRestrictAdd, Gen.coe_evenRestrictAdd, Subring.coe_mul, coe_evenOf]
    exact R.totalP_mul_single X hp hq2 z ν hz' i w.2
  zero_one X := by
    apply Subtype.ext
    rw [Gen.coe_evenRestrictAdd, OneMemClass.coe_one, ← TotalHOf.of_one (ZMod p) X,
      R.totalP_of_eq X rfl 0 (le_refl 0), R.ptotOf_one, ← TotalHOf.of_one (ZMod p) X,
      show 0 + 2 * 0 * (p - 1) = 0 by simp, projDeg_of, if_pos rfl]
    simp
  zero_cp M c := by
    apply Subtype.ext
    rw [Gen.coe_evenRestrictAdd, coe_evenOf, R.totalP_of_eq (CPtop M) rfl 0 (by norm_num),
      projDeg_apply, show 2 + 2 * 0 * (p - 1) = 2 by simp, R.component_ptotOf,
      dif_pos ⟨by omega, by omega⟩, TotalHOf.of_cohCast,
      R.of_D_congr (CPtop M) 2 (show p * 2 - 2 = 2 * (p - 1) by omega), R.zero_cp, smul_smul,
      show 2 / 2 - 0 = 1 by norm_num, pow_one, R.μinv_mul, one_smul]
  degree_two X c := by
    apply Subtype.ext
    rw [Gen.coe_evenRestrictAdd, SubmonoidClass.coe_pow, coe_evenOf,
      R.totalP_of_eq X rfl 1 (by norm_num), projDeg_apply, R.component_ptotOf,
      dif_pos ⟨by omega, by omega⟩, TotalHOf.of_cohCast,
