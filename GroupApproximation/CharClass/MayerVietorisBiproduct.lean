...
`(Functor.map id) = id` and whose rewrite pattern is `fun x => id <$> x`.  These four
lemmas are cc-thom-local (prefixed `mvDualMap_`) so that they cannot collide with
anything `cc-cohom-api` may later add to `CohomologyMayerVietoris.lean`. -/

theorem mvDualMap_comp {K L M : ChainComplex (ModuleCat.{0} (ZMod 2)) ℕ}
    (f : K ⟶ L) (g : L ⟶ M) : dualMap2 g ≫ dualMap2 f = dualMap2 (f ≫ g) := by
...
theorem mvDualMap_id (K : ChainComplex (ModuleCat.{0} (ZMod 2)) ℕ) :
    dualMap2 (𝟙 K) = 𝟙 (dualCx2 K) := by
  show dualCxFunctor.map (𝟙 K).op = 𝟙 _
  rw [CategoryTheory.op_id, CategoryTheory.Functor.map_id]

theorem mvDualMap_add {K L : ChainComplex (ModuleCat.{0} (ZMod 2)) ℕ} (f g : K ⟶ L) :
...
theorem mvDualMap_zero {K L : ChainComplex (ModuleCat.{0} (ZMod 2)) ℕ} :
    dualMap2 (0 : K ⟶ L) = 0 := by
  have h := mvDualMap_add (0 : K ⟶ L) 0
  rw [add_zero] at h
  have h3 : (0 : dualCx2 L ⟶ dualCx2 K) + dualMap2 (0 : K ⟶ L)
      = dualMap2 (0 : K ⟶ L) + dualMap2 (0 : K ⟶ L) := by rw [zero_add, ← h]
  exact ((add_left_inj (dualMap2 (0 : K ⟶ L))).mp h3).symm
...
theorem mvCxInclU_projU (U V : Opens X) :
  ...
  rw [mvCxInclU, mvCxProjU, mvDualMap_comp, biprod.inl_fst, mvDualMap_id]

theorem mvCxInclV_projV (U V : Opens X) :
    mvCxInclV U V ≫ mvCxProjV U V = 𝟙 (dualCx2 (mvCx V)) := by
  rw [mvCxInclV, mvCxProjV, mvDualMap_comp, biprod.inr_snd, mvDualMap_id]

theorem mvCxInclU_projV (U V : Opens X) : mvCxInclU U V ≫ mvCxProjV U V = 0 := by
  rw [mvCxInclU, mvCxProjV, mvDualMap_comp, biprod.inr_fst, mvDualMap_zero]

theorem mvCxInclV_projU (U V : Opens X) : mvCxInclV U V ≫ mvCxProjU U V = 0 := by
  rw [mvCxInclV, mvCxProjU, mvDualMap_comp, biprod.inl_snd, mvDualMap_zero]
...
theorem mvCx_total (U V : Opens X) :
  ...
  rw [mvCxProjU, mvCxInclU, mvCxProjV, mvCxInclV, mvDualMap_comp, mvDualMap_comp,
    ← mvDualMap_add, biprod.total, mvDualMap_id]
