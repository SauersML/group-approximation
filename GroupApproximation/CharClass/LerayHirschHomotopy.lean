...
variable {K : Type} [CommRing K] {X Y P Q : TopCat.{0}}
...
theorem bijective_colPull_of_bijective (f : Y ⟶ X) (r n : ℕ)
    (hb : ∀ m : ℕ, Function.Bijective (pull (K := K) f m)) :
    Function.Bijective (colPullOf (K := K) f r n) := by
  constructor
  · intro c c' h
...
theorem lhTerm_pull_of_coh (f : Y ⟶ X) (g : Q ⟶ P) (πP : P ⟶ X) (πQ : Q ⟶ Y)
    (hsq : ∀ (m : ℕ) (a : Hmod K X m), pull g m (pull πP m a) = pull πQ m (pull f m a))
    (ξ : Hmod K P 2) (n i : ℕ) (a : Hmod K X (n - 2 * i)) :
...
theorem bijective_lhSum_of_htpy {X X' P P' : TopCat.{0}}
    ...
    (hbX : ∀ m : ℕ, Function.Bijective (pull (K := K) eX m))
    (hbP : ∀ m : ℕ, Function.Bijective (pull (K := K) eP m))
    (hsq : ∀ (m : ℕ) (a : Hmod K X' m), pull eP m (pull f' m a) = pull f m (pull eX m a))
    (ξ : Hmod K P 2) (ξ' : Hmod K P' 2) (hξ : pull eP 2 ξ' = ξ) (r n : ℕ)
    (h : Function.Bijective (lhSum f' ξ' r n)) :
    Function.Bijective (lhSum f ξ r n) := by
  have key : ∀ c : lhDomainOf K X' r n,
      pull eP n (lhSum f' ξ' r n c) = lhSum f ξ r n (colPullOf eX r n c) := by
    intro c
    rw [lhSum_pull_of_coh eX eP f' f hsq ξ' r n c, hξ]
  have hcomp : (fun c => lhSum f ξ r n c) ∘ (colPullOf (K := K) eX r n)
      = (pull eP n) ∘ (fun c => lhSum f' ξ' r n c) :=
    funext fun c => (key c).symm
  have hb : Function.Bijective ((fun c => lhSum f ξ r n c) ∘ (colPullOf (K := K) eX r n)) := by
    rw [hcomp]
...
theorem bijective_pull_of_homotopyEquiv {A B : Type} [TopologicalSpace A]
    ...
    Function.Bijective (pull (K := K) (cmap e.toFun) n) :=
  (pullEquivOfHomotopyEquivOf K e n).bijective
