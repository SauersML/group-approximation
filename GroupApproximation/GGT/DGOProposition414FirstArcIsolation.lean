import GroupApproximation.GGT.DGOProposition414FirstArcConnectorSeparation
...
/-!
# Full auxiliary-cycle isolation of inherited first-half targets
...
/-- An inherited first-half target is isolated in its entire auxiliary cycle,
including against component starts created by the cuts. -/
theorem firstGapArcSource_cycleIsolated
    ...
    IsIsolated D.fam (B.firstGapLocalLabel j ((B.firstGapLeft j).length +
      ...
      (B.firstGapCut j ((B.firstGapLeft j).length +
        (B.firstTargetSide s - B.firstGapStartSide j))) := by
  refine ⟨⟨_, B.firstGapArcComponentField j s hs⟩, ?_⟩
  intro q hne hop
  rw [B.firstGapLocalLabel_arc j s hs] at hop ⊢
  ...
  · exact B.firstGapArcSource_not_connected_left j s hs hleft hop
  · exact B.firstGapArcSource_not_connected_arc j s hs hr hne hop
  · exact B.firstGapArcSource_not_connected_right j s hs hr hop
  · exact B.firstGapArcSource_not_connected_chord j s hs hr hop

end GroupApproximation.GGT.DGOProposition414.BalancedSplitData

#audit_axioms GroupApproximation.GGT.DGOProposition414.BalancedSplitData.firstGapArcSource_cycleIsolated
