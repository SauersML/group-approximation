...
statement this module discharges down to Hull's Theorem 7.1 for a single
relator, which `GGT/HullSCTheorem51.lean` then discharges further to Hull's
Theorem 5.1.  It is Hull's
...
lifts finite order.  This is (5) applied to the family of (4).  The condition
`C(ε, μ, ρ)` is a definition with a body -- `HullSC.RelWord.IsSmallCancellation`
of `GGT/HullSCSmallCancellation.lean`, over `GGT.RelLetter` words, with
`GGT.RelGenSet.relBall` measuring the depth of the `H`-letters -- and the
theorem is `HullSC.HullQuotientStatement`.
...
is proved.  The statement is `HullSC.HullRelatorStatement`, and
`HullSC.hullTheorem51_of_relator_of_quotient` composes it with (6): the
parameters `ε, μ, ρ` come out of (6) and go into (7), which is the direction
Hull's proof runs.

**(8) Hull, Theorem 7.1 for `m = 1`.**  (6) with the relator of (7):
`HullSC.HullOneStepStatement`.  It carries three clauses beyond the
manuscript's `thm:hull`, all of them properties of Hull's construction rather
than additions to it: the alphabet of the quotient is the image of the alphabet
of `G`, the image of a prescribed suitable subgroup stays suitable, and the
kernel is the normal closure of one element.

**This is no longer a citation.**  `GGT/HullSCTheorem51.lean` splits it along
the seam of Hull's own proof and proves `hullOneStep_of_theorem51` from
`HullSC.HullTheorem51Statement` (the quotient of §5, with the relator `t⁻¹u`
for some `u ∈ N` chosen by §6) and `HullSC.ExistsHypEmbeddedInSuitable` (a
suitable subgroup contains a hyperbolically embedded one).  The two clauses of
(8) that mention `t` -- that `q(t) ∈ q(N)` and that the kernel is one element
-- are theorems there rather than citations: the relator dies, so
`q(t) = q(u)`, and `u` was chosen in `N`.
...
`GGT/HullSCTheorem51.lean` continues the reduction: `HullOneStepStatement` is
proved there from Hull's Theorem 5.1, so the leaves of the whole lane are
(3), (5), (6) and the existence of a hyperbolically embedded subgroup inside a
suitable one -- all of them statements of DGO or of Hull's §5, none of them
about the targets `t₁, …, t_m` or about kernels.
-/
