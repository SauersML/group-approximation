...
finite index, Lemma 6.7 turns long coarse overlap into a conjugate common power,
and Theorem 4.42 supplies the relative generating set.  Quasiconvexity,
properness, and the algebraic finish of geometric separation are derived rather
than retained as further inputs.
-/
...
/-- The two remaining conclusions of DGO Lemmas 6.5 and 6.7.  Lemma 6.5 is
retained in its published finite-index form.  For Lemma 6.7 only its geometric
core is retained: long coarse overlap forces a conjugate common power;
membership in `E(h)` is derived from that equation. -/
