...
/-!
# Balanced bisection that also lifts antiparallel pairs

`ClassifiedBisection` records one balanced cut: the two child polygons, their
class accounting, and the lifting of `ClassPair` from either child. Its
producer builds the children as the inner and outer cuts, whose transports lift
`OrientedClassPair` as well. That producer returns only the structure, so the
construction is repeated here with the two oriented lifting fields added.
-/
...
/-- A classified bisection whose children also lift antiparallel pairs. -/
structure OrientedClassifiedBisection (vs : ℕ → X) (sides : ℕ → ℝ → X)
    (n : ℕ) (δ : ℝ) extends ClassifiedBisection vs sides n δ where
  leftOrientedPair : ∀ A B β D, OrientedClassPair leftVertex leftSide (m + 2)
    (leftClass A) (leftClass B) β D → OrientedClassPair vs sides n A B β D
  rightOrientedPair : ∀ A B β D, OrientedClassPair rightVertex rightSide (n - m + 2)
    (rightClass A) (rightClass B) β D → OrientedClassPair vs sides n A B β D

private theorem classLength_eq_weightSum (vs : ℕ → X) (n : ℕ) (A : Set ℕ) :
    classLength vs n A =
...
/-- The construction of `exists_classifiedBisection`, with the oriented
transports of the inner and outer cuts as the two extra fields. -/
theorem exists_orientedClassifiedBisection {δ : ℝ}
    (hδ : IsHyperbolicSpace δ X) (hδ0 : 0 ≤ δ) (hgeo : IsGeodesicSpace X)
    {vs : ℕ → X} {sides : ℕ → ℝ → X} {n : ℕ}
    (hn : 9 ≤ n) (hpoly : IsClosedPolygonAt vs sides 0 n) :
    Nonempty (OrientedClassifiedBisection vs sides n δ) := by
  classical
  ...
  refine ⟨{
    ...
    leftOrientedPair := fun _ _ _ _ h => h.of_innerCut hpoly hm hbn hs ht
    rightOrientedPair := fun _ _ _ _ h => h.of_outerCut hpoly hm hbn hs ht
    firstLength := ?_, shortLength := ?_ }⟩
  · intro A
    rw [innerClass_eq_weightSum, innerClass_eq_weightSum, classLength_eq_weightSum]
    simpa only [Set.mem_preimage, mul_zero, add_zero] using
      hweights (fun i => if i ∈ A then 1 else 0) 0
  · intro B
    rw [innerClass_compl_eq_weightSum, innerClass_compl_eq_weightSum,
      classLength_eq_weightSum]
...
#audit_axioms GroupApproximation.Olshanskii.exists_orientedClassifiedBisection
