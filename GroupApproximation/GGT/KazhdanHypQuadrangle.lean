...
with `C` the **collinearity** table --- collinear points on the point side,
...
`((q+1)(q²+1), q(q+1), q-1, q+1)`, so `C² = (k-mu)I + (lam-mu)C + mu J'` reads

```
    C² = (q²-1) I - 2 C + (q+1) J' ,
```

`J'` the same-side all-ones table.  Completing the square at the **integer**
...
```
    (A - mu I)² ⪰ 0   ⟹   A ⪯ ((2q + mu²)/(2 mu)) I
```

finishes the certificate with `gap = 1 - (2q + mu²)/(2 mu (q+1))`.  The
threshold `gap > 1/2` is exactly `mu² - mu(q+1) + 2q < 0`, which has a rational
solution precisely when `(q+1)² > 8q`, that is `q ≥ 7`; and at `q = 8` one may
take the integer `mu = 4`, giving `gap = 5/9`.  No square root appears anywhere.

## The shape of the factor

Writing `P_W` for the projection off the constant vector and off the
bipartition sign, and `B_X = P_W (X - nu I)`, the identity proved below is

```
    L/d - gap P0  =  (1/(2 mu d)) B_A²  +  (1/(4 mu d q)) B_C²  +  ((1+s)/n) w wᵀ ,
```

with `d = q+1`, `s = 1 - gap`, `n = 2(q+1)(q²+1)`.  The `C` coefficients of the
first two blocks cancel --- `1/(2 mu d)` against `-2q/(4 mu d q)` --- which is
the whole reason two squares suffice, and the `J` and `w wᵀ` coefficients match
on the nose once `n = 2(q+1)(q²+1)`, the doubled point count of the quadrangle.

`sum_projSquare` is the one lemma both blocks use: for a symmetric `X` with
`X · 1 = xi · 1` and `X · w = eta · w`, the projected column
`B_X = P_W(X - nu I)` satisfies

```
    (B_X²)(v,v') = Sq(v,v') - 2 nu X(v,v') + nu² delta - (xi-nu)²/n
                     - ((eta-nu)²/n) sigma ,
```

where `Sq = X²` and `sigma = sign v * sign v'`.  Applying it at
`(X, nu, xi, eta) = (A, mu, d, -d)` and at `(C, q-1, k, k)` is the whole
computation.
...
order `q ≥ 7` is recorded in the sources consulted.
`GGT/KazhdanHypPolygonal.lean` carries that gap as `GQEightTableExists`.
-/
...
correction coefficients are exactly the ones that make the column annihilate
both the constant vector and the sign vector. -/
theorem sum_projSquare {V : Type u} [Fintype V] [DecidableEq V]
    (X Sq : V → V → ℚ) (sign : V → ℚ) (xi eta nu : ℚ)
    (hcard : (Fintype.card V : ℚ) ≠ 0)
...
`A² = (q+1) I + C` and the strong regularity of `C`; `card_eq` is the doubled
point count `(q+1)(q²+1)`.  `mu_bound` is Zuk's threshold in square-root free
form, and it is solvable exactly when `(q+1)² > 8q`. -/
structure QuadrangleLinkData (V : Type u) [Fintype V] [DecidableEq V] where
  ...
  /-- **`A² = (q+1) I + C`**: vertices on opposite sides have no common
  neighbour, distinct collinear points have exactly one. -/
  ...
  card_eq : (Fintype.card V : ℚ) = 2 * (ord + 1) * (ord ^ 2 + 1)
...
theorem two_mu_deg_pos : (0 : ℚ) < 2 * D.mu * D.deg :=
  mul_pos (mul_pos (by norm_num) D.mu_pos) D.deg_pos
...
def wCoeff : ℚ := (1 + D.contraction) / (Fintype.card V : ℚ)
...
theorem coeffA_nonneg : 0 ≤ D.coeffA :=
  le_of_lt (div_pos one_pos D.two_mu_deg_pos)

theorem coeffC_nonneg : 0 ≤ D.coeffC :=
  le_of_lt (div_pos one_pos D.four_mu_deg_ord_pos)
...
def colA (u v : V) : ℚ :=
  D.adj u v - D.mu * (if u = v then 1 else 0)
    - (D.deg - D.mu) / (Fintype.card V : ℚ)
    - ((-D.deg - D.mu) / (Fintype.card V : ℚ)) * (D.sign u * D.sign v)

/-- The projected column of the collinearity table, at the integer shift
`q - 1`. -/
def colC (u v : V) : ℚ :=
  D.col u v - (D.ord - 1) * (if u = v then 1 else 0)
    - (D.ord * D.deg - (D.ord - 1)) / (Fintype.card V : ℚ)
    - ((D.ord * D.deg - (D.ord - 1)) / (Fintype.card V : ℚ)) *
        (D.sign u * D.sign v)
...
theorem sum_colA (v v' : V) :
    ∑ u, D.colA u v * D.colA u v'
      = (D.deg * (if v = v' then 1 else 0) + D.col v v')
        ...
        - (D.deg - D.mu) ^ 2 / (Fintype.card V : ℚ)
        - ((-D.deg - D.mu) ^ 2 / (Fintype.card V : ℚ)) * (D.sign v * D.sign v') := by
...
theorem sum_colC (v v' : V) :
    ∑ u, D.colC u v * D.colC u v'
      = ((D.ord ^ 2 - 1) * (if v = v' then 1 else 0) - 2 * D.col v v'
        ...
        - (D.ord * D.deg - (D.ord - 1)) ^ 2 / (Fintype.card V : ℚ)
        - ((D.ord * D.deg - (D.ord - 1)) ^ 2 / (Fintype.card V : ℚ))
...
abbrev QuadRow (W : Type u) := (W × Fin 4) ⊕ ((W × Fin 4) ⊕ Fin 4)
...
`L/d - gap * P0 - Qᵀ Q = 0` entrywise, with `gap = gapValue` and
`Q = gramRow`.  The `col` coefficients of the two blocks cancel,
`1/(2 mu d)` against `-2q/(4 mu d q)`; the `delta` coefficients add to
`1 - gap`; and the constant and sign coefficients match once the vertex count
is `2(q+1)(q²+1)`. -/
...
theorem linkCertificateChecks_kgon_of_quadrangle
  ...
  · intro v
    have hsum : ∑ u, ((TriangularHodgeLayer.adjacencyCount T u v : ℕ) : ℚ)
        = ∑ u, Q.adj u v := Finset.sum_congr rfl fun u _ => hadj u v
    simp only [hsum, hdegval]
    exact Q.adj_col_sum v
  · intro v v'
    have hres := Q.gram_residual v v'
    simp only [hadj, hdegval]
    exact hres
...
/-! ## The arithmetic at `q = 8`, and why the hexagon is worse -/
...
`(q+1)² > 8q`.  At `q = 7` the discriminant is `17 > 0`. -/
theorem quadrangleDiscriminant_seven : (0 : ℚ) < (7 + 1) ^ 2 - 8 * 7 := by
  norm_num

/-- At `q = 5` the discriminant is negative, so no shift works: the smallest
admissible order is `7`, and `q = 6` is not a prime power. -/
...
largest non-principal eigenvalue `√(3q)`, so the threshold is `4 * 3q < (q+1)²`,
that is `q² - 10q + 1 > 0`, first met at `q = 11`. -/
theorem hexagonThreshold_eleven : (0 : ℚ) < 11 ^ 2 - 10 * 11 + 1 := by
  norm_num

/-- At `q = 9` the hexagon threshold fails. -/
theorem hexagonThreshold_nine : ((9 : ℚ) ^ 2 - 10 * 9 + 1) < 0 := by
  norm_num

/-- And by then the hexagon has `177156` points against the quadrangle's `585`:
the hexagon's threshold rises while its point count `(1+q)(1+q²+q⁴)` grows like
`q⁵`. -/
