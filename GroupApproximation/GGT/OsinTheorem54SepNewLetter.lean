...
**Provenance.**  This module is the content that commit `f6ebb0e25` landed as
`GGT/OsinTheorem54SepTransport.lean` and that a later landing of a *different*
file under that same path overwrote.  `GGT/OsinTheorem54SepFinite.lean` still
calls the two theorems below, so the overwrite left that module --- and with it
every module downstream of it, which is the whole of the separating-coset and
isolated-component towers --- referring to identifiers that no longer existed.
The mathematics is unchanged from `f6ebb0e25`; only the module name is, so that
the file now on `GGT/OsinTheorem54SepTransport.lean` keeps its own name and its
own consumers.
