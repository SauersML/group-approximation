...
/-- **The main reduction at the weakest boundary hypothesis**, stated at the
narrow waist.  The estimating inputs are used for one thing only — building
`GGT.VanKampen.RelativeGreendlingerQuasiGeodesicStatement` — so that statement
is the hypothesis here, and both routes to it are admissible: the historical
`relativeGreendlingerQuasiGeodesic_of_components` and the corrected
`relativeGreendlingerQuasiGeodesic_of_joint`, which avoids the refuted
`EstimatingGraphData` interface of issue #204 entirely.

This is a pure refactor: `..._of_components` below is the old theorem, with the
old name and the old hypothesis list, and every existing call site is
unaffected. -/
theorem relativeGreendlingerQuasiGeodesicSpellingStatement_of_greendlinger
    (hgreendlinger :
...
/-- The historical route to the reduction: est's three construction
propositions, the vk realization at a supplied spelling, and the supplied-word
arc conversion.  Unchanged in name, hypotheses and conclusion; it now factors
through the waist. -/
theorem relativeGreendlingerQuasiGeodesicSpellingStatement_of_components
    ...
    (hreal : RelativeDiscRealizationSpellingStatement.{u, w})
    (hconv : RelativeExteriorArcConversionAtWordRotatedStatement.{u, w}) :
    RelativeGreendlingerQuasiGeodesicSpellingStatement.{u, w} :=
  relativeGreendlingerQuasiGeodesicSpellingStatement_of_greendlinger
