...
**Theorem 5.3(a) is absent from this structure, and a consumer needing it must
build it.**  DGO's 5.3(a) is the free splitting `Rot = ∗_{c ∈ C'} G_c` for some
subset `C' ⊆ C`, and their Corollary 5.4 reads it as *"the normal subgroup of
`G` generated by `H` is a free product of a (usually infinite) family of
conjugates of `H`"*.  Nothing here carries that: the fields below are `ker_eq`,
`surjective` and the dichotomy 5.3(b).  The only free splitting in the corpus is
`GGT/DGOFreeSplittingOnePoint.lean`, which establishes it for the **one-point
model alone**, to close off a repair route — it is not the general clause.
This matters because 5.3(a) is what DGO's Theorem 2.35 actually consumes, via
7.19(e), to make the normal closure of `cⁿ` free; the dichotomy 5.3(b) is
7.19(d) and does not give it.
