...
The same three letters and the same two connectors, over rotations of the
formal inverse.  Two things change and neither is structural.  The pinning
inverts the relation before spending the design's clause ---
`HullSC.exponent_eq_of_mirroredPair_ball` --- because the letters of
`RelWord.revInv` carry inverses on both sides of it.  And the match handed back
is the SECOND of the adjacent pair rather than the first: the mirrored closer
wants its FIRST gap trivial, and the element `T` above is the second gap of one
match and the inverse of the first gap of the next.  One `T`, read from either
...
/-- **The far-side letter of a mirrored match sits at the same letter of the
relator.**

`HullSC.exponent_eq_of_pieceMatch` over rotations of the formal inverse.  The
letters carry inverses, so the relation the polygon returns carries them on
both sides, and the pinning is `HullSC.exponent_eq_of_mirroredPair_ball`, which
inverts it and spends the design's direct clause on the result.  That is the
one thing the mirrored half adds; the polygon is used in the same place and for
the same reason. -/
theorem exponent_eq_of_pieceMatch_revInv
  ...
  rw [ite_apply_eq a b] at hrel
  exact ⟨f, hf,
    exponent_eq_of_mirroredPair_ball hsymm hsep he hf hxb hx'b hrel, hletk⟩

/-- **A block letter of the mirrored piece whose match has a trivial FIRST
gap.**

The same three letters, the same two connectors, and the same element

    T = (Y · vertex 1 u (d+1))⁻¹ · vertex 1 u' k₀;

what differs is which of the two matches is handed back.  `T` is the second gap
of the match at `d`, and since `k₁ = k₀ + 1` it is also the INVERSE of the first
gap of the match at `d + 1`.  The mirrored closer wants a trivial first gap, so
the pair is read from the other end and the second match is returned. -/
theorem exists_match_with_trivialGap_revInv
    (hdisj : ∀ x : G, x ∈ D.fam false → x ∈ D.fam true → x = 1)
    (hnodup : ms.Nodup)
    (hinj : ∀ s : Bool, Function.Injective (fun n : ℕ => a s ^ n))
    (hsymm : ∀ g ∈ D.base, g⁻¹ ∈ D.base)
    ...
    (hsside : ∀ (t : Bool) (k m : ℕ),
      ...
      (GGT.OsinComponents.vertex (1 : G) u' k)⁻¹ *
        GGT.OsinComponents.vertex (1 : G) u' m ∉ D.fam t) :
    ∃ (d : ℕ) (b : Bool) (e k : ℕ), d < u.length ∧ e ∈ ms ∧
      ((RelWord.revInv (relatorWord₂ p (a false) (a true) ms)).rotate c)[d]?
        = some (GGT.RelLetter.comp b
            (((if b then a true else a false) ^ e)⁻¹)) ∧
      0 < k ∧ k ≤ u'.length ∧
      (∃ x : G, u'[k - 1]? = some (GGT.RelLetter.comp b x)) ∧
      ...
      (GGT.OsinComponents.vertex (1 : G) u' (k - 1))⁻¹ *
        (GGT.RelLetter.listVal py *
          GGT.OsinComponents.vertex (1 : G) u d) = 1 := by
  have hulen : u.length
      ≤ (RelWord.revInv (relatorWord₂ p (a false) (a true) ms)).length := by
    have hL := congrArg List.length hw
    rw [List.length_rotate, List.length_append] at hL
  ...
  have hvalu : ∀ (n : ℕ) (bb : Bool) (ee : ℕ), n < u.length →
    ...
    have hlu : u[n]? = some (GGT.RelLetter.comp bb
      ...
      exact (List.getElem?_append_left hn).symm
    have hmem : GGT.RelLetter.comp bb
        (((if bb then a true else a false) ^ ee)⁻¹) ∈ u :=
      List.mem_of_getElem? hlu
    have hlet : D.IsLetter (GGT.RelLetter.comp bb
        (((if bb then a true else a false) ^ ee)⁻¹)) :=
      hlet4 _ (List.mem_append_left _ (List.mem_append_left _
        (List.mem_append_right _ hmem)))
  ...
  obtain ⟨d, b₀, b₁, b₂, e₀, e₁, e₂, hd2, he₀, he₁, he₂, hl₀, hl₁,
      hl₂⟩ :=
    exists_three_blocks_of_long_prefix_rotate_revInv (h₀ := a false)
      (h₁ := a true) (c := c) hms hlong
  have hl₂' : ((RelWord.revInv
      (relatorWord₂ p (a false) (a true) ms)).rotate c)[d + 1 + 1]?
      = some (GGT.RelLetter.comp b₂
          (((if b₂ then a true else a false) ^ e₂)⁻¹)) := hl₂
  obtain ⟨k₀, hk₀0, hk₀, ⟨x₀, hx₀⟩, h₀, hh₀, hc₀⟩ :=
    hstep d b₀ e₀ he₀ (by omega) hl₀
  ...
  obtain ⟨k₂, hk₂0, hk₂, ⟨x₂, hx₂⟩, h₂, hh₂, hc₂⟩ :=
    hstep (d + 2) b₂ e₂ he₂ (by omega) hl₂
  have halt₀ : b₁ = !b₀ :=
    index_alternates_of_rotate_revInv hp0 (by omega) (by omega) hl₀ hl₁
  have halt₁ : b₂ = !b₁ :=
    index_alternates_of_rotate_revInv hp0 (by omega) (by omega) hl₁ hl₂'
  have hne : k₀ ≠ k₁ := by
    intro hk
    have hxk : u'[k₀ - 1]? = some (GGT.RelLetter.comp b₁ x₁) := by
      rw [hk]
      exact hx₁
    rw [hx₀] at hxk
    have hb : b₀ = b₁ := by
      have hinj₂ := Option.some.inj hxk
      rw [GGT.RelLetter.comp.injEq] at hinj₂
      exact hinj₂.1
    rw [halt₀] at hb
    revert hb
  ...
  rcases Nat.lt_or_ge k₀
      (RelWord.revInv
        (relatorWord₂ p (a false) (a true) ms)).length with hlt₀ | hge₀
  · obtain ⟨f₀, hf₀, hef₀, hlk₀⟩ := exponent_eq_of_pieceMatch_revInv hsymm
      hsep hpair hp0 hw hw' hpy hpz hpy0 hpz0 hlet4 hclose hqg hqside hsside
      he₀ (by omega) hk₀0 hk₀ hl₀ hx₀ hh₀ hc₀
    obtain ⟨f₁, hf₁, hef₁, hlk₁⟩ := exponent_eq_of_pieceMatch_revInv hsymm
      hsep hpair hp0 hw hw' hpy hpz hpy0 hpz0 hlet4 hclose hqg hqside hsside
      he₁ (by omega) hk₁0 hk₁ hl₁ hx₁ hh₁ hc₁
    rw [← hef₀] at hlk₀
    rw [← hef₁] at hlk₁
    have hadj : k₁ = k₀ + 1 :=
      farSide_adjacent_of_matches_revInv hinj' hnodup (by omega) (by omega)
        (by omega) (by omega) hk₀0 hk₁0 hlt₀ hl₀ hl₁ hlk₀ hlk₁
    have hk₁1 : k₁ - 1 = k₀ := by omega
    have hxk : u'[k₀]? = some (GGT.RelLetter.comp b₁ x₁) := by
      rw [← hk₁1]
      exact hx₁
    have hvk : (GGT.OsinComponents.vertex (1 : G) u' k₀)⁻¹ *
        GGT.OsinComponents.vertex (1 : G) u' k₁ = x₁ := by
      rw [hadj]
      exact span_eq_val_of_getElem? hxk
    have hvkm : x₁ ∈ D.fam b₁ := hslet _ (List.mem_of_getElem? hxk)
    have hgap := trivialGap_of_two_connectors hdisj halt₀ hh₀ hh₁ hc₀ hc₁ rfl
      (hvalu d b₀ e₀ (by omega) hl₀) hvk hvkm
    refine ⟨d + 1, b₁, e₁, k₁, by omega, he₁, hl₁, hk₁0, hk₁, ⟨x₁, hx₁⟩,
      ⟨h₁, hh₁, hc₁⟩, ?_⟩
    rw [hk₁1]
    exact inv_mul_eq_one.mpr (inv_mul_eq_one.mp hgap).symm
  · have hlt₁ : k₁ < (RelWord.revInv
        (relatorWord₂ p (a false) (a true) ms)).length := by omega
    obtain ⟨f₁, hf₁, hef₁, hlk₁⟩ := exponent_eq_of_pieceMatch_revInv hsymm
      hsep hpair hp0 hw hw' hpy hpz hpy0 hpz0 hlet4 hclose hqg hqside hsside
      he₁ (by omega) hk₁0 hk₁ hl₁ hx₁ hh₁ hc₁
    obtain ⟨f₂, hf₂, hef₂, hlk₂⟩ := exponent_eq_of_pieceMatch_revInv hsymm
    ...
    have hgap := trivialGap_of_two_connectors hdisj halt₁ hh₁ hh₂ hc₁ hc₂ rfl
      (hvalu (d + 1) b₁ e₁ (by omega) hl₁) hvk hvkm
    refine ⟨d + 2, b₂, e₂, k₂, by omega, he₂, hl₂, hk₂0, hk₂, ⟨x₂, hx₂⟩,
      ⟨h₂, hh₂, hc₂⟩, ?_⟩
    rw [hk₂1]
    exact inv_mul_eq_one.mpr (inv_mul_eq_one.mp hgap).symm
