...
/-- Relative Greendlinger and the weighted prefix-presentation transfer imply
Hull's canonical quotient statement.  The stronger `rho` threshold is the one
...
theorem hullLemma44CanonicalQuotientStatement_of_greendlinger_of_weightedTransfer
  ...
  let fullRadius : ℕ := max R 1
  let boundaryScale : ℕ := 2 * fullRadius + 2 * eps + 1
  let rho : ℕ := max rho₀ (max (8 * boundaryScale) (100 * (eps + 1)))
  have hrho₀ : rho₀ ≤ rho := Nat.le_max_left _ _
  have hrhoScale : 8 * boundaryScale ≤ rho :=
    le_trans (Nat.le_max_left _ _) (Nat.le_max_right _ _)
  have hrhoWeighted : 100 * (eps + 1) ≤ rho :=
    le_trans (Nat.le_max_right _ _) (Nat.le_max_right _ _)
  have hscalePos : (0 : ℝ) < (boundaryScale : ℝ) := by
    dsimp [boundaryScale, fullRadius]
    positivity
