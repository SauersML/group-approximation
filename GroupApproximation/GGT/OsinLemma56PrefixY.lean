...
## A radius here, a diameter in the paper
...
Two earlier readings of that `2` are withdrawn --- that it is the price of a seam
in the inheritance, and that it is slack.  The first priced a configuration that
does not arise; the second was a guess about a statement not visible from the
binder.
