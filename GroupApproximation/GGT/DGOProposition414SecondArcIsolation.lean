import GroupApproximation.GGT.DGOProposition414SecondArcConnectorSeparation
...
/-!
# Full auxiliary-cycle isolation of inherited second-half targets

The four opponent blocks exhaust every component start. Separation from each
block has already been transported to the source's isolation in its half word;
...
/-- An inherited second-half target is isolated in its entire auxiliary cycle,
including against component starts created by the cuts. -/
theorem secondGapArcSource_cycleIsolated
    ...
    IsIsolated D.fam (B.secondGapLocalLabel j ((B.secondGapLeft j).length +
      ...
      (B.secondGapCut j ((B.secondGapLeft j).length +
        (B.secondTargetSide s - B.secondGapStartSide j))) := by
  refine ⟨⟨_, B.secondGapArcComponentField j s hs⟩, ?_⟩
  intro q hne hop
  rw [B.secondGapLocalLabel_arc j s hs] at hop ⊢
  ...
  · exact B.secondGapArcSource_not_connected_left j s hs hleft hop
  · exact B.secondGapArcSource_not_connected_arc j s hs hr hne hop
  · exact B.secondGapArcSource_not_connected_right j s hs hr hop
  · exact B.secondGapArcSource_not_connected_chord j s hs hr hop

end GroupApproximation.GGT.DGOProposition414.BalancedSplitData

#audit_axioms GroupApproximation.GGT.DGOProposition414.BalancedSplitData.secondGapArcSource_cycleIsolated
