...
Proved on the same action.

* Limit set ⇒ loxodromics (`isAcylindricallyHyperbolicOsin_of_limitSet`) is
  Osin's Theorem 1.1 with the exclusion of the lineal case
  (`actsNonElementarily_of_limitSetHasMoreThanTwoPoints`, `GGT/OsinLinealExclusion`).
* Loxodromics ⇒ limit set is `SequentialBoundary.isAcylindricallyHyperbolicLimitSet_of_osin`
  (`GGT/AcylindricallyHyperbolicLimitSetEasy`).

## Manuscript status

The limit-set spelling of "acylindrically hyperbolic" in `sec:torsion-free` agrees
with the repository's notion.
-/
...
/-- **The hard direction, in the form `sec:torsion-free` consumes**: a
non-elementary acylindrical action in the limit-set sense is non-elementary in
the independent-loxodromics sense, on the same space. -/
theorem isAcylindricallyHyperbolicOsin_of_limitSet :
    ∀ (G : Type) [Group G], IsAcylindricallyHyperbolicLimitSet G →
      IsAcylindricallyHyperbolicOsin G := by
  intro G _ h
  obtain ⟨X, instX, instA, hiso, hgeo, ⟨δ, hδ⟩, hacy, w, hlim⟩ := h
  letI := instX
  letI := instA
...
theorem isAcylindricallyHyperbolicLimitSet_iff (G : Type) [Group G] :
  ...
  exact ⟨isAcylindricallyHyperbolicOsin_of_limitSet G, isAcylindricallyHyperbolicLimitSet_of_osin⟩
...
#audit_axioms GroupApproximation.GGT.OsinClassification.isAcylindricallyHyperbolicLimitSet_iff_all
