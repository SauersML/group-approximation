...
/-! ## A concrete endpoint sublemma -/

/-- The predecessor-side exclusion for an inherited first-gap source.  This
is the first conjunct of `FirstGapArcBoundaryExclusion`; interior positions
are excluded by the parent component, while the zero-offset case is excluded
by the preceding connector and the preceding parent target component. -/
theorem firstGapArcBoundaryExclusion_start_of_source
    {D : RelGenSet G Λ}
    ...
    ∀ t : ℕ,
      (B.firstGapLeft j).length +
          (B.firstArcCut (B.firstTargetSide s) -
            B.firstArcCut (B.firstGapStartSide j)) = t + 1 →
      ∀ ht : t < (B.firstGapCycle j).length,
      ¬ ((B.firstGapCycle j)[t]'ht).IsCompOf (P.label s) := by
  have hsData := Finset.mem_filter.mp hs
  let i := B.firstArcCut (B.firstTargetSide s) -
    B.firstArcCut (B.firstGapStartSide j)
  let cycle := auxiliaryCycleWord (B.firstGapLeft j)
  ...
  · have hi0 : i = 0 := by omega
    ...
    · obtain ⟨e, he⟩ := Option.ne_none_iff_exists'.mp hp
      ...
      have hcurEq : B.firstArcCut (B.firstTargetSide s) =
          B.firstArcCut (B.firstGapStartSide j) := by
        apply Nat.le_antisymm
        · exact Nat.sub_eq_zero_iff_le.mp hdiff0
        · exact B.firstArc_isCutPath.cut.mono_le hsData.2.1
      have hpositive : 0 < B.firstArcCut (B.firstTargetSide s) := by
        rw [hcurEq, ← hprevPos]
        omega
...
/-- A wrapped terminal cycle letter identified with the parent arc's successor
is excluded by the parent's maximality clause. -/
theorem secondGapArcBoundaryExclusion_terminal_of_parent_successor
    {D : RelGenSet G Λ}
    {hsymm : ∀ x ∈ D.base, x⁻¹ ∈ D.base} {δ b n k R : ℕ}
    {hδ : Hyperbolic.IsFourPointHyperbolic D.alphabet.carrier δ}
    {P : SumBoundInput D (b : ℝ) n}
    ...
    (hn :
      (B.secondGapLeft j).length +
          (B.secondArcCut (B.secondTargetSide s) -
            B.secondArcCut (B.secondGapStartSide j)) + 1 <
        (B.secondGapCycle j).length)
    (hnext : B.secondArcCut (B.secondTargetSide s) + 1 < B.secondArc.length)
    (hadj :
      (B.secondGapCycle j)[
          (B.secondGapLeft j).length +
            (B.secondArcCut (B.secondTargetSide s) -
              B.secondArcCut (B.secondGapStartSide j)) + 1]'hn =
        B.secondArc[B.secondArcCut (B.secondTargetSide s) + 1]'hnext) :
    ¬ ((B.secondGapCycle j)[
        (B.secondGapLeft j).length +
          (B.secondArcCut (B.secondTargetSide s) -
            B.secondArcCut (B.secondGapStartSide j)) + 1]'hn).IsCompOf
  ...
  intro hcycle
  have hparent := B.secondArc_targetComponent hsData.1
  have htargetNext : B.secondTargetPos s + 1 < B.secondArc.length := by
    rw [← (B.secondArcCut_target hsData.1).1]
    exact hnext
  have hget : B.secondArc[B.secondArcCut (B.secondTargetSide s) + 1]'hnext =
      B.secondArc[B.secondTargetPos s + 1]'htargetNext := by
    apply getElem_congr_idx
    rw [(B.secondArcCut_target hsData.1).1]
  have hparentLetter :
      (B.secondArc[B.secondTargetPos s + 1]'htargetNext).IsCompOf
        (P.label s) := by
    rw [← hget, ← hadj]
    exact hcycle
  exact hparent.2.2.2.2 htargetNext hparentLetter
