import GroupApproximation.GGT.VanKampen.CactusPowerConstruction
import Mathlib.Data.List.ChainOfFn
import Mathlib.Data.List.Flatten
import GroupApproximation.GGT.HullSCRelatorWord

/-!
# The complementary boundary word of the labelled power cactus
...
theorem cactusOuterBackward_value
    ...
    GGT.RelLetter.listVal
        (Z.cactusOuterBackwardDarts.map Z.cactusLabel) = g ^ n := by
  rw [Z.cactusOuterBackwardWord, cactusOuterFaceWord,
    ...
    calc
      List.ofFn (fun i : Fin Z.cells.length ↦ (Z.cellAt i).value⁻¹) =
          (List.ofFn Z.cells.get).map
            (fun C : HullSC.Lemma44OrientedRelatorCell
              (RelWord.symmetrized v) ↦ C.value⁻¹) := by
        rw [← List.ofFn_comp']
        rfl
      _ = Z.cells.map
          (fun C : HullSC.Lemma44OrientedRelatorCell
...
theorem mem_cactusBigDarts_iff
  ...
  have hrevCast : ∀ {m k : ℕ} (h : m = k) (j : Fin k),
      ∃ i : Fin m, (Fin.cast h i).rev = j := by
    intro m k h j
    subst k
    exact hrev j
  cases d <;>
    simp [cactusBigDarts, cactusOuterBackwardDarts, cactusCellSegments,
      ...
      CactusShape.faceClass, hrev, hrevCast]
