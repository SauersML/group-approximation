import GroupApproximation.GGT.VanKampen.SurgeryInnerDiscCollapse
...
`InnerDiscRegion.diagram` retains every dart that is not internal to the collapsed region,
...
regions avoiding the collapsed faces (`SurgeryInnerDiscCollapseRegions` is the second).

The darts and faces of the collapse are, by definition, retained old darts and old or
merged faces, but only through the definitions `InnerDiscRegion.diagram`,
...
`InnerDiscRegion.keep` is the total dart map: a retained dart to itself, an internal dart
...
namespace GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion
...
theorem faceBoundary_merged_map_val :
    (R.diagram.faceBoundary R.merged).darts.map R.val = R.region.cycle :=
...
theorem head_not_internal :
    ¬ InternalDart Delta.toCombMap R.faces
      (R.region.cycle.head R.region.cycle_nonempty) := by
  have hb := (R.region.cycle_mem_iff _).mp (List.head_mem R.region.cycle_nonempty)
...
noncomputable def keep (d : Delta.toCombMap.Dart) : R.diagram.toCombMap.Dart :=
  (if h : InternalDart Delta.toCombMap R.faces d then
    ⟨R.region.cycle.head R.region.cycle_nonempty, R.head_not_internal⟩
...
end GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion

#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.map_keep_val
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.facePerm_keep
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.faceOf_keep_mem_faceSet_iff
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.cellDarts_eq
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.outerDarts_eq
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.targetBoundaryDarts_map
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.InnerDiscRegion.dartWord_map
