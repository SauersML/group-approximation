...
Step 2 of the plan for the Greendlinger residual `GL06e.AllCellsShortEnclosedRefutedBelowSectionStatement`:
cut the diagram along a minimal path `p` from the boundary to a nearest relator cell.  Osin
(arXiv:math/0411039v3, §9) passes to an O-equivalent diagram in which `p` is doubled, so that the
two copies of `p` bound a thin region of G-cells (a "ribbon").  This module builds that diagram
from edge doublings (`FaceEdgeDoubling`), one dart of `p` at a time, each inside its own face.
...
* `IsRibbonAt X ι x`: the face of `x` is a G-digon with face cycle `[x, c]`.  It is inner and
  ...
  in the range of `ι`.
* `step`: one doubling.  It creates a ribbon cell at the doubled dart, and it transports the
  faces of the other darts, their membership in the face set, their face lengths, and every
  ribbon cell whose face is not the doubled face.
* `exists_pathRibbon`: a list `p` of distinct darts, each on an inner face with at least two darts,
  can be doubled completely.  There is a collar transport (`GeodesicCollar.Transport`) from the
  faces of `p`, i.e. an O-equivalence with a dart embedding compatible with `alpha` and the
  labels.  It keeps the outer boundary and the relator cells off the faces of `p`, and every
...
@[reducible] def IsRibbonAt (X : DiscDiagram.{u, w, v} W) {β : Type v} (ι : β → X.toCombMap.Dart)
    (x : X.toCombMap.Dart) : Prop :=
...
theorem IsRibbonAt.congr {X : DiscDiagram.{u, w, v} W} {β γ : Type v}
    ...
    (h : IsRibbonAt X ι x) (hx : x' = x) (hι : ∀ a, ∃ b, ι' a = ι b) : IsRibbonAt X ι' x' := by
  subst hx
...
/-- **One doubling.**  Double the dart `d` inside its face.  The doubled dart spans a ribbon cell
whose other side is not the image of an old dart.  Every other old dart keeps its face, up to the
face map: membership in the face set, being inner and the face length are unchanged.  Every
ribbon cell off the doubled face survives. -/
theorem step (D : RelGenSet G Lambda) (hsymm : ∀ x ∈ D.base, x⁻¹ ∈ D.base)
    ...
    ∃ (X' : DiscDiagram.{u, w, v} W) (S' : Finset X'.toCombMap.Face)
      ...
      ∀ x, x ≠ d →
        ...
        (X'.toCombMap.faceOf (T.embedding x) ≠ X'.outerFace ↔
          X.toCombMap.faceOf x ≠ X.outerFace) ∧
        (X'.faceBoundary (X'.toCombMap.faceOf (T.embedding x))).darts.length =
          (X.faceBoundary (X.toCombMap.faceOf x)).darts.length ∧
        ∀ {β : Type v} (ι : β → X.toCombMap.Dart),
          X.toCombMap.faceOf x ≠ X.toCombMap.faceOf d → IsRibbonAt X ι x →
            IsRibbonAt X' (fun a => T.embedding (ι a)) (T.embedding x) := by
  ...
  subst hj
  have hemb : ∀ y, (FaceEdgeDoubling.collarTransport X f j hlen hf D hsymm hlabel hS).embedding y =
  ...
  refine ⟨FaceEdgeDoubling.diagram X f j hlen hf, FaceEdgeDoubling.collarFaceSet X f j hlen S,
    FaceEdgeDoubling.collarTransport X f j hlen hf D hsymm hlabel hS, ?_, ?_⟩
  · simp only [hemb]
  ...
  · intro x hx
    ...
    · rw [hface x hx, FaceEdgeDoubling.boundary_faceImage_darts X f j hlen hf, List.length_map]
    · intro β ι hne hrib
      ...
      · intro C hC
        obtain ⟨C₀, hC₀, hCf⟩ := FaceEdgeDoubling.exists_relatorCell_of_mem_diagram X f j hlen hf hC
        rw [hCf, hface x hx]
...
theorem aux (D : RelGenSet G Lambda) (hsymm : ∀ x ∈ D.base, x⁻¹ ∈ D.base) {α : Type v}
    ...
    ∀ (S : Finset X.toCombMap.Face) (ι : α → X.toCombMap.Dart) (r : List α),
      ...
      (∀ a ∈ q, X.toCombMap.faceOf (ι a) ∈ S ∧ X.toCombMap.faceOf (ι a) ≠ X.outerFace ∧
        1 < (X.faceBoundary (X.toCombMap.faceOf (ι a))).darts.length) →
      ∃ (X' : DiscDiagram.{u, w, v} W) (S' : Finset X'.toCombMap.Face)
        (T : GeodesicCollar.Transport D S X' S'),
  ...
  | nil =>
      ...
      refine ⟨X, S, GeodesicCollar.Transport.refl D hlabel S, fun a ha => ?_⟩
      rw [List.append_nil] at ha
      exact (hr a ha).congr rfl (fun b => ⟨b, rfl⟩)
  | cons a₀ q ih =>
      intro X hlabel S ι r hnd hinj hr hq
      obtain ⟨hS, hf, hlen⟩ := hq a₀ List.mem_cons_self
      obtain ⟨X₁, S₁, T₁, hnew, hold⟩ := step D hsymm X hlabel S (ι a₀) hS hf hlen
      ...
      have hr₁ : ∀ b ∈ r ++ [a₀],
        ...
        · have hba : b ≠ a₀ := (List.nodup_append.mp hnd).2.2 b hb a₀ List.mem_cons_self
          ...
          exact ((hold (ι b) (fun h => hba (hinj h))).2.2.2 ι hfne hrib)
        · obtain rfl := List.mem_singleton.mp hb
          exact hnew.congr rfl (fun b => ⟨ι b, rfl⟩)
      have hq₁ : ∀ b ∈ q, X₁.toCombMap.faceOf (T₁.embedding (ι b)) ∈ S₁ ∧
          X₁.toCombMap.faceOf (T₁.embedding (ι b)) ≠ X₁.outerFace ∧
          1 < (X₁.faceBoundary (X₁.toCombMap.faceOf (T₁.embedding (ι b)))).darts.length := by
        intro b hb
        have hba : b ≠ a₀ := fun h =>
          (List.nodup_cons.mp (List.nodup_append.mp hnd).2.1).1 (h ▸ hb)
        obtain ⟨h₁, h₂, h₃, -⟩ := hold (ι b) (fun h => hba (hinj h))
        obtain ⟨hS', hf', hlen'⟩ := hq b (List.mem_cons_of_mem _ hb)
        refine ⟨h₁.mpr hS', h₂.mpr hf', ?_⟩
        rw [h₃]
        exact hlen'
      obtain ⟨X₂, S₂, T₂, hrib₂⟩ :=
        ih X₁ T₁.label_isLetter S₁ (fun b => T₁.embedding (ι b)) (r ++ [a₀]) hnd₁ hinj₁ hr₁ hq₁
      refine ⟨X₂, S₂, T₁.trans T₂, fun b hb => ?_⟩
      have hb' : b ∈ r ++ [a₀] ++ q := by
        rw [List.append_assoc, List.singleton_append]
        exact hb
      exact (hrib₂ b hb').congr rfl (fun c => ⟨c, rfl⟩)

/-- **A path doubled into a ribbon** (step 2 of the plan for GL06e residual (i)).  Let `p` be a list of
distinct darts of `Δ`, each on an inner face with at least two darts, and let the labels be letters
of `D`, with `D.base` closed under inverses.  Then there is a collar transport from the faces of
`p` to an O-equivalent diagram `Δ'` in which the image of every dart of `p` spans a ribbon cell,
a G-digon whose other side is a new dart reading the inverse letter.  The transport keeps the
outer boundary and every relator cell off the faces of `p`, and its labels are letters of `D`. -/
theorem exists_pathRibbon (D : RelGenSet G Lambda) (hsymm : ∀ x ∈ D.base, x⁻¹ ∈ D.base)
  ...
  exact ⟨Delta', S', T, fun d hd => (h d (by simpa using hd)).congr rfl (fun b => ⟨b, rfl⟩)⟩
