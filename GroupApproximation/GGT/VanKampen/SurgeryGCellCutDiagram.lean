import GroupApproximation.GGT.VanKampen.SurgeryGCellCutProduct
import GroupApproximation.GGT.VanKampen.SurgeryCutReduced

/-!
# The subdiagram cut out by a closed walk

The piece a `RegionCutWithGCells` cuts out is the collapse of the complementary faces,
...
The adapter to Osin's current cut record lives in `SurgeryGCellCutSections`, so that
...
noncomputable def diagram (cut : RegionCutWithGCells Delta) :
...
theorem diagram_rCellCount (cut : RegionCutWithGCells Delta) :
...
theorem diagram_rCellCount_lt (cut : RegionCutWithGCells Delta) :
...
theorem diagram_reduced (cut : RegionCutWithGCells Delta) (hred : Delta.Reduced) :
    cut.diagram.Reduced :=
...
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.MapCollapse.RegionCutWithGCells.diagram

#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.MapCollapse.RegionCutWithGCells.diagram_reduced

#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.MapCollapse.RegionCutWithGCells.diagram_rCellCount_lt
