...
/-!
# Greendlinger leaf, piece 04: GapSpan from the remaining gap

`OsinLemma94ClassCaseGapSpanStatement` follows from `GapSpanResidualStatement` (module
`Piece04.Gap`). The steps are those of `osinLemma94ClassCaseGapSpanSmall_of_residuals`, without the
short-connector hypotheses.

* The threshold is `ε₀ = max 3 (max ε₁ (max ε₂ ε₃))`. Here `ε₁` is the Morse threshold, `ε₂` and
  `ρ₂` come from the relator-cell branch, and `ε₃` and `ρ₃` from the full-word different-kind
  branch.  The `ρ` threshold is `max ρ₂ ρ₃`.
...
* **Equal kinds.** The face walk of `exists_sameCellWalk`. Then either the value-one kills, or the
  relator-cell branch.
* **Different kinds.** `false_of_diffKind` (module `Piece04.DiffKind`), whose full-word case is the
  full-word different-kind branch.
...
/-- **GapSpan from the remaining gap.** -/
theorem proof_of_gapSpanResidual (hres : GapSpanResidualStatement.{u, w, v}) :
    OsinLemma94ClassCaseGapSpanStatement.{u, w, v} := by
  obtain ⟨hrcell, hfull⟩ := hres
  ...
  obtain ⟨eps3, heps3⟩ := hfull D hhyper lambda c mu hlambda hlambda1 hc hmu hmu1
  ...
  exact false_of_diffKind Q h3 k C hback hnogap hcut hkind
    (hrho3' rho (le_of_max_le_right hrho) W hW Delta cuts hleast hpos hbelow S hcard hmin P hmax Q
      k C hback hnogap hnotsingle hcut hkind)

end GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04

#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.proof_of_gapSpanResidual
