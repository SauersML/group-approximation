...
* `pinchCut_side_of_boundary`: a nonempty walk made of boundary darts of `P₀` has `c` on
...
theorem pinchCut_keep_of_boundaryDart {a b : RegionCandidate D eps X}
...
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P07InnerPocket.pinchCut_keep_of_boundaryDart
...
theorem pinchCut_pocketKeep_of_walkKeep {a b : RegionCandidate D eps X}
  ...
  · exact pinchCut_keep_of_boundaryDart (hbd x h)
  · exact (pinchCut_pocketKeep_alpha a b K x).mp
      (pinchCut_keep_of_boundaryDart (hbd _ h))
...
theorem pinchCut_side_of_boundary {a b : RegionCandidate D eps X}
    {K : CellPocketWalk D eps X i j} {c : X.toCombMap.Face} {w' : List X.toCombMap.Dart}
    (hne : w' ≠ [])
    (hbd : ∀ d ∈ w', Surgery.MapCollapse.IsBoundaryDart X.toCombMap
      (FourPieceWitness.keepClass a b K c) d) :
...
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P07InnerPocket.pinchCut_side_of_boundary
