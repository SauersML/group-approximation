...
/-- **Target of lane gl-p07-76: the four NoInterleave clauses of the touch tail.**  The premises
are those of `pinchAbsTouchNI_Statement`.  The conclusion is `pinchAbsTouchNI_NIPart`, which is
clauses 1–4 of `pinchCut_Tail b G₁ G₂ (invDarts X w')`, verbatim. -/
def pinchAbsTouchNI_NIStatement : Prop :=
  ∀ {G : Type u} [Group G] {Lambda : Type w} {W : Set (List (RelLetter G Lambda))}
    (D : RelGenSet G Lambda) (eps : ℕ) (X : DiscDiagram.{u, w, v} W)
    {i j : Fin X.rCellCount} (a b : RegionCandidate D eps X) (K : CellPocketWalk D eps X i j),
    i ≠ j → a.JoinsCells i j → b.JoinsCells i j → Disjoint a.1 b.1 →
      ...
      ∀ C ∈ X.relatorCells, C.face ∈ sideFaces X.toCombMap K.walk → C.face ∉ a.1 → C.face ∉ b.1 →
        ...
        ∀ comp : FaceSetCircuits.Component X.toCombMap
          ...
          EnclosedFaceSetSucc X
              (sideFaces X.toCombMap (pinchLobe_walk (pinchAbs_set a b K G₁ G₂ C.face) comp))
              (invDarts X (pinchLobe_walk (pinchAbs_set a b K G₁ G₂ C.face) comp)) →
          pinchAbsTouchNI_NIPart b G₁ G₂
            (invDarts X (pinchLobe_walk (pinchAbs_set a b K G₁ G₂ C.face) comp))

#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P07InnerPocket.pinchAbsTouchNI_NIStatement

/-- **The `_of_` reduction.**  The block reading gives the four NoInterleave clauses, by
`pinchAbsTouchNI_niPart_of_blocks` (list combinatorics only). -/
theorem pinchAbsTouchNI_ni_of_statement (h : pinchAbsTouchNI_Statement.{u, w, v}) :
    pinchAbsTouchNI_NIStatement.{u, w, v} := by
  ...
  exact pinchAbsTouchNI_niPart_of_blocks (h D eps X a b K hij hai hbi hab hlabel hW hfirst
    hsecond G₁ hG₁ G₂ hG₂ hw hfo hout hinner houter C hC hCf hCa hCb hcase hP comp hside henc)

#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P07InnerPocket.pinchAbsTouchNI_ni_of_statement
