...
/-- **Residual of lane gl-p07-28: the target in the case with no witness walk.**  Under the
premise block of `PocketFourPieceArcAuditResidualStatement`, and assuming that
`witnessFaces a b K C.face` has no bridge-free successor outside walk, branch 1 or branch 2 of that
statement holds.  See the module docstring for why it is true and strictly smaller. -/
def NoPlanarDegenerateStatement : Prop :=
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P07InnerPocket.FourPieceWitness.NoPlanarDegenerateStatement
