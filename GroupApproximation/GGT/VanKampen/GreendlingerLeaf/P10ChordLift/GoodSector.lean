import GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.Refinement
import GroupApproximation.Meta.AxiomGuard

/-!
# Chord lift: the step at an uncrossed non-first turn with a good sector, and its transport

Osin, arXiv:math/0411039v3, §9, proof of Lemma 9.7(b).  Let `c = K.boundary.cycle` be the boundary of
a pocket `K` in walk order, and `d₀ → e₀ = next d₀` an uncrossed non-first turn.

## The step at a good sector (`exists_step_of_goodSector`)
...
* Symmetrically, the rotation from `y` to `σ^q y = α d₀` meets no edge of `c`; then every face
  `face (σ^(q - n) y)`, `n < q`, lies off `K.faces` (`faceOf_sectorBefore_not_mem`), in particular
  `face (α y) = face (σ y)` (`faceOf_alpha_before_not_mem`).
* When the two corner faces are distinct, off the exterior and off the relator cells, they form a
  `PinchSplit.Input` (`exists_input`) avoiding `K.faces`, and
  `PocketFaceSet.exists_pinchStepSection_of_uncrossedTurn` performs the step, with the first arrival
  `k₀` of `e₀` from `α d₀` (`OuterPinchCorners.exists_firstArrival`) and a dart `z₀` outside the
  sector (`OuterPinchCorners.exists_outside_of_not_firstTurn`).

## Transport across a rotation refinement (`exists_step_of_refinement`)

Let `R : RotationRefinement X X'` with image map `e`, and `K'` a pocket of `X'` with
`K'.boundary.cycle = c.map e`, walk order, proper arcs and the same repeated visits.

* `e d₀ ∈ c'` and `next (e d₀) = e e₀` (`next_map_of_eq`).
* The turn stays non-first (`not_firstTurn_of_refinement`): a first turn in `X'` projects to one in
  `X` (`RotationRefinement.firstTurn_of_map`).
* The turn stays uncrossed (`huncross_of_refinement`): `SameCycle` and `RotationBetween` between
  images are those between the old darts.
* A sector in `X'` all of whose intermediate darts are new avoids the edges of `c.map e`
  (`RotationRefinement.not_walkKeep_map_of_new`).  With good corner faces (`RefinedGoodSector`), the
  step in `X'` is the step for `K`; the O-equivalences compose.
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.faceOf_alpha_sigma_symm
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.exists_input
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.faceOf_alpha_sectorAfter_not_mem
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.faceOf_sectorBefore_not_mem
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.faceOf_alpha_before_not_mem
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.exists_step_of_goodSector
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.RefinedGoodSector
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.not_firstTurn_of_refinement
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.huncross_of_refinement
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.exists_step_of_refinement
