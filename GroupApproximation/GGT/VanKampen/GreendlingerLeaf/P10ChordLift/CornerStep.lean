import GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.CornerBasic
...
/-!
# Chord lift: the corner digon at a non-monogon corner

Osin, arXiv:math/0411039v3, §9, proof of Lemma 9.7(b).  At a corner `z → σ z` of a pocket `K` whose
corner face `f = face (α z)` is outside the face set and not a monogon (`σ z ≠ α z`), both corner darts
lie on the traversal of `f`, so `1 < |f|` (`one_lt_length_of_mem_ne`).  Pick a corner dart `wd`
(`exists_corner_dart_inner` / `exists_corner_dart_outer`) and its position `k` in `f`
(`exists_corner_index`).

* `f ≠ outer`: `PocketFaceSet.faceEdgeDoubling` at `k` (the source arc avoids `wd`).
* `f = outer`: `PocketFaceSet.outerSpurThickening` at `k` (the target arc avoids `α wd`).

In both the split map is `EdgeInsertion.toCombMap M a b`, with new darts `some none` before `a` and
`none` before `b`; the digon is `[none, embed wd]`.

* `wd = α z`: lift the step by `exists_sigma_step_lift` (intermediates `none`, `some none`), `m = 0`:
  `α' (embed z) = embed (α z)` lies on the digon.
* `wd = σ z`: `σ' (embed z) = some none`, `σ' (some none) = embed (σ z)`, so `p = 2`, `m = 1`, and
  `α' (some none) = none` lies on the digon (`exists_digon_sector`).

The digon is not a face image (`faceImage_ne_digon`); the exterior and the relator cells are images,
so the digon is a G-face.  An old dart on it is `wd` (`eq_of_faceOf_embed_digon`).  Faces other than
`f` map by the injective `faceImage`, so G-faces stay G-faces.  The arcs keep their lengths and the
carriers are mapped lists, so the arcs stay proper.
...
theorem cornerDigon_of_nonMonogon (K : PocketFaceSet D eps X lo hi)
  ...
  · obtain ⟨wd, hwd, hwd1⟩ := exists_corner_dart_inner K hinv.2.2.1 hone
    ...
    exact cornerDigon_faceEdgeDoubling X _ k hlen K hfo hs (by rw [hdart]; exact hwd1) z hinv rfl
      (Or.imp (fun h => hdart.trans h) (fun h => hdart.trans h) hwd)
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.inner_source_proper
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.outer_target_proper
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.cornerDigon_faceEdgeDoubling
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.cornerDigon_outerSpur
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.cornerDigon_of_nonMonogon
