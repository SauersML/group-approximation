import GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.OuterMonogon
import GroupApproximation.Meta.AxiomGuard

/-!
# Chord lift: the outer-pinch step at an uncrossed non-first turn

Osin, arXiv:math/0411039v3, §9, proof of Lemma 9.7(b).  Outside the rose configuration some non-first
turn `d₀ → e₀ = next d₀` of the boundary cycle `c = K.boundary.cycle` is crossed by no other passage
(`exists_uncrossed_of_not_allCrossed`).  Both corners of the turn are turned into G-digons off the face
set, and the split at the turn performs the step (`nonRose`).

## Proof

1. **The turn.**  `σ e₀ ≠ α d₀` since the turn is not first (`sigma_ne_alpha_of_not_firstTurn`), and
   `d₀ ≠ e₀` (`ne_next_of_not_firstTurn`): there is a cycle dart `z₀ ≠ e₀`
   (`OuterPinchCorners.exists_outside_of_not_firstTurn`), so if `d₀ = e₀` the cycle has two distinct
   members, hence length at least two, and `next d₀ ≠ d₀`.
2. **First corner.**  `e₀ ∈ c`, so `face (α e₀) ∉ K.faces`; (H2) holds by `hone_start`.
   `cornerDigon_of_hone` gives `R₁ : X → X₁`, `K₁` with `c₁ = c.map R₁`, `p₁ > m₁`, all intermediate
   darts of the lifted step `R₁ e₀ ↦ R₁ (σ e₀)` new, and the G-digon `g₁ = face₁ (α₁ x₁)`,
   `x₁ = σ₁^m₁ (R₁ e₀)`, whose old darts are among `α e₀`, `σ e₀`.
3. **Second corner** at `z = σ₁⁻¹ (R₁ (α d₀))`.  `face₁ (α₁ z) = face₁ (R₁ (α d₀)) =
...
4. **The first digon survives** (`corner_faces_ne`): `g₁ ≠ face₁ (α₁ z)`, since otherwise `R₁ (α d₀)`
   lies on `g₁`, so `α d₀ = α e₀` (then `d₀ = e₀`, excluded) or `α d₀ = σ e₀` (excluded).  Hence
   `face₂ (R₂ (α₁ x₁))` is a G-face of `X₂`.
5. **Sector** for `R = R₁.comp R₂` (`refinedGoodSector_of_twoDigons`).  Lift `m₁` steps by
   `R₂.lift_pow`: `σ₂^K (R e₀) = R₂ x₁`; take `x = σ₂^K (R e₀)`, `m = K`, `q = p₂ - m₂`.  New darts
   before `x`: an intermediate `b < K` pulls back to an intermediate `t < m₁ < p₁` of step 2; at `b = K`
   the dart is `R₂ x₁` with `0 < m₁ < p₁` (`first_newDarts`).  The equation
   `σ₂^q y = σ₂^p₂ (R₂ z) = R₂ (σ₁ z) = R (α d₀)`, and new darts after `y` are intermediates
   `t + m₂ < p₂` of step 3.  `α₂ x = R₂ (α₁ x₁)`, so its face is a G-face; `face₂ (α₂ y) = g₂` is a
   G-face; they differ since an old dart `α₁ x₁` on `g₂` lies in `{α₁ z, σ₁ z}`, whose faces equal
   `face₁ (α₁ z) ≠ g₁`.
6. **The step.**  Repeated visits, `O`-equivalence, the invariants and `c₂ = c.map R` compose, and
   `exists_step_of_refinement` performs the step in `X₂`.
...
theorem corner_faces_ne {X X₁ : DiscDiagram.{u, w, v} W}
    (R₁ : RotationRefinement X.toCombMap X₁.toCombMap) {d₀ e₀ : X.toCombMap.Dart}
    (hde : d₀ ≠ e₀) (hne : X.toCombMap.sigma e₀ ≠ X.toCombMap.alpha d₀) {x₁ : X₁.toCombMap.Dart}
    (honly₁ : ∀ w, X₁.toCombMap.faceOf (R₁.map w) =
...
/-- **An uncrossed non-first turn is not a loop**: `d₀ ≠ next d₀`. -/
...
/-- **The step at an uncrossed non-first turn**, through two corner digons. -/
theorem exists_step_at_turn {X : DiscDiagram.{u, w, v} W} (hW : ∀ word ∈ W, 1 < word.length)
  ...
  exact exists_step_of_refinement K K₂ (R₁.comp R₂) ⟨OEquivalentDiscDiagram.trans E₁ E₂⟩ hlabel₂
    ...
    (hcyc₂.trans ((congrArg (List.map R₂.map) hcyc₁).trans List.map_map)) hd₀ hnext₀ hnot huncross
...
theorem nonRose {X : DiscDiagram.{u, w, v} W} (hW : ∀ word ∈ W, 1 < word.length)
    ...
    (hprop : K.sourceArc.length < (cellDarts X K.source).length)
    (htgt : K.targetArc.length < (outerDarts X).length)
    (hnotall : ¬ AllNonFirstTurnsCrossed K) :
    ∃ (X' : DiscDiagram.{u, w, v} W) (K' : PocketFaceSet D eps X' lo hi),
      Nonempty (OEquivalentDiscDiagram X X') ∧
        (∀ d, (symmetricLabelAlphabet D).IsLetter (X'.label d)) ∧
        K'.ClosedWalk ∧ K'.sourceArc.length < (cellDarts X' K'.source).length ∧
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.first_newDarts
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.refinedGoodSector_of_twoDigons
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.notMem_faces_second
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P10ChordLift.corner_faces_ne
