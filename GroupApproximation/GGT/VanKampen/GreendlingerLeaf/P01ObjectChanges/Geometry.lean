...
import GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P01ObjectChanges.Words
import GroupApproximation.Meta.AxiomGuard

/-!
# Object changes of a rich polygon: four sides reading `a b a b`

Step 4 of the proof in `P01ObjectChanges.Words`, the geometric half.

Rotate the walk of `face k` by `base k`; it is the concatenation of the sides.  Side `i` starts at
position `sidePos k i` of the rotated walk (`sidePos_lt_sidePos`: strictly increasing), so its
first dart is `facePerm ^ (sidePos k i - sidePos k i₁)` applied to the first dart `x₀` of side
`i₁`, for `i₁ ≤ i` (`sideHead_eq_pow`, via `OsinUnboundSameCellCycle.getElem_mod_pow`).  The face
across any dart of a side along object `o` is `objectFace S o` (`faceOf_alpha_of_mem_sideDarts`,
as `faceOf_alpha_rep`).

If sides `i₁ < i₂ < i₃ < i₄` read objects `a, b, a, b` with `a ≠ b`, then along the face walk of
`face k`, from `x₀`, the faces across positions `0 < r < q < s < L` read `A, B, A, B` with
`A = objectFace a ≠ objectFace b = B`, both different from `face k` (`objectFace_ne_face`).  This
contradicts planarity: `NeighbourFaces.false_of_interleave` (`no_interleave`).
-/
...
/-- **The face across a side along object `o`** is the face of `o`. -/
...
theorem sideHead_eq_pow (P : OsinLemma94RealizedPolygons S) (k : Fin P.count) {i1 i : ℕ}
  ...
  rw [← getElem?_rotate_sidePos P k hi, List.getElem?_rotate hposlt, hidx,
    ...
    OsinUnboundSameCellCycle.getElem_mod_pow (S.diagram.faceBoundary (P.face k)) hk0]
...
/-- **No four sides read `a b a b`**, by planarity of the diagram. -/
theorem no_interleave (P : OsinLemma94RealizedPolygons S) (k : Fin P.count) {i1 i2 i3 i4 : ℕ}
  ...
  refine NeighbourFaces.false_of_interleave S.diagram.toCombMap S.diagram.planar _
    (S.diagram.faceBoundary (P.face k)).darts.length
    (OsinUnboundSameCellCycle.pow_length (S.diagram.faceBoundary (P.face k)) hk0)
    (fun x y hx hy h =>
      OsinUnboundSameCellCycle.pow_inj (S.diagram.faceBoundary (P.face k)) hk0 hx hy h)
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P01ObjectChanges.no_interleave
