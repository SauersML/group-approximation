...
/-!
# Piece 06: refinements of the rotation

Osin, arXiv:math/0411039v3, §9, proof of Lemma 9.7(b).  An edge doubling inserts new darts into
the rotation at a vertex and changes nothing else near the old darts.  This module isolates the
map-level property that carries the turn data of a cell pocket across such a surgery (step 5 of
the plan in `Piece06/Predicate`).
...
* `project_pow`: `σ'^K (e x) = e y` gives `k ≤ K`, `K > 0 → k > 0`, `σ^k x = y`, and every
  ...
  `σ x` with `K - m` (with `k ≤ K`, an intermediate `t = 1` sits at `m < K`).
...
* `comp`: refinements compose; the lift of the old step's lift meets only images of new darts of
  the first refinement, or new darts of the second, before its end.
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.lift_pow
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.project_pow
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.rotationBetween_map_iff
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.sameCycle_map_iff
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.mem_map_iff
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.walkKeep_map_iff
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.not_walkKeep_map_of_new
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.firstTurn_of_map
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.comp
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.RotationRefinement.edgeInsertion
