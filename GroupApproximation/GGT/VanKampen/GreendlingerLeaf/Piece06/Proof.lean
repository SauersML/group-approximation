...
* every non-first turn is crossed (the rose): `Piece06.exists_step_of_allCrossed` (OPEN, gap 6);
...
* `exists_step`: the step for a fixed pocket.
* `proof`: the statement.
...
theorem exists_step {G : Type u} [Group G] {Lambda : Type w} {W : Set (List (RelLetter G Lambda))}
  ...
  · exact exists_step_of_allCrossed hlabel hlea K hK hnft hij hfirst hsecond hpinch hrose
...
/-- **Piece 06 of the Greendlinger leaf: `CellPocketOuterPinchStepSectionDistinctStatement`.** -/
theorem proof : CellPocketOuterPinchStepSectionDistinctStatement.{u, w, v} := by
  ...
  exact ⟨0, fun _ _ => ⟨1, Nat.one_pos, fun _ _ _ _ _ _ _ hij hlea hlabel K hK hnft hfirst hsecond
    hpinch => exists_step hij hlea hlabel K hK hnft hfirst hsecond hpinch⟩⟩
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece06.proof
