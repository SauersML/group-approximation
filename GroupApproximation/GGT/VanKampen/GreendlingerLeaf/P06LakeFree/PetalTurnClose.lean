...
`∂K = s₁ t̄₁ s₂ t̄₂` read backwards and cuts it at bad repeated vertices; the face of `C` stays on the
side of one piece.  Only the noncrossing hypothesis `hw` (no repeats, no bridges, closed walk) and
