...
cycle darts inside" in 33 configurations, the reverse order in 39.  So the extremal class is not
a minimiser of any such count, and a proof by well-founded minimality needs a genuinely planar
order (nesting of regions) on the face classes of `walkKeep`, not attempted here.
