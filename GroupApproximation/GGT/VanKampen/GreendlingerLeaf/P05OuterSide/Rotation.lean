import GroupApproximation.GGT.VanKampen.GreendlingerLeaf.P05OuterSide.Faces
import GroupApproximation.GGT.VanKampen.Estimating.OsinAppendixEulerPhiMapTransportClasses
import GroupApproximation.Meta.AxiomGuard

/-!
# Outer-side thickening: the collapsed rotation and `Φ'_M`
...
theorem keepEmbed : ∀ z, ¬ RegionInternal (OuterSideThickening.sectionFamilyOfTransport S j hlen
    htgt).family ((transport S j hlen htgt).darts z) ↔ ¬ RegionInternal S.family z :=
  fun z => not_congr (regionInternal_embed_iff S j hlen htgt z)
...
noncomputable def collapsedEmbedding : (collapsedMap S.family).Dart ↪
  ...
  toFun x := ⟨(transport S j hlen htgt).darts x.1, (keepEmbed S j hlen htgt x.1).mpr x.2⟩
  inj' x y h := Subtype.ext ((transport S j hlen htgt).darts.injective (congrArg Subtype.val h))
...
def sDart : (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).Dart :=
  ⟨some none, someNone_not_internal S j hlen htgt⟩

theorem collapsedEmbedding_alpha (x : (collapsedMap S.family).Dart) :
    collapsedEmbedding S j hlen htgt ((collapsedMap S.family).alpha x) =
      (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).alpha
        (collapsedEmbedding S j hlen htgt x) :=
  Subtype.ext rfl

/-- The new dart `some none` is not an old dart. -/
theorem sDart_not_mem_range :
    sDart S j hlen htgt ∉ Set.range (collapsedEmbedding S j hlen htgt) := by
  rintro ⟨x, hx⟩
...
theorem wDart_not_phiKeepO (a₀ : S.family) :
  ...
  not_phiKeepO_dart S j htgt (linkedComponentO_subset S.family a₀.1) (wDart S j) rfl
...
theorem facePerm_collapsed_of_ne (x : (collapsedMap S.family).Dart) (hx : x ≠ wDart S j)
    ...
    (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).facePerm
        (collapsedEmbedding S j hlen htgt x) =
      collapsedEmbedding S j hlen htgt ((collapsedMap S.family).facePerm x) := by
  ...
  exact Subtype.ext ((facePerm_val_of_retained (M := S.diagram.toCombMap)
    ...
    (keepAlpha S) (keepAlpha (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt))
    (keepEmbed S j hlen htgt) (dart_ne_second S j hlen) (dart_not_internal S j)
    (sigma_alpha_dart S j hlen) (someNone_not_internal S j hlen htgt) x.2
...
theorem facePerm_collapsed_of_eq (x : (collapsedMap S.family).Dart) (hx : x ≠ wDart S j)
    (hfx : (collapsedMap S.family).facePerm x = wDart S j) :
    (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).facePerm
        (collapsedEmbedding S j hlen htgt x) = sDart S j hlen htgt := by
...
theorem facePerm_sDart :
    (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).facePerm
        (sDart S j hlen htgt) =
      collapsedEmbedding S j hlen htgt ((collapsedMap S.family).facePerm (wDart S j)) :=
  Subtype.ext ((facePerm_val_some_none (M := S.diagram.toCombMap)
    ...
    (keep' := fun d => ¬ RegionInternal
      (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family d)
    (keepAlpha (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt))
    (keepEmbed S j hlen htgt) (second_not_internal S j hlen)
    (someNone_not_internal S j hlen htgt) (none_not_internal S j hlen htgt)).trans
...
noncomputable def backCollapsed :
  ...
  backDart (M := S.diagram.toCombMap)
    ...
    (keep' := fun d => ¬ RegionInternal
      (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family d)
    (keepAlpha S) (keepAlpha (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt))
    (keepEmbed S j hlen htgt) (dart_not_internal S j)

theorem backCollapsed_embedding (x : (collapsedMap S.family).Dart) :
    backCollapsed S j hlen htgt (collapsedEmbedding S j hlen htgt x) = x :=
  rfl

/-- **Face-class paths after the step are named back.** -/
theorem eqvGen_backCollapsed (a₀ : S.family)
    (x' y' : (collapsedMap (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family).Dart)
  ...
  eqvGen_backDart_of_retained (M := S.diagram.toCombMap)
    ...
    (someNone_not_internal S j hlen htgt) (none_not_internal S j hlen htgt)
    ...
    (kp' := PhiKeepO (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family
      (linkedComponentO (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).family
        ((transport S j hlen htgt).map a₀)))
    (fun z hz => phiKeepO_map_iff (transport S j hlen htgt) a₀ (collapsedEmbedding S j hlen htgt)
      (fun _ => rfl) ⟨z, hz⟩)
    (wDart_not_phiKeepO S j htgt a₀)
    (fun hh => wDart_not_phiKeepO S j htgt a₀
...
theorem faceOf_backCollapsed
    ...
    (h : (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).diagram.toCombMap.faceOf x'.1 =
  ...
  · exact ((cell (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).diagram i').face_ne_outer
      (h.symm.trans
  ...
  · have h' : (FaceEdgeDoubling.map S.diagram S.diagram.outerFace j hlen).faceOf
        (EdgeInsertion.embed S.diagram.toCombMap z) =
          (cell (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).diagram i').face := h
    ...
    · by_cases hzd : z = FaceEdgeDoubling.dart S.diagram S.diagram.outerFace j
      ...
      · exact ((cell (OuterSideThickening.sectionFamilyOfTransport S j hlen htgt).diagram
...
/-- **`Φ'_M` is carried by the outer side doubling.** -/
theorem phiTransportAt (a₀ : S.family) : PhiTransportAt (transport S j hlen htgt) a₀ :=
  phiTransportAt_of_bypass (transport S j hlen htgt) a₀ (collapsedEmbedding S j hlen htgt)
    (fun _ => rfl) (collapsedEmbedding_alpha S j hlen htgt) (sDart_not_mem_range S j hlen htgt)
    (wDart_not_phiKeepO S j htgt a₀) (facePerm_wDart_ne S j hlen)
    (facePerm_collapsed_of_ne S j hlen htgt) (facePerm_collapsed_of_eq S j hlen htgt)
    (facePerm_sDart S j hlen htgt) (backCollapsed S j hlen htgt)
    (backCollapsed_embedding S j hlen htgt) (eqvGen_backCollapsed S j hlen htgt a₀)
    (faceOf_backCollapsed S j hlen htgt)
