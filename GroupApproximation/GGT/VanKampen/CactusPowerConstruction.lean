...
/-!
# The labelled cactus realization of a geodesic power diagram

For a geodesic power diagram, this file builds the explicit polygon cactus
introduced in `CactusMap`.  Least-area reducedness rules out an empty relator
word in every cell.  The nonidentity power rules out an empty repeated outer
word, so all polygon lengths required by `CactusShape` are positive.
...
def cellAt
    ...
    (i : Fin Z.cells.length) :
      HullSC.Lemma44OrientedRelatorCell (RelWord.symmetrized v) :=
  Z.cells.get i
...
/-- The repeated exterior word is nonempty because it spells the nonidentity
power filled by the cells. -/
theorem boundaryWord_ne_nil
  ...
  have hvalue := Z.boundaryPower_value_eq_cells
  rw [hnil, GGT.RelLetter.listVal_nil, Z.cell_values,
    Z.factors_prod] at hvalue
  exact hvalue.symm
...
abbrev cactusShape
    ...
    {D : GGT.RelGenSet G Lambda}
    {v : List (GGT.RelLetter G Lambda)} {g : G} {n : ℕ}
    (Z : HullSC.Lemma49GeodesicPowerDiagram D v g n) : CactusShape where
  boundaryLength := (lemma49BoundaryPower Z.boundaryWord n).length
...
@[simp] theorem cactusShape_boundaryLength
    ...
    {v : List (GGT.RelLetter G Lambda)} {g : G} {n : ℕ}
    (Z : HullSC.Lemma49GeodesicPowerDiagram D v g n) :
    Z.cactusShape.boundaryLength =
      (lemma49BoundaryPower Z.boundaryWord n).length := rfl
...
def cactusOuterFaceWord
  ...
  RelWord.revInv (lemma49BoundaryPower Z.boundaryWord n)
...
def cactusOuterIndex
  ...
  Fin.cast (by
    rw [cactusOuterFaceWord, RelWord.length_revInv]) j
...
theorem cactus_outerFaceWord
  ...
  exact ofFn_get_cast Z.cactusOuterFaceWord (by
    rw [cactusOuterFaceWord, RelWord.length_revInv])
...
def cactusRelatorCell
    ...
    (Z : HullSC.Lemma49GeodesicPowerDiagram D v g n)
    (i : Fin Z.cells.length) :
    RelatorCell Z.cactusShape.toCombMap Z.cactusShape.outerFace
      (RelWord.symmetrized v) where
...
def cactusRelatorCells
    ...
    {v : List (GGT.RelLetter G Lambda)} {g : G} {n : ℕ}
    (Z : HullSC.Lemma49GeodesicPowerDiagram D v g n) :
    List (RelatorCell Z.cactusShape.toCombMap Z.cactusShape.outerFace
