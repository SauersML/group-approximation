import GroupApproximation.GGT.VanKampen.SurgerySpikeDeletion
...
/-!
# Darts, face sets and carriers through a spike deletion

`SpikeDeletion.diagram` retains every dart off the deleted edge, with its label, edge reversal
and (away from the face of the spike) its face rotation.  This file records that dictionary,
the first half of the transport of contiguity regions avoiding the face of the spike
(`SurgerySpikeDeletionRegions` is the second).  It follows `SurgeryGFaceMergeDarts`, with the
face of the spike in place of the two merged faces.

A dart is **off the edge** when it is neither `dart` nor `alpha dart`; every dart on a face
other than the face of the spike is off the edge (`offEdge_of_faceOf_off`).  On lists of such
darts `keep` forgets nothing (`map_keep_val`, `map_keep_injective`, `nodup_map_keep`).  The
dictionary:

* edge reversal, face rotation off the face of the spike, and faces (`alpha_keep`,
  `facePerm_keep`, `faceOf_keep_mem_faceSet_iff`);
* face sets avoiding the face of the spike, through `faceSet s = s.image kept`
  (`kept_mem_faceSet_iff`, `shrunk_not_mem_faceSet`, `faceSet_disjoint_iff`, `faceSet_inj`);
...
theorem offEdge_of_faceOf_alpha_off {d : Delta.toCombMap.Dart}
    (hd : Delta.toCombMap.faceOf (Delta.toCombMap.alpha d) ≠ R.face) :
...
theorem off_of_mem {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    {g : Delta.toCombMap.Face} (hg : g ∈ s) : g ≠ R.face :=
  fun h => hs (h ▸ hg)
...
theorem facePerm_keep {d : Delta.toCombMap.Dart} (hd : Delta.toCombMap.faceOf d ≠ R.face) :
...
/-- The faces of the deletion carrying an old face set. -/
...
theorem kept_mem_faceSet_iff {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    {g : Delta.toCombMap.Face} (hg : g ≠ R.face) : R.kept g ∈ R.faceSet s ↔ g ∈ s := by
  constructor
  · intro h
...
theorem shrunk_not_mem_faceSet {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s) :
    R.shrunk ∉ R.faceSet s := by
  intro h
  obtain ⟨g, hg, heq⟩ := R.mem_faceSet.mp h
  exact R.kept_ne_shrunk (R.off_of_mem hs hg) heq

theorem faceSet_disjoint_iff {s t : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    (ht : R.face ∉ t) : Disjoint (R.faceSet s) (R.faceSet t) ↔ Disjoint s t := by
  constructor
  · intro h
  ...
  · intro h
    ...
    exact Finset.disjoint_left.mp h hg
      (R.kept_inj (R.off_of_mem ht hg') (R.off_of_mem hs hg) heq ▸ hg')

theorem faceSet_inj {s t : Finset Delta.toCombMap.Face} (hs : R.face ∉ s) (ht : R.face ∉ t)
...
theorem faceOf_keep_mem_faceSet_iff {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
  ...
  by_cases hoff : Delta.toCombMap.faceOf d ≠ R.face
  · rw [R.faceOf_keep hoff, R.kept_mem_faceSet_iff hs hoff]
  · have hface : Delta.toCombMap.faceOf d = R.face := not_not.mp hoff
    rw [R.faceOf_keep_shrunk hd hface, hface]
    exact ⟨fun h => absurd h (R.shrunk_not_mem_faceSet hs), fun h => absurd h hs⟩

/-! ## Kept traversals -/

theorem faceBoundary_kept_darts (g : Delta.toCombMap.Face) (hg : g ≠ R.face) :
...
theorem faceWord_kept (g : Delta.toCombMap.Face) (hg : g ≠ R.face) :
...
theorem outerDarts_eq : outerDarts R.diagram = (outerDarts Delta).map R.keep := by
  ...
  calc ((R.diagram.faceBoundary R.diagram.outerFace).darts.reverse.map
    ...
    _ = (Delta.faceBoundary Delta.outerFace).darts.reverse.map Delta.toCombMap.alpha := by
        rw [R.outerFace_eq, R.faceBoundary_kept_map_val Delta.outerFace R.outer_ne]
...
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.map_keep_val
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.alpha_keep
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.facePerm_keep
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.faceOf_keep_mem_faceSet_iff
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.faceSet_inj
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.faceWord_kept
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.cellDarts_eq
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.outerDarts_eq
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.targetBoundaryDarts_map
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.dartWord_map
