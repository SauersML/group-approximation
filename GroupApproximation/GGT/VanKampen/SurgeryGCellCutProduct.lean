import GroupApproximation.GGT.VanKampen.SurgeryGCellCutRecord

/-!
# The boundary product of a cut piece

The piece cut out by a `RegionCutWithGCells` needs the van Kampen product identity:
...
theorem keptCell_value (cut : RegionCutWithGCells Delta)
...
theorem keptCells_value_prod (cut : RegionCutWithGCells Delta) :
    ((cut.cells.map
  ...
  have hshell := Embedded.listVal_dartWord_of_cellShellingWithGCells cut.shelling
...
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.MapCollapse.RegionCutWithGCells.keptCells_value_prod
