import GroupApproximation.GGT.VanKampen.SurgerySpikeDeletionDarts
import GroupApproximation.GGT.VanKampen.SurgeryGFaceMergeRegions
import GroupApproximation.Meta.AxiomGuard

/-!
# Contiguity regions and section families through a spike deletion

The transport half of the spike deletion (`Surgery.SpikeDeletion.diagram`), used in the
dart-count descent for Osin's Lemma 9.4, Case 2 (arXiv:math/0411039v3, §9).  It follows
`SurgeryGFaceMergeRegions`, with the face of the spike in place of the two merged faces.  A
face set `s` avoiding the face of the spike survives the deletion as `faceSet s = s.image kept`,
and each datum of a contiguity region on `s` transports along `keep`, which is injective on the
darts off the deleted edge, with left inverse `val`:
...
A compatible family avoiding the face of the spike transports to a compatible family of the
deletion with the same card, weight and target profiles (`regionFamily`, `regionFamily_card`,
`regionFamily_weight`, `regionFamily_pairwise`, `regionFamily_faces`, `regionFamily_profile`,
`regionFamily_avoid_shrunk`).  The unbound darts of every relator cell correspond through
`keep` (`mem_unboundDarts_regionFamily_iff`, `unboundDarts_eq`, `unboundDarts_card`,
`sum_unboundDarts_card`).  `transportSection` is the section family on the deletion, with the
same weight and card, nondegenerate, legal labels kept.
-/

namespace GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion
...
/-! ## Darts and faces of the deletion -/
...
/-- **A dart of the deletion is based in the image of `s` exactly when its old dart was based
in `s`.** -/
theorem faceOf_mem_faceSet_iff {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
...
theorem isBoundaryDart_keep_iff {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    {d : Delta.toCombMap.Dart} (hd : d ≠ R.dart ∧ d ≠ Delta.toCombMap.alpha R.dart) :
...
theorem internalMove_transport {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
...
theorem boundaryStep_transport {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
...
theorem cycle_offEdge {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
...
/-- **The boundary of a face set avoiding the face of the spike, after the deletion.** -/
noncomputable def transportBoundary {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    (B : FaceSetBoundary Delta s) : FaceSetBoundary R.diagram (R.faceSet s) where
  faces_nonempty := by
  ...
  all_gCells := by
    ...
    refine ⟨fun heq => (B.all_gCells g hg).1
      (R.kept_inj hoff R.outer_ne (heq.trans R.outerFace_eq)), ?_⟩
...
@[simp] theorem transportBoundary_cycle {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
    (B : FaceSetBoundary Delta s) : (R.transportBoundary hs B).cycle = B.cycle.map R.keep :=
  rfl

/-! ## Shellings -/

theorem shelling {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s)
...
theorem rightSide_offEdge {D : RelGenSet G Lambda} {eps : ℕ} {s : Finset Delta.toCombMap.Face}
    (hs : R.face ∉ s) (H : ContiguityGeometry D eps Delta s) :
...
/-- **A contiguity region avoiding the face of the spike, after the deletion.** -/
noncomputable def contiguityGeometry {D : RelGenSet G Lambda} {eps : ℕ}
    {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s) (H : ContiguityGeometry D eps Delta s) :
...
theorem contiguityGeometry_injective {D : RelGenSet G Lambda} {eps : ℕ}
    {s : Finset Delta.toCombMap.Face} (hs : R.face ∉ s) :
...
/-- A region candidate avoiding the face of the spike, after the deletion. -/
noncomputable def regionCandidate (a : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) :
    RegionCandidate D eps R.diagram :=
  ⟨R.faceSet a.val.1, R.contiguityGeometry a.property a.val.2⟩

theorem regionCandidate_weight (a : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) :
...
theorem regionCandidate_compatible (a b : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) :
...
theorem regionCandidate_injective :
    Function.Injective (R.regionCandidate :
      { a : RegionCandidate D eps Delta // R.face ∉ a.1 } → RegionCandidate D eps R.diagram) := by
...
/-- The deletion embedding of a family avoiding the face of the spike. -/
noncomputable def regionFamilyEmbedding (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) : family ↪ RegionCandidate D eps R.diagram where
  toFun a := R.regionCandidate ⟨a.val, havoid a.val a.property⟩
  inj' a b h := by
    apply Subtype.ext
    exact congrArg (fun x : { a : RegionCandidate D eps Delta // R.face ∉ a.1 } => x.val)
      (R.regionCandidate_injective h)

/-- **A family avoiding the face of the spike, after the deletion.** -/
noncomputable def regionFamily (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) : Finset (RegionCandidate D eps R.diagram) :=
...
theorem regionFamily_faces (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) {a : RegionCandidate D eps R.diagram}
...
theorem regionFamily_card (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) : (R.regionFamily family havoid).card = family.card := by
...
/-- No region of a transported family contains the shrunk face. -/
theorem regionFamily_avoid_shrunk (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) : ∀ a ∈ R.regionFamily family havoid, R.shrunk ∉ a.1 := by
  intro a ha
  obtain ⟨b, hb, hab⟩ := R.regionFamily_faces family havoid ha
  rw [hab]
  exact R.shrunk_not_mem_faceSet (havoid b hb)
...
theorem regionCandidate_target_eq_some_iff
    (a : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) (i : Fin Delta.rCellCount) :
...
theorem regionCandidate_target_isSome (a : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) :
    (R.regionCandidate a).2.target.isSome = a.val.2.target.isSome :=
  Option.isSome_map

theorem regionCandidate_sourceArc_darts
    (a : { a : RegionCandidate D eps Delta // R.face ∉ a.1 }) :
...
theorem mem_cellArcDarts_regionCandidate_iff
  ...
  rw [GFaceMerge.mem_cellArcDarts_iff, GFaceMerge.mem_cellArcDarts_iff,
    R.regionCandidate_source_eq_iff, R.regionCandidate_target_eq_some_iff,
    R.regionCandidate_sourceArc_darts, R.regionCandidate_targetArc_darts,
...
/-- A dart of a relator cell of the deletion is used by a transported region of either kind
...
theorem mem_boundDarts_regionFamily_iff (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) (interior : Bool)
    (i : Fin Delta.rCellCount) (x : R.diagram.toCombMap.Dart) :
  ...
  rw [GFaceMerge.mem_boundDarts_iff, GFaceMerge.mem_boundDarts_iff]
  constructor
...
/-- **A dart of a relator cell of the deletion is unbound exactly when its old dart was.** -/
theorem mem_unboundDarts_regionFamily_iff (family : Finset (RegionCandidate D eps Delta))
    (havoid : ∀ a ∈ family, R.face ∉ a.1) (i : Fin Delta.rCellCount)
    (x : R.diagram.toCombMap.Dart) :
  ...
  rw [GFaceMerge.mem_unboundDarts_iff, GFaceMerge.mem_unboundDarts_iff, R.cellDarts_eq i,
...
theorem unboundDarts_offEdge (family : Finset (RegionCandidate D eps Delta))
  ...
  R.cellDarts_offEdge i ((GFaceMerge.mem_unboundDarts_iff family i d).mp hd).1
...
/-- **A section family across a spike deletion of its diagram.** -/
...
end GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion

#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.transportBoundary
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.shelling
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.contiguityGeometry
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.contiguityGeometry_injective
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.regionCandidate_injective
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.regionFamily_card
...
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.transportSection
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.transportSection_weight
#audit_axioms GroupApproximation.GGT.VanKampen.Surgery.SpikeDeletion.transportSection_labelLegal
