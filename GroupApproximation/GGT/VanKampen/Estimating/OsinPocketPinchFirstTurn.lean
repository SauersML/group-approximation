...
/-!
# The pinch step for a pocket in first-turn order

Osin, arXiv:math/0411039v3, §9, proof of Lemma 9.7(b): the subdiagram `Γ_1` with
`∂Γ_1 = s_1 t_1 s_2 t_2`.  A pocket face set in walk order may pass twice through a vertex.  `exists_pinchStep_of_inside`
splits the vertex at two darts whose corners lie in the face set, provided every turn of the
boundary cycle runs along vertex rotation without meeting a split dart before its end.  With walk
order alone the turns at a vertex can cross, and then no split keeps them all.  This module takes
the order of `FirstTurnWalk` (`Estimating/OsinPocketFirstTurnWalk.lean`) instead: the turn from a
dart `d` of the boundary cycle into the next dart `e` rotates from `e` to `alpha d` past darts off
the edges of the cycle.  At a vertex visited twice, the split at the end of one turn keeps every
turn, keeps the order, and lowers the repeated visits.

* `PocketFaceSet.FirstTurns`: the pocket in first-turn order.  `firstTurns_iff` gives the chain
  condition of `firstTurnWalkPocketInputs` on the complement spelling, and `FirstTurns.closedWalk`
  gives walk order.
* `PocketFaceSet.exists_turn_of_not_unpinched`: a pinched pocket in first-turn order has a turn
  `d₀ → e₀`, with `d₀ ≠ e₀`, and another dart `e` of the cycle at the vertex of `e₀`.
* `PocketFaceSet.exists_turnInput`: the split at `x = alpha d₀` and the dart `y` before `e₀` has
  both faces inside the face set, when the faces of `d₀` and `e₀` are distinct G-faces.
* `PocketFaceSet.avoids_of_turn`: every turn of the cycle avoids `x` and `y` before its end.
* `PocketFaceSet.firstTurns_pinchSplitInside`: the split pocket is in first-turn order.
* `PocketFaceSet.pinchSplitInside_repeatedVisits_lt_of_turn`: the split lowers the repeated visits.
* `PocketFaceSet.GoodCorners`, `PocketPinchGoodCornerStepStatement`, `pocketPinchGoodCornerStep`:
  one step of the pinch for a pocket in first-turn order whose boundary darts lie on distinct
  G-faces.
...
/-- **A predecessor other than itself.**  In a list of distinct elements chained by `R` and closed
by `R` from its last to its first element, an element beside which the list holds another element
is reached by `R` from an element other than itself. -/
theorem exists_mem_ne_rel_of_isChain {α : Type*} {R : α → α → Prop} {l : List α}
    (hne : l ≠ []) (hnodup : l.Nodup) (hchain : l.IsChain R)
    (hclose : R (l.getLast hne) (l.head hne)) {a b : α} (ha : a ∈ l) (hb : b ∈ l)
    (hab : a ≠ b) : ∃ d ∈ l, d ≠ a ∧ R d a := by
  obtain ⟨i, hi, rfl⟩ := List.getElem_of_mem ha
  cases i with
  | zero =>
      have hlen : 0 < l.length - 1 := by
        obtain ⟨j, hj, rfl⟩ := List.getElem_of_mem hb
        by_contra h
        exact hab (by congr 1; omega)
      refine ⟨l.getLast hne, List.getLast_mem hne, ?_, ?_⟩
      · rw [List.getLast_eq_getElem]
        exact Ne.symm (List.pairwise_iff_getElem.mp hnodup 0 (l.length - 1) hi (by omega) hlen)
      · rw [← List.head_eq_getElem hne]
        exact hclose
  | succ j =>
      refine ⟨l[j], List.getElem_mem (by omega), ?_,
        List.isChain_iff_getElem.mp hchain j (by omega)⟩
      exact List.pairwise_iff_getElem.mp hnodup j (j + 1) (by omega) hi (by omega)
...
/-- **A first turn of the reversed walk, read on the walk.**  The reversed walk of `c` turns from
`alpha e` to `alpha d` exactly when rotating `k > 0` steps from `e` reaches `alpha d` past darts
off the edges of `c`. -/
theorem reverse_map_alpha_iff {c : List M.Dart} {d e : M.Dart} :
    FirstTurn M (c.reverse.map M.alpha) (M.alpha e) (M.alpha d) ↔
      ∃ k : ℕ, 0 < k ∧ (M.sigma ^ k) e = M.alpha d ∧
        ∀ t, 0 < t → t < k → ¬ walkKeep M c ((M.sigma ^ t) e) := by
  unfold FirstTurn
  rw [M.alpha_involutive e, FirstTurnWalk.walkKeep_reverse_map_alpha]
...
/-- **A pocket in first-turn order**: the turn from each dart `d` of the boundary cycle into the
next dart `e`, and from the last dart into the first, rotates from `e` to `alpha d` past darts off
the edges of the cycle.  On the complement spelling `cycle.reverse.map alpha` this is the chain
condition of `firstTurnWalkPocketInputs` (`firstTurns_iff`). -/
...
/-- **First-turn order is the chain condition of `firstTurnWalkPocketInputs`** on the complement
spelling `cycle.reverse.map alpha`. -/
theorem firstTurns_iff (K : PocketFaceSet D eps X lo hi)
    (hne : K.boundary.cycle.reverse.map X.toCombMap.alpha ≠ []) :
    K.FirstTurns ↔
      (K.boundary.cycle.reverse.map X.toCombMap.alpha).IsChain
          (FirstTurn X.toCombMap (K.boundary.cycle.reverse.map X.toCombMap.alpha)) ∧
        FirstTurn X.toCombMap (K.boundary.cycle.reverse.map X.toCombMap.alpha)
          ((K.boundary.cycle.reverse.map X.toCombMap.alpha).getLast hne)
          ((K.boundary.cycle.reverse.map X.toCombMap.alpha).head hne) := by
  unfold FirstTurns
  simp only [List.isChain_map, List.isChain_reverse, List.getLast_map, List.getLast_reverse,
    List.head_map, List.head_reverse]

/-- **A pocket in first-turn order is in walk order.** -/
theorem FirstTurns.closedWalk {K : PocketFaceSet D eps X lo hi} (hK : K.FirstTurns) :
    K.ClosedWalk := by
...
theorem boundary_alpha_not_mem (K : PocketFaceSet D eps X lo hi) {z : X.toCombMap.Dart}
...
/-- **A pinched pocket in first-turn order has a turn into a vertex visited twice**: a turn
`d₀ → e₀` of the boundary cycle with `d₀ ≠ e₀`, and another dart `e` of the cycle at the vertex
of `e₀`. -/
theorem exists_turn_of_not_unpinched (K : PocketFaceSet D eps X lo hi) (hK : K.FirstTurns)
...
/-- **Every turn avoids the split darts before its end.**  Let `x = alpha d₀` and let `y` be the
dart before `e₀`, for darts `d₀ e₀` of the boundary cycle.  A turn of the cycle rotating `k` steps
from `e` to `alpha d`, past darts off the edges of the cycle, meets neither `x` nor `y` before its
end. -/
theorem avoids_of_turn (K : PocketFaceSet D eps X lo hi) (I : PinchSplit.Input X)
...
/-- **The split at the end of a turn.**  For a turn `d₀ → e₀` of the boundary cycle, rotating
`k₀` steps from `e₀` to `alpha d₀`, whose darts lie on distinct faces other than relator faces,
`x = alpha d₀` and the dart `y` before `e₀` form a pinch with both faces inside the face set. -/
theorem exists_turnInput [DecidableEq X.toCombMap.Dart] (K : PocketFaceSet D eps X lo hi)
    {d₀ e₀ : X.toCombMap.Dart} (hd₀ : d₀ ∈ K.boundary.cycle) (he₀ : e₀ ∈ K.boundary.cycle)
...
/-- **A turn survives the split**: composing rotation with the transposition of `x` and `y`
changes no turn of the cycle, since every turn avoids `x` and `y` before its end. -/
theorem firstTurn_pinchSplitInside [DecidableEq X.toCombMap.Dart]
    (K : PocketFaceSet D eps X lo hi) (I : PinchSplit.Input X) {d₀ e₀ : X.toCombMap.Dart}
  ...
  · exact (PinchFirstTurn.mul_swap_pow_apply_of_avoids X.toCombMap.sigma I.x I.y e k
      havoid).trans hk'
  · have heq : (I.diagram.toCombMap.sigma ^ t) e = (X.toCombMap.sigma ^ t) e :=
      PinchFirstTurn.mul_swap_pow_apply_of_avoids X.toCombMap.sigma I.x I.y e t
        fun i hi => havoid i (hi.trans htk)
    rw [heq]
    exact hkeep t ht htk

/-- **The split pocket is in first-turn order.** -/
...
/-- **The split at the end of a turn lowers the repeated visits.**  The turn `d₀ → e₀` runs from
`e₀` into `x` without passing `y`; another dart `e` of the cycle at the vertex of `e₀` runs into
`y` at its first return to `e₀`, without passing `x`. -/
theorem pinchSplitInside_repeatedVisits_lt_of_turn [DecidableEq X.toCombMap.Dart]
  ...
  exact K.pinchSplitInside_repeatedVisits_lt_of_stretch I hs he₀ he (hk₀.trans hx.symm) hy₁ hy₂
    hx₂

/-! ## The step at good corners -/

/-- **One step of the pinch at a turn into a vertex visited twice**, for a pocket in first-turn
order on a diagram whose labels are letters, when the darts of the turn lie on distinct faces
other than relator faces. -/
theorem exists_firstTurnStep_of_turn
    (hlabel : ∀ d, (symmetricLabelAlphabet D).IsLetter (X.label d))
    (K : PocketFaceSet D eps X lo hi) (hK : K.FirstTurns) {d₀ e₀ e : X.toCombMap.Dart}
...
def GoodCorners (K : PocketFaceSet D eps X lo hi) : Prop :=
...
/-- **One step of the pinch for a pinched pocket in first-turn order with good corners.** -/
...
/-- **One step of the pinch for a pocket in first-turn order with good corners.**  A pinched
pocket face set in first-turn order, whose boundary darts lie on distinct faces other than relator
faces, on a diagram whose labels are letters, has an O-equivalent copy whose labels are letters,
with a pocket face set in first-turn order and fewer repeated visits.  Infrastructure for the
pinch under first-turn order; certifies no printed sentence on its own. -/
def PocketPinchGoodCornerStepStatement : Prop :=
...
theorem pocketPinchGoodCornerStep : PocketPinchGoodCornerStepStatement.{u, w, v} := by
  intro G _ Lambda D eps W X lo hi hlabel K hK hgood hpinch
  exact K.exists_firstTurnStep_of_goodCorners hlabel hK hgood hpinch

end GroupApproximation.GGT.VanKampen

#audit_axioms GroupApproximation.GGT.VanKampen.PinchFirstTurn.exists_mem_ne_rel_of_isChain
#audit_axioms GroupApproximation.GGT.VanKampen.FirstTurn.reverse_map_alpha_iff
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.firstTurns_iff
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.FirstTurns.closedWalk
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.exists_turn_of_not_unpinched
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.avoids_of_turn
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.exists_turnInput
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.firstTurns_pinchSplitInside
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.pinchSplitInside_repeatedVisits_lt_of_turn
#audit_axioms GroupApproximation.GGT.VanKampen.PocketFaceSet.exists_firstTurnStep_of_goodCorners
#audit_closed_axioms GroupApproximation.GGT.VanKampen.pocketPinchGoodCornerStep
