...
theorem targetBoundary_quasi_lower (Gamma : ContiguityGeometry D eps Delta faces)
  ...
  have hlen : (Gamma.targetArcAtSome target htarget).length = Gamma.targetArc.length :=
    CyclicArc.length_cast _ Gamma.targetArc
  have hquasi := hcondition.quasiGeodesic _
    (CyclicArc.cell_rotated_mem hcondition.toIsSmallCancellation (i := target)
      (Gamma.targetArcAtSome target htarget))
  have hlength : (Gamma.targetArcAtSome target htarget).length ≤
      (dartWord Delta (Gamma.targetArcAtSome target htarget).rotated).length := by
    simpa only [dartWord, List.length_map, CyclicArc.rotated_length]
      using (Gamma.targetArcAtSome target htarget).length_le
  have h := hquasi.2 0 _ (Nat.zero_le _) hlength
  have hprefix : (dartWord Delta (Gamma.targetArcAtSome target htarget).rotated).take
      (Gamma.targetArcAtSome target htarget).length =
        dartWord Delta (Gamma.targetArcAtSome target htarget).darts := by
    simp only [dartWord, CyclicArc.darts, List.map_take]
  have hlower : lambda * ((Gamma.targetArcAtSome target htarget).length : ℝ) - c ≤
      (wordNorm D.alphabet.carrier (RelLetter.listVal
        (dartWord Delta (Gamma.targetArcAtSome target htarget).darts)) : ℝ) := by
    simpa only [Nat.sub_zero, OsinComponents.vertex_zero,
      OsinComponents.vertex_eq_mul_listVal_take, one_mul, hprefix,
      List.take_zero, RelLetter.listVal_nil, wordDist_one_left] using h
  have hboundary : targetBoundaryDarts Delta Gamma.target Gamma.targetArc =
      (Gamma.targetArcAtSome target htarget).reverseDarts :=
