...
self-contiguous region, the equations exist at every interior edge with both arcs
nonempty of a least-area diagram, and an edge with an empty arc is charged by
quasi-geodesicity (`RegionCandidate.weight_le_of_degenerate`); together they give the
budget (`interior_total_le_of_o52`).
-/
...
noncomputable def pieceEquations_of_o52 (hO52 : O52LeastAreaStatement.{u, w, v})
    ...
    (hloop : edge.val.2.source ≠ edge.target)
    (hs : 0 < edge.val.2.sourceArc.length) (ht : 0 < edge.val.2.targetArc.length) :
    edge.val.2.PieceEquations where
  target := edge.target
  target_eq := edge.target_eq
  whole_ne := hO52 edge.val.2 edge.target edge.target_eq hlea hloop hs ht
...
/-- **Lemma 9.5's interior budget from per-edge charges**, `Σ_inn ≤ 10μΣ`, given the
charge `2μ l(∂Π)` at either endpoint of every interior edge and the endpoint-closed
planar edge bound. -/
theorem interior_total_le' (selected : Finset (RegionCandidate D eps Delta))
    ...
    {mu : ℝ} (hmu : 0 ≤ mu)
    (hweight : ∀ (i : Fin Delta.rCellCount) (edge : InteriorEdge selected),
      InteriorEdge.Incident i edge →
        (edge.val.weight : ℝ) ≤ 2 * mu * ((cell Delta i).word.length : ℝ)) :
    (∑ i : Fin Delta.rCellCount, ((boundDarts selected true i).card : ℝ)) ≤
...
theorem interior_total_le (selected : Finset (RegionCandidate D eps Delta))
  ...
  interior_total_le' selected hpairwise hplanar hmu
    (InteriorEdge.weight_le_incident hcondition.toIsSmallCancellation
      hcondition.publishedPiecesSmall hequations)

/-- **The interior budget at a least-area diagram**, from `O52LeastAreaStatement` at
the interior edges with both arcs nonempty, quasi-geodesicity at the others, no
self-contiguous selected region, and the planar edge bound. -/
theorem interior_total_le_of_o52 (hO52 : O52LeastAreaStatement.{u, w, v})
    ...
    (∑ i : Fin Delta.rCellCount, ((boundDarts selected true i).card : ℝ)) ≤
      10 * mu * ∑ i : Fin Delta.rCellCount, ((cell Delta i).word.length : ℝ) := by
  refine interior_total_le' selected hpairwise hplanar hmu.le ?_
  intro i edge hi
  by_cases hpos : 0 < edge.val.2.sourceArc.length ∧ 0 < edge.val.2.targetArc.length
  · have equations := InteriorEdge.pieceEquations_of_o52 hO52 hlea edge (hloops edge)
      hpos.1 hpos.2
    rw [weight, Nat.cast_add]
    rcases hi with hsrc | htgt
    · rw [← hsrc]
      exact edge.val.2.arcLengths_le_two_mu_source equations hcondition.toIsSmallCancellation
        hcondition.publishedPiecesSmall
    · have hj : equations.target = i :=
        Option.some.inj (equations.target_eq.symm.trans htgt)
      rw [← hj]
      exact edge.val.2.arcLengths_le_two_mu_target equations hcondition.toIsSmallCancellation
        hcondition.publishedPiecesSmall
  · have hdeg : edge.val.2.sourceArc.length = 0 ∨ edge.val.2.targetArc.length = 0 := by
      rcases Nat.eq_zero_or_pos edge.val.2.sourceArc.length with h1 | h1
      · exact Or.inl h1
      rcases Nat.eq_zero_or_pos edge.val.2.targetArc.length with h2 | h2
      · exact Or.inr h2
      exact absurd ⟨h1, h2⟩ hpos
    exact edge.val.weight_le_of_degenerate edge.target edge.target_eq hcondition hlambda hmu
      hlarge hdeg i
