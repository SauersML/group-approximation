...
def RealizedRegionFamily.LabelLegal (E : RelGenSet G Lambda)
    (S : RealizedRegionFamily D eps Delta) : Prop :=
  ∀ d, E.IsLetter (S.diagram.label d)

structure LegallyDistinguishedRegionFamily (D E : RelGenSet G Lambda) (eps : ℕ)
    (Delta : DiscDiagram.{u, w, v} W) extends RealizedRegionFamily D eps Delta where
  label_admissible : toRealizedRegionFamily.LabelLegal E
  weight_maximal : ∀ other : RealizedRegionFamily D eps Delta,
    other.LabelLegal E → other.weight ≤ toRealizedRegionFamily.weight
...
theorem exists_legallyDistinguishedRegion_of_seed
    (initial : RealizedRegionFamily D eps Delta) (hlegal : initial.LabelLegal E) :
    Nonempty (LegallyDistinguishedRegionFamily D E eps Delta) := by
  ...
  let values := (Finset.range (bound + 1)).filter fun n =>
    ∃ S : RealizedRegionFamily D eps Delta, S.LabelLegal E ∧ S.weight = n
  have hmem (S : RealizedRegionFamily D eps Delta) (hS : S.LabelLegal E) :
      S.weight ∈ values := by
...
theorem exists_legallyDistinguishedRegionFamily
    ...
    Nonempty (LegallyDistinguishedRegionFamily D (symmetricLabelAlphabet D) eps Delta) := by
  obtain ⟨R⟩ := symmetricLabelAlphabet.exists_normalization D Delta hboundary hcells
  let S : RealizedRegionFamily D eps Delta := {
    diagram := R.diagram
    equiv := R.oEquivalent _
    reduced := R.reduced _ hred
    family := ∅
    pairwise := by intro a ha; simp at ha }
  exact exists_legallyDistinguishedRegion_of_seed S R.label_admissible

/-- Changing only the selected family leaves label eligibility intact. -/
def LegallyDistinguishedRegionFamily.toDistinguishedFamily
    (S : LegallyDistinguishedRegionFamily D E eps Delta) :
  ...
  card_minimal := fun other hother heq =>
    S.card_minimal { S.toRealizedRegionFamily with family := other, pairwise := hother }
      S.label_admissible heq

end GroupApproximation.GGT.VanKampen

#audit_axioms GroupApproximation.GGT.VanKampen.exists_legallyDistinguishedRegion_of_seed
#audit_axioms GroupApproximation.GGT.VanKampen.exists_legallyDistinguishedRegionFamily
#audit_axioms GroupApproximation.GGT.VanKampen.LegallyDistinguishedRegionFamily.toDistinguishedFamily
