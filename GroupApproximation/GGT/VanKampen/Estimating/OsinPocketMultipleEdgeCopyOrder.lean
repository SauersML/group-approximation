...
import GroupApproximation.GGT.VanKampen.Estimating.OsinPocketCellSideRelatorCell
import GroupApproximation.Meta.AxiomGuard

/-!
# The multiple-edge pocket region on a copy, through the chosen order

`multipleEdgePocketRegionCopyInput_of_pinch` (`OsinPocketMultipleEdgeCopy`) builds the cell pocket
face set of the walk of the order `(a, b)` only, from a fixed-order residual saying the exterior is
off its side.  That residual is over-strong: the exterior lies in at most one of the two
complementary pockets (lane `w1-binder-3`).  This module builds the region data from the walk whose
side does not hold the exterior.

On the clean optimal family of `CellPocketCopyCleanBothOrdersStatement`:
* both walks, of the orders `(a, b)` and `(b, a)` between the same cells, are nonempty closed walks
  (`CellPocketWalk.exists_of_joinsCells_closedWalk`) with no repeated dart and no dart with its
  reverse (`CellPocketWalk.walk_nodup_and_alpha_not_mem`, from the edge conditions of that order);
* `CellPocketWalkOuterOffSideSomeOrderStatement` selects a walk with the exterior off its side;
* that walk is noncrossing by its orientation (`CellPocketWalkColour.walk_orient`,
  `ClosedWalkFaceColouring.isNoncrossingClosedWalk_of_orient`);
* `CellPocketWalkEitherFollowsStatement` gives a following cycle, and the proved
  `cellPocketKeptCellNoncrossing` puts a relator cell on its side;
* `CellPocket.exists_cellPocketFaceSet_closedWalk_of_orient` and `CellPocketFaceSet.copyRegion_of_pinch`
  give the region data on a copy.

The named residuals are `CellPocketCopyCleanBothOrdersStatement`,
`CellPocketWalkOuterOffSideSomeOrderStatement`, `CellPocketWalkEitherFollowsStatement` and
...
/-- **The region data from one walk with the exterior off its side.**  For distinct selected regions
`a`, `b` of an optimal family joining the distinct cells `i`, `j` under the edge conditions, a cell
pocket walk with the walk data of the order `(a, b)` and the closed-walk facts, whose side does not
hold the exterior, gives the region data of the copy form at `0 < ε`. -/
theorem copyRegion_of_offSideWalk
  ...
  have hkept := cellPocketKeptCellNoncrossing S ha hb hab hij hai hbi K hfirst hsecond h₁ hlea' hw
    hoff (hfollow hlea S ha hb hab hij hai hbi hclean K hfirst hsecond h₁ h₂ hw)
...
/-- **The multiple-edge pocket region on a copy, through the chosen order**, at `0 < ε`: the clean
family in both orders, both walks with their closed-walk facts, the walk with the exterior off its
side, and the region data from that walk. -/
theorem multipleEdgePocketRegionCopyInput_of_pinchOrder
  ...
  · exact copyRegion_of_offSideWalk hfollow hpinch heps hlea S' ha' hb' hab' hij' hai' hbi'
      hclean₁ K₁ hf₁ hs₁ hA₁ hB₁ hne₁ hch₁ hcl₁ hnd₁ hal₁ hoff
  · exact copyRegion_of_offSideWalk hfollow hpinch heps hlea S' hb' ha' hab'.symm hij' hbi' hai'
      hclean₂ K₂ hf₂ hs₂ hA₂ hB₂ hne₂ hch₂ hcl₂ hnd₂ hal₂ hoff

end Order

/-- **The binder on a copy through the chosen order**, with `ε₀ = 1` and `ρ₀ = 1`. -/
theorem osinMultipleEdgePocketRegionCopySection_of_pinchOrder
    (hcopy : CellPocketCopyCleanBothOrdersStatement.{u, w, v})
    (hout : CellPocketWalkOuterOffSideSomeOrderStatement.{u, w, v})
    (hfollow : CellPocketWalkEitherFollowsStatement.{u, w, v})
    (hpinch : CellPocketPinchPosStatement.{u, w, v}) :
  ...
  exact ⟨1, fun _ heps => ⟨1, Nat.one_pos, fun _ _ _ _ =>
    multipleEdgePocketRegionCopyInput_of_pinchOrder hcopy hout hfollow hpinch heps⟩⟩

/-- **Osin's Lemma 4.4 at least-area diagrams, with binder 5 through the chosen order**: the waist of
`_of_residualsV2CopySection`, with binder 5 from four named residuals through
`osinMultipleEdgePocketRegionCopySection_of_pinchOrder`. -/
theorem relativeGreendlingerQuasiGeodesicLeastArea_of_residualsV2CopyOrder
  ...
  relativeGreendlingerQuasiGeodesicLeastArea_of_residualsV2CopySection hcount hrcell htwogon
    (osinMultipleEdgePocketRegionCopySection_of_pinchOrder hcopy hout hfollow hcellPinch) hfaces
    hpinch

end GroupApproximation.GGT.VanKampen

#audit_axioms GroupApproximation.GGT.VanKampen.copyRegion_of_offSideWalk
#audit_axioms GroupApproximation.GGT.VanKampen.multipleEdgePocketRegionCopyInput_of_pinchOrder
#audit_axioms GroupApproximation.GGT.VanKampen.osinMultipleEdgePocketRegionCopySection_of_pinchOrder
#audit_axioms GroupApproximation.GGT.VanKampen.relativeGreendlingerQuasiGeodesicLeastArea_of_residualsV2CopyOrder
