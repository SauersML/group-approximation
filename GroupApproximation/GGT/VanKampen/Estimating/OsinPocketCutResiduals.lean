...
producer.  Three of them are proved:
...
(`PocketPinchLabelledStatement`, lane `hull-respell`), the collar insertion
(`GeodesicCollarStatement`, lane `kh-torsion`) and the cell transport
(`PocketCellTransportStatement`, lane `go-lemma42`), and on nothing else.
...
/-- **`SectionPocketCutInput` from the residual pieces**: the pinch, the collar insertion and the
cell transport, given the face set between the two regions. -/
theorem sectionPocketCutInput_of_residuals
  ...
  sectionPocketCutInput_of_pieces hpinch pocketRegionOfSimple
    (pocketCollarStatement_of_geodesicCollar hgeodesic) hcell pocketOuterTransport hcondition
    hlambda hc hfaces

/-- **The section pocket producer from the residual pieces, uniformly in the parameters**: the
face set producer, the pinch, the collar insertion and the cell transport. -/
theorem osinSectionPocketCutSection_of_residuals
  ...
  osinSectionPocketCutSection_of_pieces hfaces hpinch pocketRegionOfSimple
    (pocketCollarStatement_of_geodesicCollar hgeodesic) hcell pocketOuterTransport
