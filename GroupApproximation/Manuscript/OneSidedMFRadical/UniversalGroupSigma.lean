...
/-!
# `B` is finitely generated and normally generated by an element of order four

`non_mf_groups_exist.tex`, `thm:full-defect-ring`:

> the group `B` is finitely generated, has property~(T), and is normally
> generated by an element of order four

`UniversalGroupB` carries property `(T)`, the triviality of every
homomorphism to an MF group, and the homomorphism into `EL_n(R)`.  This module
carries the remaining two clauses and joins all four.

## The element

Printed, and one-indexed, `σ = e₁₂(1) e₂₁(-1) e₁₂(1)`.  That word is the tree's
`ElementaryWeyl.weyl`, so `σ = weyl 0 1` in the zero-indexed spelling, and the
Weyl API supplies its conjugation action already.

## Order four

On the `{0,1}` block `σ` is the rotation `!![0,1;-1,0]`, so `σ² = diag(-1,-1,1,1)`
and `σ⁴ = 1`.  Both are read off `ElementaryWeyl.weylUnit_val`, which presents
the Weyl unit as `1 - E₀₀ - E₁₁ + E₀₁ - E₁₀`.

`σ² ≠ 1` is exactly `2 ≠ 0` in `𝒞`, and that is where the universal property is
spent: `UniversalPair.lift` sends `𝒞` into the binary Leavitt algebra over `ℚ`,
a nontrivial `ℚ`-algebra and so of characteristic zero.  Note that `𝒞` itself is
*not* known here to have characteristic zero as a stated fact; what is proved is
the single consequence `(2 : 𝒞) ≠ 0`, which is all the order computation needs.

## Normal generation

The printed chain, zero-indexed, with `N` the normal closure of `σ`:

* `σ e₀₂(-1) σ⁻¹ = e₁₂(±1)` — the Weyl conjugation, whose sign is not pinned
  down by `weyl_conj_elGen`;
* `c = e₀₂(1) σ e₀₂(-1) σ⁻¹ = e₀₂(1) e₁₂(ε)` with `ε = ±1`, and `c ∈ N` because
  `N` is normal and contains `σ`;
* `e₀₁(1)` commutes with `e₀₂(1)` and Steinberg-shifts `e₁₂(ε)` to
  `e₀₂(ε) e₁₂(ε)`, so `e₀₁(1) c e₀₁(1)⁻¹ = e₀₂(1 + ε) e₁₂(ε)` and therefore
  `⁅e₀₁(1), c⁆ = e₀₂(ε)`.

The sign survives to the end and is harmless: `e₀₂(-1)` is the inverse of
`e₀₂(1)`, so either branch puts `e₀₂(1)` in `N`, and a root with coefficient `1`
is unit-sandwiched, so `elementaryGroup_normal_eq_top_of_elGen_mem` closes it at
rank four.
-/
...
/-- **The printed `σ = e₁₂(1) e₂₁(-1) e₁₂(1)`**, zero-indexed. -/
...
/-- The printed root `e₀₂(1)` lies in the normal closure of `σ`. -/
...
/-- **All four printed clauses about `B`**, as one closed proposition: the
three of `PrintedUniversalGroupCore`, finite generation, and normal generation
by an element of order four. -/
...
theorem manuscriptFullComplementaryIdempotentsUniversal :
  ...
  ⟨manuscriptUniversalGroupCore, finitelyGenerated,
    sigma, orderOf_sigma, normalClosure_sigma_eq_top⟩
