...
* Proof of `thm:full-defect-ring`, introducing `G = EL_n(R)` and
  `L = EL₃(R) ≤ G`: "Both groups have property~(T)
  by~[EJZ, Theorem~1.1]."
* Proof of `thm:headline`, at the manuscript's own rank: `H = EL₄(R)` has
  property~(T) by the same theorem, the ring `R` being finitely generated.
...
`Steinberg.GeneralRankFiniteFieldPropertyT` proves the general-rank statement
outright, as the named proposition
...
* `hasKazhdanPropertyT_viaEJZ` -- the theorem at `n = 4`;
* `corner_hasKazhdanPropertyT_viaEJZ` -- the theorem at `n = 3`, transported
  along the surjection of `RankFourEndpoints`;
...
Nothing here replaces `RankFourEndpoint.hasKazhdanPropertyT` or
`RankFourEndpoint.corner_hasKazhdanPropertyT`.  Those are used throughout the
rank-four development and remain the shorter proofs; they also reach ranks `1`
and `2`, which the EJZ theorem does not.  The pair of `_eq` lemmas below
records that the new declarations prove the *same* propositions -- which they
do by proof irrelevance, both being proofs of a `Prop`; the content of the
pairing is that the two statements are the same statement, not that the two
...
`n = 4`.  It is the printed derivation, not a Leavitt-specific one: nothing
...
theorem hasKazhdanPropertyT_viaEJZ : HasKazhdanPropertyT.{0, 0} H :=
  generalRankFiniteFieldElementaryPropertyT (ZMod 2) R 4 (by norm_num)

/-- **The corner half of "Both groups have property (T) by the theorem of
Ershov and Jaikin-Zapirain."**
...
upper-left corner subgroup of `H`, and `cornerEmbeddingToCorner` maps `EL₃(R)`
onto it -- injectively, in fact, but only surjectivity is used. -/
...
*the statement the rank-four development already uses*, with no change of
...
#audit_closed_axioms RankFourEndpoint.hasKazhdanPropertyT_viaEJZ
#audit_closed_axioms RankFourEndpoint.corner_hasKazhdanPropertyT_viaEJZ
