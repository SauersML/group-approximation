...
This module proves `CoronaUCPFiniteCoordinateLocalizationClaim` unconditionally. The
printed equivalence is assembled in `Full/NM16/BKSentence` as
`manuscriptSentence_blackadarKirchbergNFIff`. Lane NM16 duplicates the foreign (never
compiled) `NonMF/RelatedBK/LocalizationTheorem` here without its `RelatedBK.Sentence`
import.
...
local instance boundedMatrixSequenceCStarAlgebraForLocalizationTheoremNM16
...
local instance matrixBlockCStarAlgebraForLocalizationTheoremNM16
...
theorem coronaUCPFiniteCoordinateLocalization :
    CoronaUCPFiniteCoordinateLocalizationClaim := by
