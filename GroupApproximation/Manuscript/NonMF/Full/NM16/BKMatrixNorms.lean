...
`CoronaUCPFiniteCoordinateLocalizationClaim`. The proof route is in
`Full/NM16/BKLocalization`. Lane NM16 duplicates the foreign
`NonMF/RelatedBK/LocalizationMatrixNorms` here.
