...
*GL02 copy.*  Re-proof of the foreign module `Piece04.RotationTurnsWalk`
(origin/main blob `d05eaf16b`), moved to namespace `Piece04.GL02` since the modules it
...
theorem firstTurn_of_face_run {c : List M.Dart} {a g b : M.Dart} {k : ℕ}
    (hpa : M.facePerm a = g) (hk : Piece04.RotationTurns.SafeRun M c g b k) : FirstTurn M c a b := by
  subst hpa
  exact Piece04.RotationTurns.firstTurn_of_safeRun hk
...
theorem passHead_core {t e : M.Dart} {T' q' γ : List M.Dart}
    ...
    (he : (B ++ p).head? = some e) :
    ∃ k, Piece04.RotationTurns.SafeRun M (B ++ X) t e k := by
  have hnotT := not_keep_T h List.mem_cons_self
  have hBpA := head?_append_of_head? he A
  by_cases hγn : γ = []
  · subst hγn
    exact ⟨1, Piece04.RotationTurns.SafeRun.one hnotT (sigma_eq_of_facePerm_alpha (succ_blocks (s := q')
  ...
  have hαe : M.alpha e ∉ γ := fun hm => h.face_ne t htw _ (hγmem _ hm)
    (by rw [← Piece04.RotationTurns.facePerm_alpha hed, M.faceOf_facePerm])
  ...
  obtain ⟨k, hk⟩ :=
    Piece04.RotationTurns.loop_safeRun h.planar hγnd hγchain hg₀ hg₁ hfe hed hdg heγ hαe hdγ hγc hc
  exact ⟨k + 1, (Piece04.RotationTurns.SafeRun.one hnotT hdg).trans hk⟩
...
theorem passHead (h : PocketSetting M X T Y U q B p A) {t e : M.Dart} (ht : T.head? = some t)
    (he : (B ++ p).head? = some e) : ∃ k, Piece04.RotationTurns.SafeRun M (B ++ X) t e k := by
...
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.faceCycle_of_eq
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.ne_of_nodup
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.face_eq
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.sigma_eq_of_facePerm_alpha
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.head?_append_of_head?
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.head?_left_of_ne_nil
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.getLast?_right_of_ne_nil
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.succ_blocks
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.firstTurn_of_face_run
#audit_axioms
  GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.PocketSetting.ne_of_face
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.not_keep_T
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.not_keep_U
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.passHead_core
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.RotationTurns.passHead
