...
*GL02 copy.*  Re-proof of the foreign module `Piece04.WindowShape.ClassDisjointPosition`
(origin/main blob `776bb7e2f`), moved to namespace `Piece04.GL02`.  The original uses
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.WindowShape.ClassDisjoint.faceOf_of_mem_sideDarts
...
theorem mem_side_of_getElem?_of_length_le (Q : OsinLemma94ClassPolygons P) (k : Fin P.count)
  ...
  have hlast : (Q.classDarts k i).getLast? = some x := by
    ...
    exact hx
  rw [hdarts, List.getLast?_append, List.getLast?_eq_some_getLast hside, Option.some_or] at hlast
  refine ⟨sl, hslmem, ?_⟩
  rw [← Option.some.inj hlast]
  exact List.getLast_mem hside

#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.WindowShape.ClassDisjoint.mem_side_of_getElem?_of_length_le
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.WindowShape.ClassDisjoint.side_or_gap_succ
