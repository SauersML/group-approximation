...
*GL02 copy.*  Re-proof of the foreign module `Piece04.WindowShape.WindowHeadSplitAux`
(origin/main blob `75ba2eb0f`), moved to namespace `Piece04.GL02`.  The original uses
`List.getLast?_eq_getLast`, deprecated in Lean core (an error under `warningAsError`); the copy
uses `List.getLast?_eq_some_getLast`, which has the same statement.  Otherwise only namespaces, the
imports of copied modules, and the qualification of foreign names differ from the original.
...
theorem split_aux (Q : OsinLemma94ClassPolygons P) (k : Fin P.count) :
  ...
  | s :: L, x, y, hne, hng, hlt => by
    ...
    · have hrx : Q.runPos k (s :: L) x = x := Piece04.WindowShape.WindowHeadSplit.runPos_cons_of_le Q k s L x (by omega)
      ...
      · rw [List.length_nil, Nat.add_zero]
        exact Piece04.WindowShape.WindowHeadSplit.lt_of_runPos_lt Q k (s :: L) x y hlt
    by_cases h2 : x = (P.sideDarts k s).length
    · have hrx : Q.runPos k (s :: L) x = (P.sideDarts k s).length :=
        (Piece04.WindowShape.WindowHeadSplit.runPos_cons_of_le Q k s L x (by omega)).trans h2
      have hy : ¬ y ≤ (P.word k s).length + (Embedded.dartWord S.diagram (Q.gap k s)).length := by
        intro hy
        have _hle := Piece04.WindowShape.WindowHeadSplit.runPos_cons_le_of_le Q k s L y hy
        omega
      have hry := Piece04.WindowShape.WindowHeadSplit.runPos_cons_of_gt Q k s L y hy
      obtain ⟨s', L'', t, R', rfl, hR'⟩ := Piece04.WindowShape.WindowHeadSplit.exists_next_side Q k L
      ...
      · by_cases hg : Q.gap k s = []
        ...
        · obtain ⟨w, G', hG'⟩ := List.exists_cons_of_ne_nil hg
          ...
          refine Or.inr ⟨0, s, s', (P.sideDarts k s).getLast hside, w, by omega, ?_, ?_,
            ...
            List.getLast?_eq_some_getLast hside, by rw [hR', List.head?_cons]⟩
          · rw [hcd, List.getElem?_append_left hx1, h2]
            exact List.getLast?_eq_getElem?.symm.trans (List.getLast?_eq_some_getLast hside)
    ...
    · have hrx : Q.runPos k (s :: L) x = (P.sideDarts k s).length :=
        (Piece04.WindowShape.WindowHeadSplit.runPos_cons_of_mid Q k s L x (by omega) (by omega)).trans e1
      have hy : ¬ y ≤ (P.word k s).length + (Embedded.dartWord S.diagram (Q.gap k s)).length := by
        intro hy
        have _hle := Piece04.WindowShape.WindowHeadSplit.runPos_cons_le_of_le Q k s L y hy
        omega
      ...
      · rw [hrx, hsr, List.drop_left]
        simp only [List.flatMap_cons, hR', List.cons_append]
      · rw [hcd2, Piece04.WindowShape.WindowHeadSplit.drop_append_of_length_le hle2,
    ...
    have hgt : ¬ x ≤ (P.word k s).length + (Embedded.dartWord S.diagram (Q.gap k s)).length := by
      omega
    have hrx := Piece04.WindowShape.WindowHeadSplit.runPos_cons_of_gt Q k s L x hgt
    have hy : ¬ y ≤ (P.word k s).length + (Embedded.dartWord S.diagram (Q.gap k s)).length := by
      intro hy
      have _hle := Piece04.WindowShape.WindowHeadSplit.runPos_cons_le_of_le Q k s L y hy
      omega
    ...
    · rw [hrx, hsr, Piece04.WindowShape.WindowHeadSplit.drop_append_add_of_eq e1.symm]
      exact hR
    · rw [hcd2, Piece04.WindowShape.WindowHeadSplit.drop_append_of_length_le hle2, show x - (P.sideDarts k s ++ Q.gap k s).length =
...
#audit_axioms GroupApproximation.GGT.VanKampen.GreendlingerLeaf.Piece04.GL02.WindowShape.WindowHeadSplit.split_aux
