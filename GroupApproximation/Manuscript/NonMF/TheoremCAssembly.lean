...
/- The five admissions below emit `declaration uses 'sorry'`, and the lakefile
builds this library with `-DwarningAsError=true`, which would make those fatal.
So the exemption is necessary — but it is attached to each admission with
`set_option warningAsError false in` rather than scoped to the file, so that
every other warning here (unused variables, deprecations, linter hits) stays
fatal.  A file-scoped `set_option` would silence all of them in the module that
carries Theorem C's entire proof surface: a deprecated `push_neg` cost another
lane a probe round today, and here it would have been silent. -/
