...
/-!
# Lane nm-tww-11, part 3: diagonalising the approximately invariant density

`NuclearAmenable.HasApproxInvariantDensities τ` supplies a ucp model `φ` and a Hermitian `T`
...
(`ConnesTrick.ucp_conjLinear_comp`), and every clause turns into an explicit sum over matrix
entries:
...
`NuclearAmenable.HasApproxInvariantDensities` after the density has been diagonalised. Here
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.ConnesTrick.HasDiagonalDensities
...
theorem hasDiagonalDensities_of_hasApproxInvariantDensities {τ : A → ℂ}
    (h : NuclearAmenable.HasApproxInvariantDensities τ) : HasDiagonalDensities τ := by
  intro F ε hε
