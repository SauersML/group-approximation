...
/-!
# Lane nm-tww-12, part 2: densities of functionals on matrix algebras

This file contains elementary facts that the reduction in `Reduction` uses.

* `apply_star_of_nonneg`: a linear functional that is nonnegative on every `x⋆x` is
  hermitian.  The proof polarizes at `1 + y` and `1 + i·y`.
* `densityOf ω`: the density matrix of a functional `ω` on `M_Y`, with
  `tr(densityOf ω · X) = ω X`.
* If `ω` is hermitian and nonnegative on rank-one projections `v v⋆`, then its density
  is positive semidefinite.
* The trace identities for a Hermitian square root `T` of `densityOf ω`:
  `tr(T X T) = ω X` and `tr((zT)ᴴ(zT)) = ω(z⋆z)`.
...
/-- **A functional that is nonnegative on `x⋆x` is hermitian.**  The imaginary part of
`ω(x⋆x)` vanishes at `x = y`, at `x = 1`, at `x = 1 + y` and at `x = 1 + i·y`. -/
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.apply_star_of_nonneg
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.densityOf
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.trace_densityOf_mul

/-- The trace of the density is the value at `1`. -/
theorem trace_densityOf (ω : Matrix Y Y ℂ →ₗ[ℂ] ℂ) : Matrix.trace (densityOf ω) = ω 1 := by
  rw [← trace_densityOf_mul ω 1, mul_one]

#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.trace_densityOf

/-- The quadratic form of the density at `v` is the value at `v v⋆`. -/
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.dotProduct_densityOf_mulVec
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.densityOf_isHermitian

/-- The density of a hermitian functional that is nonnegative on every `v v⋆` is positive
semidefinite. -/
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.densityOf_posSemidef
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.trace_T_mul_T
...
#audit_axioms GroupApproximation.Manuscript.NonMF.TWWLanes.NuclearDensity.trace_conjTranspose_mul_T
