import GroupApproximation.Manuscript.NonMF.RelatedBK.LocalizationStatement
import GroupApproximation.Manuscript.NonMF.RelatedBK.UnitalizedNuclear
...
/-!
# Related work (Blackadar--Kirchberg): nuclear and MF implies NF, modulo localization
...
This module proves the converse half for unital C-star algebras from the
single named gap `CoronaUCPFiniteCoordinateLocalizationStatement`.

## Proof route

* *Trivial algebra.* If `A` is a subsingleton, the zero maps into `M_1` are an
  NF approximation, because every norm in `A` vanishes.
* *Nontrivial algebra* (`hasLocalNFApproximation_of_nuclear_of_mf_of_localization`).
...
local instance boundedMatrixSequenceCStarAlgebraForRelatedBK
...
local instance matrixBlockCStarAlgebraForRelatedBK
...
/-- **Nuclear plus MF gives local NF approximations, modulo localization.**
This is the nontrivial unital case. -/
theorem hasLocalNFApproximation_of_nuclear_of_mf_of_localization
  ...
  obtain ⟨Y, hY, ρ, θ, hρtail, hθCP, hθContract, hθρ⟩ :=
    hloc X D Ext hExtCP hExtOne (F.image (fun a ↦ L (α a))) δ hδ0 N
  letI : Nonempty Y := hY
...
/-- **Nuclear and MF implies NF, modulo the localization gap.** This covers
unital C-star algebras. The separability part of NF is the separability part
of MF. -/
theorem isNFAlgebra_of_isNuclearCStarAlgebra_of_isMFAlgebra_of_coronaUCPFiniteCoordinateLocalization
    (hloc : CoronaUCPFiniteCoordinateLocalizationStatement)
  ...
  · exact hasLocalNFApproximation_of_nuclear_of_mf_of_localization
      hloc hnuc hMF F hε
...
#audit_axioms hasLocalNFApproximation_of_nuclear_of_mf_of_localization
#audit_axioms isNFAlgebra_of_isNuclearCStarAlgebra_of_isMFAlgebra_of_coronaUCPFiniteCoordinateLocalization
