...
restriction to a homomorphic image removes any countability requirement on
the target, and the identity map is a homomorphism to an MF group, so a
nontrivial group all of
whose homomorphisms to MF groups are trivial is not MF
(`not_isOperatorMF_of_killsMFTargets`).
-/
...
theorem not_isOperatorMF_of_killsMFTargets {Q : Type} [Group Q]
    [Nontrivial Q] (h : KillsAllMFTargets Q) : ¬ IsOperatorMF Q := by
  intro hQ
  obtain ⟨x, hx⟩ := exists_ne (1 : Q)
  exact hx (h Q inferInstance hQ (MonoidHom.id Q) x)
...
theorem quotient_not_isOperatorMF_of_killsMFTargets {Q : Type} [Group Q]
    (h : KillsAllMFTargets Q) {L : Type} [Group L] (r : Q →* L)
    (hr : Function.Surjective r) [Nontrivial L] : ¬ IsOperatorMF L := by
  intro hL
  obtain ⟨y, hy⟩ := exists_ne (1 : L)
  obtain ⟨x, rfl⟩ := hr y
  exact hy (h L inferInstance hL r x)
