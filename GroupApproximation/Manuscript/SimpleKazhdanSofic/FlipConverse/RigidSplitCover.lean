...
theorem exists_split_of_disjoint {X Y : Type*} [TopologicalSpace X] [TopologicalSpace Y]
  ...
  let ι := {f : topologicalFullGroup T // SupportedIn (f : X ≃ₜ X) U}
  have hcov : U ⊆ ⋃ i : ι, movedSet ((i.1 : topologicalFullGroup T) : X ≃ₜ X) := by
    intro x hx
  ...
  obtain ⟨t, ht⟩ := hU.isClosed.isCompact.elim_finite_subcover
    (fun i : ι => movedSet ((i.1 : topologicalFullGroup T) : X ≃ₜ X))
    (fun i => (isClopen_movedSet hTa i.1.2).isOpen) hcov
  refine ⟨⋃ i ∈ t, movedSet ((Φ i.1 : topologicalFullGroup S) : Y ≃ₜ Y),
    isClopen_biUnion_finset fun i _ => isClopen_movedSet hSa (Φ i.1).2, ?_, ?_⟩
  · intro f hf
    have hN : IsOpen (movedSet ((Φ f : topologicalFullGroup S) : Y ≃ₜ Y) ∩
        (⋃ i ∈ t, movedSet ((Φ i.1 : topologicalFullGroup S) : Y ≃ₜ Y))ᶜ) :=
      (isClopen_movedSet hSa (Φ f).2).isOpen.inter
        (isClopen_biUnion_finset fun i _ => isClopen_movedSet hSa (Φ i.1).2).isClosed.isOpen_compl
    have hfix := supportedIn_compl_of_forall_commute hS hN fun σ hσ hσN => by
      have hgU : SupportedIn ((Φ.symm ⟨σ, hσ⟩ : topologicalFullGroup T) : X ≃ₜ X) Uᶜ := by
        refine supportedIn_iff_movedSet_subset.2 fun x hx hxU => ?_
  ...
  · intro k hk
    refine supportedIn_iff_movedSet_subset.2 fun y hy hyV => ?_
    obtain ⟨i, hi, hyi⟩ := Set.mem_iUnion₂.1 hyV
    have hd : Disjoint (movedSet ((i.1 : topologicalFullGroup T) : X ≃ₜ X))
        (movedSet (k : X ≃ₜ X)) := by
      refine Set.disjoint_left.2 fun x hx1 hx2 => ?_
      exact supportedIn_iff_movedSet_subset.1 hk hx2 (supportedIn_iff_movedSet_subset.1 i.2 hx1)
