...
/-!
# Shifted Pauli operators commute with a Pauli stage
...
The inclusion `𝓡 ⊗̄ 𝓡 ≅ 𝓡` used coordinatewise is realised by moving the second tensor factor past
the sites used by the first one.  A Pauli word `p = (x, z)` of `𝓡` is shifted by `j` sites,
`shiftOp j p := pauliOp (x ∘ (· - j)) (z ∘ (· - j))`.  This module proves:
...
* `commute_shiftOp_stageHom`: they commute with the stage `stageHom j : M_{2^j}(ℂ) → 𝓡`, because the
  shifted supports and the stage supports are disjoint.
* `coefficient_shiftOp_mul_stageHom`: `τ(shiftOp j p · stageHom j B) = 0` for `p ≠ 1`, because every
  product `shiftOp j p · stageOp x z` is a Pauli word with a nonzero shifted part.
...
theorem shiftCoord_apply_add (j : ℕ) (v : PauliCoord) (i : ℕ) : shiftCoord j v (i + j) = v i :=
...
theorem dot_shiftCoord (j : ℕ) (u v : PauliCoord) :
  ...
  exact Finsupp.sum_congr fun i _ ↦ congrArg (u i * ·) (shiftCoord_apply_add j v i)
...
theorem dot_stageEmb_shiftCoord (x : StageIdx j) (v : PauliCoord) :
  ...
  rw [shiftCoord_apply_lt j v (lt_of_stageEmb_ne_zero (Finsupp.mem_support_iff.mp hi)), mul_zero]
...
theorem coefficient_shiftOp_mul_stageOp {p : PauliGroup} (hp : p ≠ 1) (x z : StageIdx j) :
  ...
  have hne : ¬(shiftCoord j (Multiplicative.toAdd p).1 + stageEmb x = 0 ∧
    ...
    exact hp (Multiplicative.toAdd.injective
      (Prod.ext (eq_zero_of_shiftCoord_add_stageEmb h1) (eq_zero_of_shiftCoord_add_stageEmb h2)))
  rw [shiftOp, stageOp, pauliOp_mul, map_smul, ambientCoefficientAtOne_apply, coefficient_pauliOp,
...
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.commute_shiftOp_stageHom
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.coefficient_shiftOp_mul_stageHom
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.shiftOp_mul
