import GroupApproximation.Manuscript.SimpleKazhdanSofic.ROmega.TwoNorm
...
/-!
# `L(G) ↪ R^ω`: the bounded resolvent `q(t) = t / (1 + t²)`
...
Step 2 of the route described in `ROmega/TwoNorm`.  For a self-adjoint `a` of a unital C⋆-algebra,
`q(a) = a (1 + a²)⁻¹` is given by the continuous functional calculus, with `R_a = (1 + a²)⁻¹`:

* `‖R_a‖ ≤ 1`, `‖q(a)‖ ≤ 1/2`, and `q(a)` lies in every von Neumann algebra containing `a`
...
This is the replacement for Kaplansky's density theorem in step 3.
-/
...
theorem cfc_mem {x : H →L[ℂ] H} (hx : x ∈ M) (f : ℝ → ℝ) : cfc f x ∈ M := by
...
theorem norm_qfun_sub_apply_le (τ : TracialVector M) {a b : H →L[ℂ] H} (haM : a ∈ M)
  ...
  have hRa : cfc rfun a ∈ M := cfc_mem haM rfun
  have hRb : cfc rfun b ∈ M := cfc_mem hbM rfun
...
def qseq (r : boundedSeq N) : boundedSeq N :=
  ⟨fun n ↦ cfc qfun ((r : ℕ → (K →L[ℂ] K)) n),
    mem_boundedSeq_iff.mpr ⟨fun n ↦ cfc_mem (coord_mem r n) qfun, 1 / 2,
...
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.cfc_qfun_cfc_gfun
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.exists_poly_approx
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.norm_qfun_sub_apply_le
#audit_axioms GroupApproximation.SimpleKazhdanSofic.ROmega.twoNorm_mk_qseq_sub_le
