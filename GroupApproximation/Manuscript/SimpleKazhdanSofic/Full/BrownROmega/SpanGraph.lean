...
/-!
# Brown's formulation: the graph spanned by a family of generator pairs

`simple_kazhdan_sofic_group.tex`, subsection "Brown's formulation":
"`L(G)` embeds in `𝓡^ω`" and "`L(G) ⊗̄ 𝓡` ... embeds in `𝓡^ω`".

This is the fifth step of this lane's route to those embeddings, after
`Full/BrownROmega/GraphExtension`. Take a family `U : Λ → B(H) × N^ω` whose linear span contains
`1` and is closed under products and adjoints of generators. That span is a `⋆`-subalgebra, the
graph `spanGraph`. Membership of the first coordinates in `M`, trace compatibility and density
only have to be checked on the generators, and `exists_injective_of_generators` passes the graph
to `exists_extension`.
-/
...
theorem exists_injective_of_generators (hω : (ω : Filter ℕ) ≤ cofinite) (hUM : ∀ g, (U g).1 ∈ M)
  ...
  have hdense' :
      ∀ T ∈ M, ∀ ε > 0, ∃ p ∈ spanGraph U hone hmul hstar, ‖(p.1 - T) τ.vec‖ < ε := by
    intro T hT ε hε
...
#audit_axioms GroupApproximation.Full.SK06.exists_injective_of_generators
