...
/-!
# Brown's formulation: the bounded closure of a trace-preserving graph

`simple_kazhdan_sofic_group.tex`, subsection "Brown's formulation":
"`L(G)` embeds in `𝓡^ω`" and "`L(G) ⊗̄ 𝓡` ... embeds in `𝓡^ω`".

Third step of this lane's route to those embeddings (after `Full/BrownROmega/TwoNorm` and
`Full/BrownROmega/Resolvent`). Let `M` be a von Neumann algebra with a tracial vector `ξ`, and let
`Gr ⊆ M × N^ω` be a `⋆`-subalgebra (a *graph*) with `trω(p₂) = ⟪ξ, p₁ ξ⟫` for `p ∈ Gr`.

* `GraphRel T v`: `(T, v)` is approximated by elements `p ∈ Gr` with `‖p₁‖` and a representative
  of `p₂` uniformly bounded, and with `‖(p₁ − T) ξ‖` and `‖p₂ − v‖₂` small.
* Traciality gives `‖p₂‖₂ = ‖p₁ ξ‖` on `Gr`, so `GraphRel` is a function (`rel_unique`).
* It is closed under `+`, `⋆`, scalars and products (with `T ∈ M`), and it preserves the trace.

`Full/BrownROmega/GraphExtension` shows that every `T ∈ M` is related to something, which gives the
extension.
-/
...
def GraphRel (T : H →L[ℂ] H) (v : Ultrapower σ ω) : Prop :=
  ∃ B : ℝ, ∀ ε > 0, ∃ p ∈ Gr, ‖p.1‖ ≤ B ∧
...
theorem rel_of_mem {p : (H →L[ℂ] H) × Ultrapower σ ω} (hp : p ∈ Gr) :
    GraphRel τ Gr p.1 p.2 := by
...
theorem rel_one : GraphRel τ Gr 1 1 :=
  rel_of_mem τ Gr (one_mem Gr)

theorem rel_zero : GraphRel τ Gr 0 0 :=
  rel_of_mem τ Gr (zero_mem Gr)

theorem rel_algebraMap (c : ℂ) :
    GraphRel τ Gr (algebraMap ℂ (H →L[ℂ] H) c) (algebraMap ℂ (Ultrapower σ ω) c) :=
  rel_of_mem τ Gr (Gr.algebraMap_mem c)

/-- **`GraphRel` is a function.** -/
theorem rel_unique (htr : ∀ p ∈ Gr, ultratrace σ ω p.2 = ⟪τ.vec, p.1 τ.vec⟫_ℂ)
    {T : H →L[ℂ] H} {v w : Ultrapower σ ω} (h1 : GraphRel τ Gr T v)
    (h2 : GraphRel τ Gr T w) : v = w := by
...
theorem rel_add {T S : H →L[ℂ] H} {v w : Ultrapower σ ω} (h1 : GraphRel τ Gr T v)
    (h2 : GraphRel τ Gr S w) : GraphRel τ Gr (T + S) (v + w) := by
  obtain ⟨B, h1⟩ := h1
  obtain ⟨B', h2⟩ := h2
...
theorem mul_add_mul_lt {B C S x y ε : ℝ} (hB : 0 ≤ B) (hC : 0 ≤ C) (hS : B + C < S)
  ...
  have h4 : (B * x + C * y) * S < ε * S := by nlinarith
  exact lt_of_mul_lt_mul_right h4 hSpos.le

/-- **Products.** -/
theorem rel_mul (hGM : ∀ p ∈ Gr, p.1 ∈ M) {T S : H →L[ℂ] H} {v w : Ultrapower σ ω}
    (hTM : T ∈ M) (hT : GraphRel τ Gr T v) (hS : GraphRel τ Gr S w) :
    GraphRel τ Gr (T * S) (v * w) := by
  obtain ⟨B, hT⟩ := hT
  obtain ⟨B', hS⟩ := hS
  ...
  refine ⟨B * B', fun ε hε ↦ ?_⟩
  obtain ⟨p, hp, hpB, ⟨a, ha, haB⟩, hpT, hpv⟩ :=
    hT (ε / (B' + ‖T‖ + Cv + 1)) (div_pos hε hpos)
  obtain ⟨q, hq, hqB, ⟨b, hb, hbB⟩, hqS, hqw⟩ :=
    hS (ε / (B' + ‖T‖ + Cv + 1)) (div_pos hε hpos)
  ...
  · have e : (p * q).1 - T * S = (p.1 - T) * q.1 + T * (q.1 - S) := by
    ...
    have h2 : ‖(T * (q.1 - S)) τ.vec‖ ≤ ‖T‖ * ‖(q.1 - S) τ.vec‖ := by
      ...
      exact T.le_opNorm _
    have h3 :=
      mul_add_mul_lt hB' hTn (by linarith : B' + ‖T‖ < B' + ‖T‖ + Cv + 1) hε hpT hqS
    linarith
  · have e : (p * q).2 - mk σ ω c * w =
        (p.2 - mk σ ω c) * mk σ ω b + mk σ ω c * (q.2 - w) := by
      rw [Prod.snd_mul, ← hb]
      noncomm_ring
    rw [e]
    ...
    have h3 :=
      mul_add_mul_lt hB' hCv0 (by linarith : B' + Cv < B' + ‖T‖ + Cv + 1) hε hpv hqw
    linarith

/-- **Adjoints.** -/
theorem rel_star (hGM : ∀ p ∈ Gr, p.1 ∈ M) {T : H →L[ℂ] H} {v : Ultrapower σ ω}
    (hTM : T ∈ M) (h : GraphRel τ Gr T v) : GraphRel τ Gr (star T) (star v) := by
  obtain ⟨B, h⟩ := h
  refine ⟨B, fun ε hε ↦ ?_⟩
...
theorem rel_trace (htr : ∀ p ∈ Gr, ultratrace σ ω p.2 = ⟪τ.vec, p.1 τ.vec⟫_ℂ)
    {T : H →L[ℂ] H} {v : Ultrapower σ ω} (h : GraphRel τ Gr T v) :
...
theorem rel_smul (hGM : ∀ p ∈ Gr, p.1 ∈ M) (c : ℂ) {T : H →L[ℂ] H} {v : Ultrapower σ ω}
    (h : GraphRel τ Gr T v) : GraphRel τ Gr (c • T) (c • v) := by
  have h' :=
    rel_mul τ Gr hGM (M.toStarSubalgebra.algebraMap_mem c) (rel_algebraMap τ Gr c) h
...
theorem coord_polyEval (c : ℕ → ℂ) (m : ℕ) (r : boundedSeq N) (n : ℕ) :
  ...
  have h2 :
      polyEval c m (r : ℕ → (K →L[ℂ] K)) n = polyEval c m ((r : ℕ → (K →L[ℂ] K)) n) :=
    map_polyEval (Pi.evalAlgHom ℂ (fun _ : ℕ ↦ K →L[ℂ] K) n) c m
      (r : ℕ → (K →L[ℂ] K))
  rw [h1]
  exact h2
...
#audit_axioms GroupApproximation.Full.SK06.rel_unique
#audit_axioms GroupApproximation.Full.SK06.rel_mul
#audit_axioms GroupApproximation.Full.SK06.rel_trace
