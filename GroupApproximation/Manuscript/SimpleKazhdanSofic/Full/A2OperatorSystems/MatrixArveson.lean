...
/-!
# Arveson's extension theorem for matrix targets, from operator systems (table row A2)

`simple_kazhdan_sofic_group.tex`, tex l.54–56 (table row A2): Ozawa, *About the QWEP conjecture*,
Theorem 2.2, the ucp case into `B(ℂᵏ)`.  Choi's route:
1. the Choi functional on matrices over `E` is nonnegative on ambient positives
   (`systemChoiFunctional_nonneg`, `ChoiFunctional.lean`);
2. the Krein step extends it to a positive functional `s` on `Mₖ(A)`, hermitian by
   `StateExtension.hermitian_of_nonneg` (`exists_positive_extension_of_system`, `StateExtension.lean`);
3. the extension is read off single-entry matrices, `Ψ(x)_{pq} = s(E_{pq}(x))`;
4. complete positivity is the block-operator computation of `Analysis/LanceMatrixArveson.lean`,
   whose theorem covers closed star subalgebras in `Type`; here the domain is any operator system and
   `A` lives in any universe.
Re-proof of the never-wired orphan `Analysis/OperatorSystemMatrixArveson.lean` (left untouched).
-/
...
/-- The single-entry embedding `A → Mₖ(A)` as a linear map (universe-polymorphic copy of
`CStarExactness.singleEntryLM`). -/
...
/-- **Arveson's extension theorem for matrix targets, from an operator system** (Ozawa, *About the
QWEP conjecture*, Theorem 2.2, ucp case; table row A2, tex l.54–56).  A unital map `ψ : E → B(ℂᵏ)`
that is completely positive on the operator system `E` extends to a unital completely positive map
on `A`. -/
theorem exists_ucp_extension_of_system [PartialOrder A] [StarOrderedRing A]
  ...
  have hunit : Ψ 1 = 1 := by
    calc Ψ 1 = Ψ ((⟨1, And.left hE⟩ : E) : A) := rfl
      _ = ψ ⟨1, And.left hE⟩ := hextend ⟨1, And.left hE⟩
      _ = 1 := hψ1 ⟨1, And.left hE⟩ rfl
