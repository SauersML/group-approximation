...
theorem derivedFullGroupSubshift_fg (hinf : Infinite S.carrier)
  ...
  have hmin' : SimpleKazhdanSofic.IsMinimalSystem (SimpleKazhdanSofic.subshiftHomeo S) :=
    SimpleKazhdanSofic.minimalSubshift_dense_orbits S hmin
  have hDG : derivedFullGroupSubshift S = Subgroup.closure (gens S (2 * R + 1)) := by
    apply le_antisymm
    ...
    · rw [Subgroup.closure_le]
      rintro _ ⟨p, rfl⟩
      show (if h : TowerDisj (SimpleKazhdanSofic.subshiftHomeo S).toEquiv
          (cylW S (2 * R + 1) p.1) 4 then
            towerPerm (SimpleKazhdanSofic.subshiftHomeo S).toEquiv (cylW S (2 * R + 1) p.1)
              (towerLevels 4) h p.2.1
          else 1) ∈ derivedFullGroupSubshift S
      split_ifs with h
