...
theorem eq_one_of_col_fix_finset (L : I) (T : Finset I) (hLT : L ∉ T) :
  ...
  | insert b T hbT ih =>
    intro u hu hfix
    have hbL : b ≠ L := fun h => hLT (h ▸ Finset.mem_insert_self b T)
...
theorem eq_one_of_col_fix (L : I) {u : SteinbergGroup I R}
  ...
  exact rootSpan_mono (fun i j hij hj => ⟨hj, Finset.mem_erase.mpr ⟨hj ▸ hij, Finset.mem_univ i⟩⟩)
    hu
...
theorem eq_one_of_row_zero_finset (L : I) (T : Finset I) (hLT : L ∉ T) :
  ...
  | insert b T hbT ih =>
    ...
    obtain ⟨d, v', hv', rfl⟩ := exists_split (q := fun i j => i = L ∧ j ∈ T) hbL.symm
      ...
      (fun i j h _ _ hq => x_commute_of_ne L b i j hbL.symm h
        (by rw [hq.1]; exact hbL) (by rw [← hq.1]; exact h.symm)) hv
    have h1 : act v' (unitVec b) = unitVec b :=
      act_eq_self_of_rootSpan (p := fun i j => i = L ∧ j ∈ T)
        (fun _ j hp => by rw [unitVec_apply, if_neg (fun hjb => hbT (hjb ▸ hp.2))]) hv'
    have hd : d = 0 := by
      have e := hfix b hbL
...
theorem eq_one_of_row_zero (L : I) {v : SteinbergGroup I R}
  ...
  exact rootSpan_mono
    (fun i j hij hi => ⟨hi, Finset.mem_erase.mpr ⟨hi ▸ hij.symm, Finset.mem_univ j⟩⟩) hv
