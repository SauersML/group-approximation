...
`simple_kazhdan_sofic_group.tex` at origin/main e80dcf20a, Theorem `thm:main` (tex l.48–62):
...
* `PrintedMarkedLimitExpanders S n`: `EL_n(R S)` is such a limit of finite simple groups
  `SL_{nN}(F₂)`, with expanding Cayley graphs.

## Pieces (named propositions, with the owning lanes)

* `PrintedSubshiftRingFGStatement`: `R` is a finitely generated ring (tex l.97–99; ct-involution);
* `PrintedSimplicityStatement`: `EL_n(R)` is simple for `n ≥ 3` (section "Simplicity", tex
  l.179–237; ct-bilateral-mf and skf-consequences);
* `PrintedMarkedLimitExpandersStatement`: the marked limits and expanders (section "Finite models",
  tex l.134–164; ms-units and ms-core-3);
* `PrintedMarkedLimitLEFStatement`: a marked limit of finite groups is LEF (tex l.162–163;
  ms-core-3).
...
the generating matrices (tex l.109–110, lane ms-compress-4).  `EL_n(R)` is infinite because `R` is
infinite (tex l.113–114).  LEF groups are sofic and hyperlinear (tex l.164–166).
...
/-- **The marked-limit clause of `thm:main`** (tex l.56–58, l.60–61): `EL_n(R S)` is the limit, in
...
/-- **Theorem `thm:main` as printed** (tex l.48–62), for every `n ≥ 3`; `n = 3` is `G_X = G S`. -/
...
/-- **`R` is finitely generated** (tex l.97–99): it is generated by `u^{±1}` and the letter
indicators. -/
def PrintedSubshiftRingFGStatement : Prop :=
  ∀ (A : Type) [TopologicalSpace A] [DiscreteTopology A] [Finite A] (S : Subshift A ℤ),
    IsFinitelyGeneratedRing (R S)

/-- **Simplicity** (section "Simplicity", tex l.179–237): for an infinite minimal subshift, `EL_n(R)`
...
/-- **Marked limits and expanders** (section "Finite models", tex l.134–164). -/
def PrintedMarkedLimitExpandersStatement : Prop :=
  ∀ (A : Type) [TopologicalSpace A] [DiscreteTopology A] [Finite A] (S : Subshift A ℤ),
    Infinite S.carrier → IsMinimal S → ∀ n : ℕ, 3 ≤ n → PrintedMarkedLimitExpanders S n

/-- **A marked limit of finite groups is LEF** (tex l.162–163: "So these finite simple groups
...
theorem nontrivial_clopenCoeff (S : Subshift A ℤ) [Nonempty S.carrier] :
    Nontrivial (ClopenCoeff (subshiftHomeo S) (ZMod 2)) := by
  ...
  exact inferInstanceAs (Nontrivial (LocallyConstant S.carrier (ZMod 2)))

theorem infinite_ring (S : Subshift A ℤ) [Nonempty S.carrier] : Infinite (R S) := by
  haveI := nontrivial_clopenCoeff S
  haveI : Infinite (Multiplicative ℤ) := inferInstanceAs (Infinite ℤ)
  exact Pestov91.skewMonoidAlgebra_infinite

/-- **`EL_n(R)` is infinite** (tex l.113–114). -/
...
theorem printedSimpleKazhdanSoficMain_of_pieces (hfg : PrintedSubshiftRingFGStatement)
  ...
  exact ⟨infinite_elementaryGroup S hinf hn, KazhdanFiniteGeneration.fg_of_hasKazhdanPropertyT _ hT,
    hsimple A S hinf hmin n hn, hT, hmarked A S hinf hmin n hn, hLEF, isSofic_of_isLEF hLEF,
