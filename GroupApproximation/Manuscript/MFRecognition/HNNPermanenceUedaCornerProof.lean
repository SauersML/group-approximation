...
its covariance is the image of the covariance already given, and it is
nontrivial as soon as `e_Q ≠ 0`.  Evaluating an element of the ambient product
of `U` at that coordinate is contractive, so the family of those values is an
element of the ambient product of `P`.  It lands in `ePe` because the ambient
elements whose value does form a closed star subalgebra containing the two
generators.

The construction is carried out at abstract C-star algebras.  At the concrete
algebras of a `CoronaConjugator` the same steps are unusable: every class
derived from the C-star structure of either bounded product is then a search
over a family indexed by concrete matrix algebras, and each one exhausts the
typeclass budget.  Only the instantiation happens at the concrete algebras,
and the coordinate projection is nonzero there because a unital homomorphism
out of `M₂(D)` killing `e₀₀` also kills `e₁₁ = e₁₀e₀₀e₀₁` and hence the unit.
...
variable {theta : B0 ≃⋆ₐ[ℂ] B1} [Nonempty (CStarHNNRepresentation B0 B1 theta)]
...
variable (e : UniversalCStarAmalgam iA iB)

/-- The image of the corner projection in one compatible coordinate. -/
def coordinateProjection (Q : CStarAmalgamRepresentation iA iB) : Q.carrier :=
  universalCStarAmalgamEval iA iB Q e

theorem coordinateProjection_star (he_star : star e = e)
    (Q : CStarAmalgamRepresentation iA iB) :
    star (coordinateProjection e Q) = coordinateProjection e Q :=
  (map_star (universalCStarAmalgamEval iA iB Q) e).symm.trans
    (congrArg (universalCStarAmalgamEval iA iB Q) he_star)

theorem coordinateProjection_mul (he_mul : e * e = e)
    (Q : CStarAmalgamRepresentation iA iB) :
    coordinateProjection e Q * coordinateProjection e Q =
      coordinateProjection e Q :=
  (map_mul (universalCStarAmalgamEval iA iB Q) e e).symm.trans
    (congrArg (universalCStarAmalgamEval iA iB Q) he_mul)

variable (he_star : star e = e) (he_mul : e * e = e)

/-- Corner functoriality along one coordinate evaluation. -/
def coordinateCornerMap (Q : CStarAmalgamRepresentation iA iB) :
    CStarCorner e he_star he_mul →⋆ₐ[ℂ]
      CStarCorner (coordinateProjection e Q)
        (coordinateProjection_star e he_star Q)
        (coordinateProjection_mul e he_mul Q) :=
  CStarCorner.map (universalCStarAmalgamEval iA iB Q) rfl

variable (pi : D →⋆ₐ[ℂ] CStarCorner e he_star he_mul)
variable (u : unitary (CStarCorner e he_star he_mul))
variable (hcov : ∀ b : B0,
  (u : CStarCorner e he_star he_mul) * pi (b : D) =
    pi ((theta b : B1) : D) * (u : CStarCorner e he_star he_mul))
variable (hne : ∀ Q : CStarAmalgamRepresentation iA iB,
  coordinateProjection e Q ≠ 0)

/-- The covariant representation of the HNN datum carried by one compatible
amalgam coordinate. -/
def coordinateHNNRepresentation (Q : CStarAmalgamRepresentation iA iB) :
    CStarHNNRepresentation B0 B1 theta :=
  transportedHNNRepresentation pi u hcov (coordinateCornerMap e he_star he_mul Q)
    (cStarCorner_nontrivial (coordinateProjection_star e he_star Q)
      (coordinateProjection_mul e he_mul Q) (hne Q))

/-- The coordinate family of the corner lift. -/
def cornerLiftCoord (y : UniversalCStarHNNAmbient B0 B1 theta)
    (Q : CStarAmalgamRepresentation iA iB) : Q.carrier :=
  CStarCorner.inclusion (coordinateProjection e Q)
    (coordinateProjection_star e he_star Q)
    (coordinateProjection_mul e he_mul Q)
    (y (coordinateHNNRepresentation e he_star he_mul pi u hcov hne Q))

theorem cornerLiftCoord_norm_le (y : UniversalCStarHNNAmbient B0 B1 theta)
    (Q : CStarAmalgamRepresentation iA iB) :
    ‖cornerLiftCoord e he_star he_mul pi u hcov hne y Q‖ ≤ ‖y‖ :=
  lp.norm_apply_le_norm ENNReal.top_ne_zero y
    (coordinateHNNRepresentation e he_star he_mul pi u hcov hne Q)

/-- The corner lift, as an element of the ambient bounded product of the
amalgam. -/
def cornerLiftAmbient (y : UniversalCStarHNNAmbient B0 B1 theta) :
...
@[simp] theorem cornerLiftHom_apply (y : UniversalCStarHNNAmbient B0 B1 theta) :
...
def projectionAmbient : UniversalCStarAmalgamAmbient iA iB :=
  (universalCStarAmalgamSubalgebra iA iB).subtype e

@[simp] theorem cornerLiftAmbient_one :
    cornerLiftAmbient e he_star he_mul pi u hcov hne 1 = projectionAmbient e :=
  rfl

theorem projectionAmbient_star :
...
def cornerAmbientSet : Set (UniversalCStarAmalgamAmbient iA iB) :=
...
theorem cornerAmbientSet_isClosed :
    IsClosed (cornerAmbientSet e he_star) :=
  (universalCStarAmalgam_isClosed iA iB).inter
    (cStarCornerSubalgebra_isClosed (projectionAmbient e)
      (projectionAmbient_star e he_star))

theorem subtype_mem_cornerAmbientSet (c : CStarCorner e he_star he_mul) :
  ...
  ⟨(c : UniversalCStarAmalgam iA iB).property,
    ...
    (map_mul (universalCStarAmalgamSubalgebra iA iB).subtype
      ...
      (congrArg (universalCStarAmalgamSubalgebra iA iB).subtype c.property.2)⟩

theorem projectionAmbient_mem :
    projectionAmbient e ∈ cornerAmbientSet e he_star :=
  subtype_mem_cornerAmbientSet e he_star he_mul 1

@[simp] theorem cornerLiftAmbient_base (d : D) :
...
def cornerLiftPreimage : StarSubalgebra ℂ
    (UniversalCStarHNNAmbient B0 B1 theta) where
  carrier :=
    cornerLiftAmbient e he_star he_mul pi u hcov hne ⁻¹'
  ...
  mul_mem' {y z} hy hz := by
    ...
    exact h ▸ ⟨mul_mem hy.1 hz.1, mul_mem hy.2 hz.2⟩
  add_mem' {y z} hy hz := by
    have h : cornerLiftAmbient e he_star he_mul pi u hcov hne (y + z) =
        cornerLiftAmbient e he_star he_mul pi u hcov hne y +
          cornerLiftAmbient e he_star he_mul pi u hcov hne z :=
      map_add (cornerLiftHom e he_star he_mul pi u hcov hne) y z
    exact h ▸ ⟨add_mem hy.1 hz.1, add_mem hy.2 hz.2⟩
  algebraMap_mem' r := by
    ...
    exact h ▸
      ⟨SMulMemClass.smul_mem r (projectionAmbient_mem e he_star he_mul).1,
        SMulMemClass.smul_mem r (projectionAmbient_mem e he_star he_mul).2⟩
  star_mem' {y} hy := by
    ...
    exact h ▸ ⟨star_mem hy.1, star_mem hy.2⟩
...
theorem cornerLiftAmbient_mem_of_mem
  ...
  refine closure_minimal (StarAlgebra.adjoin_le
    (S := cornerLiftPreimage e he_star he_mul pi u hcov hne) ?_)
    ((cornerAmbientSet_isClosed e he_star).preimage
      (map_continuous (cornerLiftHom e he_star he_mul pi u hcov hne))) hy
  rintro x (⟨d, rfl⟩ | hx)
  · exact (cornerLiftAmbient_base e he_star he_mul pi u hcov hne d) ▸
      subtype_mem_cornerAmbientSet e he_star he_mul (pi d)
  · rw [Set.mem_singleton_iff] at hx
    subst hx
    exact (cornerLiftAmbient_stable e he_star he_mul pi u hcov hne) ▸
      subtype_mem_cornerAmbientSet e he_star he_mul
        (u : CStarCorner e he_star he_mul)

/-- The corner lift of the universal C-star HNN algebra. -/
def cornerLift : UniversalCStarHNN B0 B1 theta →⋆ₐ[ℂ]
    CStarCorner e he_star he_mul where
  toFun x :=
    ⟨(⟨cornerLiftAmbient e he_star he_mul pi u hcov hne
      ...
      Subtype.ext (cornerLiftAmbient_mem_of_mem e he_star he_mul pi u hcov hne
        x.property).2.1,
      Subtype.ext (cornerLiftAmbient_mem_of_mem e he_star he_mul pi u hcov hne
        x.property).2.2⟩
  ...
  map_add' x y := Subtype.ext (Subtype.ext
    (map_add (cornerLiftHom e he_star he_mul pi u hcov hne)
      (x : UniversalCStarHNNAmbient B0 B1 theta)
      (y : UniversalCStarHNNAmbient B0 B1 theta)))
  commutes' r := Subtype.ext (Subtype.ext (by
    calc cornerLiftAmbient e he_star he_mul pi u hcov hne
          ((algebraMap ℂ (UniversalCStarHNN B0 B1 theta) r :
...
@[simp] theorem cornerLift_base (d : D) :
...
theorem cornerUnit_coordinateProjection_ne_zero
    (Q : CStarAmalgamRepresentation (amalgamLeftInclusion data)
      (amalgamRightInclusion data)) :
    coordinateProjection (cornerUnit data) Q ≠ 0 :=
...
def uedaCornerLift : universalHNN data →⋆ₐ[ℂ]
  ...
  cornerLift (cornerUnit data) (cornerUnit_star data) (cornerUnit_mul data)
    ...
    (cornerUnit_coordinateProjection_ne_zero data)
...
@[simp] theorem uedaCornerLift_base (d : baseAlgebra data) :
  ...
  cornerLift_base (cornerUnit data) (cornerUnit_star data)
    ...
    (cornerStableUnitary_covariance data)
    (cornerUnit_coordinateProjection_ne_zero data) d
