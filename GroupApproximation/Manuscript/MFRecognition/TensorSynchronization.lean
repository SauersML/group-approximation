...
/-! ## Shortcut instances for the spectral order

The `C*`-structure of `CStarMatrix (E n) (E n) A_1`, and that of the reduced
product built over it, is assembled from the spectral order of the coefficient
algebra.  Without a one-step instance for that order, resolving
`CStarAlgebra (CStarMatrix (E n) (E n) A_1)` sends the search through the
`CStarMatrix` and `lp`-quotient towers looking for `PartialOrder` and
`StarOrderedRing` on every algebra it meets, and the cost of that search — not
any mathematics — is what a raised heartbeat budget used to hide.

`CStarAlgebra.spectralOrder` and `CStarAlgebra.spectralOrderedRing` are the
direct hits, so they are installed here as shortcut instances covering every
`C*`-algebra in scope at once: the coefficient algebra, the coordinate matrix
algebras, and the corona.  This is the arrangement of the two sibling modules
of this lane, `TensorSynchronizationData` and `TensorSynchronizationConjugator`,
which elaborate the same carriers at the default budget. -/

noncomputable local instance tensorSynchronizationPartialOrder
    (A : Type*) [CStarAlgebra A] : PartialOrder A :=
  CStarAlgebra.spectralOrder A

local instance tensorSynchronizationStarOrderedRing
...
The spectral-order shortcut instances at the head of this module are what let
the many occurrences of the reduced product below resolve their `C*`-structure
in one step; the proof itself adds only the nonemptiness of the coordinate
groups, which `isMFAlgebra_cstarMatrix_fintype` consumes as an instance. -/
theorem exists_syncRealization (R1 : RegularRealizationData Gamma A1)
  ...
  have hWeq : Phi (Monoid.Coprod.inr (Multiplicative.ofAdd (1 : ℤ))) = W0 := by
    ...
    show W0 ^ (1 : ℤ) = W0
    exact zpow_one W0
  refine ⟨↥(groupGeneratedCStar Phi), groupGeneratedCStar_cStarAlgebra Phi,
