...
Only `wordNorming` is still a `sorry` here; the ω-debt is recorded once for
the repository in `Higman.OmegaDebt`, and the other inputs are proved:

* `wordNorming` — the one remaining input of Shulman, Theorem 16 (amalgamated
  free products of separable `C*`-algebras are MF under compatible corona
  embeddings), in its sharpest form.  The reduction chain
  `ShulmanFillTheorem16` → `ShulmanFillDenseNorming` →
  `ShulmanFillWordNorming` proves the printed criterion from it, so
...
its sharpest form: for every element `y` of the `*`-algebra generated by the
two factor images, and every `δ > 0`, some compatible representation with an MF
carrier evaluates `y` to within `δ` of its amalgam norm.  Everything else in
Theorem 16 is proved in `Analysis/ShulmanFill*` — separability of the amalgam,
the diagonal choice of a norming family, reduced-product permanence, and the
`ε/3` passage from a dense set to the whole amalgam. -/
theorem wordNorming : ShulmanFill.WordNormingStatement := by
  sorry

/-- **Shulman, Theorem 16**, from the word form of its remaining input. -/
theorem shulmanTheorem16 : HNNPermanence.ShulmanTheorem16Statement :=
  ShulmanFill.shulmanTheorem16_of_wordNorming wordNorming
