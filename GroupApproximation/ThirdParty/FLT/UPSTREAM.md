...
## The rest of the slice, packed

The generic-library slice that was assessed separately (double complexes and
the bounded Künneth theorem, direct limits, topological-group shims, matrix
groups, free groups, the argument principle, compact convolution operators,
Haar quotients, representation theory: 2,344 modules) is preserved packed at
`vendor/flt/flt-slice.tar.gz`, with a catalogue of every statement, so that a
lane can unpack exactly the closure it needs into this directory. See
`vendor/flt/UPSTREAM.md`. Nothing unpacked here is imported until it compiles
against the pin and the lead wires it.
