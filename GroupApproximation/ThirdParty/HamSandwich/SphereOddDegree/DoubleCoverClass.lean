...
real projective space, the eventual home of the canonical class `α`.  This is a
real object (`rpCohomology n 1`), not a placeholder. -/
...
`α ∈ H¹(RP n; F₂)` is obtained, via the (still absent) degree-one universal
coefficient theorem `H¹(X; F₂) ≅ Hom(π₁(X)ᵃᵇ, F₂)`.  It is **not** a fake `α`:
it is a fully-proved homomorphism out of `π₁(RP n, x)`, stopping exactly at the
last object constructible from current infrastructure. -/
