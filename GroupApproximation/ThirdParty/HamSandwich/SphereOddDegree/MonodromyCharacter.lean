...
This file advances the *covering → cohomology* bridge of *Route A* one honest
step further than `DoubleCoverClass.lean` / `MonodromyNontrivial.lean`, toward
...
monodromy of Sⁿ → RPⁿ
  ...
  → class      α ∈ H¹(RPⁿ; F₂)               (degree-one UCT, still absent in Mathlib)
```
...
theorem `H¹(X; F₂) ≅ Hom(H₁(X; ℤ), F₂)`.  Producing the genuine `α` from this
character is the *only* remaining step, and it needs that (still absent) UCT
classifier `Hom(π₁ X, ZMod 2) ≃ H¹(X; F₂)`.
