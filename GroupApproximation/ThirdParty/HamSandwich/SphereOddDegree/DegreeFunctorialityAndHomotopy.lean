...
into the strongest build-clean statements the project supports today.
...
from pinned Mathlib (it requires excision / Mayer–Vietoris / the long exact
sequence of a topological pair / a suspension isomorphism).  **No fake degree and
no fake top-homology isomorphism are introduced.**  Likewise, homotopy invariance
is conditional on the still-missing algebraic prism operator
