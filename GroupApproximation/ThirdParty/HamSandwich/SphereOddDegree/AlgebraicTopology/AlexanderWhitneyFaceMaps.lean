...
This file finishes the **build-clean** combinatorial / simplex-level setup needed
...
to be proved in the following prompts. It builds directly on the front/back face
maps `frontFace`, `backFace` of `AlexanderWhitney.lean`, and on the cochain cup
product `cochainCup` of `CupProduct.lean`. There are **no** placeholder
definitions, **no** axioms, and **no** fake declarations: every statement below is
fully proved.
...
5. **Simplex-level corollaries** `faceSimplex`, `frontSimplex_faceSimplex_of_gt`,
   ...
   singular simplices that prompt 05 plugs directly into the cochain computation.

## Sign convention

The combinatorial identities here are sign-free; the Koszul signs `(-1)^i` enter
only at the cochain coboundary level (prompt 05). Over `ZMod 2` all signs are
...
namespace GroupApproximation.ThirdParty.HamSandwich.SphereOddDegree.AlexanderWhitney
...
computation of prompt 05 uses directly. The two `k > p` cases (front
...
prompt 05 by applying `(TopCat.toSSet.obj X).map` to the morphism identities of
§3–§4. -/
...
end GroupApproximation.ThirdParty.HamSandwich.SphereOddDegree.AlexanderWhitney
