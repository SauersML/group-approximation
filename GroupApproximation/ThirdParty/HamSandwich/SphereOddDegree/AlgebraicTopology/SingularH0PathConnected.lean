...
theorem aug_comp_boundary_eq_zero (Y : TopCat.{0}) :
  ...
  change singularBoundary ℤ Y 0 ≫ aug Y = 0
  ext c
...
theorem H0aug_natural {Y Z : TopCat.{0}} (f : Y ⟶ Z) :
    HomologicalComplex.homologyMap (singularChainℤ.map f) 0 ≫ H0aug Z = H0aug Y := by
  have h_comm : (singularChainℤ.map f).f 0 ≫ aug Z = aug Y := by
    apply CategoryTheory.Limits.colimit.hom_ext
    intro j
    apply ModuleCat.hom_ext
    apply LinearMap.ext_ring
    change (aug Z).hom ((singularChainMap ℤ f 0).hom
      (chainGenerator ℤ Y 0 j.as)) = (aug Y).hom (chainGenerator ℤ Y 0 j.as)
...
theorem isIso_descOpcycles_aug (Y : TopCat.{0}) [Nonempty Y] [PathConnectedSpace Y] :
  ...
  have h_bijective : Function.Bijective (HomologicalComplex.descOpcycles (singularChainComplex ℤ Y) (aug Y) 1 prev_zero (by simp [aug_comp_boundary_eq_zero])).hom := by
    ...
    · intro n
      ...
      have hp := HomologicalComplex.p_descOpcycles (singularChainComplex ℤ Y) (aug Y)
        1 prev_zero (aug_comp_boundary_eq_zero Y)
      calc
        _ = (aug Y).hom (n • chainGenerator ℤ Y 0 (pointSimplex Y b)) :=
          congrArg (fun g => g.hom (n • chainGenerator ℤ Y 0 (pointSimplex Y b))) hp
        _ = n := by rw [map_zsmul, aug_generator, zsmul_one]; rfl
...
theorem surjective_H0aug (Y : TopCat.{0}) [Nonempty Y] :
  ...
  have h_surjective : Function.Surjective (⇑(ModuleCat.Hom.hom ((singularChainComplex ℤ Y).descOpcycles (aug Y) 1 prev_zero (aug_comp_boundary_eq_zero Y)))) := by
    ...
    calc
      ...
      _ = n := by rw [map_zsmul, aug_generator, zsmul_one]; rfl
  intro n
  obtain ⟨z, hz⟩ := h_surjective n
  obtain ⟨x, hx⟩ :=
    (ConcreteCategory.isIso_iff_bijective ((singularChainComplex ℤ Y).homologyι 0)).mp
      (by infer_instance) |>.2 z
  refine ⟨x, ?_⟩
  change (ModuleCat.Hom.hom
    ((singularChainComplex ℤ Y).descOpcycles (aug Y) 1 prev_zero
      (aug_comp_boundary_eq_zero Y)))
      ((ModuleCat.Hom.hom ((singularChainComplex ℤ Y).homologyι 0)) x) = n
  rw [hx, hz]
...
theorem isIso_subH0aug (X : TopCat.{0}) (S : Set X) [Nonempty S] [PathConnectedSpace S] :
  ...
  exact CategoryTheory.IsIso.comp_isIso' (by infer_instance) (isIso_H0aug (TopCat.of S))
...
theorem subChainCorestrict_inclusion_square (X : TopCat.{0}) (S T : Set X) (h : S ⊆ T) :
  ...
  change (singularChainMap ℤ (sInclusion S) n).hom c =
    ((singularChainMap ℤ (setInclusionTopCat X S T h) n ≫
      singularChainMap ℤ (sInclusion T) n).hom c)
...
theorem subH0aug_natural_inclusion (X : TopCat.{0}) (S T : Set X) (h : S ⊆ T) :
    HomologicalComplex.homologyMap (subChainInclusion S T h) 0 ≫ subH0aug X T
      = subH0aug X S := by
  have h_chain :
      subChainInclusion S T h ≫ inv (subChainCorestrict ℤ X T) =
        inv (subChainCorestrict ℤ X S) ≫ singularChainℤ.map (setInclusionTopCat X S T h) := by
    rw [← CategoryTheory.cancel_epi (subChainCorestrict ℤ X S),
      ← CategoryTheory.cancel_mono (subChainCorestrict ℤ X T)]
  ...
  change
    HomologicalComplex.homologyMap (subChainInclusion S T h) 0 ≫
          HomologicalComplex.homologyMap (inv (subChainCorestrict ℤ X T)) 0 ≫
        H0aug (TopCat.of T) =
      HomologicalComplex.homologyMap (inv (subChainCorestrict ℤ X S)) 0 ≫
        H0aug (TopCat.of S)
  have h_aug := H0aug_natural (setInclusionTopCat X S T h)
