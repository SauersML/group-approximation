...
No boundary-chain theorem is proved here; this is only the missing permutation
sign identity.
-/
...
theorem factor_lastFace_of_faceData {n : ℕ}
  ...
  apply Equiv.ext
  intro x
  change π x = insertLastPerm j (extendLastPerm ρ x)
  by_cases hx : x = lastVertex n
  · subst x
    rw [← hj]
    have hext : extendLastPerm ρ (lastVertex n) = lastVertex n := by
      unfold extendLastPerm lastVertex
      simp [Equiv.Perm.viaFintypeEmbedding, Equiv.Perm.extendDomain]
    rw [hext, insertLastPerm_last]
  · have hlast : x ≠ Fin.last (n + 1) := by
      simpa [lastVertex, Fin.last] using hx
    let t : Fin (n + 1) := x.castPred hlast
    have ht : Fin.castSucc t = x := Fin.castSucc_castPred x hlast
    have hext : extendLastPerm ρ x = Fin.castSucc (ρ t) := by
      rw [← ht]
      exact Equiv.Perm.viaFintypeEmbedding_apply_image ρ
        Fin.castSuccOrderEmb.toEmbedding t
    rw [hext, insertLastPerm_castSucc]
    exact ((hρ t).trans (congrArg π ht)).symm
