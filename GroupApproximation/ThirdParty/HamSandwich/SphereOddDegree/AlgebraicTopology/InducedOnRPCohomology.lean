...
future top-class / degree comparison will evaluate on the top class. -/
...
sphere pullback `spherePullback`; it is the cohomological hook the future
...
This is the genuine cohomology-level form of the final-theorem target
`fbar^*(α)=α ⟹ fbar^*(αⁿ)=αⁿ`, using the cohomology-level `ZMod 2` cup product
`cupZMod2` and its powers `cupPowZMod2` (`CohomologyCupProduct.lean`). The only
remaining input for the final theorem is the existence of the degree-1 class
`α ∈ H¹(RPⁿ; F₂)` with `fbar^* α = α` (the universal coefficient theorem); no fake
`α` is introduced. -/
