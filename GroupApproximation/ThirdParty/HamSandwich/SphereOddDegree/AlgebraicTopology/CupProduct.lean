...
theorem cochain_ext {R : Type} [CommRing R] {Z : TopCat.{0}} {p : ℕ}
  ...
  change cochainEval p φ τ = cochainEval p ψ τ
  exact h τ
...
theorem chainmap_generator {R : Type} [CommRing R] {X Y : TopCat.{0}} (f : X ⟶ Y)
  ...
  change (TopCat.toSSet.obj X).ιChainComplex τ ≫
      (SSet.chainComplexMap (TopCat.toSSet.map f) (ModuleCat.of R R)).f n =
    (TopCat.toSSet.obj Y).ιChainComplex ((TopCat.toSSet.map f).app _ τ)
  exact SSet.ι_chainComplexMap_f (TopCat.toSSet.obj X) (TopCat.toSSet.obj Y)
    (TopCat.toSSet.map f) (ModuleCat.of R R) τ
...
theorem cochainPullback_eval {R : Type} [CommRing R] {X Y : TopCat.{0}} (f : X ⟶ Y)
  ...
  all_goals simp [ModuleCat.hom_comp, LinearMap.comp_apply]
  all_goals rfl
...
/-!
## Remaining blocker: descent to cohomology

The operation `cochainCup` above is a genuine cochain-level cup product:
`R`-bilinear (`cochainCup_add_left/right`, `cochainCup_smul_left/right`), natural
in the space (`cochainCup_naturality`), right-unital (`cochainCup_one`), with
powers (`cochainPow`). To descend it to a cohomology-level product
`H^p(X; R) × H^q(X; R) → H^{p+q}(X; R)` one needs the **Leibniz / coboundary
identity**
...
equivalently that the assembled graded map `C^•(X) ⊗ C^•(X) → C^•(X)` is a
morphism of cochain complexes. With the coboundary `δ` given by the dual of the
alternating face boundary, `(δφ)(σ) = Σ_i (-1)^i φ(d_i σ)`, this is the standard
telescoping computation over the front/back simplicial identities
`frontFace_last_eq_backFace_zero`, `frontFace_succ`, and `backFace_succ_square`
(over `ZMod 2` the signs are trivial). It is **not** proved here; once it is, the
cohomology-level `cup` is `homologyMap` of that cochain-complex morphism, and the
naturality `f^*(a ⌣ b) = f^* a ⌣ f^* b` and powers `f^*(αⁿ) = (f^* α)ⁿ` descend
from `cochainCup_naturality` / `cochainPow_naturality`.
-/
