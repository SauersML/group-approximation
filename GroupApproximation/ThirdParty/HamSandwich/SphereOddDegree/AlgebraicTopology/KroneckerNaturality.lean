...
degree-`n` universal coefficient theorem over `F₂` for the project's own singular
...
theorem kroneckerMap_naturality (f : X ⟶ Y) (n : ℕ) :
  ...
  · convert kroneckerFunctional_apply Y n φ hφ ( ( HomologicalComplex.cyclesMap ( chainMapZMod2 f ) n ).hom c ) using 1;
    · rw [ ModuleCat.comp_apply, kroneckerMap_cocycleClass, homologyDualMap_hom_apply ];
      unfold homologyPushZMod2
      simpa only [LinearMap.comp_apply, ModuleCat.comp_apply] using congrArg
        (kroneckerFunctional Y n φ hφ).hom
        (ConcreteCategory.congr_hom
          (HomologicalComplex.homologyπ_naturality (chainMapZMod2 f) n) c)
    · rw [cochainPullback_eq_comp, ModuleCat.comp_apply]
      exact congrArg φ.hom
        (ConcreteCategory.congr_hom
          (HomologicalComplex.cyclesMap_i (chainMapZMod2 f) n) c).symm
...
theorem kroneckerMap_naturality_apply (f : X ⟶ Y) (n : ℕ) (a : cohomologyZMod2 Y n) :
  ...
  rw [← homologyDualMap_hom_apply]
  change (cohPullback f n ≫ kroneckerMap X n).hom a =
    (kroneckerMap Y n ≫ homologyDualMap f n).hom a
  exact happ
