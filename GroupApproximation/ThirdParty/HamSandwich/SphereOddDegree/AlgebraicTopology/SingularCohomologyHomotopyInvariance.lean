...
This file proves **homotopy invariance for the project's singular cohomology
...
* **Prism (topological → chain homotopy).** Turning a *topological* homotopy into
  ...
  operator, but the project's backported algebraic prism together with its
  ...
  `SingularCohomologyPrism R M` isolated below is now a **theorem**
...
No forbidden placeholder tokens are used: every declaration is fully proved.
-/
...
noncomputable def homotopyOpFunctorMap {ι : Type w} {V : Type u} [Category.{v} V]
    ...
    (H : Homotopy φ ψ) :
    Homotopy ((opFunctor V c).map φ.op) ((opFunctor V c).map ψ.op) :=
  H.op
...
theorem singularCohomologyMap_eq_of_chainHomotopy
  ...
  change HomologicalComplex.homologyMap _ n = HomologicalComplex.homologyMap _ n
...
`singularCohomologyPrism` below using the project's backported algebraic prism.
...
theorem singularCohomologyPrism (R : Type) [CommRing R] (M : ModuleCat.{0} R) :
  ...
  fun H => ⟨(show TopCat.Homotopy _ _ from H).singularChainComplexFunctorObjMap M⟩
...
theorem singularCohomologyMap_eq_of_homotopy
  ...
  singularCohomologyMap_eq_of_chainHomotopy R M n
    ((show TopCat.Homotopy _ _ from H).singularChainComplexFunctorObjMap M)
