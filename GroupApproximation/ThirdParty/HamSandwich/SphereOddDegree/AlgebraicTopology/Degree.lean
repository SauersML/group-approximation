...
This is the orientation-theoretic content behind the (currently unformalized)
statement `degree (antipodal n) = (-1)^(n+1)`: the antipodal map is the
restriction to the sphere of this ambient linear map, whose determinant carries
the sign that the topological degree must reproduce. The degree theorem itself is
not stated here (it needs `Hₙ(Sⁿ;ℤ) ≅ ℤ`, which is not yet available); this lemma
records only the build-clean ambient determinant fact. -/
...
theorem inducedOnTopHomology_comp_mul {n : ℕ} (f g : C(Sphere n, Sphere n)) :
  ...
  rw [inducedOnTopHomology_comp, CategoryTheory.End.mul_def]
...
`e : Hₙ(Sⁿ; ℤ) ≅ ℤ`.  This is the genuine degree once a canonical `e` is supplied;
it is *not* a fake unconditional `degree`, since `e` (equivalently the still-missing
`Hₙ(Sⁿ; ℤ) ≅ ℤ`) is an explicit hypothesis. -/
def degreeOfIso {n : ℕ}
...
degree layer. They are stated conditionally on the still-missing prism operator
`SingularPrismOperator` — there is no fake unconditional homotopy invariance.
