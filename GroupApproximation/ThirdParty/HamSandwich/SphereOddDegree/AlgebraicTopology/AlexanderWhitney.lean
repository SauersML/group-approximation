...
by
  simpa only [frontSimplex, ConcreteCategory.comp_apply] using
    ConcreteCategory.congr_hom ((TopCat.toSSet.map f).naturality (frontFace p q).op) σ
...
lemma backSimplex_naturality {X Y : TopCat.{0}} (f : X ⟶ Y) (p q : ℕ)
    ...
    (TopCat.toSSet.map f).app (Opposite.op (⦋q⦌ : SimplexCategory)) (backSimplex X p q σ)
      = backSimplex Y p q
          ((TopCat.toSSet.map f).app (Opposite.op (⦋p + q⦌ : SimplexCategory)) σ) :=
by
  simpa only [backSimplex, ConcreteCategory.comp_apply] using
    ConcreteCategory.congr_hom ((TopCat.toSSet.map f).naturality (backFace p q).op) σ
...
linearization itself is the remaining blocker (see the module footer). -/
