...
All declarations are build-clean; no placeholders or axioms.
-/
...
theorem iCycles_cyclesMk (X : TopCat.{0}) (n : ℕ)
    (φ : singularCochainGroup (ZMod 2) X n)
    (hφ : cochainCoboundary (ZMod 2) X n φ = 0) :
    ((cochainCxZMod2 X).iCycles n).hom
      ((cochainCxZMod2 X).cyclesMk φ (n + 1) (by simp [ComplexShape.next]) hφ) = φ := by
...
theorem cocycleClass_coboundary_zero (X : TopCat.{0}) (m : ℕ)
  ...
  have h : (cochainCxZMod2 X).cyclesMk (cochainCoboundary (ZMod 2) X m η) (m + 2)
      (by simp [ComplexShape.next]) hcoc = (cochainCxZMod2 X).toCycles m (m + 1) η := by
    apply (ModuleCat.mono_iff_injective ((cochainCxZMod2 X).iCycles (m + 1))).1 inferInstance
    change ((forget₂ (ModuleCat (ZMod 2)) Ab).map
      ((cochainCxZMod2 X).iCycles (m + 1)))
        ((cochainCxZMod2 X).cyclesMk (cochainCoboundary (ZMod 2) X m η) (m + 2)
          (by simp [ComplexShape.next]) hcoc) = _
    rw [(cochainCxZMod2 X).i_cyclesMk]
    change cochainCoboundary (ZMod 2) X m η = _
    exact (ConcreteCategory.congr_hom
      ((cochainCxZMod2 X).toCycles_i m (m + 1)).symm η)
  rw [cocycleClass, h]
  change ((cochainCxZMod2 X).toCycles m (m + 1) ≫
    (cochainCxZMod2 X).homologyπ (m + 1)).hom η = 0
...
theorem cupLeftMor_cyclesMk (X : TopCat.{0}) (p q : ℕ)
  ...
  apply congrArg ((cochainCxZMod2 X).homologyπ (p + q)).hom
  apply (ModuleCat.mono_iff_injective ((cochainCxZMod2 X).iCycles (p + q))).1 inferInstance
  calc
    _ = ((cochainCxZMod2 X).iCycles p ≫ cupRightMor X p q ψ).hom
          ((cochainCxZMod2 X).cyclesMk φ (p + 1) (by simp [ComplexShape.next]) hφ) :=
      ConcreteCategory.congr_hom
        ((cochainCxZMod2 X).liftCycles_i
          ((cochainCxZMod2 X).iCycles p ≫ cupRightMor X p q ψ)
          (p + q + 1) (by simp [ComplexShape.next]) (cupRight_cocycle_cond X p q ψ hψ)) _
    _ = cochainCup p q φ ψ := by
      rw [ModuleCat.comp_apply, cupRightMor_hom, iCycles_cyclesMk]
    _ = _ := ((cochainCxZMod2 X).i_cyclesMk _ _ _ _).symm
...
theorem cupRightMor'_cyclesMk (X : TopCat.{0}) (p q : ℕ)
  ...
  apply (ModuleCat.mono_iff_injective ((cochainCxZMod2 X).iCycles (p + q))).1 inferInstance
  calc
    _ = ((cochainCxZMod2 X).iCycles q ≫ cupLeftFixedMor X p q φ).hom
          ((cochainCxZMod2 X).cyclesMk ψ (q + 1) (by simp [ComplexShape.next]) hψ) :=
      ConcreteCategory.congr_hom
        ((cochainCxZMod2 X).liftCycles_i
          ((cochainCxZMod2 X).iCycles q ≫ cupLeftFixedMor X p q φ)
          (p + q + 1) (by simp [ComplexShape.next]) (cupLeftFixed_cocycle_cond X p q φ hφ)) _
    _ = cochainCup p q φ ψ := by
...
theorem cohPullback_cocycleClass {X Y : TopCat.{0}} (f : X ⟶ Y) (n : ℕ)
  ...
  apply (ModuleCat.mono_iff_injective ((cochainCxZMod2 X).iCycles n)).1 inferInstance
  calc
    _ = (((cochainCxZMod2 Y).iCycles n ≫
          ((singularCochainComplexFunctor (ZMod 2) (ModuleCat.of (ZMod 2) (ZMod 2))).map f.op).f n).hom)
          ((cochainCxZMod2 Y).cyclesMk φ (n + 1) (by simp [ComplexShape.next]) hφ) :=
      ConcreteCategory.congr_hom
        (HomologicalComplex.cyclesMap_i
          ((singularCochainComplexFunctor (ZMod 2) (ModuleCat.of (ZMod 2) (ZMod 2))).map f.op) n) _
