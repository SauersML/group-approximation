...
with the five genuinely missing inputs as explicit hypotheses (see the module
docstring). Concretely:
...
The internal step `fbar^*(αⁿ)=αⁿ ⇒ f^*(proj^* αⁿ)=proj^* αⁿ` is discharged by the
proved naturality lemma `spherePullback_fixes_projPullback`, and the final
`(... ) ≡ 1 (mod 2) ⇒ Odd (...)` by the proved parity bridge. Once the missing
inputs are supplied as genuine theorems, this specializes to the unconditional
final theorem with no change to the proof. -/
...
/-! ## 3c. The single remaining blocker, packaged as one named comparison
...
parity assumptions**. This is the exact form of the remaining blocker. -/

/-- **The mod-two top-class / degree comparison**, the single genuine topological
input still missing from pinned Mathlib. Relative to a chosen identification
...
This is precisely the `F₂`-coefficient transfer/reduction statement (the top-class
action determining `deg f mod 2`); it is the *only* remaining gap between the
proved cohomological core and the unconditional final odd-map theorem. -/
...
The comparison `ModTwoTopClassComparison e` is here reduced — fully build-clean,
using only the project's *proved* universal-coefficient machinery over `F₂`
...
as the scalar `(degree f mod 2)`. This isolates the *exact* remaining
topological/coefficient input the comparison still needs. -/
...
It is the single precise input still missing from pinned Mathlib for this
branch. -/
...
fully proved, build-clean reduction: it derives `ModTwoTopClassComparison e` from
`ModTwoTopHomologyScalar e` using only the project's proved `F₂` universal
...
theorem modTwoTopClassComparison_of_topHomologyScalar {n : ℕ}
  ...
  intro f c hfix hc
  set ψ := (kroneckerMap (TopCat.of (Sphere n)) n).hom c with hψ
  have hψne : ψ ≠ 0 := by
    intro hcontra
    apply hc
    apply kroneckerMap_injective (TopCat.of (Sphere n)) n
...
/-! ## 3d. Strongest honest near-final assembly: three named blockers, no vacuous hypothesis

The theorems in §3b/§3c route the sphere-side nonvanishing through `projPullback`
(`proj^*`). That is a *placeholder* avatar: for `n ≥ 1` one has `proj^* α = 0` in
...
homomorphism transports this to the nonzero sphere top class. It is genuinely
missing from pinned Mathlib (it rests on the `F₂` transfer/Gysin sequence of the
double cover `Sⁿ → RPⁿ`), and is kept here as one explicit named input. It is
...
/-- **The strongest honest near-final odd-map degree theorem.**
...
Every other ingredient is the project's *proved, unconditional* machinery: the
...
`m : MonodromyFunctional n` (the single remaining degree-one Hurewicz input), this
hypothesis is now **proved** (`inducedOnRPPullback_rpAlpha`) rather than assumed,
