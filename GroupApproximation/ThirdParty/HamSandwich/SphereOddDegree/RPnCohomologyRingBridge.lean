...
/-!
# Bridge from the actual `RPⁿ` mod-two cohomology to the algebraic model ring

The classical computation of the mod-two cohomology ring of real projective space is
...
* the genuine **algebraic model ring** `RPnCohomologyRingModel n = F₂[α]/(αⁿ⁺¹)`
  with generator `modelAlpha n` and the facts `αⁿ⁺¹ = 0`, `αⁿ ≠ 0`,
  `αᵏ = 0 ↔ n+1 ≤ k` (`RPnCohomologyRingModel.lean`);
* the genuine **actual singular cohomology** objects
  `cohomologyZMod2 (TopCat.of (RP n)) k = Hᵏ(RPⁿ; F₂)` with a genuine
  **cohomology-level cup product** `cupZMod2` and its cup powers `cupPowZMod2`
  (`CohomologyCupProduct.lean`, `InducedOnRPCohomology.lean`).

What was still genuinely **absent** from pinned Mathlib (and is *not* faked) is the
ring isomorphism itself: it needs the degree-one universal coefficient class `α`
together with the inductive (cellular / LES) computation
`Hⁿ(RPⁿ; F₂) ≅ F₂`. None of these are invented here.

This file makes the *honest reduction* requested by the prompt's completion
criterion. It packages the **model-side half of the ring isomorphism** — a graded
ring homomorphism from the actual mod-two cohomology of `RPⁿ` to the model ring,
carrying a chosen degree-one class to `modelAlpha n` — as a single, explicit,
named hypothesis `RPnCohomologyToModelHom n`, and derives from it the load-bearing
consequences **directly in the actual cohomology** `Hⁿ(RPⁿ; F₂)`:

* `rpAlpha_power_ne_zero` / `rpAlpha_power_top_ne_zero` — `αᵏ ≠ 0` for `k ≤ n`
  (in particular `αⁿ ≠ 0`) in the genuine `Hᵏ(RPⁿ; F₂)`;
* `rpAlpha_power_succ_eq_zero` — `αⁿ⁺¹ = 0` in `Hⁿ⁺¹(RPⁿ; F₂)` (needs the
  injective / equivalence strengthening `RPnCohomologyRingModelEmbedding n`);
* `rpAlpha_power_eq_zero_iff` — the full truncation `αᵏ = 0 ↔ n+1 ≤ k`.

The crucial new content over previous prompts is that the conclusions are stated
on the *actual* cohomology cup powers `cupPowZMod2`, not on an abstract ring: this
is the genuine connection of the model to singular cohomology. The hypothesis is a
genuine constraint (it forces `toFun 0 1 = 1 ≠ 0` and `toFun 1 α = modelAlpha n`),
so none of these theorems is vacuous; they are exactly conditional on the
(still-open) existence of the ring isomorphism.

**No fake `α`, no fake cohomology ring, and no fake ring isomorphism is asserted
unconditionally.** Everything is parameterized by the explicit named bridge
hypothesis.
-/
...
hypothesis" the prompt asks to reduce to: an injective graded ring map to the model
