...
the honest input for the vertex homomorphism `π₁(RP n, x) →* Equiv.Perm
...
resolution. -/
abbrev projMonodromyMulAction (n : ℕ) (x : RP n) :
...
theorem inducedOnRPFiberMap_projMonodromy (n : ℕ) (f : C(Sphere n, Sphere n))
  ...
  · -- By definition of `projMonodromy`, we know that
    ...
    rfl) := by
      ...
      · ext t
        change proj n (f ((projLiftPath n (γ_path) e.1 ‹_›) t)) =
          inducedOnRP f hf (γ_path t)
        rw [← inducedOnRP_comm f hf]
        congr 1
