...
/-!
# `SharpExistence` from `GHB(7)`, with the order bounds, property (T) and Tits' lemma spent
...
The last two theorems feed `CCKWTits.cckwCosetComplex_simplyConnected` and
`CCKWTits.cckwCosetComplex_connected` for `hsc`, `hconn`, leaving `hT6` and `hhyp`.
-/
...
/-- **CCKW Theorem 3.1(ii), cyclic case, for `GHB(7)`**, over the fixed-clique theorem only. -/
theorem cckwFiniteOrderConjugateIntoVertex_of_fixedClique
    (hT6 : CCKW.SystolicInvariantCliqueStatement) : CCKWFiniteOrderConjugateIntoVertex :=
  cckwFiniteOrderConjugateIntoVertex_of_fixedCliqueTits hT6
    CCKWTits.cckwCosetComplex_simplyConnected CCKWTits.cckwCosetComplex_connected

/-- **`SharpExistence` from `GHB(7)`**, over the fixed-clique theorem and hyperbolicity. -/
theorem sharpExistence_ghb7_of_fixedCliqueHyp (hT6 : CCKW.SystolicInvariantCliqueStatement)
    (hhyp : Hyperbolic.IsHyperbolicGroup (GHB 7)) : Hyperbolic.SharpExistence :=
  sharpExistence_ghb7_of_fixedCliqueTitsHyp hT6 CCKWTits.cckwCosetComplex_simplyConnected
    CCKWTits.cckwCosetComplex_connected hhyp
...
#audit_axioms GroupApproximation.KMSGroup.cckwFiniteOrderConjugateIntoVertex_of_fixedClique
#audit_axioms GroupApproximation.KMSGroup.sharpExistence_ghb7_of_fixedCliqueHyp
