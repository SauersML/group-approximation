...
The proofs follow the drafts of lane kt41-alt-route.
...
theorem negligible_threshold_mul_card {Y : ℕ → FiniteModel} (h : ℕ → ℝ) (hh : ∀ n, 0 ≤ h n)
    ...
    Negligible (fun n ↦ (Fintype.card (Y n) : ℝ)) fun n ↦
      c * (h n / 34 * Fintype.card (Y n)) :=
  (StepNineHammingFrame.negligible_mul_card_of_vanishing (Y := Y) (c := fun n ↦ h n / 34)
    ((hvanish.const_mul (1 / 34)).congr fun n ↦ by
      show 1 / 34 * h n = h n / 34
      ring)
    fun n ↦ div_nonneg (hh n) (by norm_num)).const_mul c
