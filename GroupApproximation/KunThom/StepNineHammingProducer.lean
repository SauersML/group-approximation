...
(arXiv:2608.06222v3).  The proofs follow the drafts of lane kt41-alt-route.
...
* the near bound `card_disagreement_sandwich_lt_of_transported`, at radius `6 · scale i`.

The budget totals are negligible by `negligible_sum_stepNineBudget`.  Both bridge totals
come from `M.bridge_negligible` and `M.unmatched_negligible`.  The conjugation totals come
from `ConjugationFailureVanishing`, and the compatibility totals from the frame.
-/
...
theorem vanishing_hammingDistance_patch_of_good {G : Type} [Group G]
    {A : SoficApproximation G} {K : Type} [Group K] {ι : K →* G}
    {R : RetainedComponents A K ι} (F : ClusterFrame R)
  ...
  · exact fun n i _ m _ ↦ StepNineHammingFrame.hasTaggedExpansionAtScale_frameAction F n i m
  · intro n i _
  ...
  · intro n i hi
    ...
    subst hXi
    exact card_disagreement_sandwich_lt_of_transported F M n (ε n) (h7 n) (hε n) (a n) (b n)
      hsel g hg₁ e hg₂
  · intro n i hi
    ...
    linarith
  · exact StepNineHammingFrame.negligible_card_compl_domain F hthr a
...
theorem vanishing_hammingDistance_patch_of_transported {G : Type} [Group G]
    {A : SoficApproximation G} {K : Type} [Group K] {ι : K →* G}
    {R : RetainedComponents A K ι} (F : ClusterFrame R)
    (hthr : Vanishing F.threshold) (t : G) (M : CompressorRelativeData t F)
    (ws : ↥R.data.generators → List ↥R.data.generators) (k : ℕ)
    (hk : ∀ s, (ws s).length ≤ k)
    (hws : ∀ s, ((ws s).map fun l : ↥R.data.generators ↦ ι (l : K)).prod =
      t * ι (s : K) * t⁻¹)
    (ε : ∀ n, ((F.system n).presentation.restrict (M.retained t n : Set (F.Obj n))).Obj →
      ((F.system n).presentation.restrict (M.retained t n : Set (F.Obj n))).Obj → ℕ)
    (h7 : ∀ (n : ℕ)
