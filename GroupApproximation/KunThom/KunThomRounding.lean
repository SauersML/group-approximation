...
theorem boundaryCard_pairGeneratorGraph_eq_relationBoundary
    ...
    (generatorGraph (pairModel (A.model n)) S (pairMap A n)).boundaryCard U =
      (relationBoundary (A.model n) (multiplicativeProductLabels A n S) U).card := by
  classical
  ...
  · intro p hp
    have hpdata := (mem_relationBoundary
      (A.model n) (multiplicativeProductLabels A n S) U p).1 hp
    rw [multiplicativeProductLabels, Finset.mem_image] at hpdata
...
theorem generatorCutSize_pair_product_lt
    ...
    (ha : IsEpsilonGood (A.model n) (multiplicativeProductLabels A n S) ε a)
    (hb : IsEpsilonGood (A.model n) (multiplicativeProductLabels A n S) ε b) :
    (generatorCutSize (pairModel (A.model n)) (pairMap A n) S
        (permutationGraph (A.model n) (a * b)) : ℝ) <
      4 * ε * Fintype.card (A.model n) := by
  let labels := multiplicativeProductLabels A n S
...
theorem pairProductDisplacementNormSq_eventually_lt
    ...
    ∃ N : ℕ, ∀ n ≥ N,
      ...
      ∀ {η : ℝ} {a b : Equiv.Perm (A.model n)},
        IsEpsilonGood (A.model n) (multiplicativeProductLabels A n S) η a →
        IsEpsilonGood (A.model n) (multiplicativeProductLabels A n S) η b →
      ...
      ∀ {a b : Equiv.Perm (A.model n)},
        ...
        ∃ U : Finset (A.model n × A.model n),
          (((U ∆ permutationGraph (A.model n) (a * b)).card : ℝ) <
            36 * k ^ 2 * (S.card : ℝ)⁻¹ * η *
              Fintype.card (A.model n)) ∧
          (((relationBoundary (A.model n) (multiplicativeProductLabels A n S) U).card : ℝ) <
            β * Fintype.card (A.model n)) := by
