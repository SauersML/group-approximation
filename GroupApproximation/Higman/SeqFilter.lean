...
/-- **A sequence supported at one index codes as a single power.** -/
theorem elt_of_support_subset_singleton {i : ℤ} {f : E} (h : ∀ j ∈ f.support, j = i) :
    elt f = FreeGroup.of i ^ f i := by
  rw [elt_eq_eltOn (L := [i]) (by simp) (by simp)
    (fun j hj => by rw [h j hj]; exact List.mem_cons.mpr (Or.inl rfl)),
    eltOn_cons, eltOn_nil, mul_one]

theorem elt_single (i n : ℤ) : elt (Finsupp.single i n) = FreeGroup.of i ^ n := by
  rw [elt_of_support_subset_singleton (i := i)
    (fun j hj => by simpa using Finsupp.support_single_subset hj),
