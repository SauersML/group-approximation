...
coding and exact decoder-kernel completion used by that transport.
...
The fixed periodic kernel and final assembly are split into
`TransportFivePeriodicKernel.lean` and `TransportFive.lean` so that each
module remains small enough for isolated compilation.
-/
...
theorem completedCodeSub_eq_comap (n : ℕ)
  ...
  · intro x hx
    ...
    have hkc : sectionHom n (decodeHom n x) * c⁻¹ ∈ MonoidHom.ker (decodeHom n) := by
      ...
      rw [map_mul, map_inv, decodeHom_sectionHom, hdecode, mul_inv_cancel]
...
theorem sectionHom_mem_completedCodeSub_iff (n : ℕ)
  ...
  change decodeHom n (sectionHom n w) ∈ N ↔ w ∈ N
  rw [decodeHom_sectionHom]
