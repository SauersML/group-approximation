...
where the second copy of `K` acts by left translation and the stable letter
swaps the two copies while fixing `a`.  `N ⋊ (K × K)` is `F₃ ⋊ K` for the
action `ψ_k : a ↦ a^k, b ↦ b, c ↦ c` (partial conjugation, an automorphism of
`F₃`), so `G` is three HNN extensions over `F₃`, each along an automorphism of
a finitely generated subgroup: finitely presented by
...
`HNNBritton.isPowerTorsionFree_hnn`.  Its finite presentation is
    ...
    ⟨a, b, c, β, γ, τ | [β, b], [β, c], [γ, b], [γ, c],
                        [a, bβ], [a, cγ],
                        τaτ⁻¹ = a, τbτ⁻¹ = β, τβτ⁻¹ = b,
                        τcτ⁻¹ = γ, τγτ⁻¹ = c ⟩

and conjugation by `τ` sends `a^w` to `a^{w⁻¹}` for every `w ∈ K`, which is
the field `flip_conj`.
...
structure FlipWitness where
  ...
  /-- The element that inverts the conjugators. -/
  flip : G
  /-- ... which does. -/
  flip_conj : ∀ h : F₃, h ∈ K →
    flip * emb (h⁻¹ * a * h) * flip⁻¹ = emb (h * a * h⁻¹)
...
theorem benignTF_of_conj {G : Type} [Group G] (emb : F₃ →* G)
  ...
  have h₂ : BenignTF ((H.map emb).map (MulAut.conj t).toMonoidHom) := by
    have hc := BenignTF.congr (MulAut.conj t).symm h₁
...
theorem benignTF_conjSubInv (w : FlipWitness) {T : Set F₃}
  ...
  refine benignTF_of_conj w.emb w.emb_injective w.flip w.transport ?_ h
