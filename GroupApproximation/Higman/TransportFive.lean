...
/-- The empty code is the empty product.  Proved here rather than taken from
`Higman.Seq.elt_zero`, which the sequence-space files move between modules. -/
...
the other one.  Since `1 ∈ N`, the statement is equivalent to
`ker (decodeHom n) ≤ codeSub n N` --- that the kernel of the decoding is
generated by its sorted elements.  The memo records the two families that
generate the kernel and the telescoping identity that puts the Schreier-type
family inside `codeSub`; what is missing is the induction on descents.
