...
`Higman.FinalReduction` as one structure, `OperationClosures`.  Its fields are
exactly the operations used by the formalized generated-set construction.
Since then
...
1. `Omega.OmegaInput`;
2. the base case `S`;
3. three benign-row facts --- the positive half-row, the normal closure of `b`
...
input are both available from these inputs plus Higman's Sections 2 and 5 and the
bridge, rather than from the seven-field structure.

## What this does *not* claim

The base case and rows are discharged downstream.  The remaining mathematical
input is `OmegaInput`.
-/
...
/-- **`OperationClosures` from the inputs used by the construction.**

`k` is the `ω` residue of `Higman.OmegaClosure`; `hbase` is Higman's base case
`S`; and `hhalf`, `hzetaRow`, `hevenRow` are the three
...
replaces that input by the leaves it is actually built from. -/
...
theorem statement_of_operationInputs (k : Omega.OmegaInput)
  ...
  statement_of_four
    (operationClosures_of_inputs k hbase hhalf hzetaRow hevenRow) h₂ h₃ h₄
