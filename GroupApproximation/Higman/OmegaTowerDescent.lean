...
/-- **Inner classification in the exact form supplied by an outer pinch.**
An entry of `slimWBase` that also belongs to either signed associated subgroup
is an embedded `F₃` word, and its unique conjugate-basis spelling is supported
on the local subgroup generated by the selected window blocks. -/
theorem exists_local_cbWord_of_mem_slimWBase_toSubgroup
    {m : ℕ} {B : Set E} {u : ℤˣ} {g : SlimPi m}
    (hgW : g ∈ slimWBase m B)
    (hgA : g ∈ toSubgroup
      (slimPiF3Hom m).range (slimPiF3Hom m).range u) :
    ∃ (x : F₃) (w : FreeGroup ↥Conj.K),
      slimPiF3Hom m x = g ∧ w ∈ localCoordSub m B ∧
        Conj.cbHom w = x := by
  have hgrange : g ∈ (slimPiF3Hom m).range := by
    rcases Int.units_eq_one_or u with rfl | rfl
    · simpa [toSubgroup_one] using hgA
    · simpa [toSubgroup_neg_one] using hgA
  obtain ⟨x, hx⟩ := hgrange
  have hxW : slimPiF3Hom m x ∈ slimWBase m B := by
    rw [hx]
    exact hgW
  obtain ⟨w, hw, hwx⟩ :=
    exists_local_cbWord_of_slimPiF3Hom_mem_slimWBase hxW
  exact ⟨x, w, hx, hw, hwx⟩
...
letters match and the intervening row conjugator vanishes on the active
window.  Thus a malformed `g`-pair survives before the window test is even
needed. -/
theorem slimGen_inv_mul_aElt_mul_gen_mem_F3Range_iff
    {m : ℕ} {β γ l : E} (hβ : β ∈ blockSet m) (hγ : γ ∈ blockSet m) :
    ((slimPiOf m (slimGenCode m β))⁻¹ * slimPiF3Hom m (aElt l) *
  ...
  · rintro ⟨x, hx⟩
    have hmid : slimPiBaseRet m (slimPiF3Hom m (aElt l)) = 1 := by
      ...
      simp only [map_mul, map_inv, slimPiF3Hom_bElt, slimPiF3Hom_a,
        slimPiBaseRet_of, slimPiBaseRet_t]
      group
    have hgen (u : E) :
        genProjection3 m
            (slimPiBaseRet m (slimPiOf m (slimGenCode m u))) = aElt u := by
      rw [slimPiBaseRet_of]
      change genProjection3 m (genHom (rowOut m) (aElt u)) = aElt u
      rw [← MonoidHom.comp_apply, genProjection3_comp_genHom]
      rfl
    have hkill : genProjection3 m (slimPiBaseRet m (slimPiF3Hom m x)) = 1 := by
      change ((genProjection3 m).comp
        ((slimPiBaseRet m).comp (slimPiF3Hom m))) x = 1
      rw [genProjection3_slimPiBaseRet_comp_slimPiF3Hom]
      rfl
    have hproj : (aElt β)⁻¹ * aElt γ = 1 := by
      calc
        (aElt β)⁻¹ * aElt γ = genProjection3 m
            (slimPiBaseRet m
              ((slimPiOf m (slimGenCode m β))⁻¹ *
                slimPiF3Hom m (aElt l) * slimPiOf m (slimGenCode m γ))) := by
              simp only [map_mul, map_inv, hmid, hgen, mul_one]
        _ = genProjection3 m (slimPiBaseRet m (slimPiF3Hom m x)) := by rw [hx]
        _ = 1 := hkill
    have hβγ : β = γ := aElt_injective (inv_mul_eq_one.mp hproj)
    subst γ
    refine ⟨rfl, ?_⟩
    exact (slimGen_conj_aElt_mem_F3Range_iff hβ).mp ⟨x, hx⟩
  · rintro ⟨rfl, hout⟩
    exact (slimGen_conj_aElt_mem_F3Range_iff hβ).mpr hout
...
/-- **The first outer-scan dichotomy.**  If an original `F₃` element lies
in the concrete tower subgroup, then either it already lies in the stage-two
alphabet, or an outer spelling contains an actual Britton pinch.  In the
second case the pinched base entry still belongs to `slimWBase`.

Unlike `HNNDescent.descent`, this statement makes no stability assumption on
`slimWBase`; that subgroup is not globally preserved by the shift.  It is the
entry point for the custom Omega scan, where the matched-code seam theorem
classifies the extracted pinch. -/
theorem slimF3_mem_W_base_or_outer_pinch {m : ℕ} {B : Set E} {x : F₃}
    (hx : slimOmegaEmb m x ∈ W (slimTower m) B) :
    slimPiF3Hom m x ∈ slimWBase m B ∨
      ∃ (z₀ : SlimPi m) (l l₁ : List (ℤˣ × SlimPi m))
        (u u' : ℤˣ) (g g' : SlimPi m) (l₂ : List (ℤˣ × SlimPi m)),
        z₀ ∈ slimWBase m B ∧
          (∀ p ∈ l, p.2 ∈ slimWBase m B) ∧
          slimOmegaEmb m x =
            HNNDescent.gword (slimShiftRangeEquiv m) z₀ l ∧
          l = l₁ ++ (u, g) :: (u', g') :: l₂ ∧
          g ∈ toSubgroup
            (slimPiF3Hom m).range (slimPiF3Hom m).range u ∧
          g ∈ slimWBase m B ∧ u' = -u := by
  obtain ⟨z₀, l, hz₀, hl, heq⟩ := exists_outer_gword_of_mem_slimW hx
  by_cases hnil : l = []
  · left
    subst l
    have hbase : slimPiF3Hom m x = z₀ := by
      apply HNNExtension.of_injective (slimShiftRangeEquiv m)
      simpa [slimOmegaEmb, slimOmegaOf] using heq
    rwa [hbase]
  · right
    have hmem : HNNDescent.gword (slimShiftRangeEquiv m) z₀ l ∈
        (HNNExtension.of : SlimPi m →* SlimOmega m).range := by
      refine ⟨slimPiF3Hom m x, ?_⟩
      simpa [slimOmegaEmb, slimOmegaOf] using heq
    obtain ⟨l₁, u, u', g, g', l₂, hlist, hg, hu⟩ :=
      HNNDescent.exists_pinch_of_gword_mem_range
        (slimShiftRangeEquiv m) hnil hmem
    have hgW : g ∈ slimWBase m B := hl (u, g) (by rw [hlist]; simp)
    exact ⟨z₀, l, l₁, u, u', g, g', l₂, hz₀, hl, heq,
      hlist, hg, hgW, hu⟩
