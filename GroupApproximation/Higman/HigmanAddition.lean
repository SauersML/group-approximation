...
theorem mem_addStep_window (g : E) (r : ℤ) :
  ...
  have hidle :
    ...
    simp only [addIdle, addIdled, addY, addZ, addC, addW, pinAt,
      Set.mem_inter_iff, Set.mem_setOf_eq, h1, h2, h3, h4, and_assoc]
...
theorem addEntry_far (x y z q d : ℤ) (hq : (y.natAbs : ℤ) < q) (hd : d ≠ 0) :
    addEntry x y z q d = 0 := by
  have hq' : |y| < q := by simpa using hq
  simp [addEntry, hq', hd]
...
theorem addFun_mem_Ico (x y z i : ℤ) (h : addFun x y z i ≠ 0) :
  ...
  rw [Finset.mem_Ico]
  rcases lt_or_ge i 0 with hlt | hge
  · exact absurd (addFun_neg x y z i hlt) h
  · refine ⟨hge, ?_⟩
    rcases lt_or_ge i (8 * ((y.natAbs : ℤ) + 4)) with hlt2 | hge2
    · exact hlt2
...
theorem addWitness_mem_transition (x y z : ℤ) (hxyz : z = x + y) :
  ...
  · rcases lt_or_ge ((y.natAbs : ℤ)) r with hfar | hnear
    · refine Or.inr (Or.inr ⟨?_, ?_, ?_, ?_⟩)
