...
This file isolates the first presentation-theoretic step toward a premise-free
instance of `LiteralAffineFreeProductAvatarBlueprint.blueprint`.
...
This module deliberately does not assert `AvatarRouterInstance.Inputs`.
Padding discharges the positive-presentation and relator-floor work.  The
remaining mathematical field is the cyclic piece bound `Inputs.piece_short`;
the mechanical bridge from the unique fresh occurrence below to
`Blueprint.OccurrenceMarks` is also exposed explicitly by
`RemainingRouterChecks` so it cannot be hidden in a later constructor.
-/
...
further presentation bookkeeping: this is the concrete
`AvatarRouterInstance.Inputs` constructor. -/
