...
/-!
# Primitive-recursive operations for coded matrix microstates

This file proves that the Gaussian-rational matrix arithmetic, word evaluation,
and vector norms used by `EffectiveMatrixCode` are primitive recursive.  The
Boolean predicates are kept in a separate module so changes to them do not
re-elaborate this arithmetic layer.
-/
