...
noncomputable def commElt (q : ℕ × ℕ) : (finalStage S P.tsub).Carrier :=
  (HNNExtension.t)⁻¹ * HNNExtension.of (S.ι (tw ((q.1 : ℤ), (q.2 : ℤ)))) *
    HNNExtension.t * (HNNExtension.of (S.ι (tw ((q.1 : ℤ), (q.2 : ℤ)))))⁻¹
...
theorem wordOf_rawComm (q : ℕ × ℕ) :
  ...
  rw [rawComm, wordOf_cons_neg, wordOf_append, wordOf_cons_pos, wordOf_invWord,
    wordOf_rawTw, letterOf_kIdx]
  simp only [map_mul, map_inv, FreeGroup.map.of]
  group
  rfl
    ...
    (finalPres P).equiv (commElt P q)
      = PresentedGroup.mk (finalPres P).rels
          ((FreeGroup.of (kLetter P))⁻¹ *
            ...
            FreeGroup.of (kLetter P) *
            ((finalPres P).word (twWord ((q.1 : ℤ), (q.2 : ℤ))))⁻¹) := by
  have ht : (finalPres P).equiv
      (HNNExtension.t : HNNExtension S.Carrier P.tsub P.tsub (MulEquiv.refl _))
      = PresentedGroup.of (kLetter P) := finalEquivPres_t P
  have htw : (finalPres P).equiv (HNNExtension.of (S.ι (tw ((q.1 : ℤ), (q.2 : ℤ)))))
      = PresentedGroup.mk (finalPres P).rels
          ((finalPres P).word (twWord ((q.1 : ℤ), (q.2 : ℤ)))) :=
    mk_word_tw (finalPres P) ((q.1 : ℤ), (q.2 : ℤ))
  rw [commElt]
  simp only [map_mul, map_inv, ht, htw]
  rfl
