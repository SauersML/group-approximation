...
`simple_kazhdan_sofic_group.tex` at origin/main 37551fd93, tex l.81–85: "... since the ring is
simple and any two of its elements $p,q$ satisfy $pr+qs=0$ with $(r,s)\ne0$, by a dimension count
...
Disjointness of the translates of `U`, which "small" also asks for in the section "Simplicity", is
not used by the count.
-/
...
theorem coeff_mul_unit_zpow_mul_towerElem {U : Set X} (hU : IsClopen U) {x₀ : X}
  ...
  rw [towerElem, towerElem, coeff_mul_unit_zpow_mul_coeff_mul_unit_zpow, scal_apply, ← mul_assoc,
    ← map_mul]
  congr 2
  ext y
...
theorem mul_sum_scal_mul_towerElem {U : Set X} (hU : IsClopen U) {x₀ : X} (S F N : Finset ℤ)
  ...
  have hcent : ∀ (c : k) (x : ClopenCrossedProduct T k), x * scal T k c = scal T k c * x :=
    fun c x => Semigroup.mem_center_iff.mp (coeff_const_mem_center T k c) x
  rw [Finset.mul_sum]
...
theorem scal_mul_towerElem {U : Set X} (hU : IsClopen U) (c : k) (m : ℤ) :
  ...
  rw [scal_apply, towerElem, ← mul_assoc, ← map_mul, coeff_mul_unit_zpow_eq_single]
...
#audit_axioms GroupApproximation.SimpleKazhdanSofic.mul_towerElem_eq_sum
#audit_axioms GroupApproximation.SimpleKazhdanSofic.exists_nontrivial_relation
#audit_axioms GroupApproximation.SimpleKazhdanSofic.exists_pair_dependence
#audit_closed_axioms GroupApproximation.SimpleKazhdanSofic.printedPairDependenceClopenCrossedProduct
