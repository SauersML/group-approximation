...
/-! Batched kernel checks 18--26 for residual block (4, 3). -/

namespace Residual43Part2

/-- Expected chunk totals; these numerals are data, not trusted equalities. -/
def expected : Fin 9 → Nat := ![213100539, 150305663, 218147760, 198376259, 370077035, 237577665, 230123682, 279649787, 124130421]

/-- One independently kernel-checked chunk equality. -/
def check (u : Fin 9) : Prop :=
  initialChunk 4 3 (finProdFinEquiv ((2 : Fin 4), u)) = expected u

/-- The identity-containing chunk is split into eight coefficient checks. -/
def chunk25Expected : Fin 8 → Nat := ![47028486, 45246938, 94816097, 4250347, 34399662, 43224767, 5165451, 5518039]

def chunk25Check (u : Fin 8) : Prop :=
  (residualNumerator 4 3
...
theorem chunk25 : initialChunk 4 3 25 = 279649787 := by
  unfold initialChunk
  calc
    ∑ u : Fin 8,
        (residualNumerator 4 3
...
theorem residual_part_sum_4_3_2 : initialPart 4 3 2 = 2021488811 := by
  have hpart : initialPart 4 3 2 =
      ∑ u : Fin 9, Residual43Part2.expected u := by
    unfold initialPart
    apply Finset.sum_congr rfl
    intro u _
    exact Residual43Part2.all u
  rw [hpart]
  decide +kernel
