...
This file fixes the exact hypotheses consumed by the analytic kill theorem of
the non-MF construction.  A `MarkedCompressionData Γ E` records a Kazhdan
...
marked word is nontrivial; the analytic theorem kills the word in every
operator-norm matrix model from the fields below alone, and receives
`w ≠ 1` as a separate hypothesis.
-/
...
/-- The data of the marked Kazhdan-compression obstruction inside a group
`E`.  The propositional fields are exactly the relations used by the
finite-stage analytic argument. -/
