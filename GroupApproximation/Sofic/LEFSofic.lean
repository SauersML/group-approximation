...
/-- **The two-quotient trick**: if two quotient-like maps have jointly
trivial kernel and residually finite targets, the group is residually
finite.  This packages the separation scheme in which one quotient sees
everything except a small normal subgroup and a second map detects that
subgroup. -/
theorem residuallyFinite_of_two_quotients
    {N : Subgroup G} [N.Normal] [Group.ResiduallyFinite (G ⧸ N)]
    {Q : Type*} [Group Q] [Group.ResiduallyFinite Q]
    (q : G →* Q) (h : N ⊓ q.ker = ⊥) :
    Group.ResiduallyFinite G := by
  have hker : ((QuotientGroup.mk' N).prod q).ker = ⊥ := by
    rw [MonoidHom.ker_prod, QuotientGroup.ker_mk', h]
  exact residuallyFinite_of_injective ((QuotientGroup.mk' N).prod q)
    (((QuotientGroup.mk' N).prod q).ker_eq_bot_iff.mp hker)
...
/-- **Detected central involutions do not obstruct residual finiteness**:
if a central element of order dividing two generates the kernel of a
quotient to a residually finite group, while some homomorphism to a
residually finite group keeps it alive, the ambient group is residually
finite.  The Deligne phenomenon — finite-by-residually-finite groups
that are not residually finite — is avoided exactly by the detecting
homomorphism. -/
theorem residuallyFinite_of_central_involution_detected
    {z : G} (hz2 : z ^ 2 = 1)
    [(Subgroup.zpowers z).Normal]
    [Group.ResiduallyFinite (G ⧸ Subgroup.zpowers z)]
    {Q : Type*} [Group Q] [Group.ResiduallyFinite Q]
    (q : G →* Q) (hq : q z ≠ 1) :
    Group.ResiduallyFinite G := by
  apply residuallyFinite_of_two_quotients (N := Subgroup.zpowers z) q
  rw [Subgroup.eq_bot_iff_forall]
  intro x hx
  obtain ⟨hxz, hxk⟩ := Subgroup.mem_inf.mp hx
  obtain ⟨k, rfl⟩ := Subgroup.mem_zpowers_iff.mp hxz
  have h2 : (z : G) ^ (2 : ℤ) = 1 := by
    rw [show ((2 : ℤ)) = ((2 : ℕ) : ℤ) by norm_num, zpow_natCast, hz2]
  ...
  · exfalso
    ...
    rw [hzeq] at hxk
    exact hq (MonoidHom.mem_ker.mp hxk)
