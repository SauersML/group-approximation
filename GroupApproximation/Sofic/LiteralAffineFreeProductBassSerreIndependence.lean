...
/-! ## A two-generated geometrically suitable subgroup -/

/-- The finite prescribed subgroup generated by the two explicit independent
loxodromics.  This is the form consumed by the finite-target-word clause in
Hull's routing construction. -/
def crossingSuitableCarrier : Subgroup Ambient :=
  Subgroup.closure ({crossingDefect, separatedCrossing} : Set Ambient)

theorem crossingSuitableCarrier_le_defect :
    crossingSuitableCarrier ≤ amplifiedDefectData.core.defectNormal := by
  apply (Subgroup.closure_le _).mpr
  intro g hg
  simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hg
  rcases hg with rfl | rfl
  · exact crossingDefect_mem
  · exact separatedCrossing_mem

theorem crossingSuitableCarrier_actsNonElementarily :
    HullGeometry.ActsNonElementarily crossingSuitableCarrier
      (BassSerreFreeProduct.baseLeft Envelope :
        BassSerreHullGeometry.PathVertex Envelope) := by
  refine ⟨crossingDefect, Subgroup.subset_closure (Set.mem_insert _ _),
    separatedCrossing, Subgroup.subset_closure
      (Set.mem_insert_of_mem _ (Set.mem_singleton_iff.mpr rfl)),
    crossingDefect_isLoxodromic, separatedCrossing_isLoxodromic,
    crossing_separated_independent⟩

/-- The explicit two-generated carrier satisfies Hull's full suitability
definition for the same Bass--Serre action. -/
theorem crossingSuitableCarrier_isSuitable :
  ...
  hyperbolic := bassSerre_hyperbolic
  acylindrical := bassSerre_acylindrical
  nonElementary := crossingSuitableCarrier_actsNonElementarily
  normalizesNoNontrivialFinite :=
    HullSuitable.normalizesNoNontrivialFinite_of_torsionFree
      ambient_isPowerTorsionFree crossingSuitableCarrier

/-- The same object packaged in the finite two-generator interface used by
the prescribed-saturation algebra.  Its algebraic suitability is derived
from, rather than substituted for, the full geometric theorem above. -/
def crossingSuitableDefectSubgroup :
    HullSuitable.SuitableDefectSubgroup
      amplifiedDefectData.core.defectNormal where
  carrier := crossingSuitableCarrier
  le_defect := crossingSuitableCarrier_le_defect
  generatorOne := crossingDefect
  generatorTwo := separatedCrossing
  generates := rfl
  suitable := crossingSuitableCarrier_isSuitable.toIsAlgebraicallySuitable
