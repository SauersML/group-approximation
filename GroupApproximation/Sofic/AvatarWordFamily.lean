...
/-! ### Invertible positive relator-pair coding -/

/-- The elementary positive Nielsen code
`(x, y) ↦ (x yⁿ, x yⁿ⁺¹)` preserves the generated normal subgroup.
Indeed the quotient of the two coded entries recovers `y`, after which the
first entry recovers `x`.  Unlike adjoining a short defining relator, this
operation allows both entries to grow with `n`. -/
theorem normalClosure_pair_rightCode (G : Type*) [Group G]
    (x y : G) (n : ℕ) :
    Subgroup.normalClosure ({x * y ^ n, x * y ^ (n + 1)} : Set G) =
      Subgroup.normalClosure ({x, y} : Set G) := by
  apply le_antisymm
  · refine Subgroup.normalClosure_le_normal ?_
    intro z hz
    simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hz
    rcases hz with rfl | rfl
    · exact mul_mem (Subgroup.subset_normalClosure (by simp))
        (pow_mem (Subgroup.subset_normalClosure (by simp)) n)
    · exact mul_mem (Subgroup.subset_normalClosure (by simp))
        (pow_mem (Subgroup.subset_normalClosure (by simp)) (n + 1))
  · refine Subgroup.normalClosure_le_normal ?_
    have ha : x * y ^ n ∈
        Subgroup.normalClosure ({x * y ^ n, x * y ^ (n + 1)} : Set G) :=
    ...
    have hb : x * y ^ (n + 1) ∈
      ...
      Subgroup.subset_normalClosure (by simp)
    have hy : y ∈
        Subgroup.normalClosure ({x * y ^ n, x * y ^ (n + 1)} : Set G) := by
      have h := mul_mem (inv_mem ha) hb
      have heq : (x * y ^ n)⁻¹ * (x * y ^ (n + 1)) = y := by
        rw [pow_succ]
        group
      rw [heq] at h
      exact h
    have hx : x ∈
        Subgroup.normalClosure ({x * y ^ n, x * y ^ (n + 1)} : Set G) := by
      have h := mul_mem ha (inv_mem (pow_mem hy n))
      have heq : (x * y ^ n) * (y ^ n)⁻¹ = x := by group
      rw [heq] at h
      exact h
    intro z hz
    simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hz
...
/-- The word-level entry `u vⁿ` of the positive Nielsen code. -/
def rightRelatorCode {α : Type} (u v : List (α × Bool)) (n : ℕ) :
    List (α × Bool) :=
  u ++ (List.replicate n v).flatten

/-- A coded entry spells `mk(u) * mk(v)^n`. -/
theorem mk_rightRelatorCode {α : Type} (u v : List (α × Bool)) (n : ℕ) :
    FreeGroup.mk (rightRelatorCode u v n) = FreeGroup.mk u * FreeGroup.mk v ^ n := by
  rw [rightRelatorCode, ← FreeGroup.mul_mk,
    PeriodicOverlap.mk_flatten_replicate]

/-- Exact length of one coded entry. -/
theorem length_rightRelatorCode {α : Type} (u v : List (α × Bool)) (n : ℕ) :
    (rightRelatorCode u v n).length = u.length + n * v.length := by
  rw [rightRelatorCode, List.length_append,
    PeriodicOverlap.length_flatten_replicate]

/-- A nonempty anchor makes the coding parameter a literal length floor. -/
theorem le_length_rightRelatorCode {α : Type} (u : List (α × Bool))
    {v : List (α × Bool)} (hv : v ≠ []) (n : ℕ) :
    n ≤ (rightRelatorCode u v n).length := by
  rw [length_rightRelatorCode]
  have hvlen : 1 ≤ v.length := List.length_pos_iff.mpr hv
  have hn : n ≤ n * v.length := by
    simpa using Nat.mul_le_mul_left n hvlen
  omega

/-- Positivity is preserved by the relator-pair code. -/
theorem rightRelatorCode_positive {α : Type} {u v : List (α × Bool)}
    (hu : ∀ c ∈ u, c.2 = true) (hv : ∀ c ∈ v, c.2 = true) (n : ℕ) :
    ∀ c ∈ rightRelatorCode u v n, c.2 = true := by
  intro c hc
...
/-- At word-presentation level, replacing `(u,v)` by the two consecutive
coded entries preserves the presented normal subgroup exactly. -/
theorem wordSubgroup_pair_rightRelatorCode {α : Type}
    (u v : List (α × Bool)) (n : ℕ) :
    wordSubgroup
        ({rightRelatorCode u v n, rightRelatorCode u v (n + 1)} :
          Set (List (α × Bool))) =
      wordSubgroup ({u, v} : Set (List (α × Bool))) := by
  simp only [wordSubgroup, Set.image_insert_eq, Set.image_singleton,
    mk_rightRelatorCode]
  exact normalClosure_pair_rightCode (FreeGroup α) (FreeGroup.mk u) (FreeGroup.mk v) n
...
/-! ## 2'''.  Private positive null-pair codes

The presentation-level marker construction needs a finite family of padding
words before it can prove that the padding is a Tietze transformation.  The
combinatorial part is independent: give every relator and every padding slot
its own barred pair.  Distinct codewords then have disjoint alphabets, hence no
nonempty common cyclic prefix, while the slot parameter makes them as long as
needed. -/

section PrivateNullPairCode

/-- A private positive-presentation letter records its owning relator, its
padding slot, and which side of the formal null pair it is. -/
structure PrivateNullLetter (I : Type) (slots : ℕ) where
  owner : I
  slot : Fin slots
  barred : Bool

/-- The positive pair `x x̄` belonging to one private slot. -/
def privateNullPair {I : Type} {slots : ℕ} (i : I) (j : Fin slots) :
    List (PrivateNullLetter I slots × Bool) :=
  [(⟨i, j, false⟩, true), (⟨i, j, true⟩, true)]

/-- The private codeword owned by `i`, with one positive null pair per slot. -/
def privateNullPairCode {I : Type} (slots : ℕ) (i : I) :
    List (PrivateNullLetter I slots × Bool) :=
  (List.finRange slots).flatMap (privateNullPair i)

/-- Private null-pair codewords have exactly twice the requested slot length. -/
theorem length_privateNullPairCode {I : Type} (slots : ℕ) (i : I) :
    (privateNullPairCode slots i).length = 2 * slots := by
  simp [privateNullPairCode, privateNullPair, Nat.mul_comm]

/-- Every letter of a private null-pair codeword is positive. -/
theorem privateNullPairCode_positive {I : Type} (slots : ℕ) (i : I) :
    ∀ c ∈ privateNullPairCode slots i, c.2 = true := by
  intro c hc
  simp only [privateNullPairCode, List.mem_flatMap] at hc
  obtain ⟨j, -, hc⟩ := hc
  simp [privateNullPair] at hc
  rcases hc with rfl | rfl <;> rfl

/-- Membership recovers the owner of a private codeword. -/
theorem owner_eq_of_mem_privateNullPairCode {I : Type} {slots : ℕ}
    {i : I} {c : PrivateNullLetter I slots × Bool}
    (hc : c ∈ privateNullPairCode slots i) : c.1.owner = i := by
  simp only [privateNullPairCode, List.mem_flatMap] at hc
  obtain ⟨j, -, hc⟩ := hc
  simp [privateNullPair] at hc
  rcases hc with rfl | rfl <;> rfl

/-- Distinct private codewords have no nonempty common prefix, even after
independent cyclic rotations.  Thus their common cyclic overlap is exactly
zero at the combinatorial-code level. -/
theorem privateNullPairCode_common_cyclic_prefix_eq_nil {I : Type} {slots : ℕ}
    {i j : I} (hij : i ≠ j) {a b : ℕ}
    {p : List (PrivateNullLetter I slots × Bool)}
    (hi : p <+: (privateNullPairCode slots i).rotate a)
    (hj : p <+: (privateNullPairCode slots j).rotate b) : p = [] := by
  by_contra hp
  obtain ⟨c, t, rfl⟩ := List.exists_cons_of_ne_nil hp
  obtain ⟨si, hsi⟩ := hi
  obtain ⟨sj, hsj⟩ := hj
  have hciRot : c ∈ (privateNullPairCode slots i).rotate a := by
    rw [← hsi]
    exact List.mem_append_left _ (List.mem_cons_self)
  have hcjRot : c ∈ (privateNullPairCode slots j).rotate b := by
    rw [← hsj]
    exact List.mem_append_left _ (List.mem_cons_self)
  have hci : c ∈ privateNullPairCode slots i := by simpa using hciRot
  have hcj : c ∈ privateNullPairCode slots j := by simpa using hcjRot
  apply hij
  exact (owner_eq_of_mem_privateNullPairCode hci).symm.trans
    (owner_eq_of_mem_privateNullPairCode hcj)

end PrivateNullPairCode
