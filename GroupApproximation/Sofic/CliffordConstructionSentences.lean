...
Two printed clauses have no counterpart and are not built here (see the module docstring's final
section for exactly why): the homomorphism `Cl(X) → SignedModel X` is proved onto, but never
proved *injective* --- no normal-form theorem for `PresentedGroup (relators X)` exists in the
repository --- so neither "the expression is unique and the homomorphism is an isomorphism" nor
the exact order `2^{|Y|+1}` (which is a direct consequence of that injectivity) has a carrier.
Local finiteness and countability of `Cl(X)` themselves *are* proved, but by an unrelated route
(`Sofic/CentralInvolutionFinite.lean`'s generic central-involution argument), so that half of the
sentence is still given its own lemma below.
-/
...
/-- **Printed sentence** (the "countable and locally finite" half only; see the module docstring
for the still-open "order `2^{|Y|+1}`" half).

> ... so `Cl(X)` is countable and locally finite.

Proved, but by an unrelated route: `Countable (CliffordLamp X)` is the presented group's generic
countable-generators instance, and `IsLocallyFiniteGroup (CliffordLamp X)` is
...
theorem manuscriptSentence_cliffordLampCountableLocallyFinite {X : Type} [Countable X] :
    Countable (CliffordLamp X) ∧ IsLocallyFiniteGroup (CliffordLamp X) :=
  ⟨inferInstance, isLocallyFiniteGroup_cliffordLamp X⟩
