...
One printed clause has no counterpart and is not built here: the exact order `2^{|Y|+1}`, for a
finite `Y ⊆ X`, of the subgroup generated by `ε` and the `c_y` with `y ∈ Y`.
`Sofic/CliffordLampNormalForm.lean` now proves `toModel X` bijective --- the normal-form theorem
that makes this order computation possible --- but the finite-subgroup-order corollary itself is
not yet drawn from it. Local finiteness and countability of `Cl(X)` themselves *are* proved, but
by an unrelated route (`Sofic/CentralInvolutionFinite.lean`'s generic central-involution
argument), so that half of the sentence is still given its own lemma below.
-/
