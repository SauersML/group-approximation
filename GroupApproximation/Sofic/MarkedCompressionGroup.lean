...
include hα in
theorem verticalLift_inl_level (g : Γ →* H) (c : H)
...
def lampAction : Vertical α hα →* MulAut (LampGroup (Cosets α hα)) :=
...
abbrev Ambient : Type u :=
  LampGroup (Cosets α hα) ⋊[lampAction α hα] Vertical α hα
...
theorem signAmbient_ne_one : signAmbient α hα ≠ 1 := by
  ...
  have h1 : sign (Cosets α hα) = 1 := by
    apply (inl_injective (N := LampGroup (Cosets α hα))
      (G := Vertical α hα))
