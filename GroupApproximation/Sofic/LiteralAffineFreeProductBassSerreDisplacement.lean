...
/-! # Displacement bounds for the explicit Bass--Serre defect -/
...
theorem crossingDefect_displacement_le_four :
  ...
  have hdiFix : markedDefect⁻¹ • o = o :=
    (BassSerreFreeProduct.smul_baseLeft_eq_iff Envelope markedDefect⁻¹).mpr
      ((BassSerreFreeProduct.leftFactor Envelope).inv_mem
        markedDefect_mem_leftFactor)
  ...
  rw [show (rightShift * markedDefect) *
      (rightShift⁻¹ * markedDefect⁻¹) = crossingDefect by
        simp only [crossingDefect, mul_assoc]] at hsub
  calc
