...
Five statements in the `GGT/HullSCConeOffHeavy*` files used to assume it as a
binder.  All five have been discharged and their redundant universe-zero
wrappers retired.  Consumers now use the corresponding `_of_fourPoint` forms,
which are stated at `Type u`.
