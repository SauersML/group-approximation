...
theorem exists_places_over {π : L} (hπ : w π = exp (-1 : ℤ)) :
  ...
  haveI hfin : Finite (HeightOneSpectrum C) := by
    haveI : Finite ((IsLocalRing.maximalIdeal O).primesOver C) :=
      (IsDedekindDomain.primesOver_finite (IsLocalRing.maximalIdeal O) C).to_subtype
    refine Finite.of_injective (fun P : HeightOneSpectrum C ↦
      (⟨P.asIdeal, P.isPrime, ⟨(hunder P).symm⟩⟩ : (IsLocalRing.maximalIdeal O).primesOver C)) ?_
    intro P Q h
  ...
  · have hπO : π ∈ O := by
    ...
    have hmax : (⟨π, hπO⟩ : O) ∈ IsLocalRing.maximalIdeal O := by
      ...
      norm_num
    have hmem : algebraMap O C ⟨π, hπO⟩ ∈ (e.symm j).asIdeal := by
      rw [← hunder (e.symm j)] at hmax
      exact hmax
    have h := ((e.symm j).valuation_lt_one_iff_mem (K := K) (algebraMap O C ⟨π, hπO⟩)).mpr hmem
    exact h
