...
The reverse Frobenius estimate gives a dual certificate.  It is deliberately
strict: the MF normal form supplies separation one, so rational perturbation
...
theorem opNorm_gt_of_certificate (hn : Nonempty n) (A : Matrix n n ℂ) {q : ℝ}
  ...
  have hcard : (0 : ℝ) < (Fintype.card n : ℝ) ^ 2 := by
    ...
    positivity
  have hf := froSq_le B
  have hnormB : ‖B‖ ^ 2 = ‖A‖ ^ 2 ^ (m + 2) := by
    dsimp [B]
    rw [norm_conjTranspose_mul_pow]
    rw [← pow_mul, ← pow_succ]
  rw [hnormB] at hf
  have hp : q ^ 2 ^ (m + 2) < ‖A‖ ^ 2 ^ (m + 2) := by
    exact lt_of_mul_lt_mul_left (h.trans_le hf) hcard.le
...
theorem exists_certificate_of_lt_opNorm (hn : Nonempty n) (A : Matrix n n ℂ)
  ...
  have hDpos : 0 < D := by
    rw [hD]
    exact_mod_cast (Fintype.card_pos : 0 < Fintype.card n)
  set C : ℝ := D ^ 6 with hC
  have hCnonneg : 0 ≤ C := by rw [hC]; positivity
  set r : ℝ := q / ‖A‖ with hr
  have hAnorm : 0 < ‖A‖ := lt_of_le_of_lt hq h
  have hr0 : 0 ≤ r := by rw [hr]; positivity
  have hr1 : r < 1 := by rw [hr]; exact (div_lt_one hAnorm).2 h
  obtain ⟨N, hN⟩ :=
    exists_pow_lt_of_lt_one (show (0 : ℝ) < 1 / (C + 1) by positivity) hr1
  have hNle : N ≤ 2 ^ (N + 2) := by
    exact (self_le_two_pow N).trans
      (Nat.pow_le_pow_right (by norm_num) (by omega))
  have hpow : r ^ 2 ^ (N + 2) < 1 / (C + 1) :=
    lt_of_le_of_lt (pow_antitone_of_le_one hr0 hr1.le N (2 ^ (N + 2)) hNle) hN
  have hfrac : C * r ^ 2 ^ (N + 2) < 1 := by
    rcases eq_or_lt_of_le hCnonneg with hzero | hpos
    · rw [← hzero, zero_mul]; norm_num
    · refine (mul_lt_mul_of_pos_left hpow hpos).trans ?_
      rw [mul_one_div, div_lt_one (by linarith)]
      linarith
  have hq : q = r * ‖A‖ := by rw [hr]; field_simp
  have hmain : C * q ^ 2 ^ (N + 2) < ‖A‖ ^ 2 ^ (N + 2) := by
    rw [hq, mul_pow]
    calc
      C * (r ^ 2 ^ (N + 2) * ‖A‖ ^ 2 ^ (N + 2))
          = (C * r ^ 2 ^ (N + 2)) * ‖A‖ ^ 2 ^ (N + 2) := by ring
      _ < 1 * ‖A‖ ^ 2 ^ (N + 2) :=
        mul_lt_mul_of_pos_right hfrac (by positivity)
      _ = ‖A‖ ^ 2 ^ (N + 2) := one_mul _
  have hop := opNorm_sq_le_froSq ((Aᴴ * A) ^ 2 ^ N)
  rw [norm_conjTranspose_mul_pow] at hop
  have hexp : (‖A‖ ^ 2 ^ (N + 1)) ^ 2 = ‖A‖ ^ 2 ^ (N + 2) := by
    rw [← pow_mul, ← pow_succ]
  rw [hexp] at hop
  refine ⟨N, ?_⟩
  have hD4 : 0 < D ^ 4 := by positivity
  apply lt_of_mul_lt_mul_left (a := D ^ 4) (a0 := hD4.le)
  calc D ^ 4 * (D ^ 2 * q ^ 2 ^ (N + 2))
      = C * q ^ 2 ^ (N + 2) := by rw [hC]; ring
    _ < ‖A‖ ^ 2 ^ (N + 2) := hmain
    _ ≤ D ^ 4 * froSq ((Aᴴ * A) ^ 2 ^ N) := by simpa [hD] using hop
