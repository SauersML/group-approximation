...
* `mul_eq_one_symm_matrixCorona` — in `Matrix I I (NormMatrixCStarCorona X)`,
  `x y = 1` implies `y x = 1`;
* `cstarMatrix_mul_star_eq_one_of_hasMFEmbedding` — for an MF algebra `A`,
  every isometry of `CStarMatrix I I A` is a unitary, i.e. `A` is stably
  finite, which is the printed sentence of `p:D-preamble` in full;
* `mfAlgebra_isStablyFinite` — the same statement with the manuscript's
  `k ≥ 1` indexing.

The transfer from `M_I(Q_X)` to `M_I(A)` is the `k = 1` argument verbatim: the
embedding `e` is not assumed unital, so `E 1` is a projection of `M_I(Q_X)` and
the argument runs in its corner, through `mul_star_eq_of_corner_isometry`.
Nothing about the amplification is special there.

The empty index type is handled separately and trivially: `Matrix I I A` is a
subsingleton when `I` is empty, so both statements hold for want of anything to
say.
-/
...
@[simp] theorem block_assemble (a : I → I → BoundedMatrixSequence (fun n ↦ X n))
    (i j : I) (n : ℕ) : block i j (assemble X a n) = (a i j) n := rfl
...
theorem mul_eq_one_symm_matrixCorona [Nonempty I]
    ...
    y * x = 1 := by
  classical
  -- lifts, entry by entry
  set lift : NormMatrixCStarCorona (fun n ↦ X n) →
      BoundedMatrixSequence (fun n ↦ X n) :=
    Function.surjInv (normMatrixCStarCoronaMk_surjective (fun n ↦ X n))
    with hliftdef
  have hlift : ∀ z, normMatrixCStarCoronaMk (fun n ↦ X n) (lift z) = z := fun z ↦
    Function.surjInv_eq (normMatrixCStarCoronaMk_surjective (fun n ↦ X n)) z
  set a : I → I → BoundedMatrixSequence (fun n ↦ X n) :=
    fun i j ↦ lift (x i j) with hadef
  set b : I → I → BoundedMatrixSequence (fun n ↦ X n) :=
    fun i j ↦ lift (y i j) with hbdef
  -- the amplified coordinate sequences
  set A : ∀ n, Matrix (I × X n) (I × X n) ℂ := assemble X a with hAdef
  set B : ∀ n, Matrix (I × X n) (I × X n) ℂ := assemble X b with hBdef
  -- the blockwise defect of `A B - 1` is null, hence so is the whole
  have hblock : ∀ i j : I, Tendsto
      (fun n ↦ ‖block i j (A n * B n - 1)‖) cofinite (nhds 0) := by
    intro i j
    have hprod : ∀ n, block i j (A n * B n - 1)
        = ((∑ k : I, a i k * b k j) -
            (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n))
              else 0)) n := by
      intro n
      have h1 : block i j (A n * B n) = (∑ k : I, a i k * b k j) n := by
        rw [hAdef, hBdef, assemble_mul, block_assemble]
      have h2 : block i j (1 : Matrix (I × X n) (I × X n) ℂ)
          = (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n))
              else 0) n := by
        rw [← assemble_one (I := I) X n, block_assemble]
      have h3 : block i j (A n * B n - 1)
          = block i j (A n * B n) - block i j (1 : Matrix (I × X n) (I × X n) ℂ) := by
        ext p q
        simp [block]
      rw [h3, h1, h2]
      cases hij : (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n))
          else 0) with
      | _ => rfl
    have hzero : normMatrixCStarCoronaMk (fun n ↦ X n)
        ...
        = 0 := by
      rw [map_sub]
      have hL : normMatrixCStarCoronaMk (fun n ↦ X n) (∑ k : I, a i k * b k j)
          = (x * y) i j := by
        rw [map_sum, Matrix.mul_apply]
        exact Finset.sum_congr rfl fun k _ ↦ by
          rw [map_mul, hadef, hbdef, hlift, hlift]
      have hR : normMatrixCStarCoronaMk (fun n ↦ X n)
          (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n)) else 0)
    ...
    · filter_upwards [hhalf] with n hn
      refine le_trans
        (UltraproductDedekindFinite.norm_swap_sub_one_le
          (Y := I × X n) inferInstance (A n) (B n) hn) ?_
      have h1 : ‖A n‖ * ‖B n‖ ≤ (∑ i : I, ∑ j : I, ‖a i j‖) *
          (∑ i : I, ∑ j : I, ‖b i j‖) :=
        mul_le_mul (hMa n) (hMb n) (norm_nonneg _) hMa0
      have h2 : (0 : ℝ) ≤ ‖A n * B n - 1‖ := norm_nonneg _
  ...
  have hnull : Tendsto (fun n ↦
    ...
    have hprod : ((∑ k : I, b i k * a k j) -
        (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n)) else 0)) n
        = block i j (B n * A n - 1) := by
      have h1 : block i j (B n * A n) = (∑ k : I, b i k * a k j) n := by
        rw [hAdef, hBdef, assemble_mul, block_assemble]
      have h2 : block i j (1 : Matrix (I × X n) (I × X n) ℂ)
          = (if i = j then (1 : BoundedMatrixSequence (fun n ↦ X n))
              else 0) n := by
        rw [← assemble_one (I := I) X n, block_assemble]
      have h3 : block i j (B n * A n - 1)
          = block i j (B n * A n) - block i j (1 : Matrix (I × X n) (I × X n) ℂ) := by
        ext p q
        simp [block]
      rw [h3, h1, h2]
      rfl
    rw [hprod]
    exact norm_block_le i j (B n * A n - 1)
  ...
  rw [map_sub, sub_eq_zero] at hmk
  have hL : normMatrixCStarCoronaMk (fun n ↦ X n) (∑ k : I, b i k * a k j)
      = (y * x) i j := by
    rw [map_sum, Matrix.mul_apply]
    exact Finset.sum_congr rfl fun k _ ↦ by
      rw [map_mul, hadef, hbdef, hlift, hlift]
