...
noncomputable instance normMatrixCoronaAlgebraStarRing :
    StarRing (NormMatrixCoronaAlgebra X) where
  star_add x y := by
    induction x using QuotientAddGroup.induction_on with
    | _ a =>
...
quotient norm. -/
noncomputable instance normMatrixCoronaAlgebraNormedAlgebra :
    NormedAlgebra ℂ (NormMatrixCoronaAlgebra X) :=
  ⟨fun z x ↦ (norm_smul z x).le⟩

/-- Coordinatewise adjoint on the quotient is conjugate-linear. -/
noncomputable instance normMatrixCoronaAlgebraStarModule :
    StarModule ℂ (NormMatrixCoronaAlgebra X) := by
  constructor
  intro z x
  induction x using QuotientAddGroup.induction_on with
  | _ a =>
      change star (Ideal.Quotient.mk (c0MatrixSequenceIdeal X) (z • a)) =
        star z • star (Ideal.Quotient.mk (c0MatrixSequenceIdeal X) a)
      rw [normMatrixCorona_star_mk, normMatrixCorona_star_mk, star_smul]
      rfl
...
/-! ## An opaque C-star-corona interface

`NormMatrixCoronaAlgebra` above remains reducible while its quotient norm and
operations are constructed.  Exposing that raw abbreviation directly to
...
def normMatrixCStarCoronaQuotient :
  ...
  commutes' z := by
    ...
    exact Ideal.Quotient.mk_algebraMap ℂ (c0MatrixSequenceIdeal X) z
  map_star' a := by
    unfold normMatrixCStarCoronaMk NormMatrixCStarCorona
    exact (normMatrixCorona_star_mk X a).symm
...
@[simp] theorem normMatrixCStarCorona_star_mk (a : BoundedMatrixSequence X) :
  ...
  unfold normMatrixCStarCoronaMk NormMatrixCStarCorona
  exact normMatrixCorona_star_mk X a

theorem normMatrixCStarCoronaMk_eq_zero_iff (a : BoundedMatrixSequence X) :
    normMatrixCStarCoronaMk X a = 0 ↔ IsC0MatrixSequence X a := by
  unfold normMatrixCStarCoronaMk NormMatrixCStarCorona
  exact normMatrixCoronaMk_eq_zero_iff X a
