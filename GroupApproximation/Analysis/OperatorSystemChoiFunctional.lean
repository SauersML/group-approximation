...
/-!
# The Choi functional of a completely positive map on an operator system

The Choi step of Arveson's extension theorem (Ozawa, *About the QWEP
conjecture*, Theorem 2.2), for a map `ψ : E → B(ℂᵏ)` on an operator system
`E`.  Matrices over `E` form an operator system of `Mₖ(A)`, and the functional

  `s(b) = ∑ p q, ⟪e_p, ψ (b p q) e_q⟫`

on it is nonnegative on ambient positives exactly because `ψ` is completely
positive on `E`.

* `matrixSystem E k`: matrices with entries in `E`.  It contains `1` and is
  closed under the star when `E` is an operator system.
* `systemChoiFunctional_nonneg`: factor an ambient positive `b = star N * N`.
  Complete positivity on `E` gives `[ψ (b p q)] = star P * P`, and the value is
  `∑ r ‖∑ q P r q e_q‖²`.
* `systemChoiFunctional_single`: on a single-entry matrix the functional
  recovers the matrix coefficients of `ψ`.

The same argument for closed star subalgebras is
`Analysis/LanceChoiFunctional.lean`.  There positivity comes from a factor
with entries in the subalgebra.  Here complete positivity on `E` is stated
against ambient positivity, so no factor with entries in `E` is needed.
-/
...
/-- **The Choi functional is nonnegative on ambient positives.**  Factor
`b = star N * N` over `A`.  Complete positivity of `ψ` on `E` factors the
...
theorem systemChoiFunctional_nonneg [PartialOrder A] [StarOrderedRing A]
  ...
  exact Finset.sum_congr rfl fun r _ ↦
    (inner_self_eq_norm_sq_to_K _).trans (Complex.ofReal_pow _ _).symm

/-- Evaluation of the Choi functional on a single-entry matrix recovers the
matrix coefficient of `ψ`. -/
