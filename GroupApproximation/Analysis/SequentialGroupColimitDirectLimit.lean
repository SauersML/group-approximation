...
@[simp] theorem towerIota_apply (n : ℕ) (x : G n) : T.towerIota n x = T.mkAt n x := rfl
...
instance directedSystem : DirectedSystem G T.towerMap where
  map_self {_} x := T.map_self le_rfl x
  map_map {_ _ _} hij hjk x := T.map_trans hij hjk x
...
theorem mk_eq_mk_iff {n m : ℕ} {x : G n} {y : G m} :
    T.mkAt n x = T.mkAt m y ↔ ∃ (k : ℕ) (hx : n ≤ k) (hy : m ≤ k), T.map hx x = T.map hy y :=
  ⟨fun h => Quotient.exact h, fun ⟨_k, hx, hy, e⟩ => T.mkAt_eq_mkAt hx hy e⟩
...
instance isDirectLimit : IsDirectLimit T.towerMap T.towerIota where
  ...
  compatibility n m h x := T.mkAt_map h x
