...
theorem leftRegularOperator_comm_rightRegularOperator (g h : G) :
  ...
  apply lp.ext
  funext x
  simp only [ContinuousLinearMap.mul_apply, leftRegularOperator_apply,
...
theorem reduced_commutes_right (T : ReducedGroupCStar G) (g : G) :
  ...
  have hgen : StarSubalgebra.adjoin ℂ (Set.range (leftRegularOperator G)) ≤ C := by
    apply StarSubalgebra.adjoin_le
    rintro L ⟨h, rfl⟩
  ...
  exact (hmem (rightRegularOperator G g) (Set.mem_singleton _)).1
...
theorem apply_single (T : ReducedGroupCStar G) (g : G) (c : ℂ) :
  ...
  rw [← lp.single_smul, ← rightRegular_deltaOne]
  rw [map_smul]
  congr 1
...
theorem hasSum_product_coefficient (T S : ReducedGroupCStar G) :
  ...
  convert hs using 1
  · ext g
    simp only [evalAfterT, ContinuousLinearMap.comp_apply, apply_single,
      rightRegularOperator_apply, Pi.smul_apply, smul_eq_mul, one_mul]
  · rfl
...
theorem canonicalCoefficientAtOne_star_mul_self (T : ReducedGroupCStar G) :
  ...
  rw [← lp.inner_single_left (𝕜 := ℂ) (1 : G) (1 : ℂ)]
  simp only [deltaOne, inner_one_left, ContinuousLinearMap.mul_apply,
