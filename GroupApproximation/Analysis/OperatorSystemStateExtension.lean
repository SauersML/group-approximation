...
/-!
# Positive extension of functionals from operator systems

The Krein extension step of Arveson's theorem (Ozawa, *About the QWEP
conjecture*, Theorem 2.2), for an **operator system**: a complex subspace `B`
of a unital C⋆-algebra that contains `1` and is closed under the star.  A
linear functional on `B` that is nonnegative on the positives of the ambient
algebra extends to a functional on the whole algebra with the same
positivity.

`Analysis/StateExtension.lean` proves this for unital star subalgebras.  Its
proof uses only three closure properties of the domain: it is a complex
subspace, it contains `r • 1` for real `r`, and it is closed under the star.
So the same M. Riesz argument runs here with the domain relaxed to a
`Submodule`.  The reassembly of a complex functional from its real part
(`StateExtension.complexify`) and the positive cone are reused unchanged.
-/
...
/-- An operator system contains every real scalar. -/
...
/-- The self-adjoint elements of the operator system, as the domain of the
...
/-- On self-adjoint elements of the operator system a positive functional
takes real values: shift by a real scalar to reduce to the positive case. -/
...
/-- The real-linear partial functional: real part of `φ` on the self-adjoint
part of the operator system. -/
...
elements, nonnegative on positives, agreeing with `Re φ` on the operator
system. -/
...
/-- **Positive extension from an operator system.**  A functional on a
complex subspace that contains `1` and is closed under the star, nonnegative
on ambient positives, extends to the whole algebra with the same
positivity. -/
