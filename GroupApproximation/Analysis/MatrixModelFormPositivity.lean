...
/-!
# Completely positive operator models are form-positive matrix models

`Quasidiagonal.AmenableTraceModel` asks its maps `A → M_Y` to be completely
positive in the form sense `Quasidiagonal.IsCompletelyPositiveOnMatrices`.
Arveson's extension theorem (`Analysis/OperatorSystemMatrixArveson.lean`)
produces maps into `B(ℓ²(Fin |Y|))`, completely positive in the factorization
sense `CStarExactness.IsCompletelyPositive`.  This file moves between the two.

* `matrixOfEuclidean Y`: the inverse of `Quasidiagonal.euclideanize Y`, i.e.
  the matrix of an operator on `ℓ²(Fin |Y|)`, reindexed to `Y`.
* `isCompletelyPositiveOnMatrices_matrixOfEuclidean`: if `Ψ` is completely
  positive then `matrixOfEuclidean Y ∘ Ψ` is form-positive.  Each form value is
  a form of `Ψ` at reindexed vectors, and
  `CStarExactness.IsCompletelyPositive.form_nonneg` makes it a real
  nonnegative number.
-/
...
/-- The matrix of an operator on `ℓ²(Fin |Y|)`, reindexed to `Y`: the inverse
of `Quasidiagonal.euclideanize Y`. -/
...
/-- A matrix form over `Y` is the operator form of its `euclideanize` at the
reindexed vectors. -/
...
/-- **Completely positive operator models are form-positive matrix
models.** -/
theorem isCompletelyPositiveOnMatrices_matrixOfEuclidean {A : Type u}
  ...
  · haveI : NeZero m := ⟨Nat.pos_iff_ne_zero.mp hm⟩
    set h : Fin m → EuclideanSpace ℂ (Fin (Fintype.card Y)) := fun i ↦
      WithLp.toLp 2 fun p : Fin (Fintype.card Y) ↦ w i ((enum Y).symm p) with hh
    obtain ⟨hre, him⟩ := hΨ.form_nonneg a h
    have hsum : (∑ i : Fin m, ∑ j : Fin m, ∑ x : Y, ∑ y : Y,
        (starRingEnd ℂ) (w i x) * matrixOfEuclidean Y (Ψ (star (a i) * a j)) x y
          * w j y)
        = ∑ i : Fin m, ∑ j : Fin m, ⟪h i, Ψ (star (a i) * a j) (h j)⟫_ℂ := by
      refine Finset.sum_congr rfl fun i _ ↦ Finset.sum_congr rfl fun j _ ↦ ?_
      rw [sum_form_eq_inner_euclideanize, euclideanize_matrixOfEuclidean]
    rw [hsum]
    exact ⟨him, hre⟩
