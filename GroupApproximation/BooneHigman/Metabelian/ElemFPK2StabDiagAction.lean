...
* `LastRootAction.T_add`, `T_commute`, `T_adjacent`: the relators then hold for *all* indices.  If no
  index is last, every root comes from `St_n(R)` along `castSucc`.  Rewrite with `compat`, pull
