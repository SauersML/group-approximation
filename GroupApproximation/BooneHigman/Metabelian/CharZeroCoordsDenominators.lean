...
theorem mvPolynomial_mem_range_of_den_dvd {σ : Type*} {m : ℕ} (hm : m ≠ 0)
    ...
    (hp : ∀ c ∈ p.coeffs, c.den ∣ m) : p ∈ Set.range (MvPolynomial.map (σ := σ) θ) :=
  (MvPolynomial.mem_range_map_iff_coeffs_subset (f := θ) (x := p)).2
...
theorem matrix_mem_range_mapMatrix {α β : Type*} [Semiring α] [Semiring β] {N : ℕ} (f : α →+* β)
    {M : Matrix (Fin N) (Fin N) β} (hM : ∀ i j, M i j ∈ Set.range f) :
    M ∈ Set.range (f.mapMatrix : Matrix (Fin N) (Fin N) α →+* Matrix (Fin N) (Fin N) β) := by
  choose M' hM' using hM
  exact ⟨M', Matrix.ext fun i j => by
