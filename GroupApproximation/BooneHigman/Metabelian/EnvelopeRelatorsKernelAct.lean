...
def rnKerWord (H : Subgroup (TreeAut X)) (v : ↥(higmanThompsonV X))
...
theorem rnKerWord_mk (H : Subgroup (TreeAut X)) (N : Subgroup (RNFree X H)) [N.Normal]
...
its recorded word `w`. -/
theorem rnKerWord_toPerm (H : Subgroup (TreeAut X)) (x₀ : X) (v : ↥(higmanThompsonV X))
    (L : List (↥(higmanThompsonV X) × List X × ↥H))
