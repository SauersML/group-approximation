...
theorem higmanVCTauDecomp_flexUS {d : ℕ} {α β γ g v0 w0 u0 : Fin d} (u v' : List (Fin d))
  ...
  refine ⟨[β, γ, u0], [α, w0], β :: g :: v', α :: w0 :: u, [β, γ, u0], [α, w0],
    α :: v0 :: v', α :: w0 :: u, hPQ.1, hPQ.2, by simp, by simp, ?_, ?_, ?_, ?_, ?_, by simp,
    by simp, ?_, ?_, ?_⟩
...
theorem higmanVCTauDecomp_flexES {d : ℕ} {α β γ g v0 w0 v1 : Fin d} (v'' : List (Fin d))
  ...
  unfold higmanVCTauComm_FlexA
  refine ⟨[β, g, v1], [α, w0], α :: w0 :: v'', [β, γ], [α, v0, v1], [α, w0],
    α :: w0 :: v'', [β, γ], hPQ.1, hPQ.2, by simp, by simp, ?_, ?_, ?_, ?_, ?_, by simp,
    by simp, ?_, ?_, ?_⟩
