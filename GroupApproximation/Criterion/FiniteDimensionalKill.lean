...
theorem map_marked_commutator_eq_one [FiniteDimensional k V]
  ...
  have hπconj : ∀ h : H, π (t * h * t⁻¹) = π t * π h * π t⁻¹ := by
    intro h
    rw [map_mul, map_mul]
  -- undoing the sandwich from the left
  ...
  have hinj : Function.Injective (sandwich (π t) (π t⁻¹)) := by
    ...
    have h1 : π t⁻¹ * (π t * x * π t⁻¹) * π t
      ...
      rw [← sandwich_apply (π t) (π t⁻¹) x, ← sandwich_apply (π t) (π t⁻¹) y,
        hxy]
    rwa [hres, hres] at h1
  -- commuting with the conjugated element transports through the sandwich
  have transport : ∀ g x : Module.End k V,
    ...
    have h2 : π t⁻¹ * (x * (π t * g * π t⁻¹)) * π t
      ...
      rw [hTinvT, mul_one]
    rw [← h1, ← h2, hgx]
  -- the commutant is carried into itself by conjugation ...
  ...
  -- ... hence onto itself: the equal-dimension pinning
  ...
  have heq : repCommutant π Γ
      = (repCommutant π Γ).map (sandwich (π t) (π t⁻¹)) :=
