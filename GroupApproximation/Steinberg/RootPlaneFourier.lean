...
/-!
# Fourier components of a finite free-Steinberg-root plane
...
namespace SteinbergRootPlaneFourier

open FreeAlgebraDegree SteinbergGroup
...
noncomputable def planeFamily
    (i j k : Fin 3) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k) (n : ℕ) :
    Fin (Nat.card (Plane X i j k hij hik hjk n)) →
      SteinbergGroup (Fin 3) (FreeRing X) :=
...
noncomputable def forwardConjugatedPlaneSucc
    ...
    (x : X) (n : ℕ) :
    Plane X i j k hij hik hjk n → Plane X i j k hij hik hjk (n + 1) :=
  fun g ↦ ⟨SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x) * g.1 *
      (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x))⁻¹,
...
noncomputable def oppositeConjugatedPlaneSucc
    ...
    (x : X) (n : ℕ) :
    Plane X i j k hij hik hjk n → Plane X i j k hij hik hjk (n + 1) :=
  fun g ↦ ⟨SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x) * g.1 *
      (SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x))⁻¹,
...
noncomputable def forwardUnitConjugatedPlane
  ...
  fun g ↦ ⟨SteinbergGroup.x i j hij 1 * g.1 * (SteinbergGroup.x i j hij 1)⁻¹,
...
noncomputable def oppositeUnitConjugatedPlane
    (i j k : Fin 3) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k)
    (n : ℕ) : Plane X i j k hij hik hjk n → Plane X i j k hij hik hjk n :=
  fun g ↦ ⟨SteinbergGroup.x j i hij.symm 1 * g.1 *
      (SteinbergGroup.x j i hij.symm 1)⁻¹,
...
theorem planeFamily_forwardUnitConjugatedPlaneIndex
    ...
    SteinbergGroup.x i j hij 1 * planeFamily X i j k hij hik hjk n q *
        (SteinbergGroup.x i j hij 1)⁻¹ =
...
theorem planeFamily_oppositeUnitConjugatedPlaneIndex
    (i j k : Fin 3) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k)
    (n : ℕ) (q : Fin (Nat.card (Plane X i j k hij hik hjk n))) :
    SteinbergGroup.x j i hij.symm 1 * planeFamily X i j k hij hik hjk n q *
        (SteinbergGroup.x j i hij.symm 1)⁻¹ =
      planeFamily X i j k hij hik hjk n
...
noncomputable def planeComponent
    (i j k : Fin 3) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k) (n : ℕ)
    (rho : SteinbergGroup (Fin 3) (FreeRing X) →* (E ≃ₗᵢ[ℝ] E))
...
theorem map_planeComponent_forward_eq_sum_conjugated_extensions
    ...
    (sign : Fin (Nat.card (Plane X i j k hij hik hjk n)) → Bool)
    (z : E) :
    rho (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x))
        (planeComponent X i j k hij hik hjk n rho sign z) =
      ∑ fineSign ∈ (Finset.univ.filter fun fineSign :
          Fin (Nat.card (Plane X i j k hij hik hjk (n + 1))) → Bool ↦
          sign = fun q ↦ fineSign
        ...
        planeComponent X i j k hij hik hjk (n + 1) rho fineSign
          (rho (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x)) z) := by
  rw [planeComponent, map_iteratedPart]
  exact iteratedPart_eq_sum_fine_extensions rho
    (Nat.card (Plane X i j k hij hik hjk (n + 1)))
    (Nat.card (Plane X i j k hij hik hjk n))
    (planeFamily X i j k hij hik hjk (n + 1))
    (fun q ↦ SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x) *
      planeFamily X i j k hij hik hjk n q *
      (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x))⁻¹)
...
theorem map_planeComponent_opposite_eq_sum_conjugated_extensions
    ...
    (sign : Fin (Nat.card (Plane X i j k hij hik hjk n)) → Bool)
    (z : E) :
    rho (SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x))
        (planeComponent X i j k hij hik hjk n rho sign z) =
      ∑ fineSign ∈ (Finset.univ.filter fun fineSign :
          Fin (Nat.card (Plane X i j k hij hik hjk (n + 1))) → Bool ↦
          sign = fun q ↦ fineSign
  ...
  exact iteratedPart_eq_sum_fine_extensions rho
    ...
    (Nat.card (Plane X i j k hij hik hjk n))
    (planeFamily X i j k hij hik hjk (n + 1))
    (fun q ↦ SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x) *
      planeFamily X i j k hij hik hjk n q *
      (SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x))⁻¹)
...
theorem sum_norm_oppositeConjugatedRestriction_sq
  ...
  have hsq : SteinbergGroup.x j i hij.symm
    ...
    have hadd : FreeAlgebra.ι (ZMod 2) x + FreeAlgebra.ι (ZMod 2) x =
      ...
      calc
        ...
        _ = 0 := by
          ...
          rw [htwo, zero_smul]
    rw [pow_two, SteinbergGroup.x_mul, hadd, SteinbergGroup.x_zero]
      ...
      ∑ coarseSign ∈ coarseSigns,
        ‖planeComponent X i j k hij hik hjk n rho coarseSign
          (rho (SteinbergGroup.x i j hij
            (FreeAlgebra.ι (ZMod 2) x)) z)‖ ^ 2 := by
  have hsq : SteinbergGroup.x i j hij
      (FreeAlgebra.ι (ZMod 2) x) ^ 2 = 1 := by
    have hadd : FreeAlgebra.ι (ZMod 2) x + FreeAlgebra.ι (ZMod 2) x =
        (0 : FreeRing X) := by
      calc
...
theorem sum_planeComponent
    (i j k : Fin 3) (hij : i ≠ j) (hik : i ≠ k) (hjk : j ≠ k) (n : ℕ)
    (rho : SteinbergGroup (Fin 3) (FreeRing X) →* (E ≃ₗᵢ[ℝ] E)) (z : E) :
...
theorem sum_norm_forwardUnitConjugatedRestriction_sq
    ...
    ∑ fineSign ∈ fineRestrictionSignSet
      ...
      ∑ coarseSign ∈ coarseSigns,
        ‖planeComponent X i j k hij hik hjk n rho coarseSign
          (rho (SteinbergGroup.x i j hij 1) z)‖ ^ 2 := by
  have hsq : SteinbergGroup.x i j hij (1 : FreeRing X) ^ 2 = 1 := by
    rw [pow_two, SteinbergGroup.x_mul, FreeAlgebraDegree.add_self_eq_zero,
      SteinbergGroup.x_zero]
  exact sum_norm_fineRestriction_conjugated_sq rho (SteinbergGroup.x i j hij 1)
    hsq
    (Nat.card (Plane X i j k hij hik hjk n))
    (Nat.card (Plane X i j k hij hik hjk n))
    (planeFamily X i j k hij hik hjk n)
...
theorem sum_norm_oppositeUnitConjugatedRestriction_sq
    ...
    ∑ fineSign ∈ fineRestrictionSignSet
      ...
      ∑ coarseSign ∈ coarseSigns,
        ‖planeComponent X i j k hij hik hjk n rho coarseSign
          (rho (SteinbergGroup.x j i hij.symm 1) z)‖ ^ 2 := by
  have hsq : SteinbergGroup.x j i hij.symm (1 : FreeRing X) ^ 2 = 1 := by
    rw [pow_two, SteinbergGroup.x_mul, FreeAlgebraDegree.add_self_eq_zero,
      SteinbergGroup.x_zero]
  exact sum_norm_fineRestriction_conjugated_sq rho (SteinbergGroup.x j i hij.symm 1)
    hsq
    (Nat.card (Plane X i j k hij hik hjk n))
    (Nat.card (Plane X i j k hij hik hjk n))
    (planeFamily X i j k hij hik hjk n)
...
theorem abs_generatorShearedSecondCharacterMass_sub_le
    ...
    |(∑ sign ∈ negativePlaneSignSet X i j k hij hik hjk (n + 1)
      ...
      (∑ sign ∈ negativePlaneSignSet X i j k hij hik hjk n
        ...
        ‖planeComponent X i j k hij hik hjk n rho sign z‖ ^ 2)| ≤
      2 * ‖z‖ *
        ‖z - rho (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x)) z‖ := by
  ...
  have htransport :=
    InvolutionSplitting.abs_norm_negativePart_conjugate_sq_sub_le rho
      (SteinbergGroup.x i j hij (FreeAlgebra.ι (ZMod 2) x)) hc z
...
theorem abs_generatorShearedFirstCharacterMass_sub_le
    ...
    |(∑ sign ∈ negativePlaneSignSet X i j k hij hik hjk (n + 1)
      ...
      (∑ sign ∈ negativePlaneSignSet X i j k hij hik hjk n
        ...
        ‖planeComponent X i j k hij hik hjk n rho sign z‖ ^ 2)| ≤
      2 * ‖z‖ *
        ‖z - rho (SteinbergGroup.x j i hij.symm
          (FreeAlgebra.ι (ZMod 2) x)) z‖ := by
  rw [← norm_negativePart_sq_eq_planeCharacterMass X i j k hij hik hjk
      (n + 1) rho z
      (generatorShearedFirstCoordinate X (ZMod 2) i j k hij hik hjk x n a),
  ...
  have htransport :=
    InvolutionSplitting.abs_norm_negativePart_conjugate_sq_sub_le rho
      (SteinbergGroup.x j i hij.symm (FreeAlgebra.ι (ZMod 2) x)) hc z
