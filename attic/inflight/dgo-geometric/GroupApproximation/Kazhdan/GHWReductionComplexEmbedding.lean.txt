...
fact behind every archimedean place of their proof of Theorem 4 is
`GHWComplexEmbeddingExtension` (`Kazhdan/GHWInterfaces`), proved here as
...
The proof: `ℚ(t)` and `ℚ(z)` are both the fraction field of `ℚ[X₁, …, Xₙ]`
(`AlgebraicIndependent.aevalEquivField`), which gives `ℚ(t) → ℂ` with `tᵢ ↦ zᵢ`
...
The composite is built at the level of ring homomorphisms: the fraction field of
`ℚ[X₁, …, Xₙ]` carries both its localization algebra and the rational algebra of a
characteristic-zero field, and every ring homomorphism between `ℚ`-algebras is a `ℚ`-algebra
homomorphism (`RingHom.toRatAlgHom`). -/
theorem exists_algHom_adjoin_apply {K : Type} [Field K] [CharZero K] {ι : Type}
  ...
  have hinjt : Function.Injective
      ((MvPolynomial.aeval t : MvPolynomial ι ℚ →ₐ[ℚ] K) : MvPolynomial ι ℚ →+* K) :=
    algebraicIndependent_iff_injective_aeval.1 ht
  have hinjz : Function.Injective
      ((MvPolynomial.aeval z : MvPolynomial ι ℚ →ₐ[ℚ] ℂ) : MvPolynomial ι ℚ →+* ℂ) :=
    algebraicIndependent_iff_injective_aeval.1 hz
  let τ₀ : IntermediateField.adjoin ℚ (Set.range t) →+* ℂ :=
    (IsFractionRing.lift (K := FractionRing (MvPolynomial ι ℚ)) hinjz).comp
      (ht.reprField : IntermediateField.adjoin ℚ (Set.range t) →+*
        FractionRing (MvPolynomial ι ℚ))
  refine ⟨τ₀.toRatAlgHom, fun i => ?_⟩
  set p : IntermediateField.adjoin ℚ (Set.range t) :=
    ⟨t i, IntermediateField.subset_adjoin ℚ _ (Set.mem_range_self i)⟩ with hp
  have h1 : ht.reprField p =
      algebraMap (MvPolynomial ι ℚ) (FractionRing (MvPolynomial ι ℚ)) (MvPolynomial.X i) := by
    apply (IsFractionRing.lift (K := FractionRing (MvPolynomial ι ℚ)) hinjt).injective
    rw [IsFractionRing.lift_algebraMap]
    refine (AlgebraicIndependent.lift_reprField ht p).trans ?_
    show t i = MvPolynomial.aeval t (MvPolynomial.X i)
    rw [MvPolynomial.aeval_X]
  rw [RingHom.toRatAlgHom_apply]
  show IsFractionRing.lift (K := FractionRing (MvPolynomial ι ℚ)) hinjz (ht.reprField p) = z i
  rw [h1, IsFractionRing.lift_algebraMap]
  show MvPolynomial.aeval z (MvPolynomial.X i) = z i
  rw [MvPolynomial.aeval_X]
