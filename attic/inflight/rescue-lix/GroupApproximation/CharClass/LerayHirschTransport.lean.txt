...
theorem bijective_pull_iso {X Y : TopCat.{0}} (e : X ≅ Y) (n : ℕ) :
    Function.Bijective (pull (K := K) e.hom n) := by
  constructor
  · intro a b h
...
theorem colPull_comp {X Y Z : TopCat.{0}} (f : Y ⟶ X) (g : Z ⟶ Y) (r n : ℕ)
    ...
    colPullOf g r n (colPullOf f r n c) = colPullOf (g ≫ f) r n c :=
  lhDomain_ext fun i => by
    rw [colPull_apply, colPull_apply, colPull_apply, pull_comp]

theorem colPull_id {X : TopCat.{0}} (r n : ℕ) (c : lhDomainOf K X r n) :
    colPullOf (𝟙 X) r n c = c :=
  lhDomain_ext fun i => by rw [colPull_apply, pull_id]

theorem bijective_colPull {X Y : TopCat.{0}} (e : X ≅ Y) (r n : ℕ) :
    Function.Bijective (colPullOf (K := K) e.hom r n) := by
  constructor
  · intro c c' h
    have h' := congrArg (colPullOf e.inv r n) h
    rwa [colPull_comp, colPull_comp, e.inv_hom_id, colPull_id, colPull_id] at h'
  · intro c
    exact ⟨colPullOf e.inv r n c, by rw [colPull_comp, e.hom_inv_id, colPull_id]⟩
...
theorem bijective_lhSum_of_iso {X X' P P' : TopCat.{0}}
    ...
    (ξ : Hmod K P 2) (ξ' : Hmod K P' 2) (hξ : pull eP.hom 2 ξ' = ξ) (r n : ℕ)
    (h : Function.Bijective (lhSum f' ξ' r n)) :
    Function.Bijective (lhSum f ξ r n) := by
  have key : ∀ c : lhDomainOf K X' r n,
      pull eP.hom n (lhSum f' ξ' r n c) = lhSum f ξ r n (colPullOf eX.hom r n c) := by
    intro c
    rw [lhSum_pull eX.hom eP.hom f' f hsq ξ' r n c, hξ]
  have hcomp : (fun c => lhSum f ξ r n c) ∘ (colPullOf eX.hom r n)
      = (pull eP.hom n) ∘ (fun c => lhSum f' ξ' r n c) :=
    funext fun c => (key c).symm
  have hb : Function.Bijective ((fun c => lhSum f ξ r n c) ∘ (colPullOf eX.hom r n)) := by
    rw [hcomp]
