...
the shifted-sum column, in the same `iff/∃` element form as `MVSequenceOf`'s own
...
* `lhTerm`, `lhSum`, `colPullOf` — the Leray–Hirsch combination as a homomorphism
  out of `lhDomainOf`, and pullback on the column.
...
variable {K : Type} [CommRing K] {X U V W P UP VP WP Y Q : TopCat.{0}}
...
theorem pull_cohCast {A B : TopCat.{0}} (f : A ⟶ B) {m m' : ℕ} (h : m = m')
    (a : Hmod K B m) : pull f m' (cohCast h a) = cohCast h (pull f m a) := by
  subst h; rfl

/-- A pullback commutes with a finite sum. -/
theorem pull_sum {A B : TopCat.{0}} (f : A ⟶ B) (n : ℕ) {ι : Type} (s : Finset ι)
    (F : ι → Hmod K B n) : pull f n (∑ i ∈ s, F i) = ∑ i ∈ s, pull f n (F i) := by
  have h := map_sum (pullLinear f n) F s
  simpa only [pullLinear_apply] using h

/-- A pullback commutes with a difference.  `CohomologyBasic` publishes `pull_add`,
`pull_zero` and `pullLinear` but no `pull_sub`; this is the two-line consequence,
kept here beside `pull_sum` rather than added upstream while `sp-coeff`'s file is
theirs.  Grepped: nothing else in the tree declares it. -/
theorem pull_sub {A B : TopCat.{0}} (f : A ⟶ B) (n : ℕ) (a b : Hmod K B n) :
    pull f n (a - b) = pull f n a - pull f n b := by
  have h := map_sub (pullLinear f n) a b
  simpa only [pullLinear_apply] using h
...
/-! ## 1. Componentwise laws for `lhDomainOf` -/

@[simp] theorem lhDomain_zero_apply {r n : ℕ} (i : Fin r) :
    ((0 : lhDomainOf K X r n) : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i = 0 := rfl

@[simp] theorem lhDomain_add_apply {r n : ℕ} (c d : lhDomainOf K X r n) (i : Fin r) :
    ((c + d : lhDomainOf K X r n) : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i
      = (c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i
        + (d : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i := rfl

theorem lhDomain_apply_eq {r n : ℕ} {c d : lhDomainOf K X r n} (h : c = d) (i : Fin r) :
    (c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i
      = (d : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i := by rw [h]
...
satisfied by an element of `lhDomainOf`.  This is how each exactness statement of
the column is assembled from the base sequence. -/
theorem lhDomain_choice {r n : ℕ} (Qp : (i : Fin r) → Hmod K X (n - 2 * (i : ℕ)) → Prop)
    (h0 : ∀ i : Fin r, n < 2 * (i : ℕ) → Qp i 0)
    (hex : ∀ i : Fin r, 2 * (i : ℕ) ≤ n → ∃ a, Qp i a) :
    ∃ c : lhDomainOf K X r n,
      ∀ i : Fin r, Qp i ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i) := by
  have key : ∀ i : Fin r, ∃ a : Hmod K X (n - 2 * (i : ℕ)),
...
the geometry that `MVSequenceOf` deliberately forgets, and it is what makes the
restrictions multiplicative.  `cc-thom`'s `mvResU_eq_pull` family instantiates
it in one line each. -/
structure MVPulls (mv : MVSequenceOf K X U V W) where
  ...
  resU_eq : ∀ (n : ℕ) (x : Hmod K X n), mv.resU n x = pull iU n x
  /-- Restriction to the second open set is a pullback. -/
  resV_eq : ∀ (n : ℕ) (x : Hmod K X n), mv.resV n x = pull iV n x
  /-- Restriction from the first open set to the intersection is a pullback. -/
  resWU_eq : ∀ (n : ℕ) (x : Hmod K U n), mv.resWU n x = pull jU n x
  /-- Restriction from the second open set to the intersection is a pullback. -/
  resWV_eq : ∀ (n : ℕ) (x : Hmod K V n), mv.resWV n x = pull jV n x
...
theorem resW_compat (mv : MVSequenceOf K X U V W) (n : ℕ) (x : Hmod K X n) :
...
theorem resU_cohCast (mv : MVSequenceOf K X U V W) {m m' : ℕ} (h : m = m') (a : Hmod K X m) :
...
theorem resV_cohCast (mv : MVSequenceOf K X U V W) {m m' : ℕ} (h : m = m') (a : Hmod K X m) :
    mv.resV m' (cohCast h a) = cohCast h (mv.resV m a) := by subst h; rfl

/-- The connecting map commutes with a transport of degrees. -/
theorem delta_cohCast (mv : MVSequenceOf K X U V W) {m m' : ℕ} (h : m = m') (a : Hmod K W m) :
...
field. -/
structure MVLadder {mvX : MVSequenceOf K X U V W} {mvP : MVSequenceOf K P UP VP WP}
...
def lhTerm (π : P ⟶ X) (ξ : Hmod K P 2) (n i : ℕ) (a : Hmod K X (n - 2 * i)) : Hmod K P n :=
  if h : 2 * i ≤ n then
    cohCast (Nat.sub_add_cancel h) (cup (pull π (n - 2 * i) a) (cupPowE ξ i))
  else 0

theorem lhTerm_of_le (π : P ⟶ X) (ξ : Hmod K P 2) {n i : ℕ} (h : 2 * i ≤ n)
    (a : Hmod K X (n - 2 * i)) :
    lhTerm π ξ n i a
      = cohCast (Nat.sub_add_cancel h) (cup (pull π (n - 2 * i) a) (cupPowE ξ i)) :=
  dif_pos h

theorem lhTerm_of_gt (π : P ⟶ X) (ξ : Hmod K P 2) {n i : ℕ} (h : ¬ 2 * i ≤ n)
    (a : Hmod K X (n - 2 * i)) : lhTerm π ξ n i a = 0 :=
  dif_neg h

@[simp] theorem lhTerm_zero (π : P ⟶ X) (ξ : Hmod K P 2) (n i : ℕ) :
    lhTerm π ξ n i 0 = 0 := by
  by_cases h : 2 * i ≤ n
  · rw [lhTerm_of_le π ξ h, pull_zero, zero_cup, cohCast_zero]
  · rw [lhTerm_of_gt π ξ h]

theorem lhTerm_add (π : P ⟶ X) (ξ : Hmod K P 2) (n i : ℕ) (a b : Hmod K X (n - 2 * i)) :
...
subtype coercion. -/
def lhSumPi (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ) :
    ((i : Fin r) → Hmod K X (n - 2 * (i : ℕ))) →+ Hmod K P n where
...
def lhSum (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ) : lhDomainOf K X r n →+ Hmod K P n :=
  (lhSumPi π ξ r n).comp (lhDomainOf K X r n).subtype

theorem lhSum_apply (π : P ⟶ X) (ξ : Hmod K P 2) (r n : ℕ) (c : lhDomainOf K X r n) :
    lhSum π ξ r n c = ∑ i : Fin r,
      lhTerm π ξ n (i : ℕ) ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i) := rfl

/-- Pullback on the ambient family, componentwise. -/
def colPullPiOf (f : Y ⟶ X) (r n : ℕ) :
    ((i : Fin r) → Hmod K X (n - 2 * (i : ℕ))) →+
      ((i : Fin r) → Hmod K Y (n - 2 * (i : ℕ))) where
...
def colPullOf (f : Y ⟶ X) (r n : ℕ) :
    lhDomainOf K X r n →+ lhDomainOf K Y r n :=
  AddMonoidHom.codRestrict ((colPullPiOf f r n).comp (lhDomainOf K X r n).subtype)
    (lhDomainOf K Y r n) (by
      intro c i hi
      show pull f (n - 2 * (i : ℕ))
        ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i) = 0
      rw [lhDomain_vanish c i hi, pull_zero])

/-- The mod-2 spelling of pullback on the column, kept reducible so that
`LerayHirschHomotopy`'s partial applications `Function.Bijective (colPull f r n)`
are byte-unchanged. -/
abbrev colPull (f : Y ⟶ X) (r n : ℕ) : lhDomain X r n →+ lhDomain Y r n :=
  colPullOf (K := ZMod 2) f r n

@[simp] theorem colPull_apply (f : Y ⟶ X) (r n : ℕ) (c : lhDomainOf K X r n) (i : Fin r) :
    ((colPullOf f r n c : (j : Fin r) → Hmod K Y (n - 2 * (j : ℕ))) i)
      = pull f (n - 2 * (i : ℕ))
          ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i) := rfl
...
theorem lhTerm_pull (f : Y ⟶ X) (g : Q ⟶ P) (πP : P ⟶ X) (πQ : Q ⟶ Y)
    (hsq : g ≫ πP = πQ ≫ f) (ξ : Hmod K P 2) (n i : ℕ) (a : Hmod K X (n - 2 * i)) :
...
theorem lhSum_pull (f : Y ⟶ X) (g : Q ⟶ P) (πP : P ⟶ X) (πQ : Q ⟶ Y)
    (hsq : g ≫ πP = πQ ≫ f) (ξ : Hmod K P 2) (r n : ℕ) (c : lhDomainOf K X r n) :
    pull g n (lhSum πP ξ r n c)
      = lhSum πQ (pull g 2 ξ) r n (colPullOf f r n c) := by
  rw [lhSum_apply, lhSum_apply, pull_sum]
  refine Finset.sum_congr rfl ?_
  intro i _
  rw [lhTerm_pull f g πP πQ hsq, colPull_apply]

theorem colResU_eq_colPull {mv : MVSequenceOf K X U V W} (p : MVPulls mv) (r n : ℕ)
    (c : lhDomainOf K X r n) : colResU mv r n c = colPullOf p.iU r n c :=
  lhDomain_ext fun i => by rw [colResU_apply, colPull_apply, p.resU_eq]

theorem colResV_eq_colPull {mv : MVSequenceOf K X U V W} (p : MVPulls mv) (r n : ℕ)
    (c : lhDomainOf K X r n) : colResV mv r n c = colPullOf p.iV r n c :=
  lhDomain_ext fun i => by rw [colResV_apply, colPull_apply, p.resV_eq]

theorem colResWU_eq_colPull {mv : MVSequenceOf K X U V W} (p : MVPulls mv) (r n : ℕ)
    (c : lhDomainOf K U r n) : colResWU mv r n c = colPullOf p.jU r n c :=
  lhDomain_ext fun i => by rw [colResWU_apply, colPull_apply, p.resWU_eq]

theorem colResWV_eq_colPull {mv : MVSequenceOf K X U V W} (p : MVPulls mv) (r n : ℕ)
    (c : lhDomainOf K V r n) : colResWV mv r n c = colPullOf p.jV r n c :=
...
theorem lhSum_resU (pX : MVPulls mvX) (pP : MVPulls mvP) (L : MVLadder pX pP)
    (ξ : Hmod K P 2) (r n : ℕ) (c : lhDomainOf K X r n) :
...
theorem lhSum_resWU (pX : MVPulls mvX) (pP : MVPulls mvP) (L : MVLadder pX pP)
    (ξU : Hmod K UP 2) (r n : ℕ) (c : lhDomainOf K U r n) :
...
theorem lhSum_resWV (pX : MVPulls mvX) (pP : MVPulls mvP) (L : MVLadder pX pP)
    (ξV : Hmod K VP 2) (r n : ℕ) (c : lhDomainOf K V r n) :
...
def deltaTerm (mv : MVSequenceOf K X U V W) (n i : ℕ) (a : Hmod K W (n - 2 * i)) :
    Hmod K X (n + 1 - 2 * i) :=
  if h : 2 * i ≤ n then cohCast (by omega) (mv.δ (n - 2 * i) a) else 0

theorem deltaTerm_of_le (mv : MVSequenceOf K X U V W) {n i : ℕ} (h : 2 * i ≤ n)
    (a : Hmod K W (n - 2 * i)) :
    deltaTerm mv n i a = cohCast (by omega) (mv.δ (n - 2 * i) a) :=
  dif_pos h

theorem deltaTerm_of_gt (mv : MVSequenceOf K X U V W) {n i : ℕ} (h : ¬ 2 * i ≤ n)
    (a : Hmod K W (n - 2 * i)) : deltaTerm mv n i a = 0 :=
  dif_neg h

theorem colDelta_apply' (mv : MVSequenceOf K X U V W) (r n : ℕ) (c : lhDomainOf K W r n)
    (i : Fin r) :
    ((colDelta mv r n c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i)
      = deltaTerm mv n (i : ℕ)
          ((c : (j : Fin r) → Hmod K W (n - 2 * (j : ℕ))) i) := rfl
...
theorem lhTerm_delta (pP : MVPulls mvP) (hP : MVFacts mvP) (π : P ⟶ X) (πW : WP ⟶ W)
    (hdel : ∀ (m : ℕ) (a : Hmod K W m),
      mvP.δ m (pull πW m a) = pull π (m + 1) (mvX.δ m a))
    (ξ : Hmod K P 2) (n i : ℕ) (a : Hmod K W (n - 2 * i)) :
...
theorem colExactW (mv : MVSequenceOf K X U V W) (r n : ℕ) (c : lhDomainOf K W r n) :
    colDelta mv r n c = 0 ↔ ∃ (a : lhDomainOf K U r n) (b : lhDomainOf K V r n),
      colResWU mv r n a + colResWV mv r n b = c := by
  constructor
  · intro h
    have key : ∀ i : Fin r, ∃ ab : Hmod K U (n - 2 * (i : ℕ)) × Hmod K V (n - 2 * (i : ℕ)),
        (n < 2 * (i : ℕ) → ab.1 = 0 ∧ ab.2 = 0) ∧
        mv.resWU (n - 2 * (i : ℕ)) ab.1 + mv.resWV (n - 2 * (i : ℕ)) ab.2
          = (c : (j : Fin r) → Hmod K W (n - 2 * (j : ℕ))) i := by
      intro i
      by_cases hi : n < 2 * (i : ℕ)
      · refine ⟨(0, 0), fun _ => ⟨rfl, rfl⟩, ?_⟩
        rw [map_zero, map_zero, add_zero, lhDomain_vanish c i hi]
      · have hd : mv.δ (n - 2 * (i : ℕ))
            ((c : (j : Fin r) → Hmod K W (n - 2 * (j : ℕ))) i) = 0 := by
...
theorem colExactX (mv : MVSequenceOf K X U V W) (hf : MVFacts mv) (r n : ℕ)
    (c : lhDomainOf K X r (n + 1)) :
    (colResU mv r (n + 1) c = 0 ∧ colResV mv r (n + 1) c = 0)
      ↔ ∃ w : lhDomainOf K W r n, colDelta mv r n w = c := by
  constructor
  · rintro ⟨hU, hV⟩
    have hUc : ∀ i : Fin r, mv.resU (n + 1 - 2 * (i : ℕ))
        ((c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i) = 0 := by
      intro i
    ...
    have hzero : ∀ i : Fin r, n < 2 * (i : ℕ) →
      ...
      · have he : n + 1 - 2 * (i : ℕ) = 0 := by omega
        have h0 : cohCast he ((c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i) = 0 := by
    ...
    have hchoice : ∃ w : lhDomainOf K W r n, ∀ i : Fin r,
        deltaTerm mv n (i : ℕ)
            ((w : (j : Fin r) → Hmod K W (n - 2 * (j : ℕ))) i)
          = (c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i := by
      refine lhDomain_choice
        (fun i wi => deltaTerm mv n (i : ℕ) wi
          = (c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i) ?_ ?_
      · intro i hi
        rw [deltaTerm_of_gt mv (by omega), hzero i hi]
      · intro i hi
        have he : n + 1 - 2 * (i : ℕ) = (n - 2 * (i : ℕ)) + 1 := by omega
        obtain ⟨w, hw⟩ := (mv.exact_X (n - 2 * (i : ℕ))
          (cohCast he ((c : (j : Fin r) → Hmod K X (n + 1 - 2 * (j : ℕ))) i))).mp
...
Stated as the *equality* of the two restrictions, which is exactly the shape of
`MVSequenceOf.exact_sum` and is sign-free: it says the pair `(a, b)` is in the
kernel of the difference map, without committing to how that map is written.  The
`F₂` spelling that the five lemma consumes — "the sum is zero" — is
`colExactSum_add` below. -/
theorem colExactSum (mv : MVSequenceOf K X U V W) (r n : ℕ)
    (a : lhDomainOf K U r n) (b : lhDomainOf K V r n) :
    colResWU mv r n a = colResWV mv r n b
      ↔ ∃ c : lhDomainOf K X r n, colResU mv r n c = a ∧ colResV mv r n c = b := by
  constructor
  · intro h
    have hcomp : ∀ i : Fin r,
        mv.resWU (n - 2 * (i : ℕ)) ((a : (j : Fin r) → Hmod K U (n - 2 * (j : ℕ))) i)
          = mv.resWV (n - 2 * (i : ℕ))
              ((b : (j : Fin r) → Hmod K V (n - 2 * (j : ℕ))) i) := by
      intro i
      have h' := lhDomain_apply_eq h i
      rwa [colResWU_apply, colResWV_apply] at h'
    have hchoice : ∃ c : lhDomainOf K X r n, ∀ i : Fin r,
        mv.resU (n - 2 * (i : ℕ))
              ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i)
            = (a : (j : Fin r) → Hmod K U (n - 2 * (j : ℕ))) i ∧
          mv.resV (n - 2 * (i : ℕ))
              ((c : (j : Fin r) → Hmod K X (n - 2 * (j : ℕ))) i)
            = (b : (j : Fin r) → Hmod K V (n - 2 * (j : ℕ))) i := by
      refine lhDomain_choice
        (fun i x => mv.resU (n - 2 * (i : ℕ)) x
            = (a : (j : Fin r) → Hmod K U (n - 2 * (j : ℕ))) i ∧
          mv.resV (n - 2 * (i : ℕ)) x
            = (b : (j : Fin r) → Hmod K V (n - 2 * (j : ℕ))) i) ?_ ?_
      · intro i hi
  ...
  · rintro ⟨c, rfl, rfl⟩
    exact lhDomain_ext fun i => by
      rw [colResWU_apply, colResWV_apply, colResU_apply, colResV_apply, resW_compat]

/-- The `F₂` spelling of `colExactSum`: the middle map of the ladder is written as
a sum, so exactness there reads "the sum vanishes".  This is the form
`LerayHirschInduction`'s five lemma consumes, and it is the only thing in this
file that uses characteristic two — over `K` the map is the difference and
`colExactSum` is reached by `sub_eq_zero`. -/
theorem colExactSum_add (mv : MVSequence X U V W) (r n : ℕ)
  ...
  rw [← colExactSum mv r n a b]
  constructor
  · intro h
    refine lhDomain_ext fun i => ?_
    have h' := lhDomain_apply_eq h i
    rw [lhDomain_add_apply, lhDomain_zero_apply] at h'
    exact (add_eq_zero_iff_eq _ _).mp h'
  · intro h
    refine lhDomain_ext fun i => ?_
    rw [lhDomain_add_apply, lhDomain_zero_apply, lhDomain_apply_eq h i]
