...
* `PuncturedAcyclicOf.congr` — top-punctured acyclicity transports along a homeomorphism,
  generic in the coefficient ring; `PuncturedAcyclic.congr` is its `ZMod 2` spelling.
...
`CohomologyToolkitOf.homeo`: the first directly, the second after restricting the
homeomorphism to the punctured spaces (`homeomorphCompl`).  Generic in the coefficient
ring, which the toolkit determines; `PuncturedAcyclic.congr` is the `ZMod 2` spelling. -/
theorem congr (T : CohomologyToolkitOf K) {Z W : Type} [TopologicalSpace Z]
    [TopologicalSpace W] {d : ℕ} {z : Z} (e : Z ≃ₜ W) (h : PuncturedAcyclicOf K W d (e z)) :
...
theorem congr' (T : CohomologyToolkitOf K) {Z W : Type} [TopologicalSpace Z]
    ...
    (h : PuncturedAcyclicOf K W d w) : PuncturedAcyclicOf K Z d z :=
  congr T e (by rw [hw]; exact h)

end PuncturedAcyclicOf

/-- The two transport lemmas at `ZMod 2`, as real wrappers rather than `export` aliases.  See
`ThomPuncturedRecursion`'s `PuncturedAcyclic.prod` for why an alias would break dot notation
on these very names. -/
theorem PuncturedAcyclic.congr (T : CohomologyToolkit) {Z W : Type} [TopologicalSpace Z]
    [TopologicalSpace W] {d : ℕ} {z : Z} (e : Z ≃ₜ W) (h : PuncturedAcyclic W d (e z)) :
    PuncturedAcyclic Z d z :=
  PuncturedAcyclicOf.congr T e h

/-- The `ZMod 2` spelling of `PuncturedAcyclicOf.congr'`. -/
theorem PuncturedAcyclic.congr' (T : CohomologyToolkit) {Z W : Type} [TopologicalSpace Z]
    [TopologicalSpace W] {d : ℕ} {z : Z} {w : W} (e : Z ≃ₜ W) (hw : e z = w)
    (h : PuncturedAcyclic W d w) : PuncturedAcyclic Z d z :=
  PuncturedAcyclicOf.congr' T e hw h

/-! ## Iterating the binary step

There is deliberately no `Fin`-indexed version of `PuncturedAcyclicOf.prod` here.  The
...
and built by chaining `PuncturedAcyclicOf.prod` one factor at a time, each step supplying
