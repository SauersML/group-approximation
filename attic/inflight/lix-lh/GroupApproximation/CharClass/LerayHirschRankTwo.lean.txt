...
theorem lhTerm_zero_index (π : P ⟶ X) (ξ : Hmod K P 2) (n : ℕ)
    (a : Hmod K X (n - 2 * 0)) : lhTerm π ξ n 0 a = pull π n a := by
...
def colLow (X : TopCat.{0}) (n : ℕ) (hn : n < 2) (x : Hmod K X n) : lhDomainOf K X 2 n :=
  ⟨Fin.cases (motive := fun j : Fin 2 => Hmod K X (n - 2 * (j : ℕ))) x (fun _ => 0), by
    intro j hj
    revert hj
...
@[simp] theorem colLow_zero (n : ℕ) (hn : n < 2) (x : Hmod K X n) :
    ((colLow X n hn x : lhDomainOf K X 2 n) :
      (j : Fin 2) → Hmod K X (n - 2 * (j : ℕ))) 0 = x := rfl

theorem lhSum_two_low (π : P ⟶ X) (ξ : Hmod K P 2) (n : ℕ) (hn : n < 2)
    (c : lhDomainOf K X 2 n) :
    lhSum π ξ 2 n c = pull π n ((c : (j : Fin 2) → Hmod K X (n - 2 * (j : ℕ))) 0) := by
  have h1 : (c : (j : Fin 2) → Hmod K X (n - 2 * (j : ℕ))) 1 = 0 :=
    lhDomain_vanish c 1 (by change n < 2; omega)
  rw [lhSum_apply, Fin.sum_univ_two]
  show lhTerm π ξ n 0 ((c : (j : Fin 2) → Hmod K X (n - 2 * (j : ℕ))) 0)
    + lhTerm π ξ n 1 ((c : (j : Fin 2) → Hmod K X (n - 2 * (j : ℕ))) 1) = _
  rw [h1, lhTerm_zero, add_zero, lhTerm_zero_index]

/-! ## 1b. In degree `n + 2`, over `K`

`GysinFromGraded` writes the rank-two combination out at `F₂` (`lhSum_two_apply`, `colTwo`);
the interface below is stated over any `K`, so it uses these forms instead. -/

theorem cupPowE_oneOf (e : Hmod K P 2) : cupPowE e 1 = e := by
  show cup (cupPowE e 0) e = e
  rw [cupPowE_zero, one_cup']
  exact cohCast_self _ _

theorem lhTerm_index_zeroOf (π : P ⟶ X) (ξ : Hmod K P 2) (n : ℕ)
    (a : Hmod K X (n + 2 - 2 * 0)) : lhTerm π ξ (n + 2) 0 a = pull π (n + 2) a := by
  rw [lhTerm_of_le π ξ (by omega : 2 * 0 ≤ n + 2), cupPowE_zero, cup_one]
  exact cohCast_self _ _

theorem lhTerm_index_oneOf (π : P ⟶ X) (ξ : Hmod K P 2) (n : ℕ)
    (b : Hmod K X (n + 2 - 2 * 1)) :
    lhTerm π ξ (n + 2) 1 b = cup (pull π n b) ξ := by
  rw [lhTerm_of_le π ξ (by omega : 2 * 1 ≤ n + 2), cupPowE_oneOf]
  exact cohCast_self _ _

/-- **The rank-two combination in degree `n + 2`, written out**, over any `K`. -/
theorem lhSum_two_applyOf (π : P ⟶ X) (ξ : Hmod K P 2) (n : ℕ)
    (c : lhDomainOf K X 2 (n + 2)) :
    lhSum π ξ 2 (n + 2) c
      = pull π (n + 2) ((c : (j : Fin 2) → Hmod K X (n + 2 - 2 * (j : ℕ))) 0)
        + cup (pull π n ((c : (j : Fin 2) → Hmod K X (n + 2 - 2 * (j : ℕ))) 1)) ξ := by
  rw [lhSum_apply, Fin.sum_univ_two]
  show lhTerm π ξ (n + 2) 0 ((c : (j : Fin 2) → Hmod K X (n + 2 - 2 * (j : ℕ))) 0)
    + lhTerm π ξ (n + 2) 1 ((c : (j : Fin 2) → Hmod K X (n + 2 - 2 * (j : ℕ))) 1) = _
  rw [lhTerm_index_zeroOf, lhTerm_index_oneOf]

/-- The rank-two column element with prescribed components, over any `K`. -/
def colTwoOf (X : TopCat.{0}) (n : ℕ) (a : Hmod K X (n + 2)) (b : Hmod K X n) :
...
@[simp] theorem colTwoOf_zero (n : ℕ) (a : Hmod K X (n + 2)) (b : Hmod K X n) :
    ((colTwoOf X n a b : lhDomainOf K X 2 (n + 2)) :
      (j : Fin 2) → Hmod K X (n + 2 - 2 * (j : ℕ))) 0 = a := rfl

@[simp] theorem colTwoOf_one (n : ℕ) (a : Hmod K X (n + 2)) (b : Hmod K X n) :
    ((colTwoOf X n a b : lhDomainOf K X 2 (n + 2)) :
...
theorem bijective_lhSum_two (π : P ⟶ X) (ξ : Hmod K P 2)
    (hlow : ∀ n : ℕ, n < 2 → Function.Bijective (pull (K := K) π n))
    (hsurj : ∀ (n : ℕ) (z : Hmod K P (n + 2)),
  ...
  | (m + 2) =>
    ...
    · intro z
      ...
      exact ⟨colTwoOf X m a b, by rw [lhSum_two_applyOf, colTwoOf_zero, colTwoOf_one, ← hab]⟩

/-- **The interface, over a trivialising open set.** -/
theorem LHOver_two_of_decomposition (f : P ⟶ X) (ξ : Hmod K P 2) (U : Opens X)
    (hlow : ∀ n : ℕ, n < 2 → Function.Bijective (pull (K := K) (opensRestrict f U) n))
    (hsurj : ∀ (n : ℕ) (z : Hmod K (opSpace (opensComap f U)) (n + 2)),
      ∃ (a : Hmod K (opSpace U) (n + 2)) (b : Hmod K (opSpace U) n),
        z = pull (opensRestrict f U) (n + 2) a
          + cup (pull (opensRestrict f U) n b) (lhClass f ξ U))
    (huniq : ∀ (n : ℕ) (a : Hmod K (opSpace U) (n + 2)) (b : Hmod K (opSpace U) n),
