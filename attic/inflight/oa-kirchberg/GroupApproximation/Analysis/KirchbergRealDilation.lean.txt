...
import GroupApproximation.Analysis.KirchbergRealForms
import Mathlib.Analysis.InnerProductSpace.LinearMap
import Mathlib.LinearAlgebra.Finsupp.LSum

/-!
# The real dilation of a positive definite matrix function
...
namespace KirchbergRealDilation

open Matrix ComplexConjugate UniformSpace KirchbergRealForms
...
theorem kernelForm_add_left (f₁ f₂ f' : G →₀ (Y → ℂ)) :
  ...
  unfold kernelForm
  refine Finsupp.sum_add_index' (fun g ↦ ?_) (fun g v v' ↦ ?_)
...
theorem kernelForm_smul_left (r : ℝ) (f f' : G →₀ (Y → ℂ)) :
  ...
  unfold kernelForm
  rw [Finsupp.sum_smul_index' (fun g ↦ by simp [pairing_zero_left]), Finsupp.mul_sum]
...
noncomputable def preRep : G →* (Pre X hX ≃ₗᵢ[ℝ] Pre X hX) where
  ...
  map_one' := LinearIsometryEquiv.ext fun f ↦ (toPre X hX).symm.injective <| by
    rw [translate_apply]
    ext a x
    simp only [Finsupp.equivMapDomain_apply, Equiv.mulLeft_symm_apply, inv_one, one_mul]
    rfl
  map_mul' g h := LinearIsometryEquiv.ext fun f ↦ (toPre X hX).symm.injective <| by
    rw [translate_apply, LinearIsometryEquiv.coe_mul, Function.comp_apply, translate_apply,
      translate_apply]
    ext a x
    simp only [Finsupp.equivMapDomain_apply, Equiv.mulLeft_symm_apply, mul_inv_rev, mul_assoc]

/-- **The orthogonal representation on the dilation space.** -/
noncomputable def rep : G →* (Space X hX ≃ₗᵢ[ℝ] Space X hX) :=
  KirchbergCompletion.completionRep (preRep X hX)
...
theorem rep_vec (g : G) (w : Y → ℂ) :
  ...
  rw [rep, vec, KirchbergCompletion.completionRep_coe]
  congr 1
  refine (toPre X hX).symm.injective ?_
  change (toPre X hX).symm (translate X hX g (toPre X hX (Finsupp.single 1 w)))
    = Finsupp.single g w
  rw [translate_apply]
  simp only [LinearEquiv.symm_apply_apply, Finsupp.equivMapDomain_single, Equiv.coe_mulLeft,
    mul_one]
...
theorem inner_vec_vec (v w : Y → ℂ) :
  ...
  simpa using inner_vec_rep_vec X hX v w 1

end KirchbergRealDilation
