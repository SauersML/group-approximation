...
dart. -/
def BadCorner (K : PocketFaceSet D eps X lo hi) (d : X.toCombMap.Dart) : Prop :=
...
noncomputable def badCorners (K : PocketFaceSet D eps X lo hi) : ℕ :=
  K.boundary.cycle.countP fun d => decide (K.BadCorner d)

/-- A pocket without good corners has a bad corner on its boundary cycle. -/
theorem exists_badCorner_of_not_goodCorners (K : PocketFaceSet D eps X lo hi)
...
letter. -/
theorem one_lt_length_of_badCorner (K : PocketFaceSet D eps X lo hi)
...
theorem exists_goodCorners (K : PocketFaceSet D eps X lo hi) (hW : ∀ word ∈ W, 1 < word.length)
...
theorem exists_simple_of_firstTurns (K : PocketFaceSet D eps X lo hi)
...
theorem pocketPinchFirstTurns : PocketPinchFirstTurnsStatement.{u, w, v} := by
  intro G _ Lambda D eps W hW X lo hi hlabel K hK
...
theorem pocketPinchLabelledFirstTurnSection :
  ...
  refine ⟨0, fun eps _ => ⟨2, Nat.zero_lt_two,
    fun rho hrho W hcondition X lo hi _ hlabel K hK _ => ?_⟩⟩
