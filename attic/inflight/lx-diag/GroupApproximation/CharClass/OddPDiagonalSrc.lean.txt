import GroupApproximation.CharClass.OddPDiagonalAug
import GroupApproximation.CharClass.OddPSourceFree
import GroupApproximation.CharClass.OddPSingular

/-!
# The source augmentation and the degree-`0` diagonal of the odd-primary comparison

Two inputs of `acyclicModelsMap` that do not wait on acyclicity:

* `srcAug p` — **the augmentation of `W ⊗ C(X)`** at the concrete signed singular boundary,
  into the trivial module of `OddPDiagonalAug.lean`: a generator `c · (e_0 ⊗ x)` goes to
  `grAug c`.  It kills boundaries because `grAug (T − 1) = 0` and a 1-simplex has two faces of
  opposite sign.
* `diagF0 p r s hs X` — **the degree-`0` diagonal** `e_0 ⊗ x ↦ (x, …, x)`, natural and
  compatible with the two augmentations.
-/
...
/-! ## 1. The source augmentation -/

/-- The augmentation of the degree-`0` source carrier, over the group ring. -/
def srcAugLin (X : TopCat.{0}) :
    @LinearMap (GroupRingZMod p) (GroupRingZMod p) _ _ (RingHom.id (GroupRingZMod p))
      (OddWTensor p 0 X) (ZMod p) _ _ _ (trivModule p) :=
  letI := trivModule p
  Finsupp.linearCombination (GroupRingZMod p) (fun _ => (1 : ZMod p))

omit [NeZero p] in
theorem srcAugLin_single (X : TopCat.{0}) (q : WSIndex 0 X) :
    srcAugLin p X (Finsupp.single q (1 : GroupRingZMod p)) = 1 := by
  letI := trivModule p
...
theorem srcAugLin_bdRHom (X : TopCat.{0}) (σ : stdSimplexTop (0 + 1) ⟶ X) :
    letI := trivModule p
    Finsupp.linearCombination (GroupRingZMod p) (fun _ : WSIndex 0 X => (1 : ZMod p))
      (Finsupp.lmapDomain (GroupRingZMod p) (GroupRingZMod p)
        ...
        (bdRHom (GroupRingZMod p) X 0 (Finsupp.single σ 1))) = 0 := by
  letI := trivModule p
  rw [Finsupp.lmapDomain_apply, lc_const_mapDomain, bdRHom_apply, lc_const_mapDomain,
    Finsupp.mapDomain_single, singFreeR_d_single, map_sum,
    Fin.sum_univ_two, map_zsmul, map_zsmul, Finsupp.linearCombination_single,
    Finsupp.linearCombination_single, one_smul, Fin.val_zero, Fin.val_one, pow_zero, pow_one,
    one_zsmul, neg_one_zsmul, add_neg_cancel]

/-- **The augmentation of the odd-primary source.** -/
def srcAug : Augmentation (GroupRingZMod p) (oddSrc p (oddSingularBoundary p)) (trivObj p) where
  ε X :=
    letI := trivModule p
    ModuleCat.ofHom (srcAugLin p X)
  ε_natural X Y φ := by
    letI := trivModule p
  ...
  ε_d X := by
    letI := trivModule p
    change (oddSrcComplex p (oddSingularBoundary p) X).d 1 0 ≫ _ = 0
    rw [oddSrcComplex_d]
    apply ModuleCat.hom_ext
    apply Finsupp.lhom_ext'
    intro q
    apply LinearMap.ext_ring
    show srcAugLin p X (oddDiff p (oddSingularBoundary p) X 0
      (Finsupp.single q (1 : GroupRingZMod p))) = 0
    rw [oddDiff_single]
    obtain ⟨n, σ⟩ := q
...
theorem srcAug_ε_hom_single (X : TopCat.{0}) (q : WSIndex 0 X) :
    ((srcAug p).ε X).hom (Finsupp.single q (1 : GroupRingZMod p)) = 1 :=
  srcAugLin_single p X q

/-! ## 2. The degree-`0` diagonal -/

omit [NeZero p] in
/-- The constant tuple of a point. -/
def diagPt (X : TopCat.{0}) (r : ℕ) (x : stdSimplexTop 0 ⟶ X) : TupIdx X r 0 :=
...
/-- **The degree-`0` diagonal** `e_0 ⊗ x ↦ (x, …, x)`. -/
def diagF0 (r s : ℕ) (hs : r ∣ s * p) (X : TopCat.{0}) :
    ((oddSrc p (oddSingularBoundary p)).obj X).X 0 ⟶ ((oddTgt p r s hs).obj X).X 0 :=
  letI := tupModule p X r 0 s hs
  ModuleCat.ofHom (diagF0Lin p X r s hs)

theorem diagF0_natural (r s : ℕ) (hs : r ∣ s * p) (X Y : TopCat.{0}) (φ : X ⟶ Y) :
    ((oddSrc p (oddSingularBoundary p)).map φ).f 0 ≫ diagF0 p r s hs Y
      = diagF0 p r s hs X ≫ ((oddTgt p r s hs).map φ).f 0 := by
  letI := tupModule p X r 0 s hs
...
theorem diagF0_aug (r s : ℕ) (hs : r ∣ s * p) (X : TopCat.{0}) :
  ...
  intro q
  apply LinearMap.ext_ring
  show tupAug (ZMod p) X r (diagF0Lin p X r s hs (Finsupp.single q (1 : GroupRingZMod p)))
    = srcAugLin p X (Finsupp.single q 1)
  rw [diagF0Lin_single, tupAug_single, srcAugLin_single]
