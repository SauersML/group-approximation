...
- **Projection measure (Theorem 6).**
  - For `q = r^k`, drop the last of `k` coordinates to a fixed symbol. The result is iid and uniform on
    `r^(k-1)` symbols.
  - Its support is a full shift on those symbols, with the measure as its unique uniform Gibbs measure.
  - Completion counts are `r^((k-1)|E|)`. Lemma 2 for `M^-1` gives `k|I_E| <= (k-1)|E|`, so the divisibility by
    `q^|I_E|` holds.
  - R3 follows from `r^(-(k-1)|E|) = q^(-(1-1/k)|E|) <= q^(-|int_N E|)`.
