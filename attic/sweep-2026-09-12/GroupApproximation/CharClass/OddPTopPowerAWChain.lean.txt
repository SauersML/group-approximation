...
theorem tupPush_snocT {r : ℕ} (t : TupAll X r) (τ : TagSimp X) :
  ...
  | last =>
    have h1 : snocT X t τ (Fin.last r) = τ := Fin.snoc_last (α := fun _ => TagSimp X) τ t
    have h2 : snocT Y (tupPush f t) (tagPush f τ) (Fin.last r) = tagPush f τ :=
      Fin.snoc_last (α := fun _ => TagSimp Y) (tagPush f τ) (tupPush f t)
    show tagPush f (snocT X t τ (Fin.last r)) = snocT Y (tupPush f t) (tagPush f τ) (Fin.last r)
    rw [h1, h2]
  | cast j =>
    have h1 : snocT X t τ j.castSucc = t j := Fin.snoc_castSucc (α := fun _ => TagSimp X) τ t j
    have h2 : snocT Y (tupPush f t) (tagPush f τ) j.castSucc = tagPush f (t j) :=
      Fin.snoc_castSucc (α := fun _ => TagSimp Y) (tagPush f τ) (tupPush f t) j
    show tagPush f (snocT X t τ j.castSucc) = snocT Y (tupPush f t) (tagPush f τ) j.castSucc
    rw [h1, h2]
...
theorem tupAllMap_catLin {r : ℕ} (x : tupAllMod K X r) (τ : TagSimp X) :
  ...
  | single t c =>
    ...
    simp only [map_smul, LinearMap.smul_apply, catLin_single, tupAllMap_single, tupPush_snocT]
