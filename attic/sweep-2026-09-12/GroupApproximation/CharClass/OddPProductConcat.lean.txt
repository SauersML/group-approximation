...
def sqSumEquiv (m : ℕ) : Fin m ⊕ Fin m ≃ Fin (2 * m) where
  ...
  left_inv x := by
    rcases x with a | b
    · refine (dif_pos a.2).trans ?_
      exact congrArg Sum.inl (Fin.ext rfl)
    · refine (dif_neg (show ¬ (m + (b : ℕ) < m) by omega)).trans ?_
