...
- **Joint configuration: the models break on mixed-prime finite groups; the gate stays OPEN** (w7-v-gate-c3;
  ...
  - **Which relation breaks the models** (artifact §1).
    ...
    - In each case the first relation that no extension satisfies is Theorem 1.1 on a finite group `B ⋊ G`, where `B`
      is a clopen `q`-cycle group with `q != p` (5, or 3 in characteristic 2). This uses disjoint commutation, the
      conjugation data `g c_v g^(-1) = c_(g v)`, and the torsion law.
