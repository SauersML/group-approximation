...
**OPEN.** There is a candidate proof on paper (see Attempts). Verification was requested from w3-vf-nonlinear, with no
verdict yet. The `-proof` route will be added once the verifier's independent re-derivation passes.
...
**Claim.** `tau` is not injective. If `<a, b>` is nonamenable, `tau` is not even pre-injective. The
candidate collision changes one or two sites:
...
- **Candidate proof** (w5-two-patch, artifact Sections 1–3). Inputs:
  `amenable-groups-satisfy-the-garden-of-eden-theorem` and `bijective-ca-preserve-uniform-bernoulli-measure`.
  ...
  2. **Nonamenable `H`.** Then `a^2 = b^2 = 1`, `ab = 1`, `b = a^2` and `a = b^2` all fail.
     ...
     - **`b^2 = 1`:** Move B. Its conflicts force `H` abelian or virtually cyclic.
     - In each case the table shows equal outputs at every touched window, and the untouched windows read
       identical configurations.
  - **Where it could fail.**
    - A pair of window sites that can coincide but was left out of the tables in Lemmas 1–3.
    - A relation among `a, b` that breaks a table while `H` stays nonamenable.

    The verifier re-derives each table.
