...
#   MERGED     GK_ACK="path=<origin blob> ..." says you merged onto that exact blob
...
guard() {
  local T="$1" bad=0 p b src ob last
  while IFS=$'\t' read -r p b src; do
    if git cat-file -e "${T}:${p}" 2>/dev/null; then
      ...
      if [ "$ob" = "$b" ]; then echo "  SAME      $p"
      elif [ -n "$last" ] && [ "$ob" = "$last" ]; then echo "  OVERWRITE $p (own last push)"
      elif printf ' %s ' "${GK_ACK:-}" | grep -qF " ${p}=${ob} "; then echo "  MERGED    $p (onto origin blob ${ob:0:10})"
      else
...
lint() {
  local N="$1" bad=0 p b src base F id kd key want r rk
  while IFS=$'\t' read -r p b src; do
...
for attempt in $(seq 1 40); do
  ...
  if push_new "$NEW" > "$WORK/push.log" 2>&1; then
    while IFS=$'\t' read -r p b src; do
      printf '%s\t%s\n' "$p" "$b" >> "$REC"
      if [ "$src" = "$R/$p" ] && [ "$(git hash-object -- "$p" 2>/dev/null)" = "$b" ]; then
  ...
  if grep -qiE "denied|403|authentication failed|could not read Username" "$WORK/push.log"; then
