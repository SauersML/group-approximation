#!/usr/bin/env bash
# gqland-lean.sh <lane> <msgfile> <path> [<path> ...]  (gqland.sh + Lean/Palomar paths; build GREEN on MSI via scripts/remote-build.sh BEFORE landing Lean)
#
...
for p in "$@"; do
  case "$p" in
    ...
    GroupApproximation.lean|GroupApproximation/*.lean|Palomar/*|scripts/PalomarBooneHigman*.lean|scripts/check_palomar_submission.py|formalization.yaml|lakefile.lean|lakefile.toml) ;;
...
for p in "$@"; do
  case "$p" in research/artifacts/*|GroupApproximation*|Palomar/*|scripts/*|formalization.yaml|lakefile.lean|lakefile.toml) continue;; esac
