#!/usr/bin/env python3
"""Complete search over the fill of binary_hier.py with conflict-directed backjumping (lane bh-free-18).
...
def search(assign):
    ...
    if STATS["nodes"] % 500 == 0:
        print("  nodes=%d jumps=%d depth=%d %.0fs" % (STATS["nodes"], STATS["jumps"], len(assign), time.time() - T0), flush=True)
    M = L.make(assign)
    r = run_checks(M)
    if r[0] == "ok":
    ...
    if not e.inv:
        ...
        for key in L.FILL_KEYS[R]:
            ...
            if fact not in ng:
                ...
                return ng
            conflict |= ng - {fact}
    return None if found else frozenset(conflict)
