#!/usr/bin/env python3
"""(Harvest copy of bh_sat.py: no spot-check, dumps the learned clauses to clauses3.pkl.) Lazy SAT (counterexample-guided) decision of the free rules of binary_hier.py (lane bh-free-18).
...
def main(mode, nproc, budget):
    ...
    import pickle
    pickle.dump(list(CL), open("clauses3.pkl", "wb"))
    print("dumped %d clauses to clauses3.pkl" % len(CL), flush=True)
