...
- CREATIVITY (USER 09-18 ~09:40): "instead of looking for existing mechanisms, objects, etc., they can and should get highly creative and do their own unique math, constructions, strategies, and creative inputs, not just re-using what is known to exist elsewhere." Invent your own groups, actions, hosts, rings, invariants and proof strategies. Use the literature and main to avoid duplication and to check priority and correctness, but not as the only source of ideas. A new construction that you design and prove beats a survey of why known ones fail.

## PALOMAR BH MEGASUBMISSION (USER 09-18 ~09:50)
- User decisions: ONE combined Comparator config for all BH results; switch formalization.yaml to it NOW (marked WIP); integrate properly with the other session's metabelian Lean chain (GroupApproximation/BooneHigman/**, bh-met-* lanes) — editing their files is allowed, but only via the clobber-guarded lander with a fresh GQ_BASE copy, and never while one of their commits to the same file is minutes old; scope = ONLY open problems WE solved that others have not (no literature-only results); include lean-eval's higman_infinite_simple (via the repo's closed Q₂ theorem).
- Plan: research/artifacts/gq-bh-palomar-plan.md (29f374a9b); Lean ecosystem survey: research/artifacts/gq-bh-bh-lit-lean-ecosystem.md (f9c8e7c95).
- Lean lander: `$GQ/gqland-lean.sh <lane> <msgfile> <paths>` (same guards as gqland.sh; also allows GroupApproximation/**, GroupApproximation.lean, Palomar/**, scripts/PalomarBooneHigman*.lean, scripts/check_palomar_submission.py, formalization.yaml, lakefile.lean). Land Lean ONLY after a GREEN trusted MSI build of the affected modules via scripts/remote-build.sh (fleet mutex); an unmutexed or stale build is not evidence. Read memory: msi-build-workflow, fleet-mutex-only-remote-build, root-docstring-import-trap, cyclecheck-preflight, stale-artifact-trap, memory-index-lean-gotchas, submission-metadata-one-config-american-no-md-refs, lix-palomar-submission-2026-09-10, formalization-yaml-has-six-sections-we-skipped.
