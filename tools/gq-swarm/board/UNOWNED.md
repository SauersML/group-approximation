...
- Cornulier–Guyot–Pitsch Question 4 — owner: 
...
- BBCMP Q1.16 mapping class groups continuous automatic? — owner: 
...
- Kourovka 21.73/21.74 (conjugacy and element-property decidability in CT(Z)): now a question about full groups of one-vertex k-graphs (Li Garside tools) — owner:
...
- Kourovka 21.45 / 21.46 (f.p. simple group needing ≥3 generators / of finite cd > 2) — owner:
## Added by bh-openq-papers (09-18; source list research/artifacts/gq-bh-openq-papers-list.md)
- LISW 2510.01952 Q1.10: f.g. H ≤ GL_n(C) as subgroup + quasi-retract of a simple group, same finiteness (`fg-complex-linear-groups-are-quasi-retracts-of-simple-groups`) — owner: 
- 2609.01868: Z wr_X PSL_2(Z[1/2]) into an fp self-similar group (`z-wr-psl2-z-half-embeds-in-fp-self-similar-group`) — owner: 
- Skipper–Zaremsky 1709.06524: V_d(G) F_∞ for all f.g. contracting G (`contracting-rover-nekrashevych-groups-are-f-infinity`) — owner: 
- Lodha 1710.06220: fp infinite simple group not 2-generated (`fp-infinite-simple-group-not-two-generated`) — owner: 
- Matui 1602.00383: D([[G_φ]]) fp for free minimal Z^N actions? (`free-minimal-zn-derived-full-groups-are-never-fp`) — owner: 
- Zaremsky 2201.00711: hyperbolic groups quasi-isometrically into fp simple groups of type F_∞ (`hyperbolic-groups-quasi-isometrically-embed-in-fp-simple-groups`) — owner: 
...
- (bh-openq-lists, 054ee2eb8) AimPL Groups of dynamical origin 3.1(c): fp simple group with commutator width >= 2 (`fp-simple-group-with-commutator-width-at-least-two`) — owner: 
- (bh-openq-lists, 054ee2eb8) MO 491978 (Minasyan): Neumann's group into an fp RF group (`neumann-alternating-product-group-embeds-in-fp-rf-group`) — owner: 
- (bh-openq-lists, 054ee2eb8) MO 412219: E(G) = (G*Z)/J_1(G) recursively presented for fp G? (`e-of-finitely-presented-group-is-recursively-presented`) — owner: 
