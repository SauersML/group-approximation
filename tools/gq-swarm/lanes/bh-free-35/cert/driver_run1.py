...
def relations(sft, W):
    ...
    seen = set()  # locally legal but non-extendable patterns on dominoes and on B_1(z) cap W
    for z in W:
...
def case(sft, vs, detour, deg, tmax, label):
    global NCUR
    NCUR = sft.n
    for v in vs:
...
def main():
    ...
    case(led, V6, 2, 4, 60, 'led')                      # calibration: expect all certified
    case(ledk, [(2, 1), (3, 0)], 2, 4, 60, 'ledk')      # planted rigid with local coordinates: expect certified
    case(full, [(2, 1), (3, 0)], 0, 4, 40, 'full')      # negative: expect none certified
    case(colp, [(2, 1), (3, 0)], 0, 5, 60, 'colp')      # (2,1) certifiable by marginals; (3,0) has a wall: must be NO
    case(labbe, [(2, 1), (1, 2), (2, -1), (1, -2)], 0, 4, 75, 'labbe')  # add-on data, rectangle windows
    log('done')
