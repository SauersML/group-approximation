...
/-!
# Brown's formulation: `L(G)` embeds in `𝓡^ω` for a countable sofic group
...
This is the sixth step of this lane's route, after `Full/BrownROmega/SpanGraph`.

Take a sofic approximation `σₙ` whose fixed-point counts tend to `[g = 1]`. It gives a unitary
representation `g ↦ [σₙ(g)]` of `G` in the matrix ultraproduct `∏ M_{dₙ}/ω`, and the ultratrace of
`[σₙ(g)]` is `[g = 1]` (`ShulmanTrace.soficUnitaryHomUltrafilter`). The matrix ultraproduct maps
into `𝓡^ω` preserving the trace (`Hyperfinite.matrixUltraproductToHyperfiniteUltrapower`). The vector
state of `δ₁` on `λ(g)` is also `[g = 1]`, so the pairs `(λ(g), [σₙ(g)])` are trace compatible.
Their first coordinates are `‖·δ₁‖`-dense by the point-mass expansion, and `δ₁` is separating.
...
local instance sk06GroupModelDecEq : DecidableEq G := Classical.decEq G
...
local instance sk06GroupModelDecEqSofic : DecidableEq G := Classical.decEq G

/-- The sofic representation `g ↦ [σₙ(g)]`, landed in `𝓡^ω`. -/
...
/-- The sofic representation in `𝓡^ω` sends inverses to adjoints. -/
...
/-- The ultratrace of `[σₙ(g)]` in `𝓡^ω` is `[g = 1]`. -/
...
/-- Products of generator pairs are generator pairs. -/
theorem groupPair_mul_mem (g h : G) :
    groupPair S ω hω g * groupPair S ω hω h
      ∈ Submodule.span ℂ (Set.range (groupPair S ω hω)) := by
...
/-- Adjoints of generator pairs are generator pairs. -/
...
/-- The generator pairs are trace compatible. -/
...
/-- **`L(G)` embeds in `𝓡^ω`** ("Brown's formulation"). For a countable sofic group `G` and a free
ultrafilter `ω`, there is an injective `⋆`-homomorphism from `L(G)` into `𝓡^ω` that carries the
canonical trace `T ↦ ⟪δ₁, T δ₁⟫` of `L(G)` to the ultratrace. -/
