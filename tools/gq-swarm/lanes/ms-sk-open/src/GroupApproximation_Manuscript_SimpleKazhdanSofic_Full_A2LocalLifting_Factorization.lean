...
/-!
# Factorization through the local lifting property (table row A2)

`simple_kazhdan_sofic_group.tex`, Introduction, tex l.54–59 (table row A2):
...
Ozawa, *About the QWEP conjecture*, Internat. J. Math. **15** (2004), preprint p. 24: for a
hyperlinear `Γ`, "because of the LLP of `C*𝔽_∞`, the trace `τ_Γ` … is liftable".  The same argument
with `C⋆(G)` in place of `C⋆𝔽_∞` gives:

* `localLiftingFactorization`: if `C⋆(G)` has the local lifting property and `G` has a
  trace-preserving matrix model, then `G` has the factorization property (at its trace side,
  `NinetyNineProblems.HasFactorizationProperty`).
* `not_hasLocalLiftingProperty_of_not_hasFactorizationProperty`: a countable hyperlinear group
  without the factorization property has a full C⋆-algebra without the LLP.

Route.
1. `exists_modelLift`: the model unitaries give `π : C⋆(G) → ∏ M_{k(n)} / J₂`, unital and
   completely positive.
2. The LLP lifts `π` on the operator system `generatorSystem (generatorSet F)`.
3. Each coordinate of the lift, followed into `B(ℓ²)`, is completely positive there, and Arveson's
   theorem (`A2OperatorSystems.exists_ucp_extension_of_system'`) extends it to `C⋆(G)`.
4. On generators the extensions are the coordinates of the lift, so their multiplicative defects
   and trace errors are Hilbert–Schmidt null (`exists_approximateModel`).
5. A diagonal choice and the dense-generator upgrade
   (`Quasidiagonal.amenableTraceModel_of_dense_generators`) give an amenable trace model.

This is a fork, into lane `A2LocalLifting`, of the second half of the never-wired module
`Analysis/LocalLiftingFactorization.lean` (left untouched), with its Arveson binder discharged.
-/
...
open Filter CStarExactness Quasidiagonal TracialUltraproduct ShulmanTrace
open scoped Matrix.Norms.L2Operator

noncomputable section

/-- The bounded matrix product is a C⋆-algebra, assembled from its existing normed, star and
complete structure. -/
local instance modelBoundedSequenceCStarAlgebraForA2
