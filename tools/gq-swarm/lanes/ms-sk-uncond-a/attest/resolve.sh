...
while read -r full; do last=${full##*.}
  ...
  for c in $cands; do f=${c%%:*}; ln=${c##*:}
    ns=$(git show "refs/remotes/origin/main:$f" | awk -v L=$ln 'NR>=L{exit} /^namespace /{st[++d]=$2} /^end /{ if(d>0 && $2==st[d]) d--} END{s=""; for(i=1;i<=d;i++) s=s (s==""?"":".") st[i]; print s}')
    dn=$(git show "refs/remotes/origin/main:$f" | sed -n "${ln}p" | awk '{for(i=1;i<=NF;i++) if ($i ~ /^(theorem|lemma|def|abbrev|instance|structure|class|inductive)$/) {print $(i+1); exit}}')
    cand="${ns:+$ns.}$dn"
