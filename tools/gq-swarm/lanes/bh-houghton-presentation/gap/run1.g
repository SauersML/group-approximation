# Lane bh-houghton-presentation, run 1: calibration + bounded forward/backward closure search.
# c(g) := nu*g*nu^-1 in GAP's right-action convention, i.e. n -> g(n+1)-1.
LoadPackage("rcwa");
out := OutputTextFile("/projects/standard/hsiehph/sauer354/gqsrc/bh-houghton-presentation/run1.out", false);
...
Search := function(name, g, L, maxel, maxmod)
...
for sd in seeds do
  for L in [1,2,3] do
    if Runtime() - t0 < 480000 then Search(sd[1], sd[2], L, 4000, 2^10); fi;
  od;
od;
