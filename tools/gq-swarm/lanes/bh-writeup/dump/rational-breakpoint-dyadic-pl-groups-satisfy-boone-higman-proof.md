...
3. **Periodic points.** Write `s = r/(2^a b)` with `b > 1` odd. Its binary expansion is eventually
   ...
   the expansion. So every breakpoint of an element of a finitely generated `H` lies in finitely
   many tail classes, and `Π` is chosen to be their set of periods.
