...
kind: claim
refuted_by:
  - leavitt-resolvent-derived-units-mod-centre-not-fp-simple
...
**REFUTED (2026-09-17)** by `leavitt-resolvent-derived-units-mod-centre-not-fp-simple`:
`Q^x` lies in `[D,D]` and in `Z(D)`, so `D/Z(D)` is never a finitely presented perfect
group (Attempt 4). The refuted statement: let `R_L` be the ring of `leavitt-resolvent-ring-is-fp-simple-of-char-zero`,
...
4. **Refuted, unconditionally (2026-09-17, lane gq-leavitt-units).**
   `leavitt-central-units-lie-in-derived-normal-subgroups` removes every unproved input of
   Attempt 3. For `λ ∈ Q^x`:
   - the cone scalars `e_a(λ) = λ s_a t_a + (1 - s_a t_a)` are all `V`-conjugate;
   - `V = [V,V] ≤ [D,D]`, and `[D,D] ⊴ R_L^x`, so conjugation by `V` fixes cosets of
     `[D,D]`;
   - the codes `{1,2}` and `{1,21,22}` give `λ ≡ c^2 ≡ c^3` modulo `[D,D]`, hence
     `λ ∈ [D,D]`.

   So `Q^x ≤ [D,D] ∩ Z(D)`, with no `K_1`, AGP or exact-sequence input. If `D/Z(D)` were
   finitely presented and simple, it would be perfect. Then `[D,D]` would be perfect with
   `[D,D]/([D,D] ∩ Z(D)) ≅ D/Z(D)`, and the Schur multiplier bound would make
   `[D,D] ∩ Z(D) ⊇ Q^x` finitely generated. Perfectness of `D` is not needed. See
   `leavitt-resolvent-derived-units-mod-centre-not-fp-simple`. The same theorem applies to
   every normal subgroup of `R_L^x` that contains `V`, taken modulo any central subgroup
   containing `Q^x`. The weaker target of Attempt 2, a subquotient that is not of this
   form, is untouched.
