...
5. **Finite presentation: answered, no** (2026-09-18,
   `reid-higman-thompson-variant-is-not-finitely-presented`). Neither `G` nor `G'` is finitely
   presented. The original discussion follows. Is `G` (equivalently `G'`) finitely presented? Li's Garside finiteness
   ...
   - That heuristic is correct: the far-commutation relations are exactly those that fail
     after wrapping `Z` onto `Z/n` (`stabilizer-engines-force-isolated-vertex-groups`).
