...
**REFUTED** (2026-09-18) at `B = Z` by `stabilizer-engines-force-isolated-vertex-groups`: a
finitely presented engine forces `B` to be isolated (finitely presented and finitely
discriminable), and `Z` is not finitely discriminable. So Reid's group is not finitely presented
(`reid-higman-thompson-variant-is-not-finitely-presented`). The corrected conjecture is
`isolated-vertex-groups-have-fp-stabilizer-engines`. The text below is the original statement.

Originally OPEN (stated by bh-emitter-b, 2026-09-18). Notation as in
...
2. **It must pass Reid's group.** The first case is `B = Z`. It fails
