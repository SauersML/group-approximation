...
consume() { # $1 request dir
  ...
  # DEFERRED is never a verdict. A DEFERRED request whose batch did run (timeout, lost contact) still has a partial log:
  # read it for modules with their own error lines only, and never for GREEN.
  ...
  if [ $partial = yes ]; then complete=no
    echo "$RAW" | sed -nE 's/^error: (GroupApproximation\/[^:]+\.lean):.*/\1/p' | sort -u > $RS.own; mv $RS.own $RS
    say "partial read of DEFERRED $(basename $r) (job $jid): red only from own error lines, no green"
  fi
  OTHER=$W/other.$jid.$n
  ...
  while read -r p; do
    ...
    case "$st:$v" in
      ...
      RESOLVED:RED|RESOLVED:BLOCKED) say "  !! RESOLVED $m is $v again (job $jid)"
        echo "sed -i '' -E '/^RESOLVED [^ ]+ $m /s/\$/ (RED AGAIN job $jid)/' $B" >> $BOARD;;
    esac
...
rootclo() { # $1 sha -> root closure paths
  local e; e=$(edges $1)
...
scan_landings() { # keep the graph current with lanes' (often unprobed) Lean landings on main
  local SCAN=$W/scan-sha old new EDITP="" BOARD=$W/scan-edits.sh st c sub own id
  new=$(git rev-parse $M); [ -s $SCAN ] || echo e6c93a1fe4 > $SCAN; old=$(cat $SCAN)
  [ "$(git rev-parse $old)" = "$new" ] && return 0
  git merge-base --is-ancestor $old $new || { say "scan: $old is not an ancestor of main; restarting the scan at main"; echo $new > $SCAN; return 0; }
  git diff --name-status $old $new -- 'GroupApproximation/*.lean' > $W/scan.tmp
  [ -s $W/scan.tmp ] || { echo $new > $SCAN; return 0; }
  rootclo $new > $W/root-closure.txt; : > $BOARD
  while read -r s p; do
    m=$(p2m "$p"); c=$(git log -1 --format='%h' $old..$new -- "$p"); sub=$(git log -1 --format='%s' $old..$new -- "$p" | tr -d "'|" | cut -c1-90)
    st=$(awk -v p="$p" -v m="$m" '($1=="PENDING" && $2==p) || (($1=="OPEN"||$1=="RESOLVED") && $3==m) || ($1=="GREEN" && $2==p){print $1; exit}' $B)
    case "$st:$s" in
      OPEN:D) say "scan: OPEN module $m deleted at $c"; echo "sed -i '' '/^OPEN [^ ]* $m /s|\$| (module deleted at $c)|' $B" >> $BOARD;;
      OPEN:*)
        id=$(id_of "$m"); git show $M:research/$id.md > $W/research/$id.md 2>/dev/null || continue
        ...
        git show $M:research/$id.md | cmp -s - $W/research/$id.md || EDITP="$EDITP research/$id.md"
        echo "sed -i '' -E '/^OPEN $id /s/ \(repair landed [0-9a-f]+, unprobed\)//; /^OPEN $id /s/\$/ (repair landed $c, unprobed)/' $B" >> $BOARD
        say "scan: unprobed repair of OPEN $m at $c";;
      GREEN:*) own=$(awk -v p="$p" '$1=="GREEN" && $2==p{print $3}' $B)
        echo "sed -i '' 's|^GREEN $p .*|PENDING $p $own|' $B" >> $BOARD; say "scan: GREEN $m changed at $c -> PENDING";;
      RESOLVED:*) echo "sed -i '' -E '/^RESOLVED [^ ]+ $m /s/ \(changed [0-9a-f]+, unverified\)//; /^RESOLVED [^ ]+ $m /s/\$/ (changed $c, unverified)/' $B" >> $BOARD
        echo "$m" >> $W/recheck.txt; say "scan: RESOLVED $m changed at $c; queued for recheck";;
      PENDING:*) ;;
      :D) ;;
      :*) grep -qxF "$p" $W/root-closure.txt && { say "scan: rooted $m changed at $c (root build gate covers it)"; continue; }
        own=$(owner "$p"); echo "printf '%s\n' 'PENDING $p $own' >> $B" >> $BOARD; say "scan: new unwired Lean $m at $c -> PENDING ($own)";;
    esac
  done < $W/scan.tmp
  [ "${DRY:-0}" = 1 ] && { say "DRY scan: edit=$EDITP"; cat $BOARD >> $LOG; return 0; }
  if [ -n "$EDITP" ]; then
    printf 'Cairn fix graph: note unprobed repairs landed on main (%s)\n\nEach OPEN fix node whose module changed on main since the last scan records the landing commit. It stays OPEN until a trusted probe builds it green.\n' "${new:0:10}" > $GQ/msgs/fix-graph-lead-scan.txt
    # shellcheck disable=SC2086
    out=$(GQ_BASE=$new GQ_BASE_PATHS="$EDITP" timeout 900 bash $GQ/gqland.sh fix-graph-lead $GQ/msgs/fix-graph-lead-scan.txt $EDITP 2>&1 | grep -E 'LANDED|REFUSED|ABORT|error' | head -3)
    say "scan land: $out"; echo "$out" | grep -q LANDED || return 1
  fi
  bash $BOARD; echo $new > $SCAN; sort -u $W/recheck.txt -o $W/recheck.txt 2>/dev/null; return 0
}
...
for tick in $(seq 1 72); do
  ...
  if [ -e $GQ/state/SLURM-BLOCKED ]; then
    [ "${blocked:-no}" = yes ] || say "SLURM-BLOCKED present: not submitting probes (still reading verdicts and landings)"; blocked=yes; sleep 600; continue
  elif [ "${blocked:-no}" = yes ]; then say "SLURM-BLOCKED gone: resuming probes now"; blocked=no; echo 0 > $LASTPROBE; fi
  ...
  if [ -z "$inflight" ] && [ $((now - $(cat $LASTPROBE))) -ge 2700 ]; then
    ...
    [ -s $W/recheck.txt ] && { cat $W/recheck.txt >> $W/want.txt; : > $W/recheck.txt; }
    sort -u $W/want.txt -o $W/want.txt
