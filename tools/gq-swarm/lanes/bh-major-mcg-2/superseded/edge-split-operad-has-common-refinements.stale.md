...
3. **Correction to Attempt 1** (lane bh-major-mcg-2, 2026-09-18).
   - Attempt 1 concluded that LD_atom is the only open input. That route is dead:
     LD_atom is refuted by the ten-piece dissection of
     `edge-split-respecting-a-split-plane-need-not-refine-it`.
   - **What stands.** This statement is equivalent, in every rank, to WLD
     (`edge-split-dissections-refine-into-any-first-split`), by the route
     `edge-split-ore-via-first-split-refinements`.
   - **The header.** The header bullet "for `m = 2` it reduces … to LD_atom" is
