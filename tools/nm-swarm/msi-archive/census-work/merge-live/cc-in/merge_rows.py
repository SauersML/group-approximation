...
Keys.  A 12-hex sentence key of the current tex, or LINE:<n>, where n numbers the tex before 45483f699 (a8cc132c8,
exported by census_merge.sh as $ROWS_TEX) and census_linemap.py carries n to the current tex by a line diff before
anything below reads it; the merge exits nonzero when $ROWS_TEX is unreadable.  `sentence_census.py` records only the
...
same bytes; else a formalized/definition row becomes partial.  Since 2026-09-13 (COMMON_RULES: DONE is a closed,
ROOT-REACHABLE carrier) a compiled carrier outside the root import closure also makes the row partial.  Existing
map rows no lane row replaces get the same root-reachability check, with a restorable suffix: the next merge
restores the grade once every carrier module is in the closure.
"""
import difflib
import hashlib
import json
...
overrides, drops, keeplatest, supersede = {}, {}, {}, {}
superseded_seen = set()
if os.path.exists(overrides_path):
    for line in open(overrides_path, encoding="utf-8"):
        p = line.rstrip("\n").split("\t")
        if len(p) >= 4 and p[2] == "DROP" and not line.startswith("#"):
            prev = drops.get((p[0], p[1]), (set(), ""))
            drops[(p[0], p[1])] = (prev[0] | set(p[3].split()), (prev[1] + " " + (p[4] if len(p) > 4 else "")).strip())
        # Census successor 09-13: lane<TAB>given key<TAB>KEEPLATEST<TAB>reason keeps only the lane's last row at that
        # given key, for rows their writer declared superseded without writing SUPERSEDES in the note.
        elif len(p) >= 3 and p[2] == "KEEPLATEST" and not line.startswith("#"):
            keeplatest[(p[0], p[1])] = p[3] if len(p) > 3 else ""
        # Census successor 09-13 23:20: lane<TAB>given key<TAB>SUPERSEDE<TAB>declarations<TAB>reason replaces the lane's
        # rows at that given key by one formalized row naming only the given closed endpoints.  It is for an owner
        # supersede that a live lane proposed and main ordered while the owner lane is down.  Every named declaration still
        # has to be defined, compiled and root-reachable, and the verifier still classifies it.
        elif len(p) >= 4 and p[2] == "SUPERSEDE" and not line.startswith("#"):
            supersede[(p[0], p[1])] = (p[3].split(), p[4] if len(p) > 4 else "")
        elif len(p) >= 3 and not line.startswith("#"):
...
def rooted(rp: str) -> bool:
    """Is the module of `rp` in origin's root import closure?  COMMON_RULES 0 (09-13): DONE is root-reachable."""
    return (rp[:-5].replace("/", ".") if rp.endswith(".lean") else rp) in closure
...
# LINE:<n> numbers the tex before 45483f699 (census_merge.sh exports it as ROWS_TEX), and census_linemap carries n to
# the current tex.  A merge that cannot read that tex must not resolve LINE keys against moved lines.
from census_linemap import load as load_linemap  # noqa: E402
if not os.path.isfile(os.environ.get("ROWS_TEX", "")):
    sys.exit("merge_rows: ROWS_TEX (the tex the LINE:<n> row keys number) is not set or not a file")
linemap = load_linemap(os.environ["ROWS_TEX"], str(ROOT / "non_mf_groups_exist.tex"))
# Census successor, 2026-09-13 18:30: the lanes launched by session nonsofic-existence-49 (ct-*, ms-*, w1-*) number
# LINE:<n> at the current tex (origin 68481e4d7 and later; the tex is unchanged since 73a84cd9c), so their keys are
# read without the line map.  Merge 0913-171131 carried ct-rank-budget LINE:1700 to 2185 and skipped nine ct rows.
CURRENT_TEX_LANES = re.compile(os.environ.get("CURRENT_TEX_LANES", r"^(ct|ms|w1)-"))
...
def resolve(lane: str, given: str, note: str):
    ...
    if m:
        k = m.group(1)
    # Census successor 09-13: an override may re-key a row to several sentence keys (space-separated), when the row's
    # note quotes each of those sentences.
    if re.fullmatch(r"[0-9a-f]{12}( [0-9a-f]{12})+", k):
        ks = k.split()
        return (ks, how0 + "keys") if all(x in by_key for x in ks) else ([], how0 + "STALE-KEY")
    if re.fullmatch(r"[0-9a-f]{12}", k):
        return ([k], how0 + "key") if k in by_key else ([], how0 + "STALE-KEY")
    m = re.fullmatch(r"LINE:(\d+)", k)
    if not m:
        return [], how0 + "BAD-KEY"
    n = int(m.group(1)) if CURRENT_TEX_LANES.match(lane) else linemap.to_new(int(m.group(1)))
    if n != int(m.group(1)):
        how0 += "line %s->%d " % (m.group(1), n)
...
for rf in row_files:
    ...
    for line in open(rf, encoding="utf-8"):
        ...
        if re.search(r"@(SHA|TAG)@", p[3]):
            ...
            continue
        sup = supersede.get((lane, p[0].strip()))
        if sup:
            if (lane, p[0].strip()) in superseded_seen:
                report.append(["SKIP", lane, p[0], "-", "-", "-", p[1], "census override SUPERSEDE", "a later row of the "
                               "lane at this key, already replaced", "", clean(p[3])[:220]])
                continue
            superseded_seen.add((lane, p[0].strip()))
            p[1], p[2] = "formalized", " ".join(sup[0])
            p[3] = "[census override SUPERSEDE: this lane's rows at %s are replaced by %s (%s)] superseded row note: %s" % (
                p[0].strip(), " ".join(sup[0]), sup[1], p[3])
        dr, drop_note = drops.get((lane, p[0].strip())), ""
        if dr and dr[0] & set(p[2].split()):
            gone = [d for d in p[2].split() if d in dr[0]]
            kept = [d for d in p[2].split() if d not in dr[0]]
            if not kept:
                report.append(["SKIP", lane, p[0], "-", "-", "-", p[1], "census override DROP", "every declaration "
                               "retired: %s (%s)" % (" ".join(gone), dr[1]), "", clean(p[3])[:220]])
                continue
            p[2] = " ".join(kept)
            drop_note = " [census override: dropped %s (%s)]" % (" ".join(gone), dr[1])
        ...
        for key in keys:
            ...
            elif (lane, p[0].strip()) in keeplatest:
                g[:] = [x for x in g if not (x[0] == lane and x[1][0].strip() == p[0].strip())]
...
for key, g in groups.items():
    ...
    unrooted = sorted({relpath(index[d]) for d in decls if compiled(relpath(index[d])) and not rooted(relpath(index[d]))})
    if unrooted:
        reasons.append("carrier not root-reachable at origin %s (compiled, outside the GroupApproximation import "
                       "closure): %s" % (sha9, " ".join(x[:-5].replace("/", ".") for x in unrooted)))
    new_status = status
    if (missing or notcomp or unrooted) and status in GRADED:
        new_status = "partial"
        downgrades += 1
    newnote = "%s [census merge %s]" % (" | ".join(notes), sha9)
    if missing or notcomp or unrooted:
...
audit = [["key", "start_line", "old_status", "new_status", "unrooted carrier modules"]]
existing_downgrades = 0
restored = 0
DOWN_SUFFIX = re.compile(r" \[census merge [0-9a-f]+: DOWNGRADED from (formalized|definition): carrier not "
                         r"(?:compiled|root-reachable) at origin \([^)]*\): [^\]]*\]$")
for key, i in map_rows().items():
    ...
    if cols[1] == "partial" and m:
        still = sorted({relpath(index[d]) for d in cols[2].split() if d in index and not rooted(relpath(index[d]))})
        if not still and all(d in index for d in cols[2].split()):
            audit.append([key, str(start_line[key]), "partial", m.group(1), "RESTORED: carriers now root-reachable"])
            cols[1] = m.group(1)
    ...
    if cols[1] not in GRADED:
        continue
    badm = sorted({relpath(index[d]) for d in cols[2].split() if d in index and not rooted(relpath(index[d]))})
    if badm:
        mods = " ".join(x[:-5].replace("/", ".") for x in badm)
        how = "not compiled" if any(not compiled(x) for x in badm) else "compiled"
        audit.append([key, str(start_line[key]), cols[1], "partial", mods])
        cols[3] = "%s [census merge %s: DOWNGRADED from %s: carrier not root-reachable at origin (%s, outside the " \
                  "GroupApproximation import closure): %s]" % (cols[3], sha9, cols[1], how, mods)
        cols[1] = "partial"
        map_lines[i] = "\t".join(cols)
