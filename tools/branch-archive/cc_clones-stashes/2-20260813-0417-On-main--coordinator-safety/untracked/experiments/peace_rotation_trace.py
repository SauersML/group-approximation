...
# PEACE `simp2`: erase defining-relator items only.
SIMP2 = reduce_word("".join(word for kind, word in ITEMS if kind != "relator"))
...
expected_inner = u + v + u + inverse(v) + inverse(u) + v + inverse(u) \
    + inverse(v) + inverse(u) + inverse(v) + u + v
expected = reduce_word(expected_inner * 2)
print(f"expected={expected!r}")
assert SIMP1 == "y"
assert SIMP2 == expected
