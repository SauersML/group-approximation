...
noncomputable local instance (priority := 2000) fastCoronaNormedRing :
    NormedRing (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraNormedRing (fun n ↦ X n)

noncomputable local instance (priority := 2000) fastCoronaStarRing :
    StarRing (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraStarRing (fun n ↦ X n)

-- `NormMatrixCoronaAlgebra` is a reducible ideal-quotient abbreviation.
-- Pin its intended structures locally so elaboration does not search through
-- the raw quotient implementation at every occurrence of `unitary`.
local instance coronaNormedRing :
    NormedRing (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraNormedRing (fun n ↦ X n)

local instance coronaStar :
    Star (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraStar (fun n ↦ X n)

local instance coronaInvolutiveStar :
    InvolutiveStar (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraInvolutiveStar (fun n ↦ X n)

local instance coronaStarRing :
    StarRing (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraStarRing (fun n ↦ X n)

local instance coronaNormedStarGroup :
    NormedStarGroup (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraNormedStarGroup (fun n ↦ X n)

local instance coronaCStarRing :
    CStarRing (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
  normMatrixCoronaAlgebraCStarRing (fun n ↦ X n)
...
def unitarySequenceToCorona :
    (∀ n, Matrix.unitaryGroup (X n) ℂ) →*
      unitary (NormMatrixCoronaAlgebra (fun n ↦ X n)) where
  toFun u := ⟨normMatrixCoronaMk (fun n ↦ X n) (unitarySequenceBounded X u), by
    constructor
    · rw [normMatrixCorona_star_mk]
      change normMatrixCoronaMk (fun n ↦ X n)
          (star (unitarySequenceBounded X u) * unitarySequenceBounded X u) = 1
      congr 1
      ext n i j
      simpa using congrArg (fun A : Matrix (X n) (X n) ℂ ↦ A i j)
        (Unitary.star_mul_self_of_mem (u n).property)
    · rw [normMatrixCorona_star_mk]
      change normMatrixCoronaMk (fun n ↦ X n)
          (unitarySequenceBounded X u * star (unitarySequenceBounded X u)) = 1
      congr 1
      ext n i j
      simpa using congrArg (fun A : Matrix (X n) (X n) ℂ ↦ A i j)
...
theorem unitarySequenceToCorona_eq_one_iff
  ...
  change normMatrixCoronaMk (fun n ↦ X n) (unitarySequenceBounded X u) = 1 ↔ _
  rw [← sub_eq_zero]
  change normMatrixCoronaMk (fun n ↦ X n) (unitarySequenceBounded X u - 1) = 0 ↔ _
  rw [normMatrixCoronaMk_eq_zero_iff]
  rfl
...
def unitaryCoronaToCStarCoronaUnitary :
    NormMatrixCoronaUnitary X →*
      unitary (NormMatrixCoronaAlgebra (fun n ↦ X n)) :=
...
theorem unitaryCoronaToCStarCoronaUnitary_injective :
  ...
  | _ u =>
    ...
    exact (QuotientGroup.eq_one_iff u).symm
...
theorem unitaryCoronaToCStarCoronaUnitary_surjective :
  ...
  obtain ⟨a, rfl⟩ := QuotientAddGroup.mk'_surjective
    (c0MatrixSequenceIdeal (fun n ↦ X n)) x
  have hgramZero :
      normMatrixCoronaMk (fun n ↦ X n) (star a * a - 1) = 0 := by
    rw [map_sub, map_mul, normMatrixCorona_star_mk, map_one]
    exact sub_eq_zero.mpr hx.1
  have hgram : IsC0MatrixSequence (fun n ↦ X n) (star a * a - 1) :=
    (normMatrixCoronaMk_eq_zero_iff (fun n ↦ X n) _).mp hgramZero
  have hgramTendsto :
      Tendsto (fun n ↦
        ‖KazhdanCornerMatrices.cornerGram (a n) - 1‖) cofinite (nhds 0) := by
    simpa [IsC0MatrixSequence, KazhdanCornerMatrices.cornerGram,
      lp.star_apply] using hgram
  ...
  have hdiff : IsC0MatrixSequence (fun n ↦ X n)
    ...
    · simpa only [mul_zero] using
        tendsto_const_nhds.mul (tendsto_const_nhds.const_mul 2 hgramTendsto)
  refine ⟨QuotientGroup.mk u, ?_⟩
  rw [unitaryCoronaToCStarCoronaUnitary_mk]
  apply Subtype.ext
  change normMatrixCoronaMk (fun n ↦ X n) (unitarySequenceBounded X u) =
    normMatrixCoronaMk (fun n ↦ X n) a
  rw [← sub_eq_zero, ← map_sub]
  exact (normMatrixCoronaMk_eq_zero_iff (fun n ↦ X n) _).mpr hdiff
