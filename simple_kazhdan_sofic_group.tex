...
hyperbolic Kazhdan groups~\cite{Gromov87} and as Kac--Moody lattices, some
...
subgroup of $G_X$. Simplicity of $G_X$ also follows from Stepanov's
...
count on one cylinder. Our proof is direct: a nontrivial normal subgroup
contains a nontrivial commutator lying in a copy of the finite simple
group $\GL_d(\F_2)$ over a clopen tower, so it contains this group and
with it an elementary matrix. For derived topological full groups, Matui
showed in the same way that a nontrivial normal subgroup meets a simple
union of alternating groups on towers~\cite[Lemma~3.4 and
Theorem~4.9]{Matui}.
...
finite simple groups $\SL_{nN_k}(\F_2)$, whose Cayley graphs form a
family of expanders.
\end{theorem}
...
$N_k\to\infty$. Since $\EL_3(F)$ has property~\textup{(T)}~\cite[Theorem~1.1]{EJZ},
its finite quotients form a family of expanders~\cite{Margulis,Kassabov}.\qed

\subsection*{Brown's formulation}
LEF groups are sofic~\cite[Example~4.5]{Pestov}, and sofic groups are
