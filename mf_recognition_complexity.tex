...
and asymptotically separate nonidentity elements in operator norm.  Finite
semialgebraic certificates characterize the MF condition.  We construct a
...
computable function has infinite domain.  If the domain is finite, the
presented group contains the non-MF group of~\cite{NonMF}.  If the domain
is infinite, the presented group is MF by a permanence theorem for HNN
...
MF\@.  There is a computable map $e\mapsto\widehat R_e$ into finite
presentations such that $\widehat R_e$ presents an MF group if and only if
the $e$-th partial computable function has infinite domain.
\end{mainthm}
...
over the real numbers, so Tarski's theorem decides whether a certificate
is valid.  A finite presentation defines an MF group if and only if it has
a valid certificate for every $n$.  Hence
...
extension is implemented by a unitary of a norm matrix corona.
Theorem~\ref{thm:hnn-permanence} then shows that the presented group is MF
(Sections~\ref{sec:hnn} and~\ref{sec:compiler}).
