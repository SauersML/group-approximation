...
Section~\ref{sec:compiler} is an executable parametric finite-seed switch.
The passage from recursive to finite
...
rational vector.  Hence every field is finite data and the complete answer
checker is primitive recursive.  Density and polar correction show that
every strict matrix answer in~\eqref{eq:open-challenges} has such a coded
rational answer, while soundness of the norm certificates gives the
converse.
...
$E$ & a fixed finitely presented non-MF Shalom-cover group \\[1pt]
$C_e$ & recursively presented; trivial for $e\in\mathrm{INF}$, contains
  $E$ for $e\in\mathrm{FIN}$ \\[1pt]
...
\begin{proof}
Use the finitely presented Shalom-cover marked-compression group.  Its
distinguished nontrivial mark is killed by every homomorphism to an
operator-norm matrix corona, so the group is not MF.  Fix one finite
presentation code $P_-$ for this group.
\end{proof}
...
\begin{proof}
Apply the parametric finite-seed switch to the fixed code $P_-$ and the
program index $e$.  Its relator enumerator is computable jointly in the seed
code and $e$.  On an infinite domain every seed generator is eventually
killed, giving the trivial group.  On a finite domain the construction has
an explicit inverse pair of homomorphisms identifying the presented group
with $G_{P_-}=E$.
\end{proof}
