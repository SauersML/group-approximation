...
\begin{abstract}
The finite presentations of MF groups form a $\Pi^0_2$-complete set, and
the finite presentations of non-MF groups form a $\Sigma^0_2$-complete
set.  Neither set is recursively enumerable: it is impossible to
construct an algorithm that decides from a finite presentation whether
the group it defines is MF\@.  Here a countable group is MF if it admits
finite-dimensional unitary models that are asymptotically multiplicative
and asymptotically separate nonidentity elements in operator norm.  A
presentation defines an MF group if and only if it admits a finite
certificate at every scale, and whether a candidate certificate is valid
is decidable, by Tarski's theorem.  We construct a computable map
$e\mapsto\widehat R_e$ into finite presentations such that
$\widehat R_e$ presents an MF group if and only if the $e$-th partial
computable function has infinite domain: if the domain is finite,
$\widehat R_e$ contains the non-MF group of~\cite{NonMF}; if it is
infinite, $\widehat R_e$ is MF by a permanence theorem for HNN
...
recursively enumerable: it is impossible to construct an algorithm that
decides from a finite presentation whether the group it defines is
MF\@.  There is a computable map $e\mapsto\widehat R_e$ such that
$\widehat R_e$ presents an MF group if and only if the $e$-th partial
...
A finite presentation defines an MF group if and only if, at every scale,
it admits a finite certificate consisting of normal-closure witnesses and
a system of polynomial equations and inequalities with a real solution.
Whether such a system has a real solution is decidable, by Tarski's
theorem, so
...
$\widehat R_e$ in Section~\ref{sec:compiler}.  If the corresponding partial
function has finite domain, then $H\le\widehat R_e$, so $\widehat R_e$ is
not MF\@.  If its domain is infinite, then $\widehat R_e$ satisfies the hypotheses of
