...
nonidentity elements in operator norm.  The finite presentations of MF
groups form a $\Pi^0_2$-complete set, and their complement is
$\Sigma^0_2$-complete.  MF recognition from finite presentations is
undecidable.  Neither the MF presentations nor the non-MF presentations
can be recursively enumerated.  For each $n$, a finite certificate records matrix
approximations for words of length at most $n$ with relator error at most
$2^{-n}$; validity is decidable by Tarski's theorem.  We also construct,
computably from any program, a
...
groups is $\Sigma^0_2$-complete.  Neither set is recursively enumerable.
MF recognition from finite presentations is undecidable.  There is a
computable map $e\mapsto\widehat R_e$ into finite
...
The presented group is then MF by Theorem~\ref{thm:hnn-permanence}
...
undecidable from finite presentations~\cite{Adian,Rabin}.  A property is
...
property, and the Adian--Rabin theorem implies undecidability.  Lempp proved
that recognizing torsion-free finitely presented groups is
...
step of Section~\ref{sec:compiler}.  Our reduction must replace a recursive
presentation by a finite one without changing whether the group is MF.  A
general Higman embedding does not preserve this property.  The HNN
extensions in Section~\ref{sec:compiler} are constructed to satisfy the
hypotheses of Theorem~\ref{thm:hnn-permanence}.
...
We use the following form of Korchagin's finite-presentation criterion.

\begin{lemma}[Korchagin's criterion]\label{lem:local-models}
Let $G=\langle x_1,\dots,x_k\mid r_1,\dots,r_m\rangle$.  Then $G$ is MF
if and only if, for every $\varepsilon>0$ and every finite family
$\mathcal W$ of words that are nontrivial in $G$, there are $d\ge1$ and
$U_1,\dots,U_k\in\U(d)$ such that
\[
 \opnorm{r_i(U)-1}\le\varepsilon\quad(i\le m),
 \qquad
 \opnorm{w(U)-1}\ge\tfrac12\quad(w\in\mathcal W).
\]
...
\begin{proof}
Korchagin proves the criterion with one chosen representative for each
element of a finite subset of $G$ and separation constant $1$
\cite[Propositions~7--8]{Korchagin}.  If $\omega$ is his representative
of the same element as a prescribed word $w$, then $w\omega^{-1}$ is a
product of finitely many conjugates of the relators.  For a finite family
$\mathcal W$, a sufficiently small relator tolerance therefore preserves
separation at least $1/2$ for every $w\in\mathcal W$.  Conversely,
Korchagin's amplification changes any fixed positive separation constant
to $1$.
\end{proof}
...
computable coding of finite presentations by natural numbers.  From a code
$P$ one can recover its generators and relators.  Write $G_P$ for the
group defined by $P$, and set
\[
...
A decidable predicate $C(P,n,c)$ satisfies
\[
...
\begin{proof}
Fix a presentation
$P=\langle x_1,\dots,x_k\mid r_1,\dots,r_m\rangle$, and let $W_n$ be the
finite set of reduced words of length at most $n$.  A certificate $c$
contains the following data:
\begin{enumerate}
\item a positive integer $d$;
\item a partition $W_n=T\sqcup S$;
\item for every $w\in T$, an expression of $w$ as a product of conjugates
of the relators and their inverses.
\end{enumerate}
A certificate is valid if every expression in item~3 is correct and there
are unitaries $U_1,\dots,U_k\in\U(d)$ such
that
\[
 \opnorm{r_i(U)-1}\le2^{-n}\quad(i\le m),
 \qquad
...
Free reduction checks the expressions attached to the words in $T$.  When
such an expression is correct, its word is trivial in $G_P$.  It remains
to decide whether the required unitaries exist.  Write
$U_j=A_j+iB_j$ with $A_j$ and $B_j$ real matrices.  Unitarity is
polynomial in their entries, and for $t\ge0$,
\[
 \begin{aligned}
  \opnorm{X}\le t
 ...
 \end{aligned}
\]
Positive semidefiniteness is characterized by the principal minors.  After
separating real and imaginary parts, both conditions are semialgebraic.
Thus the existence of the matrices is a sentence in the first-order theory
of the real numbers, and validity is decidable by Tarski's
theorem~\cite{Tarski}.  Define $C(P,n,c)$ to mean that $c$ is valid.

Suppose first that $G_P$ is MF.  For each $n$, label a word of length at
most $n$ by $T$ precisely when it is trivial in $G_P$, and put the remaining
words in $S$.  Choose a normal-closure expression for every word in $T$.
Lemma~\ref{lem:local-models}, applied to the words in $S$, supplies
unitaries with relator error at most $2^{-n}$ and separation at least
$1/2$.  These data form a valid certificate.

Conversely, suppose that a valid certificate exists for every $n$.  Let
$\mathcal W$ be a finite family of words that are nontrivial in $G_P$, and
let $\varepsilon>0$.  Choose $n$ larger than the length of every word in
$\mathcal W$ and such that $2^{-n}\le\varepsilon$.  No word in
$\mathcal W$ belongs to $T$, because membership in $T$ includes a proof
that the word is trivial.  The matrices certified for $n$
therefore have relator error at most $\varepsilon$ and separate every word
in $\mathcal W$ from the identity by at least $1/4$.  Korchagin's criterion,
whose fixed separation constant is immaterial by amplification, now implies
that $G_P$ is MF.

The two implications prove the equivalence in the statement.  Since $C$
is decidable, it also proves the asserted upper bounds in the arithmetical
hierarchy.
\end{proof}
...
where $u$ denotes the canonical unitary.  This is the universal
$C^*$-HNN extension of $D$ by $\Theta$.
...
Let $C=B_0\oplus B_1$, $A_1=M_2(D)$, and $A_2=M_2(B_0)$, with the unital
inclusions
\[
...
$e_{ij}$ of $A_1$ and $f_{ij}$ of $A_2$.

Shulman's criterion reduces the MF property of $P$ to compatible
embeddings of $A_1$ and $A_2$ in a norm matrix corona
\cite[Theorem~16]{Shulman}.  Identify $M_2(\mathcal Q)$ with the corona
of the doubled dimension sequence.  Let $\varphi_A$ be the entrywise
embedding of $A_1=M_2(D)$, and let $\varphi_B$ be the entrywise embedding
of $A_2=M_2(B_0)$ followed by conjugation by $\operatorname{diag}(1,W)$.
Both are injective, and for $(c_0,c_1)\in C$,
\[
...
\]
Hence $P$ is MF, and so is its corner $U\cong ePe$.

The functional $T=\tau\circ\iota^{-1}$ is a tracial state on
$\iota(A)\supseteq D$ with
