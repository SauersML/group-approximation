...
can be recursively enumerated.  For each $n$, a finite certificate
gives normal-closure proofs for words declared trivial and requires a
unitary model that separates every remaining word, with relator error at
most $2^{-n}$.  Validity is decidable by Tarski's theorem.  We also
...
$\Sigma^0_2$-complete.  MF recognition from finite presentations is
undecidable.  Neither the MF presentations nor the non-MF presentations
can be recursively enumerated.  There is a computable map
...
does not preserve the MF property.  We therefore use HNN extensions
...
their inverses.  For the finite family under consideration, taking the
relator tolerance sufficiently small therefore preserves separation at
least $1/2$ for the prescribed words.  Conversely, Korchagin's
...
Every word admitted to $T$ is therefore trivial in $G_P$.  It remains to
...
equalities and inequalities over $\mathbb R$.  The existence of the
matrices is therefore a sentence in the first-order theory of real closed
fields, which is decidable by Tarski's theorem~\cite{Tarski}.  Let
...
We have proved the displayed equivalence.  It places
$\mathrm{MF}_{\mathrm{fp}}$ in $\Pi^0_2$ and its complement in
$\Sigma^0_2$.
\end{proof}
