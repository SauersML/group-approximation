...
presentation defines an MF group if and only if certificates exist at
every scale; the certificates are finite, and checking one is decidable,
by Tarski's theorem.  We also construct, computably from any program, a
...
many inputs, the presented group contains the non-MF group
of~\cite{NonMF}; if it halts on infinitely many inputs, the group is MF
by a permanence theorem for HNN extensions whose edge isomorphism is
