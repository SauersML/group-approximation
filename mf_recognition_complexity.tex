...
over the real numbers, so whether a certificate is valid is decidable,
by Tarski's theorem.  A finite presentation defines an MF group if and only if it has
...
$L_{\F_2}(1,2)$, and it represents $1$ in $H$ if and only
if that matrix is the identity.  The set of words representing $1$ is
